...
class Charts {
	...
	function pieChart($r, $percents, $legend,$imgname='', $title='',$bottom_label='',$config=array())
	{
      ...
      $TextHeight = $TextHeight + $Position[1]-$Position[7];
		...
		$Test->Render($imgname);  
		return  array("name" => '/cieprev/'.$imgname,"w" => $w, "h" => $h);
	}
	...
	function cartesianChart($type,$x,$y,$w,$h,$imgname='',$config=array())
	{
		...
		$Test->drawGraphArea($settings['ImgR'],$settings['ImgG'],$settings['ImgB'],$settings['Decay']);  
		...
		$Test->setGraphArea($WYmax+$TextHeightY+35,30,$w-25,$h-$HXmax-$TextHeightX-$TextboxHeight-15);  
		$Test->drawScale($DataSet->GetData(),$DataSet->GetDataDescription(),SCALE_ADDALL,0,0,0,TRUE,$settings['XAngle'],0,TRUE);  
		$Test->drawGraphArea($settings['BGR'],$settings['BGG'],$settings['BGB'],$settings['Decay2']);  
		...
		if(strlen($settings['Textbox']))
		{
			...
			$FontName  = $this->_ext_path."/fonts/".$settings['TextboxFontName'];
			$Test->setFontProperties($FontName,$FontSize);  
			$Test->drawTitle(50,22,$settings['Textbox'],50,50,50,585);	
			
			#$Test->setFontProperties($TextboxFontName,$TextboxFontSize);  
			#$Test->drawTitle(0,$TextboxHeight,$settings['Textbox'],50,50,50,585);
		...
		{
			$this->obj->load->helper('string');
			$imgname = $this->_img_path."/".random_string('alnum', 16).".png";  
		}	
			
		$Test->Render($imgname);  
		return  array("name" => '/cieprev/'.$imgname,"w" => $w+4, "h" => $h+4);
	}
