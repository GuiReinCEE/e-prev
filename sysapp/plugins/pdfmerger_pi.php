...
class PDFMerger_pi
{
	...
	public function addPDFArray($array, $pages = 'all')
	{
		foreach ($array as $key => $item) 
		{
			if(file_exists($item))
			{
				...
				$this->_files[] = array($item, $pages);
			}
			else
			{
				throw new exception("Could not locate PDF on ".$item);
			}
		}
		...
		foreach($this->_files as $file)
		{
			...
			if($filepages == 'all')
			{
				for($i=1; $i<=$count; $i++)
				{
					...
					$this->fpdi->AddPage('P', array($size['w'], $size['h']));
