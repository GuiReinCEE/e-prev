<?PHP
	...
	while ($ar_reg = pg_fetch_array($ob_resul)) 
	{
		...
		$EMPRESA = substr($ar_reg['empresa'],0,35);
		...
		$ob_pdf->Text($ob_pdf->GetX() + (33.5 - ($ob_pdf->GetStringWidth($NOME)/2)),$ob_pdf->GetY() + 10, $NOME);	
		$ob_pdf->SetFont('Courier','',8);
		$ob_pdf->Text($ob_pdf->GetX() + (33.5 - ($ob_pdf->GetStringWidth($EMPRESA)/2)),$ob_pdf->GetY() + 13.5, $EMPRESA);	
