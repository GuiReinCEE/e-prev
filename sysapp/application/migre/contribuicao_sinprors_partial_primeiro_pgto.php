...
using(array('projetos.contribuicao_controle', 'public.controles_cobrancas'));

class contribuicao_sinprors_partial_primeiro_pgto
{
	private $service;
	private $command;
	private $params;
	...
	private function start()
	{
		...
		else
		{
			...
			$totais_bdl = $this->service->contribuicao_sinpro__totais__get('bdl', $this->params);
			$totais_bco = $this->service->contribuicao_sinpro__totais__get('bco', $this->params);
		}
		...
		if( floatval($totais_bdl["contador"])!=floatval($gera->tot_bdl_gerado) )
		{
	...
	private function ajax_gerar()
	{
		...
		{
			$b = contribuicao_controle::criar_cobranca_1ro_pagamento_sinprors( $this->params['mes'], $this->params['ano'] );
		}
		echo "true";
	}
	...
	{
		$controles_pp = array();
		$controles_pp = $this->service->contribuicao_sinpro__controle_geracao_cobranca__get( 'confirmacao', $this->params );
	...
	{
		$controles_g = array();
		$controles_g = $this->service->contribuicao_sinpro__controle_geracao_cobranca__get( 'geracao', $this->params );
	...
	{
		$controles_gf = array();
		$controles_gf = $this->service->contribuicao_sinpro__controle_geracao_cobranca__get( 'envio', $this->params );
	...
	private function get_quantidade_emails_enviar()
	{
		...
		return $this->service->contribuicao_sinpro__emails_enviar_primeiro__get( $this->params );
	}

	private function get_lista_sem_email_bdl()
	{
		$this->params['forma_pagamento'] = 'BDL';
		return $this->service->contribuicao_sinpro__lista_sem_emails_primeiro( $this->params );
	}
...
$esta = new contribuicao_sinprors_partial_primeiro_pgto($db);
...
>
	...
	<tr>
		...
		<td style="font-family:arial;padding-left:20px;"><b>SINPRORS - Cobrança de Primeiro Pagamento - Lista de participantes sem email</b></td>
	</tr>
	</table>
	<br>
