<?PHP
	...
	$sql = " 
			SELECT scb.cd_barra
			 ...
			 WHERE scb.cd_seminario_codigo_barra_lote = ".$_REQUEST['cd_lote']."
		   ";
	...
	while ($ar_reg = pg_fetch_array($ob_resul)) 
	{
		...
		$ob_pdf->EAN13($ob_pdf->GetX() + 17,$ob_pdf->GetY() + 7,$ar_reg['cd_barra']);
		//$ob_pdf->SetTextColor(64,64,64);
		//$ob_pdf->SetFont('Courier','B',10);
		//$ob_pdf->Text($ob_pdf->GetX() + (33.5 - ($ob_pdf->GetStringWidth($NOME)/2)),$ob_pdf->GetY() + 5.5, $NOME);	
