...
//------------------------------------------------------------------------------------------ Lista Analista
	if ($tm == 's') {
	//		$sql = "SELECT * FROM projetos.usuarios_controledi WHERE divisao='$S' and indic_02 = '*' ORDER BY nome" ;
		$sql = "SELECT * FROM projetos.usuarios_controledi WHERE indic_02 = 'S' AND tipo <> 'X' ORDER BY nome" ;
	}
	else {
//		$sql = "SELECT * FROM projetos.usuarios_controledi WHERE tipo in('N','G','U') AND divisao in ( ";
//		$sql = $sql . " SELECT distinct ls.codigo as codigo  ";
//		$sql = $sql . " FROM listas            ls,           ";
//		$sql = $sql . "      projetos.projetos pp            ";
//		$sql = $sql . " WHERE ls.categoria  = 'DIVI'         ";
...
//		$sql = $sql . " ) ORDER BY nome";
		...
		//if( isset($cbo_divisao_atendente) ){ $divisao_do_analista=$cbo_divisao_atendente; }else{ $divisao_do_analista=''; }
		//echo 'aa:'.$aa;
		...
		{
			$divisao_do_analista=$aa;
		}
		
		//echo 'analista:'.$divisao_do_analista;

		$sql = "SELECT * FROM projetos.usuarios_controledi WHERE tipo in('N','G','U') AND divisao = '" . $divisao_do_analista . "' or ''='".$divisao_do_analista."' order by nome";
	}
	$rs = pg_query($db, $sql);
...
//------------------------------------------------------------------------------------ Finaliza construção da página
	...
	if((($D == "GAP") or ($regOs['divisao'] == "GAP")) and ($aa == "GB"))
	{
		...
		$cd_usuario_abrir_ao_encerrar = (((intval($regOs['cd_usuario_abrir_ao_encerrar']) == 0) and ($D == "GAP" )) ? 203 : intval($regOs['cd_usuario_abrir_ao_encerrar']));
		...
		$qr_sql = "
					SELECT codigo,
					 ...
					 WHERE (tipo in('N','G','U') OR codigo = ".$cd_usuario_abrir_ao_encerrar.")
					   AND divisao = '".(trim($regOs['divisao']) == "" ? $D : trim($regOs['divisao']))."'
					 ORDER BY nome
		          ";
		$ob_resul = pg_query($db, $qr_sql);
