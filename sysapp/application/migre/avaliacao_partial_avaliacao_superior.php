...
class avaliacao_partial_avaliacao_superior
{   #begin_class
    ...
    function avaliacao_partial_avaliacao_superior( $_db )
    {
        $this->db = $_db;
    ...
    private function pode_ver_essa_pagina( entity_projetos_avaliacao_capa_extended $capa )
    {
		...
		else
		{
			...
			else
			{
				...
				elseif ( $this->cd_usuario_logado==$capa->get_cd_gerente_avaliado() )
				{
					// gerente do avaliado
					$sr = true;
				}
    ...
    function load()
    {
        ...
        if( ! isset($this->capa)) 
        {
        	echo 'Algum problema impediu a exibição dessa página, favor entrar em contato com a equipe técnica responsável.';
        	exit;
        }
        ...
        {
            // encontra avaliação do tipo 'S' que indica que foi realizada pelo Superior
        ...
        if(($this->avaliacao->get_cd_avaliacao()==''))
        {
        	echo 'Algum problema impediu a exibição dessa página, favor entrar em contato com a equipe técnica responsável.';
        	exit;
        }
    ...
    /**
     * O processo de avaliação para ser completo precisa atender a uma regra definida em 10/6/2008 pelo Gilberto:
     * O Avaliado deve ter sido admitido antes do fechamento do processo de avaliação do ano anterior,
     * caso contrário ele só poderá preencher as espectativas.
     */
    public function is_complete_proccess()
    {
        ...
        // Somente abrir avaliação para contratados depois de 30/06 do ano anterior
        // aos demais abrir apenas espectativas!
            
        $data_fechamento = '01/07/'.$year;
            
        // Código abaixo resgata a data de encerramento do período anterior para decidir sobre as novas avaliações
		...
		if(isset($avaliacao->aspectos)) {
			foreach( $avaliacao->aspectos as $aspecto ){ 
				...
				if($this->is_avaliador()) {
					$output .= '<nobr>
										<a href="javascript:void(0)" onclick="thisPage.aspecto_editar__Click(this);" registroId="' . $aspecto->cd_avaliacao_aspecto . '" url="avaliacao_partial_avaliacao_superior.php"><img src="img/avaliacao_manutencao.png" border="0" /></a>
										<a href="javascript:void(0)" sender="avaliado" onclick="thisPage.aspecto_deletar__Click(this);" registroId="' . $aspecto->cd_avaliacao_aspecto . '" url="avaliacao_partial_avaliacao_superior.php"><img src="img/avaliacao_deletar.png" border="0" /></a>
								<nobr>';
				}
				$output .= '
									</td>
	...
	public function treinamento_lista_render()
	{
        global $db;
		$qr_sql = "
					SELECT funcoes.nr_treinamento_colaborador(tc.ano, tc.numero) AS numero,
                      ...
                      JOIN projetos.usuarios_controledi uc
                       ...
                       AND uc.cd_registro_empregado::INTEGER = tct.cd_registro_empregado
                       AND 0 = tct.seq_dependencia
					 WHERE uc.codigo       = ".intval($this->capa->get_cd_usuario_avaliado())."
		...
		$output.= '	
					...
					<table class="sort-table" id="table-x" align="center" width="100%" cellspacing="2" cellpadding="2">
						<thead>
							<tr> 
								<td>Número</td>					
								<td>Nome</td>
								...
								<td>Dt Final</td>
								<td>Tipo</td>
								<td>Carga<BR>Horária(h)</td>
							</tr>
						</thead>
						<tbody>	
			      ';			
		
				while($ar_item = pg_fetch_array($ob_resul))
				{
					$output.= '	
								<tr onmouseover="sortSetClassOver(this);" onmouseout="sortSetClassOut(this);">
									<td align="center">'.$ar_item['numero'].'</td>					
									<td>'.$ar_item['nome'].'</td>
									<td>'.$ar_item['promotor'].'</td>
									<td align="center">'.$ar_item['dt_inicio'].'</td>
									<td align="center">'.$ar_item['dt_final'].'</td>
									<td align="center">'.$ar_item['ds_treinamento_colaborador_tipo'].'</td>
									<td align="center">'.$ar_item['carga_horaria'].'</td>
								</tr>
						';
				}
	...
	public function ajax_espectativas_render()
    {
        ...
        $output = '
			...
			<a href="javascript:void(0)" onclick="thisPage.espectativa_salvar__Click(this);" url="avaliacao_partial_avaliacao_superior.php"><img src="img/avaliacao_salvar.png" border="0" /></a>
		...
		/*
	    	<input type="button" value="Salvar" class="botao" onclick="thisPage.espectativa_salvar__Click(this);" url="avaliacao_partial_avaliacao_superior.php" />
	...
	public function ajax_espectativas__save()
        ...
        if($ret)
        	...
        	<a href="javascript:void(0)" onclick="thisPage.espectativa__Hide(); espectativa_carregar(\'avaliacao_partial_avaliacao_superior.php\');">Voltar</a>
        	
        	';
        else
        	echo 'Não foi possível gravar as espectativas!';
	}
        
	public function ajax_espectativas__delete()
    {
        $ret = $this->service->avaliacao_aspecto__delete( $_POST['id'] );
...
$esta = new avaliacao_partial_avaliacao_superior( $db );
?>
...
<!-- START QUADRO RESUMO -->
                      ...
                      <? if( $esta->is_complete_proccess() ) : #IS_COMPLETE_PROCCESS ?>
					  ...
					  ?>
                          <tr bgcolor="#F4F4F4" 
							...
							<td style="background-color: #FFFFFF; width: 20px; text-align:center;">
                      ...
                      <tr bgcolor="#BFA260"> 
                        ...
                        <td colspan="6" valign="top" align="right"><font color="#FFFFFF" size="3" face="Arial, Helvetica, sans-serif"><strong><?= number_format( $esta->get_val_esc(), 2 ) ?></strong></font></td>
                      </tr>
                      <!-- END BLOCK : ES -->
								...
								<table width="100%">
									<tr>
										<td align="right">
											...
											<!--<input class="botao" type="button_disabled" value="Nova expectativa" onclick="thisPage.nova_espectativa__Click(this);" url="avaliacao_partial_avaliacao_superior.php" />-->
										</td>
									</tr>
								</table>
							<? endif; ?>								
...
<!-- END QUADRO RESUMO -->
			...
			<table width="700px" align="center">
				<tr>
					<td align="left">
						<span style="font-family: Verdana, Arial, Helvetica, sans-serif; font-weight:bold; font-size: 12px;">Treinamento(s) Realizado(s):</span>
					</td>
				</tr>
				<tr>
					<td align="left">
						<div style="overflow:auto; width: 100%; height: 300px; "><? $esta->treinamento_lista_render(); ?></div>
						<hr />
					</td>									
				</tr>
			</table>
            ...
            <? if($esta->is_avaliador()) { ?>
				...
				-->
                    ...
                    <a  href="javascript:void(0)"
                            id="confirmar_by_avaliador_image"
                            onclick="thisPage.<?= $js_button_close_send; ?>;"
							command="<?= $js_command; ?>"
