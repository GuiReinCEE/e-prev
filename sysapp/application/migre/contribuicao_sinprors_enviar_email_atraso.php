<?
	...
	if( $ret )
	{
		...
		send_email_confirmando_envio($dal, $db, $mm, $aaaa,'COBRANÇA ATRASADO');
		echo 'true';
	}
	else
	{
...
http://www.e-prev.com.br/controle_projetos/contribuicao_sinprors_relatorio.php?aba=retornados&mes={MES}&ano={ANO}";
		...
		$texto = str_replace( '{NOME_DO_PROCESSO}', 'Envio de cobrança de contribuições atrasadas do SINPRORS', $texto );
...
Em decorrência de não ter sido possível o desconto automático em conta corrente, sua contribuição do Plano SINPRORS Previdência encontra-se disponível para pagamento conforme instruções abaixo (continuaremos encaminhando as próximas contribuições para desconto em conta corrente).
...
Por favor, pague a contribuição do plano, através do link abaixo ou através da área de auto-atendimento de nosso site:
{LINK}
...
';
		...
		$assunto = "Contribuição do Plano SINPRORS Previdência disponível para pagamento";
		...
		$link="https://www.fundacaoceee.com.br/sinprors_pagamento.php?re=".$reg['re']."&comp=".$reg['competencia']."";
...
Em decorrência de não ter sido possível o desconto em folha de pagamento, sua contribuição do Plano SINPRORS Previdência encontra-se disponível para pagamento conforme instruções abaixo (continuaremos encaminhando as próximas contribuições para a folha de pagamento).
...
';
	...
	function send_mail_primeiro($reg, $dal)
	{
		...
		$msg = $msg . "Informamos que não identificamos o recebimento da primeira contribuição ao Plano SINPRORS Previdência gerada para pagamento em períodos anteriores. Sua contribuição do Plano SINPRORS Previdência encontra-se disponível para pagamento conforme instruções abaixo (lembramos que a inclusão como participante do Plano está condicionada a efetivação do primeiro pagamento) :" . $vbcrlf;
