<?php
	set_title('Contribuição instituidor - Relatório Primeiro Pagamento');
...
<script>
	...
	function load()
	{
		if(($("#cd_plano_empresa").val() != "") && ($("#cd_plano").val() != "") && ($("#nr_mes").val() != "") && ($("#nr_ano").val() != ""))
		{
			...
			$.post('<?php echo base_url() . index_page(); ?>/planos/contribuicao_instituidor_primeiro/relatorioListar',
				{
	...
	function ir_primeiro()
	{
		location.href='<?php echo site_url("planos/contribuicao_instituidor_primeiro"); ?>';
	}
</script>
<?php
	$abas[] = array('aba_lista', 'Primeiro Pagamento', FALSE, 'ir_primeiro();');
