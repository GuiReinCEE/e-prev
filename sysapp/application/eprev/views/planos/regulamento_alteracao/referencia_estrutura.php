...
<script>
    ...
    function ir_referencia_unidade_basica()
    {
        location.href = "<?= site_url('planos/regulamento_alteracao/referencia/'.$regulamento_alteracao['cd_regulamento_alteracao'].'/'.$cd_regulamento_alteracao_unidade_basica) ?>";
    }
    ...
    function salvar_item(cd_regulamento_alteracao_estrutura, fl_checked)
    {
        $.post("<?= site_url('planos/regulamento_alteracao/salvar_referencia_estrutura') ?>",
        {
            ...
            cd_regulamento_alteracao_estrutura_referenciado      : cd_regulamento_alteracao_estrutura,
...
<?php
    ...
    $abas2[] = array('aba_referencia_estrutura', 'Estrutura', TRUE, 'location.reload();');
    $abas2[] = array('aba_referencia_unidade_basica', 'Unidade Básica', FALSE, 'ir_referencia_unidade_basica();');
    ...
    {
        if(trim($regulamento_alteracao['dt_alteracao_finalizada']) == '' OR (in_array($item['cd_regulamento_alteracao_estrutura'], $referenciado)))
        {
            $campo_check = array(
                'name'     => 'cd_estrutura_'.$item['cd_regulamento_alteracao_estrutura'],
                'id'       => 'cd_estrutura_'.$item['cd_regulamento_alteracao_estrutura'],
                'value'    => $item['cd_regulamento_alteracao_estrutura'],
                'checked'  => (in_array($item['cd_regulamento_alteracao_estrutura'], $referenciado) ? TRUE : FALSE),
                'onchange' => 'check_item($(this))'  
            ); 

            $body[] = array(
                form_checkbox($campo_check),
                array('<span class="'.$item['ds_class_label'].'">'.$item['ds_tipo'].' - '.$item['ds_regulamento_alteracao_estrutura'].'</span>', 'text-align:left;'),
            );
        }
    }
