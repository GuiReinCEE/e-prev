<?php
set_title('SINPRORS - Usuários Cadastro');
...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("planos/sinprors_usuario"); ?>';
	}
	...
	function excluir(cd_usuario)
	{
		...
		{
			location.href='<?php echo site_url("planos/sinprors_usuario/excluir/".$row['cd_usuario']); ?>';
		}
	}
	...
	{
		checkTipo();
		$('#tp_usuario').change(function(){ checkTipo(); });
	});		
</script>
<?php
...
$ar_tp_usuario[] = Array('text' => 'Instituidor', 'value' => 'S');

$ar_cd_empresa[] = Array('text' => 'SINPRO', 'value' => 8);
$ar_cd_empresa[] = Array('text' => 'SINTAE', 'value' => 10);

$ar_fl_troca_senha[] = Array('text' => 'Sim', 'value' => 'S');
$ar_fl_troca_senha[] = Array('text' => 'Não', 'value' => 'N');
	
echo aba_start( $abas );
	echo form_open('planos/sinprors_usuario/salvar');
	echo form_start_box( "default_box", "Usuário" );
		...
		echo form_default_dropdown('cd_empresa', 'Instituidor:*', $ar_cd_empresa, Array($row['cd_empresa']));			
