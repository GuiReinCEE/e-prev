<?php
    ...
    foreach($collection as $item)
    {
        ...
        $body[] = array(
			...
			$item['ds_progressao'],
			$item['ds_promocao'],
			$item['nr_pontuacao_treinamento'],
			anchor('cadastro/rh_avaliacao/formulario/'.$item['cd_avaliacao_usuario'], '[avaliação]', 'target="_blank"').$link.br().
			anchor('cadastro/rh_avaliacao_abertura/treinamentos/'.$item['cd_avaliacao_usuario'], '[treinamentos]').$editar.br().
			anchor('cadastro/rh_avaliacao_abertura/pdf/'.$item['cd_avaliacao_usuario'], '[pdf]', 'target="_blank"').br().
			anchor('cadastro/rh_avaliacao_abertura/capacitacao/'.$item['cd_avaliacao_usuario'], '[capacitação]', 'target="_blank"')
        );
    }
