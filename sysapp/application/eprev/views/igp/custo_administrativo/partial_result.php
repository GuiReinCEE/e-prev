...
foreach($collection as $item)
{
	...
	$body[] = array(
		$item['mes_referencia'],
		array( number_format($item['nr_despesa'],2,',','.'),'text-align:right;' ),
		array( number_format($item['nr_contribuicao'],2,',','.'),'text-align:right;' ),
		array( number_format($item['nr_despesa_percentual'],2,',','.').'%','text-align:right;' ),
		array( number_format($item['nr_meta'], 2,',','.' ).'%', 'text-align:right;' ),
		array( number_format($item['nr_despesa_acumulado'],2,',','.'),'text-align:right;' ),
		array( number_format($item['nr_contribuicao_acumulado'],2,',','.'),'text-align:right;' ),
		array( number_format($item['nr_despesa_percentual_acumulado'],2,',','.').'%','text-align:right;' ),
		...
		array( number_format($item['nr_despesa_percentual_acumulado_meta'],2,',','.').'%','text-align:right;' ),
		...
		($item['fl_editar'] == "S" ? anchor("igp/custo_administrativo/detalhe/" . $item["cd_custo_administrativo"], "editar") : "")
	);
}
