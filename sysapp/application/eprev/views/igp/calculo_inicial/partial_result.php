...
foreach($collection as $item)
{
	...
	$body[] = array(
		$item['mes_referencia'],
		array( number_format($item['nr_concedido'],0,',','.'),'text-align:right;' ),
		array( number_format($item['nr_erro'],0,',','.'),'text-align:right;' ),
		array( number_format($item['nr_erro_percentual'],2,',','.').'%','text-align:right;' ),
		array( number_format(100-floatval($item['nr_meta']), 2,',','.' ).'%', 'text-align:right;' ),
		array( number_format($item['nr_concedido_acumulado'],0,',','.'),'text-align:right;' ),
		array( number_format($item['nr_erro_acumulado'],0,',','.'),'text-align:right;' ),
		array( number_format($item['nr_erro_percentual_acumulado'],2,',','.').'%','text-align:right;' ),
		array( number_format($item['nr_peso'],2,',','.'),'text-align:right;' ),
		array( number_format($item['nr_resultado_meta'],3,',','.').'%','text-align:right;' ),
		array( number_format($item['nr_referencia_mes'],2,',','.'),'text-align:right;' ),
		array( number_format($item['nr_erro_percentual_acumulado_meta'],3,',','.').'%','text-align:right;' ),
		...
		($item['fl_editar'] == "S" ? anchor("igp/calculo_inicial/detalhe/" . $item["cd_calculo_inicial"], "editar") : "")
	);
}
