...
<script>
	...
	function load()
	{
		...
		$.post('<?php echo base_url().index_page(); ?>/igp/informatica/listar',
			{},
			function(data)
			{ 
	...
	function gerar_graficos()
	{
		...
		{
			$.post('<?php echo site_url("igp/informatica/criar_indicador"); ?>', 
				{
				}, 
				function(data)
				{ 
		...
		else if( confirm('Fechar o período?') )
		{
			$.post('<?php echo site_url("igp/informatica/criar_indicador/"); ?>',
				...
				function(data)
				{
					...
					location.href = '<?php echo site_url("igp/informatica/fechar_periodo")?>';
				});
		}
	}
	...
	function novo()
	{
		location.href='<?php echo site_url("igp/informatica/detalhe/0"); ?>';
	}
...
echo form_start_box("default_box", "Cadastro");
	...
	echo form_default_row("", "", '<span class="label label-success">Este indicador é atualizado pelo indicador da Informática</span>');
	#echo form_default_row("","",'<span style="color:green; font-weight:bold;">Este indicador é atualizado pelo indicador da GI</span>');
