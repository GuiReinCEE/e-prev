...
<script>
	...
	function load()
	{
		...
		$.post('<?php echo base_url().index_page(); ?>/igp/equilibrio/listar',
			{}, 
			function(data)
			{ 
	...
	function gerar_graficos()
	{
		...
		{
			$.post('<?php echo site_url("igp/equilibrio/criar_indicador"); ?>', 
				{
				}, 
				function(data)
				{ 
		...
		else if( confirm('Fechar o período?') )
		{
			$.post('<?php echo site_url("igp/equilibrio/criar_indicador/"); ?>',
				...
				function(data)
				{
					...
					location.href = '<?php echo site_url("igp/equilibrio/fechar_periodo")?>';
				});
		}
	}
...
echo form_start_box("default_box", "Cadastro");
	...
	echo form_default_row("", "Período aberto:", '<span class="label label-important">'.$tabela[0]['ds_periodo'].'</span>'); 		echo form_default_row("","","");
