...
<script>
	...
	function load()
	{
		...
		$.post('<?php echo base_url().index_page(); ?>/igp/satisfacao_colab/listar',
			{}, 
			function(data)
			{ 
	...
	function gerar_graficos()
	{
		...
		{
			$.post('<?php echo site_url("igp/satisfacao_colab/criar_indicador"); ?>', 
				{
				}, 
				function(data)
				{ 
		...
		else if( confirm('Fechar o período?') )
		{
			$.post('<?php echo site_url("igp/satisfacao_colab/criar_indicador/"); ?>',
				...
				function(data)
				{
					...
					location.href = '<?php echo site_url("igp/satisfacao_colab/fechar_periodo")?>';
				});
		}
	}	
