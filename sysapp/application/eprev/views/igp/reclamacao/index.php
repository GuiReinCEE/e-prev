...
<script>
	...
	function load()
	{
		...
		$.post('<?php echo base_url().index_page(); ?>/igp/reclamacao/listar',
			{}, 
			function(data)
			{ 
	...
	function gerar_graficos()
	{
		...
		{
			$.post('<?php echo site_url("igp/reclamacao/criar_indicador"); ?>', 
				{
				}, 
				function(data)
				{ 
		...
		else if( confirm('Fechar o período?') )
		{
			$.post('<?php echo site_url("igp/reclamacao/criar_indicador/"); ?>',
				...
				function(data)
				{
					...
					location.href = '<?php echo site_url("igp/reclamacao/fechar_periodo")?>';
				});
		}
	}	
