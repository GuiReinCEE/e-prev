<?php
set_title('Aviso Inadimplência Empréstimo - Emails');
...
<script>	
	function filtrar()
    {
		if($("#dt_email_ini").val() != "" && $("#dt_email_fim").val() != "")
		{
			...
			$.post('<?php echo site_url('atividade/avisos_inadimplencia_emprestimo/emails_listar'); ?>',$('#filter_bar_form').serialize(),
			function(data)
			{
		...
		{
			alert("Informe o Período do email e Retornado.");
		}
    }
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("atividade/avisos_inadimplencia_emprestimo"); ?>';
	}
...
echo aba_start( $abas );
	...
	echo '<div id="result_div">'.br(2).'<span style="color:green;"><b>Informe o Período do email e Retornado.</b></span></div>';
	echo br();
echo aba_end(); 
