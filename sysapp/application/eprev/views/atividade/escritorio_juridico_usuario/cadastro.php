<?php
set_title('Escritório Jurídico - Usuários Cadastro');
$this->load->view('header');
?>
<script>
	<?php
		echo form_default_js_submit(Array('nome', 'cpf', 'cd_escritorio', 'senha', 'tp_usuario', 'fl_troca_senha', 'email'));
	?>
	function ir_lista()
	{
		location.href='<?php echo site_url("atividade/escritorio_juridico_usuario"); ?>';
	}
	
	function ir_escritorio()
	{
		location.href='<?php echo site_url("atividade/escritorio_juridico"); ?>';
	}
	...
	function excluir(cd_usuario)
	{
		...
		{
			location.href='<?php echo site_url("atividade/escritorio_juridico_usuario/excluir/".$row['cd_usuario']); ?>';
		}
	}
</script>
<?php
...
$abas[] = array('aba_cadastro', 'Escritórios', false, 'ir_escritorio();');
	
$ar_fl_troca_senha[] = Array('text' => 'Sim', 'value' => 'S');
$ar_fl_troca_senha[] = Array('text' => 'Não', 'value' => 'N');
	
echo aba_start( $abas );
	echo form_open('atividade/escritorio_juridico_usuario/salvar');
	echo form_start_box( "default_box", "Usuário" );
		...
		echo form_default_dropdown('cd_escritorio', 'Escritório Jurídico:*', $arr_escritorio, Array($row['cd_escritorio']));			
