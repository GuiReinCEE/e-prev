<?php
	set_title('Registro de Solicitações, Fiscalizações e Auditorias - Minhas Conferências');
...
<script>
	function filtrar()
	{
		...
		$.post("<?= site_url('atividade/solic_fiscalizacao_audit/minhas_conferencia_listar') ?>",
...
<?php
    $abas[] = array('aba_minhas', 'Minhas', FALSE, 'ir_minhas();');
    $abas[] = array('aba_minhas_conferencias', 'Conferências', TRUE, 'location.reload();');

    $status = array(
        array('value' => 'E',  'text' => 'Encaminhado'),
        array('value' => 'A',  'text' => 'Atendeu'),
        array('value' => 'NA', 'text' => 'Não atendeu')
    );
