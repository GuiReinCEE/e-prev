...
?>
	...
	function set_tipo($tipo)
	{
		if($tipo == 'E48H')
		{
			alert('Os pedidos em caráter Emergencial terão custos mais elevados.');
		}
	}
	...
	$(function(){
		...
		$("#cd_substituto option[value='<?= $row['cod_atendente'] ?>']").remove();
    ...
    echo form_open('atividade/atividade_solicitacao/salvar', 'name="filter_bar_form"');
        echo form_start_box("default_box", "Cadastro");
            ...
            if(intval($row['numero']) > 0)
            {
       		...
       		if(intval($row['numero']) == 0)
            {
            	$row['descricao'] = 'Empresa: 
RE: 
Sequência: 
Número do Protocolo:
Tipo de Documento: 
Data da Indexação: 
Caminho do Arquivo no Liquid: 
Número da Caixa:';

				$row['tipo_solicitacao'] = 'SPCA';
            }

            echo form_default_row('gerencia_destino', 'Gerência de Destino :', 'GFC – Arquivo');
            ...
            echo form_default_dropdown('tipo', 'Tipo da Atividade :*', $arr_tipo_atividade, $row['tipo'], 'onchange="set_tipo($(this).val())"');
            echo form_default_text('titulo', 'Título :*', $row['titulo'], 'style="width:450px;"');
            echo form_default_textarea('descricao', 'Descrição da Solicitação :*', $row['descricao'], 'style="width:450px; height:200px;"');
            echo form_default_textarea('problema', 'Justificativa da Solicitação :', $row['problema'], 'style="width:450px; height:50px;"');
            echo form_default_dropdown('', 'Atendente da Atividade :*', $arr_atendente, $row['cod_atendente'], 'disabled="disabled"');
