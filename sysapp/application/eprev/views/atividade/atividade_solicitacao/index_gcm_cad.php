...
<?php
	echo form_default_js_submit
		 (
			Array('cod_solicitante', 'tipo_solicitacao', 'tipo', 'titulo', 'descricao', 'problema', 'cod_atendente'),
			'formValidar(form)'
		 );	
...
?>    
	...
	function proximo_dia_util()
	{
		if($("#tipo_solicitacao").val() != '' && $("#tipo").val() != '') 
		{
			$.post('<?php echo site_url("atividade/atividade_solicitacao/proximo_dia_util_cadastro"); ?>',
			{
			...
			function(data)
			{
				if(data)
				{
					...
					$("#dt_limite_r_item").html(data.dt_util);
				}
			}, 'json');
...
echo aba_start( $abas );
    echo form_open('atividade/atividade_solicitacao/salvar', 'name="filter_bar_form"');
        echo form_start_box("default_box", "Cadastro");
            ...
            echo form_default_dropdown('tipo_solicitacao', 'Tipo da Manutenção :*', $arr_tipo_manutencao, $row['tipo_solicitacao'], 'onchange="proximo_dia_util()"');
            echo form_default_dropdown('tipo', 'Tipo da Atividade :*', $arr_tipo_atividade, $row['tipo'], 'onchange="proximo_dia_util()"');
            ...
            if(intval($row['numero']) == 0 OR $this->session->userdata('divisao') != 'GCM')
            {
            	echo form_default_hidden('cod_atendente', '', $row['cod_atendente']);
            	echo form_default_hidden('cd_substituto', '', $row['cd_substituto']);
            	echo form_default_dropdown('', 'Atendente da Atividade :*', $arr_atendente, $row['cod_atendente'],  'disabled=""');                   
	            echo form_default_row('', '', '<i>Indique para quem você vai encaminhar esta solicitação.</i>');
	            echo form_default_dropdown('', 'Atendente Substituto :', $arr_atendente, $row['cd_substituto'], 'disabled=""');    
	            echo form_default_row('', '', '<i>Indique o substituto para encaminhar esta solicitação.</i>');

	            echo form_default_hidden('dt_limite', 'Dt. Limite:', $row['dt_limite']);
            	echo form_default_row('dt_limite_r', 'Dt. Limite:', $row['dt_limite']);
            }
            else
            {
