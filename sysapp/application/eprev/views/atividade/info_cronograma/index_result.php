...
foreach ($collection as $item)
{
	...
	foreach($item['item'] as $item2)
    {
		...
		$body[] = array(
			...
			'<span id="ajax_concluido_valor_'.$item2['cd_cronograma_item'].'"></span> '.($item2['fl_concluido'] == 'S' ? '<span id="concluido_valor_'.$item2['cd_cronograma_item'].'" class="label label-info";>Sim</span>' : '<span id="concluido_valor_'.$item2['cd_cronograma_item'].'" class="label label-important">Não</span>')
			.
			form_dropdown("fl_concluido_".$item2['cd_cronograma_item'], Array('S'=> 'Sim', 'N' => 'Não'), array($item2['fl_concluido']))
			."<script> 
