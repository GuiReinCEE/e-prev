...
<script>
	...
	function formValidar(form)
	{
		...
		else if($("#cd_atividade_classificacao").val() < 1)
		{
			alert("Informe a Classificação");
			$("#cd_atividade_classificacao").focus();
			return false;			
		}
		else
		{
    ...
    function ir_script()
    {
        location.href='<?php echo site_url('atividade/atividade_script/index/'.$ar_atividade['numero'].'/'.$ar_atividade['cd_gerencia_destino']);?>';
    }
...
echo aba_start( $abas );
    echo form_open('atividade/atividade_atendimento/salvar');
		...
		echo form_start_box("default_encaminhamento", "Encaminhamento");	
			...
			echo form_default_dropdown('cd_atividade_classificacao', 'Classificação:(*)', $ar_classificacao, $ar_atividade['cd_atividade_classificacao']);
