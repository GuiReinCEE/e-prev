<?php
set_title('Atividades - Comunicação - Atendimento');
...
<script>
	...
	function formValidar(form)
	{
		...
		{
			alert("Informe a Dt Limite para Análise do Solicitante");
			$("#dt_limite_teste").focus();
			return false;
		}
		else if(($("#status_atual").val() == "AOCS") && (dt_limite_teste < dt_minima))
		{
			alert("A Dt Limite para Análise do Solicitante deve ser maior ou igual a data de hoje");
			$("#dt_limite_teste").focus();
			return false;
		}		
		else if(($("#status_atual").val() == "AOCS") && ($("#cod_testador").val() == ""))
		{
			alert("Informe o Responsável pela Análise");
			$("#cod_testador").focus();
			return false;
		}		
		else
...
echo aba_start( $abas );
    echo form_open('atividade/atividade_atendimento/salvar');
        ...
        echo form_start_box("default_box", "Cadastro");
			...
			echo form_default_row('dt_env_teste_1', 'Dt Envio p/ Análise:', $ar_atividade['dt_env_teste']);
		...
		echo form_start_box("default_encaminhamento", "Encaminhamento");	
			echo form_default_hidden('sistema', '', 215); #Atendimento de atividade de Comunicação 
			echo form_default_hidden('status_anterior', '', trim($ar_atividade['status_atual']));
			
			echo form_default_date('dt_inicio_real', 'Dt Início Real:', $ar_atividade['dt_inicio_real']);
			echo form_default_dropdown('status_atual', 'Status:(*)', $ar_status_atual, $ar_atividade['status_atual']);

			echo form_default_date('dt_limite_teste', 'Dt Limite para Análise do Solicitante:', $ar_atividade['dt_limite_teste']);
			echo form_default_dropdown('cod_testador', 'Responsável pela Análise:', $ar_testador, $ar_atividade['cod_testador']);					
