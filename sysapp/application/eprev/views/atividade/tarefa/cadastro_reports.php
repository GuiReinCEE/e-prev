...
?>
	...
	function valida_form_parametros()
	{
		if( $("#ds_campo").val()=="" )
		{
	...
	{
		if(valida_form_parametros())
		{
			$.post( '<?php echo site_url('atividade/tarefa/salvar_parametros'); ?>',
			{
				cd_atividade : $('#cd_atividade').val(),
				cd_tarefa    : $('#cd_tarefa').val(),
				ds_campo     : $('#ds_campo').val(),
				ds_tipo      : $('#ds_tipo').val(),
	...
	{
		$('#result_parametros_div').html("<?php echo loader_html(); ?>");
	
		$.post( '<?php echo site_url('atividade/tarefa/listar_parametros'); ?>',
		{
			cd_atividade : $('#cd_atividade').val(),
			cd_tarefa    : $('#cd_tarefa').val(),
		...
		function(data)
		{
			$('#result_parametros_div').html(data);
			//configure_result_table();
		});
	}
	
	function excluir_parametros(cd_tarefas_parametros)
	{
		if( confirm('Deseja excluir a parâmetros?') )
		{
			$.post( '<?php echo site_url('atividade/tarefa/excluir_parametros'); ?>',
			{
				cd_atividade          : $('#cd_atividade').val(),
			    cd_tarefa             : $('#cd_tarefa').val(),
				cd_tarefas_parametros : cd_tarefas_parametros
			},
			function()
			{
				listar_parametros();
	...
	function valida_form_relatorios()
	{
		if( $("#cd_db").val()=="" )
		{
			alert( "Informe os campos obrigatórios! \n\n(os campos obrigatórios tem um * logo após a identificação).\n\n[cd_db]" );
	...
	{
		if(valida_form_relatorios())
		{
			$.post( '<?php echo site_url('atividade/tarefa/salvar_relatorios'); ?>',
			{
				cd_atividade : $('#cd_atividade').val(),
				cd_tarefa    : $('#cd_tarefa').val(),
				cd_db        : $('#cd_db').val(),
	...
	{
		$('#result_relatorios_div').html("<?php echo loader_html(); ?>");
	
		$.post( '<?php echo site_url('atividade/tarefa/listar_relatorios'); ?>',
		{
			cd_atividade : $('#cd_atividade').val(),
			cd_tarefa    : $('#cd_tarefa').val(),
			cd_mandante  : $('#cd_mandante').val()
		...
		function(data)
		{
			$('#result_relatorios_div').html(data);
			//configure_result_table();
		});
	}
	
	function excluir_relatorios(cd_tarefas_tabelas)
	{
		if( confirm('Deseja excluir esse detalhe do relatório?') )
		{
			$.post( '<?php echo site_url('atividade/tarefa/excluir_relatorios'); ?>',
			{
				cd_atividade       : $('#cd_atividade').val(),
			    cd_tarefa          : $('#cd_tarefa').val(),
	...
	function atualiza_relatorio(cd_tarefas_tabelas)
	{
		if( confirm('Deseja salvar a ordenação?') )
		{
			$.post( '<?php echo site_url('atividade/tarefa/atualiza_relatorio'); ?>',
			{
				cd_atividade       : $('#cd_atividade').val(),
				cd_tarefa          : $('#cd_tarefa').val(),
				cd_tarefas_tabelas : cd_tarefas_tabelas,
...
$arr_orientacao[] = array('value' => 'R', 'text' => 'Retrato');
$arr_orientacao[] = array('value' => 'P', 'text' => 'Paisagem');
...
echo aba_start($abas);
	...
	echo form_open('atividade/tarefa/salvar', 'name="filter_bar_form"');
		echo form_start_box( "default_box", "Cadastro" );
			...
			echo form_default_dropdown('fl_orientacao', 'Orientação:', $arr_orientacao, array($row['fl_orientacao']));
	...
	if(intval($row['cd_tarefa']) > 0)
	{
		echo form_start_box( "default_box_parametros", "Parâmetros" );
			echo form_default_hidden('cd_atividade', '', $row['cd_atividade']);
			echo form_default_hidden('cd_tarefa', '', $row['cd_tarefa']);
			echo form_default_text("ds_campo", "Nome Campo:*", '', 'style="width: 300px;"');
			echo form_default_text("ds_tipo", "Tipo Campo:*", '', 'style="width: 300px;"');
			echo form_default_integer("nr_ordem", "Ordem:*", '', 'style="width: 300px;"');								
		echo form_end_box("default_box_parametros");
		echo form_command_bar_detail_start(); 
			if(intval($row['cd_mandante']) == $this->session->userdata('codigo'))
			{
				echo button_save("Adicionar", "salvar_parametros()");
			}
		echo form_command_bar_detail_end();	
		echo '<div id="result_parametros_div"></div>'.br();
		
		echo form_start_box( "default_box_relatorios", "Detalhes do Relatório" );
			...
			echo form_default_text('ds_label', 'Label:', '', 'style="width: 300px;"');
		echo form_end_box("default_box_relatorios");
		echo form_command_bar_detail_start(); 
			if(intval($row['cd_mandante']) == $this->session->userdata('codigo'))
			{
				echo button_save("Adicionar", "salvar_relatorios()");
			}
		echo form_command_bar_detail_end();	
		echo '<div id="result_relatorios_div"></div>'.br();
	}
echo br(2);
