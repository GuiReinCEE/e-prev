<?php
set_title('Indicador - Desenho da tabela de parâmetros'); 
	...
	if( $('#cd_indicador_tabela').val()!='' )
	{
		...
		{ 
			if(data.erro=='')
			{
				...
				{
					$('#ds_indicador_tabela').val( '[Informe o Título]' );
				}
				$('#linhas').val('');
				$('#colunas').val('');
...
{
	if(confirm('Aplicar o estilo padrão a tabela?'))
	{
		ls = $('#linhas').val();
		cs = $('#colunas').val();
...
{
	if( $('#coluna_inicio_estilo').val()=='' ){alert( 'A coluna inicial é obrigatória' );$('#coluna_inicio_estilo').focus();return false;}
	if( $('#linha_inicio_estilo').val()=='' ){alert( 'A linha inicial é obrigatória' );$('#linha_inicio_estilo').focus();return false;}
...
echo 
	form_start_box('configuracao_box', 'Configuração')
...
echo 
	form_start_box('estilo_wizard_box', 'Estilo da célula', false, true, " style='width:600px;position:absolute;' ")
	.form_end_box('estilo_wizard_box', false)
	;

echo
	form_start_box('estilo_box', 'Estilo da célula')
	.form_default_text( 'coluna_inicio_estilo', 'Começar na Coluna', '', "onblur='ver_alteracao_estilo()'" )
	.form_default_text( 'coluna_fim_estilo', 'Terminar na Coluna', '', "onblur='ver_alteracao_estilo()'" )
	.form_default_integer( 'linha_inicio_estilo', 'Começar na Linha', '', "onblur='ver_alteracao_estilo()'" )
	.form_default_integer( 'linha_fim_estilo', 'Terminar na Linha', '', "onblur='ver_alteracao_estilo()'" )
	.form_default_text( 'definir_estilo', 'Definição do estilo', '', "style='width:300px;'" )
...
echo 
	form_start_box('exibir_box', 'Exibir', false)
		."
			...
			| <a href='javascript:void(0)' onclick='exibir_resultado_formula();'>exibir resultado das fórmulas</a>
			| <a href='javascript:void(0)' onclick='aplicar_estilo_padrao();'>aplicar estilo padrão</a>
			"
		.br(2)
		."<table border='0'>

				<tr><td style='text-align:center;'><input id='ds_indicador_tabela' name='ds_indicador_tabela' style='width:100%;text-align:center;border-style:solid;border-width:1px;background-color:white;' type='text' value='[Informe o Título]' /></td></tr>
