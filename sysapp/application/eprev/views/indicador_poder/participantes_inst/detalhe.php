...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_poder/participantes_inst"); ?>';
	}
...
echo form_open( 'indicador_poder/participantes_inst/salvar' );
echo form_hidden( 'cd_participantes_inst', intval($row['cd_participantes_inst']) );
...
if( intval($row['cd_participantes_inst'])>0  )
{
	echo button_delete("indicador_poder/participantes_inst/excluir",$row["cd_participantes_inst"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_poder/participantes_inst')."'; }");
