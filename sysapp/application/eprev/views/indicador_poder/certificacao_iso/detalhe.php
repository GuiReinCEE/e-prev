...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_poder/certificacao_iso"); ?>';
	}
...
echo form_open( 'indicador_poder/certificacao_iso/salvar' );
echo form_hidden( 'cd_certificacao_iso', intval($row['cd_certificacao_iso']) );
...
if( intval($row['cd_certificacao_iso'])>0  )
{
	echo button_delete("indicador_poder/certificacao_iso/excluir",$row["cd_certificacao_iso"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_poder/certificacao_iso')."'; }");
