...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_poder/participantes_patro"); ?>';
	}
...
echo form_open( 'indicador_poder/participantes_patro/salvar' );
echo form_hidden( 'cd_participantes_patro', intval($row['cd_participantes_patro']) );
...
if( intval($row['cd_participantes_patro'])>0  )
{
	echo button_delete("indicador_poder/participantes_patro/excluir",$row["cd_participantes_patro"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_poder/participantes_patro')."'; }");
