...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_poder/realizacao_orcam"); ?>';
	}
...
echo form_open( 'indicador_poder/realizacao_orcam/salvar' );
echo form_hidden( 'cd_realizacao_orcam', intval($row['cd_realizacao_orcam']) );
...
if( intval($row['cd_realizacao_orcam'])>0  )
{
	echo button_delete("indicador_poder/realizacao_orcam/excluir",$row["cd_realizacao_orcam"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_poder/realizacao_orcam')."'; }");
