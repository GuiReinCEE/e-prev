...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_poder/performance_carte_invest"); ?>';
	}
...
echo form_open( 'indicador_poder/performance_carte_invest/salvar' );
echo form_hidden( 'cd_performance_carte_invest', intval($row['cd_performance_carte_invest']) );
...
echo form_default_numeric("nr_valor_1", $label_1, app_decimal_para_php($row['nr_valor_1']), "class='indicador_text'", array("centsLimit" => 4)); 
echo form_default_numeric("nr_valor_2", $label_2, app_decimal_para_php($row['nr_valor_2']), "class='indicador_text'", array("centsLimit" => 4));
echo form_default_numeric("nr_percentual_f", $label_3, app_decimal_para_php($row['nr_percentual_f']), "class='indicador_text'", array("centsLimit" => 4));
...
if( intval($row['cd_performance_carte_invest'])>0  )
{
	echo button_delete("indicador_poder/performance_carte_invest/excluir",$row["cd_performance_carte_invest"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_poder/performance_carte_invest')."'; }");
