...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/ri_midia_fav_des"); ?>';
	}
    function manutencao()
    {
...
echo form_open( 'indicador_plugin/ri_midia_fav_des/salvar' );
echo form_hidden( 'cd_ri_midia_fav_des', intval($row['cd_ri_midia_fav_des']) );
...
if( intval($row['cd_ri_midia_fav_des'])>0  )
{
	echo button_delete("indicador_plugin/ri_midia_fav_des/excluir",$row["cd_ri_midia_fav_des"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/ri_midia_fav_des')."'; }");
