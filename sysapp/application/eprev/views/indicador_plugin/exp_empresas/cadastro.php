...
<script>
	<?php 
	echo form_default_js_submit(array("mes_referencia", "ano_referencia", "cd_indicador_tabela", "nr_valor_3", "nr_meta"),'_salvar(form)');	?>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/exp_empresas"); ?>';
	}
	...
	function excluir()
	{
		location.href = '<?php echo site_url("indicador_plugin/exp_empresas/excluir/".$row["cd_exp_empresas"]); ?>';
	}
...
?>
	...
	<?
	echo form_open('indicador_plugin/exp_empresas/salvar');
		echo form_start_box("default_box", 'Cadastro');
			echo form_default_hidden('cd_indicador_tabela', 'Código indicador tabela', $tabela[0]['cd_indicador_tabela']);
			echo form_default_hidden('cd_exp_empresas', 'Código da tabela', intval($row['cd_exp_empresas']));
		...
		echo form_command_bar_detail_start();
			...
			if(intval($row['cd_exp_empresas']) > 0)
			{
