...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/financeiro_exec_fin"); ?>';
	}
	...
	function excluir()
	{
		location.href = '<?php echo site_url("indicador_plugin/financeiro_exec_fin/excluir/".$row["cd_financeiro_exec_fin"]); ?>';
	}
...
echo aba_start($abas);
	echo form_open('indicador_plugin/financeiro_exec_fin/salvar');
		echo form_start_box("default_box", 'Cadastro');
			echo form_default_hidden('cd_indicador_tabela', 'Código indicador tabela', $tabela[0]['cd_indicador_tabela']);
			echo form_default_hidden('cd_financeiro_exec_fin', 'Código da tabela', intval($row['cd_financeiro_exec_fin']));
		...
		echo form_command_bar_detail_start();
			...
			if(intval($row['cd_financeiro_exec_fin']) > 0)
			{
