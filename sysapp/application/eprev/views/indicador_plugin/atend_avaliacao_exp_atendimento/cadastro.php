...
<script>
	...
	function ir_lista()
	{
		location.href = "<?= site_url('indicador_plugin/atend_avaliacao_exp_atendimento') ?>";
	}
	...
	function excluir()
	{
		location.href = "<?= site_url('indicador_plugin/atend_avaliacao_exp_atendimento/excluir/'.$row['cd_atend_avaliacao_exp_atendimento']) ?>";
	}
...
?>
		...
		<? else: ?>
			...
			<?= form_open('indicador_plugin/atend_avaliacao_exp_atendimento/salvar')?>
				<?= form_start_box("default_box", 'Cadastro')?>
					...
					<?= form_default_hidden('cd_atend_avaliacao_exp_atendimento', 'Código da tabela', intval($row['cd_atend_avaliacao_exp_atendimento']))?>
					...
					<?= form_default_integer('nr_avaliacao', $label_2.': (*)', number_format($row['nr_avaliacao'], 0, ',', '.'), 'class="indicador_text"')?>
				...
				<?= form_command_bar_detail_start()?>
					...
					<? if(intval($row['cd_atend_avaliacao_exp_atendimento']) > 0) : ?>
