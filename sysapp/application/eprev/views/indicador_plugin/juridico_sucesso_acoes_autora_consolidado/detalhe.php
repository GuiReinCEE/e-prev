...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/juridico_sucesso_acoes_autora_consolidado"); ?>';
	}
	...
	function excluir()
	{
		location.href = '<?php echo site_url("indicador_plugin/juridico_sucesso_acoes_autora_consolidado/excluir/".$row["cd_juridico_sucesso_acoes_autora_consolidado"]); ?>';
	}
	...
	function getValores()
	{
		if(($("#mes_referencia").val() != "") && ($("#ano_referencia").val() != ""))
		{
			...
			$.post('<?php echo site_url("indicador_plugin/juridico_sucesso_acoes_autora_consolidado/get_valores"); ?>', 
			{
			...
			function(data)
			{
				if(data)
				{
					...
					$("#nr_valor_3").val(data.nr_valor_3);
					$("#nr_valor_4").val(data.nr_valor_4);
				}
...
echo aba_start($abas);
	echo form_open('indicador_plugin/juridico_sucesso_acoes_autora_consolidado/salvar');
		echo form_start_box("default_box", 'Cadastro');
			...
			echo form_default_hidden('cd_juridico_sucesso_acoes_autora_consolidado', 'Código da tabela', intval($row['cd_juridico_sucesso_acoes_autora_consolidado']));
		...
		echo form_command_bar_detail_start();
			...
			if(intval($row['cd_juridico_sucesso_acoes_autora_consolidado']) > 0)
			{
