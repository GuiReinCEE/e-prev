...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/beneficio_inc_sindha"); ?>';
	}
    function manutencao()
    {
...
echo form_open( 'indicador_plugin/beneficio_inc_sindha/salvar' );
echo form_hidden( 'cd_beneficio_inc_sindha', intval($row['cd_beneficio_inc_sindha']) );
...
if( intval($row['cd_beneficio_inc_sindha'])>0  )
{
	echo button_delete("indicador_plugin/beneficio_inc_sindha/excluir",$row["cd_beneficio_inc_sindha"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/beneficio_inc_sindha')."'; }");
