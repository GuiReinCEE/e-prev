...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/atuarial_eap_senge"); ?>';
	}
...
echo form_open( 'indicador_plugin/atuarial_eap_senge/salvar' );
echo form_hidden( 'cd_atuarial_eap_senge', intval($row['cd_atuarial_eap_senge']) );
...
if( intval($row['cd_atuarial_eap_senge'])>0  )
{
	echo button_delete("indicador_plugin/atuarial_eap_senge/excluir",$row["cd_atuarial_eap_senge"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/atuarial_eap_senge')."'; }");
