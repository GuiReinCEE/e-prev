...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/atend_central"); ?>';
	}
...
echo form_open( 'indicador_plugin/atend_central/salvar' );
echo form_hidden( 'cd_atend_central', intval($row['cd_atend_central']) );
...
if( intval($row['cd_atend_central'])>0  )
{
	echo button_delete("indicador_plugin/atend_central/excluir",$row["cd_atend_central"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/atend_central')."'; }");
