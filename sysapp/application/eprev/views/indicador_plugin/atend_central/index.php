...
function load()
{
	...
	$.post( '<?php echo base_url().index_page(); ?>/indicador_plugin/atend_central/listar',{},function(data){ $("#result_div").html(data);configure_result_table(); } );
}
...
function novo()
{
	location.href='<?php echo site_url("indicador_plugin/atend_central/detalhe/0"); ?>';
}
...
function gerar_grafico()
{
	...
	{
		url = '<?php echo site_url("indicador_plugin/atend_central/criar_indicador/"); ?>';
...
function fechar_periodo()
{
	...
	else if( confirm('Fechar o período?') )
	{
		...
		$.post(url,{},function(data){
			...
			url = '<?php echo site_url("indicador_plugin/atend_central/fechar_periodo")?>';
			location.href=url;
		});
	}
}
