...
<script>
	...
	function ir_lista()
	{
		location.href = '<?= site_url("indicador_plugin/exp_adesao_projetado_crmprev") ?>';
	}
	...
	function excluir()
	{
		location.href = '<?= site_url("indicador_plugin/exp_adesao_projetado_crmprev/excluir/".$row["cd_exp_adesao_projetado_crmprev"]) ?>';
	}
...
<? else: ?>
	<?= form_open('indicador_plugin/exp_adesao_projetado_crmprev/salvar')?>
		<?= form_start_box("default_box", 'Indicador')?>
			...
			<?= form_default_hidden('cd_exp_adesao_projetado_crmprev', 'Código da tabela', intval($row['cd_exp_adesao_projetado_crmprev']))?>
		...
		<?= form_command_bar_detail_start()?>
			...
			<? if(intval($row['cd_exp_adesao_projetado_crmprev']) > 0) : ?>
