...
<script>
	<?= form_default_js_submit(array('cd_indicador_tabela', 'mes_referencia', 'ano_referencia','nr_quantidade', 'nr_resgate', 'nr_captacao', 'nr_meta'), 'validacao(form);') ?>
    ...
    function ir_lista()
	{
		location.href = "<?= site_url('indicador_plugin/exp_captacao_liquida') ?>";
	}
	...
	function excluir()
	{
		...
		{
			location.href = "<?= site_url('indicador_plugin/exp_captacao_liquida/excluir/'.$row['cd_exp_captacao_liquida']) ?>";
		}
	}
...
<? else: ?>
	...
	<?= form_open('indicador_plugin/exp_captacao_liquida/salvar') ?>
		...
		<?= form_start_box('default_box', 'Cadastro') ?>
			...
			<?= form_default_hidden('cd_exp_captacao_liquida', '', intval($row['cd_exp_captacao_liquida'])) ?>
			...
			<?= form_default_numeric("nr_quantidade", $label_7.' (*) : ', number_format($row['nr_quantidade'], 2, ',', '.'), 'class="indicador_text"') ?>
			<?= form_default_numeric("nr_resgate", $label_1.' (*) : ', number_format($row['nr_resgate'], 2, ',', '.'), 'class="indicador_text"') ?>
			<?= form_default_numeric("nr_captacao", $label_2.' (*) : ', number_format($row['nr_captacao'], 2, ',', '.'), 'class="indicador_text"') ?>
		...
		<?= form_command_bar_detail_start() ?>
			...
			<? if(intval($row['cd_exp_captacao_liquida']) > 0) :?>	
