...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/atend_sat_planej_fin"); ?>';
	}
...
echo form_open( 'indicador_plugin/atend_sat_planej_fin/salvar' );
echo form_hidden( 'cd_atend_sat_planej_fin', intval($row['cd_atend_sat_planej_fin']) );
...
echo form_default_float("nr_valor_0", $label_1, app_decimal_para_php($row['nr_valor_0']), "class='indicador_text'");
echo form_default_float("nr_valor_1", $label_2, app_decimal_para_php($row['nr_valor_1']), "class='indicador_text'");
...
if( intval($row['cd_atend_sat_planej_fin'])>0  )
{
	echo button_delete("indicador_plugin/atend_sat_planej_fin/excluir",$row["cd_atend_sat_planej_fin"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/atend_sat_planej_fin')."'; }");
