...
<script>
	...
	function ir_lista()
	{
		location.href = "<?= site_url('indicador_plugin/financeiro_variacao_receita_direta') ?>";
	}
	...
	function excluir()
	{
		location.href = "<?= site_url('indicador_plugin/financeiro_variacao_receita_direta/excluir/'.$row['cd_financeiro_variacao_receita_direta']) ?>";
	}
...
?>
		...
		<? else: ?>
			...
			<?= form_open('indicador_plugin/financeiro_variacao_receita_direta/salvar')?>
				<?= form_start_box("default_box", 'Cadastro')?>
					...
					<?= form_default_hidden('cd_financeiro_variacao_receita_direta', 'Código da tabela', intval($row['cd_financeiro_variacao_receita_direta']))?>
					...
					<?= form_default_integer('nr_receita', $label_1.': (*)', number_format($row['nr_receita'], 2, ',', '.'), 'class="indicador_text"')?> 
					<?= form_default_integer('nr_meta_valor', $label_2.': (*)', number_format($row['nr_meta_valor'], 2, ',', '.'), 'class="indicador_text"')?>
				...
				<?= form_command_bar_detail_start()?>
					...
					<? if(intval($row['cd_financeiro_variacao_receita_direta']) > 0) : ?>
