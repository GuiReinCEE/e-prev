...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/beneficio_inc_inpel"); ?>';
	}
    function manutencao()
{
...
echo form_open( 'indicador_plugin/beneficio_inc_inpel/salvar' );
echo form_hidden( 'cd_beneficio_inc_inpel', intval($row['cd_beneficio_inc_inpel']) );
...
echo form_default_numeric("nr_meta", $label_4, app_decimal_para_php($row['nr_meta']), "class='indicador_text'");
...
if( intval($row['cd_beneficio_inc_inpel'])>0  )
{
	echo button_delete("indicador_plugin/beneficio_inc_inpel/excluir",$row["cd_beneficio_inc_inpel"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/beneficio_inc_inpel')."'; }");
