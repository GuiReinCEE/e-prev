...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/beneficio_inc_ceran"); ?>';
	}
...
echo form_open( 'indicador_plugin/beneficio_inc_ceran/salvar' );
echo form_hidden( 'cd_beneficio_inc_ceran', intval($row['cd_beneficio_inc_ceran']) );
...
elseif( sizeof($tabela)>1 )
{
	...
	echo form_default_row( "", "", "<span style='font-size:12;'>Existe mais de um período aberto, no entanto são serão possÃ­vel incluir valores para o novo periodo depois de fechar o mais antigo.</span>".br(2) );
}
else
{
...
if( $row['fl_media']=='S' )
{
	//echo form_default_row("", "MÃ©dia", "<input id='fl_media' name='fl_media' type='checkbox' checked value='S' />");
}
else
{
	//echo form_default_row("", "MÃ©dia", "<input id='fl_media' name='fl_media' type='checkbox' value='S' />");
}
...
if( intval($row['cd_beneficio_inc_ceran'])>0  )
{
	echo button_delete("indicador_plugin/beneficio_inc_ceran/excluir",$row["cd_beneficio_inc_ceran"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/beneficio_inc_ceran')."'; }");
