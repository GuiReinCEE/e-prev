...
<script>
	<?php 
	echo form_default_js_submit(array('mes_referencia', 'ano_referencia', 'cd_indicador_tabela', 'nr_valor_1', 'nr_valor_2', 'nr_atas_disp_10_dias', 'nr_meta'),'_salvar(form)');	?>
	...
	function ir_lista()
	{
		location.href = '<?= site_url("indicador_plugin/secretaria_atas_cf_assinadas_fora_prazo") ?>';
	}
	...
	function excluir()
	{
		location.href = '<?= site_url("indicador_plugin/secretaria_atas_cf_assinadas_fora_prazo/excluir/".$row["cd_secretaria_atas_cf_assinadas_fora_prazo"]) ?>';
	}
...
<? else: ?>
	<?= form_open('indicador_plugin/secretaria_atas_cf_assinadas_fora_prazo/salvar')?>
		<?= form_start_box("default_box", 'Cadastro')?>
			...
			<?= form_default_hidden('cd_secretaria_atas_cf_assinadas_fora_prazo', 'Código da tabela', intval($row['cd_secretaria_atas_cf_assinadas_fora_prazo']))?>
			...
			<?= form_default_integer('nr_atas_disp_10_dias', $label_2.': (*)',($row['nr_atas_disp_10_dias']), "class='indicador_text'")?>
		...
		<?= form_command_bar_detail_start()?>
			...
			<? if(intval($row['cd_secretaria_atas_cf_assinadas_fora_prazo']) > 0) : ?>
