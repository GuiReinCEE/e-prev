...
<script>
	...
	function ir_lista()
	{
		location.href = "<?= site_url('indicador_plugin/atuarial_exigencia_previc_regulamento') ?>";
	}
	...
	function excluir()
	{
		location.href = "<?= site_url('indicador_plugin/atuarial_exigencia_previc_regulamento/excluir/'.$row['cd_atuarial_exigencia_previc_regulamento']) ?>";
	}
...
?>
		...
		<? else: ?>
			<?= form_open('indicador_plugin/atuarial_exigencia_previc_regulamento/salvar')?>
				<?= form_start_box("default_box", 'Cadastro')?>
					...
					<?= form_default_hidden('cd_atuarial_exigencia_previc_regulamento', 'Código da tabela', intval($row['cd_atuarial_exigencia_previc_regulamento']))?>
				...
				<?= form_command_bar_detail_start()?>
					...
					<? if(intval($row['cd_atuarial_exigencia_previc_regulamento']) > 0) : ?>
