...
<script>
	...
	function ir_lista()
	{
		location.href = "<?= site_url('indicador_plugin/atuarial_divulgacao_extratos_prazo') ?>";
	}
	...
	function excluir()
	{
		location.href = "<?= site_url('indicador_plugin/atuarial_divulgacao_extratos_prazo/excluir/'.$row['cd_atuarial_divulgacao_extratos_prazo']) ?>";
	}
...
?>
		...
		<? else: ?>
			...
			<?= form_open('indicador_plugin/atuarial_divulgacao_extratos_prazo/salvar')?>
				<?= form_start_box("default_box", 'Cadastro')?>
					...
					<?= form_default_hidden('cd_atuarial_divulgacao_extratos_prazo', 'Código da tabela', intval($row['cd_atuarial_divulgacao_extratos_prazo']))?>
					...
					<?= form_default_integer('nr_atendidas', $label_2.': (*)', number_format($row['nr_atendidas'], 0, ',', '.'), 'class="indicador_text"')?>
				...
				<?= form_command_bar_detail_start()?>
					...
					<? if(intval($row['cd_atuarial_divulgacao_extratos_prazo']) > 0) : ?>
