...
foreach($collection as $item)
{
	...
	{
		$link = anchor("indicador_plugin/secretaria_retorno/cadastro/".$item["cd_secretaria_retorno"], "editar");
	...
	$body[] = array(
		...
		number_format($item['nr_sumula_disp_24h'], 0, ',', '.'),
		number_format($item['nr_percent_disp_24h'], 2, ',', '.').' %',
		number_format($item['nr_valor_2'], 0, ',', '.'),
...
if($contador_ano_atual > 0)
{
	...
	$body[] = array(
		...
		'<b>'.number_format($nr_sumula_disp_24h, 0, ',', '.').'</b>', 
		'<b>'.number_format($nr_percent_disp_24h, 2, ',', '.').' %</b>',
		'<b>'.number_format($nr_valor_2, 0, ',', '.').'</b>',
