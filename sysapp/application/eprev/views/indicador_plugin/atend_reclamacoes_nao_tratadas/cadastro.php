...
<script>
	<?= form_default_js_submit(array('cd_indicador_tabela', 'mes_referencia', 'ano_referencia', 'nr_reclamacao', 'nr_reclamacao_nao_tratadas', 'nr_meta'), 'validacao(form);') ?>
    ...
    function ir_lista()
	{
		location.href = "<?= site_url('indicador_plugin/atend_reclamacoes_nao_tratadas') ?>";
	}
	...
	function excluir()
	{
		...
		{
			location.href = "<?= site_url('indicador_plugin/atend_reclamacoes_nao_tratadas/excluir/'.$row['cd_atend_reclamacoes_nao_tratadas']) ?>";
		}
	}
...
<? else: ?>
	...
	<?= form_open('indicador_plugin/atend_reclamacoes_nao_tratadas/salvar') ?>
		...
		<?= form_start_box('default_box', 'Cadastro') ?>
			...
			<?= form_default_hidden('cd_atend_reclamacoes_nao_tratadas', '', intval($row['cd_atend_reclamacoes_nao_tratadas'])) ?>
			...
			<?= form_default_integer('nr_reclamacao_nao_tratadas', $label_2.' :', $row['nr_reclamacao_nao_tratadas'], 'class="indicador_text"') ?>			
		...
		<?= form_command_bar_detail_start() ?>
			...
			<? if(intval($row['cd_atend_reclamacoes_nao_tratadas']) > 0) :?>	
