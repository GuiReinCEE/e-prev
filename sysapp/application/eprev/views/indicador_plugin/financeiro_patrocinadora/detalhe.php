...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/financeiro_patrocinadora"); ?>';
	}
    function manutencao()
    {
...
echo form_open( 'indicador_plugin/financeiro_patrocinadora/salvar' );
echo form_hidden( 'cd_financeiro_patrocinadora', intval($row['cd_financeiro_patrocinadora']) );
...
echo form_default_float("nr_valor", $label_1, app_decimal_para_php($row['nr_valor']), "class='indicador_text'");
echo form_default_float("nr_execucao", $label_2, app_decimal_para_php($row['nr_execucao']), "class='indicador_text'");
...
if( intval($row['cd_financeiro_patrocinadora'])>0  )
{
	echo button_delete("indicador_plugin/financeiro_patrocinadora/excluir",$row["cd_financeiro_patrocinadora"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/financeiro_patrocinadora')."'; }");
