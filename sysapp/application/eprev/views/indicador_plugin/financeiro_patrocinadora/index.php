<?php
$tabela = indicador_tabela_aberta( intval(enum_indicador::FINANCEIRO_EXECUCAO_TOTAL_DAS_PATROCINADORAS) );
if($tabela)
{
...
function load()
{
	...
	$.post( '<?php echo base_url().index_page(); ?>/indicador_plugin/financeiro_patrocinadora/listar',{},function(data){ $("#result_div").html(data);configure_result_table(); } );
}
...
function novo()
{
	location.href='<?php echo site_url("indicador_plugin/financeiro_patrocinadora/detalhe/0"); ?>';
}
...
function gerar_grafico()
{
	...
	{
		url = '<?php echo site_url("indicador_plugin/financeiro_patrocinadora/criar_indicador/"); ?>';
...
function fechar_periodo()
{
	...
	else if( confirm('Fechar o período?') )
	{
		...
		$.post(url,{},function(data){
			...
			url = '<?php echo site_url("indicador_plugin/financeiro_patrocinadora/fechar_periodo")?>';
			location.href=url;
		});
	}
}
