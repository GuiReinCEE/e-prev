...
<script>
	...
	function ir_lista()
	{
		location.href = "<?= site_url('indicador_plugin/atend_sat_ppa') ?>";
	}
	...
	function excluir()
	{
		...
		{
			location.href = "<?= site_url('indicador_plugin/atend_sat_ppa/excluir/'.$row['cd_atend_sat_ppa']) ?>";
		}
	}
...
<? else: ?>
	...
	<?= form_open('indicador_plugin/atend_sat_ppa/salvar') ?>
		...
		<?= form_start_box('default_box', 'Cadastro') ?>
			...
			<?= form_default_hidden('cd_atend_sat_ppa', '', intval($row['cd_atend_sat_ppa'])) ?>

			<?= form_default_mes_ano('mes_referencia', 'ano_referencia', $label_0.': (*)', $row['dt_referencia']) ?>
			<?= form_default_hidden('dt_referencia', 'Mês', $row) ?>
			<?= form_default_integer('nr_valor_0', $label_1.': ', $row['nr_valor_0'], 'class="indicador_text"') ?>
		...
		<?= form_command_bar_detail_start() ?>
			...
			<? if(intval($row['cd_atend_sat_ppa']) > 0) :?>	
