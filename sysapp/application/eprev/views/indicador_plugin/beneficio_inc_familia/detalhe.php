...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/beneficio_inc_familia"); ?>';
	}
    function manutencao()
    {
...
echo form_open( 'indicador_plugin/beneficio_inc_familia/salvar' );
echo form_hidden( 'cd_beneficio_inc_familia', intval($row['cd_beneficio_inc_familia']) );
...
if( intval($row['cd_beneficio_inc_familia'])>0  )
{
	echo button_delete("indicador_plugin/beneficio_inc_familia/excluir",$row["cd_beneficio_inc_familia"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/beneficio_inc_familia')."'; }");
