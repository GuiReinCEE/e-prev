...
<script>
    ...
    function ir_lista()
	{
		location.href = "<?= site_url('indicador_plugin/exp_volume_recursos_municipios') ?>";
	}
	...
	function excluir()
	{
		...
		{
			location.href = "<?= site_url('indicador_plugin/exp_volume_recursos_municipios/excluir/'.$row['cd_exp_volume_recursos_municipios']) ?>";
		}
	}
...
<? else: ?>
	...
	<?= form_open('indicador_plugin/exp_volume_recursos_municipios/salvar') ?>
		...
		<?= form_start_box('default_box', 'Cadastro') ?>
			...
			<?= form_default_hidden('cd_exp_volume_recursos_municipios', '', intval($row['cd_exp_volume_recursos_municipios'])) ?>
		...
		<?= form_command_bar_detail_start() ?>
			...
			<? if(intval($row['cd_exp_volume_recursos_municipios']) > 0) :?>	
