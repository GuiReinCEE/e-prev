...
echo anchor_popup("indicador/apresentacao/detalhe/".intval( $tabela[0]['cd_indicador_tabela'] ), 'Visualizar apresentação', $ar_janela);
	...
	foreach( $collection as $item )
	{
		...
		{
			$link = anchor("indicador_plugin/atend_sat_cli_atendimento/detalhe/" . $item["cd_atend_sat_cli_atendimento"], "editar");
		...
		$body[] = array(
            ...
            , ($nr_meta_1!='')?number_format($nr_meta_1,2,',','.').' %':''
			, ($nr_valor_2!='')?number_format($nr_valor_2,2,',','.').' %':''
            , ($nr_meta_2!='')?number_format($nr_meta_2,2,',','.').' %':''
            , ($nr_valor_3!='')?number_format($nr_valor_3,2,',','.').' %':''
            , ($nr_meta_3!='')?number_format($nr_meta_3,2,',','.').' %':''
