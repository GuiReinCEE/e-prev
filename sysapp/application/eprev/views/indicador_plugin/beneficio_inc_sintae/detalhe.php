...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/beneficio_inc_sintae"); ?>';
	}
    function manutencao()
{
...
echo form_open( 'indicador_plugin/beneficio_inc_sintae/salvar' );
echo form_hidden( 'cd_beneficio_inc_sintae', intval($row['cd_beneficio_inc_sintae']) );
...
if( intval($row['cd_beneficio_inc_sintae'])>0  )
{
	echo button_delete("indicador_plugin/beneficio_inc_sintae/excluir",$row["cd_beneficio_inc_sintae"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/beneficio_inc_sintae')."'; }");
