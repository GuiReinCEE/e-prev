...
<script>
	...
	function ir_lista()
	{
		location.href = '<?= site_url("indicador_plugin/exp_adesao_projetado_ieab") ?>';
	}
	...
	function excluir()
	{
		location.href = '<?= site_url("indicador_plugin/exp_adesao_projetado_ieab/excluir/".$row["cd_exp_adesao_projetado_ieab"]) ?>';
	}
...
<? else: ?>
	<?= form_open('indicador_plugin/exp_adesao_projetado_ieab/salvar')?>
		<?= form_start_box("default_box", 'Indicador')?>
			...
			<?= form_default_hidden('cd_exp_adesao_projetado_ieab', 'Código da tabela', intval($row['cd_exp_adesao_projetado_ieab']))?>
		...
		<?= form_command_bar_detail_start()?>
			...
			<? if(intval($row['cd_exp_adesao_projetado_ieab']) > 0) : ?>
