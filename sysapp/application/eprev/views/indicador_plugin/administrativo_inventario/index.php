...
function load()
{
	...
	$.post( '<?php echo base_url().index_page(); ?>/indicador_plugin/administrativo_inventario/listar',{},function(data){ $("#result_div").html(data);configure_result_table(); } );
}
...
function novo()
{
	location.href='<?php echo site_url("indicador_plugin/administrativo_inventario/detalhe/0"); ?>';
}
...
function gerar_grafico()
{
	...
	{
		url = '<?php echo site_url("indicador_plugin/administrativo_inventario/criar_indicador/"); ?>';
...
function fechar_periodo()
{
	...
	else if( $('#mes_input').val()!='2' )
	{
		alert( "Informar os dois semestres do ano." );
	}
	else if( confirm('Fechar o período?') )
	{
		...
		$.post(url,{},function(data){
			...
			url = '<?php echo site_url("indicador_plugin/administrativo_inventario/fechar_periodo")?>';
			location.href=url;
		});
	}
}
