...
<script>
	...
	function ir_lista()
	{
		location.href = "<?= site_url("indicador_plugin/exp_satisfacao_com_acoes") ?>";
	}
	...
	function excluir()
	{
		location.href = "<?= site_url("indicador_plugin/exp_satisfacao_com_acoes/excluir/".$row["cd_exp_satisfacao_com_acoes"]) ?>";
	}
...
<?= form_end_box('indicador_box') ?>
	<?= form_open('indicador_plugin/exp_satisfacao_com_acoes/salvar')?>
		<?= form_start_box('default_box', 'Cadastro')?>
			...
			<?= form_default_hidden('cd_exp_satisfacao_com_acoes', 'Código da tabela', intval($row['cd_exp_satisfacao_com_acoes']))?>
			<?= form_default_row('','','')?>
			...
			<?= form_default_integer('nr_valor_2', $label_2.' : (*)', ($row['nr_valor_2']), "class='indicador_text'")?>
		...
		<?= form_command_bar_detail_start()?>
			<?= button_save()?>
			<? if(intval($row['cd_exp_satisfacao_com_acoes']) > 0): ?>
