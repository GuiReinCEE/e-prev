...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/administrativo_ept"); ?>';
	}
...
echo form_open( 'indicador_plugin/administrativo_ept/salvar' );
echo form_hidden( 'cd_administrativo_ept', intval($row['cd_administrativo_ept']) );
...
if( intval($row['cd_administrativo_ept'])>0  )
{
	echo button_delete("indicador_plugin/administrativo_ept/excluir",$row["cd_administrativo_ept"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/administrativo_ept')."'; }");
