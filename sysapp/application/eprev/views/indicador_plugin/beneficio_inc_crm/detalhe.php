...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/beneficio_inc_crm"); ?>';
	}
    function manutencao()
{
...
echo form_open( 'indicador_plugin/beneficio_inc_crm/salvar' );
echo form_hidden( 'cd_beneficio_inc_crm', intval($row['cd_beneficio_inc_crm']) );
...
if( intval($row['cd_beneficio_inc_crm'])>0  )
{
	echo button_delete("indicador_plugin/beneficio_inc_crm/excluir",$row["cd_beneficio_inc_crm"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/beneficio_inc_crm')."'; }");
