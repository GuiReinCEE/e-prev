...
<script>
	...
	function ir_lista()
	{
		location.href = '<?= site_url("indicador_plugin/secretaria_atas_cd_assinadas_fora_prazo") ?>';
	}
	...
	function excluir()
	{
		location.href = '<?= site_url("indicador_plugin/secretaria_atas_cd_assinadas_fora_prazo/excluir/".$row["cd_secretaria_atas_cd_assinadas_fora_prazo"]) ?>';
	}
...
<? else: ?>
	<?= form_open('indicador_plugin/secretaria_atas_cd_assinadas_fora_prazo/salvar')?>
		<?= form_start_box("default_box", 'Cadastro')?>
			...
			<?= form_default_hidden('cd_secretaria_atas_cd_assinadas_fora_prazo', 'Código da tabela', intval($row['cd_secretaria_atas_cd_assinadas_fora_prazo']))?>
		...
		<?= form_command_bar_detail_start()?>
			...
			<? if(intval($row['cd_secretaria_atas_cd_assinadas_fora_prazo']) > 0) : ?>
