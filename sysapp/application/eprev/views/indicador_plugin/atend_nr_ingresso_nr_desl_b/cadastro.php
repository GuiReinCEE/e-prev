...
<script>
	...
	function ir_lista()
	{
		location.href = '<?= site_url("indicador_plugin/atend_nr_ingresso_nr_desl_b") ?>';
	}
	...
	function excluir()
	{
		location.href = '<?= site_url("indicador_plugin/atend_nr_ingresso_nr_desl_b/excluir/".$row["cd_atend_nr_ingresso_nr_desl_b"]) ?>';
	}
...
echo aba_start($abas);
	echo form_open('indicador_plugin/atend_nr_ingresso_nr_desl_b/salvar');
		echo form_start_box("default_box", 'Cadastro');
			echo form_default_hidden('cd_indicador_tabela', 'Código indicador tabela', $tabela[0]['cd_indicador_tabela']);
			echo form_default_hidden('cd_atend_nr_ingresso_nr_desl_b', 'Código da tabela', intval($row['cd_atend_nr_ingresso_nr_desl_b']));
			...
			echo form_default_integer("nr_valor_3", $label_2.' :', $row['nr_valor_3'], "class='indicador_text'");
		...
		echo form_command_bar_detail_start();
			...
			if(intval($row['cd_atend_nr_ingresso_nr_desl_b']) > 0)
			{
