...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/beneficio_inc_ffp"); ?>';
	}
    function manutencao()
    {
...
echo form_open( 'indicador_plugin/beneficio_inc_ffp/salvar' );
echo form_hidden( 'cd_beneficio_inc_ffp', intval($row['cd_beneficio_inc_ffp']) );
...
if( intval($row['cd_beneficio_inc_ffp'])>0  )
{
	echo button_delete("indicador_plugin/beneficio_inc_ffp/excluir",$row["cd_beneficio_inc_ffp"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/beneficio_inc_ffp')."'; }");
