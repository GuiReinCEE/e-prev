...
<script>
	<?php 
	echo form_default_js_submit(array("mes_referencia", "ano_referencia", "cd_indicador_tabela", "nr_horas_previstas","nr_horas_realiazadas", "nr_meta"),'_salvar(form)');	?>
	...
	function ir_lista()
	{
		location.href = '<?= site_url("indicador_plugin/auditoria_atendimento_prazo") ?>';
	}
	...
	function excluir()
	{
		location.href = '<?= site_url("indicador_plugin/auditoria_atendimento_prazo/excluir/".$row["cd_auditoria_atendimento_prazo"]) ?>';
	}
...
?>
	...
	<?
	echo form_open('indicador_plugin/auditoria_atendimento_prazo/salvar');
		echo form_start_box("default_box", 'Cadastro');
			echo form_default_hidden('cd_indicador_tabela', 'Código indicador tabela', $tabela[0]['cd_indicador_tabela']);

			echo form_default_hidden('cd_auditoria_atendimento_prazo', 'Código da tabela', intval($row['cd_auditoria_atendimento_prazo']));
			...
			echo form_default_integer("nr_solicitacoes", $label_1.' :', $row['nr_solicitacoes'], "class='indicador_text'"); 

			echo form_default_integer("nr_respondidos_prazo", $label_2.' :', $row['nr_respondidos_prazo'], "class='indicador_text'");
		...
		echo form_command_bar_detail_start();
			...
			if(intval($row['cd_auditoria_atendimento_prazo']) > 0)
			{
