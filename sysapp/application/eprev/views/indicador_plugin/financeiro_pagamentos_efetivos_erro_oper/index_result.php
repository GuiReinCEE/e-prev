<?php
	...
	foreach($collection as $item)
	{
		...
		{
			$link = anchor('indicador_plugin/financeiro_pagamentos_efetivos_erro_oper/cadastro/'.$item['cd_financeiro_pagamentos_efetivos_erro_oper'], 'editar');
		...
		$body[] = array(
			$contador--,
			$referencia,
			number_format($item['nr_total_borderos'], 0, ',', '.'),
			number_format($item['nr_volume_debito_conta'], 0, ',', '.'),
			number_format($item['nr_pagamento_erro_oper'], 2, ',', '.').' %',
	...
	if($contador_ano_atual > 0)
	{
		...
		$body[] = array(
			...
			'<b>'.number_format($nr_total_borderos, 0, ',', '.').'</b>',
			'<b>'.number_format($nr_volume_debito_conta, 0, ',', '.').'</b>', 
			'<b>'.number_format($nr_pagamento_erro_oper, 2, ',', '.').' %</b>',
