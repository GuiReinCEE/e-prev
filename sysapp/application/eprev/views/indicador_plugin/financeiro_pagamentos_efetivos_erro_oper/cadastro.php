...
<script>
	...
	function ir_lista()
	{
		location.href = "<?= site_url('indicador_plugin/financeiro_pagamentos_efetivos_erro_oper') ?>";
	}
	...
	function excluir()
	{
		location.href = "<?= site_url('indicador_plugin/financeiro_pagamentos_efetivos_erro_oper/excluir/'.$row['cd_financeiro_pagamentos_efetivos_erro_oper']) ?>";
	}
...
?>
	...
	<? endif; ?>
			<?= form_open('indicador_plugin/financeiro_pagamentos_efetivos_erro_oper/salvar')?>
				<?= form_start_box("default_box", 'Cadastro')?>
					...
					<?= form_default_hidden('cd_financeiro_pagamentos_efetivos_erro_oper', 'Código da tabela', intval($row['cd_financeiro_pagamentos_efetivos_erro_oper']))?>
					...
					<?= form_default_integer('nr_total_borderos', $label_1.': (*)', number_format($row['nr_total_borderos'], 0, ',', '.'), 'class="indicador_text"')?> 
					<?= form_default_integer('nr_volume_debito_conta', $label_2.': (*)', number_format($row['nr_volume_debito_conta'], 0, ',', '.'), 'class="indicador_text"')?>
				...
				<?= form_command_bar_detail_start()?>
					...
					<? if(intval($row['cd_financeiro_pagamentos_efetivos_erro_oper']) > 0) : ?>
