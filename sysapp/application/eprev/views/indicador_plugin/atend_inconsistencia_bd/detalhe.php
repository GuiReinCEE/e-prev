...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/atend_inconsistencia_bd"); ?>';
	}
...
echo form_open( 'indicador_plugin/atend_inconsistencia_bd/salvar' );
echo form_hidden( 'cd_atend_inconsistencia_bd', intval($row['cd_atend_inconsistencia_bd']) );
...
if( intval($row['cd_atend_inconsistencia_bd'])>0  )
{
	echo button_delete("indicador_plugin/atend_inconsistencia_bd/excluir",$row["cd_atend_inconsistencia_bd"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/atend_inconsistencia_bd')."'; }");
