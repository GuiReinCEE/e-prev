...
else
{
	...
	foreach( $collection as $item )
	{
		...
		{
			$link = anchor("indicador_plugin/atend_inconsistencia_bd/detalhe/" . $item["cd_atend_inconsistencia_bd"], "editar");
		...
		$body[] = array(
			...
			, (trim($nr_percentual_f) != '' ? number_format($nr_percentual_f,3,',','.').' %' : '0')
	...
	if( sizeof($media_ano)>0 )
	{
		...
		$body[] = array(
			...
			, ($contador_ano_atual > 0 ? number_format($nr_media_1/$contador_ano_atual,2,',','.') : 0)
			, ($contador_ano_atual > 0 ? number_format($nr_media_2/$contador_ano_atual,2,',','.') : 0)
