...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/beneficio_inc_senge"); ?>';
	}
    function manutencao()
{
...
echo form_open( 'indicador_plugin/beneficio_inc_senge/salvar' );
echo form_hidden( 'cd_beneficio_inc_senge', intval($row['cd_beneficio_inc_senge']) );
...
if( intval($row['cd_beneficio_inc_senge'])>0  )
{
	echo button_delete("indicador_plugin/beneficio_inc_senge/excluir",$row["cd_beneficio_inc_senge"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/beneficio_inc_senge')."'; }");
