...
<script>
	<?= form_default_js_submit(array('cd_indicador_tabela', 'mes_referencia', 'ano_referencia', 'nr_pessoal', 'nr_telefonico', 'nr_email', 'nr_correspondencia', 'nr_virtual', 'nr_whatsapp'), 'validacao(form);') ?>
    ...
    function ir_lista()
	{
		location.href = "<?= site_url('indicador_plugin/atend_numero_atendimentos') ?>";
	}
	...
	function excluir()
	{
		...
		{
			location.href = "<?= site_url('indicador_plugin/atend_numero_atendimentos/excluir/'.$row['cd_atend_numero_atendimentos']) ?>";
		}
	}
...
<? else: ?>
	...
	<?= form_open('indicador_plugin/atend_numero_atendimentos/salvar') ?>
		...
		<?= form_start_box('default_box', 'Cadastro') ?>
			...
			<?= form_default_hidden('cd_atend_numero_atendimentos', '', intval($row['cd_atend_numero_atendimentos'])) ?>
			...
			<?= form_default_integer('nr_pessoal', $label_1.' (*) :', $row['nr_pessoal'], 'class="indicador_text"') ?>
			<?= form_default_integer('nr_telefonico', $label_2.' (*) :', $row['nr_telefonico'], 'class="indicador_text"') ?>
			<?= form_default_integer('nr_email', $label_3.' (*) :', $row['nr_email'], 'class="indicador_text"') ?>
			<?= form_default_integer('nr_correspondencia', $label_6.' (*) :', $row['nr_correspondencia'], 'class="indicador_text"') ?>
			<?= form_default_integer('nr_virtual', $label_7.' (*) :', $row['nr_virtual'], 'class="indicador_text"') ?>
			<?= form_default_integer('nr_whatsapp', $label_8.' (*) :', $row['nr_whatsapp'], 'class="indicador_text"') ?>
		...
		<?= form_command_bar_detail_start() ?>
			...
			<? if(intval($row['cd_atend_numero_atendimentos']) > 0) :?>	
