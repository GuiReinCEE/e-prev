...
<script>
	<?php 
	echo form_default_js_submit(array('mes_referencia', 'ano_referencia', 'cd_indicador_tabela', 'nr_vendas', 'nr_ingressos', 'nr_meta'),'_salvar(form)');	?>
	...
	function ir_lista()
	{
		location.href = '<?= site_url("indicador_plugin/exp_vendas_ingressos") ?>';
	}
	...
	function excluir()
	{
		location.href = '<?= site_url("indicador_plugin/exp_vendas_ingressos/excluir/".$row["cd_exp_vendas_ingressos"]) ?>';
	}
...
<? else: ?>
	...
	<?= form_open('indicador_plugin/exp_vendas_ingressos/salvar') ?>
		...
		<?= form_start_box('default_box', 'Cadastro')?>
			...
			<?= form_default_hidden('cd_exp_vendas_ingressos', $label_0.": (*)", $row)?> 
			<?= form_default_mes_ano('mes_referencia', 'ano_referencia', $label_0.": (*)", $row['dt_referencia'])?>
			<?= form_default_integer('nr_vendas', $label_1.": (*)", number_format($row['nr_vendas'], 0,",","."), "class='indicador_text'")?> 
			<?= form_default_integer('nr_ingressos', $label_2.": (*)", number_format($row['nr_ingressos'], 0,",","."), "class='indicador_text'")?>
		...
		<?= form_command_bar_detail_start()?>
			...
			<? if(intval($row['cd_exp_vendas_ingressos']) > 0) : ?>
