...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/beneficio_inc_sintep"); ?>';
	}
    function manutencao()
{
...
echo form_open( 'indicador_plugin/beneficio_inc_sintep/salvar' );
echo form_hidden( 'cd_beneficio_inc_sintep', intval($row['cd_beneficio_inc_sintep']) );
...
if( intval($row['cd_beneficio_inc_sintep'])>0  )
{
	echo button_delete("indicador_plugin/beneficio_inc_sintep/excluir",$row["cd_beneficio_inc_sintep"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/beneficio_inc_sintep')."'; }");
