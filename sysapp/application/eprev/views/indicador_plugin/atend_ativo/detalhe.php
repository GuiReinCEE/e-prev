...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/atend_ativo"); ?>';
	}
...
echo form_open( 'indicador_plugin/atend_ativo/salvar' );
echo form_hidden( 'cd_atend_ativo', intval($row['cd_atend_ativo']) );
...
if( intval($row['cd_atend_ativo'])>0  )
{
	echo button_delete("indicador_plugin/atend_ativo/excluir",$row["cd_atend_ativo"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/atend_ativo')."'; }");
