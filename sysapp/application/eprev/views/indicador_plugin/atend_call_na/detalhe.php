...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/atend_call_na"); ?>';
	}
...
echo form_open( 'indicador_plugin/atend_call_na/salvar' );
echo form_hidden( 'cd_atend_call_na', intval($row['cd_atend_call_na']) );
...
if( intval($row['cd_atend_call_na'])>0  )
{
	echo button_delete("indicador_plugin/atend_call_na/excluir",$row["cd_atend_call_na"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/atend_call_na')."'; }");
