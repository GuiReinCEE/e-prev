...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/atend_doc_faltante"); ?>';
	}
...
echo form_open( 'indicador_plugin/atend_doc_faltante/salvar' );
echo form_hidden( 'cd_atend_doc_faltante', intval($row['cd_atend_doc_faltante']) );
...
if( intval($row['cd_atend_doc_faltante'])>0  )
{
	echo button_delete("indicador_plugin/atend_doc_faltante/excluir",$row["cd_atend_doc_faltante"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/atend_doc_faltante')."'; }");
