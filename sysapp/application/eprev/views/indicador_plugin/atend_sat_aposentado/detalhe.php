...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/atend_sat_aposentado"); ?>';
	}
...
echo form_open( 'indicador_plugin/atend_sat_aposentado/salvar' );
echo form_hidden( 'cd_atend_sat_aposentado', intval($row['cd_atend_sat_aposentado']) );
...
if( intval($row['cd_atend_sat_aposentado'])>0  )
{
	echo button_delete("indicador_plugin/atend_sat_aposentado/excluir",$row["cd_atend_sat_aposentado"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/atend_sat_aposentado')."'; }");
