<?php
$tabela = indicador_tabela_aberta( intval(enum_indicador::SECRETARIA_DC_ENCAMINHADA_PARA_DIVULGACAO_FORA_PRAZO) );
if($tabela)
{
...
function load()
{
	...
	$.post( '<?php echo base_url().index_page(); ?>/indicador_plugin/secretaria_dc/listar',{},function(data){ $("#result_div").html(data);configure_result_table(); } );
}
...
function novo()
{
	location.href='<?php echo site_url("indicador_plugin/secretaria_dc/detalhe/0"); ?>';
}
...
function gerar_grafico()
{
	...
	{
		url = '<?php echo site_url("indicador_plugin/secretaria_dc/criar_indicador/"); ?>';
...
function fechar_periodo()
{
	...
	else if( confirm('Fechar o período?') )
	{
		...
		$.post(url,{},function(data){
			...
			url = '<?php echo site_url("indicador_plugin/secretaria_dc/fechar_periodo")?>';
			location.href=url;
		});
	}
}
