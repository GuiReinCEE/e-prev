...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/secretaria_dc"); ?>';
	}
    function manutencao()
    {
...
echo form_open( 'indicador_plugin/secretaria_dc/salvar' );
echo form_hidden( 'cd_secretaria_dc', intval($row['cd_secretaria_dc']) );
...
if( intval($row['cd_secretaria_dc'])>0  )
{
	echo button_delete("indicador_plugin/secretaria_dc/excluir",$row["cd_secretaria_dc"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/secretaria_dc')."'; }");
