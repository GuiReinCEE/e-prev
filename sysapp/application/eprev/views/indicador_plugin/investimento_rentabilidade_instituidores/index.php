...
<script>
	function filtrar()
	{
		...
		$.post('<?= site_url('indicador_plugin/investimento_rentabilidade_instituidores/listar') ?>',
		function(data)
		{ 
	...
	function fechar_periodo()
	{
		...
		{
			$.post('<?= site_url("indicador_plugin/investimento_rentabilidade_instituidores/criar_indicador") ?>',
			function(data)
			{
				$('#output_tela').html("Indicadores atualizados com sucesso, aguarde enquanto o período é fechado ..." );

				location.href = '<?= site_url("indicador_plugin/investimento_rentabilidade_instituidores/fechar_periodo") ?>';
			});
		}
	}
	...
	function novo()
	{
		location.href = '<?= site_url("indicador_plugin/investimento_rentabilidade_instituidores/cadastro") ?>';
	}
...
echo '
	...
	<div id="result_div">'.br(2).'
		<center>
			<span class="label label-success">nominalize um filtro para exibir a lista</span>
		</center>
	</div>';
echo br();
