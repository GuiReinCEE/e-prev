...
<script>
	function filtrar()
	{
		...
		$.post('<?= site_url('indicador_plugin/comunicacao_nr_acesso_site/listar') ?>',
		function(data)
		{ 
	...
	function fechar_periodo()
	{
		...
		{
			$.post('<?= site_url("indicador_plugin/comunicacao_nr_acesso_site/criar_indicador") ?>',
			function(data)
			{
				$('#output_tela').html("Indicadores atualizados com sucesso, aguarde enquanto o período é fechado ..." );

				location.href = '<?= site_url("indicador_plugin/comunicacao_nr_acesso_site/fechar_periodo") ?>';
			});
		}
	}
	...
	function novo()
	{
		location.href = '<?= site_url("indicador_plugin/comunicacao_nr_acesso_site/cadastro") ?>';
	}
