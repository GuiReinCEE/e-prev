...
<script>
	...
	function ir_lista()
	{
		location.href = '<?= site_url("indicador_plugin/comunicacao_nr_acesso_site") ?>';
	}
	...
	function excluir()
	{
		location.href = '<?= site_url("indicador_plugin/comunicacao_nr_acesso_site/excluir/".$row["cd_comunicacao_nr_acesso_site"]) ?>';
	}
...
echo aba_start($abas);
	echo form_open('indicador_plugin/comunicacao_nr_acesso_site/salvar');
		echo form_start_box("default_box", 'Cadastro');
			echo form_default_hidden('cd_indicador_tabela', 'Código indicador tabela', $tabela[0]['cd_indicador_tabela']);
			echo form_default_hidden('cd_comunicacao_nr_acesso_site', 'Código da tabela', intval($row['cd_comunicacao_nr_acesso_site']));
		...
		echo form_command_bar_detail_start();
			...
			if(intval($row['cd_comunicacao_nr_acesso_site']) > 0)
			{
