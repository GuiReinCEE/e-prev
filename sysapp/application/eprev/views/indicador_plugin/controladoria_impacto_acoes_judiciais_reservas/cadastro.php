...
<script>
	<?php 
	echo form_default_js_submit(array("ano_referencia", "cd_indicador_tabela", "nr_valor_1", "nr_valor_2", "nr_meta"),"_salvar(form)");?>
	...
	function ir_lista()
	{
		location.href = "<?= site_url("indicador_plugin/controladoria_impacto_acoes_judiciais_reservas") ?>";
	}
	...
	function excluir()
	{
		location.href = "<?= site_url("indicador_plugin/controladoria_impacto_acoes_judiciais_reservas/excluir/".$row["cd_controladoria_impacto_acoes_judiciais_reservas"]) ?>";
	}
...
<? else: ?>
<?= form_open('indicador_plugin/controladoria_impacto_acoes_judiciais_reservas/salvar')?>
...
<?= form_end_box('indicador_box') ?>
		...
		<?= form_start_box('default_box', 'Cadastro')?>
			...
			<?= form_default_hidden('cd_controladoria_impacto_acoes_judiciais_reservas', 'Código da tabela', intval($row['cd_controladoria_impacto_acoes_judiciais_reservas']))?>
			<?= form_default_row('','','')?>
		...
		<?= form_command_bar_detail_start()?>
			<?= button_save()?>
			<?if(intval($row['cd_controladoria_impacto_acoes_judiciais_reservas']) > 0) ?>
