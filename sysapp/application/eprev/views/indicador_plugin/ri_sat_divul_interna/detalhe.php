...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/ri_sat_divul_interna"); ?>';
	}
    function manutencao()
    {
...
echo form_open( 'indicador_plugin/ri_sat_divul_interna/salvar' );
echo form_hidden( 'cd_ri_sat_divul_interna', intval($row['cd_ri_sat_divul_interna']) );
...
if( intval($row['cd_ri_sat_divul_interna'])>0  )
{
	echo button_delete("indicador_plugin/ri_sat_divul_interna/excluir",$row["cd_ri_sat_divul_interna"]);
}
