<?php
    ...
    foreach($collection as $item)
	{
		...
		{
			$link = anchor('indicador_plugin/juridico_sucesso_acoes_castro_barcellos_civel_mensal/cadastro/'.$item['cd_juridico_sucesso_acoes_castro_barcellos_civel_mensal'], 'editar');
    ...
    if($contador_ano_atual > 0)
    {
		...
		//STATUS RESULTADO ANO PARCIAL
			//META
		if(floatval($pr_parcial) <= floatval($nr_parcial_min_total))
		{
			$fl_meta_parcial_ref = 'S';
		}
		else if(floatval($pr_parcial) >= floatval($nr_parcial_min_total))
		{
		...
		//STATUS RESULTADO ANO PROCEDE
			//META
		if(floatval($pr_procede) <= floatval($nr_proc_min_total))
		{
			$fl_meta_proc_ref = 'S';
		}
		else if(floatval($pr_procede) >= floatval($nr_proc_min_total))
		{
			$fl_meta_proc_ref = 'N';
		}
		else
