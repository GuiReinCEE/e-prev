<?php
    ...
    foreach ($collection as $item)
    {
		...
		{
			$link = anchor('indicador_plugin/atendimento_retencao_cliente_valores/cadastro/'.$item['cd_atendimento_retencao_cliente_valores'], '[editar]');
        ...
        $body[] = array(
            ...
            $referencia,
            $item['nr_interesse'],
            number_format($item['nr_cliente'], 2, ',', '.'),
            number_format($item['nr_efetivas'], 2, ',', '.'),
            number_format($item['nr_nao_retido'], 2, ',', '.'),
            number_format($item['nr_negociacao'], 2, ',', '.'),
    ...
    if($contador_ano_atual > 0)
    {
        ...
        $body[] = array(
			...
			'<b>'.number_format($nr_total_cliente, 2, ',', '.').'</b>',
            '<b>'.number_format($nr_total_efetivas, 2, ',', '.').'</b>',
            '<b>'.number_format($nr_total_nao_retido, 2, ',', '.').'</b>',
            '<b>'.number_format($nr_total_negociacao, 2, ',', '.').'</b>',
