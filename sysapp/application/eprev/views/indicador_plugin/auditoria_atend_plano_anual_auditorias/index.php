...
<script>
	function filtrar()
	{
		...
		$.post('<?= site_url('indicador_plugin/auditoria_atend_plano_anual_auditorias/listar') ?>',
		function(data)
		{ 
	...
	{
		if($("#contador_input").val() != "3")
		{
			alert("Falta algum quadrimestre.");
		}
		else if(confirm("Fechar o período?"))
		{
			$.post("<?= site_url('indicador_plugin/auditoria_atend_plano_anual_auditorias/criar_indicador') ?>", 
			function(data)
			{
				$("#div-output").html("Indicadores atualizados com sucesso, aguarde enquanto o período é fechado ...");

				location.href = '<?= site_url("indicador_plugin/auditoria_atend_plano_anual_auditorias/fechar_periodo") ?>';
			});
		}
	}
	...
	function novo()
	{
		location.href = '<?= site_url("indicador_plugin/auditoria_atend_plano_anual_auditorias/cadastro") ?>';
	}
...
$abas[] = array('aba_cadastro', 'Lançamento', true, 'location.reload();');
