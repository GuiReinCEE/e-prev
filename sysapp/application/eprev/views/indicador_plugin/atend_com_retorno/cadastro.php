...
<script>
	...
	function ir_lista()
	{
		location.href = '<?= site_url("indicador_plugin/atend_com_retorno") ?>';
	}
	...
	function excluir()
	{
		location.href = '<?= site_url("indicador_plugin/atend_com_retorno/excluir/".$row["cd_atend_com_retorno"]) ?>';
	}
...
echo aba_start($abas);
	echo form_open('indicador_plugin/atend_com_retorno/salvar');
		echo form_start_box("default_box", 'Cadastro');
			echo form_default_hidden('cd_indicador_tabela', 'Código indicador tabela', $tabela[0]['cd_indicador_tabela']);
			echo form_default_hidden('cd_atend_com_retorno', 'Código da tabela', intval($row['cd_atend_com_retorno']));
		...
		echo form_command_bar_detail_start();
			...
			if(intval($row['cd_atend_com_retorno']) > 0)
			{
