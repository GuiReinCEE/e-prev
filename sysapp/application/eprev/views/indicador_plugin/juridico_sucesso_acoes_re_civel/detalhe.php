...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/juridico_sucesso_acoes_re_civel"); ?>';
	}
	...
	function excluir()
	{
		location.href = '<?php echo site_url("indicador_plugin/juridico_sucesso_acoes_re_civel/excluir/".$row["cd_juridico_sucesso_acoes_re_civel"]); ?>';
	}
	...
	function getValores()
	{
		if(($("#mes_referencia").val() != "") && ($("#ano_referencia").val() != ""))
		{
			...
			$.post('<?php echo site_url("indicador_plugin/juridico_sucesso_acoes_re_civel/get_valores"); ?>', 
			{
...
echo aba_start($abas);
	echo form_open('indicador_plugin/juridico_sucesso_acoes_re_civel/salvar');
		echo form_start_box("default_box", 'Cadastro');
			...
			echo form_default_hidden('cd_juridico_sucesso_acoes_re_civel', 'Código da tabela', intval($row['cd_juridico_sucesso_acoes_re_civel']));
		...
		echo form_command_bar_detail_start();
			...
			if(intval($row['cd_juridico_sucesso_acoes_re_civel']) > 0)
			{
