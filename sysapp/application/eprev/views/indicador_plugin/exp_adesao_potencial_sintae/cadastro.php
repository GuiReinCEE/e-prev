...
<script>
	...
	function ir_lista()
	{
		location.href = '<?= site_url("indicador_plugin/exp_adesao_potencial_sintae") ?>';
	}
	...
	function excluir()
	{
		location.href = '<?= site_url("indicador_plugin/exp_adesao_potencial_sintae/excluir/".$row["cd_exp_adesao_potencial_sintae"]) ?>';
	}
...
echo aba_start($abas);
	echo form_open('indicador_plugin/exp_adesao_potencial_sintae/salvar');
		echo form_start_box("default_box", 'Cadastro');
			echo form_default_hidden('cd_indicador_tabela', 'Código indicador tabela', $tabela[0]['cd_indicador_tabela']);
			echo form_default_hidden('cd_exp_adesao_potencial_sintae', 'Código da tabela', intval($row['cd_exp_adesao_potencial_sintae']));
		...
		echo form_command_bar_detail_start();
			...
			if(intval($row['cd_exp_adesao_potencial_sintae']) > 0)
			{
