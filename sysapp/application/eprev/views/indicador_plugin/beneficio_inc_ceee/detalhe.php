...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/beneficio_inc_ceee"); ?>';
	}
    function manutencao()
{
...
echo form_open( 'indicador_plugin/beneficio_inc_ceee/salvar' );
echo form_hidden( 'cd_beneficio_inc_ceee', intval($row['cd_beneficio_inc_ceee']) );
...
if( intval($row['cd_beneficio_inc_ceee'])>0  )
{
	echo button_delete("indicador_plugin/beneficio_inc_ceee/excluir",$row["cd_beneficio_inc_ceee"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/beneficio_inc_ceee')."'; }");
