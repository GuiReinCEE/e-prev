...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/beneficio_inc_ceape"); ?>';
	}
    function manutencao()
    {
...
echo form_open( 'indicador_plugin/beneficio_inc_ceape/salvar' );
echo form_hidden( 'cd_beneficio_inc_ceape', intval($row['cd_beneficio_inc_ceape']) );
...
if( intval($row['cd_beneficio_inc_ceape'])>0  )
{
	echo button_delete("indicador_plugin/beneficio_inc_ceape/excluir",$row["cd_beneficio_inc_ceape"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/beneficio_inc_ceape')."'; }");
