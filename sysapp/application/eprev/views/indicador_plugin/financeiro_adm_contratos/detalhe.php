...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/financeiro_adm_contratos"); ?>';
	}
    function manutencao()
    {
...
echo form_open( 'indicador_plugin/financeiro_adm_contratos/salvar' );
echo form_hidden( 'cd_financeiro_adm_contratos', intval($row['cd_financeiro_adm_contratos']) );
...
if( intval($row['cd_financeiro_adm_contratos'])>0  )
{
	echo button_delete("indicador_plugin/financeiro_adm_contratos/excluir",$row["cd_financeiro_adm_contratos"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/financeiro_adm_contratos')."'; }");
