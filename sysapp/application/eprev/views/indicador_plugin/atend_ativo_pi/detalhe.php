...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/atend_ativo_pi"); ?>';
	}
...
echo form_open( 'indicador_plugin/atend_ativo_pi/salvar' );
echo form_hidden( 'cd_atend_ativo_pi', intval($row['cd_atend_ativo_pi']) );
...
echo form_default_float("nr_valor_ceee", $label_1, app_decimal_para_php($row['nr_valor_ceee']), "class='indicador_text'"); 
#echo form_default_float("nr_valor_aes", $label_2, app_decimal_para_php($row['nr_valor_aes']), "class='indicador_text'"); 
echo form_default_float("nr_valor_cgtee", $label_3, app_decimal_para_php($row['nr_valor_cgtee']), "class='indicador_text'"); 
echo form_default_float("nr_valor_crm", $label_4, app_decimal_para_php($row['nr_valor_crm']), "class='indicador_text'");
...
if( intval($row['cd_atend_ativo_pi'])>0  )
{
	echo button_delete("indicador_plugin/atend_ativo_pi/excluir",$row["cd_atend_ativo_pi"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/atend_ativo_pi')."'; }");
