...
<script>
	function filtrar()
	{
		...
		$.post('<?= site_url('indicador_plugin/investimento_rentabilidade_rge/listar') ?>',
		function(data)
		{ 
	...
	function fechar_periodo()
	{
		...
		{
			$.post('<?= site_url("indicador_plugin/investimento_rentabilidade_rge/criar_indicador") ?>',
			function(data)
			{
				$('#output_tela').html("Indicadores atualizados com sucesso, aguarde enquanto o período é fechado ..." );

				location.href = '<?= site_url("indicador_plugin/investimento_rentabilidade_rge/fechar_periodo") ?>';
			});
		}
	}
	...
	function novo()
	{
		location.href = '<?= site_url("indicador_plugin/investimento_rentabilidade_rge/cadastro") ?>';
	}
