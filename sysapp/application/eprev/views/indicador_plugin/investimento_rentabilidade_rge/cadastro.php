...
<script>
	...
	function ir_lista()
	{
		location.href = '<?= site_url("indicador_plugin/investimento_rentabilidade_rge") ?>';
	}
	...
	function excluir()
	{
		location.href = '<?= site_url("indicador_plugin/investimento_rentabilidade_rge/excluir/".$row["cd_investimento_rentabilidade_rge"]) ?>';
	}
...
echo aba_start($abas);
	echo form_open('indicador_plugin/investimento_rentabilidade_rge/salvar');
		echo form_start_box("default_box", 'Cadastro');
			echo form_default_hidden('cd_indicador_tabela', 'Código indicador tabela', $tabela[0]['cd_indicador_tabela']);
			echo form_default_hidden('cd_investimento_rentabilidade_rge', 'Código da tabela', intval($row['cd_investimento_rentabilidade_rge']));
		...
		echo form_command_bar_detail_start();
			...
			if(intval($row['cd_investimento_rentabilidade_rge']) > 0)
			{
