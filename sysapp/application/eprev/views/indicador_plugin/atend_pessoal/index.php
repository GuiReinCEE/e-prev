...
function load()
{
	...
	$.post( '<?php echo base_url().index_page(); ?>/indicador_plugin/atend_pessoal/listar',{},function(data){ $("#result_div").html(data);configure_result_table(); } );
}
...
function novo()
{
	location.href='<?php echo site_url("indicador_plugin/atend_pessoal/detalhe/0"); ?>';
}
...
function gerar_grafico()
{
	...
	{
		url = '<?php echo site_url("indicador_plugin/atend_pessoal/criar_indicador/"); ?>';
...
function fechar_periodo()
{
	...
	{
		$.post("<?= site_url('indicador_plugin/atend_pessoal/criar_indicador') ?>", 
		function(data)
		{
			$("#div-output").html("Indicadores atualizados com sucesso, aguarde enquanto o período é fechado ...");

			location.href = "<?= site_url('indicador_plugin/atend_pessoal/fechar_periodo') ?>";
		});
	}
}
