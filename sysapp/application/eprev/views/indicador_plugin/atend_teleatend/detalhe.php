...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/atend_teleatend"); ?>';
	}
...
echo form_open( 'indicador_plugin/atend_teleatend/salvar' );
echo form_hidden( 'cd_atend_teleatend', intval($row['cd_atend_teleatend']) );
...
if( intval($row['cd_atend_teleatend'])>0  )
{
	echo button_delete("indicador_plugin/atend_teleatend/excluir",$row["cd_atend_teleatend"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/atend_teleatend')."'; }");
