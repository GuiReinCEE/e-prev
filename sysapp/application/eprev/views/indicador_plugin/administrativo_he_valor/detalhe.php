...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/administrativo_he_valor"); ?>';
	}
...
echo form_open( 'indicador_plugin/administrativo_he_valor/salvar' );
echo form_hidden( 'cd_administrativo_he_valor', intval($row['cd_administrativo_he_valor']) );
...
echo form_default_float("nr_referencial", "Referencial ", app_decimal_para_php($row['nr_referencial']), "class='indicador_text'"); 
...
if( intval($row['cd_administrativo_he_valor'])>0  )
{
	echo button_delete("indicador_plugin/administrativo_he_valor/excluir",$row["cd_administrativo_he_valor"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/administrativo_he_valor')."'; }");
