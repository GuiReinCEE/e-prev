...
<script>
	<?php 
	echo form_default_js_submit(array("mes_referencia", "ano_referencia", "cd_indicador_tabela", "nr_contratado", "nr_ingresso"),'_salvar(form)');	?>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/exp_valor_contratado_media"); ?>';
	}
	...
	function excluir()
	{
		location.href = '<?php echo site_url("indicador_plugin/exp_valor_contratado_media/excluir/".$row["cd_exp_valor_contratado_media"]); ?>';
	}
...
echo aba_start($abas);
	echo form_open('indicador_plugin/exp_valor_contratado_media/salvar');
		echo form_start_box("default_box", 'Cadastro');
			echo form_default_hidden('cd_indicador_tabela', 'Código indicador tabela', $tabela[0]['cd_indicador_tabela']);
			echo form_default_hidden('cd_exp_valor_contratado_media', 'Código da tabela', intval($row['cd_exp_valor_contratado_media']));
			...
			echo form_default_numeric("nr_contratado", $label_1.": (*)", number_format($row['nr_contratado'],2,',','.'), "class='indicador_text'");
			echo form_default_integer("nr_ingresso", $label_2.": (*)", intval($row['nr_ingresso']), "class='indicador_text'");			
		...
		echo form_command_bar_detail_start();
			...
			if(intval($row['cd_exp_valor_contratado_media']) > 0)
			{
