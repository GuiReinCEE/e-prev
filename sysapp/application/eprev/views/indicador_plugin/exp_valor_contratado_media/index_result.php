...
foreach($collection as $item)
{
	...
	{
		$link = anchor("indicador_plugin/exp_valor_contratado_media/cadastro/".$item["cd_exp_valor_contratado_media"], "editar");
	...
	$body[] = array(
		...
		(trim($nr_ingresso) != '' ? number_format($nr_ingresso, 0, ',', '.') : ''),
		(trim($nr_contratado_media) != '' ? number_format($nr_contratado_media, 2, ',', '.') : ''),
...
if($contador_ano_atual > 0)
{
	$body[] = array(
		...
		'<big><b>'.(trim($nr_ingresso_total) != '' ? number_format($nr_ingresso_total, 2, ',', '.') : '').'</b></big>',
		'<big><b>'.number_format(floatval($nr_contratado_total / $nr_ingresso_total), 2, ',', '.').'</b></big>',
		'<big><b>'.(trim($nr_meta) != '' ? number_format($nr_meta, 2, ',', '.') : '').'</b></big>',
		'',
		''
	);
}
