...
<script>
	<?= form_default_js_submit(array('cd_indicador_tabela', 'mes_referencia', 'ano_referencia', 'nr_orcado', 'nr_realizado', 'nr_meta'), 'validacao(form);') ?>
    ...
    function ir_lista()
	{
		location.href = "<?= site_url('indicador_plugin/administrativo_horas_ext_realizado_orcado') ?>";
	}
	...
	function excluir()
	{
		...
		{
			location.href = "<?= site_url('indicador_plugin/administrativo_horas_ext_realizado_orcado/excluir/'.$row['cd_administrativo_horas_ext_realizado_orcado']) ?>";
		}
	}
...
<? else: ?>
	...
	<?= form_open('indicador_plugin/administrativo_horas_ext_realizado_orcado/salvar') ?>
		...
		<?= form_start_box('default_box', 'Cadastro') ?>
			...
			<?= form_default_hidden('cd_administrativo_horas_ext_realizado_orcado', '', intval($row['cd_administrativo_horas_ext_realizado_orcado'])) ?>
			...
			<?= form_default_numeric('nr_meta', $label_7.' (*) :', number_format($row['nr_meta'], 2, ',', '.'), 'class="indicador_text"') ?>
		...
		<?= form_command_bar_detail_start() ?>
			...
			<? if(intval($row['cd_administrativo_horas_ext_realizado_orcado']) > 0) :?>	
