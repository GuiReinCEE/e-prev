...
<script>
    ...
    {
        location.href = '<?= site_url("indicador_plugin/juridico_sucesso_acoes_consolidado_civel_mensal/index") ?>';
    }
    ...
    function excluir()
    {
        location.href = '<?= site_url("indicador_plugin/juridico_sucesso_acoes_consolidado_civel_mensal/excluir/".$row['cd_juridico_sucesso_acoes_consolidado_civel_mensal']) ?>';
    }
    ...
    function getValores()
	{
		if(($("#mes_referencia").val() != "") && ($("#ano_referencia").val() != ""))
		{
			...
			$.post('<?php echo site_url("indicador_plugin/juridico_sucesso_acoes_consolidado_civel_mensal/get_valores"); ?>', 
			{
			...
			function(data)
			{
				if(data)
				{ 
                    $("#nr_inicial").val(data.nr_inicial);
                    
					$("#nr_improcede_1").val(data.nr_improcede_1);
					$("#nr_parcial_1").val(data.nr_parcial_1);
                    $("#nr_procede_1").val(data.nr_procede_1);
                    
					$("#nr_improcede_2").val(data.nr_improcede_2);
					$("#nr_parcial_2").val(data.nr_parcial_2);
                    $("#nr_procede_2").val(data.nr_procede_2);
                    
					$("#nr_improcede_3").val(data.nr_improcede_3);
					$("#nr_parcial_3").val(data.nr_parcial_3);
					$("#nr_procede_3").val(data.nr_procede_3);
...
<?php
        ...
        echo form_open('indicador_plugin/juridico_sucesso_acoes_consolidado_civel_mensal/salvar');
        echo form_start_box('cadastro_box', 'Cadastro');
            echo form_default_hidden('cd_juridico_sucesso_acoes_consolidado_civel_mensal', '', $row);
        ...
        if(intval($row['cd_juridico_sucesso_acoes_consolidado_civel_mensal']) > 0) : 
