<?php
$body = array();
$head = array( 
    "#", $label_0, $label_1, $label_2, $label_3, $label_4, $label_5, $label_6, $label_7, $label_8, $label_9, ''
);

$ar_janela = array(
    'width'      => '700',
    'height'     => '500',
    'scrollbars' => 'yes',
    'status'     => 'yes',
    'resizable'  => 'yes',
    'screenx'    => '0',
    'screeny'    => '0'
);

$contador_ano_atual = 0;
$contador           = sizeof($collection);
$ultimo_mes         = 0;

$referencia = '';

$nr_valor_1 = 0;
$nr_valor_2 = 0;
$nr_valor_3 = 0; 
$nr_valor_4 = 0;
$nr_valor_5 = 0;
$nr_valor_6 = 0;
$nr_meta    = 0;

foreach($collection as $key => $item)
{
    if(trim($item['fl_media']) == 'S')
    {
        $referencia = 'Resultado de '.intval($item['ano_referencia']);

        $link = '';
    }
    else
    {
        $referencia = $item['mes_ano_referencia'];

        $link = anchor('indicador_plugin/administrativo_resul_superior/cadastro/'.$item['cd_administrativo_resul_superior'], '[editar]');
    }

    if((intval($item['ano_referencia']) == intval($tabela[0]['nr_ano_referencia'])) && (trim($item['fl_media']) != 'S'))
    {
        $contador_ano_atual++;

        $ultimo_mes = $item['mes_referencia'];

        $nr_valor_1 += $item['nr_valor_1'];
        $nr_valor_2 += $item['nr_valor_2'];
        $nr_valor_3 += $item['nr_valor_3']; 
        $nr_meta    = $item['nr_meta']; 
    }

    $body[] = array(
        $contador--,
        $referencia,
        number_format($item['nr_valor_1'], 0, ',', '.'),
        number_format($item['nr_valor_4'], 2, ',', '.'). '%',
        number_format($item['nr_valor_2'], 0, ',', '.'),
        number_format($item['nr_valor_5'], 2, ',', '.'). '%',
        number_format($item['nr_valor_3'], 0, ',', '.'),
        number_format($item['nr_valor_6'], 2, ',', '.'). '%',
        number_format($item['nr_percentual_f'], 0, ',', '.'),
        number_format($item['nr_meta'], 2, ',', '.'). '%',
        array(nl2br($item['observacao']), 'text-align:left'),
        $link 
    );
}

if($contador_ano_atual > 0)
{
    $nr_percentual_f = $nr_valor_1 + $nr_valor_2 + $nr_valor_3;

    if($nr_percentual_f > 0)
    {
        $nr_valor_4 = ($nr_valor_1 / $nr_percentual_f) * 100;
        $nr_valor_5 = ($nr_valor_2 / $nr_percentual_f) * 100;
        $nr_valor_6 = ($nr_valor_3 / $nr_percentual_f) * 100;
    }

    $body[] = array(
        0,
        '<b>Resultado de '.intval($tabela[0]['nr_ano_referencia']).'</b>',
        number_format($nr_valor_1, 0, ',', '.'),
        number_format($nr_valor_4, 2, ',', '.'). '%',
        number_format($nr_valor_2, 0, ',', '.'),
        number_format($nr_valor_5, 2, ',', '.'). '%',
        number_format($nr_valor_3, 0, ',', '.'),
        number_format($nr_valor_6, 2, ',', '.'). '%',
        number_format($nr_percentual_f, 0, ',', '.'),
        number_format($nr_meta, 2, ',', '.'). '%',
        '',
        ''
    );
}

$this->load->helper('grid');
$grid = new grid();
$grid->head = $head;
$grid->body = $body;
$grid->view_count = false;
?>

<input type="hidden" id="ultimo_mes" name="ultimo_mes" value="<?= $ultimo_mes ?>"/>
<input type="hidden" id="contador" name="contador" value="<?= $contador_ano_atual ?>"/>
<br/>
<div style="text-align:center;"> 
    <?= anchor_popup('indicador/apresentacao/detalhe/'.intval($tabela[0]['cd_indicador_tabela']), '[Visualizar Apresentação]', $ar_janela) ?>
</div>
<br/>
<?= $grid->render() ?>