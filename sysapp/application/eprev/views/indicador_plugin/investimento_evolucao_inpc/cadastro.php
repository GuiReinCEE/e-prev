...
<script>
	<?php 
	echo form_default_js_submit(array("mes_referencia", "ano_referencia", "cd_indicador_tabela", "nr_valor_1", "nr_valor_2"),'_salvar(form)');	?>
	...
	function ir_lista()
	{
		location.href = '<?= site_url("indicador_plugin/investimento_evolucao_inpc") ?>';
	}
	...
	function excluir()
	{
		location.href = '<?= site_url("indicador_plugin/investimento_evolucao_inpc/excluir/".$row["cd_investimento_evolucao_inpc"]) ?>';
	}
	...
	function get_valores()
	{
		if(($("#mes_referencia").val() != "") && ($("#ano_referencia").val() != ""))
		{
			...
			$.post("<?= site_url('indicador_plugin/investimento_evolucao_inpc/get_valores') ?>", 
			{
...
echo aba_start($abas);
	echo form_open('indicador_plugin/investimento_evolucao_inpc/salvar');
		echo form_start_box("default_box", 'Cadastro');
			echo form_default_hidden('cd_indicador_tabela', 'Código indicador tabela', $tabela[0]['cd_indicador_tabela']);
			echo form_default_hidden('cd_investimento_evolucao_inpc', 'Código da tabela', intval($row['cd_investimento_evolucao_inpc']));
			...
			echo form_default_numeric("nr_valor_1", $label_2.' :', number_format($row['nr_valor_1'],4,",","."), "class='indicador_text'", array("centsLimit" => 4)); 
			echo form_default_numeric("nr_valor_2", $label_1.' :', number_format($row['nr_valor_2'],4,",","."), "class='indicador_text'", array("centsLimit" => 4));
		...
		echo form_command_bar_detail_start();
			...
			if(intval($row['cd_investimento_evolucao_inpc']) > 0)
			{
