...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/administrativo_suprimento"); ?>';
	}
...
echo form_open( 'indicador_plugin/administrativo_suprimento/salvar' );
echo form_hidden( 'cd_administrativo_suprimento', intval($row['cd_administrativo_suprimento']) );
...
if( intval($row['cd_administrativo_suprimento'])>0  )
{
	echo button_delete("indicador_plugin/administrativo_suprimento/excluir",$row["cd_administrativo_suprimento"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/administrativo_suprimento')."'; }");
