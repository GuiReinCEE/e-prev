...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/secretaria_rd"); ?>';
	}
    function manutencao()
    {
...
echo form_open( 'indicador_plugin/secretaria_rd/salvar' );
echo form_hidden( 'cd_secretaria_rd', intval($row['cd_secretaria_rd']) );
...
if( intval($row['cd_secretaria_rd'])>0  )
{
	echo button_delete("indicador_plugin/secretaria_rd/excluir",$row["cd_secretaria_rd"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/secretaria_rd')."'; }");
