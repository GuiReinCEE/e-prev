...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/beneficio_inc_sinpro"); ?>';
	}
    function manutencao()
{
...
echo form_open( 'indicador_plugin/beneficio_inc_sinpro/salvar' );
echo form_hidden( 'cd_beneficio_inc_sinpro', intval($row['cd_beneficio_inc_sinpro']) );
...
if( intval($row['cd_beneficio_inc_sinpro'])>0  )
{
	echo button_delete("indicador_plugin/beneficio_inc_sinpro/excluir",$row["cd_beneficio_inc_sinpro"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/beneficio_inc_sinpro')."'; }");
