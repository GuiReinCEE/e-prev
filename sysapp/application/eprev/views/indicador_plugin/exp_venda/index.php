...
<script>
	function filtrar()
	{
		...
		$.post('<?php echo site_url('indicador_plugin/exp_venda/listar'); ?>',
		function(data)
		{ 
	...
	function fechar_periodo()
	{
		...
		{
			$.post('<?php echo site_url("indicador_plugin/exp_venda/criar_indicador/"); ?>',
			function(data)
			{
				$('#output_tela').html("Indicadores atualizados com sucesso, aguarde enquanto o período é fechado ..." );

				location.href = '<?php echo site_url("indicador_plugin/exp_venda/fechar_periodo")?>';
			});
		}
	}
	...
	function novo()
	{
		location.href='<?php echo site_url("indicador_plugin/exp_venda/cadastro"); ?>';
	}
