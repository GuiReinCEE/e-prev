...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/financeiro_execucao_participante"); ?>';
	}
    function manutencao()
    {
...
echo form_open( 'indicador_plugin/financeiro_execucao_participante/salvar' );
echo form_hidden( 'cd_financeiro_execucao_participante', intval($row['cd_financeiro_execucao_participante']) );
...
if( intval($row['cd_financeiro_execucao_participante'])>0  )
{
	echo button_delete("indicador_plugin/financeiro_execucao_participante/excluir",$row["cd_financeiro_execucao_participante"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/financeiro_execucao_participante')."'; }");
