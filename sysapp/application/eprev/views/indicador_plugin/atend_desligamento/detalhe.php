...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/atend_desligamento"); ?>';
	}
...
echo form_open( 'indicador_plugin/atend_desligamento/salvar' );
echo form_hidden( 'cd_atend_desligamento', intval($row['cd_atend_desligamento']) );
...
if( intval($row['cd_atend_desligamento'])>0  )
{
	echo button_delete("indicador_plugin/atend_desligamento/excluir",$row["cd_atend_desligamento"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/atend_desligamento')."'; }");
