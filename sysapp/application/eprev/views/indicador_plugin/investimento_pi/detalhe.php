...
set_title('Atendimento - Total de Participantes');
...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/investimento_pi"); ?>';
	}
    function manutencao()
    {
...
echo form_open( 'indicador_plugin/investimento_pi/salvar' );
echo form_hidden( 'cd_investimento_pi', intval($row['cd_investimento_pi']) );

// Registros da tabela principal ...
echo form_start_box( "default_box", "Atendimento - Total de Participantes" );
...
if( intval($row['cd_investimento_pi'])>0  )
{
	echo button_delete("indicador_plugin/investimento_pi/excluir",$row["cd_investimento_pi"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/investimento_pi')."'; }");
