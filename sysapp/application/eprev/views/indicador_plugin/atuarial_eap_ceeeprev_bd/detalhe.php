...
set_title('Atuarial - Equilíbrio Atuarial Consolidado');
...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/atuarial_eap_ceeeprev_bd"); ?>';
	}
...
echo form_open( 'indicador_plugin/atuarial_eap_ceeeprev_bd/salvar' );
echo form_hidden( 'cd_atuarial_eap_ceeeprev_bd', intval($row['cd_atuarial_eap_ceeeprev_bd']) );

// Registros da tabela principal ...
echo form_start_box( "default_box", "Atuarial - Equilíbrio Atuarial Consolidado" );
...
echo form_default_float("nr_reserva_tecnica", $label_1, app_decimal_para_php($row['nr_reserva_tecnica']), "class='indicador_text'"); 
echo form_default_float("nr_provisao_matematica", $label_2, app_decimal_para_php($row['nr_provisao_matematica']), "class='indicador_text'"); 
...
if( intval($row['cd_atuarial_eap_ceeeprev_bd'])>0  )
{
	echo button_delete("indicador_plugin/atuarial_eap_ceeeprev_bd/excluir",$row["cd_atuarial_eap_ceeeprev_bd"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/atuarial_eap_ceeeprev_bd')."'; }");
echo form_command_bar_detail_end();
?>
<script>
	$('#nr_reserva_tecnica').focus();
</script>
<?php
echo aba_end();
echo form_close();
