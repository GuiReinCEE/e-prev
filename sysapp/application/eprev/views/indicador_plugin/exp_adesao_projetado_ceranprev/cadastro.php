...
<script>
	...
	function ir_lista()
	{
		location.href = '<?= site_url("indicador_plugin/exp_adesao_projetado_ceranprev") ?>';
	}
	...
	function excluir()
	{
		location.href = '<?= site_url("indicador_plugin/exp_adesao_projetado_ceranprev/excluir/".$row["cd_exp_adesao_projetado_ceranprev"]) ?>';
	}
...
<? else: ?>
	<?= form_open('indicador_plugin/exp_adesao_projetado_ceranprev/salvar')?>
		<?= form_start_box("default_box", 'Indicador')?>
			...
			<?= form_default_hidden('cd_exp_adesao_projetado_ceranprev', 'Código da tabela', intval($row['cd_exp_adesao_projetado_ceranprev']))?>
		...
		<?= form_command_bar_detail_start()?>
			...
			<? if(intval($row['cd_exp_adesao_projetado_ceranprev']) > 0) : ?>
