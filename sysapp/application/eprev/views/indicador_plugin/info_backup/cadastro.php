...
<script>
	...
	<?= form_default_js_submit(array('cd_indicador_tabela', 'mes_referencia', 'ano_referencia', 'nr_meta'), 'validacao(form);') ?>
	...
	function ir_lista()
	{
		location.href = "<?= site_url('indicador_plugin/info_backup') ?>";
	}
	...
	function excluir()
	{
		...
		{
			location.href = "<?= site_url('indicador_plugin/info_backup/excluir/'.$row['cd_info_backup']) ?>";
		}
	}
...
<? else: ?>
	...
	<?= form_open('indicador_plugin/info_backup/salvar') ?>
		...
		<?= form_start_box('default_box', 'Cadastro') ?>
			...
			<?= form_default_hidden('cd_info_backup', '', intval($row['cd_info_backup'])) ?>
			...
			<?= form_default_numeric('nr_soma', $label_1.': ', number_format($row['nr_soma'], 2, ',', '.'), "class='indicador_text'") ?>
			<?= form_default_integer('nr_processo', $label_2.': ', $row['nr_processo'], "class='indicador_text'") ?>
			<?= form_default_numeric('nr_percentual', $label_3.': ', number_format($row['nr_percentual'], 2, ',', '.'), "class='indicador_text'") ?>
