...
<script>
	function filtrar()
	{
		...
		$.post('<?= site_url('indicador_plugin/secretaria_sat_workshop_dirigente/listar') ?>',
		function(data)
		{ 
	...
	function atualizar_grafico()
	{
		...
		{
			$.post("<?= site_url('indicador_plugin/secretaria_sat_workshop_dirigente/criar_indicador') ?>", 
			function(data)
			{ 
	...
	function novo()
	{
		location.href = '<?= site_url("indicador_plugin/secretaria_sat_workshop_dirigente/cadastro") ?>';
	}
