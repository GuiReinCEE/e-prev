...
<script>
	<?php 
	echo form_default_js_submit(array("ano_referencia", "cd_indicador_tabela", "nr_percentual_f", "nr_meta"),'_salvar(form)');	?>
	...
	function ir_lista()
	{
		location.href = '<?= site_url("indicador_plugin/exp_adesao_potencial") ?>';
	}
	...
	function excluir()
	{
		location.href = '<?= site_url("indicador_plugin/exp_adesao_potencial/excluir/".$row["cd_exp_adesao_potencial"]) ?>';
	}
...
echo aba_start($abas);
	echo form_open('indicador_plugin/exp_adesao_potencial/salvar');
		echo form_start_box("default_box", 'Cadastro');
			echo form_default_hidden('cd_indicador_tabela', 'Código indicador tabela', $tabela[0]['cd_indicador_tabela']);
			echo form_default_hidden('cd_exp_adesao_potencial', 'Código da tabela', intval($row['cd_exp_adesao_potencial']));
			...
			echo form_default_numeric("nr_meta", $label_2.' :', number_format(100,2,",","."), "readonly");
			echo form_default_textarea("observacao", $label_3.":", $row['observacao'], "style='height: 80px;'");
		echo form_end_box("default_box");
		echo form_command_bar_detail_start();
			echo button_save();

			if(intval($row['cd_exp_adesao_potencial']) > 0)
			{
