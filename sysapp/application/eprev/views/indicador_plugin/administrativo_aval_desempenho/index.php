...
<script>
	function filtrar()
	{
		...
		$.post('<?= site_url('indicador_plugin/administrativo_aval_desempenho/listar') ?>',
		function(data)
		{ 
	...
	function atualizar_grafico()
	{
		...
		{
			$.post("<?= site_url('indicador_plugin/administrativo_aval_desempenho/criar_indicador') ?>", 
			function(data)
			{ 
	...
	function novo()
	{
		location.href = '<?= site_url("indicador_plugin/administrativo_aval_desempenho/cadastro/0") ?>';
	}
