...
<script>
	...
	function ir_lista()
	{
		location.href = "<?= site_url("indicador_plugin/ri_sat_participante") ?>";
	}
	...
	function excluir()
	{
		location.href = "<?= site_url("indicador_plugin/ri_sat_participante/excluir/".$row["cd_ri_sat_participante"]) ?>";
	}
...
<?= form_end_box('indicador_box') ?>
	<?= form_open('indicador_plugin/ri_sat_participante/salvar')?>
		<?= form_start_box('default_box', 'Cadastro')?>
			...
			<?= form_default_hidden('cd_ri_sat_participante', 'Código da tabela', intval($row['cd_ri_sat_participante']))?>
			<?= form_default_row('','','')?>
		...
		<?= form_command_bar_detail_start()?>
			<?= button_save()?>
			<? if(intval($row['cd_ri_sat_participante']) > 0): ?>
