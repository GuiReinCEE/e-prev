...
<script>
	function filtrar()
	{
		...
		$.post('<?= site_url('indicador_plugin/ri_sat_participante/listar') ?>',
		function(data)
		{ 
	...
	function atualizar_grafico()
	{
		...
		{
			$.post("<?= site_url('indicador_plugin/ri_sat_participante/criar_indicador') ?>", 
			function(data)
			{ 
	...
	function novo()
	{
		location.href = '<?= site_url("indicador_plugin/ri_sat_participante/cadastro") ?>';
	}
