...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/atend_partic_email"); ?>';
	}
...
echo form_open( 'indicador_plugin/atend_partic_email/salvar' );
echo form_hidden( 'cd_atend_partic_email', intval($row['cd_atend_partic_email']) );
...
if( intval($row['cd_atend_partic_email'])>0  )
{
	echo button_delete("indicador_plugin/atend_partic_email/excluir",$row["cd_atend_partic_email"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/atend_partic_email')."'; }");
