...
<script>
	...
	function ir_lista()
	{
		location.href = '<?= site_url("indicador_plugin/comunicacao_sat_canais") ?>';
	}
	...
	function excluir()
	{
		location.href = '<?= site_url("indicador_plugin/comunicacao_sat_canais/excluir/".$row["cd_comunicacao_sat_canais"]) ?>';
	}
...
echo aba_start($abas);
	echo form_open('indicador_plugin/comunicacao_sat_canais/salvar');
		echo form_start_box("default_box", 'Cadastro');
			echo form_default_hidden('cd_indicador_tabela', 'Código indicador tabela', $tabela[0]['cd_indicador_tabela']);
			echo form_default_hidden('cd_comunicacao_sat_canais', 'Código da tabela', intval($row['cd_comunicacao_sat_canais']));
		...
		echo form_command_bar_detail_start();
			...
			if(intval($row['cd_comunicacao_sat_canais']) > 0)
			{
