...
<script>
	function filtrar()
	{
		...
		$.post('<?= site_url('indicador_plugin/comunicacao_sat_canais/listar') ?>',
		function(data)
		{ 
	...
	function fechar_periodo()
	{
		...
		{
			$.post('<?= site_url("indicador_plugin/comunicacao_sat_canais/criar_indicador") ?>',
			function(data)
			{
				$('#output_tela').html("Indicadores atualizados com sucesso, aguarde enquanto o período é fechado ..." );

				location.href = '<?= site_url("indicador_plugin/comunicacao_sat_canais/fechar_periodo") ?>';
			});
		}
	}
	...
	function novo()
	{
		location.href = '<?= site_url("indicador_plugin/comunicacao_sat_canais/cadastro") ?>';
	}
