...
<script>
	...
	function ir_lista()
	{
		location.href = '<?= site_url("indicador_plugin/financeiro_inadimplencia_participantes") ?>';
	}
	...
	function excluir()
	{
		location.href = '<?= site_url("indicador_plugin/financeiro_inadimplencia_participantes/excluir/".$row["cd_financeiro_inadimplencia_participantes"]) ?>';
	}
...
<? else: ?>
	...
	<?= form_open('indicador_plugin/financeiro_inadimplencia_participantes/salvar') ?>
		...
		<?= form_start_box('default_box', 'Cadastro')?>
			...
			<?= form_default_hidden('cd_financeiro_inadimplencia_participantes', '', intval($row['cd_financeiro_inadimplencia_participantes'])) ?>
		...
		<?= form_command_bar_detail_start()?>
			...
			<? if(intval($row['cd_financeiro_inadimplencia_participantes']) > 0) : ?>
