...
<script>
	...
	function ir_lista()
	{
		location.href = "<?= site_url("indicador_plugin/atuarial_aessul") ?>";
	}
	...
	function excluir()
	{
		location.href = "<?= site_url("indicador_plugin/atuarial_aessul/excluir/".$row["cd_atuarial_aessul"]) ?>";
	}
...
<? else: ?>
<?= form_open('indicador_plugin/atuarial_aessul/salvar') ?>
	...
	<?= form_start_box('default_box', 'Cadastro') ?>
		...
		<?= form_default_hidden('cd_atuarial_aessul', '', intval($row['cd_atuarial_aessul'])) ?>
	...
	<?= form_command_bar_detail_start()?>
		<?= button_save()?>
		<? if(intval($row['cd_atuarial_aessul']) > 0) :?>
