...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/atend_end_desat"); ?>';
	}
...
<? endif; ?>
<?
echo form_open( 'indicador_plugin/atend_end_desat/salvar' );
echo form_hidden( 'cd_atend_end_desat', intval($row['cd_atend_end_desat']) );
...
if( intval($row['cd_atend_end_desat'])>0  )
{
	echo button_delete("indicador_plugin/atend_end_desat/excluir",$row["cd_atend_end_desat"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/atend_end_desat')."'; }");
