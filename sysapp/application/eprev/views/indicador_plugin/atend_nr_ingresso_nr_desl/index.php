...
<script>
	function filtrar()
	{
		...
		$.post('<?= site_url('indicador_plugin/atend_nr_ingresso_nr_desl/listar') ?>',
		function(data)
		{ 
	...
	function fechar_periodo()
	{
		...
		{
			$.post('<?= site_url("indicador_plugin/atend_nr_ingresso_nr_desl/criar_indicador") ?>',
			function(data)
			{
				$('#output_tela').html("Indicadores atualizados com sucesso, aguarde enquanto o período é fechado ..." );

				location.href = '<?= site_url("indicador_plugin/atend_nr_ingresso_nr_desl/fechar_periodo") ?>';
			});
		}
	}
	...
	function novo()
	{
		location.href = '<?= site_url("indicador_plugin/atend_nr_ingresso_nr_desl/cadastro") ?>';
	}
