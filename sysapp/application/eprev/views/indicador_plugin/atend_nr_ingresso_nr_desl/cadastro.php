...
<script>
	...
	function ir_lista()
	{
		location.href = '<?= site_url("indicador_plugin/atend_nr_ingresso_nr_desl") ?>';
	}
	...
	function excluir()
	{
		location.href = '<?= site_url("indicador_plugin/atend_nr_ingresso_nr_desl/excluir/".$row["cd_atend_nr_ingresso_nr_desl"]) ?>';
	}
...
<?
	echo form_open('indicador_plugin/atend_nr_ingresso_nr_desl/salvar');
		echo form_start_box("default_box", 'Cadastro');
			echo form_default_hidden('cd_indicador_tabela', 'Código indicador tabela', $tabela[0]['cd_indicador_tabela']);
			echo form_default_hidden('cd_atend_nr_ingresso_nr_desl', 'Código da tabela', intval($row['cd_atend_nr_ingresso_nr_desl']));
		...
		echo form_command_bar_detail_start();
			...
			if(intval($row['cd_atend_nr_ingresso_nr_desl']) > 0)
			{
