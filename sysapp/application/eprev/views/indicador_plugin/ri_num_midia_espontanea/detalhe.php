...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/ri_num_midia_espontanea"); ?>';
	}
    function manutencao()
    {
...
echo form_open( 'indicador_plugin/ri_num_midia_espontanea/salvar' );
echo form_hidden( 'cd_ri_num_midia_espontanea', intval($row['cd_ri_num_midia_espontanea']) );
...
if( intval($row['cd_ri_num_midia_espontanea'])>0  )
{
	echo button_delete("indicador_plugin/ri_num_midia_espontanea/excluir",$row["cd_ri_num_midia_espontanea"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/ri_num_midia_espontanea')."'; }");
