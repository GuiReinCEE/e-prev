...
<script>
	function filtrar()
	{
		...
		$.post('<?php echo site_url('indicador_plugin/igp_sat_participantes_instituidores/listar'); ?>',
		function(data)
		{ 
	...
	{
		if( $('#contador_input').val()!='2' )
		{
			alert( "Falta algum semestre." );
		}
		else if( confirm('Fechar o período?') )
		{
			$.post('<?php echo site_url("indicador_plugin/igp_sat_participantes_instituidores/criar_indicador/"); ?>',
			function(data)
			{
				$('#output_tela').html("Indicadores atualizados com sucesso, aguarde enquanto o período é fechado ..." );
				location.href = '<?php echo site_url("indicador_plugin/igp_sat_participantes_instituidores/fechar_periodo")?>';
			});
		}
	}	
	...
	function novo()
	{
		location.href='<?php echo site_url("indicador_plugin/igp_sat_participantes_instituidores/cadastro"); ?>';
	}
...
echo aba_start( $abas );
	echo form_start_box("default_box", "Cadastro");
		...
		echo form_default_row("","",button_save('Informar valores', 'novo()') . button_save('Atualizar apresentação', 'gerar_graficos()','botao_disabled'));
