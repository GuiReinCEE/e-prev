...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/beneficio_inc_tche"); ?>';
	}
    function manutencao()
    {
...
echo form_open( 'indicador_plugin/beneficio_inc_tche/salvar' );
echo form_hidden( 'cd_beneficio_inc_tche', intval($row['cd_beneficio_inc_tche']) );
...
if( intval($row['cd_beneficio_inc_tche'])>0  )
{
	echo button_delete("indicador_plugin/beneficio_inc_tche/excluir",$row["cd_beneficio_inc_tche"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/beneficio_inc_tche')."'; }");
