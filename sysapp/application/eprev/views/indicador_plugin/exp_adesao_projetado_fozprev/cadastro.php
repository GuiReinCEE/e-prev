...
<script>
	...
	function ir_lista()
	{
		location.href = '<?= site_url("indicador_plugin/exp_adesao_projetado_fozprev") ?>';
	}
	...
	function excluir()
	{
		location.href = '<?= site_url("indicador_plugin/exp_adesao_projetado_fozprev/excluir/".$row["cd_exp_adesao_projetado_fozprev"]) ?>';
	}
...
<? else: ?>
	<?= form_open('indicador_plugin/exp_adesao_projetado_fozprev/salvar')?>
		<?= form_start_box("default_box", 'Indicador')?>
			...
			<?= form_default_hidden('cd_exp_adesao_projetado_fozprev', 'Código da tabela', intval($row['cd_exp_adesao_projetado_fozprev']))?>
		...
		<?= form_command_bar_detail_start()?>
			...
			<? if(intval($row['cd_exp_adesao_projetado_fozprev']) > 0) : ?>
