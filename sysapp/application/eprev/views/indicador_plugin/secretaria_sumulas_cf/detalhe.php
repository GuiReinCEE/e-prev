...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/secretaria_sumulas_cf"); ?>';
	}
    function manutencao()
    {
...
echo form_open( 'indicador_plugin/secretaria_sumulas_cf/salvar' );
echo form_hidden( 'cd_secretaria_sumulas_cf', intval($row['cd_secretaria_sumulas_cf']) );
...
if( intval($row['cd_secretaria_sumulas_cf'])>0  )
{
	echo button_delete("indicador_plugin/secretaria_sumulas_cf/excluir",$row["cd_secretaria_sumulas_cf"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/secretaria_sumulas_cf')."'; }");
