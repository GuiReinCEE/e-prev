...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/atend_ligacao_transf"); ?>';
	}
...
echo form_open( 'indicador_plugin/atend_ligacao_transf/salvar' );
echo form_hidden( 'cd_atend_ligacao_transf', intval($row['cd_atend_ligacao_transf']) );
...
echo form_hidden('nr_meta');
//echo form_default_float("nr_meta", $label_4, app_decimal_para_php($row['nr_meta']), "style='visibility:hidden;'");
...
if( intval($row['cd_atend_ligacao_transf'])>0  )
{
	echo button_delete("indicador_plugin/atend_ligacao_transf/excluir",$row["cd_atend_ligacao_transf"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/atend_ligacao_transf')."'; }");
