...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/ri_sat_cliente_internos"); ?>';
	}
	...
	function excluir()
	{
		location.href = '<?php echo site_url("indicador_plugin/ri_sat_cliente_internos/excluir/".$row["cd_ri_sat_cliente_internos"]); ?>';
	}
...
echo aba_start($abas);
	echo form_open('indicador_plugin/ri_sat_cliente_internos/salvar');
		echo form_start_box("default_box", $tabela[0]['ds_indicador']);
			...
			echo form_default_hidden('cd_ri_sat_cliente_internos', 'Código da tabela', intval($row['cd_ri_sat_cliente_internos']));
		...
		echo form_command_bar_detail_start();
			...
			if(intval($row['cd_ri_sat_cliente_internos']) > 0)
			{
