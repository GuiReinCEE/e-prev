...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/atend_consulta_auto"); ?>';
	}
...
echo form_open( 'indicador_plugin/atend_consulta_auto/salvar' );
echo form_hidden( 'cd_atend_consulta_auto', intval($row['cd_atend_consulta_auto']) );
...
if( intval($row['cd_atend_consulta_auto'])>0  )
{
	echo button_delete("indicador_plugin/atend_consulta_auto/excluir",$row["cd_atend_consulta_auto"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/atend_consulta_auto')."'; }");
