...
<script>
	<?php 
	echo form_default_js_submit(array("mes_referencia", "ano_referencia", "cd_indicador_tabela", "nr_valor1", "nr_valor_2", "nr_meta"),'_salvar(form)');	?>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/ri_sat_cliente_externos"); ?>';
	}
	...
	function excluir()
	{
		location.href = '<?php echo site_url("indicador_plugin/ri_sat_cliente_externos/excluir/".$row["cd_ri_sat_cliente_externos"]); ?>';
	}
	...
	function getValores()
	{
		if(($("#mes_referencia").val() != "") && ($("#ano_referencia").val() != ""))
		{
			...
			$.post('<?php echo site_url("/ecrm/ri_controle_evento/resumoListar"); ?>', 
			{
				...
				cd_controle_evento_tipo : 1, //externo
				fl_json                 : "S"
			},
			function(data)
			{
				if(data[0])
				{
					$("#nr_valor_1").val(data[0].nr_respondente);
					$("#nr_valor_2").val(data[0].nr_satisfeito);
				}
...
?>
	...
	<?
	echo form_open('indicador_plugin/ri_sat_cliente_externos/salvar');
		echo form_start_box("default_box", $tabela[0]['ds_indicador']);
			...
			echo form_default_hidden('cd_ri_sat_cliente_externos', 'Código da tabela', intval($row['cd_ri_sat_cliente_externos']));
		...
		echo form_command_bar_detail_start();
			...
			if(intval($row['cd_ri_sat_cliente_externos']) > 0)
			{
