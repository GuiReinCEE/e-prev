...
<script>
	...
	function ir_lista()
	{
		location.href = "<?= site_url("indicador_plugin/controladoria_cobertura_benf_cd_inpel") ?>";
	}
	...
	function excluir()
	{
		location.href = "<?= site_url("indicador_plugin/controladoria_cobertura_benf_cd_inpel/excluir/".$row["cd_controladoria_cobertura_benf_cd_inpel"]) ?>";
	}
...
<? else: ?>
	<?= form_open('indicador_plugin/controladoria_cobertura_benf_cd_inpel/salvar')?>
		...
		<?= form_start_box('default_box', 'Cadastro')?>
			...
			<?= form_default_hidden('cd_controladoria_cobertura_benf_cd_inpel', 'Código da tabela', intval($row['cd_controladoria_cobertura_benf_cd_inpel']))?>
			<?= form_default_row('','','')?>
		...
		<?= form_command_bar_detail_start()?>
			<?= button_save()?>
			<?if(intval($row['cd_controladoria_cobertura_benf_cd_inpel']) > 0) ?>
