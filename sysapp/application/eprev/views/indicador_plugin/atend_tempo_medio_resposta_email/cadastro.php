...
<script>
	...
	function ir_lista()
	{
		location.href = '<?= site_url("indicador_plugin/atend_tempo_medio_resposta_email") ?>';
	}
	...
	function excluir()
	{
		location.href = '<?= site_url("indicador_plugin/atend_tempo_medio_resposta_email/excluir/".$row["cd_atend_tempo_medio_resposta_email"]) ?>';
	}
...
echo aba_start($abas);
	echo form_open('indicador_plugin/atend_tempo_medio_resposta_email/salvar');
		echo form_start_box("default_box", 'Cadastro');
			echo form_default_hidden('cd_indicador_tabela', 'Código indicador tabela', $tabela[0]['cd_indicador_tabela']);
			echo form_default_hidden('cd_atend_tempo_medio_resposta_email', 'Código da tabela', intval($row['cd_atend_tempo_medio_resposta_email']));
		...
		echo form_command_bar_detail_start();
			...
			if(intval($row['cd_atend_tempo_medio_resposta_email']) > 0)
			{
