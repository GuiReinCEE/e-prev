...
<script>
	...
	function ir_lista()
	{
		location.href = "<?= site_url("indicador_plugin/controladoria_eabd") ?>";
	}
	...
	function excluir()
	{
		location.href = "<?= site_url("indicador_plugin/controladoria_eabd/excluir/".$row["cd_controladoria_eabd"]) ?>";
	}
...
<? else: ?>
<?= form_open('indicador_plugin/controladoria_eabd/salvar') ?>
	...
	<?= form_start_box('default_box', 'Cadastro') ?>
		...
		<?= form_default_hidden('cd_controladoria_eabd', '', intval($row['cd_controladoria_eabd'])) ?>
	...
	<?= form_command_bar_detail_start()?>
		<?= button_save()?>
		<? if(intval($row['cd_controladoria_eabd']) > 0) :?>
