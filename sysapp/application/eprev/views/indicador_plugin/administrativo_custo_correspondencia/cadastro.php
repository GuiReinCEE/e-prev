...
<script>
	<?php 
	echo form_default_js_submit(array("mes_referencia", "ano_referencia", "cd_indicador_tabela", "nr_valor_1", "nr_meta"),'_salvar(form)');	?>
	...
	function ir_lista()
	{
		location.href = '<?= site_url("indicador_plugin/administrativo_custo_correspondencia") ?>';
	}
	...
	function excluir()
	{
		location.href = '<?= site_url("indicador_plugin/administrativo_custo_correspondencia/excluir/".$row["cd_administrativo_custo_correspondencia"]) ?>';
	}
...
echo aba_start($abas);
	echo form_open('indicador_plugin/administrativo_custo_correspondencia/salvar');
		echo form_start_box("default_box", 'Cadastro');
			echo form_default_hidden('cd_indicador_tabela', 'Código indicador tabela', $tabela[0]['cd_indicador_tabela']);
			echo form_default_hidden('cd_administrativo_custo_correspondencia', 'Código da tabela', intval($row['cd_administrativo_custo_correspondencia']));
		...
		echo form_command_bar_detail_start();
			...
			if(intval($row['cd_administrativo_custo_correspondencia']) > 0)
			{
