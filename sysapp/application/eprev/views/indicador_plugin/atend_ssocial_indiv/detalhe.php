...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/atend_ssocial_indiv"); ?>';
	}
...
echo form_open( 'indicador_plugin/atend_ssocial_indiv/salvar' );
echo form_hidden( 'cd_atend_ssocial_indiv', intval($row['cd_atend_ssocial_indiv']) );
...
if( intval($row['cd_atend_ssocial_indiv'])>0  )
{
	echo button_delete("indicador_plugin/atend_ssocial_indiv/excluir",$row["cd_atend_ssocial_indiv"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/atend_ssocial_indiv')."'; }");
