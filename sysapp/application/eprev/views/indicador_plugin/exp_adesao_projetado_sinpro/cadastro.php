...
<script>
	...
	function ir_lista()
	{
		location.href = '<?= site_url("indicador_plugin/exp_adesao_projetado_sinpro") ?>';
	}
	...
	function excluir()
	{
		location.href = '<?= site_url("indicador_plugin/exp_adesao_projetado_sinpro/excluir/".$row["cd_exp_adesao_projetado_sinpro"]) ?>';
	}
...
<? else: ?>
	<?= form_open('indicador_plugin/exp_adesao_projetado_sinpro/salvar')?>
		<?= form_start_box("default_box", 'Indicador')?>
			...
			<?= form_default_hidden('cd_exp_adesao_projetado_sinpro', 'Código da tabela', intval($row['cd_exp_adesao_projetado_sinpro']))?>
		...
		<?= form_command_bar_detail_start()?>
			...
			<? if(intval($row['cd_exp_adesao_projetado_sinpro']) > 0) : ?>
