...
<script>
	...
	function ir_lista()
	{
		location.href = '<?= site_url("indicador_plugin/financeiro_originais_retificacoes_dirf_erros_operacionais") ?>';
	}
	...
	function excluir()
	{
		location.href = '<?= site_url("indicador_plugin/financeiro_originais_retificacoes_dirf_erros_operacionais/excluir/".$row["cd_financeiro_originais_retificacoes_dirf_erros_operacionais"]) ?>';
	}
...
echo aba_start($abas);
	echo form_open('indicador_plugin/financeiro_originais_retificacoes_dirf_erros_operacionais/salvar');
		echo form_start_box("default_box", 'Cadastro');
			echo form_default_hidden('cd_indicador_tabela', 'Código indicador tabela', $tabela[0]['cd_indicador_tabela']);
			echo form_default_hidden('cd_financeiro_originais_retificacoes_dirf_erros_operacionais', 'Código da tabela', intval($row['cd_financeiro_originais_retificacoes_dirf_erros_operacionais']));
			...
			echo form_default_integer("nr_meta", $label_4.' :', $row['nr_meta'], "class='indicador_text'");
			echo form_default_textarea("observacao", $label_6.":", $row['observacao']);
		echo form_end_box("default_box");
		echo form_command_bar_detail_start();
			echo button_save();

			if(intval($row['cd_financeiro_originais_retificacoes_dirf_erros_operacionais']) > 0)
			{
