...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/atuarial_eap"); ?>';
	}
...
echo form_open( 'indicador_plugin/atuarial_eap/salvar' );
echo form_hidden( 'cd_atuarial_eap', intval($row['cd_atuarial_eap']) );
...
if( intval($row['cd_atuarial_eap'])>0  )
{
	echo button_delete("indicador_plugin/atuarial_eap/excluir",$row["cd_atuarial_eap"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/atuarial_eap')."'; }");
