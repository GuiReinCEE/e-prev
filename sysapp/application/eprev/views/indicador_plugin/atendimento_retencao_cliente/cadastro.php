...
<script>
    ...
    {
        location.href = '<?= site_url("indicador_plugin/atendimento_retencao_cliente/index") ?>';
    }

        function excluir()
    {
        location.href = '<?= site_url("indicador_plugin/atendimento_retencao_cliente/excluir/".$row['cd_atendimento_retencao_cliente']) ?>';
    }
    ...
    function getValores()
	{
		if(($("#mes_referencia").val() != "") && ($("#ano_referencia").val() != ""))
		{
			...
			$.post("<?= site_url('indicador_plugin/atendimento_retencao_cliente/get_valores') ?>", 
			{
			...
			function(data)
			{
                if(data)
                {
                    if(data.error.status == 0)
                    {
                        ...
                        $("#nr_cliente").val(result.total_clientes);                    
                        $("#nr_efetivas").val(result.total_retidos);                    
                        $("#nr_nao_retido").val(result.total_nao_retidos);
                        $("#nr_negociacao").val(result.total_em_negociacao);
                    }
                    else
                    {
                ...
                {
                    $("#msg_importar").html("Erro de comunicação com o Oracle");  
                }			
...
<?php
	...
	<? endif; ?>
	<?
        echo form_open('indicador_plugin/atendimento_retencao_cliente/salvar');
        echo form_start_box('cadastro_box', 'Cadastro');
            echo form_default_hidden('cd_atendimento_retencao_cliente', '', $row);
            ...
            echo form_default_integer('nr_interesse', $label_1.': (*)', intval($row['nr_interesse']), 'class="indicador_text"');    
            echo form_default_integer('nr_cliente', $label_8.': (*)', intval($row['nr_cliente']), 'class="indicador_text"'); 	
            echo form_default_integer('nr_efetivas', $label_2.': (*)', intval($row['nr_efetivas']), 'class="indicador_text"'); 	
            echo form_default_integer('nr_nao_retido', $label_5.': (*)', intval($row['nr_nao_retido']), 'class="indicador_text"'); 	
            echo form_default_integer('nr_negociacao', $label_4.': (*)', intval($row['nr_negociacao']), 'class="indicador_text"');  
        ...
        if(intval($row['cd_atendimento_retencao_cliente']) > 0) : 
