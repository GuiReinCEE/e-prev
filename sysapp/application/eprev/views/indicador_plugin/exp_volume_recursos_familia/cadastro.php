...
<script>
    ...
    function ir_lista()
	{
		location.href = "<?= site_url('indicador_plugin/exp_volume_recursos_familia') ?>";
	}
	...
	function excluir()
	{
		...
		{
			location.href = "<?= site_url('indicador_plugin/exp_volume_recursos_familia/excluir/'.$row['cd_exp_volume_recursos_familia']) ?>";
		}
	}
...
<? else: ?>
	...
	<?= form_open('indicador_plugin/exp_volume_recursos_familia/salvar') ?>
		...
		<?= form_start_box('default_box', 'Cadastro') ?>
			...
			<?= form_default_hidden('cd_exp_volume_recursos_familia', '', intval($row['cd_exp_volume_recursos_familia'])) ?>
		...
		<?= form_command_bar_detail_start() ?>
			...
			<? if(intval($row['cd_exp_volume_recursos_familia']) > 0) :?>	
