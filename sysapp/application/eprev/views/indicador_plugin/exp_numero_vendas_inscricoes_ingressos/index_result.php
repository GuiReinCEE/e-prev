...
foreach($collection as $item)
{
	...
	{
		$link = anchor("indicador_plugin/exp_numero_vendas_inscricoes_ingressos/cadastro/".$item["cd_exp_numero_vendas_inscricoes_ingressos"], "editar");
	...
	$body[] = array(
		...
		(trim($item["nr_valor_3"]) != '' ? $item["nr_valor_3"] : ''),
...
if(sizeof($media_ano) >0)
{
	$body[] = array(
		...
		(trim($nr_valor_3_total) != '' ? $nr_valor_3_total : ''),
		'<big><b>'.(trim($nr_valor_1_total) != '' ? number_format(($nr_valor_3_total/$nr_valor_1_total) * 100, 2, ',', '.').' %' : '').' </b></big>', 
