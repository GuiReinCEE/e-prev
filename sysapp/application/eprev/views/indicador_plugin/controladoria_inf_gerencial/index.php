...
function load()
{
	...
	$.post( '<?php echo base_url().index_page(); ?>/indicador_plugin/controladoria_inf_gerencial/listar',{},function(data){ $("#result_div").html(data);configure_result_table(); } );
}
...
function novo()
{
	location.href='<?php echo site_url("indicador_plugin/controladoria_inf_gerencial/detalhe/0"); ?>';
}
...
function gerar_grafico()
{
	...
	{
		url = '<?php echo site_url("indicador_plugin/controladoria_inf_gerencial/criar_indicador/"); ?>';
...
function fechar_periodo()
{
	...
	{
		alert( "Falta algum trimestre." );
	}
	else if( $('#mes_input').val()!='04' )
	{
		alert( "Último trimestre deve ser o 4." );
	}
	else if( confirm('Fechar o período?') )
	{
		...
		$.post(url,{},function(data){
			...
			url = '<?php echo site_url("indicador_plugin/controladoria_inf_gerencial/fechar_periodo")?>';
			location.href=url;
		});
	}
}
