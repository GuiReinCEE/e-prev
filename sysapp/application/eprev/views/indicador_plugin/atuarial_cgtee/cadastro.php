...
<script>
	...
	function ir_lista()
	{
		location.href = "<?= site_url("indicador_plugin/atuarial_cgtee") ?>";
	}
	...
	function excluir()
	{
		location.href = "<?= site_url("indicador_plugin/atuarial_cgtee/excluir/".$row["cd_atuarial_cgtee"]) ?>";
	}
...
<? else: ?>
<?= form_open('indicador_plugin/atuarial_cgtee/salvar') ?>
	...
	<?= form_start_box('default_box', 'Cadastro') ?>
		...
		<?= form_default_hidden('cd_atuarial_cgtee', '', intval($row['cd_atuarial_cgtee'])) ?>
	...
	<?= form_command_bar_detail_start()?>
		<?= button_save()?>
		<? if(intval($row['cd_atuarial_cgtee']) > 0) :?>
