...
<script>
	function filtrar()
	{
		...
		$.post('<?= site_url('indicador_plugin/controladoria_atend_requi_prazo/listar') ?>',
		function(data)
		{ 
	...
	function fechar_periodo()
	{
		...
		{
			$.post('<?= site_url("indicador_plugin/controladoria_atend_requi_prazo/criar_indicador") ?>',
			function(data)
			{
				$('#output_tela').html("Indicadores atualizados com sucesso, aguarde enquanto o período é fechado ..." );

				location.href = '<?= site_url("indicador_plugin/controladoria_atend_requi_prazo/fechar_periodo") ?>';
			});
		}
	}
	...
	function novo()
	{
		location.href = '<?= site_url("indicador_plugin/controladoria_atend_requi_prazo/cadastro") ?>';
	}
