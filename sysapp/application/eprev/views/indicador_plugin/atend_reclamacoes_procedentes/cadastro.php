...
<script>
	<?= form_default_js_submit(array('cd_indicador_tabela', 'mes_referencia', 'ano_referencia', 'nr_reclamacao', 'nr_reclamacao_procede', 'nr_meta'), 'validacao(form);') ?>
    ...
    function ir_lista()
	{
		location.href = "<?= site_url('indicador_plugin/atend_reclamacoes_procedentes') ?>";
	}
	...
	function excluir()
	{
		...
		{
			location.href = "<?= site_url('indicador_plugin/atend_reclamacoes_procedentes/excluir/'.$row['cd_atend_reclamacoes_procedentes']) ?>";
		}
	}
...
<? else: ?>
	...
	<?= form_open('indicador_plugin/atend_reclamacoes_procedentes/salvar') ?>
		...
		<?= form_start_box('default_box', 'Cadastro') ?>
			...
			<?= form_default_hidden('cd_atend_reclamacoes_procedentes', '', intval($row['cd_atend_reclamacoes_procedentes'])) ?>
			...
			<?= form_default_integer('nr_reclamacao_procede', $label_2.' :', $row['nr_reclamacao_procede'], 'class="indicador_text"') ?>			
		...
		<?= form_command_bar_detail_start() ?>
			...
			<? if(intval($row['cd_atend_reclamacoes_procedentes']) > 0) :?>	
