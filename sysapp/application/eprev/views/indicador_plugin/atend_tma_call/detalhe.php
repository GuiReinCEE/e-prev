...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/atend_tma_call"); ?>';
	}
...
echo form_open( 'indicador_plugin/atend_tma_call/salvar' );
echo form_hidden( 'cd_atend_tma_call', intval($row['cd_atend_tma_call']) );
...
if( intval($row['cd_atend_tma_call'])>0  )
{
	echo button_delete("indicador_plugin/atend_tma_call/excluir",$row["cd_atend_tma_call"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/atend_tma_call')."'; }");
