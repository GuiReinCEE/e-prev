...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/beneficio_inc_rge"); ?>';
	}
    function manutencao()
{
...
echo form_open( 'indicador_plugin/beneficio_inc_rge/salvar' );
echo form_hidden( 'cd_beneficio_inc_rge', intval($row['cd_beneficio_inc_rge']) );
...
if( intval($row['cd_beneficio_inc_rge'])>0  )
{
	echo button_delete("indicador_plugin/beneficio_inc_rge/excluir",$row["cd_beneficio_inc_rge"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/beneficio_inc_rge')."'; }");
