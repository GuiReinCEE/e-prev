...
foreach($collection as $key => $item)
{
	...
	else
	{
		...
		$link = anchor('indicador_plugin/atend_reclamacoes_fora_prazo/cadastro/'.$item['cd_atend_reclamacoes_fora_prazo'], '[editar]');
	}
	...
	$body[] = array(
		...
		number_format($item['nr_reclamacao_fora_prazo'], 0, ',' ,'.'),
		number_format($item['nr_meta'], 2, ',', '.').' %',
		number_format($item['nr_percent_fora_prazo'], 2, ',' ,'.').'%',
...
if(intval($contador_ano_atual) > 0)
{
	...
	$body[] = array(
		...
		'<b>'.number_format($nr_reclamacao_fora_prazo_total, 0, ',', '.').'</b>',
		'<b>'.number_format($nr_meta, 2, ',', '.').' %</b>',
		'<b>'.number_format($nr_percent_fora_prazo_total, 2, ',', '.').'%</b>',
		'',
		''
	);
}
