...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/atuarial_eap_crmprev"); ?>';
	}
...
echo form_open( 'indicador_plugin/atuarial_eap_crmprev/salvar' );
echo form_hidden( 'cd_atuarial_eap_crmprev', intval($row['cd_atuarial_eap_crmprev']) );
...
if( intval($row['cd_atuarial_eap_crmprev'])>0  )
{
	echo button_delete("indicador_plugin/atuarial_eap_crmprev/excluir",$row["cd_atuarial_eap_crmprev"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/atuarial_eap_crmprev')."'; }");
