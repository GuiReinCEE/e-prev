...
<script>
	...
	function ir_lista()
	{
		location.href = '<?= site_url("indicador_plugin/exp_adesao_potencial_sintee") ?>';
	}
	...
	function excluir()
	{
		location.href = '<?= site_url("indicador_plugin/exp_adesao_potencial_sintee/excluir/".$row["cd_exp_adesao_potencial_sintee"]) ?>';
	}
...
echo aba_start($abas);
	echo form_open('indicador_plugin/exp_adesao_potencial_sintee/salvar');
		echo form_start_box("default_box", 'Cadastro');
			echo form_default_hidden('cd_indicador_tabela', 'Código indicador tabela', $tabela[0]['cd_indicador_tabela']);
			echo form_default_hidden('cd_exp_adesao_potencial_sintee', 'Código da tabela', intval($row['cd_exp_adesao_potencial_sintee']));
		...
		echo form_command_bar_detail_start();
			...
			if(intval($row['cd_exp_adesao_potencial_sintee']) > 0)
			{
