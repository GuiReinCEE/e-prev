...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/administrativo_area_nloc"); ?>';
	}
    function manutencao()
    {
...
<? endif; ?>
<?
echo form_open( 'indicador_plugin/administrativo_area_nloc/salvar' );
echo form_hidden( 'cd_administrativo_area_nloc', intval($row['cd_administrativo_area_nloc']) );
...
if( intval($row['cd_administrativo_area_nloc'])>0  )
{
	echo button_delete("indicador_plugin/administrativo_area_nloc/excluir",$row["cd_administrativo_area_nloc"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/administrativo_area_nloc')."'; }");
