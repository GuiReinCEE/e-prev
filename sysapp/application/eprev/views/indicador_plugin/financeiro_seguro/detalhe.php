...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/financeiro_seguro"); ?>';
	}
    function manutencao()
    {
...
echo form_open( 'indicador_plugin/financeiro_seguro/salvar' );
echo form_hidden( 'cd_financeiro_seguro', intval($row['cd_financeiro_seguro']) );
...
if( intval($row['cd_financeiro_seguro'])>0  )
{
	echo button_delete("indicador_plugin/financeiro_seguro/excluir",$row["cd_financeiro_seguro"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/financeiro_seguro')."'; }");
