...
<script>
	function filtrar()
	{
		...
		$.post('<?= site_url('indicador_plugin/controladoria_cobertura_benf_cd_foz/listar') ?>',
		function(data)
		{ 
	...
	function atualizar_grafico()
	{
		...
		{
			$.post("<?= site_url('indicador_plugin/controladoria_cobertura_benf_cd_foz/criar_indicador') ?>", 
			function(data)
			{ 
	...
	function novo()
	{
		location.href = '<?= site_url("indicador_plugin/controladoria_cobertura_benf_cd_foz/cadastro") ?>';
	}
