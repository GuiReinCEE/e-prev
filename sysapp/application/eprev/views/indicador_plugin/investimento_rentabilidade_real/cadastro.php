...
<script>
	<?php 
	echo form_default_js_submit(array("mes_referencia", "ano_referencia", "cd_indicador_tabela", "nr_valor_1", "nr_valor_2", "nr_valor_3", "nr_valor_4"),'_salvar(form)');	?>
	...
	function ir_lista()
	{
		location.href = '<?= site_url("indicador_plugin/investimento_rentabilidade_real") ?>';
	}
	...
	function excluir()
	{
		location.href = '<?= site_url("indicador_plugin/investimento_rentabilidade_real/excluir/".$row["cd_investimento_rentabilidade_real"]) ?>';
	}
	...
	function get_valores()
	{
		if(($("#mes_referencia").val() != "") && ($("#ano_referencia").val() != ""))
		{
			...
			$.post("<?= site_url('indicador_plugin/investimento_rentabilidade_real/get_valores') ?>", 
			{
...
{
	$label_3 = str_replace("1", number_format($row["nr_valor_1_label"],1,",","."), $label_3);
}
/*
if($row["nr_valor_2_label"] > 0)
{
	$label_4 = str_replace("2", number_format($row["nr_valor_2_label"],1,",","."), $label_4);
}
*/
echo aba_start($abas);
	echo form_open('indicador_plugin/investimento_rentabilidade_real/salvar');
		echo form_start_box("default_box", 'Cadastro');
			echo form_default_hidden('cd_indicador_tabela', 'Código indicador tabela', $tabela[0]['cd_indicador_tabela']);
			echo form_default_hidden('cd_investimento_rentabilidade_real', 'Código da tabela', intval($row['cd_investimento_rentabilidade_real']));
		...
		echo form_command_bar_detail_start();
			...
			if(intval($row['cd_investimento_rentabilidade_real']) > 0)
			{
