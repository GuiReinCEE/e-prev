...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/ri_venda_previd"); ?>';
	}
	...
	function excluir()
	{
		location.href = '<?php echo site_url("indicador_plugin/ri_venda_previd/excluir/".$row["cd_ri_venda_previd"]); ?>';
	}
...
echo aba_start($abas);
	echo form_open('indicador_plugin/ri_venda_previd/salvar');
		echo form_start_box("default_box", $tabela[0]['ds_indicador']);
			echo form_hidden('cd_ri_venda_previd', intval($row['cd_ri_venda_previd']));
		...
		echo form_command_bar_detail_start();
			...
			if(intval($row['cd_ri_venda_previd']) > 0)
			{
