...
function load()
{
	...
	$.post( '<?php echo base_url().index_page(); ?>/indicador_plugin/administrativo_resul_colaborador/listar',{},function(data){ $("#result_div").html(data);configure_result_table(); } );
}
...
function novo()
{
	location.href='<?php echo site_url("indicador_plugin/administrativo_resul_colaborador/detalhe/0"); ?>';
}
...
function gerar_grafico()
{
	...
	{
		url = '<?php echo site_url("indicador_plugin/administrativo_resul_colaborador/criar_indicador/"); ?>';
...
function fechar_periodo()
{
	...
	{
		alert( "Falta um semestre." );
	}
	else if( $('#mes_input').val()!='02' )
	{
		alert( "O último semestre deve ser informado." );
	}
	else if( confirm('Fechar o período?') )
	{
		...
		$.post(url,{},function(data){
			...
			url = '<?php echo site_url("indicador_plugin/administrativo_resul_colaborador/fechar_periodo")?>';
			location.href=url;
		});
	}
}
