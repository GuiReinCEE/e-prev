...
<script>
	function filtrar()
	{
		...
		$.post('<?= site_url('indicador_plugin/atend_indice_recl/listar') ?>',
		function(data)
		{ 
	...
	function fechar_periodo()
	{
		...
		{
			$.post("<?= site_url('indicador_plugin/atend_indice_recl/criar_indicador') ?>",
			function(data){
				$("#div-output").html("Indicadores atualizados com sucesso, aguarde enquanto o período é fechado ...");

				location.href = "<?= site_url('indicador_plugin/atend_indice_recl/fechar_periodo') ?>";
			});
		}
	}
	...
	function novo()
	{
		location.href = '<?= site_url("indicador_plugin/atend_indice_recl/cadastro/0") ?>';
	}
