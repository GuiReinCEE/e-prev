...
<script>
	function filtrar()
	{
		...
		$.post('<?php echo site_url('indicador_plugin/juridico_solicitacao_parecer_gerencia_novo/listar'); ?>',
		function(data)
		{ 
    ...
    function fechar_periodo()
	{
		...
		{
			$.post("<?= site_url('indicador_plugin/juridico_solicitacao_parecer_gerencia_novo/criar_indicador') ?>", 
			function(data)
			{
				$("#div-output").html("Indicadores atualizados com sucesso, aguarde enquanto o período é fechado ...");

				location.href = "<?= site_url('indicador_plugin/juridico_solicitacao_parecer_gerencia_novo/fechar_periodo') ?>";
			});
		}
	}
	...
	function novo()
	{
		location.href='<?php echo site_url("indicador_plugin/juridico_solicitacao_parecer_gerencia_novo/detalhe"); ?>';
	}
