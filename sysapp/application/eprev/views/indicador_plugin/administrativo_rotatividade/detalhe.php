...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/administrativo_rotatividade"); ?>';
	}
...
echo form_open( 'indicador_plugin/administrativo_rotatividade/salvar' );
echo form_hidden( 'cd_administrativo_rotatividade', intval($row['cd_administrativo_rotatividade']) );
...
echo form_default_float("nr_referencial", $label_5, app_decimal_para_php($row['nr_referencial']), "class='indicador_text'");
...
if( intval($row['cd_administrativo_rotatividade'])>0  )
{
	echo button_delete("indicador_plugin/administrativo_rotatividade/excluir",$row["cd_administrativo_rotatividade"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/administrativo_rotatividade')."'; }");
