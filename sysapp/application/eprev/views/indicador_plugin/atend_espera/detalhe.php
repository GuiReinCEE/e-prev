...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/atend_espera"); ?>';
	}
...
<? endif; ?>
<?
echo form_open( 'indicador_plugin/atend_espera/salvar' );
echo form_hidden( 'cd_atend_espera', intval($row['cd_atend_espera']) );
...
if( intval($row['cd_atend_espera'])>0  )
{
	echo button_delete("indicador_plugin/atend_espera/excluir",$row["cd_atend_espera"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/atend_espera')."'; }");
