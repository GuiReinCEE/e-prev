...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/atuarial_eap_familia"); ?>';
	}
...
echo form_open( 'indicador_plugin/atuarial_eap_familia/salvar' );
echo form_hidden( 'cd_atuarial_eap_familia', intval($row['cd_atuarial_eap_familia']) );
...
if( intval($row['cd_atuarial_eap_familia'])>0  )
{
	echo button_delete("indicador_plugin/atuarial_eap_familia/excluir",$row["cd_atuarial_eap_familia"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/atuarial_eap_familia')."'; }");
