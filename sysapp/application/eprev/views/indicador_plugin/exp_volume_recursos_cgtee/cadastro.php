...
<script>
    ...
    function ir_lista()
	{
		location.href = "<?= site_url('indicador_plugin/exp_volume_recursos_cgtee') ?>";
	}
	...
	function excluir()
	{
		...
		{
			location.href = "<?= site_url('indicador_plugin/exp_volume_recursos_cgtee/excluir/'.$row['cd_exp_volume_recursos_cgtee']) ?>";
		}
	}
...
<? else: ?>
	...
	<?= form_open('indicador_plugin/exp_volume_recursos_cgtee/salvar') ?>
		...
		<?= form_start_box('default_box', 'Cadastro') ?>
			...
			<?= form_default_hidden('cd_exp_volume_recursos_cgtee', '', intval($row['cd_exp_volume_recursos_cgtee'])) ?>
		...
		<?= form_command_bar_detail_start() ?>
			...
			<? if(intval($row['cd_exp_volume_recursos_cgtee']) > 0) :?>	
