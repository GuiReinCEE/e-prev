...
<script>
	<?= form_default_js_submit(array('cd_indicador_tabela', 'mes_referencia', 'ano_referencia', 'nr_extratos', 'nr_extratos_incons', 'nr_meta'), 'validacao(form);') ?>
    ...
    function ir_lista()
	{
		location.href = "<?= site_url('indicador_plugin/financeiro_extratos_incons_inst') ?>";
	}
	...
	function excluir()
	{
		...
		{
			location.href = "<?= site_url('indicador_plugin/financeiro_extratos_incons_inst/excluir/'.$row['cd_financeiro_extratos_incons_inst']) ?>";
		}
	}
...
<? else: ?>
	...
	<?= form_open('indicador_plugin/financeiro_extratos_incons_inst/salvar') ?>
		...
		<?= form_start_box('default_box', 'Cadastro') ?>
			...
			<?= form_default_hidden('cd_financeiro_extratos_incons_inst', '', intval($row['cd_financeiro_extratos_incons_inst'])) ?>
			...
			<?= form_default_integer('nr_extratos_incons', $label_1.": (*)", number_format($row['nr_extratos_incons'], 0, ',', '.'), "class='indicador_text'") ?>
			<?= form_default_integer('nr_extratos', $label_2.": (*)", number_format($row['nr_extratos'], 0, ',', '.'), "class='indicador_text'") ?>
		...
		<?= form_command_bar_detail_start() ?>
			...
			<? if(intval($row['cd_financeiro_extratos_incons_inst']) > 0): ?>	
