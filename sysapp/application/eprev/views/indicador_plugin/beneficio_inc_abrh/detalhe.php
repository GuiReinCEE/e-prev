...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/beneficio_inc_abrh"); ?>';
	}
    function manutencao()
    {
...
echo form_open( 'indicador_plugin/beneficio_inc_abrh/salvar' );
echo form_hidden( 'cd_beneficio_inc_abrh', intval($row['cd_beneficio_inc_abrh']) );
...
if( intval($row['cd_beneficio_inc_abrh'])>0  )
{
	echo button_delete("indicador_plugin/beneficio_inc_abrh/excluir",$row["cd_beneficio_inc_abrh"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/beneficio_inc_abrh')."'; }");
