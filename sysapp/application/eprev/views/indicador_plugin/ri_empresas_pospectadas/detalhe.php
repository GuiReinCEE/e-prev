...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/ri_empresas_pospectadas"); ?>';
	}
    function manutencao()
    {
...
echo form_open( 'indicador_plugin/ri_empresas_pospectadas/salvar' );
echo form_hidden( 'cd_ri_empresas_pospectadas', intval($row['cd_ri_empresas_pospectadas']) );
...
if( intval($row['cd_ri_empresas_pospectadas'])>0  )
{
	echo button_delete("indicador_plugin/ri_empresas_pospectadas/excluir",$row["cd_ri_empresas_pospectadas"]);
}
