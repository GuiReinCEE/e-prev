...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/atend_senha"); ?>';
	}
...
echo form_open( 'indicador_plugin/atend_senha/salvar' );
echo form_hidden( 'cd_atend_senha', intval($row['cd_atend_senha']) );
...
if( intval($row['cd_atend_senha'])>0  )
{
	echo button_delete("indicador_plugin/atend_senha/excluir",$row["cd_atend_senha"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/atend_senha')."'; }");
