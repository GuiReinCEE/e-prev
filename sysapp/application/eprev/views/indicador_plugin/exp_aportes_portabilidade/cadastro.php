...
<script>
    ...
    function ir_lista()
	{
		location.href = "<?= site_url('indicador_plugin/exp_aportes_portabilidade') ?>";
	}
	...
	function excluir()
	{
		...
		{
			location.href = "<?= site_url('indicador_plugin/exp_aportes_portabilidade/excluir/'.$row['cd_exp_aportes_portabilidade']) ?>";
		}
	}
...
<? else: ?>
	...
	<?= form_open('indicador_plugin/exp_aportes_portabilidade/salvar') ?>
		...
		<?= form_start_box('default_box', 'Cadastro') ?>
			...
			<?= form_default_hidden('cd_exp_aportes_portabilidade', '', intval($row['cd_exp_aportes_portabilidade'])) ?>
		...
		<?= form_command_bar_detail_start() ?>
			...
			<? if(intval($row['cd_exp_aportes_portabilidade']) > 0) :?>	
