...
<script>
    ...
    function ir_lista()
	{
		location.href = "<?= site_url('indicador_plugin/exp_volume_recursos_ceran') ?>";
	}
	...
	function excluir()
	{
		...
		{
			location.href = "<?= site_url('indicador_plugin/exp_volume_recursos_ceran/excluir/'.$row['cd_exp_volume_recursos_ceran']) ?>";
		}
	}
...
<? else: ?>
	...
	<?= form_open('indicador_plugin/exp_volume_recursos_ceran/salvar') ?>
		...
		<?= form_start_box('default_box', 'Cadastro') ?>
			...
			<?= form_default_hidden('cd_exp_volume_recursos_ceran', '', intval($row['cd_exp_volume_recursos_ceran'])) ?>
		...
		<?= form_command_bar_detail_start() ?>
			...
			<? if(intval($row['cd_exp_volume_recursos_ceran']) > 0) :?>	
