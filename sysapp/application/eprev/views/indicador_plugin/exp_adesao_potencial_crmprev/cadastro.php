...
<script>
	...
	function ir_lista()
	{
		location.href = "<?= site_url("indicador_plugin/exp_adesao_potencial_crmprev") ?>";
	}
	...
	function excluir()
	{
		location.href = "<?= site_url("indicador_plugin/exp_adesao_potencial_crmprev/excluir/".$row["cd_exp_adesao_potencial_crmprev"]) ?>";
	}
...
<?= aba_start($abas)?>
	<?= form_open('indicador_plugin/exp_adesao_potencial_crmprev/salvar')?>
		...
		<?= form_start_box('default_box', 'Cadastro') ?>
			...
			<?= form_default_hidden('cd_exp_adesao_potencial_crmprev', '', intval($row['cd_exp_adesao_potencial_crmprev'])) ?>
			...
			<?= form_default_integer('nr_valor_4', $label_7.': (*)', $row['nr_valor_4'], 'class="indicador_text"')?>
		...
		<?= form_command_bar_detail_start()?>
			...
			<? if(intval($row['cd_exp_adesao_potencial_crmprev']) > 0) : ?>
