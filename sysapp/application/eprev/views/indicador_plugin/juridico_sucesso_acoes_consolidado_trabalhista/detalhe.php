...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/juridico_sucesso_acoes_consolidado_trabalhista"); ?>';
	}
	...
	function excluir()
	{
		location.href = '<?php echo site_url("indicador_plugin/juridico_sucesso_acoes_consolidado_trabalhista/excluir/".$row["cd_juridico_sucesso_acoes_consolidado_trabalhista"]); ?>';
	}
	
	function getValores()
	{
		if(($("#cd_etapa").val() != "") && ($("#ano_referencia").val() != ""))
		{
			$("#msg_importar").show();	
			
			$("#command_bar").hide();
			
			$.post('<?php echo site_url("indicador_plugin/juridico_sucesso_acoes_consolidado_trabalhista/get_valores"); ?>', 
			{
			...
			function(data)
			{
				if(data)
				{
					...
					$("#nr_improcede").val(data.nr_improcede);
					$("#nr_parcial").val(data.nr_parcial);
					$("#nr_procede").val(data.nr_procede);
				}
...
echo aba_start($abas);
	echo form_open('indicador_plugin/juridico_sucesso_acoes_consolidado_trabalhista/salvar');
		echo form_start_box("default_box", 'Cadastro');
			...
			echo form_default_hidden('cd_juridico_sucesso_acoes_consolidado_trabalhista', 'Código da tabela', intval($row['cd_juridico_sucesso_acoes_consolidado_trabalhista']));
		...
		echo form_command_bar_detail_start();
			...
			if(intval($row['cd_juridico_sucesso_acoes_consolidado_trabalhista']) > 0)
			{
