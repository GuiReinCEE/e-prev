...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_plugin/administrativo_ept_hora"); ?>';
	}
...
echo form_open( 'indicador_plugin/administrativo_ept_hora/salvar' );
echo form_hidden( 'cd_administrativo_ept_hora', intval($row['cd_administrativo_ept_hora']) );
...
if( intval($row['cd_administrativo_ept_hora'])>0  )
{
	echo button_delete("indicador_plugin/administrativo_ept_hora/excluir",$row["cd_administrativo_ept_hora"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_plugin/administrativo_ept_hora')."'; }");
