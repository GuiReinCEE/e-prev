...
<script>
	...
	function ir_lista()
	{
		location.href = "<?= site_url('indicador_plugin/rh_rotatividade') ?>";
	}
	...
	function excluir()
	{
		location.href = "<?= site_url('indicador_plugin/rh_rotatividade/excluir/'.$row['cd_rh_rotatividade']) ?>";
	}
...
?>
		...
		<? else: ?>
			<?= form_open('indicador_plugin/rh_rotatividade/salvar')?>
				<?= form_start_box("default_box", 'Cadastro')?>
					...
					<?= form_default_hidden('cd_rh_rotatividade', 'Código da tabela', intval($row['cd_rh_rotatividade']))?>
					...
					<?= form_default_integer('nr_desligamentos', $label_1.': (*)', number_format($row['nr_desligamentos'], 0, ',', '.'), 'class="indicador_text"')?> 
					<?= form_default_integer('nr_admissoes', $label_2.': (*)', number_format($row['nr_admissoes'], 0, ',', '.'), 'class="indicador_text"')?>
					<?= form_default_integer('nr_funcionarios', $label_3.': (*)', number_format($row['nr_funcionarios'], 0, ',', '.'), 'class="indicador_text"')?>
					<?= form_default_numeric('nr_limite_max', $label_4.': (*)', number_format($row['nr_limite_max'], 2, ',', '.'), 'class="indicador_text"')?>
					<?= form_default_numeric('nr_referencial', $label_5.': (*)', number_format($row['nr_referencial'], 2, ',', '.'), 'class="indicador_text"')?>
				...
				<?= form_command_bar_detail_start()?>
					...
					<? if(intval($row['cd_rh_rotatividade']) > 0) : ?>
