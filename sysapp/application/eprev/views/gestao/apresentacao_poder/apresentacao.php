<html>
	<head>
		<title>Apresentação Indicadores PODER</title>
		<style type="text/css">
			@font-face {
		...
		<script>
			...
			function load_slide(slide, callback)
			{
				...
				$.post("<?= site_url('gestao/apresentacao_poder/indicador_result') ?>", 
				{
			...
			function gera_image(i)
			{
				var fun_image = function (){
					...
					while(indicador_table > slide_indicador)
					{
						...
						if(indicador_table == $(".indicador_table").width())
						{
							break;
						}

						indicador_table = $(".indicador_table").width();
					}

					html2canvas($("#slide_indicador"), {
						onrendered: function(canvas) {
							$.post("<?= site_url('gestao/apresentacao_poder/salvar_imagem') ?>", 
							{
								id_imagem : i,
			...
			{
				location.href = "<?= site_url('gestao/apresentacao_poder/gera_pdf'); ?>/" + qt;
			}
	...
	<body>
		...
		<div id="header">
			...
			<div id="titulo" class="text-center">
				<h2 class="font_yannoka">Apresentação Indicadores PODER</h2>
			</div>
			<div id="numeracao">
