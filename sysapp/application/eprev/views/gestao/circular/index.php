...
function filtrar()
{
    ...
    $.post('<?=site_url('gestao/circular/listar')?>',
...
function divulgar()
{
	...
	{		
		...
		{
			$("#result_div").html("<?=loader_html()?>");
			$.post('<?=site_url('gestao/circular/divulgar')?>',
			{
				"arr[]" : arr
			},
			function(data)
		...
		{
			alert("Nenhuma Circular foi selecionada.");
		}
	}
}
...
{
	$config['button'][] = array('Nova Circular', 'novo()');
...
echo aba_start( $abas );
	...
	echo filter_integer_ano('nr_ano', 'nr_circular', 'Ano/Número:');
	echo filter_date_interval('dt_ini', 'dt_fim', 'Dt. Circular:');
	echo filter_text('ds_circular', 'Descrição:', '', 'style="width:300px;"');
