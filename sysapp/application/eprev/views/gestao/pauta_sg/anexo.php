...
<?php
	...
	{	
	    $body[] = array(
			...
			(trim($item['fl_rds']) == 'S' ? '<a href="javascript:void(0);"" onclick="historico('.$item['cd_pauta_sg_assunto_anexo'].')">[histórico]</a> ' : ''),
		);
	}
	...
	echo aba_start($abas);
		echo form_start_box('default_box', 'Pauta');
			...
			if(trim($row['dt_aprovacao']) != '')
			{
				...
				echo form_default_row('ds_usuario_responsavel', 'Usuário:', $row['ds_usuario_aprovacao']);
			}
		...
		echo form_start_box('default_assunto_box', 'Assunto');
			...
			echo form_default_row('nome', 'Responsável:', $assunto['ds_usuario_responsavel']);
			...
			{
				echo form_default_row('ds_diretoria', 'Área de Atuação:', $assunto['ds_diretoria']);
			}
		...
		if(trim($row['dt_aprovacao']) == '')
		{
			echo form_open('gestao/pauta_sg/anexo_salvar');
				...
				echo form_command_bar_detail_start();
					echo button_save('Validar Documentação Pydio', 'validar_documentacao()', 'botao_verde');
