...
{
	location.href='<?php echo site_url("gestao/sumula_conselho/resposta/".$row['cd_sumula_conselho_item']); ?>';
}
...
function listar()
{
	...
	$.post( '<?php echo site_url('gestao/sumula_conselho/listar_anexo'); ?>/',
	{
		cd_sumula_conselho_item : $('#cd_sumula_conselho_item').val()
	},
	function(data)
	{
...
function excluir_anexo(cd_sumula_conselho_item_anexo)
{
	if(confirm("ATENÇÃO\n\nDeseja excluir?\n\n"))
	{
		location.href='<?php echo site_url("gestao/sumula_conselho/excluir_anexo/".$row['cd_sumula_conselho_item']); ?>' + "/" + cd_sumula_conselho_item_anexo;
	}
}
...
echo aba_start( $abas );
	echo form_open('gestao/sumula_conselho/salvar_anexo', 'name="filter_bar_form"');
		...
		if(true)
		{
			echo form_start_box( "default_box", "Anexo" );
				echo form_default_upload_multiplo('arquivo_m', 'Arquivo :*', 'sumula_conselho', 'validaArq');
