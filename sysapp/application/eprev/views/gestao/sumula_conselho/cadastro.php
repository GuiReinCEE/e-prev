<?php
set_title('Súmulas do Conselho - Cadastro');
$this->load->view('header');
?>
<script>
<?php
    echo form_default_js_submit(Array('nr_sumula_conselho', 'dt_sumula_conselho', 'dt_divulgacao'), ' validacao(form);');
?>
    ...
    {
        location.href='<?php echo site_url("gestao/sumula_conselho/responsabilidade/".$row['cd_sumula_conselho']); ?>';
    }
    ...
    {
        $.post('<?php echo site_url('/gestao/sumula_conselho/validar_nr_sumula'); ?>',
        {
            nr_sumula_conselho : $('#nr_sumula_conselho').val(),
	...
	function publicar()
	{
        ...
        {
			var cd_sumula_conselho   = $('#cd_sumula_conselho').val();
			...
			$.post('<?php echo site_url('gestao/sumula_conselho/publicar') ?>',
			{
				cd_sumula_conselho   : cd_sumula_conselho,
	...
	function getSumulaAssinatura()
	{
		...
		$.post("<?php echo site_url('gestao/sumula_conselho/getSumulaAssinatura'); ?>/",
		{
			cd_sumula_conselho : $('#cd_sumula_conselho').val()
		},
		function(data)
	...
	function getAtaAssinatura()
	{
		...
		$.post("<?php echo site_url('gestao/sumula_conselho/getAtaAssinatura'); ?>/",
		{
			cd_sumula_conselho : $('#cd_sumula_conselho').val()
		},
		function(data)
...
if(intval($row['cd_sumula_conselho']) > 0)
{
...
echo aba_start( $abas );
    echo form_open('gestao/sumula_conselho/salvar', 'name="filter_bar_form"');
        echo form_start_box("default_box", "Cadastro" );
             echo form_default_hidden('cd_sumula_conselho', '', $row['cd_sumula_conselho']);
             echo form_default_hidden('nr_sumula_salvo', '', $row['nr_sumula_conselho']);
             echo form_default_integer('nr_sumula_conselho', 'Número:(*)', $row);
             echo form_default_date('dt_sumula_conselho', 'Dt Súmula:(*)', $row);
             echo form_default_date('dt_divulgacao', 'Dt Divulgação:(*)', $row);
             echo form_default_upload_iframe('arquivo', 'sumula_conselho', 'Arquivo:(*)', array($row['arquivo'],$row['arquivo_nome']), 'sumula_conselho', (($total_enviados['tl'] > 0 OR intval($row['cd_sumula_conselho']) == 0) ? true : false));
        echo form_end_box("default_box");
        echo form_command_bar_detail_start();     
            if($fl_editar OR intval($row['cd_sumula_conselho']) == 0)
            {
	...
	if(intval($row['cd_sumula_conselho']) > 0)
	{
		...
		{
			echo form_open('gestao/sumula_conselho/salvarAta', 'name="form_ata_salvar" id="form_ata_salvar"');
				echo form_start_box("default_ata_box", "Ata" );
					echo form_default_hidden('cd_sumula_ata', '', $row['cd_sumula_conselho']);
					echo form_default_upload_iframe('arquivo_ata', 'sumula_conselho', 'Arquivo :*', array($row['arquivo_ata'],$row['arquivo_ata_nome']), 'sumula_conselho', true);
