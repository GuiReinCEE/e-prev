<?php
set_title("Caderno CCI - Índice");
$this->load->view("header");
?>
<script>
	<?php
		echo form_default_js_submit(array("ds_caderno_cci_indice"));
	?>
	...
	function valores()
	{
		location.href = "<?= site_url("gestao/caderno_cci/indice_valor/".$row["cd_caderno_cci"]) ?>";
	}
	...
	function excluir(cd_caderno_cci_indice)
	{	
		var confirmacao = "Deseja excluir o índice?\n\n"+
			"Clique [Ok] para Sim\n\n"+
			"Clique [Cancelar] para Não\n\n";

		if(confirm(confirmacao))
		{ 
			location.href = "<?= site_url("gestao/caderno_cci/indice_excluir/".$row["cd_caderno_cci"]) ?>/" + cd_caderno_cci_indice;
		}
	}
	...
	$(function(){
		...
		$.post("<?= site_url("gestao/caderno_cci/indice_listar") ?>",
		{
...
$abas2[] = array("aba_indice2", "Índice", TRUE, "location.reload();");
$abas2[] = array("aba_benchmark", "Benchmark", FALSE, "ir_benchmark();");

echo aba_start($abas);
	echo aba_start($abas2);
	echo form_open("gestao/caderno_cci/indice_salvar");
		echo form_start_box("default_box", "Cadastro");
			echo form_default_hidden("cd_caderno_cci_indice", "", $indice);	
			...
			echo form_default_integer("nr_ordem", "Ordem :", $indice);
			echo form_default_text("ds_caderno_cci_indice", "Índice :*", $indice, 'style="width:300px;"');
		echo form_end_box("default_box");
		echo form_command_bar_detail_start();
			echo button_save("Salvar");	
			
			if(intval($indice["cd_caderno_cci_indice"]) > 0)
			{
