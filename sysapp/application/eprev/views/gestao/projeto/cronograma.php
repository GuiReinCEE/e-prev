...
<script>
	...
	function excluir(cd_projeto, cd_projeto_cronograma)
	{
		...
		{
			location.href = "<?= site_url('gestao/projeto/excluir_cronograma') ?>"+"/"+cd_projeto+"/"+cd_projeto_cronograma;
		}
	}
...
<?php
	...
	foreach($collection as $item)
	{
		...
		{
			$link_excluir = '<br><a href="javascript:void(0);" onclick="excluir('.$row['cd_projeto'].', '.$item['cd_projeto_cronograma'].');" title="Excluir">[Excluir]</a>';
		}
		...
		$body[] = array(
			...
			($item['dt_projeto_cronograma_realizado_fim'] == '' ? array(anchor('gestao/projeto/cronograma/'.$row['cd_projeto'].'/'.$item['cd_projeto_cronograma'], $item['ds_projeto_cronograma']), 'text-align:left;') : array($item['ds_projeto_cronograma'], 'text-align:left;')),
	...
	echo aba_start($abas);
		echo form_open('gestao/projeto/salvar_cronograma');
			...
			echo form_start_box('default_indicador_box', 'Cronograma');
				...
				echo form_default_integer('nr_ordem', 'Ordem :*', $cronograma);	
				echo form_default_text('ds_projeto_cronograma', 'Etapa :*', $cronograma, 'style="width:350px;"');
				echo form_default_checkbox_group('gerencia', 'Gerência Responsável :*', $gerencia, $cronograma_gerencia, 120);
				echo form_default_date('dt_projeto_cronograma_ini', 'Início Previsto :', $cronograma);
				echo form_default_date('dt_projeto_cronograma_fim', 'Fim Previsto :', $cronograma);
			echo form_end_box('default_indicador_box');
			
			echo form_command_bar_detail_start();
				if(($this->session->userdata('divisao') == trim($row['cd_gerencia_resposanvel'])) OR ($this->session->userdata('divisao') == 'GC'))
				{
