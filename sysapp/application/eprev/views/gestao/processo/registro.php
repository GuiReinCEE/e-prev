...
<script>
    <?php
        ...
        {
            echo form_default_js_submit(array('codigo', 'ds_processos_registro_anexo'));
        }
    ?>
    ...
    function editar(cd_processos_registro_anexo)
    {
        location.href = "<?= site_url('gestao/processo/registro/'.$processo['cd_processo']) ?>/"+cd_processos_registro_anexo;
    }
    ...
    function excluir(cd_processos_registro_anexo)
    {
        ...
        {
            location.href = "<?= site_url('gestao/processo/excluir_registro/'.$processo['cd_processo']) ?>/"+cd_processos_registro_anexo;
        }
    }

    function enviar_email(cd_processos_registro_anexo)
    {
        ...
        if(confirm(confirmacao))
        {
            location.href = "<?= site_url('gestao/processo/registro_enviar_email/'.$processo['cd_processo']) ?>/"+cd_processos_registro_anexo;
        }
    }
...
<?php
    ...
    $abas[] = array('aba_registro', 'POP', FALSE, 'ir_pop();');
    $abas[] = array('aba_registros', 'Registros', TRUE, 'location.reload();');
    ...
    foreach($collection as $item)
    {
        $body[] = array(
            ...
            array($item['ds_processos_registro_anexo'], 'text-align:left;'),
            ...
            '<a href="javascript:void(0);" onclick="editar('.$item['cd_processos_registro_anexo'].')">[editar]</a> '.
            '<a href="javascript:void(0);" onclick="enviar_email('.$item['cd_processos_registro_anexo'].')">[enviar e-mail]</a> '.
            '<a href="javascript:void(0);" onclick="excluir('.$item['cd_processos_registro_anexo'].')">[excluir]</a>'
        );
    }
    ...
    echo aba_start($abas);
    	echo form_open('gestao/processo/salvar_registro');
    		...
    		echo form_start_box('default_anexo_box', 'Anexo');
                ...
                {
                    echo form_default_row('arquivo', 'Arquivo:', $registro['arquivo_nome']);
                    echo form_default_hidden('cd_processos_registro_anexo', '', $registro['cd_processos_registro_anexo']);
                    echo form_default_text('codigo', 'Código: (*)', $registro['codigo'], 'style="width:400px;"');
                    echo form_default_text('ds_processos_registro_anexo', 'Descrição: (*)', $registro['ds_processos_registro_anexo'], 'style="width:400px;"');
                }
