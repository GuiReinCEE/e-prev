...
<script>
    ...
    function ir_lista()
	{
		location.href='<?php echo site_url("indicador_pga/evolucao_fundo_adm"); ?>';
	}
	...
	function excluir()
	{
		...
		{
			location.href='<?php echo site_url("indicador_pga/evolucao_fundo_adm/excluir/".$row["cd_evolucao_fundo_adm"]); ?>';
		}
	}
...
echo form_open('indicador_pga/evolucao_fundo_adm/salvar');
	echo form_start_box("default_box", "Cadastro");
		...
		echo form_default_hidden('cd_evolucao_fundo_adm', 'Código da tabela', intval($row['cd_evolucao_fundo_adm']));
	...
	echo form_command_bar_detail_start();
		...
		if(intval($row['cd_evolucao_fundo_adm']) > 0)
		{
			echo button_save('Excluir', 'excluir('.$row["cd_evolucao_fundo_adm"].');', 'botao_vermelho');
		}
