...
<script>
    ...
    function ir_lista()
	{
		location.href = "<?= site_url('indicador_pga/despesa_patrimonio_participante') ?>";
	}
	...
	function excluir()
	{
		...
		{
			location.href = "<?= site_url('indicador_pga/despesa_patrimonio_participante/excluir/'.$row['cd_despesa_patrimonio_participante']) ?>";
		}
	}
...
<? else: ?>
	...
	<?= form_open('indicador_pga/despesa_patrimonio_participante/salvar') ?>
		...
		<?= form_start_box('default_box', 'Cadastro') ?>
			...
			<?= form_default_hidden('cd_despesa_patrimonio_participante', '', intval($row['cd_despesa_patrimonio_participante'])) ?>
			...
			<?= form_default_numeric('nr_valor_3', $label_3.": (*)", number_format($row['nr_valor_3'], 2, ',', '.'), "class='indicador_text'") ?>
		...
		<?= form_command_bar_detail_start() ?>
			...
			<? if(intval($row['cd_despesa_patrimonio_participante']) > 0): ?>	
