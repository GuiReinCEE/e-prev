...
<script>
	<?= form_default_js_submit(array('cd_indicador_tabela', 'mes_referencia', 'ano_referencia', 'nr_recurso_garantidor', 'nr_deducao_limite', 'nr_custeio_adm'), 'validacao(form);') ?>
    ...
    function ir_lista()
	{
		location.href = "<?= site_url('indicador_pga/limite_taxa_adm') ?>";
	}
	...
	function excluir()
	{
		...
		{
			location.href = "<?= site_url('indicador_pga/limite_taxa_adm/excluir/'.$row['cd_limite_taxa_adm']) ?>";
		}
	}
...
<? else: ?>
	...
	<?= form_open('indicador_pga/limite_taxa_adm/salvar') ?>
		...
		<?= form_start_box('default_box', 'Cadastro') ?>
			...
			<?= form_default_hidden('cd_limite_taxa_adm', '', intval($row['cd_limite_taxa_adm'])) ?>
			...
			<?= form_default_numeric('nr_recurso_garantidor', $label_1.": (*)", number_format($row['nr_recurso_garantidor'], 2, ',', '.'), "class='indicador_text'") ?>
			<?= form_default_numeric('nr_deducao_limite', $label_3.": (*)", number_format($row['nr_deducao_limite'], 2, ',', '.'), "class='indicador_text'") ?>
			<?= form_default_numeric('nr_custeio_adm', $label_5.": (*)", number_format($row['nr_custeio_adm'], 2, ',', '.'), "class='indicador_text'") ?>
			
			<?= form_default_textarea('ds_observacao', $label_9.":", $row['ds_observacao']) ?>
		...
		<?= form_command_bar_detail_start() ?>
			...
			<? if(intval($row['cd_limite_taxa_adm']) > 0): ?>	
