...
<script>
	function filtrar()
	{
		...
		$.post("<?= site_url('indicador_pga/avaliacao_prestadores_servico/listar') ?>",
		function(data)
		{ 
	...
	function novo()
	{
		location.href = "<?= site_url('indicador_pga/avaliacao_prestadores_servico/cadastro') ?>";
	}
	...
	function atualizar_grafico()
	{
		...
		{
			$.post("<?= site_url('indicador_pga/avaliacao_prestadores_servico/criar_indicador') ?>", 
			function(data)
			{ 
	...
	function fechar_periodo()
	{
		...
		else if(confirm("Fechar o período?"))
		{
			...
			function(data){
				...
				location.href = "<?= site_url('indicador_pga/avaliacao_prestadores_servico/fechar_periodo') ?>";
			});
		}
	}
	...
	<?= form_start_box('default_box', 'Cadastro') ?>
		...
		<tr>
			<td colspan="2" style="padding-top:10px;">
				...
				<?php //button_save('Fechar Período', 'fechar_periodo()','botao_disabled') ?>
			</td>
		</tr>
