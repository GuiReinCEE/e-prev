...
<script>
	<?= form_default_js_submit(array('cd_indicador_tabela', 'mes_referencia', 'nr_valor_total', 'nr_valor_1', 'nr_meta'), 'validacao(form);') ?>
    ...
    function ir_lista()
	{
		location.href = "<?= site_url('indicador_pga/atend_objetivo_treinamento') ?>";
	}
	...
	function excluir()
	{
		...
		{
			location.href = "<?= site_url('indicador_pga/atend_objetivo_treinamento/excluir/'.$row['cd_atend_objetivo_treinamento']) ?>";
		}
	}
	...
	function get_valores()
	{
		if(($("#mes_referencia").val() != "") && ($("#ano_referencia").val() != ""))
		{
			...
			$.post('<?php echo site_url("indicador_pga/atend_objetivo_treinamento/get_valores"); ?>', 
			{
			...
			function(data)
			{
				if(data)
				{
					...
					$("#nr_valor_total").val(data.nr_valor_total);
				}
...
<? else: ?>
	...
	<?= form_open('indicador_pga/atend_objetivo_treinamento/salvar') ?>
		...
		<?= form_start_box('default_box', 'Cadastro') ?>
			...
			<?= form_default_hidden('cd_atend_objetivo_treinamento', '', intval($row['cd_atend_objetivo_treinamento'])) ?>
			...
			<?= form_default_integer('nr_valor_total', $label_7.' (*) :', number_format($row['nr_valor_total'], 0), 'class="indicador_text"') ?>
		...
		<?= form_command_bar_detail_start() ?>
			...
			<? if(intval($row['cd_atend_objetivo_treinamento']) > 0) :?>	
