...
<script>
    ...
    function ir_lista()
	{
		location.href='<?php echo site_url("indicador_pga/taxa_administracao"); ?>';
	}
	...
	function excluir()
	{
		...
		{
			location.href='<?php echo site_url("indicador_pga/taxa_administracao/excluir/".$row["cd_taxa_administracao"]); ?>';
		}
	}
...
echo form_open('indicador_pga/taxa_administracao/salvar');
	echo form_start_box("default_box", "Cadastro");
		...
		echo form_default_hidden('cd_taxa_administracao', 'Código da tabela', intval($row['cd_taxa_administracao']));
	...
	echo form_command_bar_detail_start();
		...
		if(intval($row['cd_taxa_administracao']) > 0)
		{
			echo button_save('Excluir', 'excluir('.$row["cd_taxa_administracao"].');', 'botao_vermelho');
		}
