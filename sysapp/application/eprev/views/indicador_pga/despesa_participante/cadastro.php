...
<script>
	<?= form_default_js_submit(array('cd_indicador_tabela', 'mes_referencia', 'ano_referencia', 'nr_valor_1', 'nr_valor_2', 'nr_meta', 'nr_meta_max'), 'validacao(form);') ?>
    ...
    function ir_lista()
	{
		location.href = "<?= site_url('indicador_pga/despesa_participante') ?>";
	}
	...
	function excluir()
	{
		...
		{
			location.href = "<?= site_url('indicador_pga/despesa_participante/excluir/'.$row['cd_despesa_participante']) ?>";
		}
	}
...
<? else: ?>
	...
	<?= form_open('indicador_pga/despesa_participante/salvar') ?>
		...
		<?= form_start_box('default_box', 'Cadastro') ?>
			...
			<?= form_default_hidden('cd_despesa_participante', '', intval($row['cd_despesa_participante'])) ?>
			...
			<?= form_default_integer('nr_valor_2', $label_2.": (*)", number_format($row['nr_valor_2'], 0, ',', '.'), "class='indicador_text'") ?>
			<?= form_default_numeric('nr_meta', $label_4.": (*)", number_format($row['nr_meta'], 2, ',', '.'), "class='indicador_text'") ?>
			<?= form_default_numeric('nr_meta_max', $label_6.": (*)", number_format($row['nr_meta_max'], 2, ',', '.'), "class='indicador_text'") ?>
		...
		<?= form_command_bar_detail_start() ?>
			...
			<? if(intval($row['cd_despesa_participante']) > 0): ?>	
