...
<script>
	function filtrar()
	{
		...
		$.post("<?= site_url('ecrm/contrib_percentual_programada/listar') ?>",
	...
	function ir_lista()
	{
		location.href = "<?= site_url('ecrm/contrib_programada') ?>";
	}
	...
	function atualizar()
	{
        ...
        {
			var confirmacao = "Deseja atualizar (Total Selecionado: "+contribuicao_programada.length+")?\n\n"+
				"Clique [Ok] para Sim\n\n"+
			...
			if(confirm(confirmacao))
			{
				...
				$.post("<?= site_url('ecrm/contrib_percentual_programada/atualizar_itens') ?>",
				{
	...
	function cancelar()
	{
        ...
        if(contribuicao_programada.length > 0)
        {
			...
			if(confirm(confirmacao))
			{
				...
				$.post("<?= site_url('ecrm/contrib_percentual_programada/cancelar') ?>",
				{
					"contribuicao_programada[]" : contribuicao_programada
				},
				function(data)
...
<?php
	$abas[0] = array('aba_lista', 'Valor Instituidor', false, 'ir_lista();');
	$abas[1] = array('aba_lista_percent', '% Patroc.', true, 'location.reload();');
