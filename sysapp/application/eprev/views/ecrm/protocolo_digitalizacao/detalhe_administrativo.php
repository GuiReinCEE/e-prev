<?php
set_title('Protocolo para Digitalização - GGS');
...
<script>
    ...
    function listar_documento()
    {
        if($('#cd_documento_protocolo').val() > 0)
        {
            ...
            $.post('<?php echo base_url() . index_page(); ?>/ecrm/protocolo_digitalizacao/listar_documento_administrativo',
            {
        ...
        {		
            if( confirm('Enviar protocolo?') )
            {
                ...
                function(data)
                {
                    if(data=='true')
                    {
                        location.href='<?php echo site_url("ecrm/protocolo_digitalizacao/detalhe_administrativo/"); ?>/'+$('#cd_documento_protocolo').val();
                    }
                });
            }
        }
        ...
        if($('#cd_documento_protocolo').val() == '0')
        {
            ...
            else
            {
                if( confirm('Criar um novo protocolo?') )
                {
                    ...
                    function(data)
                    {
                        location.href='<?php echo site_url("ecrm/protocolo_digitalizacao/detalhe_administrativo/"); ?>/'+data.cd_documento_protocolo;
                    }, 
                    'json');
                }
            }
    ...
    function adicionar_documento(form)
    {
        ...
        {
            $.post("<?php echo site_url('ecrm/protocolo_digitalizacao/adicionar_documento_adm'); ?>", myObj, 
		    function(data)
		    {
			    if(data=='true')
                { 
    ...
    function callback_buscar_tipo_documento()
    {
        ...
        {
			if ($('#cd_tipo_doc').val() == 366)
			{
				//OS:42046
				var aviso = "É LAUDO INDEFERIDO?\n\nSIM clique [Ok]\n\nNÃO clique [Cancelar]\n\n";
				if(!confirm(aviso))
				{
					alert("ATENÇÃO\n\nVerifique o processo.\n\nDocumento deve ser encaminhado para folha pagamento.\n\n");
					$('#cd_tipo_doc').val("");
					$('#nome_documento').val("");
				}
			}
...
if (intval($row["cd_documento_protocolo"]) > 0)
{
    ...
    if ($row["dt_envio"] == '' && usuario_id() == intval($row['cd_usuario_cadastro']))
    {
        ...
        echo form_default_row("", "", br() . button_save("Enviar Protocolo", "enviar_protocolo(this.form)").' '. button_save("Excluir Protocolo", "excluir_protocolo(this.form)", "botao_vermelho"));
    }
    else
    {
...
else
{
	...
	echo form_hidden('cd_gerencia', 'Gerência', 'GGS');
