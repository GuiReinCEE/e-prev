...
<script>
    ...
    function listar_documento()
    {
        if( $('#cd_documento_protocolo').val()>0 )
        {
            ...
            url="<?php echo site_url('ecrm/protocolo_digitalizacao/listar_documento_juridico') ?>";
            $.post(url,
            {
        ...
        {		
            if( confirm('Enviar protocolo?') )
            {
                ...
                $.post( url, {cd_documento_protocolo:$('#cd_documento_protocolo').val()}, function(data){
                    if(data=='true')
                    {
                        location.href='<?php echo site_url("ecrm/protocolo_digitalizacao/detalhe_juridico/"); ?>/'+$('#cd_documento_protocolo').val();
                    }
                } );
            }
        }
            ...
            else
            {
                if( $('#cd_documento_protocolo').val()=='0' )
                {
                    if( confirm('Criar um novo protocolo?') )
                    {
                        ...
                        function(data)
                        {
                            location.href='<?php echo site_url("ecrm/protocolo_digitalizacao/detalhe_juridico/"); ?>/'+data.cd_documento_protocolo;					
                        }, 
                        'json');
                    }
                }
    ...
    function adicionar_documento(form)
    {
        ...
        if(confirm('Adicionar?'))
        {
            ...
            url="<?php echo site_url('ecrm/protocolo_digitalizacao/adicionar_documento_juridico'); ?>";
            $.post( url,{
