<?php
set_title('Protocolo para Digitalização - GAP');
...
<script>
    ...
    {
        <?php if(!gerencia_in(array('GP'))): ?>
        if( ($('#cd_empresa').val()=='' && $('#cd_registro_empregado').val()=='' && $('#seq_dependencia').val()=='') )
        {
            alert('Informe a Empresa/RE/Sequência.');
            $('#cd_empresa').focus();
            return false;
        }
        <?php endif; ?>
        ...
        $.post("<?= site_url('ecrm/protocolo_digitalizacao/adicionar_documento') ?>",
        {
            cd_documento_protocolo      : $('#cd_documento_protocolo').val(),
...
else
{
	...
	echo form_hidden('cd_gerencia', 'Gerência', 'GAP');
