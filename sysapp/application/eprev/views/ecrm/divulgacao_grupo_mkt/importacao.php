...
<script>
    ...
    {
        location.href = "<?= site_url('ecrm/divulgacao_grupo_mkt/atualiza_registro/'.intval($row['cd_divulgacao_grupo']).'/I') ?>";
    }	

    function excluir(cd_divulgacao_grupo_email)
    {
    	var confirmacao = "Deseja EXCLUIR o E-mail?\n\n"+
                          "Clique [Ok] para Sim\n\n"+
                          "Clique [Cancelar] para Não\n\n"; 

        if(confirm(confirmacao))
        {
            location.href = "<?= site_url('ecrm/divulgacao_grupo_mkt/excluir_email/'.$row['cd_divulgacao_grupo']) ?>/"+cd_divulgacao_grupo_email;
        }
    }
...
<?php
    ...
    {
		$body[] = array(
			$item['ds_divulgacao_grupo_email'],
			$item['dt_inclusao'],
			'<a href="javascript:void(0);" onclick="excluir('.$item['cd_divulgacao_grupo_email'].');">[excluir]</a>'
		);
	}
    ...
    echo aba_start($abas);
    	echo form_open('ecrm/divulgacao_grupo_mkt/salvar_email');
            echo form_start_box('default_box', 'Cadastro');
                ...
                echo form_default_row('', '', '<i>Arquivo .csv ou .txt com os e-mails separados por linha</i>');
