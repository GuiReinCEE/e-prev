<?php
	...
	foreach($collection as $item)
	{
		...
		$body[] = array(
			...
			'<span class="'.(((trim($item['dt_prorrogacao']) == '') AND ((trim($item['dt_encerramento']) == '') AND (trim($item['dt_cancela']) == ''))) ? 'label label-important' : '').'">'.$item['dt_prazo'].'</span>',		
			'<span class="'.(((trim($item['dt_prorrogacao']) != '') AND ((trim($item['dt_encerramento']) == '') AND (trim($item['dt_cancela']) == ''))) ? 'label label-important' : "").'">'.$item['dt_prorrogacao'].'</span>',
	...
	if ((intval($qt_anterior) > 0) AND (intval($cd_reclamacao) == ''))
	{
		echo "
			<script> 
				$('#msg_anterior').html('Este participante tem reclamação(ões) anterior(es), confira antes de registrar uma nova.');
				$('#msg_anterior_row').show();
			</script>";
	}
?>