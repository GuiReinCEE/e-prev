...
?>
	...
	function listar_anexos()
    {
        ...
        $.post( '<?php echo site_url('ecrm/relacionamento_empresa/listar_anexos'); ?>',
		{
	...
	function excluir_anexo(cd_empresa_anexo)
	{	
		if(confirm("ATENÇÃO\n\nDeseja excluir o anexo?\n\nClique [Ok] para Sim\nClique [Cancelar] para Não\n\n"))
		{
			$.post("<?php echo site_url('ecrm/relacionamento_empresa/excluir_anexo'); ?>", 
			{
				cd_empresa_anexo : cd_empresa_anexo
			}, 
			function(data)
			{ 
				listar_anexos();
...
$abas[] = array('aba_pessoa', 'Anexo', TRUE, 'location.reload();');

echo aba_start($abas);
	echo form_open('ecrm/relacionamento_empresa/salvar_anexo');
