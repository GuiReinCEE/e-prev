<?php
	set_title('Avaliação - Comparativo - Competências Específicas');
...
$abas[] = array('aba_competencia', 'Competências Específicas', TRUE, 'location.reload();');
echo aba_start( $abas );
	...
	{
		$body[] = Array (
					array($item['nome_comp_espec'],'text-align:left;'),
					number_format($item['grau'],2,",","."),
					array(number_format($superior[$item['cd_comp_espec']],2,",","."), 'text-align:center;'.($item['grau'] != $superior[$item['cd_comp_espec']] ? "font-weight:bold; color:green;" : ""))
				);
		
		
		$sum_avaliado+= floatval($item['grau']);
		$sum_superior+= floatval($superior[$item['cd_comp_espec']]);
		$qt_reg++;
	}
	...
	echo 
	form_start_box('grau_box','Competências Específicas ', false)
		.$lista
