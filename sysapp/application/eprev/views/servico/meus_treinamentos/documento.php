...
<script>
    ...
    {
        location.href = "<?= site_url('servico/meus_treinamentos/anexo/'.$row['cd_treinamento_colaborador_item']) ?>"
    }

    function excluir(cd_treinamento_colaborador_documento)
        {
            ...
            {
                location.href = "<?= site_url('servico/meus_treinamentos/excluir_documento/'.$row['cd_treinamento_colaborador_item']) ?>/"+ cd_treinamento_colaborador_documento;
            }
        }
...
<?php
    ...
    foreach ($collection as $item)
    {
        $body[] = array(
            ...
            (intval($item['cd_usuario_inclusao']) == $this->session->userdata('codigo') ? '<a href="javascript:void(0);"" onclick="excluir('.$item['cd_treinamento_colaborador_documento'].')">[excluir]</a>' : '')
        );
    }
    ...
    echo aba_start($abas);
        echo form_open('servico/meus_treinamentos/salvar_documento');
