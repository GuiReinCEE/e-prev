...
<head>
	...
	<!-- Editor HTML TINYMCE -->
	<script type="text/javascript" src="<?php echo base_url(); ?>js/tinymce/js/tinymce/jquery.tinymce.min.js"></script>
...
<script type="text/javascript">  
	...
	dateRanges.last60days   = function(d1,d2) {
		d1.addDays(-60);
	},	
	dateRanges.last90days   = function(d1,d2) {
		d1.addDays(-90);
	},		
	...
	dateRanges.next60days   = function(d1,d2) {
		d2.addDays(+60);
	},	
	dateRanges.next90days   = function(d1,d2) {
		d2.addDays(+90);
	},	
        ...
        function(data)
        {
			if(data.qt_pendencia > 0)
			{
				if ($.browser.msie) 
				{
					...
					$("#ob_notifica_pendencia").html('<div class="notify_conta">**</div>');
				}				
				else
				{
					//$("#ob_notifica_pendencia").html('<id class="notify_conta">'+data.qt_pendencia+'</id>');
					$("#ob_notifica_pendencia").html('<id class="notify_conta">**</id>');
				}
				
				$("#ob_notifica_pendencia").removeClass("notify_nao");
				$("#ob_notifica_pendencia").addClass("notify_sim");	
				
				if(data.qt_atrasada > 0)
				{
					$('.notify_sim').css('background-image', "url('<?php echo base_url(); ?>img/notify_pendencia_atrasada.png')");
				}
				else
				{
					$('.notify_sim').css('background-image', "url('<?php echo base_url(); ?>img/notify_pendencia.png')");
				}
...
<style>
	...
	.notify_doc_digitalizado_nao{
		background-position: 0px 6px;
		background-image: url('<?php echo base_url(); ?>img/notify_doc_digitalizado.png');
		background-repeat: no-repeat;
		width: 16px;
		height: 24px;
		text-align: left;
...
<script type="text/javascript">
	function getNotificaoDocDigitalizado()
	{
		$.post('<?php echo site_url('ecrm/digitalizado/notificacao');?>',
        {},
        function(data)
        {
			if(data.qt_doc > 0)
			{
				if ($.browser.msie) 
				{
					$("#ob_notifica_doc_digitalizado").html('<div class="notify_doc_digitalizado_conta">'+data.qt_doc+'</div>');
				}				
				else
				{
					$("#ob_notifica_doc_digitalizado").html('<id class="notify_doc_digitalizado_conta">'+data.qt_doc+'</id>');
				}
				
				$("#ob_notifica_doc_digitalizado").removeClass("notify_doc_digitalizado_nao");
				$("#ob_notifica_doc_digitalizado").addClass("notify_doc_digitalizado_sim");	
			}
			else
			{
				$("#ob_notifica_doc_digitalizado").html("");
				$("#ob_notifica_doc_digitalizado").removeClass("notify_doc_digitalizado_sim");
				$("#ob_notifica_doc_digitalizado").addClass("notify_doc_digitalizado_nao");	
			}
        },
		'json');	
	}
	...
	$(function(){
		getNotificaoDocDigitalizado();
	});
</script>

<style>
	.notify_assinatura_digitalconta {
		<?php
			if(preg_match('/(?i)MSIE [1-8].0/',$_SERVER['HTTP_USER_AGENT']))
			{
				echo 'background: #4D90FE;';
	...
	.notify_assinatura_digital_nao{
		background-position: 0px 6px;
		background-image: url('<?php echo base_url(); ?>img/notify_assinatura.png');
		background-repeat: no-repeat;
		width: 16px;
		height: 24px;
		text-align: left;
								...
								<td align="right" class="header_usuario">
									<?php
										...
										else
										{
											...
											if(strtolower(trim($this->db->hostname)) != "srvpg.eletroceee.com.br")
											{
												echo '<BR><span class="label label-important">'.$this->db->hostname.'</span>';
											}
											else
											{
												echo '<span style="font-size: 70%;"><BR>'.$this->db->hostname.'</span>';
					...
					<td class="header_menu">
						<table border="0" cellpadding="0" cellspacing="0">
							<tr>
								...
								?>	
								<?php
									#if($this->session->userdata('usuario') == "coliveira")
									#{
								?>
								<td class="header_menu_meio" style="padding-left: 3px;">
									<a href="<?php echo site_url('ecrm/digitalizado'); ?>">
									<div id="ob_notifica_doc_digitalizado" class="notify_doc_digitalizado_nao" title="Meus docs digitalizados"></div>	
									</a>
								</td>		
								<?php
									#}
								...
								<td class="header_menu_meio" style="padding-left: 3px;">
									<a href="<?php echo site_url('clicksign/clicksign_documento'); ?>">
									<div id="ob_notifica_assinatura_digital" class="notify_assinatura_digital_nao" title="Docs enviados para assinatura"></div>	
									</a>
								</td>		
