<?php
	...
	$FL_PROTOCOLO_INTERNO = (gerencia_in(array('GTI','GCM'))) ? TRUE : FALSE;
	...
	foreach ($collection as $key => $item) 
	{
		...
		$FL_PROTOCOLO_INTERNO_CAMPOS = $FL_PROTOCOLO_INTERNO;
		if ($item['fl_status'] != 'CLOSED')
		{
			$FL_PROTOCOLO_INTERNO_CAMPOS = FALSE;
		}		
		...
		$ar_linha = array();
		$ar_linha[] = ((($item['fl_status'] == 'CLOSED') and (trim($item['nr_documento_recebido']) == "")) ? form_checkbox($campo_check) : "");
		$ar_linha[] = $item['cd_documento'];
		...
		$ar_linha[] = '<span class="'.$item["cor_status"].'">'.$item["ds_status"].'</span>'.($FL_CHECK_RECUSA == TRUE ? "<br><span id='obDocRecusado_".$item['id_doc']."'><a href=\"javascript:getRecusa('".$item['id_doc']."')\">[verificar recusa]</a></span>" : "");
		...
		if($FL_PROTOCOLO_INTERNO_CAMPOS)
		{
			if(trim($item['nr_documento_recebido']) != "")
			{
				$ar_linha[] = "Documento vinculado no protocolo interno: <b>".trim($item['nr_documento_recebido'])."</b>";
				$ar_linha[] = "";
			}
			else
			{
