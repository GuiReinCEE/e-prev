...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_atendimento/participantes"); ?>';
	}
...
echo form_open( 'indicador_atendimento/participantes/salvar' );
echo form_hidden( 'cd_participantes', intval($row['cd_participantes']) );
...
if( intval($row['cd_participantes'])>0  )
{
	echo button_delete("indicador_atendimento/participantes/excluir",$row["cd_participantes"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_atendimento/participantes')."'; }");
