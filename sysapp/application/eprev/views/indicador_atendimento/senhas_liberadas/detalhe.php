...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_atendimento/senhas_liberadas"); ?>';
	}
...
echo form_open( 'indicador_atendimento/senhas_liberadas/salvar' );
echo form_hidden( 'cd_senhas_liberadas', intval($row['cd_senhas_liberadas']) );
...
if( intval($row['cd_senhas_liberadas'])>0  )
{
	echo button_delete("indicador_atendimento/senhas_liberadas/excluir",$row["cd_senhas_liberadas"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_atendimento/senhas_liberadas')."'; }");
