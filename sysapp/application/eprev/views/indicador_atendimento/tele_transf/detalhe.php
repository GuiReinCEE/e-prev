...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_atendimento/tele_transf"); ?>';
	}
...
echo form_open( 'indicador_atendimento/tele_transf/salvar' );
echo form_hidden( 'cd_tele_transf', intval($row['cd_tele_transf']) );
...
if( intval($row['cd_tele_transf'])>0  )
{
	echo button_delete("indicador_atendimento/tele_transf/excluir",$row["cd_tele_transf"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_atendimento/tele_transf')."'; }");
