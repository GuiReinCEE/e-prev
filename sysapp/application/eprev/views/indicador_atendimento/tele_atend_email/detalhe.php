...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_atendimento/tele_atend_email"); ?>';
	}
...
echo form_open( 'indicador_atendimento/tele_atend_email/salvar' );
echo form_hidden( 'cd_tele_atend_email', intval($row['cd_tele_atend_email']) );
...
if( intval($row['cd_tele_atend_email'])>0  )
{
	echo button_delete("indicador_atendimento/tele_atend_email/excluir",$row["cd_tele_atend_email"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_atendimento/tele_atend_email')."'; }");
