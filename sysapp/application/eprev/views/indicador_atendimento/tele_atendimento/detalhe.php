...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_atendimento/tele_atendimento"); ?>';
	}
...
echo form_open( 'indicador_atendimento/tele_atendimento/salvar' );
echo form_hidden( 'cd_tele_atendimento', intval($row['cd_tele_atendimento']) );
...
if( intval($row['cd_tele_atendimento'])>0  )
{
	echo button_delete("indicador_atendimento/tele_atendimento/excluir",$row["cd_tele_atendimento"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_atendimento/tele_atendimento')."'; }");
