...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_atendimento/ingressos_patro"); ?>';
	}
...
echo form_open( 'indicador_atendimento/ingressos_patro/salvar' );
echo form_hidden( 'cd_ingressos_patro', intval($row['cd_ingressos_patro']) );
...
if( intval($row['cd_ingressos_patro'])>0  )
{
	echo button_delete("indicador_atendimento/ingressos_patro/excluir",$row["cd_ingressos_patro"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_atendimento/ingressos_patro')."'; }");
