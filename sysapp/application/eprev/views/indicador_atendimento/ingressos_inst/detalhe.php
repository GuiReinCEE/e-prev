...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_atendimento/ingressos_inst"); ?>';
	}
...
echo form_open( 'indicador_atendimento/ingressos_inst/salvar' );
echo form_hidden( 'cd_ingressos_inst', intval($row['cd_ingressos_inst']) );
...
if( intval($row['cd_ingressos_inst'])>0  )
{
	echo button_delete("indicador_atendimento/ingressos_inst/excluir",$row["cd_ingressos_inst"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_atendimento/ingressos_inst')."'; }");
