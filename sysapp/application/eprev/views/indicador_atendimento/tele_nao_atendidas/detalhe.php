...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_atendimento/tele_nao_atendidas"); ?>';
	}
...
echo form_open( 'indicador_atendimento/tele_nao_atendidas/salvar' );
echo form_hidden( 'cd_tele_nao_atendidas', intval($row['cd_tele_nao_atendidas']) );
...
if( intval($row['cd_tele_nao_atendidas'])>0  )
{
	echo button_delete("indicador_atendimento/tele_nao_atendidas/excluir",$row["cd_tele_nao_atendidas"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_atendimento/tele_nao_atendidas')."'; }");
