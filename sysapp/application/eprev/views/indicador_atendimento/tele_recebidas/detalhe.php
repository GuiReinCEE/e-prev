...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("indicador_atendimento/tele_recebidas"); ?>';
	}
...
echo form_open( 'indicador_atendimento/tele_recebidas/salvar' );
echo form_hidden( 'cd_tele_recebidas', intval($row['cd_tele_recebidas']) );
...
if( intval($row['cd_tele_recebidas'])>0  )
{
	echo button_delete("indicador_atendimento/tele_recebidas/excluir",$row["cd_tele_recebidas"]);
}

echo form_command_bar_detail_button("Voltar para lista", "if( confirm('Voltar?') ){ location.href='".site_url('indicador_atendimento/tele_recebidas')."'; }");
