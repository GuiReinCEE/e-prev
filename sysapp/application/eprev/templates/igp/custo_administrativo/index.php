...
function load()
{
	...
	$.post( '<?php echo base_url() . index_page(); ?>/igp/custo_administrativo/listar',{  nome: $('#nome').val()
...
function novo()
{
	location.href='<?php echo site_url("igp/custo_administrativo/detalhe/0"); ?>';
}
...
function gerar_graficos()
{
	...
	{
		url = '<?php echo site_url("igp/custo_administrativo/criar_indicador"); ?>';
...
{
	url = '<?php echo site_url("indicador/apresentacao/detalhe/".enum_indicador::CUSTO_ADMINISTRATIVO); ?>';
...
function fechar_periodo()
{
	...
	else if( confirm('Fechar o período?') )
	{
		...
		$.post(url,{},function(data){
			...
			url = '<?php echo site_url("igp/custo_administrativo/fechar_periodo")?>';
			location.href=url;
		});
	}
}
