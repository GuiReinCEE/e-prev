...
function load()
{
	...
	$.post( '<?php echo base_url() . index_page(); ?>/igp/equilibrio/listar',{  nome: $('#nome').val()
...
function novo()
{
	location.href='<?php echo site_url("igp/equilibrio/detalhe/0"); ?>';
}
...
function gerar_graficos()
{
	...
	{
		url = '<?php echo site_url("igp/equilibrio/criar_indicador"); ?>';
...
{
	url = '<?php echo site_url("indicador/apresentacao/detalhe/".enum_indicador::EQUILIBRIO); ?>';
...
function fechar_periodo()
{
	...
	else if( confirm('Fechar o período?') )
	{
		...
		$.post(url,{},function(data){
			...
			url = '<?php echo site_url("igp/equilibrio/fechar_periodo")?>';
			location.href=url;
		});
	}
}
