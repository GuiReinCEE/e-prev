...
function load()
{
	...
	$.post( '<?php echo base_url() . index_page(); ?>/igp/rentabilidade_ci/listar',{},function(data){ $("#result_div").html(data);configure_result_table(); } );
}
...
function novo()
{
	location.href='<?php echo site_url("igp/rentabilidade_ci/detalhe/0"); ?>';
}
...
function gerar_graficos()
{
	...
	{
		url = '<?php echo base_url() . index_page(); ?>/igp/rentabilidade_ci/criar_indicador';
...
{
	url = '<?php echo base_url().index_page(); ?>/indicador/apresentacao/detalhe/<?php echo enum_indicador::RENTABILIDADE_CI; ?>';
	window.open(url);
}
...
function fechar_periodo()
{
	...
	{
		url = '<?php echo site_url("igp/rentabilidade_ci/criar_indicador/"); ?>';
		$.post(url,{},function(data){

			$('#output_tela').html( "Indicadores atualizados com sucesso, aguarde enquanto o período é fechado ..." );

			url = '<?php echo site_url("igp/rentabilidade_ci/fechar_periodo")?>';
			location.href=url;
		});
	}
}
