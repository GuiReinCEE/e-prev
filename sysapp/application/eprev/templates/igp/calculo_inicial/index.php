...
function load()
{
	...
	$.post( '<?php echo base_url() . index_page(); ?>/igp/calculo_inicial/listar',{  nome: $('#nome').val()
...
function novo()
{
	location.href='<?php echo site_url("igp/calculo_inicial/detalhe/0"); ?>';
}
...
function gerar_graficos()
{
	...
	{
		url = '<?php echo site_url("igp/calculo_inicial/criar_indicador"); ?>';
...
function fechar_periodo()
{
	...
	else if( confirm('Fechar o período?') )
	{
		...
		$.post(url,{},function(data){
			...
			url = '<?php echo site_url("igp/calculo_inicial/fechar_periodo")?>';
			location.href=url;
		});
	}
}
