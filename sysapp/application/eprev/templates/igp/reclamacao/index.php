...
function load()
{
	...
	$.post( '<?php echo base_url() . index_page(); ?>/igp/reclamacao/listar',{},function(data){ $("#result_div").html(data);configure_result_table(); } );
}
...
function configure_result_table()
{
	...
	[
		'Number',null,null,null,null,null,null,null,null,null,null,null,null,null,null,null,null
	]);
...
function novo()
{
	location.href='<?php echo site_url("igp/reclamacao/detalhe/0"); ?>';
}
...
function gerar_graficos()
{
	...
	{
		url = '<?php echo base_url() . index_page(); ?>/igp/reclamacao/criar_indicador';
...
{
	url = '<?php echo site_url("indicador/apresentacao/detalhe/".enum_indicador::RECLAMACAO); ?>';
	window.open(url);
}
...
function fechar_periodo()
{
	...
	{
		url = '<?php echo site_url("igp/reclamacao/criar_indicador/"); ?>';
		$.post(url,{},function(data){

			$('#output_tela').html( "Indicadores atualizados com sucesso, aguarde enquanto o período é fechado ..." );

			url = '<?php echo site_url("igp/reclamacao/fechar_periodo")?>';
			location.href=url;
		});
	}
}
...
$config['button'][]=array('Atualizar apresentação', 'gerar_graficos()');
