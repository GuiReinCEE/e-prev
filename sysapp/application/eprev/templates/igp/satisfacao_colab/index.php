...
function load()
{
	...
	$.post( '<?php echo base_url() . index_page(); ?>/igp/satisfacao_colab/listar',{  nome: $('#nome').val()
...
function novo()
{
	location.href='<?php echo site_url("igp/satisfacao_colab/detalhe/0"); ?>';
}
...
function gerar_graficos()
{
	...
	{
		url = '<?php echo site_url("igp/satisfacao_colab/criar_indicador"); ?>';
...
{
	url = '<?php echo site_url("indicador/apresentacao/detalhe/".enum_indicador::SATISFACAO_COLABORADOR); ?>';
...
{
	url = '<?php echo site_url("indicador/tabela/index/".enum_indicador::SATISFACAO_COLABORADOR); ?>';
	location.href = url;
}
...
function fechar_periodo()
{
	...
	else if( confirm('Fechar o período?') )
	{
		...
		$.post(url,{},function(data){
			...
			url = '<?php echo site_url("igp/satisfacao_colab/fechar_periodo")?>';
			location.href=url;
		});
	}
}
