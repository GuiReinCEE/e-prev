...
function load()
{
	...
	$.post( '<?php echo base_url() . index_page(); ?>/igp/treinamento/listar',{  nome: $('#nome').val()
...
function novo()
{
	location.href='<?php echo site_url("igp/treinamento/detalhe/0"); ?>';
}
...
function gerar_graficos()
{
	...
	{
		url = '<?php echo site_url("igp/treinamento/criar_indicador"); ?>';
...
{
	url = '<?php echo site_url("indicador/apresentacao/detalhe/".enum_indicador::TREINAMENTO); ?>';
...
function fechar_periodo()
{
	...
	else if( confirm('Fechar o período?') )
	{
		...
		$.post(url,{},function(data){
			...
			url = '<?php echo site_url("igp/treinamento/fechar_periodo")?>';
			location.href=url;
		});
	}
}
