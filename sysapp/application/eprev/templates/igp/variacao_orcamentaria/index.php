...
function load()
{
	...
	$.post( '<?php echo base_url() . index_page(); ?>/igp/variacao_orcamentaria/listar',{  nome: $('#nome').val()
...
function gerar_graficos()
{
	...
	{
		url = '<?php echo site_url("igp/variacao_orcamentaria/criar_indicador"); ?>';
...
{
	url = '<?php echo site_url("indicador/apresentacao/detalhe/".enum_indicador::REALIZACAO_ORCAMENTARIA); ?>';
...
function fechar_periodo()
{
	...
	else if( confirm('Fechar o período?') )
	{
		...
		$.post(url,{},function(data){
			...
			url = '<?php echo site_url("igp/variacao_orcamentaria/fechar_periodo")?>';
			location.href=url;
		});
	}
}
