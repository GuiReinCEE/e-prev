...
<script>
	...
	<?php echo form_default_js_submit(array(
		...
		,array("nr_orcado", "float") 
		,array("nr_realizado", "float") 
		,array("nr_meta", "float") 
		,array("nr_peso", "float") 
	),'_salvar(form)');	?>
...
echo form_hidden( 'cd_variacao_orcamentaria', intval($row['cd_variacao_orcamentaria']) );
...
echo form_default_float("nr_orcado", $label_1." *", app_decimal_para_php($row['nr_orcado']), "class='indicador_text'");
echo form_default_float("nr_realizado", $label_2." *", app_decimal_para_php($row['nr_realizado']), "class='indicador_text'");
