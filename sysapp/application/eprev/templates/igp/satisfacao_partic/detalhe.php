...
<script>
	...
	function ir_lista()
	{
		location.href='<?php echo site_url("igp/satisfacao_partic"); ?>';
	}
...
echo form_open('igp/satisfacao_partic/salvar');
echo form_hidden( 'cd_satisfacao_partic', intval($row['cd_satisfacao_partic']) );
...
echo form_default_float("nr_satisfacao", $label_1." *", app_decimal_para_php($row['nr_satisfacao']), "class='indicador_text'");
echo form_default_float("nr_meta", $label_2." *", app_decimal_para_php($row['nr_meta']), "class='indicador_text'");
echo form_default_float("nr_peso", $label_3." *", app_decimal_para_php($row['nr_peso']), "class='indicador_text'");
...
if( intval($row['cd_satisfacao_partic'])>0  )
{
	echo button_delete("igp/satisfacao_partic/excluir",$row["cd_satisfacao_partic"]);
}
