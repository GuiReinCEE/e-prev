<?php
class manual_investimento_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT ce.cd_manual_investimento,
			       TO_CHAR(ce.dt_referencia, 'DD/MM/YYYY') AS dt_referencia,
			       ce.arquivo,
			       ce.arquivo_nome,
			       (CASE WHEN (SELECT ce2.cd_manual_investimento
			                     FROM gestao.manual_investimento ce2
			                    WHERE ce2.dt_exclusao IS NULL
			                    ORDER BY ce2.dt_referencia DESC 
			                    LIMIT 1) = ce.cd_manual_investimento
			             THEN 'S'
			             ELSE 'N'
			        END) AS fl_editar
			  FROM gestao.manual_investimento ce
	...
	public function carrega($cd_manual_investimento)
	{
		$qr_sql = "
			SELECT cd_manual_investimento,
			       TO_CHAR(dt_referencia, 'DD/MM/YYYY') AS dt_referencia,
			       arquivo,
			       arquivo_nome,
			       TO_CHAR(dt_envio, 'DD/MM/YYYY HH24:MI:SS') AS dt_envio,
			       funcoes.get_usuario_nome(cd_usuario_envio) AS ds_usuario_envio
			  FROM gestao.manual_investimento 
			 WHERE cd_manual_investimento = ".intval($cd_manual_investimento).";";
	...
	{
		$qr_sql = "
			INSERT INTO gestao.manual_investimento
			     (
			       dt_referencia,
			       arquivo,
			       arquivo_nome,
	...
	public function atualizar($cd_manual_investimento, $args = array())
	{
		$qr_sql = "
			UPDATE gestao.manual_investimento
	...
	public function get_manual_investimento()
	{
		$qr_sql = "
			SELECT arquivo, 
			       arquivo_nome
	...
	public function enviar($cd_manual_investimento, $cd_usuario)
	{
		$qr_sql = "
			UPDATE gestao.manual_investimento
