<?php
class manual_emprestimo_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT ce.cd_manual_emprestimo,
			       TO_CHAR(ce.dt_referencia, 'DD/MM/YYYY') AS dt_referencia,
			       ce.arquivo,
			       ce.arquivo_nome,
			       (CASE WHEN (SELECT ce2.cd_manual_emprestimo
			                     FROM gestao.manual_emprestimo ce2
			                    WHERE ce2.dt_exclusao IS NULL
			                    ORDER BY ce2.dt_referencia DESC 
			                    LIMIT 1) = ce.cd_manual_emprestimo
			             THEN 'S'
			             ELSE 'N'
			        END) AS fl_editar
			  FROM gestao.manual_emprestimo ce
	...
	public function carrega($cd_manual_emprestimo)
	{
		$qr_sql = "
			SELECT cd_manual_emprestimo,
			       TO_CHAR(dt_referencia, 'DD/MM/YYYY') AS dt_referencia,
			       arquivo,
			       arquivo_nome,
			       TO_CHAR(dt_envio, 'DD/MM/YYYY HH24:MI:SS') AS dt_envio,
			       funcoes.get_usuario_nome(cd_usuario_envio) AS ds_usuario_envio
			  FROM gestao.manual_emprestimo 
			 WHERE cd_manual_emprestimo = ".intval($cd_manual_emprestimo).";";
	...
	{
		$qr_sql = "
			INSERT INTO gestao.manual_emprestimo
			     (
			       dt_referencia,
			       arquivo,
			       arquivo_nome,
	...
	public function atualizar($cd_manual_emprestimo, $args = array())
	{
		$qr_sql = "
			UPDATE gestao.manual_emprestimo
	...
	public function get_manual_emprestimo()
	{
		$qr_sql = "
			SELECT arquivo, 
			       arquivo_nome
	...
	public function enviar($cd_manual_emprestimo, $cd_usuario)
	{
		$qr_sql = "
			UPDATE gestao.manual_emprestimo
