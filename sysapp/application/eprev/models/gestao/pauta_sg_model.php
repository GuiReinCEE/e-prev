...
class Pauta_sg_model extends Model
{
	...
	public function listar_ano($nr_ano, $fl_sumula)
    {
    	$qr_sql = "
            SELECT p.cd_pauta_sg,
                   p.nr_ata,
              ...
              FROM gestao.pauta_sg p
              WHERE p.dt_exclusao IS NULL
			    AND TO_CHAR(p.dt_pauta_sg, 'YYYY')::integer = ".intval($nr_ano)."
				AND p.fl_sumula = '".trim($fl_sumula)."'
			  ORDER BY p.nr_ata ASC;";

    	return $this->db->query($qr_sql)->result_array();
    }
	
	public function sumula_diretoria($cd_pauta_sg)
	{
		$qr_sql = "
            SELECT s.cd_sumula,
			  ...
			  JOIN gestao.sumula s 
			    ON s.nr_sumula    = p.nr_ata
             WHERE p.fl_sumula = 'DE'
			   AND p.cd_pauta_sg = ".intval($cd_pauta_sg).";";

        return $this->db->query($qr_sql)->row_array();
	}

	public function sumula_conselho_fiscal($cd_pauta_sg)
	{
		$qr_sql = "
            SELECT s.cd_sumula_conselho_fiscal,
			       MD5('DE'||s.cd_sumula_conselho_fiscal::TEXT) AS cd_sumula_md5,
                   s.nr_sumula_conselho_fiscal,
                   TO_CHAR(s.dt_sumula_conselho_fiscal,'DD/MM/YYYY') AS dt_sumula,
                   TO_CHAR(s.dt_divulgacao,'DD/MM/YYYY') AS dt_divulgacao,
                   TO_CHAR(s.dt_publicacao_libera,'DD/MM/YYYY') AS dt_publicacao_libera,
                   TO_CHAR(s.dt_publicacao,'DD/MM/YYYY HH24:MI:SS') AS dt_publicacao,
				   funcoes.get_usuario_nome(s.cd_usuario_publicacao) AS usuario_publicacao,
			  ...
			  JOIN gestao.sumula_conselho_fiscal s 
			    ON s.nr_sumula_conselho_fiscal    = p.nr_ata
             WHERE p.fl_sumula = 'CF'
			   AND p.cd_pauta_sg = ".intval($cd_pauta_sg).";";

        return $this->db->query($qr_sql)->row_array();
	}
	
	public function sumula_conselho_deliberativo($cd_pauta_sg)
	{
		$qr_sql = "
            SELECT s.cd_sumula_conselho,
			       MD5('DE'||s.cd_sumula_conselho::TEXT) AS cd_sumula_md5,
                   s.nr_sumula_conselho,
                   TO_CHAR(s.dt_sumula_conselho,'DD/MM/YYYY') AS dt_sumula,
                   TO_CHAR(s.dt_divulgacao,'DD/MM/YYYY') AS dt_divulgacao,
                   TO_CHAR(s.dt_publicacao_libera,'DD/MM/YYYY') AS dt_publicacao_libera,
                   TO_CHAR(s.dt_publicacao,'DD/MM/YYYY HH24:MI:SS') AS dt_publicacao,
				   funcoes.get_usuario_nome(s.cd_usuario_publicacao) AS usuario_publicacao,
			  ...
			  JOIN gestao.sumula_conselho s 
			    ON s.nr_sumula_conselho    = p.nr_ata
             WHERE p.fl_sumula = 'CD'
			   AND p.cd_pauta_sg = ".intval($cd_pauta_sg).";";
        ...
        $qr_sql = "
            ...
            VALUES 
                 (
                    ...
                    ".(trim($args['dt_pauta_sg']) != '' ? 
                        (trim($args['fl_sumula']) == 'CD'
                            ? "TO_DATE('".trim($args['dt_pauta_sg'])."', 'DD/MM/YYYY') - '6 days'::interval"
                            : "funcoes.dia_util('ANTES', TO_DATE('".trim($args['dt_pauta_sg'])."', 'DD/MM/YYYY'), 3)" 
                            )
                    : "DEFAULT").",
                    ".(trim($args['dt_pauta_sg']) != '' ? 
                        (trim($args['fl_sumula']) == 'CD'
    ...
    public function delete_integrante_presente($cd_pauta_sg, $cd_usuario)
    {
        $qr_sql = "
        ...
        $qr_sql = "
            SELECT psip.cd_pauta_sg_integrante_presente,
                   ...
                   (CASE WHEN psip.fl_tipo = 'S' THEN 'Suplente'
                         ELSE 'Titular'
                    END) AS ds_tipo,
                    (CASE WHEN psip.fl_indicado_eleito = 'I' THEN 'Indicado'
                         WHEN psip.fl_indicado_eleito = 'E' THEN 'Eleito'
                     ELSE ''
    ...
    public function get_presentes($cd_pauta_sg)
    {
        $qr_sql = "
            SELECT psip.cd_pauta_sg_integrante_presente AS cd_titular,
                   ...
                   (CASE WHEN psip.fl_indicado_eleito = 'I'
                         THEN 2
                         WHEN psip.fl_indicado_eleito = 'E'
                         THEN 1
                         ELSE 0
                   END) AS nr_indicador_eleito,
			  ...
			  JOIN gestao.pauta_sg ps
			    ON ps.cd_pauta_sg = psip.cd_pauta_sg
              LEFT JOIN gestao.pauta_sg_integrante_presente psip2
                ON psip2.cd_pauta_sg_integrante_presente_titular = psip.cd_pauta_sg_integrante_presente
               AND psip2.dt_exclusao IS NULL
             WHERE psip.dt_exclusao IS NULL
               AND psip.cd_pauta_sg = ".intval($cd_pauta_sg)."
               AND psip.fl_tipo     != 'S'
             ORDER BY nr_presidente desc, nr_indicador_eleito desc, psip.ds_pauta_sg_integrante_presente;";
        ...
        $qr_sql = "
            UPDATE gestao.pauta_sg
               SET nr_ata               = ".(trim($args['nr_ata']) != '' ? intval($args['nr_ata']) : "DEFAULT").",
                   ...
                   dt_limite            = ".(trim($args['dt_pauta_sg']) != '' ? 
                                                (trim($args['fl_sumula']) == 'CD'
                                                    ? "TO_DATE('".trim($args['dt_pauta_sg'])."', 'DD/MM/YYYY') - '6 days'::interval"
                                                    : "funcoes.dia_util('ANTES', TO_DATE('".trim($args['dt_pauta_sg'])."', 'DD/MM/YYYY'), 3)" 
                                                )
                                          : "DEFAULT").",
                   dt_limite_anexo      = ".(trim($args['dt_pauta_sg']) != '' ? 
                                                (trim($args['fl_sumula']) == 'CD'
                                                    ? "TO_DATE('".trim($args['dt_pauta_sg'])."', 'DD/MM/YYYY') - '6 days'::interval"
                                                    : "funcoes.dia_util('ANTES', TO_DATE('".trim($args['dt_pauta_sg'])."', 'DD/MM/YYYY'), 3)" 
                                                )
    ...
    public function anexos_assuntos_removidos($cd_pauta_sg)
    {
    	$qr_sql = "
    		SELECT pa.nr_item_sumula,
			       paa.arquivo,
			       funcoes.remove_acento(paa.arquivo_nome) AS arquivo_nome
    ...
    public function assunto_carrega($cd_pauta_sg_assunto)
    {
        $qr_sql = "
            SELECT a.cd_pauta_sg_assunto,
                   ...
                   gestao.nr_controle_rds(nr_ano_rds, nr_rds) AS nr_ano_numero_rds,
                   TO_CHAR(a.dt_encerramento, 'DD/MM/YYYY HH24:MI:SS') AS dt_encerramento,
                   TO_CHAR(a.dt_reaberto, 'DD/MM/YYYY HH24:MI:SS') AS dt_reaberto,
                   d.ds_diretoria,
                   (SELECT COUNT(*)
                      FROM gestao.pauta_sg_assunto_anexo an
                     WHERE an.dt_exclusao IS NULL
    ...
    public function anexo_listar($cd_pauta_sg_assunto, $fl_rds = '', $fl_ordem_fornecimento = '', $fl_quadro_comparativo = '')
    {
        $qr_sql = "
            SELECT a.cd_pauta_sg_assunto_anexo,
                   ...
                   TO_CHAR(COALESCE(a.dt_alteracao, a.dt_inclusao), 'DD/MM/YYYY HH24:MI:SS') AS dt_inclusao,
    ...
    {
        $cd_pauta_sg_assunto_anexo = intval($this->db->get_new_id('gestao.pauta_sg_assunto_anexo', 'cd_pauta_sg_assunto_anexo'));
		
		$qr_sql = "
            INSERT INTO gestao.pauta_sg_assunto_anexo
                 (
                    cd_pauta_sg_assunto_anexo,
					cd_pauta_sg_assunto,
                    arquivo,
                    arquivo_nome,
                    fl_rds,
            ...
            VALUES
                 (
                    ".intval($cd_pauta_sg_assunto_anexo).",
                    ".intval($args['cd_pauta_sg_assunto']).",
                    '".trim($args['arquivo'])."',
                    funcoes.remove_acento('".trim($args['arquivo_nome'])."'),
		...
		return $cd_pauta_sg_assunto_anexo;
    }
        ...
        $qr_sql = "
            UPDATE gestao.pauta_sg_assunto_anexo
               SET arquivo                = '".trim($args['arquivo'])."',
                   arquivo_nome           = funcoes.remove_acento('".trim($args['arquivo_nome'])."'),
    ...
    public function historico_listar($cd_pauta_sg_assunto_anexo)
    {
        $qr_sql = "
            SELECT a.cd_pauta_sg_assunto_anexo,
                   a.arquivo,
                   ...
                   TO_CHAR(COALESCE(a.dt_alteracao, a.dt_inclusao), 'DD/MM/YYYY HH24:MI:SS') AS dt_inclusao,
                   funcoes.get_usuario_nome(a.cd_usuario_inclusao) AS ds_usuario
              FROM gestao.pauta_sg_assunto_anexo_rds_historico a
    ...
    public function set_historico_rds($cd_pauta_sg_assunto_anexo, $args = array())
    {
        $qr_sql = "
            INSERT INTO gestao.pauta_sg_assunto_anexo_rds_historico
                 (
                    cd_pauta_sg_assunto_anexo, 
                    cd_pauta_sg_assunto, 
                    arquivo, 
    ...
    public function pesquisa_listar($args = array())
    {
        $qr_sql = "
            SELECT p.cd_pauta_sg,
                   ...
                   END) AS ds_tipo_reuniao,
                   (SELECT COUNT(*)
                      FROM gestao.pauta_sg_assunto_anexo pa
                     WHERE pa.cd_pauta_sg_assunto = a.cd_pauta_sg_assunto
                       AND pa.dt_exclusao IS NULL) AS qt_anexo
              FROM gestao.pauta_sg_assunto a
    ...
    public function getPautaAssinatura(&$result, $args)
    {
        $qr_sql = "
					SELECT p.cd_pauta_sg,
					  ...
					  JOIN clicksign.documento d
						ON d.cd_gestao_tipo      = 'PAUTA'
					   AND d.cd_gestao_colegiado = p.fl_sumula
					   AND d.nr_gestao_doc       = p.nr_ata
                     WHERE p.cd_pauta_sg = 	".intval($args['cd_pauta_sg'])."		
