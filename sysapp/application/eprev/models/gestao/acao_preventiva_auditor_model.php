...
class acao_preventiva_auditor_model extends Model
{
    ...
    function listar(&$result, $args=array())
    {
        $qr_sql = "
                   SELECT p.cd_processo,
                     ...
                     LEFT JOIN gestao.acao_preventiva_auditor nca
    ...
    {
        $qr_sql = "
                   SELECT apa.cd_usuario_titular,
                          apa.cd_usuario_substituto,
                          p.procedimento
                     FROM projetos.processos p
                     LEFT JOIN gestao.acao_preventiva_auditor apa
                       ON apa.cd_processo = p.cd_processo                    
    ...
    function usuario_auditor(&$result, $args=array())
    {
        $qr_sql = "SELECT uc.codigo AS value,
                    ...
                    WHERE uc.indic_12 = '*'
                      AND (tipo <> 'X' OR 0 < (SELECT COUNT(apa.*) 
                                                 FROM gestao.acao_preventiva_auditor apa
                                                WHERE apa.cd_usuario_titular = uc.codigo 
                                                   OR apa.cd_usuario_substituto = uc.codigo))
                    ORDER BY uc.nome";
        ...
        $qr_sql = "SELECT codigo AS value,
                    ...
                    WHERE indic_12 = '*'
                      AND (tipo <> 'X' OR 0 < (SELECT COUNT(*) 
                                                 FROM gestao.acao_preventiva_auditor
    ...
    {        
        ...
        {
            $qr_sql = "UPDATE gestao.acao_preventiva_auditor
        ...
        {
            $qr_sql = "INSERT INTO gestao.acao_preventiva_auditor
                              (
