...
class Contribuicao_programada_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT cd_contribuicao_programada,
				   cd_empresa,
				   ...
				   vl_valor_anterior as vl_anterior
			  FROM autoatendimento.contribuicao_programada 
	...
	public function confirma($cd_contribuicao_programada, $cd_usuario)
	{
		$qr_sql = "
			UPDATE autoatendimento.contribuicao_programada
			   SET cd_usuario_confirmacao = ".intval($cd_usuario).",
			   	   dt_confirmacao         = CURRENT_TIMESTAMP
			 WHERE cd_contribuicao_programada = ".intval($cd_contribuicao_programada).";

		
			SELECT oracle.fnc_atualiza_vlr_contrib_web(
    			   	cp.cd_empresa::integer,
				   	cp.cd_registro_empregado::integer,
				   	cp.seq_dependencia::integer,
				   	cp.dt_inicio::date,
				   	cp.vl_valor)
	...
	public function cancelar($cd_contribuicao_programada, $ip_cancelado, $cd_usuario)
	{
		$qr_sql = "
			UPDATE autoatendimento.contribuicao_programada
