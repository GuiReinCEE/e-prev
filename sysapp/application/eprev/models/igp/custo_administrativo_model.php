<?php
class Custo_administrativo_model extends Model
{
	...
	{
		$qr_sql = "
					SELECT i.cd_custo_administrativo,
					       ...
					       i.nr_despesa_percentual,
					       i.nr_contribuicao_acumulado,
					       i.nr_despesa_acumulado,
					       i.nr_despesa_percentual_acumulado,
					       i.nr_resultado_meta,
					       i.nr_referencia_mes,
					       i.nr_despesa_percentual_acumulado_meta, 
	...
	{
		$qr_sql = " 
					SELECT cd_custo_administrativo,
					 ...
					 WHERE cd_custo_administrativo = ".intval($cd)."
			      ";
...
function salvar($args,&$msg=array())
	{
		if(intval($args['cd_custo_administrativo']) == 0)
		{
			$qr_sql = "
						INSERT INTO igp.custo_administrativo 
						     ( 
							    dt_referencia,
								nr_contribuicao,
								nr_despesa, 
					    ...
					    VALUES 
						     ( 
								...
								".(trim($args["nr_contribuicao"]) == "" ? "DEFAULT" : floatval($args["nr_contribuicao"])).",
								".(trim($args["nr_despesa"]) == "" ? "DEFAULT" : floatval($args["nr_despesa"])).",
		...
		{
			$qr_sql = "
						UPDATE igp.custo_administrativo 
						   SET dt_referencia        = ".(trim($args["dt_referencia"]) == "" ? "DEFAULT" : "TO_DATE('".trim($args["dt_referencia"])."', 'DD/MM/YYYY')").",
                               nr_contribuicao      = ".(trim($args["nr_contribuicao"]) == "" ? "DEFAULT" : floatval($args["nr_contribuicao"])).",
                               nr_despesa           = ".(trim($args["nr_despesa"]) == "" ? "DEFAULT" : floatval($args["nr_despesa"])).",
			             ...
			             WHERE cd_custo_administrativo = ".intval($args['cd_custo_administrativo'])."
			          ";			
		}
	...
	function excluir($id)
	{
		$qr_sql = " 
					UPDATE igp.custo_administrativo 
		             ...
		             WHERE MD5(cd_custo_administrativo::TEXT) = '".trim($id)."' 
		          "; 
		$this->db->query($qr_sql);
	}	
}
