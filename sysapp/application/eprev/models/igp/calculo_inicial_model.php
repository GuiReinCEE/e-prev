...
class Calculo_inicial_model extends Model
{
	...
	function carregar($cd)
	{
		$qr_sql = " 
					SELECT cd_calculo_inicial,
					 ...
					 WHERE cd_calculo_inicial = ".intval($cd)."
			      ";
	...
	function salvar($args,&$msg=array())
	{
		...
		{
			$qr_sql = "
						INSERT INTO igp.calculo_inicial 
						     ( 
							    dt_referencia,
								nr_concedido,
								nr_erro, 
		...
		else
		{
			$qr_sql = "
						UPDATE igp.calculo_inicial 
			             ...
			             WHERE cd_calculo_inicial = ".intval($args['cd_calculo_inicial'])."
			          ";			
		}
	...
	function excluir($id)
	{
		$qr_sql = " 
					UPDATE igp.calculo_inicial 
		             ...
		             WHERE MD5(cd_calculo_inicial::TEXT) = '".trim($id)."' 
		          "; 
		$this->db->query($qr_sql);
	}	
}
