...
class Contribuicao_instituidor_mensal_model extends Model
{
	...
	function mensal(&$result, $args=array())
	{
		...
		$qr_sql = "
					SELECT CASE WHEN b.cd_empresa IN (8,10,12,19,20,24,25,26,27,28,29,30,31) AND b.codigo_lancamento = 2502 THEN 'PMBDL'
					...
					SELECT CASE WHEN c.cd_empresa IN (8,10,12,19,20,24,25,26,27,28,29,30,31) AND c.codigo_lancamento = 2501 THEN 'PMDCC'
					 ...
					 WHERE c.cd_plano   = ".intval($args['cd_plano'])."
					   ...
					   /*
					  AND funcoes.cripto_re(c.cd_empresa,c.cd_registro_empregado,c.seq_dependencia) NOT IN (
					  funcoes.cripto_re(25,108,0)
	...
	function mensalParticipantes(&$result, $args=array())
	{
		...
		$qr_sql = "
					SELECT DISTINCT b.cd_empresa, b.cd_registro_empregado, b.seq_dependencia, p.nome,
					       CASE WHEN b.cd_empresa IN (8,10,12,19,20,24,25,26,27,28,29,30,31) AND b.codigo_lancamento = 2502 THEN 'PMBDL'
								WHEN b.cd_empresa = 7 AND b.codigo_lancamento = 2400 THEN 'PMBDL'
								ELSE 'ERRO'
					       END AS tp_pagamento,
					       '' AS ds_instituto
					...
					SELECT DISTINCT c.cd_empresa, c.cd_registro_empregado, c.seq_dependencia, p.nome,
						   CASE WHEN c.cd_empresa IN (8,10,12,19,20,24,25,26,27,28,29,30,31) AND c.codigo_lancamento = 2501 THEN 'PMDCC'
					            WHEN c.cd_empresa = 7 AND c.codigo_lancamento = 2410 THEN 'PMDCC'
					       ELSE 'ERRO'
					       END AS tp_pagamento,
					       '' AS ds_instituto				
	...
	function sem_email(&$result, $args=array())
	{
		$qr_sql = "
					SELECT p.cd_empresa, 
					       ...
					       p.seq_dependencia,
						   p.nome,
                           CASE WHEN b.cd_empresa IN (8,10,12,19,20,24,25,26,27,28,29,30,31) AND b.codigo_lancamento = 2502 THEN 'BDL'
					...
					SELECT p.cd_empresa, 
					       ...
					       p.seq_dependencia,
						   p.nome,
						   CASE WHEN c.cd_empresa IN (8,10,12,19,20,24,25,26,27,28,29,30,31) AND c.codigo_lancamento = 2501 THEN 'BCO'
...
/*					   
AND funcoes.cripto_re(p.cd_empresa,p.cd_registro_empregado,p.seq_dependencia) NOT IN (
					  funcoes.cripto_re(8,7722,0),
					  funcoes.cripto_re(8,15270,0),
					  funcoes.cripto_re(8,10782,0),
					  funcoes.cripto_re(8,3662,0),
					  funcoes.cripto_re(8,16659,0),
					  funcoes.cripto_re(8,9539,0)
					  funcoes.cripto_re(8,18589,0)
					  funcoes.cripto_re(8,20745,0)
					  funcoes.cripto_re(8,2607,0)
