...
class Atendimento_model extends Model
{
	...
	function listar( &$result, $args=array() )
	{
		$qr_sql = "
					SELECT a.cd_atendimento,
						   ...
						   COALESCE(INITCAP(oa.desc_origem_atendimento),'Não Informado') AS tp_atendimento,
					  ...
					  LEFT JOIN public.origens_atendimentos oa
					    ON oa.cd_origem_atendimento = a.indic_ativo
					 ...
					 WHERE CAST(a.dt_hora_inicio_atendimento AS  DATE) BETWEEN TO_DATE('".trim($args["dt_inicio"])."','DD/MM/YYYY')  AND TO_DATE('".trim($args["dt_fim"])."','DD/MM/YYYY')
					   ...
					   ".(trim($args["tipo_atendimento"]) == 'A'      ? "AND (a.indic_ativo IN ('P', 'E') OR (a.indic_ativo = 'T' AND (CASE WHEN TRIM(COALESCE(act.nome_arquivo,'')) <> '' 
								 ...
								 ELSE TRIM(COALESCE('xcally/' || acx.nome_arquivo,''))
						   END) != ''))" : "")."
					   ".(trim($args["tipo_atendimento"]) != ''      ? "AND a.indic_ativo = '".trim($args["tipo_atendimento"])."'" : "")."
	...
	function listar_atendimento_tipo_dd()
	{
		$qr_sql = "
					SELECT oa.cd_origem_atendimento AS value, 
					       COALESCE(INITCAP(oa.desc_origem_atendimento),'Não Informado') AS text
					  FROM public.origens_atendimentos oa
					 ORDER BY text;				  
		          ";		  
	...
	function integracaoTipoAtendimentoELETRO(&$result, $args=array())
    {
		$qr_sql = "
					UPDATE projetos.atendimento
					   SET indic_ativo    = '".$args['tipo_atendimento']."'
					 WHERE cd_atendimento = ".intval($args['cd_atendimento'])."
			      ";
		$result = $this->db->query($qr_sql);
	}	
