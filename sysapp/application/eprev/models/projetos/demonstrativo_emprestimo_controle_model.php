<?php
class demonstrativo_emprestimo_controle_model extends Model
{
		...
		$qr_sql = "
			SELECT pp.sigla AS ds_empresa,
				   ...
				   CASE WHEN ccc.dt_envio_email IS NOT NULL THEN ''
						...
						ELSE ''
				   END AS class_status,
                   oracle.fnc_retorna_opcao(d.cd_empresa::INTEGER, d.cd_registro_empregado::INTEGER, d.seq_dependencia::INTEGER, 'WEB0001', COALESCE(ccc.dt_inclusao::DATE, CURRENT_DATE)) AS fl_eletronico			   
			  ...
			  LEFT JOIN projetos.demonstrativo_emprestimo_controle ccc 
			   ...
			   AND ccc.dt_documento          = d.dt_documento
			   AND ccc.dt_exclusao IS NULL
			 WHERE d.cd_tipo_doc = 215 -- DEMONSTRATIVO DE EMPRESTIMO DO CALLCENTER
			   AND d.dt_documento BETWEEN TO_DATE('".$args['dt_inicio']."','DD/MM/YYYY') AND TO_DATE('".$args['dt_final']."','DD/MM/YYYY')
			   ".(trim($args['fl_eletronico']) != "" ? "AND oracle.fnc_retorna_opcao(d.cd_empresa::INTEGER, d.cd_registro_empregado::INTEGER, d.seq_dependencia::INTEGER, 'WEB0001', COALESCE(ccc.dt_inclusao::DATE, CURRENT_DATE)) = '".trim($args['fl_eletronico'])."'" : "")."
    ...
    {
		$qr_sql = "
			INSERT INTO projetos.demonstrativo_emprestimo_controle
				 (
					cd_empresa, 
			...
			SELECT p.cd_empresa,
				   ...
				   (SELECT d.dt_documento 
					  ...
					  LEFT JOIN projetos.demonstrativo_emprestimo_controle ccc 
					   ...
					   AND ccc.dt_documento          = d.dt_documento
					   AND ccc.dt_exclusao IS NULL
					 WHERE d.cd_tipo_doc           = 215
					   AND ccc.dt_documento IS NULL
			...
			SELECT rotinas.email_demonstrativo_emprestimo_controle(".intval($args['cd_usuario']).");";
