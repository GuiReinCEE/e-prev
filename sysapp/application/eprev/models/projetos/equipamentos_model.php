...
class Equipamentos_model extends Model
{
		...
		$sql = "
			...
			$where_login
						  ...
						  UNION ALL
							...
							SELECT eq.codigo_patrimonio AS nr_patrimonio,
								   'SERVIDOR VIRTUAL' AS ds_equipamento,
								   NULL AS dt_equipamento,
								   NULL AS dt_baixa_bi,
								   TO_CHAR(eq.dt_baixa, 'DD/MM/YYYY') AS dt_baixa,
								   l1.descricao AS ds_situacao, 
								   eq.sistema_operacional, 
								   ...
								   TO_CHAR(uc.dt_hora_scanner_computador, 'DD/MM/YY HH24:MI') AS dt_rede,
								   -- CPU SCANNER
								   CASE WHEN eq.tipo_equipamento IN (1,8,11)
										THEN CASE WHEN eq.tipo_equipamento = 8 
												  THEN 'S'
							  ...
							  LEFT JOIN projetos.usuarios_controledi uc
								ON uc.codigo = eq.usuario
							 WHERE eq.codigo_patrimonio >= 999999
	...
	function resumoTipoEquipamento(&$result, $args=array())
	{
		$qr_sql = "
					SELECT x.tipo_equipamento,
					 ...
					 GROUP BY x.tipo_equipamento,
							  x.codigo                  
					 ORDER BY x.tipo_equipamento 									  
		          ";
		$result = $this->db->query($qr_sql);
	}	
	...
	function resumoSituacaoEquipamento(&$result, $args=array())
	{
		$qr_sql = "
					SELECT x.situacao_equipamento,
					 ...
					 GROUP BY x.situacao_equipamento,
							  x.codigo                  
					 ORDER BY x.situacao_equipamento 
		          ";		
		$result = $this->db->query($qr_sql);
	}		
	...
	function resumoDiscoEquipamento(&$result, $args=array())
	{
		$qr_sql = "
					SELECT x.tipo_equipamento,
						   x.codigo,
						   SUM(x.espaco_disco_total) AS espaco_disco_total,
						   SUM(x.espaco_disco_livre) AS espaco_disco_livre,
						   SUM(x.espaco_disco_usado) AS espaco_disco_usado
					  FROM 
						 (		
							SELECT l2.descricao AS tipo_equipamento,
								   l2.codigo,
	...
	{
		$qr_sql = "
					SELECT x.memoria_ram_categoria,
						   x.memoria_ram_categoria_ordem,
						   SUM(x.quantidade) AS quantidade
					  FROM 
						 (			
						 ...
						 GROUP BY x.memoria_ram_categoria,
								  x.memoria_ram_categoria_ordem 							 
						 ORDER BY x.memoria_ram_categoria_ordem 
		          ";
		$result = $this->db->query($qr_sql);
	}	
	...
	{
		$qr_sql = "
					SELECT x.processador_categoria,
						   SUM(x.quantidade) AS quantidade
					  FROM 
						 (		
				     ...
				     GROUP BY x.processador_categoria						 
					 ORDER BY quantidade 
		          ";
		$result = $this->db->query($qr_sql);
	}	
	...
	{
		$qr_sql = "
					SELECT x.sistema_operacional_categoria,
						   SUM(x.quantidade) AS quantidade
					  FROM 
						 (		
				     ...
				     GROUP BY x.sistema_operacional_categoria
