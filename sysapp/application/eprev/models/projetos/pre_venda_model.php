...
class Pre_venda_model extends Model
{
		...
		$qr_sql = "
					SELECT mesano.mes, 
						   ...
						   COALESCE((SELECT COUNT(DISTINCT a.cd_registro_empregado)
									  ...
									  WHERE (CASE WHEN a.dt_transferencia_plano IS NOT NULL THEN t.dt_ingresso_eletro ELSE a.dt_ingresso_plano END)    IS NOT NULL
										AND date_part('MONTH', (CASE WHEN a.dt_transferencia_plano IS NOT NULL THEN t.dt_ingresso_eletro ELSE a.dt_ingresso_plano END)) = mesano.mes
										AND date_part('YEAR', (CASE WHEN a.dt_transferencia_plano IS NOT NULL THEN t.dt_ingresso_eletro ELSE a.dt_ingresso_plano END))  = mesano.ano
										AND DATE_TRUNC('month',(CASE WHEN a.dt_transferencia_plano IS NOT NULL THEN t.dt_ingresso_eletro ELSE a.dt_ingresso_plano END)) <> DATE_TRUNC('month', COALESCE(t.dt_desligamento_eletro, CURRENT_TIMESTAMP + '1 year'))
									  ...
									  GROUP BY date_part('MONTH', (CASE WHEN a.dt_transferencia_plano IS NOT NULL THEN t.dt_ingresso_eletro ELSE a.dt_ingresso_plano END)),
											   date_part('YEAR', (CASE WHEN a.dt_transferencia_plano IS NOT NULL THEN t.dt_ingresso_eletro ELSE a.dt_ingresso_plano END))
									  ORDER BY date_part('MONTH', (CASE WHEN a.dt_transferencia_plano IS NOT NULL THEN t.dt_ingresso_eletro ELSE a.dt_ingresso_plano END)),
											   date_part('YEAR', (CASE WHEN a.dt_transferencia_plano IS NOT NULL THEN t.dt_ingresso_eletro ELSE a.dt_ingresso_plano END))),0) AS quantos_ingresso_fceee,
	...
	public function get_protocolo_interno_inscricao($cd_empresa, $cd_registro_empregado, $seq_dependencia)
	{
		$qr_sql = "
			SELECT DISTINCT a.nr_ano,
             ...
             WHERE b.dt_exclusao IS NULL
			   AND b.cd_tipo_doc           = 225
               AND b.cd_empresa            = ".intval($cd_empresa)."
               AND b.cd_registro_empregado = ".intval($cd_registro_empregado)."
               AND b.seq_dependencia       = ".intval($seq_dependencia).";";
