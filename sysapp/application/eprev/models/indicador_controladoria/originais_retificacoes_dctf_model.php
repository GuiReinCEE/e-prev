<?php
class Originais_retificacoes_dctf_model extends Model
{
	...
	{
		$qr_sql = "
            SELECT i.cd_originais_retificacoes_dctf, 
                   ...
                   i.nr_meta
              FROM indicador_controladoria.originais_retificacoes_dctf i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia_n, 
				   nr_meta, 
				   cd_indicador_tabela 
			  FROM indicador_controladoria.originais_retificacoes_dctf
	...
	{
		$qr_sql = "
            SELECT cd_originais_retificacoes_dctf,
                   ...
                   observacao
		      FROM indicador_controladoria.originais_retificacoes_dctf 
			 WHERE cd_originais_retificacoes_dctf = ".intval($args['cd_originais_retificacoes_dctf']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
                INSERT INTO indicador_controladoria.originais_retificacoes_dctf
                     (
                       dt_referencia, 
                       cd_indicador_tabela, 
                       nr_original, 
		...
		{
			$qr_sql = "
                UPDATE indicador_controladoria.originais_retificacoes_dctf
