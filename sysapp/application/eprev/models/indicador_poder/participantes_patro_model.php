<?php
class participantes_patro_model extends Model
{
	...
	{
		$qr_sql = "
					SELECT i.cd_participantes_patro,
					  ...
					  FROM indicador_poder.participantes_patro i
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_participantes_patro=".intval($cd);
	...
	{
		if(intval($args['cd_participantes_patro'])==0)
		{
			$sql="
			INSERT INTO indicador_poder.participantes_patro
                      (
                        dt_referencia,
                        dt_inclusao,
                        cd_usuario_inclusao ,
		...
		{
			$sql="
			UPDATE indicador_poder.participantes_patro
			   SET cd_participantes_patro = ".intval($args["cd_participantes_patro"]).",
			 ...
			 WHERE cd_participantes_patro = ".intval($args["cd_participantes_patro"])."
			";
		}
	...
	function excluir($id)
	{
		...
		UPDATE indicador_poder.participantes_patro
		 ...
		 WHERE md5(cd_participantes_patro::varchar)='".$id."'
		"; 
