<?php
class acoes_preventivas_model extends Model
{
	...
	function listar( &$result, $args=array() )
	{
		$qr_sql = "
					SELECT i.cd_acoes_preventivas,
					  ...
					  FROM indicador_poder.acoes_preventivas i
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_acoes_preventivas=".intval($cd);
	...
	{
		if(intval($args['cd_acoes_preventivas'])==0)
		{
			$sql="
			INSERT INTO indicador_poder.acoes_preventivas
                      (
                        dt_referencia,
                        dt_inclusao,
                        cd_usuario_inclusao,
		...
		{
			$sql="
			UPDATE indicador_poder.acoes_preventivas
			   SET cd_acoes_preventivas = ".intval($args["cd_acoes_preventivas"]).",
			 ...
			 WHERE cd_acoes_preventivas = ".intval($args["cd_acoes_preventivas"])."
			";
		}
	...
	function excluir($id)
	{
		...
		UPDATE indicador_poder.acoes_preventivas
		 ...
		 WHERE md5(cd_acoes_preventivas::varchar)='".$id."'
		"; 
