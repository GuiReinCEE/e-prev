<?php
class nao_conformidades_acoes_corre_model extends Model
{
	...
	{
		$qr_sql = "
					SELECT i.cd_nao_conformidades_acoes_corre,
					  ...
					  FROM indicador_poder.nao_conformidades_acoes_corre i
	...
	{
		$sql = " 
            SELECT cd_nao_conformidades_acoes_corre,
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_nao_conformidades_acoes_corre=".intval($cd);
	...
	{
		if(intval($args['cd_nao_conformidades_acoes_corre'])==0)
		{
			$sql="
			INSERT INTO indicador_poder.nao_conformidades_acoes_corre
                      (
                        dt_referencia,
                        dt_inclusao,
                        cd_usuario_inclusao ,
		...
		{
			$sql="
			UPDATE indicador_poder.nao_conformidades_acoes_corre
			   SET cd_nao_conformidades_acoes_corre = ".intval($args["cd_nao_conformidades_acoes_corre"]).",
			 ...
			 WHERE cd_nao_conformidades_acoes_corre = ".intval($args["cd_nao_conformidades_acoes_corre"])."
			";
		}
	...
	function excluir($id)
	{
		...
		UPDATE indicador_poder.nao_conformidades_acoes_corre
		 ...
		 WHERE md5(cd_nao_conformidades_acoes_corre::varchar)='".$id."'
		"; 
