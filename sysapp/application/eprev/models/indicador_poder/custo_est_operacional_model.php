<?php
class custo_est_operacional_model extends Model
{
	...
	{
		$qr_sql = "
					SELECT i.cd_custo_est_operacional,
					  ...
					  FROM indicador_poder.custo_est_operacional i
	...
	{
		$sql = " 
            SELECT cd_custo_est_operacional,
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_custo_est_operacional=".intval($cd);
	...
	{
		if(intval($args['cd_custo_est_operacional'])==0)
		{
			$sql="
			INSERT INTO indicador_poder.custo_est_operacional
                      (
                        dt_referencia,
                        dt_inclusao,
                        cd_usuario_inclusao ,
		...
		{
			$sql="
			UPDATE indicador_poder.custo_est_operacional
			   SET cd_custo_est_operacional = ".intval($args["cd_custo_est_operacional"]).",
			 ...
			 WHERE cd_custo_est_operacional = ".intval($args["cd_custo_est_operacional"])."
			";
		}
	...
	function excluir($id)
	{
		...
		UPDATE indicador_poder.custo_est_operacional
		 ...
		 WHERE md5(cd_custo_est_operacional::varchar)='".$id."'
		"; 
