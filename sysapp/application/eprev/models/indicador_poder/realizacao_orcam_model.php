<?php
class realizacao_orcam_model extends Model
{
	...
	function listar( &$result, $args=array() )
	{
		$qr_sql = "
					SELECT i.cd_realizacao_orcam,
					  ...
					  FROM indicador_poder.realizacao_orcam i
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_realizacao_orcam=".intval($cd);
	...
	{
		if(intval($args['cd_realizacao_orcam'])==0)
		{
			$sql="
			INSERT INTO indicador_poder.realizacao_orcam
                      (
                        dt_referencia,
                        dt_inclusao,
                        cd_usuario_inclusao ,
		...
		{
			$sql="
			UPDATE indicador_poder.realizacao_orcam
			   SET cd_realizacao_orcam = ".intval($args["cd_realizacao_orcam"]).",
			 ...
			 WHERE cd_realizacao_orcam = ".intval($args["cd_realizacao_orcam"])."
			";
		}
	...
	function excluir($id)
	{
		...
		UPDATE indicador_poder.realizacao_orcam
		 ...
		 WHERE md5(cd_realizacao_orcam::varchar)='".$id."'
		"; 
