<?php
class equilibrio_atu_plano_model extends Model
{
	...
	{
		$qr_sql = "
					SELECT i.cd_equilibrio_atu_plano,
					  ...
					  FROM indicador_poder.equilibrio_atu_plano i
	...
	{
		$sql = " 
            SELECT cd_equilibrio_atu_plano,
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_equilibrio_atu_plano=".intval($cd);
	...
	{
		if(intval($args['cd_equilibrio_atu_plano'])==0)
		{
			$sql="
			INSERT INTO indicador_poder.equilibrio_atu_plano
                      (
                        dt_referencia,
                        dt_inclusao,
                        cd_usuario_inclusao ,
		...
		{
			$sql="
			UPDATE indicador_poder.equilibrio_atu_plano
			   SET cd_equilibrio_atu_plano = ".intval($args["cd_equilibrio_atu_plano"]).",
			 ...
			 WHERE cd_equilibrio_atu_plano = ".intval($args["cd_equilibrio_atu_plano"])."
			";
		}
	...
	function excluir($id)
	{
		...
		UPDATE indicador_poder.equilibrio_atu_plano
		 ...
		 WHERE md5(cd_equilibrio_atu_plano::varchar)='".$id."'
		"; 
