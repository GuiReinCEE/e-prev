<?php
class Despesa_participante_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT cd_despesa_participante,
				   cd_indicador_tabela,
				   ...
				   fl_media,
				   observacao
			  FROM indicador_pga.despesa_participante
	...
	public function carrega_referencia($cd_despesa_participante)
	{
		$qr_sql = "
	...
	public function carrega($cd_indicador_tabela)
	{
		$qr_sql = "
			SELECT cd_despesa_participante,
				   ...
				   observacao
			  FROM indicador_pga.despesa_participante
			 WHERE cd_despesa_participante = ".intval($cd_indicador_tabela).";";
	...
	{
		if(intval($args['cd_despesa_participante']) == 0)
		{
			$qr_sql = "
				INSERT INTO indicador_pga.despesa_participante
				     (
				       cd_indicador_tabela,
                       dt_referencia, 
					   nr_valor_1, 
                ...
                VALUES 
				     (
					   ...
					   ".(trim($args['nr_meta_max']) != '' ? floatval($args['nr_meta_max']) : "DEFAULT").",
		...
		{
			$qr_sql = "
				UPDATE indicador_pga.despesa_participante
				   SET dt_referencia        = ".(trim($args['dt_referencia']) != '' ? "TO_DATE('".trim($args['dt_referencia'])."', 'DD/MM/YYYY')" : "DEFAULT").",
					   ...
					   nr_meta_max          = ".(trim($args['nr_meta_max']) != '' ? floatval($args['nr_meta_max']) : "DEFAULT").",
					   observacao           = ".(trim($args['observacao']) != '' ?  str_escape($args['observacao']) : "DEFAULT").",
					   cd_usuario_alteracao = ".intval($args['cd_usuario']).",
					   dt_alteracao         = CURRENT_TIMESTAMP
				 WHERE cd_despesa_participante = ".intval($args['cd_despesa_participante']).";";
		}
	
		$this->db->query($qr_sql);		
	}

	public function excluir($despesa_participante, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_pga.despesa_participante 
			   SET dt_exclusao         = CURRENT_TIMESTAMP, 
			   	   cd_usuario_exclusao = ".intval($cd_usuario)." 
			 WHERE cd_despesa_participante = ".intval($despesa_participante).";"; 
