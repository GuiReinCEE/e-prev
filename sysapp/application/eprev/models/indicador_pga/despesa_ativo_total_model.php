<?php
class Despesa_ativo_total_model extends Model
{
	...
	public function listar($cd_indicador_tabela)
	{
		$qr_sql = "
			SELECT cd_despesa_ativo_total,
				   ...
				   fl_media,
				   observacao
			  FROM indicador_pga.despesa_ativo_total
	...
	public function carrega_referencia($cd_despesa_ativo_total)
	{
		$qr_sql = "
	...
	public function carrega($cd_indicador_tabela)
	{
		$qr_sql = "
			SELECT cd_despesa_ativo_total,
				   ...
				   observacao
			  FROM indicador_pga.despesa_ativo_total
			 WHERE cd_despesa_ativo_total = ".intval($cd_indicador_tabela).";";
	...
	{
		if(intval($args['cd_despesa_ativo_total']) == 0)
		{
			$qr_sql = "
				INSERT INTO indicador_pga.despesa_ativo_total
				     (
				       cd_indicador_tabela,
                       dt_referencia, 
					   nr_valor_1, 
		...
		{
			$qr_sql = "
				UPDATE indicador_pga.despesa_ativo_total
				 ...
				 WHERE cd_despesa_ativo_total = ".intval($args['cd_despesa_ativo_total']).";";
		}
	
		$this->db->query($qr_sql);		
	}

	public function excluir($despesa_ativo_total, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_pga.despesa_ativo_total 
			   SET dt_exclusao         = CURRENT_TIMESTAMP, 
			   	   cd_usuario_exclusao = ".intval($cd_usuario)." 
			 WHERE cd_despesa_ativo_total = ".intval($despesa_ativo_total).";"; 
