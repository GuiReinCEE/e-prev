<?php
class Avaliacao_colaboradores_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT cd_avaliacao_colaboradores,
				   cd_indicador_tabela,
				   ...
				   fl_media,
				   observacao
			  FROM indicador_pga.avaliacao_colaboradores 
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 year'::interval, 'DD/MM/YYYY') AS dt_referencia_n, 
				   TO_CHAR(dt_referencia + '1 year'::interval, 'YYYY') AS ano_referencia_n, 
				   nr_meta, 
				   nr_meta_2,
				   cd_indicador_tabela 
			  FROM indicador_pga.avaliacao_colaboradores 
	...
	function carrega(&$result, $args=array())
	{
		$qr_sql = "
			SELECT cd_avaliacao_colaboradores,
				   ...
				   observacao
			  FROM indicador_pga.avaliacao_colaboradores
			 WHERE cd_avaliacao_colaboradores = ".intval($args['cd_avaliacao_colaboradores']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_pga.avaliacao_colaboradores
				     (
                       dt_referencia, 
					   nr_valor_1, 
                       nr_valor_2, 
		...
		{
			$qr_sql = "
				UPDATE indicador_pga.avaliacao_colaboradores
