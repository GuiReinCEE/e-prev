<?php
class despesa_patrimonio_participante_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT cd_despesa_patrimonio_participante,
				   cd_indicador_tabela,
				   ...
				   fl_media,
				   observacao
			  FROM indicador_pga.despesa_patrimonio_participante
	...
	public function carrega_referencia($cd_despesa_patrimonio_participante)
	{
		$qr_sql = "
	...
	public function carrega($cd_indicador_tabela)
	{
		$qr_sql = "
			SELECT cd_despesa_patrimonio_participante,
				   ...
				   observacao
			  FROM indicador_pga.despesa_patrimonio_participante
			 WHERE cd_despesa_patrimonio_participante = ".intval($cd_indicador_tabela).";";
	...
	{
		if(intval($args['cd_despesa_patrimonio_participante']) == 0)
		{
			$qr_sql = "
				INSERT INTO indicador_pga.despesa_patrimonio_participante
				     (
				       cd_indicador_tabela,
                       dt_referencia, 
					   nr_valor_1, 
		...
		{
			$qr_sql = "
				UPDATE indicador_pga.despesa_patrimonio_participante
				 ...
				 WHERE cd_despesa_patrimonio_participante = ".intval($args['cd_despesa_patrimonio_participante']).";";
		}
	
		$this->db->query($qr_sql);		
	}

	public function excluir($despesa_patrimonio_participante, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_pga.despesa_patrimonio_participante 
			   SET dt_exclusao         = CURRENT_TIMESTAMP, 
			   	   cd_usuario_exclusao = ".intval($cd_usuario)." 
			 WHERE cd_despesa_patrimonio_participante = ".intval($despesa_patrimonio_participante).";"; 
