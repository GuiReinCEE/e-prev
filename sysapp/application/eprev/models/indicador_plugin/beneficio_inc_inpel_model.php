<?php
class beneficio_inc_inpel_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_beneficio_inc_inpel={cd_beneficio_inc_inpel} ";
			esc( "{cd_beneficio_inc_inpel}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_beneficio_inc_inpel'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.beneficio_inc_inpel ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.beneficio_inc_inpel 
			SET 
			
			cd_beneficio_inc_inpel = {cd_beneficio_inc_inpel} 
		...
		esc("{cd_beneficio_inc_inpel}", $args["cd_beneficio_inc_inpel"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_beneficio_inc_inpel::varchar)='{cd_beneficio_inc_inpel}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_beneficio_inc_inpel}', $id, $sql, 'str'); 
