<?php
class atend_recad_assistidos_model extends Model
{
	...
	function listar( &$result, $args=array() )
	{
		// mount query
		$qr_sql = "
					SELECT i.cd_atend_recad_assistidos,
						   ...
						   CASE WHEN (SELECT MAX(i1.dt_referencia)
						                FROM indicador_plugin.atend_recad_assistidos i1
					                   WHERE i1.dt_exclusao IS NULL) = i.dt_referencia
								THEN 'S'
								ELSE 'N'
						   END AS fl_editar
					  FROM indicador_plugin.atend_recad_assistidos i
	...
	function carregar($cd)
	{
		...
		cd_atend_recad_assistidos
		, to_char(dt_referencia,'YYYY') as ano_referencia
		, to_char(dt_referencia,'MM/YYYY') as mes_referencia
		, to_char(dt_referencia,'MM') as semestre_referencia
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atend_recad_assistidos={cd_atend_recad_assistidos} ";
			esc( "{cd_atend_recad_assistidos}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atend_recad_assistidos'])==0)
		{
			$sql="
		...
		{
			$sql="
			UPDATE indicador_plugin.atend_recad_assistidos
			SET 
			
			cd_atend_recad_assistidos = {cd_atend_recad_assistidos}
		...
		esc("{cd_atend_recad_assistidos}", $args["cd_atend_recad_assistidos"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atend_recad_assistidos::varchar)='{cd_atend_recad_assistidos}'
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atend_recad_assistidos}', $id, $sql, 'str');
