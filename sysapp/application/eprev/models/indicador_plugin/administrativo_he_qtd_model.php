<?php
class administrativo_he_qtd_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_administrativo_he_qtd={cd_administrativo_he_qtd} ";
			esc( "{cd_administrativo_he_qtd}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_administrativo_he_qtd'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.administrativo_he_qtd ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.administrativo_he_qtd 
			SET 
			
			cd_administrativo_he_qtd = {cd_administrativo_he_qtd} 
		...
		esc("{cd_administrativo_he_qtd}", $args["cd_administrativo_he_qtd"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_administrativo_he_qtd::varchar)='{cd_administrativo_he_qtd}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_administrativo_he_qtd}', $id, $sql, 'str'); 
