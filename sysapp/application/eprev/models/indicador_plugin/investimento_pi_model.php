<?php
class Investimento_pi_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_investimento_pi={cd_investimento_pi} ";
			esc( "{cd_investimento_pi}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_investimento_pi'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.investimento_pi ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.investimento_pi 
			SET 

			cd_investimento_pi = {cd_investimento_pi} 
			...
			WHERE cd_investimento_pi = {cd_investimento_pi} 
			";
		}

		esc("{cd_investimento_pi}", $args["cd_investimento_pi"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_investimento_pi::varchar)='{cd_investimento_pi}'
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int');
		esc('{cd_investimento_pi}', $id, $sql, 'str');
