<?php
class Juridico_sucesso_acoes_consolidado_trab_mensal_model extends Model
{
    public function listar($cd_indicador_tabela)
    {
        $qr_sql = "
            SELECT cd_juridico_sucesso_acoes_consolidado_trab_mensal,
				   ...
				   fl_meta_proc,
				   fl_direcao_proc
              FROM indicador_plugin.juridico_sucesso_acoes_consolidado_trab_mensal
    ...
    public function carrega($cd_juridico_sucesso_acoes_consolidado_trab_mensal)
    {
        $qr_sql = "
            SELECT cd_juridico_sucesso_acoes_consolidado_trab_mensal,
				   ...
				   nr_proc_max
              FROM indicador_plugin.juridico_sucesso_acoes_consolidado_trab_mensal
             WHERE cd_juridico_sucesso_acoes_consolidado_trab_mensal = ".intval($cd_juridico_sucesso_acoes_consolidado_trab_mensal).";";
    ...
    {
        $qr_sql = "
            INSERT INTO indicador_plugin.juridico_sucesso_acoes_consolidado_trab_mensal
            (
                cd_indicador_tabela, 
                dt_referencia, 
                ds_observacao,
    ...
    public function atualizar($cd_juridico_sucesso_acoes_consolidado_trab_mensal, $args = array())
    {
        $qr_sql = "
            UPDATE indicador_plugin.juridico_sucesso_acoes_consolidado_trab_mensal 
    ...
    public function excluir($cd_juridico_sucesso_acoes_consolidado_trab_mensal, $cd_usuario)
    {
        $qr_sql = "
            UPDATE indicador_plugin.juridico_sucesso_acoes_consolidado_trab_mensal 
            ...
            SELECT COALESCE(SUM(x.nr_inicial), 0) AS nr_inicial,   
              ...
              FROM (
                    SELECT nr_inicial,   
                           ...
                           nr_parcial_3,
                           nr_procede_3
                      FROM indicador_plugin.juridico_sucesso_acoes_castro_barcellos_trab_mensal
                     WHERE dt_exclusao IS NULL
                     ...
                     SELECT nr_inicial,                            
                            ...
                            nr_parcial_3,
                            nr_procede_3
                       FROM indicador_plugin.juridico_sucesso_acoes_feiden_trab_mensal
                      WHERE dt_exclusao IS NULL
                        AND TO_CHAR(dt_referencia, 'MM')::INT   = ".intval($mes)."
                        AND TO_CHAR(dt_referencia, 'YYYY')::INT = ".intval($ano)."
              ) x;";
