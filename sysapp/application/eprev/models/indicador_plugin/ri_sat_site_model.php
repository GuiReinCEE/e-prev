<?php
class ri_sat_site_model extends Model
{
		...
		$qr_sql = "
					SELECT i.cd_ri_sat_site,
						   ...
						   CASE WHEN (SELECT MAX(i1.dt_referencia)
						                FROM indicador_plugin.ri_sat_site i1
					                   WHERE i1.dt_exclusao IS NULL) = i.dt_referencia
								THEN 'S'
								ELSE 'N'
						   END AS fl_editar
					  FROM indicador_plugin.ri_sat_site i
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_ri_sat_site={cd_ri_sat_site} ";
			esc( "{cd_ri_sat_site}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_ri_sat_site'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.ri_sat_site (
		...
		{
			$sql="
			UPDATE indicador_plugin.ri_sat_site
			SET 
			
			cd_ri_sat_site = {cd_ri_sat_site}
		...
		esc("{cd_ri_sat_site}", $args["cd_ri_sat_site"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_ri_sat_site::varchar)='{cd_ri_sat_site}'
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_ri_sat_site}', $id, $sql, 'str');
