<?php
class exp_adesao_projetado_municipios_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_exp_adesao_projetado_municipios,
				   ...
				   i.nr_percentual_f,
				   i.nr_meta
		      FROM indicador_plugin.exp_adesao_projetado_municipios i
	...
	public function carrega_referencia()
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia_n,
				   nr_valor_1, 
				   cd_indicador_tabela 
			  FROM indicador_plugin.exp_adesao_projetado_municipios
	...
	public function carrega($cd_exp_adesao_projetado_municipios)
	{
		$qr_sql = "
            SELECT cd_exp_adesao_projetado_municipios,
                   ...
                   observacao
		      FROM indicador_plugin.exp_adesao_projetado_municipios 
			 WHERE cd_exp_adesao_projetado_municipios = ".intval($cd_exp_adesao_projetado_municipios).";";
	...
	{
		if(intval($args['cd_exp_adesao_projetado_municipios']) == 0)
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.exp_adesao_projetado_municipios 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_meta, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.exp_adesao_projetado_municipios
			     ...
			     WHERE cd_exp_adesao_projetado_municipios = ".intval($args['cd_exp_adesao_projetado_municipios']).";";
		}

		$this->db->query($qr_sql);
	}
	
	public function excluir($cd_exp_adesao_projetado_municipios, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_plugin.exp_adesao_projetado_municipios
