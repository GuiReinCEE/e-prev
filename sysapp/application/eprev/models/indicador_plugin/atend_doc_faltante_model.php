<?php
class atend_doc_faltante_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atend_doc_faltante={cd_atend_doc_faltante} ";
			esc( "{cd_atend_doc_faltante}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atend_doc_faltante'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.atend_doc_faltante ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.atend_doc_faltante 
			SET 
			
			cd_atend_doc_faltante = {cd_atend_doc_faltante} 
		...
		esc("{cd_atend_doc_faltante}", $args["cd_atend_doc_faltante"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atend_doc_faltante::varchar)='{cd_atend_doc_faltante}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atend_doc_faltante}', $id, $sql, 'str'); 
