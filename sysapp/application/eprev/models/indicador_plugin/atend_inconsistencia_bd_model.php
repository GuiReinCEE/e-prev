<?php
class Atend_inconsistencia_bd_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atend_inconsistencia_bd={cd_atend_inconsistencia_bd} ";
			esc( "{cd_atend_inconsistencia_bd}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atend_inconsistencia_bd'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.atend_inconsistencia_bd ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.atend_inconsistencia_bd 
			SET 
			
			cd_atend_inconsistencia_bd = {cd_atend_inconsistencia_bd} 
		...
		esc("{cd_atend_inconsistencia_bd}", $args["cd_atend_inconsistencia_bd"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atend_inconsistencia_bd::varchar)='{cd_atend_inconsistencia_bd}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atend_inconsistencia_bd}', $id, $sql, 'str'); 
