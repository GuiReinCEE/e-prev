<?php
class Atend_call_na_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atend_call_na={cd_atend_call_na} ";
			esc( "{cd_atend_call_na}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atend_call_na'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.atend_call_na ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.atend_call_na 
			SET 
			
			cd_atend_call_na = {cd_atend_call_na} 
		...
		esc("{cd_atend_call_na}", $args["cd_atend_call_na"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atend_call_na::varchar)='{cd_atend_call_na}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atend_call_na}', $id, $sql, 'str'); 
