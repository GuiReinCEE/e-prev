<?php
class juridico_par_acoes_jud_model extends Model
{
	...
	{
		$qr_sql = "
					SELECT i.cd_juridico_par_acoes_jud,
						   ...
						   i.nr_meta
					  FROM indicador_plugin.juridico_par_acoes_jud i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
					SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia_n, 
						   nr_meta, 
						   cd_indicador_tabela 
					  FROM indicador_plugin.juridico_par_acoes_jud
	...
	{
		$qr_sql = "
					SELECT cd_juridico_par_acoes_jud,
						   ...
						   observacao
					  FROM indicador_plugin.juridico_par_acoes_jud 
					 WHERE cd_juridico_par_acoes_jud = ".intval($args['cd_juridico_par_acoes_jud']).";
			      ";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
						INSERT INTO indicador_plugin.juridico_par_acoes_jud 
							 (
								dt_referencia, 
								qt_assistidos,
								qt_acoes,
		...
		{
			$qr_sql = "
						UPDATE indicador_plugin.juridico_par_acoes_jud
