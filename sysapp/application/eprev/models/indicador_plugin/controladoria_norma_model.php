<?php
class controladoria_norma_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_controladoria_norma,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.controladoria_norma i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.controladoria_norma i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
				   nr_meta, 
				   cd_indicador_tabela 
			  FROM indicador_plugin.controladoria_norma
	...
	function carrega(&$result, $args=array())
	{
		$qr_sql = "
            SELECT cd_controladoria_norma,
                   ...
                   observacao
		      FROM indicador_plugin.controladoria_norma 
			 WHERE cd_controladoria_norma = ".intval($args['cd_controladoria_norma']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.controladoria_norma 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_valor_2, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.controladoria_norma
