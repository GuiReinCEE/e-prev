<?php
class Controladoria_tempo_elaboracao_novo_reg_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_controladoria_tempo_elaboracao_novo_reg,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.controladoria_tempo_elaboracao_novo_reg i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.controladoria_tempo_elaboracao_novo_reg i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
				   nr_meta, 
				   cd_indicador_tabela 
			  FROM indicador_plugin.controladoria_tempo_elaboracao_novo_reg
	...
	{
		$qr_sql = "
            SELECT cd_controladoria_tempo_elaboracao_novo_reg,
                   ...
                   observacao
		      FROM indicador_plugin.controladoria_tempo_elaboracao_novo_reg 
			 WHERE cd_controladoria_tempo_elaboracao_novo_reg = ".intval($args['cd_controladoria_tempo_elaboracao_novo_reg']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.controladoria_tempo_elaboracao_novo_reg 
				     (
						dt_referencia, 
						ds_evento,
					    nr_valor_1, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.controladoria_tempo_elaboracao_novo_reg
