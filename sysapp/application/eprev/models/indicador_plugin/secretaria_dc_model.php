<?php
class Secretaria_dc_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_secretaria_dc={cd_secretaria_dc} ";
			esc( "{cd_secretaria_dc}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_secretaria_dc'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.secretaria_dc ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.secretaria_dc 
			SET 
			
			cd_secretaria_dc = {cd_secretaria_dc} 
		...
		esc("{cd_secretaria_dc}", $args["cd_secretaria_dc"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_secretaria_dc::varchar)='{cd_secretaria_dc}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_secretaria_dc}', $id, $sql, 'str'); 
