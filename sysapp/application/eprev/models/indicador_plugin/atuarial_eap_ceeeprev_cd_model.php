<?php
class atuarial_eap_ceeeprev_cd_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atuarial_eap_ceeeprev_cd={cd_atuarial_eap_ceeeprev_cd} ";
			esc( "{cd_atuarial_eap_ceeeprev_cd}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atuarial_eap_ceeeprev_cd'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.atuarial_eap_ceeeprev_cd ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.atuarial_eap_ceeeprev_cd 
			SET 
			
			cd_atuarial_eap_ceeeprev_cd = {cd_atuarial_eap_ceeeprev_cd} 
		...
		esc("{cd_atuarial_eap_ceeeprev_cd}", $args["cd_atuarial_eap_ceeeprev_cd"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atuarial_eap_ceeeprev_cd::varchar)='{cd_atuarial_eap_ceeeprev_cd}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atuarial_eap_ceeeprev_cd}', $id, $sql, 'str'); 
