<?php
class atend_sat_planej_fin_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atend_sat_planej_fin={cd_atend_sat_planej_fin} ";
			esc( "{cd_atend_sat_planej_fin}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atend_sat_planej_fin'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.atend_sat_planej_fin ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.atend_sat_planej_fin 
			SET 
			
			cd_atend_sat_planej_fin = {cd_atend_sat_planej_fin} 
		...
		esc("{cd_atend_sat_planej_fin}", $args["cd_atend_sat_planej_fin"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atend_sat_planej_fin::varchar)='{cd_atend_sat_planej_fin}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atend_sat_planej_fin}', $id, $sql, 'str'); 
