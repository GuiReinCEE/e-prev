<?php
class ri_retrabalho_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_ri_retrabalho={cd_ri_retrabalho} ";
			esc( "{cd_ri_retrabalho}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_ri_retrabalho'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.ri_retrabalho ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.ri_retrabalho 
			SET 
			
			cd_ri_retrabalho = {cd_ri_retrabalho} 
		...
		esc("{cd_ri_retrabalho}", $args["cd_ri_retrabalho"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_ri_retrabalho::varchar)='{cd_ri_retrabalho}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_ri_retrabalho}', $id, $sql, 'str'); 
