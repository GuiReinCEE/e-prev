<?php
class Atend_call_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atend_call={cd_atend_call} ";
			esc( "{cd_atend_call}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atend_call'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.atend_call ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.atend_call 
			SET 
			
			cd_atend_call = {cd_atend_call} 
		...
		esc("{cd_atend_call}", $args["cd_atend_call"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atend_call::varchar)='{cd_atend_call}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atend_call}', $id, $sql, 'str'); 
