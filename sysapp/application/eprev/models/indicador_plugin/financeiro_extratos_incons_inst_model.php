<?php
class Financeiro_extratos_incons_inst_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT cd_financeiro_extratos_incons_inst,
				   cd_indicador_tabela,
				   ...
				   fl_media,
				   ds_observacao
			  FROM indicador_plugin.financeiro_extratos_incons_inst
	...
	public function carrega_referencia($cd_financeiro_extratos_incons_inst)
	{
		$qr_sql = "
	...
	public function carrega($cd_indicador_tabela)
	{
		$qr_sql = "
			SELECT cd_financeiro_extratos_incons_inst,
				   ...
				   ds_observacao
			  FROM indicador_plugin.financeiro_extratos_incons_inst
			 WHERE cd_financeiro_extratos_incons_inst = ".intval($cd_indicador_tabela).";";
	...
	{
		if(intval($args['cd_financeiro_extratos_incons_inst']) == 0)
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.financeiro_extratos_incons_inst
				     (
				       cd_indicador_tabela,
                       dt_referencia, 
					   nr_extratos,
                ...
                VALUES 
				     (
					   ...
					   ".(trim($args['nr_extratos']) != '' ? floatval($args['nr_extratos']) : "DEFAULT").",
  					   ".(trim($args['nr_extratos_incons']) != '' ? floatval($args['nr_extratos_incons']) : "DEFAULT").",
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.financeiro_extratos_incons_inst
				   SET dt_referencia         = ".(trim($args['dt_referencia']) != '' ? "TO_DATE('".trim($args['dt_referencia'])."', 'DD/MM/YYYY')" : "DEFAULT").",
				       nr_extratos           = ".(trim($args['nr_extratos']) != '' ? floatval($args['nr_extratos']) : "DEFAULT").",
					   nr_extratos_incons    = ".(trim($args['nr_extratos_incons']) != '' ? floatval($args['nr_extratos_incons']) : "DEFAULT").",
				 ...
				 WHERE cd_financeiro_extratos_incons_inst = ".intval($args['cd_financeiro_extratos_incons_inst']).";";
		}
	
		$this->db->query($qr_sql);		
	}

	public function excluir($cd_financeiro_extratos_incons_inst, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_plugin.financeiro_extratos_incons_inst 
			   SET dt_exclusao         = CURRENT_TIMESTAMP, 
			   	   cd_usuario_exclusao = ".intval($cd_usuario)." 
			 WHERE cd_financeiro_extratos_incons_inst = ".intval($cd_financeiro_extratos_incons_inst).";"; 
