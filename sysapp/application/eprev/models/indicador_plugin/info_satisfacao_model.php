<?php
class info_satisfacao_model extends Model
{
		...
		$qr_sql = "
					SELECT i.cd_info_satisfacao,
						   ...
						   CASE WHEN (SELECT MAX(i1.dt_referencia)
						                FROM indicador_plugin.info_satisfacao i1
					                   WHERE i1.dt_exclusao IS NULL) = i.dt_referencia
								THEN 'S'
								ELSE 'N'
						   END AS fl_editar
					  FROM indicador_plugin.info_satisfacao i
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_info_satisfacao={cd_info_satisfacao} ";
			esc( "{cd_info_satisfacao}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_info_satisfacao'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.info_satisfacao (
		...
		{
			$sql="
			UPDATE indicador_plugin.info_satisfacao
			SET 
			
			cd_info_satisfacao = {cd_info_satisfacao}
		...
		esc("{cd_info_satisfacao}", $args["cd_info_satisfacao"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_info_satisfacao::varchar)='{cd_info_satisfacao}'
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_info_satisfacao}', $id, $sql, 'str');
