<?php
class beneficio_inc_abrh_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_beneficio_inc_abrh={cd_beneficio_inc_abrh} ";
			esc( "{cd_beneficio_inc_abrh}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_beneficio_inc_abrh'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.beneficio_inc_abrh ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.beneficio_inc_abrh 
			SET 
			
			cd_beneficio_inc_abrh = {cd_beneficio_inc_abrh} 
		...
		esc("{cd_beneficio_inc_abrh}", $args["cd_beneficio_inc_abrh"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_beneficio_inc_abrh::varchar)='{cd_beneficio_inc_abrh}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_beneficio_inc_abrh}', $id, $sql, 'str'); 
