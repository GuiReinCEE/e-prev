<?php
class Juridico_sucesso_acoes_re_consolidado_model extends Model
{
	...
	{
		$qr_sql = "
					SELECT i.cd_juridico_sucesso_acoes_re_consolidado,
						   ...
						   i.nr_percentual_f,
						   i.nr_meta
					  FROM indicador_plugin.juridico_sucesso_acoes_re_consolidado i
		...
		$qr_sql = "
					SELECT x.value, 
					 ...
					 WHERE x.value NOT IN (SELECT i.cd_etapa
					                         FROM indicador_plugin.juridico_sucesso_acoes_re_consolidado i
								            WHERE i.dt_exclusao IS NULL
											  AND i.cd_juridico_sucesso_acoes_re_consolidado <> ".intval($args['cd_juridico_sucesso_acoes_re_consolidado'])."
	...
	{
		$qr_sql = "
					SELECT cd_juridico_sucesso_acoes_re_consolidado,
						   ...
						   nr_percentual_f,
						   nr_meta
					  FROM indicador_plugin.juridico_sucesso_acoes_re_consolidado 
					 WHERE cd_juridico_sucesso_acoes_re_consolidado = ".intval($args['cd_juridico_sucesso_acoes_re_consolidado']).";
			      ";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
						INSERT INTO indicador_plugin.juridico_sucesso_acoes_re_consolidado 
							 (
								dt_referencia, 
						        cd_etapa,
						        nr_valor_1,
		...
		{
			$qr_sql = "
						UPDATE indicador_plugin.juridico_sucesso_acoes_re_consolidado
	...
	{
		$qr_sql = "
			SELECT SUM(nr_valor_1) AS nr_valor_1,
			       SUM(nr_valor_2) AS nr_valor_2,
			       SUM(nr_valor_3) AS nr_valor_3,
			       SUM(nr_valor_4) AS nr_valor_4,
			       SUM(nr_meta) AS nr_meta
			  FROM (
			SELECT nr_valor_1,
			       nr_valor_2,
			       nr_valor_3,
			       nr_valor_4,
			       nr_meta
			  FROM indicador_plugin.juridico_sucesso_acoes_re_civel
			...
			SELECT nr_valor_1,
			       ...
			       nr_valor_4,
			       nr_meta
			  FROM indicador_plugin.juridico_sucesso_acoes_re_trabalhista
			 WHERE TO_CHAR(dt_referencia, 'YYYY')::integer = ".intval($nr_ano)."
			   AND cd_etapa                                = ".intval($cd_etapa)."
			   AND dt_exclusao                             IS NULL
