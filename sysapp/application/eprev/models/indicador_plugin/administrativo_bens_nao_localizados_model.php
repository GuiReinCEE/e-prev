<?php
class Administrativo_bens_nao_localizados_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_administrativo_bens_nao_localizados,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.administrativo_bens_nao_localizados i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.administrativo_bens_nao_localizados i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
				   ...
				   nr_valor_1,
                   nr_valor_2
			  FROM indicador_plugin.administrativo_bens_nao_localizados
	...
	{
		$qr_sql = "
            SELECT cd_administrativo_bens_nao_localizados,
                   ...
                   observacao
		      FROM indicador_plugin.administrativo_bens_nao_localizados 
			 WHERE cd_administrativo_bens_nao_localizados = ".intval($args['cd_administrativo_bens_nao_localizados']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.administrativo_bens_nao_localizados 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_valor_2, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.administrativo_bens_nao_localizados
