<?php
class Administrativo_ept_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_administrativo_ept={cd_administrativo_ept} ";
			esc( "{cd_administrativo_ept}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_administrativo_ept'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.administrativo_ept ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.administrativo_ept 
			SET 
			
			cd_administrativo_ept = {cd_administrativo_ept} 
		...
		esc("{cd_administrativo_ept}", $args["cd_administrativo_ept"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_administrativo_ept::varchar)='{cd_administrativo_ept}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_administrativo_ept}', $id, $sql, 'str'); 
