<?php
class Controladoria_div_caderno_acom_resultado_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_controladoria_div_caderno_acom_resultado,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.controladoria_div_caderno_acom_resultado i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.controladoria_div_caderno_acom_resultado i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
				   nr_meta, 
				   cd_indicador_tabela 
			  FROM indicador_plugin.controladoria_div_caderno_acom_resultado
	...
	{
		$qr_sql = "
            SELECT cd_controladoria_div_caderno_acom_resultado,
                   ...
                   observacao
		      FROM indicador_plugin.controladoria_div_caderno_acom_resultado 
			 WHERE cd_controladoria_div_caderno_acom_resultado = ".intval($args['cd_controladoria_div_caderno_acom_resultado']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.controladoria_div_caderno_acom_resultado 
				     (
						dt_referencia, 
					    nr_valor_1, 
					    nr_meta, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.controladoria_div_caderno_acom_resultado
			...
			INSERT INTO indicador_plugin.controladoria_div_caderno_acom_resultado 
			 ...
			 VALUES 
				  ( 
					".(trim($args['dt_referencia']) == "" ? "DEFAULT" : "TO_DATE('".trim($args["dt_referencia"])."', 'DD/MM/YYYY')").",
					".(trim($args['nr_valor_1']) == "" ? "DEFAULT" : floatval($args['nr_valor_1'])).",
					".(trim($args['nr_meta']) == "" ? "DEFAULT" : $args['nr_meta']).",
					".(intval($args['cd_indicador_tabela']) == 0 ? "DEFAULT" : intval($args['cd_indicador_tabela'])).",
					'S',
					".intval($args['cd_usuario']).",
					".intval($args['cd_usuario'])."
