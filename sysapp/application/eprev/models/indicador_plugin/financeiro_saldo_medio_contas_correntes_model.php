<?php
class Financeiro_saldo_medio_contas_correntes_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_financeiro_saldo_medio_contas_correntes,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.financeiro_saldo_medio_contas_correntes i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.financeiro_saldo_medio_contas_correntes i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
				   ...
				   nr_valor_1,
                   nr_valor_2
			  FROM indicador_plugin.financeiro_saldo_medio_contas_correntes
	...
	{
		$qr_sql = "
            SELECT cd_financeiro_saldo_medio_contas_correntes,
                   ...
                   observacao
		      FROM indicador_plugin.financeiro_saldo_medio_contas_correntes 
			 WHERE cd_financeiro_saldo_medio_contas_correntes = ".intval($args['cd_financeiro_saldo_medio_contas_correntes']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.financeiro_saldo_medio_contas_correntes 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_valor_2, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.financeiro_saldo_medio_contas_correntes
