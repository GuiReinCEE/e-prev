<?php
class Investimento_evolucao_inpc_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_investimento_evolucao_inpc,
				   ...
				   i.nr_valor_4,
				   i.nr_meta
		      FROM indicador_plugin.investimento_evolucao_inpc i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
                   ...
                   nr_valor_3,
                   nr_valor_4
			  FROM indicador_plugin.investimento_evolucao_inpc
	...
	{
		$qr_sql = "
            SELECT cd_investimento_evolucao_inpc,
                   ...
                   observacao
		      FROM indicador_plugin.investimento_evolucao_inpc 
			 WHERE cd_investimento_evolucao_inpc = ".intval($args['cd_investimento_evolucao_inpc']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.investimento_evolucao_inpc 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_valor_2, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.investimento_evolucao_inpc
