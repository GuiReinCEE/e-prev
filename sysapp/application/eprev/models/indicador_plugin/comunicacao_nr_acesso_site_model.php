<?php
class Comunicacao_nr_acesso_site_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_comunicacao_nr_acesso_site,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.comunicacao_nr_acesso_site i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.comunicacao_nr_acesso_site i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
				   nr_meta, 
				   cd_indicador_tabela 
			  FROM indicador_plugin.comunicacao_nr_acesso_site
	...
	{
		$qr_sql = "
            SELECT cd_comunicacao_nr_acesso_site,
                   ...
                   observacao
		      FROM indicador_plugin.comunicacao_nr_acesso_site 
			 WHERE cd_comunicacao_nr_acesso_site = ".intval($args['cd_comunicacao_nr_acesso_site']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.comunicacao_nr_acesso_site 
				     (
						dt_referencia, 
					    nr_valor_1, 
					    nr_meta, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.comunicacao_nr_acesso_site
