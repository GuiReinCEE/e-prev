<?php
class Atend_desligamento_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atend_desligamento={cd_atend_desligamento} ";
			esc( "{cd_atend_desligamento}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atend_desligamento'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.atend_desligamento ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.atend_desligamento 
			SET 
			
			cd_atend_desligamento = {cd_atend_desligamento} 
		...
		esc("{cd_atend_desligamento}", $args["cd_atend_desligamento"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atend_desligamento::varchar)='{cd_atend_desligamento}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atend_desligamento}', $id, $sql, 'str'); 
