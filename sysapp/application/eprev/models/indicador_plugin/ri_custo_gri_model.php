<?php
class ri_custo_gri_model extends Model
{
		...
		$qr_sql = "
					SELECT i.cd_ri_custo_gri,
						   ...
						   CASE WHEN (SELECT MAX(i1.dt_referencia)
						                FROM indicador_plugin.ri_custo_gri i1
					                   WHERE i1.dt_exclusao IS NULL) = i.dt_referencia
								THEN 'S'
								ELSE 'N'
						   END AS fl_editar
					  FROM indicador_plugin.ri_custo_gri i
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_ri_custo_gri={cd_ri_custo_gri} ";
			esc( "{cd_ri_custo_gri}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_ri_custo_gri'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.ri_custo_gri (
		...
		{
			$sql="
			UPDATE indicador_plugin.ri_custo_gri
			SET 
			
			cd_ri_custo_gri = {cd_ri_custo_gri}
		...
		esc("{cd_ri_custo_gri}", $args["cd_ri_custo_gri"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_ri_custo_gri::varchar)='{cd_ri_custo_gri}'
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_ri_custo_gri}', $id, $sql, 'str');
