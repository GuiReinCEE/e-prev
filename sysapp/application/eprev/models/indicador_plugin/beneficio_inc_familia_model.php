<?php
class beneficio_inc_familia_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_beneficio_inc_familia={cd_beneficio_inc_familia} ";
			esc( "{cd_beneficio_inc_familia}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_beneficio_inc_familia'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.beneficio_inc_familia ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.beneficio_inc_familia 
			SET 
			
			cd_beneficio_inc_familia = {cd_beneficio_inc_familia} 
		...
		esc("{cd_beneficio_inc_familia}", $args["cd_beneficio_inc_familia"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_beneficio_inc_familia::varchar)='{cd_beneficio_inc_familia}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_beneficio_inc_familia}', $id, $sql, 'str'); 
