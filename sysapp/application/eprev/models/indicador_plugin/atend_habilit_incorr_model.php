<?php
class atend_habilit_incorr_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atend_habilit_incorr={cd_atend_habilit_incorr} ";
			esc( "{cd_atend_habilit_incorr}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atend_habilit_incorr'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.atend_habilit_incorr ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.atend_habilit_incorr 
			SET 
			
			cd_atend_habilit_incorr = {cd_atend_habilit_incorr} 
		...
		esc("{cd_atend_habilit_incorr}", $args["cd_atend_habilit_incorr"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atend_habilit_incorr::varchar)='{cd_atend_habilit_incorr}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atend_habilit_incorr}', $id, $sql, 'str'); 
