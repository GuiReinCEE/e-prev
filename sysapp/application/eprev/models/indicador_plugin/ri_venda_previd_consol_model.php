<?php
class ri_venda_previd_consol_model extends Model
{
	...
	function listar( &$result, $args=array() )
	{
		// mount query
		$qr_sql = "
					SELECT i.cd_ri_venda_previd_consol,
						   ...
						   CASE WHEN (SELECT MAX(i1.dt_referencia)
						                FROM indicador_plugin.ri_venda_previd_consol i1
					                   WHERE i1.dt_exclusao IS NULL) = i.dt_referencia
								THEN 'S'
								ELSE 'N'
						   END AS fl_editar
					  FROM indicador_plugin.ri_venda_previd_consol i
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_ri_venda_previd_consol={cd_ri_venda_previd_consol} ";
			esc( "{cd_ri_venda_previd_consol}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_ri_venda_previd_consol'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.ri_venda_previd_consol (
		...
		{
			$sql="
			UPDATE indicador_plugin.ri_venda_previd_consol
			SET 
			
			cd_ri_venda_previd_consol = {cd_ri_venda_previd_consol}
		...
		esc("{cd_ri_venda_previd_consol}", $args["cd_ri_venda_previd_consol"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_ri_venda_previd_consol::varchar)='{cd_ri_venda_previd_consol}'
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_ri_venda_previd_consol}', $id, $sql, 'str');
