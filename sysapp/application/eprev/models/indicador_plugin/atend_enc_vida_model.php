<?php
class atend_enc_vida_model extends Model
{
		...
		$qr_sql = "
					SELECT i.cd_atend_enc_vida,
						   ...
						   CASE WHEN (SELECT MAX(i1.dt_referencia)
						                FROM indicador_plugin.atend_enc_vida i1
					                   WHERE i1.dt_exclusao IS NULL) = i.dt_referencia
								THEN 'S'
								ELSE 'N'
						   END AS fl_editar
					  FROM indicador_plugin.atend_enc_vida i
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atend_enc_vida={cd_atend_enc_vida} ";
			esc( "{cd_atend_enc_vida}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atend_enc_vida'])==0)
		{
			$sql="
		...
		{
			$sql="
			UPDATE indicador_plugin.atend_enc_vida
			SET 
			
			cd_atend_enc_vida = {cd_atend_enc_vida}
		...
		esc("{cd_atend_enc_vida}", $args["cd_atend_enc_vida"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atend_enc_vida::varchar)='{cd_atend_enc_vida}'
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atend_enc_vida}', $id, $sql, 'str');
