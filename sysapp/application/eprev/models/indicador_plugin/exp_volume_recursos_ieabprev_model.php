<?php
class exp_volume_recursos_ieabprev_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_exp_volume_recursos_ieabprev,
			  ...
			  FROM indicador_plugin.exp_volume_recursos_ieabprev i
	...
	public function carrega_referencia($cd_indicador_tabela)
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS dt_referencia_n, 
				   nr_meta, 
				   cd_indicador_tabela 
			  FROM indicador_plugin.exp_volume_recursos_ieabprev 
	...
	public function carrega($cd_exp_volume_recursos_ieabprev)
	{
		$qr_sql = "
			SELECT cd_exp_volume_recursos_ieabprev,
			  ...
			  FROM indicador_plugin.exp_volume_recursos_ieabprev 
			 WHERE dt_exclusao IS NULL
			   AND cd_exp_volume_recursos_ieabprev = ".intval($cd_exp_volume_recursos_ieabprev)."
	...
	{
		$qr_sql = "
			INSERT INTO indicador_plugin.exp_volume_recursos_ieabprev
			     (
                   dt_referencia, 
                   cd_indicador_tabela, 
                   fl_media, 
	...
	public function atualizar($cd_exp_volume_recursos_ieabprev, $args)
	{
		$qr_sql = "
			UPDATE indicador_plugin.exp_volume_recursos_ieabprev
			 ...
			 WHERE cd_exp_volume_recursos_ieabprev = ".intval($cd_exp_volume_recursos_ieabprev).";";

		$this->db->query($qr_sql);	
	}
	
	public function excluir($cd_exp_volume_recursos_ieabprev, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_plugin.exp_volume_recursos_ieabprev 
