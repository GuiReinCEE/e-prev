<?php
class beneficio_inicial_incorreto_model extends Model
{
	...
	function listar( &$result, $args=array() )
	{
		...
		cd_beneficio_inicial_incorreto
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_beneficio_inicial_incorreto={cd_beneficio_inicial_incorreto} ";
			esc( "{cd_beneficio_inicial_incorreto}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_beneficio_inicial_incorreto'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.beneficio_inicial_incorreto ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.beneficio_inicial_incorreto 
			SET 
			
			cd_beneficio_inicial_incorreto = {cd_beneficio_inicial_incorreto} 
		...
		esc("{cd_beneficio_inicial_incorreto}", $args["cd_beneficio_inicial_incorreto"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_beneficio_inicial_incorreto::varchar)='{cd_beneficio_inicial_incorreto}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_beneficio_inicial_incorreto}', $id, $sql, 'str'); 
