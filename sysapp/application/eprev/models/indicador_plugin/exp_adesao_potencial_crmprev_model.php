<?php
class Exp_adesao_potencial_crmprev_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_exp_adesao_potencial_crmprev,
				   ...
				   i.nr_percentual_f,
				   i.nr_meta
			  FROM indicador_plugin.exp_adesao_potencial_crmprev i
	...
	public function carrega_referencia($args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
				   ...
				   nr_meta, 
				   cd_indicador_tabela
			  FROM indicador_plugin.exp_adesao_potencial_crmprev
	...
	public function carrega($cd_exp_adesao_potencial_crmprev)
	{
		$qr_sql = "
            SELECT cd_exp_adesao_potencial_crmprev,
                   ...
                   observacao
		      FROM indicador_plugin.exp_adesao_potencial_crmprev 
			 WHERE cd_exp_adesao_potencial_crmprev = ".intval($cd_exp_adesao_potencial_crmprev).";";
	...
	{
		if(intval($args['cd_exp_adesao_potencial_crmprev']) == 0)
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.exp_adesao_potencial_crmprev 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_valor_2, 
			     ...
			     VALUES 
				      ( 
						...
						".(trim($args['nr_valor_4']) != '' ? floatval($args['nr_valor_4']) : "DEFAULT").",
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.exp_adesao_potencial_crmprev
			     ...
			     WHERE cd_exp_adesao_potencial_crmprev = ".intval($args['cd_exp_adesao_potencial_crmprev']).";";
		}

		$this->db->query($qr_sql);
	}
	
	public function excluir($cd_exp_adesao_potencial_crmprev, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_plugin.exp_adesao_potencial_crmprev
