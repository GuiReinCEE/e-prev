<?php
class info_peti_model extends Model
{
	...
	function listar( &$result, $args=array() )
	{
        ...
        , CASE WHEN (SELECT MAX(i1.dt_referencia)
                            FROM indicador_plugin.info_peti i1
                            WHERE i1.dt_exclusao IS NULL) = i.dt_referencia
						THEN 'S'
						ELSE 'N'
				   END AS fl_editar
		FROM indicador_plugin.info_peti i
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_info_peti={cd_info_peti} ";
			esc( "{cd_info_peti}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_info_peti'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.info_peti (
		...
		{
			$sql="
			UPDATE indicador_plugin.info_peti
			SET 
			
			cd_info_peti = {cd_info_peti}
		...
		esc("{cd_info_peti}", $args["cd_info_peti"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_info_peti::varchar)='{cd_info_peti}'
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_info_peti}', $id, $sql, 'str');
