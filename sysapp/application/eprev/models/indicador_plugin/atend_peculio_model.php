<?php
class Atend_peculio_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atend_peculio={cd_atend_peculio} ";
			esc( "{cd_atend_peculio}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atend_peculio'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.atend_peculio ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.atend_peculio 
			SET 
			
			cd_atend_peculio = {cd_atend_peculio} 
		...
		esc("{cd_atend_peculio}", $args["cd_atend_peculio"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atend_peculio::varchar)='{cd_atend_peculio}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atend_peculio}', $id, $sql, 'str'); 
