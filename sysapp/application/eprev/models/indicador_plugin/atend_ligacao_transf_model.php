<?php
class Atend_ligacao_transf_model extends Model
{
	...
	function carregar( $cd )
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atend_ligacao_transf={cd_atend_ligacao_transf} ";
			esc( "{cd_atend_ligacao_transf}", intval($cd), $sql );
	...
	{
		if( intval($args['cd_atend_ligacao_transf'])==0 )
		{
			$sql="
			INSERT INTO indicador_plugin.atend_ligacao_transf ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.atend_ligacao_transf 
			SET 
			
			cd_atend_ligacao_transf = {cd_atend_ligacao_transf} 
		...
		esc("{cd_atend_ligacao_transf}", $args["cd_atend_ligacao_transf"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atend_ligacao_transf::varchar)='{cd_atend_ligacao_transf}'
		";
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int');
		esc('{cd_atend_ligacao_transf}', $id, $sql, 'str');
