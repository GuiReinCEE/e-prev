<?php
class ri_pub_eventos_internos_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_ri_pub_eventos_internos,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.ri_pub_eventos_internos i1
                               WHERE i1.dt_exclusao IS NULL) = i.dt_referencia THEN 'S'
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.ri_pub_eventos_internos i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT CASE WHEN TO_CHAR(dt_referencia, 'MM') = '02' THEN TO_CHAR(dt_referencia + '1 year'::interval, 'YYYY')
				   ...
				   nr_meta, 
				   cd_indicador_tabela 
			  FROM indicador_plugin.ri_pub_eventos_internos
	...
	{
		$qr_sql = "
            SELECT cd_ri_pub_eventos_internos,
                   ...
                   observacao
		      FROM indicador_plugin.ri_pub_eventos_internos 
			 WHERE cd_ri_pub_eventos_internos = ".intval($args['cd_ri_pub_eventos_internos']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.ri_pub_eventos_internos
				     (
                       dt_referencia, 
					   nr_valor_1, 
                       nr_valor_2, 
                ...
                VALUES 
				     (
					   ...
					   ".(trim($args['nr_valor_3']) == "" ? "DEFAULT" : intval($args['nr_valor_3'])).",
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.ri_pub_eventos_internos
				   SET dt_referencia        = ".(trim($args['dt_referencia']) == "" ? "DEFAULT" : "TO_DATE('".trim($args["dt_referencia"])."', 'DD/MM/YYYY')").",
					   ...
					   nr_valor_3           = ".(trim($args['nr_valor_3']) == "" ? "DEFAULT" : intval($args['nr_valor_3'])).",
