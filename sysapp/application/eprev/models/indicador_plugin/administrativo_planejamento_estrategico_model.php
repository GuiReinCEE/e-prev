<?php
class Administrativo_planejamento_estrategico_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_administrativo_planejamento_estrategico,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.administrativo_planejamento_estrategico i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.administrativo_planejamento_estrategico i
	...
	function carrega_referencia()
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia_n, 
				   ...
				   nr_meta, 
				   cd_indicador_tabela 
			  FROM indicador_plugin.administrativo_planejamento_estrategico
	...
	function carrega($cd_administrativo_planejamento_estrategico)
	{
		$qr_sql = "
            SELECT cd_administrativo_planejamento_estrategico,
                   ...
                   observacao
		      FROM indicador_plugin.administrativo_planejamento_estrategico 
			 WHERE cd_administrativo_planejamento_estrategico = ".intval($cd_administrativo_planejamento_estrategico).";";
	...
	{
		$qr_sql = "
			INSERT INTO indicador_plugin.administrativo_planejamento_estrategico 
			     (
					dt_referencia, 
				    nr_valor_1, 
                    nr_valor_2, 
	...
	public function atualizar($cd_administrativo_planejamento_estrategico, $args = array())
	{
		$qr_sql = "
			UPDATE indicador_plugin.administrativo_planejamento_estrategico
	...
	function excluir($cd_administrativo_planejamento_estrategico, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_plugin.administrativo_planejamento_estrategico
