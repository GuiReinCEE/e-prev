<?php
class Juridico_sucesso_acoes_re_civel_model extends Model
{
	...
	{
		$qr_sql = "
					SELECT i.cd_juridico_sucesso_acoes_re_civel,
						   ...
						   i.nr_percentual_f,
						   i.nr_meta
					  FROM indicador_plugin.juridico_sucesso_acoes_re_civel i
		...
		$qr_sql = "
					SELECT x.value, 
					 ...
					 WHERE x.value NOT IN (SELECT i.cd_etapa
					                         FROM indicador_plugin.juridico_sucesso_acoes_re_civel i
								            WHERE i.dt_exclusao IS NULL
											  AND i.cd_juridico_sucesso_acoes_re_civel <> ".intval($args['cd_juridico_sucesso_acoes_re_civel'])."
	...
	{
		$qr_sql = "
					SELECT cd_juridico_sucesso_acoes_re_civel,
					 ...
					 WHERE cd_juridico_sucesso_acoes_re_civel = ".intval($args['cd_juridico_sucesso_acoes_re_civel']).";
			      ";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
						INSERT INTO indicador_plugin.juridico_sucesso_acoes_re_civel 
							 (
								dt_referencia, 
						        cd_etapa,
						        nr_valor_1,
		...
		{
			$qr_sql = "
						UPDATE indicador_plugin.juridico_sucesso_acoes_re_civel
