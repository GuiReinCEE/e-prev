<?php
class Beneficio_incorrecao_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_beneficio_incorrecao={cd_beneficio_incorrecao} ";
			esc( "{cd_beneficio_incorrecao}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_beneficio_incorrecao'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.beneficio_incorrecao ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.beneficio_incorrecao 
			SET 
			
			cd_beneficio_incorrecao = {cd_beneficio_incorrecao} 
		...
		esc("{cd_beneficio_incorrecao}", $args["cd_beneficio_incorrecao"], $sql, "int", FALSE);
		...
		esc("{nr_beneficio}", $args["nr_beneficio"], $sql, "float", FALSE);
		esc("{nr_beneficio_erro}", $args["nr_beneficio_erro"], $sql, "float", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_beneficio_incorrecao::varchar)='{cd_beneficio_incorrecao}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_beneficio_incorrecao}', $id, $sql, 'str'); 
