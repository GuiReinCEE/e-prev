<?php
class atuarial_eap_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atuarial_eap={cd_atuarial_eap} ";
			esc( "{cd_atuarial_eap}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atuarial_eap'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.atuarial_eap ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.atuarial_eap 
			SET 
			
			cd_atuarial_eap = {cd_atuarial_eap} 
		...
		esc("{cd_atuarial_eap}", $args["cd_atuarial_eap"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atuarial_eap::varchar)='{cd_atuarial_eap}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atuarial_eap}', $id, $sql, 'str'); 
