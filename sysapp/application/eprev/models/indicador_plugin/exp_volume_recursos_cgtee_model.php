<?php
class Exp_volume_recursos_cgtee_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_exp_volume_recursos_cgtee,
			  ...
			  FROM indicador_plugin.exp_volume_recursos_cgtee i
	...
	public function carrega_referencia($cd_indicador_tabela)
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS dt_referencia_n, 
				   nr_meta, 
				   cd_indicador_tabela 
			  FROM indicador_plugin.exp_volume_recursos_cgtee 
	...
	public function carrega($cd_exp_volume_recursos_cgtee)
	{
		$qr_sql = "
			SELECT cd_exp_volume_recursos_cgtee,
			  ...
			  FROM indicador_plugin.exp_volume_recursos_cgtee 
			 WHERE dt_exclusao IS NULL
			   AND cd_exp_volume_recursos_cgtee = ".intval($cd_exp_volume_recursos_cgtee)."
	...
	{
		$qr_sql = "
			INSERT INTO indicador_plugin.exp_volume_recursos_cgtee
			     (
                   dt_referencia, 
                   cd_indicador_tabela, 
                   fl_media, 
	...
	public function atualizar($cd_exp_volume_recursos_cgtee, $args)
	{
		$qr_sql = "
			UPDATE indicador_plugin.exp_volume_recursos_cgtee
			 ...
			 WHERE cd_exp_volume_recursos_cgtee = ".intval($cd_exp_volume_recursos_cgtee).";";

		$this->db->query($qr_sql);	
	}
	
	public function excluir($cd_exp_volume_recursos_cgtee, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_plugin.exp_volume_recursos_cgtee 
