<?php
class Exp_adesao_potencial_sintae_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_exp_adesao_potencial_sintae,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.exp_adesao_potencial_sintae i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.exp_adesao_potencial_sintae i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
                   ...
                   nr_valor_2,
                   nr_valor_3
			  FROM indicador_plugin.exp_adesao_potencial_sintae
	...
	{
		$qr_sql = "
            SELECT cd_exp_adesao_potencial_sintae,
                   ...
                   observacao
		      FROM indicador_plugin.exp_adesao_potencial_sintae 
			 WHERE cd_exp_adesao_potencial_sintae = ".intval($args['cd_exp_adesao_potencial_sintae']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.exp_adesao_potencial_sintae 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_valor_2, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.exp_adesao_potencial_sintae
