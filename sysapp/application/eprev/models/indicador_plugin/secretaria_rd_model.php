<?php
class secretaria_rd_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_secretaria_rd={cd_secretaria_rd} ";
			esc( "{cd_secretaria_rd}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_secretaria_rd'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.secretaria_rd ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.secretaria_rd 
			SET 
			
			cd_secretaria_rd = {cd_secretaria_rd} 
		...
		esc("{cd_secretaria_rd}", $args["cd_secretaria_rd"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_secretaria_rd::varchar)='{cd_secretaria_rd}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_secretaria_rd}', $id, $sql, 'str'); 
