<?php
class beneficio_inc_patroc_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_beneficio_inc_patroc={cd_beneficio_inc_patroc} ";
			esc( "{cd_beneficio_inc_patroc}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_beneficio_inc_patroc'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.beneficio_inc_patroc ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.beneficio_inc_patroc 
			SET 
			
			cd_beneficio_inc_patroc = {cd_beneficio_inc_patroc} 
		...
		esc("{cd_beneficio_inc_patroc}", $args["cd_beneficio_inc_patroc"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_beneficio_inc_patroc::varchar)='{cd_beneficio_inc_patroc}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_beneficio_inc_patroc}', $id, $sql, 'str'); 
