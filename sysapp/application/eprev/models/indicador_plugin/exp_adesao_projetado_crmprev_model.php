<?php
class exp_adesao_projetado_crmprev_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_exp_adesao_projetado_crmprev,
				   ...
				   i.nr_percentual_f,
				   i.nr_meta
		      FROM indicador_plugin.exp_adesao_projetado_crmprev i
	...
	public function carrega_referencia()
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia_n,
				   nr_valor_1, 
				   cd_indicador_tabela 
			  FROM indicador_plugin.exp_adesao_projetado_crmprev
	...
	public function carrega($cd_exp_adesao_projetado_crmprev)
	{
		$qr_sql = "
            SELECT cd_exp_adesao_projetado_crmprev,
                   ...
                   observacao
		      FROM indicador_plugin.exp_adesao_projetado_crmprev 
			 WHERE cd_exp_adesao_projetado_crmprev = ".intval($cd_exp_adesao_projetado_crmprev).";";
	...
	{
		if(intval($args['cd_exp_adesao_projetado_crmprev']) == 0)
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.exp_adesao_projetado_crmprev 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_meta, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.exp_adesao_projetado_crmprev
			     ...
			     WHERE cd_exp_adesao_projetado_crmprev = ".intval($args['cd_exp_adesao_projetado_crmprev']).";";
		}

		$this->db->query($qr_sql);
	}
	
	public function excluir($cd_exp_adesao_projetado_crmprev, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_plugin.exp_adesao_projetado_crmprev
