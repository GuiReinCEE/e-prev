<?php
class controladoria_divulgacao_ci_prazo_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT c1.cd_controladoria_divulgacao_ci_prazo,
				   ...
				   c1.fl_media,
				   c1.ds_observacao
		      FROM indicador_plugin.controladoria_divulgacao_ci_prazo c1
	...
	public function carrega_referencia($nr_ano_referencia)
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia_n, 
				   ...
				   cd_indicador_tabela,
				   (SELECT COUNT(*)
				      FROM indicador_plugin.controladoria_divulgacao_ci_prazo
	...
	public function carrega($cd_controladoria_divulgacao_ci_prazo)
	{
		$qr_sql = "
            SELECT cd_controladoria_divulgacao_ci_prazo,
                   ...
                   ds_observacao
		      FROM indicador_plugin.controladoria_divulgacao_ci_prazo 
			 WHERE cd_controladoria_divulgacao_ci_prazo = ".intval($cd_controladoria_divulgacao_ci_prazo).";";
	...
	{
		if(intval($args['cd_controladoria_divulgacao_ci_prazo']) == 0)
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.controladoria_divulgacao_ci_prazo 
				     (
				        cd_indicador_tabela, 
						dt_referencia, 
					    fl_media,
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.controladoria_divulgacao_ci_prazo
			     ...
			     WHERE cd_controladoria_divulgacao_ci_prazo = ".intval($args['cd_controladoria_divulgacao_ci_prazo']).";";
		}

		$this->db->query($qr_sql);
	}
	
	public function excluir($cd_controladoria_divulgacao_ci_prazo, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_plugin.controladoria_divulgacao_ci_prazo
