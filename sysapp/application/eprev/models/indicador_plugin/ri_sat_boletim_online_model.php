<?php
class ri_sat_boletim_online_model extends Model
{
	...
	function listar( &$result, $args=array() )
	{
		// mount query
		$qr_sql = "
					SELECT i.cd_ri_sat_boletim_online,
						   ...
						   CASE WHEN (SELECT MAX(i1.dt_referencia)
						                FROM indicador_plugin.ri_sat_boletim_online i1
					                   WHERE i1.dt_exclusao IS NULL) = i.dt_referencia
								THEN 'S'
								ELSE 'N'
						   END AS fl_editar
					  FROM indicador_plugin.ri_sat_boletim_online i
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_ri_sat_boletim_online={cd_ri_sat_boletim_online} ";
			esc( "{cd_ri_sat_boletim_online}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_ri_sat_boletim_online'])==0)
		{
			$sql="
		...
		{
			$sql="
			UPDATE indicador_plugin.ri_sat_boletim_online
			SET 
			
			cd_ri_sat_boletim_online = {cd_ri_sat_boletim_online}
		...
		esc("{cd_ri_sat_boletim_online}", $args["cd_ri_sat_boletim_online"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_ri_sat_boletim_online::varchar)='{cd_ri_sat_boletim_online}'
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_ri_sat_boletim_online}', $id, $sql, 'str');
