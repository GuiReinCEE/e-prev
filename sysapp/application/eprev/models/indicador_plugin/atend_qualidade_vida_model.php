<?php
class atend_qualidade_vida_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atend_qualidade_vida={cd_atend_qualidade_vida} ";
			esc( "{cd_atend_qualidade_vida}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atend_qualidade_vida'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.atend_qualidade_vida ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.atend_qualidade_vida 
			SET 
			
			cd_atend_qualidade_vida = {cd_atend_qualidade_vida} 
		...
		esc("{cd_atend_qualidade_vida}", $args["cd_atend_qualidade_vida"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atend_qualidade_vida::varchar)='{cd_atend_qualidade_vida}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atend_qualidade_vida}', $id, $sql, 'str'); 
