<?php
class Juridico_sucesso_acoes_autora_consolidado_model extends Model
{
	...
	{
		$qr_sql = "
					SELECT i.cd_juridico_sucesso_acoes_autora_consolidado,
						   ...
						   i.nr_percentual_f,
						   i.nr_meta
					  FROM indicador_plugin.juridico_sucesso_acoes_autora_consolidado i
		...
		$qr_sql = "
					SELECT x.value, 
					 ...
					 WHERE x.value NOT IN (SELECT i.cd_etapa
					                         FROM indicador_plugin.juridico_sucesso_acoes_autora_consolidado i
								            WHERE i.dt_exclusao IS NULL
											  AND i.cd_juridico_sucesso_acoes_autora_consolidado <> ".intval($args['cd_juridico_sucesso_acoes_autora_consolidado'])."
	...
	{
		$qr_sql = "
					SELECT cd_juridico_sucesso_acoes_autora_consolidado,
						   ...
						   nr_percentual_f,
						   nr_meta
					  FROM indicador_plugin.juridico_sucesso_acoes_autora_consolidado 
					 WHERE cd_juridico_sucesso_acoes_autora_consolidado = ".intval($args['cd_juridico_sucesso_acoes_autora_consolidado']).";
			      ";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
						INSERT INTO indicador_plugin.juridico_sucesso_acoes_autora_consolidado 
							 (
								dt_referencia, 
						        cd_etapa,
						        nr_valor_1,
		...
		{
			$qr_sql = "
						UPDATE indicador_plugin.juridico_sucesso_acoes_autora_consolidado
	...
	public function get_valores($nr_ano, $cd_etapa)
	{
		$qr_sql = "
			...
			SELECT nr_valor_1,
			       ...
			       nr_valor_4,
			       nr_meta
			  FROM indicador_plugin.juridico_sucesso_acoes_autora_civel
