<?php
class Secretaria_sumulas_cf_model extends Model
{
	...
	public function listar( &$result, $args=array() )
	{
		...
		$qr_sql = "
			SELECT i.cd_secretaria_sumulas_cf,
				   ...
				   i.nr_percentual_f,
				   i.nr_meta
			  FROM indicador_plugin.secretaria_sumulas_cf i
	...
	public function carrega_referencia(&$result)
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS mes_referencia, 
				   ...
				   nr_meta, 
				   cd_indicador_tabela
			  FROM indicador_plugin.secretaria_sumulas_cf 
	...
	{
		$qr_sql = " 
			SELECT cd_secretaria_sumulas_cf, 
				   ...
				   nr_meta 
			  FROM indicador_plugin.secretaria_sumulas_cf 
		     WHERE cd_secretaria_sumulas_cf = ".intval($args['cd_secretaria_sumulas_cf']).";";
	...
	function salvar($args)
	{
		...
		{
			$qr_sql="
			  INSERT INTO indicador_plugin.secretaria_sumulas_cf 
			  		(
			  			dt_referencia, 
						dt_inclusao,
						nr_meta, 
		...
		{
			$qr_sql="
				UPDATE indicador_plugin.secretaria_sumulas_cf 
				   SET cd_secretaria_sumulas_cf = ".(intval($args['cd_secretaria_sumulas_cf']) == 0 ? "DEFAULT" : intval($args['cd_secretaria_sumulas_cf'])).", 
	...
	public function excluir($cd_secretaria_sumulas_cf, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_plugin.secretaria_sumulas_cf 
			   SET dt_exclusao         = CURRENT_TIMESTAMP, 
			   	   cd_usuario_exclusao = ".intval($cd_usuario)." 
			 WHERE cd_secretaria_sumulas_cf = ".intval($cd_secretaria_sumulas_cf).";";
