<?php
class Financeiro_inadimplencia_patrocinadoras_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_financeiro_inadimplencia_patrocinadoras,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.financeiro_inadimplencia_patrocinadoras i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.financeiro_inadimplencia_patrocinadoras i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
				   ...
				   nr_valor_1,
                   nr_valor_2
			  FROM indicador_plugin.financeiro_inadimplencia_patrocinadoras
	...
	{
		$qr_sql = "
            SELECT cd_financeiro_inadimplencia_patrocinadoras,
                   ...
                   observacao
		      FROM indicador_plugin.financeiro_inadimplencia_patrocinadoras 
			 WHERE cd_financeiro_inadimplencia_patrocinadoras = ".intval($args['cd_financeiro_inadimplencia_patrocinadoras']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.financeiro_inadimplencia_patrocinadoras 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_valor_2, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.financeiro_inadimplencia_patrocinadoras
