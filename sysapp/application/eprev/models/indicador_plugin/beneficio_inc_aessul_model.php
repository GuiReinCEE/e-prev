<?php
class beneficio_inc_aessul_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_beneficio_inc_aessul={cd_beneficio_inc_aessul} ";
			esc( "{cd_beneficio_inc_aessul}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_beneficio_inc_aessul'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.beneficio_inc_aessul ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.beneficio_inc_aessul 
			SET 
			
			cd_beneficio_inc_aessul = {cd_beneficio_inc_aessul} 
		...
		esc("{cd_beneficio_inc_aessul}", $args["cd_beneficio_inc_aessul"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_beneficio_inc_aessul::varchar)='{cd_beneficio_inc_aessul}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_beneficio_inc_aessul}', $id, $sql, 'str'); 
