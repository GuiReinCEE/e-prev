<?php
class Financeiro_inadimplencia_participantes_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT cd_financeiro_inadimplencia_participantes,
				   cd_indicador_tabela,
				   ...
				   nr_meta,
				   fl_media
			  FROM indicador_plugin.financeiro_inadimplencia_participantes 
	...
	public function carrega($cd_financeiro_inadimplencia_participantes)
	{
		$qr_sql = "
            SELECT cd_financeiro_inadimplencia_participantes,
                   ...
                   observacao
		      FROM indicador_plugin.financeiro_inadimplencia_participantes 
			 WHERE cd_financeiro_inadimplencia_participantes = ".intval($cd_financeiro_inadimplencia_participantes).";";
	...
	{
		if(intval($args['cd_financeiro_inadimplencia_participantes']) == 0)
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.financeiro_inadimplencia_participantes 
				     (
				     	cd_indicador_tabela, 
						dt_referencia, 
					    nr_valor_1, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.financeiro_inadimplencia_participantes
			     ...
			     WHERE cd_financeiro_inadimplencia_participantes = ".intval($args['cd_financeiro_inadimplencia_participantes']).";";
		}

		$this->db->query($qr_sql);
	}
	
	public function excluir($cd_financeiro_inadimplencia_participantes, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_plugin.financeiro_inadimplencia_participantes
