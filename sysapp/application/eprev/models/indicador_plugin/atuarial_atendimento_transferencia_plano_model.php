<?php
class Atuarial_atendimento_transferencia_plano_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_atuarial_atendimento_transferencia_plano,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.atuarial_atendimento_transferencia_plano i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.atuarial_atendimento_transferencia_plano i
	...
	function carrega_referencia($nr_ano_referencia)
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia_n, 
                   ...
                   nr_realizadas,
				   (SELECT COUNT(*)
				      FROM indicador_plugin.atuarial_atendimento_transferencia_plano
	...
	function carrega($cd_atuarial_atendimento_transferencia_plano)
	{
		$qr_sql = "
            SELECT cd_atuarial_atendimento_transferencia_plano,
                   ...
                   ds_observacao
		      FROM indicador_plugin.atuarial_atendimento_transferencia_plano 
			 WHERE cd_atuarial_atendimento_transferencia_plano = ".intval($cd_atuarial_atendimento_transferencia_plano).";";
	...
	{
		$qr_sql = "
			INSERT INTO indicador_plugin.atuarial_atendimento_transferencia_plano 
			     (
					dt_referencia, 
				    nr_tarefas, 
                    nr_realizadas, 
	...
	public function atualizar($cd_atuarial_atendimento_transferencia_plano, $args = array())
	{
		$qr_sql = "
			UPDATE indicador_plugin.atuarial_atendimento_transferencia_plano
	...
	function excluir($cd_atuarial_atendimento_transferencia_plano, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_plugin.atuarial_atendimento_transferencia_plano
