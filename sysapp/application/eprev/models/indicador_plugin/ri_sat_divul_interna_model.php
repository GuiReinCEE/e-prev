<?php
class ri_sat_divul_interna_model extends Model
{
	...
	function listar( &$result, $args=array() )
	{
		// mount query
		$qr_sql = "
					SELECT i.cd_ri_sat_divul_interna,
						   ...
						   CASE WHEN (SELECT MAX(i1.dt_referencia)
						                FROM indicador_plugin.ri_sat_divul_interna i1
					                   WHERE i1.dt_exclusao IS NULL) = i.dt_referencia
								THEN 'S'
								ELSE 'N'
						   END AS fl_editar
					  FROM indicador_plugin.ri_sat_divul_interna i
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_ri_sat_divul_interna={cd_ri_sat_divul_interna} ";
			esc( "{cd_ri_sat_divul_interna}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_ri_sat_divul_interna'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.ri_sat_divul_interna (
		...
		{
			$sql="
			UPDATE indicador_plugin.ri_sat_divul_interna
			SET 
			
			cd_ri_sat_divul_interna = {cd_ri_sat_divul_interna}
		...
		esc("{cd_ri_sat_divul_interna}", $args["cd_ri_sat_divul_interna"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_ri_sat_divul_interna::varchar)='{cd_ri_sat_divul_interna}'
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_ri_sat_divul_interna}', $id, $sql, 'str');
