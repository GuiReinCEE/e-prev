<?php
class beneficio_inc_sintep_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_beneficio_inc_sintep={cd_beneficio_inc_sintep} ";
			esc( "{cd_beneficio_inc_sintep}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_beneficio_inc_sintep'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.beneficio_inc_sintep ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.beneficio_inc_sintep 
			SET 
			
			cd_beneficio_inc_sintep = {cd_beneficio_inc_sintep} 
		...
		esc("{cd_beneficio_inc_sintep}", $args["cd_beneficio_inc_sintep"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_beneficio_inc_sintep::varchar)='{cd_beneficio_inc_sintep}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_beneficio_inc_sintep}', $id, $sql, 'str'); 
