<?php
class administrativo_he_valor_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_administrativo_he_valor={cd_administrativo_he_valor} ";
			esc( "{cd_administrativo_he_valor}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_administrativo_he_valor'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.administrativo_he_valor ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.administrativo_he_valor 
			SET 
			
			cd_administrativo_he_valor = {cd_administrativo_he_valor} 
		...
		esc("{cd_administrativo_he_valor}", $args["cd_administrativo_he_valor"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_administrativo_he_valor::varchar)='{cd_administrativo_he_valor}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_administrativo_he_valor}', $id, $sql, 'str'); 
