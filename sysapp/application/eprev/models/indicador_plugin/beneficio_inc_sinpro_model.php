<?php
class beneficio_inc_sinpro_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_beneficio_inc_sinpro={cd_beneficio_inc_sinpro} ";
			esc( "{cd_beneficio_inc_sinpro}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_beneficio_inc_sinpro'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.beneficio_inc_sinpro ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.beneficio_inc_sinpro 
			SET 
			
			cd_beneficio_inc_sinpro = {cd_beneficio_inc_sinpro} 
		...
		esc("{cd_beneficio_inc_sinpro}", $args["cd_beneficio_inc_sinpro"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_beneficio_inc_sinpro::varchar)='{cd_beneficio_inc_sinpro}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_beneficio_inc_sinpro}', $id, $sql, 'str'); 
