<?php
class Atend_com_retorno_model extends Model
{
		...
		$qr_sql = "
			SELECT i.cd_atend_com_retorno,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.atend_com_retorno i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.atend_com_retorno i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
				   nr_meta, 
				   cd_indicador_tabela 
			  FROM indicador_plugin.atend_com_retorno
	...
	function carrega(&$result, $args=array())
	{
		$qr_sql = "
            SELECT cd_atend_com_retorno,
                   ...
                   observacao
		      FROM indicador_plugin.atend_com_retorno 
			 WHERE cd_atend_com_retorno = ".intval($args['cd_atend_com_retorno']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.atend_com_retorno 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_valor_2, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.atend_com_retorno
