<?php
class atuarial_eap_crmprev_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atuarial_eap_crmprev={cd_atuarial_eap_crmprev} ";
			esc( "{cd_atuarial_eap_crmprev}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atuarial_eap_crmprev'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.atuarial_eap_crmprev ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.atuarial_eap_crmprev 
			SET 
			
			cd_atuarial_eap_crmprev = {cd_atuarial_eap_crmprev} 
		...
		esc("{cd_atuarial_eap_crmprev}", $args["cd_atuarial_eap_crmprev"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atuarial_eap_crmprev::varchar)='{cd_atuarial_eap_crmprev}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atuarial_eap_crmprev}', $id, $sql, 'str'); 
