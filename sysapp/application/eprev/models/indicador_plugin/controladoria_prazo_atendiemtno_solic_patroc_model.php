<?php
class Controladoria_prazo_atendiemtno_solic_patroc_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_controladoria_prazo_atendiemtno_solic_patroc,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.controladoria_prazo_atendiemtno_solic_patroc i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.controladoria_prazo_atendiemtno_solic_patroc i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
				   nr_meta, 
				   cd_indicador_tabela 
			  FROM indicador_plugin.controladoria_prazo_atendiemtno_solic_patroc
	...
	{
		$qr_sql = "
            SELECT cd_controladoria_prazo_atendiemtno_solic_patroc,
                   ...
                   observacao
		      FROM indicador_plugin.controladoria_prazo_atendiemtno_solic_patroc 
			 WHERE cd_controladoria_prazo_atendiemtno_solic_patroc = ".intval($args['cd_controladoria_prazo_atendiemtno_solic_patroc']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.controladoria_prazo_atendiemtno_solic_patroc 
				     (
						dt_referencia, 
						ds_evento,
					    nr_valor_1, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.controladoria_prazo_atendiemtno_solic_patroc
