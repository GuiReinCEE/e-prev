<?php
class Exp_adesao_potencial_model extends Model
{
		...
		$qr_sql = "
			SELECT i.cd_exp_adesao_potencial,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.exp_adesao_potencial i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
				   ...
				   cd_indicador_tabela,
				   nr_resultado
			  FROM indicador_plugin.exp_adesao_potencial
	...
	{
		$qr_sql = "
            SELECT cd_exp_adesao_potencial,
                   ...
                   observacao
		      FROM indicador_plugin.exp_adesao_potencial 
			 WHERE cd_exp_adesao_potencial = ".intval($args['cd_exp_adesao_potencial']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.exp_adesao_potencial 
				     (
						dt_referencia, 
					    nr_meta, 
					    cd_indicador_tabela, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.exp_adesao_potencial
