<?php
class Comunicacao_sat_canais_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_comunicacao_sat_canais,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.comunicacao_sat_canais i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.comunicacao_sat_canais i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 year'::interval, 'DD/MM/YYYY') AS dt_referencia, 
				   ...
				   nr_meta, 
				   cd_indicador_tabela 
			  FROM indicador_plugin.comunicacao_sat_canais
	...
	{
		$qr_sql = "
            SELECT cd_comunicacao_sat_canais,
                   ...
                   observacao
		      FROM indicador_plugin.comunicacao_sat_canais 
			 WHERE cd_comunicacao_sat_canais = ".intval($args['cd_comunicacao_sat_canais']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.comunicacao_sat_canais 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_valor_2, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.comunicacao_sat_canais
