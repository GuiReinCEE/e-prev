<?php
class beneficio_inc_ceran_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_beneficio_inc_ceran={cd_beneficio_inc_ceran} ";
			esc( "{cd_beneficio_inc_ceran}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_beneficio_inc_ceran'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.beneficio_inc_ceran ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.beneficio_inc_ceran 
			SET 
			
			cd_beneficio_inc_ceran = {cd_beneficio_inc_ceran} 
		...
		esc("{cd_beneficio_inc_ceran}", $args["cd_beneficio_inc_ceran"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_beneficio_inc_ceran::varchar)='{cd_beneficio_inc_ceran}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_beneficio_inc_ceran}', $id, $sql, 'str'); 
