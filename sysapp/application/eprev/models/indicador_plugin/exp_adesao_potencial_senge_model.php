<?php
class Exp_adesao_potencial_senge_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_exp_adesao_potencial_senge,
				   ...
				   i.nr_percentual_f,
				   i.nr_meta
		      FROM indicador_plugin.exp_adesao_potencial_senge i
	...
	public function carrega_referencia()
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
				   ...
				   nr_meta
			  FROM indicador_plugin.exp_adesao_potencial_senge
	...
	public function carrega($cd_exp_adesao_potencial_senge)
	{
		$qr_sql = "
            SELECT cd_exp_adesao_potencial_senge,
                   ...
                   observacao
		      FROM indicador_plugin.exp_adesao_potencial_senge 
			 WHERE cd_exp_adesao_potencial_senge = ".intval($cd_exp_adesao_potencial_senge).";";
	...
	{
		if(intval($args['cd_exp_adesao_potencial_senge']) == 0)
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.exp_adesao_potencial_senge 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_valor_2, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.exp_adesao_potencial_senge
			     ...
			     WHERE cd_exp_adesao_potencial_senge = ".intval($args['cd_exp_adesao_potencial_senge']).";";
		}

		$this->db->query($qr_sql);
	}
	
	public function excluir($cd_exp_adesao_potencial_senge, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_plugin.exp_adesao_potencial_senge
