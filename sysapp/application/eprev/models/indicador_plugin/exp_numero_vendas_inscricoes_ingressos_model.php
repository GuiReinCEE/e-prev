<?php
class exp_numero_vendas_inscricoes_ingressos_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_exp_numero_vendas_inscricoes_ingressos,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.exp_numero_vendas_inscricoes_ingressos i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.exp_numero_vendas_inscricoes_ingressos i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
                   ...
                   nr_valor_2,
                   nr_valor_3
			  FROM indicador_plugin.exp_numero_vendas_inscricoes_ingressos
	...
	{
		$qr_sql = "
            SELECT cd_exp_numero_vendas_inscricoes_ingressos,
                   ...
                   observacao
		      FROM indicador_plugin.exp_numero_vendas_inscricoes_ingressos 
			 WHERE cd_exp_numero_vendas_inscricoes_ingressos = ".intval($args['cd_exp_numero_vendas_inscricoes_ingressos']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.exp_numero_vendas_inscricoes_ingressos 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_valor_2, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.exp_numero_vendas_inscricoes_ingressos
