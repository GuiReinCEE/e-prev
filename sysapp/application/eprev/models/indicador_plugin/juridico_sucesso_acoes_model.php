<?php
class juridico_sucesso_acoes_model extends Model
{
	...
	{
		$qr_sql = "
					SELECT i.cd_juridico_sucesso_acoes,
						   ...
						   i.nr_meta_min,
						   i.nr_meta_max
					  FROM indicador_plugin.juridico_sucesso_acoes i
		...
		$qr_sql = "
					SELECT x.value, 
					 ...
					 WHERE x.value NOT IN (SELECT i.cd_etapa
					                         FROM indicador_plugin.juridico_sucesso_acoes i
								            WHERE i.dt_exclusao IS NULL
											  AND i.cd_juridico_sucesso_acoes <> ".intval($args['cd_juridico_sucesso_acoes'])."
	...
	{
		$qr_sql = "
					SELECT cd_juridico_sucesso_acoes,
						   ...
						   nr_meta_min,
						   nr_meta_max
					  FROM indicador_plugin.juridico_sucesso_acoes 
					 WHERE cd_juridico_sucesso_acoes = ".intval($args['cd_juridico_sucesso_acoes']).";
			      ";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
						INSERT INTO indicador_plugin.juridico_sucesso_acoes 
							 (
								dt_referencia, 
						        cd_etapa,
						        nr_valor_1,
		...
		{
			$qr_sql = "
						UPDATE indicador_plugin.juridico_sucesso_acoes
