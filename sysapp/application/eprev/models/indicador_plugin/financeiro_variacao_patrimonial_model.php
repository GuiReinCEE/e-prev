<?php
class Financeiro_variacao_patrimonial_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_financeiro_variacao_patrimonial,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.financeiro_variacao_patrimonial i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.financeiro_variacao_patrimonial i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
				   ...
				   nr_valor_1,
                   nr_valor_2
			  FROM indicador_plugin.financeiro_variacao_patrimonial
	...
	{
		$qr_sql = "
            SELECT cd_financeiro_variacao_patrimonial,
                   ...
                   observacao
		      FROM indicador_plugin.financeiro_variacao_patrimonial 
			 WHERE cd_financeiro_variacao_patrimonial = ".intval($args['cd_financeiro_variacao_patrimonial']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.financeiro_variacao_patrimonial 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_valor_2, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.financeiro_variacao_patrimonial
