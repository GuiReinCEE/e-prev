<?php
class administrativo_suprimento_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_administrativo_suprimento={cd_administrativo_suprimento} ";
			esc( "{cd_administrativo_suprimento}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_administrativo_suprimento'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.administrativo_suprimento ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.administrativo_suprimento 
			SET 
			
			cd_administrativo_suprimento = {cd_administrativo_suprimento} 
		...
		esc("{cd_administrativo_suprimento}", $args["cd_administrativo_suprimento"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_administrativo_suprimento::varchar)='{cd_administrativo_suprimento}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_administrativo_suprimento}', $id, $sql, 'str'); 
