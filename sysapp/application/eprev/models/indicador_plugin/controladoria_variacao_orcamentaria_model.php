<?php
class Controladoria_variacao_orcamentaria_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT cd_controladoria_variacao_orcamentaria,
				   cd_indicador_tabela,
				   ...
				   fl_media,
				   observacao
			  FROM indicador_plugin.controladoria_variacao_orcamentaria 
	...
	function carrega(&$result, $args=array())
	{
		$qr_sql = "
			SELECT cd_controladoria_variacao_orcamentaria,
				   ...
				   observacao
			  FROM indicador_plugin.controladoria_variacao_orcamentaria
			 WHERE cd_controladoria_variacao_orcamentaria = ".intval($args['cd_controladoria_variacao_orcamentaria']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.controladoria_variacao_orcamentaria
				     (
                       dt_referencia, 
					   nr_valor_1, 
                       nr_valor_2, 
                ...
                VALUES 
				     (
					   ...
					   ".(trim($args['nr_meta_2']) == "" ? "DEFAULT" : floatval($args['nr_meta_2'])).",
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.controladoria_variacao_orcamentaria
				   SET dt_referencia        = ".(trim($args['dt_referencia']) == "" ? "DEFAULT" : "TO_DATE('".trim($args["dt_referencia"])."', 'DD/MM/YYYY')").",
	                   ...
	                   nr_meta_2            = ".(trim($args['nr_meta_2']) == "" ? "DEFAULT" : floatval($args['nr_meta_2'])).",
