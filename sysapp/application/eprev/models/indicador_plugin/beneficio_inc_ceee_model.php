<?php
class beneficio_inc_ceee_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_beneficio_inc_ceee={cd_beneficio_inc_ceee} ";
			esc( "{cd_beneficio_inc_ceee}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_beneficio_inc_ceee'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.beneficio_inc_ceee ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.beneficio_inc_ceee 
			SET 
			
			cd_beneficio_inc_ceee = {cd_beneficio_inc_ceee} 
		...
		esc("{cd_beneficio_inc_ceee}", $args["cd_beneficio_inc_ceee"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_beneficio_inc_ceee::varchar)='{cd_beneficio_inc_ceee}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_beneficio_inc_ceee}', $id, $sql, 'str'); 
