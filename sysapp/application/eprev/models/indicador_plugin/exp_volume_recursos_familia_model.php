<?php
class Exp_volume_recursos_familia_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_exp_volume_recursos_familia,
			  ...
			  FROM indicador_plugin.exp_volume_recursos_familia i
	...
	public function carrega_referencia($cd_indicador_tabela)
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS dt_referencia_n, 
				   nr_meta, 
				   cd_indicador_tabela 
			  FROM indicador_plugin.exp_volume_recursos_familia 
	...
	public function carrega($cd_exp_volume_recursos_familia)
	{
		$qr_sql = "
			SELECT cd_exp_volume_recursos_familia,
			  ...
			  FROM indicador_plugin.exp_volume_recursos_familia 
			 WHERE dt_exclusao IS NULL
			   AND cd_exp_volume_recursos_familia = ".intval($cd_exp_volume_recursos_familia)."
	...
	{
		$qr_sql = "
			INSERT INTO indicador_plugin.exp_volume_recursos_familia
			     (
                   dt_referencia, 
                   cd_indicador_tabela, 
                   fl_media, 
	...
	public function atualizar($cd_exp_volume_recursos_familia, $args)
	{
		$qr_sql = "
			UPDATE indicador_plugin.exp_volume_recursos_familia
			 ...
			 WHERE cd_exp_volume_recursos_familia = ".intval($cd_exp_volume_recursos_familia).";";

		$this->db->query($qr_sql);	
	}
	
	public function excluir($cd_exp_volume_recursos_familia, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_plugin.exp_volume_recursos_familia 
