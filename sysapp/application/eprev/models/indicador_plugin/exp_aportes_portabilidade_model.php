<?php
class Exp_aportes_portabilidade_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_exp_aportes_portabilidade,
			  ...
			  FROM indicador_plugin.exp_aportes_portabilidade i
	...
	public function carrega_referencia($cd_indicador_tabela)
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS dt_referencia_n, 
				   nr_meta, 
				   cd_indicador_tabela 
			  FROM indicador_plugin.exp_aportes_portabilidade 
	...
	public function carrega($cd_exp_aportes_portabilidade)
	{
		$qr_sql = "
			SELECT cd_exp_aportes_portabilidade,
			  ...
			  FROM indicador_plugin.exp_aportes_portabilidade 
			 WHERE dt_exclusao IS NULL
			   AND cd_exp_aportes_portabilidade = ".intval($cd_exp_aportes_portabilidade)."
	...
	{
		$qr_sql = "
			INSERT INTO indicador_plugin.exp_aportes_portabilidade
			     (
                   dt_referencia, 
                   cd_indicador_tabela, 
                   fl_media, 
	...
	public function atualizar($cd_exp_aportes_portabilidade, $args)
	{
		$qr_sql = "
			UPDATE indicador_plugin.exp_aportes_portabilidade
			 ...
			 WHERE cd_exp_aportes_portabilidade = ".intval($cd_exp_aportes_portabilidade).";";

		$this->db->query($qr_sql);	
	}
	
	public function excluir($cd_exp_aportes_portabilidade, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_plugin.exp_aportes_portabilidade 
