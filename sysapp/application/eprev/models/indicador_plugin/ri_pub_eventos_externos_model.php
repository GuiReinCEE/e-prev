<?php
class ri_pub_eventos_externos_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_ri_pub_eventos_externos,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.ri_pub_eventos_externos i1
                               WHERE i1.dt_exclusao IS NULL) = i.dt_referencia THEN 'S'
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.ri_pub_eventos_externos i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT CASE WHEN TO_CHAR(dt_referencia, 'MM') = '02' THEN TO_CHAR(dt_referencia + '1 year'::interval, 'YYYY')
				   ...
				   nr_meta, 
				   cd_indicador_tabela 
			  FROM indicador_plugin.ri_pub_eventos_externos
	...
	{
		$qr_sql = "
            SELECT cd_ri_pub_eventos_externos,
                   ...
                   observacao
		      FROM indicador_plugin.ri_pub_eventos_externos 
			 WHERE cd_ri_pub_eventos_externos = ".intval($args['cd_ri_pub_eventos_externos']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.ri_pub_eventos_externos
				     (
                       dt_referencia, 
					   nr_valor_1, 
                       nr_valor_2, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.ri_pub_eventos_externos
