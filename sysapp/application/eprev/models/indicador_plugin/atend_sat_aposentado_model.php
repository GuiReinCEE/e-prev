<?php
class atend_sat_aposentado_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atend_sat_aposentado={cd_atend_sat_aposentado} ";
			esc( "{cd_atend_sat_aposentado}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atend_sat_aposentado'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.atend_sat_aposentado ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.atend_sat_aposentado 
			SET 
			
			cd_atend_sat_aposentado = {cd_atend_sat_aposentado} 
		...
		esc("{cd_atend_sat_aposentado}", $args["cd_atend_sat_aposentado"], $sql, "int", FALSE);
		...
		esc("{nr_valor_0}", $args["nr_valor_0"], $sql, "float", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atend_sat_aposentado::varchar)='{cd_atend_sat_aposentado}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atend_sat_aposentado}', $id, $sql, 'str'); 
