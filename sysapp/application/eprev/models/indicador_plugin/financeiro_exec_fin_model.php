<?php
class financeiro_exec_fin_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_financeiro_exec_fin,
				   ...
				   i.nr_percentual_f,
				   i.nr_meta
		      FROM indicador_plugin.financeiro_exec_fin i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia_n, 
				   nr_meta, 
				   cd_indicador_tabela 
			  FROM indicador_plugin.financeiro_exec_fin
	...
	function carrega(&$result, $args=array())
	{
		$qr_sql = "
            SELECT cd_financeiro_exec_fin,
                   ...
                   observacao
		      FROM indicador_plugin.financeiro_exec_fin 
			 WHERE cd_financeiro_exec_fin = ".intval($args['cd_financeiro_exec_fin']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.financeiro_exec_fin 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_valor_2, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.financeiro_exec_fin
