<?php
class beneficio_inc_ffp_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_beneficio_inc_ffp={cd_beneficio_inc_ffp} ";
			esc( "{cd_beneficio_inc_ffp}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_beneficio_inc_ffp'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.beneficio_inc_ffp ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.beneficio_inc_ffp 
			SET 
			
			cd_beneficio_inc_ffp = {cd_beneficio_inc_ffp} 
		...
		esc("{cd_beneficio_inc_ffp}", $args["cd_beneficio_inc_ffp"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_beneficio_inc_ffp::varchar)='{cd_beneficio_inc_ffp}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_beneficio_inc_ffp}', $id, $sql, 'str'); 
