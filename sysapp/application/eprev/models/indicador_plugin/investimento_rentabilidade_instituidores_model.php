<?php
class Investimento_rentabilidade_instituidores_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_investimento_rentabilidade_instituidores,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.investimento_rentabilidade_instituidores i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.investimento_rentabilidade_instituidores i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
                   ...
                   nr_valor_9,
				   nr_valor_10
			  FROM indicador_plugin.investimento_rentabilidade_instituidores
	...
	{
		$qr_sql = "
            SELECT cd_investimento_rentabilidade_instituidores,
                   ...
                   observacao
		      FROM indicador_plugin.investimento_rentabilidade_instituidores 
			 WHERE cd_investimento_rentabilidade_instituidores = ".intval($args['cd_investimento_rentabilidade_instituidores']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.investimento_rentabilidade_instituidores 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_valor_2, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.investimento_rentabilidade_instituidores
