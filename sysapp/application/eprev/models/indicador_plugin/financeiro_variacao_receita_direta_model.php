<?php
class Financeiro_variacao_receita_direta_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT c1.cd_financeiro_variacao_receita_direta,
				   ...
				   c1.fl_media,
				   c1.ds_observacao
		      FROM indicador_plugin.financeiro_variacao_receita_direta c1
	...
	public function carrega_referencia($nr_ano_referencia)
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia_n, 
				   ...
				   cd_indicador_tabela,
				   (SELECT COUNT(*)
				      FROM indicador_plugin.financeiro_variacao_receita_direta
	...
	public function carrega($cd_financeiro_variacao_receita_direta)
	{
		$qr_sql = "
            SELECT cd_financeiro_variacao_receita_direta,
                   ...
                   1 AS qt_ano
		      FROM indicador_plugin.financeiro_variacao_receita_direta 
			 WHERE cd_financeiro_variacao_receita_direta = ".intval($cd_financeiro_variacao_receita_direta).";";
	...
	{
		if(intval($args['cd_financeiro_variacao_receita_direta']) == 0)
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.financeiro_variacao_receita_direta 
				     (
				        cd_indicador_tabela, 
						dt_referencia, 
					    fl_media,
			     ...
			     VALUES 
				      ( 
					    ...
					    ".(trim($args['nr_meta_valor']) != '' ? floatval($args['nr_meta_valor']) : "DEFAULT").",
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.financeiro_variacao_receita_direta
				   SET dt_referencia          = ".(trim($args['dt_referencia']) != '' ? "TO_DATE('".trim($args['dt_referencia'])."', 'DD/MM/YYYY')" : "DEFAULT").",
				       nr_meta_valor      = ".(trim($args['nr_meta_valor']) != '' ? floatval($args['nr_meta_valor']) : "DEFAULT").",
			     ...
			     WHERE cd_financeiro_variacao_receita_direta = ".intval($args['cd_financeiro_variacao_receita_direta']).";";
		}

		$this->db->query($qr_sql);
	}
	
	public function excluir($cd_financeiro_variacao_receita_direta, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_plugin.financeiro_variacao_receita_direta
