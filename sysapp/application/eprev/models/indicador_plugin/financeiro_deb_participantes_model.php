<?php
class financeiro_deb_participantes_model extends Model
{
	...
	function listar( &$result, $args=array() )
	{
		...
		cd_financeiro_deb_participantes
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_financeiro_deb_participantes={cd_financeiro_deb_participantes} ";
			esc( "{cd_financeiro_deb_participantes}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_financeiro_deb_participantes'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.financeiro_deb_participantes ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.financeiro_deb_participantes 
			SET 
			
			cd_financeiro_deb_participantes = {cd_financeiro_deb_participantes} 
		...
		esc("{cd_financeiro_deb_participantes}", $args["cd_financeiro_deb_participantes"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_financeiro_deb_participantes::varchar)='{cd_financeiro_deb_participantes}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_financeiro_deb_participantes}', $id, $sql, 'str'); 
