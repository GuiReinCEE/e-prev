<?php
class Administrativo_proposicao_acao_fora_prazo_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_administrativo_proposicao_acao_fora_prazo,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.administrativo_proposicao_acao_fora_prazo i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.administrativo_proposicao_acao_fora_prazo i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
				   ...
				   nr_valor_1,
                   nr_valor_2
			  FROM indicador_plugin.administrativo_proposicao_acao_fora_prazo
	...
	{
		$qr_sql = "
            SELECT cd_administrativo_proposicao_acao_fora_prazo,
                   ...
                   observacao
		      FROM indicador_plugin.administrativo_proposicao_acao_fora_prazo 
			 WHERE cd_administrativo_proposicao_acao_fora_prazo = ".intval($args['cd_administrativo_proposicao_acao_fora_prazo']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.administrativo_proposicao_acao_fora_prazo 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_valor_2, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.administrativo_proposicao_acao_fora_prazo
