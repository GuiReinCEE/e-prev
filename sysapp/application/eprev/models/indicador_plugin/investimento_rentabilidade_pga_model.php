<?php
class Investimento_rentabilidade_pga_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_investimento_rentabilidade_pga,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.investimento_rentabilidade_pga i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.investimento_rentabilidade_pga i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
                   ...
                   nr_valor_5,
                   nr_valor_6
			  FROM indicador_plugin.investimento_rentabilidade_pga
	...
	{
		$qr_sql = "
            SELECT cd_investimento_rentabilidade_pga,
                   ...
                   observacao
		      FROM indicador_plugin.investimento_rentabilidade_pga 
			 WHERE cd_investimento_rentabilidade_pga = ".intval($args['cd_investimento_rentabilidade_pga']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.investimento_rentabilidade_pga 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_valor_2, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.investimento_rentabilidade_pga
