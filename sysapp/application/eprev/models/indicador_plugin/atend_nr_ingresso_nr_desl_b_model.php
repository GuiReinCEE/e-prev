<?php
class Atend_nr_ingresso_nr_desl_b_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_atend_nr_ingresso_nr_desl_b,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.atend_nr_ingresso_nr_desl_b i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.atend_nr_ingresso_nr_desl_b i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia_n, 
				   nr_meta, 
				   cd_indicador_tabela 
			  FROM indicador_plugin.atend_nr_ingresso_nr_desl_b
	...
	{
		$qr_sql = "
            SELECT cd_atend_nr_ingresso_nr_desl_b,
                   ...
                   observacao
		      FROM indicador_plugin.atend_nr_ingresso_nr_desl_b 
			 WHERE cd_atend_nr_ingresso_nr_desl_b = ".intval($args['cd_atend_nr_ingresso_nr_desl_b']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.atend_nr_ingresso_nr_desl_b 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_valor_2, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.atend_nr_ingresso_nr_desl_b
