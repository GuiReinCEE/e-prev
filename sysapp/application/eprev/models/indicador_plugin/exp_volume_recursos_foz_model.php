<?php
class Exp_volume_recursos_foz_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_exp_volume_recursos_foz,
			  ...
			  FROM indicador_plugin.exp_volume_recursos_foz i
	...
	public function carrega_referencia($cd_indicador_tabela)
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS dt_referencia_n, 
				   nr_meta, 
				   cd_indicador_tabela 
			  FROM indicador_plugin.exp_volume_recursos_foz 
	...
	public function carrega($cd_exp_volume_recursos_foz)
	{
		$qr_sql = "
			SELECT cd_exp_volume_recursos_foz,
			  ...
			  FROM indicador_plugin.exp_volume_recursos_foz 
			 WHERE dt_exclusao IS NULL
			   AND cd_exp_volume_recursos_foz = ".intval($cd_exp_volume_recursos_foz)."
	...
	{
		$qr_sql = "
			INSERT INTO indicador_plugin.exp_volume_recursos_foz
			     (
                   dt_referencia, 
                   cd_indicador_tabela, 
                   fl_media, 
	...
	public function atualizar($cd_exp_volume_recursos_foz, $args)
	{
		$qr_sql = "
			UPDATE indicador_plugin.exp_volume_recursos_foz
			 ...
			 WHERE cd_exp_volume_recursos_foz = ".intval($cd_exp_volume_recursos_foz).";";

		$this->db->query($qr_sql);	
	}
	
	public function excluir($cd_exp_volume_recursos_foz, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_plugin.exp_volume_recursos_foz 
