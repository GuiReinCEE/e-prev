<?php
class Investimento_rentabilidade_plano_unico_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_investimento_rentabilidade_plano_unico,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.investimento_rentabilidade_plano_unico i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.investimento_rentabilidade_plano_unico i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
                   ...
                   nr_valor_9,
				   nr_valor_10
			  FROM indicador_plugin.investimento_rentabilidade_plano_unico
	...
	{
		$qr_sql = "
            SELECT cd_investimento_rentabilidade_plano_unico,
                   ...
                   observacao
		      FROM indicador_plugin.investimento_rentabilidade_plano_unico 
			 WHERE cd_investimento_rentabilidade_plano_unico = ".intval($args['cd_investimento_rentabilidade_plano_unico']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.investimento_rentabilidade_plano_unico 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_valor_2, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.investimento_rentabilidade_plano_unico
