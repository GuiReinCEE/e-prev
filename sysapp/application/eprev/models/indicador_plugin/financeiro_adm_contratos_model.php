<?php
class financeiro_adm_contratos_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_financeiro_adm_contratos={cd_financeiro_adm_contratos} ";
			esc( "{cd_financeiro_adm_contratos}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_financeiro_adm_contratos'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.financeiro_adm_contratos ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.financeiro_adm_contratos 
			SET 
			
			cd_financeiro_adm_contratos = {cd_financeiro_adm_contratos} 
		...
		esc("{cd_financeiro_adm_contratos}", $args["cd_financeiro_adm_contratos"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_financeiro_adm_contratos::varchar)='{cd_financeiro_adm_contratos}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_financeiro_adm_contratos}', $id, $sql, 'str'); 
