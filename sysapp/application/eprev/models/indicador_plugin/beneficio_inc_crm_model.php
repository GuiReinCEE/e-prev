<?php
class beneficio_inc_crm_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_beneficio_inc_crm={cd_beneficio_inc_crm} ";
			esc( "{cd_beneficio_inc_crm}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_beneficio_inc_crm'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.beneficio_inc_crm ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.beneficio_inc_crm 
			SET 
			
			cd_beneficio_inc_crm = {cd_beneficio_inc_crm} 
		...
		esc("{cd_beneficio_inc_crm}", $args["cd_beneficio_inc_crm"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_beneficio_inc_crm::varchar)='{cd_beneficio_inc_crm}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_beneficio_inc_crm}', $id, $sql, 'str'); 
