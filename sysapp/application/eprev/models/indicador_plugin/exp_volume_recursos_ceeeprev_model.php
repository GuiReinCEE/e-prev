<?php
class Exp_volume_recursos_ceeeprev_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_exp_volume_recursos_ceeeprev,
			  ...
			  FROM indicador_plugin.exp_volume_recursos_ceeeprev i
	...
	public function carrega_referencia($cd_indicador_tabela)
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS dt_referencia_n, 
				   nr_meta, 
				   cd_indicador_tabela 
			  FROM indicador_plugin.exp_volume_recursos_ceeeprev 
	...
	public function carrega($cd_exp_volume_recursos_ceeeprev)
	{
		$qr_sql = "
			SELECT cd_exp_volume_recursos_ceeeprev,
			  ...
			  FROM indicador_plugin.exp_volume_recursos_ceeeprev 
			 WHERE dt_exclusao IS NULL
			   AND cd_exp_volume_recursos_ceeeprev = ".intval($cd_exp_volume_recursos_ceeeprev)."
	...
	{
		$qr_sql = "
			INSERT INTO indicador_plugin.exp_volume_recursos_ceeeprev
			     (
                   dt_referencia, 
                   cd_indicador_tabela, 
                   fl_media, 
	...
	public function atualizar($cd_exp_volume_recursos_ceeeprev, $args)
	{
		$qr_sql = "
			UPDATE indicador_plugin.exp_volume_recursos_ceeeprev
			 ...
			 WHERE cd_exp_volume_recursos_ceeeprev = ".intval($cd_exp_volume_recursos_ceeeprev).";";

		$this->db->query($qr_sql);	
	}
	
	public function excluir($cd_exp_volume_recursos_ceeeprev, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_plugin.exp_volume_recursos_ceeeprev 
