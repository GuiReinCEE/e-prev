<?php
class atend_espera_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atend_espera={cd_atend_espera} ";
			esc( "{cd_atend_espera}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atend_espera'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.atend_espera ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.atend_espera 
			SET 
			
			cd_atend_espera = {cd_atend_espera} 
		...
		esc("{cd_atend_espera}", $args["cd_atend_espera"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atend_espera::varchar)='{cd_atend_espera}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atend_espera}', $id, $sql, 'str'); 
