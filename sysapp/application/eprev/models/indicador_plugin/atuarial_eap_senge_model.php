<?php
class atuarial_eap_senge_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atuarial_eap_senge={cd_atuarial_eap_senge} ";
			esc( "{cd_atuarial_eap_senge}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atuarial_eap_senge'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.atuarial_eap_senge ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.atuarial_eap_senge 
			SET 
			
			cd_atuarial_eap_senge = {cd_atuarial_eap_senge} 
		...
		esc("{cd_atuarial_eap_senge}", $args["cd_atuarial_eap_senge"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atuarial_eap_senge::varchar)='{cd_atuarial_eap_senge}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atuarial_eap_senge}', $id, $sql, 'str'); 
