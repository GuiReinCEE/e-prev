<?php
class Atend_email_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atend_email={cd_atend_email} ";
			esc( "{cd_atend_email}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atend_email'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.atend_email ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.atend_email 
			SET 
			
			cd_atend_email = {cd_atend_email} 
		...
		esc("{cd_atend_email}", $args["cd_atend_email"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atend_email::varchar)='{cd_atend_email}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atend_email}', $id, $sql, 'str'); 
