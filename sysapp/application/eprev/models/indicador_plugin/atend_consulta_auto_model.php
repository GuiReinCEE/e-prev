<?php
class Atend_consulta_auto_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atend_consulta_auto={cd_atend_consulta_auto} ";
			esc( "{cd_atend_consulta_auto}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atend_consulta_auto'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.atend_consulta_auto ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.atend_consulta_auto 
			SET 
			
			cd_atend_consulta_auto = {cd_atend_consulta_auto} 
		...
		esc("{cd_atend_consulta_auto}", $args["cd_atend_consulta_auto"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atend_consulta_auto::varchar)='{cd_atend_consulta_auto}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atend_consulta_auto}', $id, $sql, 'str'); 
