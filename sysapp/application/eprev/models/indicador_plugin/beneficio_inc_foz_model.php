<?php
class beneficio_inc_foz_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_beneficio_inc_foz={cd_beneficio_inc_foz} ";
			esc( "{cd_beneficio_inc_foz}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_beneficio_inc_foz'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.beneficio_inc_foz ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.beneficio_inc_foz 
			SET 
			
			cd_beneficio_inc_foz = {cd_beneficio_inc_foz} 
		...
		esc("{cd_beneficio_inc_foz}", $args["cd_beneficio_inc_foz"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_beneficio_inc_foz::varchar)='{cd_beneficio_inc_foz}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_beneficio_inc_foz}', $id, $sql, 'str'); 
