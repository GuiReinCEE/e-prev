<?php
class administrativo_rotatividade_model extends Model
{
	...
	function listar( &$result, $args=array() )
	{
		...
		cd_administrativo_rotatividade
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_administrativo_rotatividade={cd_administrativo_rotatividade} ";
			esc( "{cd_administrativo_rotatividade}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_administrativo_rotatividade'])==0)
		{
			$sql="
		...
		{
			$sql="
			UPDATE indicador_plugin.administrativo_rotatividade 
			SET 
			
			cd_administrativo_rotatividade = {cd_administrativo_rotatividade} 
		...
		esc("{cd_administrativo_rotatividade}", $args["cd_administrativo_rotatividade"], $sql, "int", FALSE);
		...
		esc("{nr_referencial}", $args["nr_referencial"], $sql, "float", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_administrativo_rotatividade::varchar)='{cd_administrativo_rotatividade}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_administrativo_rotatividade}', $id, $sql, 'str'); 
