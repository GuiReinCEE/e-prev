<?php
class Investimento_rentabilidade_nominal_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_investimento_rentabilidade_nominal,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.investimento_rentabilidade_nominal i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.investimento_rentabilidade_nominal i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
                   ...
                   nr_valor_7,
                   nr_valor_8
			  FROM indicador_plugin.investimento_rentabilidade_nominal
	...
	{
		$qr_sql = "
            SELECT cd_investimento_rentabilidade_nominal,
                   TO_CHAR(dt_referencia,'DD/MM/YYYY') AS dt_referencia,
                   cd_indicador_tabela,
                   fl_media,
                   nr_valor_1 AS nr_valor_1_label,
                   nr_valor_2 AS nr_valor_2_label,
                   nr_valor_1,
                   nr_valor_2,
                   nr_valor_3,
                   nr_valor_4,
                   ...
                   observacao
		      FROM indicador_plugin.investimento_rentabilidade_nominal 
			 WHERE cd_investimento_rentabilidade_nominal = ".intval($args['cd_investimento_rentabilidade_nominal']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.investimento_rentabilidade_nominal 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_valor_2, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.investimento_rentabilidade_nominal
