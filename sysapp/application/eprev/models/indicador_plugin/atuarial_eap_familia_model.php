<?php
class atuarial_eap_familia_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atuarial_eap_familia={cd_atuarial_eap_familia} ";
			esc( "{cd_atuarial_eap_familia}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atuarial_eap_familia'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.atuarial_eap_familia ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.atuarial_eap_familia 
			SET 
			
			cd_atuarial_eap_familia = {cd_atuarial_eap_familia} 
		...
		esc("{cd_atuarial_eap_familia}", $args["cd_atuarial_eap_familia"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atuarial_eap_familia::varchar)='{cd_atuarial_eap_familia}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atuarial_eap_familia}', $id, $sql, 'str'); 
