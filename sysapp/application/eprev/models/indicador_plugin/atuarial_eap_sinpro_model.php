<?php
class atuarial_eap_sinpro_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atuarial_eap_sinpro={cd_atuarial_eap_sinpro} ";
			esc( "{cd_atuarial_eap_sinpro}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atuarial_eap_sinpro'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.atuarial_eap_sinpro ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.atuarial_eap_sinpro 
			SET 
			
			cd_atuarial_eap_sinpro = {cd_atuarial_eap_sinpro} 
		...
		esc("{cd_atuarial_eap_sinpro}", $args["cd_atuarial_eap_sinpro"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atuarial_eap_sinpro::varchar)='{cd_atuarial_eap_sinpro}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atuarial_eap_sinpro}', $id, $sql, 'str'); 
