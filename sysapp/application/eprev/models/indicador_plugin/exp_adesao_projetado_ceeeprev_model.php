<?php
class exp_adesao_projetado_ceeeprev_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_exp_adesao_projetado_ceeeprev,
				   ...
				   i.nr_percentual_f,
				   i.nr_meta
		      FROM indicador_plugin.exp_adesao_projetado_ceeeprev i
	...
	public function carrega_referencia()
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia_n,
				   nr_valor_1, 
				   cd_indicador_tabela 
			  FROM indicador_plugin.exp_adesao_projetado_ceeeprev
	...
	public function carrega($cd_exp_adesao_projetado_ceeeprev)
	{
		$qr_sql = "
            SELECT cd_exp_adesao_projetado_ceeeprev,
                   ...
                   observacao
		      FROM indicador_plugin.exp_adesao_projetado_ceeeprev 
			 WHERE cd_exp_adesao_projetado_ceeeprev = ".intval($cd_exp_adesao_projetado_ceeeprev).";";
	...
	{
		if(intval($args['cd_exp_adesao_projetado_ceeeprev']) == 0)
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.exp_adesao_projetado_ceeeprev 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_meta, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.exp_adesao_projetado_ceeeprev
			     ...
			     WHERE cd_exp_adesao_projetado_ceeeprev = ".intval($args['cd_exp_adesao_projetado_ceeeprev']).";";
		}

		$this->db->query($qr_sql);
	}
	
	public function excluir($cd_exp_adesao_projetado_ceeeprev, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_plugin.exp_adesao_projetado_ceeeprev
