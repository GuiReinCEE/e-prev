<?php
class ri_num_patro_instituidores_model extends Model
{
	...
	function listar( &$result, $args=array() )
	{
		// mount query
		$qr_sql = "
					SELECT i.cd_ri_num_patro_instituidores,
						   ...
						   CASE WHEN (SELECT MAX(i1.dt_referencia)
						                FROM indicador_plugin.ri_num_patro_instituidores i1
					                   WHERE i1.dt_exclusao IS NULL) = i.dt_referencia
								THEN 'S'
								ELSE 'N'
						   END AS fl_editar
					  FROM indicador_plugin.ri_num_patro_instituidores i
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_ri_num_patro_instituidores={cd_ri_num_patro_instituidores} ";
			esc( "{cd_ri_num_patro_instituidores}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_ri_num_patro_instituidores'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.ri_num_patro_instituidores (
		...
		{
			$sql="
			UPDATE indicador_plugin.ri_num_patro_instituidores
			SET 
			
			cd_ri_num_patro_instituidores = {cd_ri_num_patro_instituidores}
		...
		esc("{cd_ri_num_patro_instituidores}", $args["cd_ri_num_patro_instituidores"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_ri_num_patro_instituidores::varchar)='{cd_ri_num_patro_instituidores}'
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_ri_num_patro_instituidores}', $id, $sql, 'str');
