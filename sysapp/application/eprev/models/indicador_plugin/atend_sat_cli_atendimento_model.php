<?php
class atend_sat_cli_atendimento_model extends Model
{
	...
	function listar( &$result, $args=array() )
	{
		// mount query
		$qr_sql = "
					SELECT i.cd_atend_sat_cli_atendimento,
						   ...
						   CASE WHEN (SELECT MAX(i1.dt_referencia)
						                FROM indicador_plugin.atend_sat_cli_atendimento i1
					                   WHERE i1.dt_exclusao IS NULL) = i.dt_referencia
								THEN 'S'
								ELSE 'N'
						   END AS fl_editar
					  FROM indicador_plugin.atend_sat_cli_atendimento i
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atend_sat_cli_atendimento={cd_atend_sat_cli_atendimento} ";
			esc( "{cd_atend_sat_cli_atendimento}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atend_sat_cli_atendimento'])==0)
		{
			$sql="
		...
		{
			$sql="
			UPDATE indicador_plugin.atend_sat_cli_atendimento
			SET 
			
			cd_atend_sat_cli_atendimento = {cd_atend_sat_cli_atendimento}
		...
		esc("{cd_atend_sat_cli_atendimento}", $args["cd_atend_sat_cli_atendimento"], $sql, "int", FALSE);
        ...
        esc("{nr_meta_1}", $args["nr_meta_1"], $sql, "float", FALSE);
        esc("{nr_meta_2}", $args["nr_meta_2"], $sql, "float", FALSE);
        esc("{nr_meta_3}", $args["nr_meta_3"], $sql, "float", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atend_sat_cli_atendimento::varchar)='{cd_atend_sat_cli_atendimento}'
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atend_sat_cli_atendimento}', $id, $sql, 'str');
