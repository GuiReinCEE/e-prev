<?php
class beneficio_inc_sindha_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_beneficio_inc_sindha={cd_beneficio_inc_sindha} ";
			esc( "{cd_beneficio_inc_sindha}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_beneficio_inc_sindha'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.beneficio_inc_sindha ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.beneficio_inc_sindha 
			SET 
			
			cd_beneficio_inc_sindha = {cd_beneficio_inc_sindha} 
		...
		esc("{cd_beneficio_inc_sindha}", $args["cd_beneficio_inc_sindha"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_beneficio_inc_sindha::varchar)='{cd_beneficio_inc_sindha}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_beneficio_inc_sindha}', $id, $sql, 'str'); 
