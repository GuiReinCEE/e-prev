<?php
class Exp_captacao_liquida_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_exp_captacao_liquida,
			  ...
			  FROM indicador_plugin.exp_captacao_liquida i
	...
	public function carrega_referencia($cd_indicador_tabela, $nr_ano_referencia)
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS dt_referencia_n, 
				   ...
				   cd_indicador_tabela ,
				   (SELECT COUNT(*)
				      FROM indicador_plugin.exp_captacao_liquida
	...
	public function carrega($cd_exp_captacao_liquida)
	{
		$qr_sql = "
			SELECT cd_exp_captacao_liquida,
			  ...
			  FROM indicador_plugin.exp_captacao_liquida 
			 WHERE dt_exclusao IS NULL
			   AND cd_exp_captacao_liquida = ".intval($cd_exp_captacao_liquida)."
	...
	{
		$qr_sql = "
			INSERT INTO indicador_plugin.exp_captacao_liquida
			     (
                   dt_referencia, 
                   cd_indicador_tabela, 
                   fl_media, 
            ...
            VALUES 
			     (
				   ...
				   ".(trim($args['nr_resgate']) != '' ? floatval($args['nr_resgate']) : "DEFAULT").",
				   ".(trim($args['nr_captacao']) != '' ? floatval($args['nr_captacao']) : "DEFAULT").",
				   ".(trim($args['nr_quantidade']) != '' ? intval($args['nr_quantidade']) : "DEFAULT").",
	...
	public function atualizar($cd_exp_captacao_liquida, $args)
	{
		$qr_sql = "
			UPDATE indicador_plugin.exp_captacao_liquida
			   SET dt_referencia        = ".(trim($args['dt_referencia']) != '' ? "TO_DATE('".trim($args['dt_referencia'])."', 'DD/MM/YYYY')" : "DEFAULT").",
			       ...
			       nr_resgate           = ".(trim($args['nr_resgate']) != '' ? floatval($args['nr_resgate']) : "DEFAULT").",
			       nr_captacao          = ".(trim($args['nr_captacao']) != '' ? floatval($args['nr_captacao']) : "DEFAULT").",
			       nr_quantidade        = ".(trim($args['nr_quantidade']) != '' ? intval($args['nr_quantidade']) : "DEFAULT").",
			 ...
			 WHERE cd_exp_captacao_liquida = ".intval($cd_exp_captacao_liquida).";";

		$this->db->query($qr_sql);	
	}
	
	public function excluir($cd_exp_captacao_liquida, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_plugin.exp_captacao_liquida 
