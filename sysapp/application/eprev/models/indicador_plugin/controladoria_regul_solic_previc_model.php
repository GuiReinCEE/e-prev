<?php
class Controladoria_regul_solic_previc_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_controladoria_regul_solic_previc,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.controladoria_regul_solic_previc i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.controladoria_regul_solic_previc i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
				   nr_meta_f, 
				   cd_indicador_tabela 
			  FROM indicador_plugin.controladoria_regul_solic_previc
	...
	{
		$qr_sql = "
            SELECT cd_controladoria_regul_solic_previc,
                   ...
                   observacao
		      FROM indicador_plugin.controladoria_regul_solic_previc 
			 WHERE cd_controladoria_regul_solic_previc = ".intval($args['cd_controladoria_regul_solic_previc']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.controladoria_regul_solic_previc 
				     (
						dt_referencia, 
						ds_evento,
					    nr_valor_1, 
			     ...
			     VALUES 
				      ( 
					    ...
					    ".(trim($args['nr_meta_f']) == "" ? "DEFAULT" : intval($args['nr_meta_f'])).",
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.controladoria_regul_solic_previc
				   SET dt_referencia        = ".(trim($args['dt_referencia']) == "" ? "DEFAULT" : "TO_DATE('".trim($args["dt_referencia"])."', 'DD/MM/YYYY')").",
	                   ...
	                   nr_meta_f            = ".(trim($args['nr_meta_f']) == "" ? "DEFAULT" : intval($args['nr_meta_f'])).",
