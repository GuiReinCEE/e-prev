<?php
class beneficio_pg_30dias_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_beneficio_pg_30dias={cd_beneficio_pg_30dias} ";
			esc( "{cd_beneficio_pg_30dias}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_beneficio_pg_30dias'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.beneficio_pg_30dias ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.beneficio_pg_30dias 
			SET 
			
			cd_beneficio_pg_30dias = {cd_beneficio_pg_30dias} 
		...
		esc("{cd_beneficio_pg_30dias}", $args["cd_beneficio_pg_30dias"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_beneficio_pg_30dias::varchar)='{cd_beneficio_pg_30dias}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_beneficio_pg_30dias}', $id, $sql, 'str'); 
