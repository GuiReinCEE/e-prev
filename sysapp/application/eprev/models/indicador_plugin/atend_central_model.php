<?php
class atend_central_model extends Model
{
		...
		$qr_sql = "
					SELECT i.cd_atend_central,
						   ...
						   CASE WHEN (SELECT MAX(i1.dt_referencia)
								...
								ELSE 'N'
							END AS fl_editar
					 FROM indicador_plugin.atend_central i
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atend_central={cd_atend_central} ";
			esc( "{cd_atend_central}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atend_central'])==0)
		{
			$sql="
		...
		{
			$sql="
			UPDATE indicador_plugin.atend_central 
			SET 
			
			cd_atend_central = {cd_atend_central} 
		...
		esc("{cd_atend_central}", $args["cd_atend_central"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atend_central::varchar)='{cd_atend_central}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atend_central}', $id, $sql, 'str'); 
