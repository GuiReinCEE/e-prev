<?php
class Atend_pessoal_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atend_pessoal={cd_atend_pessoal} ";
			esc( "{cd_atend_pessoal}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atend_pessoal'])==0)
		{
			$sql="
		...
		{
			$sql="
			UPDATE indicador_plugin.atend_pessoal 
			SET 
			
			cd_atend_pessoal = {cd_atend_pessoal} 
            ...
            , nr_atendimento_re = {nr_atendimento_re}
		...
		esc("{cd_atend_pessoal}", $args["cd_atend_pessoal"], $sql, "int", FALSE);
		...
		esc("{nr_atendimento_re}", $args["nr_atendimento_re"], $sql, "float", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atend_pessoal::varchar)='{cd_atend_pessoal}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atend_pessoal}', $id, $sql, 'str'); 
