<?php
class beneficio_inc_tche_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_beneficio_inc_tche={cd_beneficio_inc_tche} ";
			esc( "{cd_beneficio_inc_tche}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_beneficio_inc_tche'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.beneficio_inc_tche ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.beneficio_inc_tche 
			SET 
			
			cd_beneficio_inc_tche = {cd_beneficio_inc_tche} 
		...
		esc("{cd_beneficio_inc_tche}", $args["cd_beneficio_inc_tche"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_beneficio_inc_tche::varchar)='{cd_beneficio_inc_tche}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_beneficio_inc_tche}', $id, $sql, 'str'); 
