<?php
class ri_num_midia_espontanea_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_ri_num_midia_espontanea={cd_ri_num_midia_espontanea} ";
			esc( "{cd_ri_num_midia_espontanea}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_ri_num_midia_espontanea'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.ri_num_midia_espontanea (
		...
		{
			$sql="
			UPDATE indicador_plugin.ri_num_midia_espontanea
			SET 
			
			cd_ri_num_midia_espontanea = {cd_ri_num_midia_espontanea}
		...
		esc("{cd_ri_num_midia_espontanea}", $args["cd_ri_num_midia_espontanea"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_ri_num_midia_espontanea::varchar)='{cd_ri_num_midia_espontanea}'
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_ri_num_midia_espontanea}', $id, $sql, 'str');
