<?php
class ri_sat_participante_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_ri_sat_participante,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
				                FROM indicador_plugin.ri_sat_participante i1
			                   WHERE i1.dt_exclusao IS NULL) = i.dt_referencia
						THEN 'S'
						ELSE 'N'
				   END AS fl_editar
			  FROM indicador_plugin.ri_sat_participante i
	...
	public function carrega_referencia()
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '2 year'::interval, 'DD/MM/YYYY') AS dt_referencia, 
			       ...
			       nr_meta, 
				   cd_indicador_tabela
			  FROM indicador_plugin.ri_sat_participante 
	...
	public function carrega($cd_ri_sat_participante)
	{
		$qr_sql = "
            SELECT cd_ri_sat_participante,
                   ...
                   observacao
		      FROM indicador_plugin.ri_sat_participante 
			 WHERE cd_ri_sat_participante = ".intval($cd_ri_sat_participante).";";
	...
	{
		if(intval($args['cd_ri_sat_participante']) == 0)
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.ri_sat_participante 
				( 
					dt_referencia,  
					dt_inclusao, 
					cd_usuario_inclusao,  
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.ri_sat_participante 
				SET cd_ri_sat_participante = ".intval($args['cd_ri_sat_participante']).",
			  ...
			  WHERE cd_ri_sat_participante = ".intval($args['cd_ri_sat_participante']).";";
		}
			
		$this->db->query($qr_sql);
	}

	public function excluir($cd_ri_sat_participante, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_plugin.ri_sat_participante 
