<?php
class Financeiro_contas_com_saldos_inconsistentes_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_financeiro_contas_com_saldos_inconsistentes,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.financeiro_contas_com_saldos_inconsistentes i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.financeiro_contas_com_saldos_inconsistentes i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
				   ...
				   nr_valor_1,
                   nr_valor_2
			  FROM indicador_plugin.financeiro_contas_com_saldos_inconsistentes
	...
	{
		$qr_sql = "
            SELECT cd_financeiro_contas_com_saldos_inconsistentes,
                   ...
                   observacao
		      FROM indicador_plugin.financeiro_contas_com_saldos_inconsistentes 
			 WHERE cd_financeiro_contas_com_saldos_inconsistentes = ".intval($args['cd_financeiro_contas_com_saldos_inconsistentes']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.financeiro_contas_com_saldos_inconsistentes 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_valor_2, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.financeiro_contas_com_saldos_inconsistentes
