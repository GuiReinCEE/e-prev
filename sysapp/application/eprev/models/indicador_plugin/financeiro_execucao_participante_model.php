<?php
class Financeiro_execucao_participante_model extends Model
{
	...
	function listar( &$result, $args=array() )
	{
		...
		cd_financeiro_execucao_participante
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_financeiro_execucao_participante={cd_financeiro_execucao_participante} ";
			esc( "{cd_financeiro_execucao_participante}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_financeiro_execucao_participante'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.financeiro_execucao_participante ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.financeiro_execucao_participante 
			SET 
			
			cd_financeiro_execucao_participante = {cd_financeiro_execucao_participante} 
		...
		esc("{cd_financeiro_execucao_participante}", $args["cd_financeiro_execucao_participante"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_financeiro_execucao_participante::varchar)='{cd_financeiro_execucao_participante}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_financeiro_execucao_participante}', $id, $sql, 'str'); 
