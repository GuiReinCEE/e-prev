<?php
class Administrativo_nr_correspondencia_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_administrativo_nr_correspondencia,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.administrativo_nr_correspondencia i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.administrativo_nr_correspondencia i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
				   ...
				   cd_indicador_tabela,
				   nr_valor_1
			  FROM indicador_plugin.administrativo_nr_correspondencia
	...
	{
		$qr_sql = "
            SELECT cd_administrativo_nr_correspondencia,
                   ...
                   observacao
		      FROM indicador_plugin.administrativo_nr_correspondencia 
			 WHERE cd_administrativo_nr_correspondencia = ".intval($args['cd_administrativo_nr_correspondencia']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.administrativo_nr_correspondencia 
				     (
						dt_referencia, 
					    nr_valor_1, 
					    nr_meta, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.administrativo_nr_correspondencia
