<?php
class administrativo_inventario_model extends Model
{
	...
	function listar( &$result, $args=array() )
	{
		// mount query
		$sql = "
					SELECT i.cd_administrativo_inventario,
						   ...
						   CASE WHEN (SELECT MAX(i1.dt_referencia)
						                FROM indicador_plugin.administrativo_inventario i1
					                   WHERE i1.dt_exclusao IS NULL) = i.dt_referencia
								THEN 'S'
								ELSE 'N'
						   END AS fl_editar
					  FROM indicador_plugin.administrativo_inventario i
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_administrativo_inventario={cd_administrativo_inventario} ";
			esc( "{cd_administrativo_inventario}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_administrativo_inventario'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.administrativo_inventario ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.administrativo_inventario 
			SET 
			
			cd_administrativo_inventario = {cd_administrativo_inventario} 
		...
		esc("{cd_administrativo_inventario}", $args["cd_administrativo_inventario"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_administrativo_inventario::varchar)='{cd_administrativo_inventario}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_administrativo_inventario}', $id, $sql, 'str'); 
