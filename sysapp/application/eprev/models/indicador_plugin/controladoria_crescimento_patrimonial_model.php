<?php
class controladoria_crescimento_patrimonial_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT c1.cd_controladoria_crescimento_patrimonial,
				   ...
				   c1.dt_referencia,
				   c1.nr_patrimonio_ano,
				   c1.nr_patrimonio_ffp,
				   c1.nr_resultado,
				   c1.nr_meta,
				   c1.fl_media,
				   c1.ds_observacao
		      FROM indicador_plugin.controladoria_crescimento_patrimonial c1
	...
	public function carrega_referencia($nr_ano_referencia)
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia_n, 
				   ...
				   cd_indicador_tabela,
				   (SELECT COUNT(*)
				      FROM indicador_plugin.controladoria_crescimento_patrimonial
	...
	public function carrega($cd_controladoria_crescimento_patrimonial)
	{
		$qr_sql = "
            SELECT cd_controladoria_crescimento_patrimonial,
                   ...
                   1 AS qt_ano
		      FROM indicador_plugin.controladoria_crescimento_patrimonial 
			 WHERE cd_controladoria_crescimento_patrimonial = ".intval($cd_controladoria_crescimento_patrimonial).";";
	...
	{
		if(intval($args['cd_controladoria_crescimento_patrimonial']) == 0)
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.controladoria_crescimento_patrimonial 
				     (
				        cd_indicador_tabela, 
						dt_referencia, 
					    fl_media,
			     ...
			     VALUES 
				      ( 
					    ...
					    ".(trim($args['nr_patrimonio_ano']) != '' ? floatval($args['nr_patrimonio_ano']) : "DEFAULT").",
					    ".(trim($args['nr_patrimonio_ffp']) != '' ? floatval($args['nr_patrimonio_ffp']) : "DEFAULT").",
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.controladoria_crescimento_patrimonial
				   SET dt_referencia          = ".(trim($args['dt_referencia']) != '' ? "TO_DATE('".trim($args['dt_referencia'])."', 'DD/MM/YYYY')" : "DEFAULT").",
				       nr_patrimonio_ano      = ".(trim($args['nr_patrimonio_ano']) != '' ? floatval($args['nr_patrimonio_ano']) : "DEFAULT").",
				       nr_patrimonio_ffp = ".(trim($args['nr_patrimonio_ffp']) != '' ? floatval($args['nr_patrimonio_ffp']) : "DEFAULT").",
			     ...
			     WHERE cd_controladoria_crescimento_patrimonial = ".intval($args['cd_controladoria_crescimento_patrimonial']).";";
		}

		$this->db->query($qr_sql);
	}
	
	public function excluir($cd_controladoria_crescimento_patrimonial, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_plugin.controladoria_crescimento_patrimonial
