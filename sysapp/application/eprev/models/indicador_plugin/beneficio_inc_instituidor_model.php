<?php
class beneficio_inc_instituidor_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_beneficio_inc_instituidor={cd_beneficio_inc_instituidor} ";
			esc( "{cd_beneficio_inc_instituidor}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_beneficio_inc_instituidor'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.beneficio_inc_instituidor ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.beneficio_inc_instituidor 
			SET 
			
			cd_beneficio_inc_instituidor = {cd_beneficio_inc_instituidor} 
		...
		esc("{cd_beneficio_inc_instituidor}", $args["cd_beneficio_inc_instituidor"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_beneficio_inc_instituidor::varchar)='{cd_beneficio_inc_instituidor}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_beneficio_inc_instituidor}', $id, $sql, 'str'); 
