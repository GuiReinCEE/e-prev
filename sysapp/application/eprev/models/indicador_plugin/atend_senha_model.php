<?php
class Atend_senha_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atend_senha={cd_atend_senha} ";
			esc( "{cd_atend_senha}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atend_senha'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.atend_senha ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.atend_senha 
			SET 
			
			cd_atend_senha = {cd_atend_senha} 
		...
		esc("{cd_atend_senha}", $args["cd_atend_senha"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atend_senha::varchar)='{cd_atend_senha}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atend_senha}', $id, $sql, 'str'); 
