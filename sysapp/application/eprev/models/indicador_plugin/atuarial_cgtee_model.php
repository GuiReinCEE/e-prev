<?php
class Atuarial_cgtee_model extends Model
{
	...
	function listar($cd_indicador_tabela)
	{
		$qr_sql = "
			SELECT i.cd_atuarial_cgtee,
				   ...
				   i.nr_percentual_f,
				   i.nr_meta
			  FROM indicador_plugin.atuarial_cgtee i
		     WHERE i.dt_exclusao IS NULL
	           AND (fl_media ='S' OR i.cd_indicador_tabela = ".intval($cd_indicador_tabela).")
	...
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS dt_referencia_n,nr_meta, 
				   cd_indicador_tabela
	...
	public function carrega($cd_atuarial_cgtee)
	{
		$qr_sql = "
            SELECT cd_atuarial_cgtee,
                   ...
                   observacao
		      FROM indicador_plugin.atuarial_cgtee 
			 WHERE cd_atuarial_cgtee = ".intval($cd_atuarial_cgtee).";";
	...
	{
		if(intval($args['cd_atuarial_cgtee']) == 0)
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.atuarial_cgtee 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_valor_2, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.atuarial_cgtee
			     ...
			     WHERE cd_atuarial_cgtee = ".intval($args['cd_atuarial_cgtee']).";";
		}

		$this->db->query($qr_sql);
	}
	
	public function excluir($cd_atuarial_cgtee, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_plugin.atuarial_cgtee
