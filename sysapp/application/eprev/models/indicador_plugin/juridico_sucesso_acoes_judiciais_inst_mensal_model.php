<?php
class Juridico_sucesso_acoes_judiciais_inst_mensal_model extends Model
{
    public function listar($cd_indicador_tabela)
    {
        $qr_sql = "
            SELECT cd_juridico_sucesso_acoes_judiciais_inst_mensal,
				   ...
				   fl_meta_proc,
				   fl_direcao_proc
              FROM indicador_plugin.juridico_sucesso_acoes_judiciais_inst_mensal
    ...
    public function carrega($cd_juridico_sucesso_acoes_judiciais_inst_mensal)
    {
        $qr_sql = "
            SELECT cd_juridico_sucesso_acoes_judiciais_inst_mensal,
				   ...
				   nr_proc_max
              FROM indicador_plugin.juridico_sucesso_acoes_judiciais_inst_mensal
             WHERE cd_juridico_sucesso_acoes_judiciais_inst_mensal = ".intval($cd_juridico_sucesso_acoes_judiciais_inst_mensal).";";
    ...
    {
        $qr_sql = "
            INSERT INTO indicador_plugin.juridico_sucesso_acoes_judiciais_inst_mensal
            (
                cd_indicador_tabela, 
                dt_referencia, 
                ds_observacao,
    ...
    public function atualizar($cd_juridico_sucesso_acoes_judiciais_inst_mensal, $args = array())
    {
        $qr_sql = "
            UPDATE indicador_plugin.juridico_sucesso_acoes_judiciais_inst_mensal 
    ...
    public function excluir($cd_juridico_sucesso_acoes_judiciais_inst_mensal, $cd_usuario)
    {
        $qr_sql = "
            UPDATE indicador_plugin.juridico_sucesso_acoes_judiciais_inst_mensal 
    ...
    {
        $qr_sql = "
            SELECT COALESCE(SUM(x.nr_inicial), 0) AS nr_inicial,   
                   COALESCE(SUM(x.nr_improcede_1), 0)  AS nr_improcede_1,  
                   COALESCE(SUM(x.nr_parcial_1), 0)  AS nr_parcial_1,  
                   COALESCE(SUM(x.nr_procede_1), 0)  AS nr_procede_1,  
                   COALESCE(SUM(x.nr_improcede_2), 0)  AS nr_improcede_2,  
                   COALESCE(SUM(x.nr_parcial_2), 0)  AS nr_parcial_2,  
                   COALESCE(SUM(x.nr_procede_2), 0)  AS nr_procede_2,  
                   COALESCE(SUM(x.nr_improcede_3), 0)  AS nr_improcede_3,  
                   COALESCE(SUM(x.nr_parcial_3), 0)  AS nr_parcial_3,  
                   COALESCE(SUM(x.nr_procede_3), 0)   AS nr_procede_3
              FROM (
                    SELECT nr_inicial,   
                           nr_improcede_1,
                           nr_parcial_1,
                           ...
                           nr_parcial_3,
                           nr_procede_3
                      FROM indicador_plugin.juridico_sucesso_acoes_civel_fundacao_autora
                     WHERE dt_exclusao IS NULL
                       AND TO_CHAR(dt_referencia, 'MM')::INT   = ".intval($mes)."
                       AND TO_CHAR(dt_referencia, 'YYYY')::INT = ".intval($ano)."
                     ...
                     SELECT nr_inicial,                            
                            ...
                            nr_parcial_3,
                            nr_procede_3
                       FROM indicador_plugin.juridico_sucesso_acoes_civel_fundacao_re
                      WHERE dt_exclusao IS NULL
                        AND TO_CHAR(dt_referencia, 'MM')::INT   = ".intval($mes)."
                        AND TO_CHAR(dt_referencia, 'YYYY')::INT = ".intval($ano)."
                    ...
                    SELECT nr_inicial,   
                           ...
                           nr_parcial_3,
                           nr_procede_3
                      FROM indicador_plugin.juridico_sucesso_acoes_trab_fundacao_autora
                     WHERE dt_exclusao IS NULL
                       AND TO_CHAR(dt_referencia, 'MM')::INT   = ".intval($mes)."
                       AND TO_CHAR(dt_referencia, 'YYYY')::INT = ".intval($ano)."
                     ...
                     SELECT nr_inicial,                            
                            ...
                            nr_parcial_3,
                            nr_procede_3
                       FROM indicador_plugin.juridico_sucesso_acoes_trab_fundacao_re
                      WHERE dt_exclusao IS NULL
                        AND TO_CHAR(dt_referencia, 'MM')::INT   = ".intval($mes)."
                        AND TO_CHAR(dt_referencia, 'YYYY')::INT = ".intval($ano)."
              ) x;";
