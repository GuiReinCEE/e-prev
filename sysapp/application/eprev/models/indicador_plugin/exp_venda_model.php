<?php
class exp_venda_model extends Model
{
	...
	function listar( &$result, $args=array() )
	{
		$qr_sql = "
					SELECT i.cd_exp_venda,
					  ...
					  FROM indicador_plugin.exp_venda i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
					SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia_n, 
						   nr_meta, 
						   cd_indicador_tabela 
					  FROM indicador_plugin.exp_venda
	...
	function carrega(&$result, $args=array())
	{
		$qr_sql = "
					SELECT cd_exp_venda,
						   ...
						   observacao
					  FROM indicador_plugin.exp_venda 
					 WHERE cd_exp_venda = ".intval($args['cd_exp_venda'])."
			      ";
		$result = $this->db->query($qr_sql);
	}
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
						INSERT INTO indicador_plugin.exp_venda 
							 (
								dt_referencia, 
								nr_venda,
								nr_ingresso,
						 ...
						 VALUES 
							  ( 
								...
								".(trim($args['nr_venda']) == "" ? "DEFAULT" : floatval($args['nr_venda'])).",
								".(trim($args['nr_ingresso']) == "" ? "DEFAULT" : floatval($args['nr_ingresso'])).",
								".(trim($args['nr_desligamento']) == "" ? "DEFAULT" : floatval($args['nr_desligamento'])).",
		...
		{
			$qr_sql = "
						UPDATE indicador_plugin.exp_venda
						   SET dt_referencia        = ".(trim($args['dt_referencia']) == "" ? "DEFAULT" : "TO_DATE('".trim($args["dt_referencia"])."', 'DD/MM/YYYY')").",
							   nr_venda             = ".(trim($args['nr_venda']) == "" ? "DEFAULT" : floatval($args['nr_venda'])).",
							   nr_ingresso          = ".(trim($args['nr_ingresso']) == "" ? "DEFAULT" : floatval($args['nr_ingresso'])).",
							   nr_desligamento      = ".(trim($args['nr_desligamento']) == "" ? "DEFAULT" : floatval($args['nr_desligamento'])).",
