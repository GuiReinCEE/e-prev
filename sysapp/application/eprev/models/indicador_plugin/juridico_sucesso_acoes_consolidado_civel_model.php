<?php
class juridico_sucesso_acoes_consolidado_civel_model extends Model
{
	...
	{
		$qr_sql = "
					SELECT i.cd_juridico_sucesso_acoes_consolidado_civel,
						   ...
						   i.nr_total_geral,
						   i.nr_meta
					  FROM indicador_plugin.juridico_sucesso_acoes_consolidado_civel i
		...
		$qr_sql = "
					SELECT x.value, 
					 ...
					 WHERE x.value NOT IN (SELECT i.cd_etapa
					                         FROM indicador_plugin.juridico_sucesso_acoes_consolidado_civel i
								            WHERE i.dt_exclusao IS NULL
											  AND i.cd_juridico_sucesso_acoes_consolidado_civel <> ".intval($args['cd_juridico_sucesso_acoes_consolidado_civel'])."
	...
	{
		$qr_sql = "
					SELECT cd_juridico_sucesso_acoes_consolidado_civel,
						   ...
						   nr_total_geral,
						   nr_meta
					  FROM indicador_plugin.juridico_sucesso_acoes_consolidado_civel 
					 WHERE cd_juridico_sucesso_acoes_consolidado_civel = ".intval($args['cd_juridico_sucesso_acoes_consolidado_civel']).";
			      ";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
						INSERT INTO indicador_plugin.juridico_sucesso_acoes_consolidado_civel 
							 (
								dt_referencia, 
						        cd_etapa,
						        nr_inicial,
		...
		{
			$qr_sql = "
						UPDATE indicador_plugin.juridico_sucesso_acoes_consolidado_civel
	...
	public function get_valores($nr_ano, $cd_etapa)
	{
		$qr_sql = "
			...
			SELECT nr_inicial,
			       ...
			       nr_procede,
			       nr_total
			  FROM indicador_plugin.juridico_sucesso_acoes_cenco_civel
			...
			SELECT nr_inicial,
			       ...
			       nr_procede,
			       nr_total
			  FROM indicador_plugin.juridico_sucesso_acoes_juchem_civel
			 WHERE TO_CHAR(dt_referencia, 'YYYY')::integer = ".intval($nr_ano)."
			   AND cd_etapa                                = ".intval($cd_etapa)."
			   AND dt_exclusao                             IS NULL
