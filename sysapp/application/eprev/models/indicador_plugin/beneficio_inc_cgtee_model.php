<?php
class beneficio_inc_cgtee_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_beneficio_inc_cgtee={cd_beneficio_inc_cgtee} ";
			esc( "{cd_beneficio_inc_cgtee}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_beneficio_inc_cgtee'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.beneficio_inc_cgtee ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.beneficio_inc_cgtee 
			SET 
			
			cd_beneficio_inc_cgtee = {cd_beneficio_inc_cgtee} 
		...
		esc("{cd_beneficio_inc_cgtee}", $args["cd_beneficio_inc_cgtee"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_beneficio_inc_cgtee::varchar)='{cd_beneficio_inc_cgtee}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_beneficio_inc_cgtee}', $id, $sql, 'str'); 
