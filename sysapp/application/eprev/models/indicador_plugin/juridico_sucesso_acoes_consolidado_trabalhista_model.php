<?php
class Juridico_sucesso_acoes_consolidado_trabalhista_model extends Model
{
	...
	{
		$qr_sql = "
					SELECT i.cd_juridico_sucesso_acoes_consolidado_trabalhista,
						   ...
						   i.nr_total_geral,
						   i.nr_meta
					  FROM indicador_plugin.juridico_sucesso_acoes_consolidado_trabalhista i
		...
		$qr_sql = "
					SELECT x.value, 
					 ...
					 WHERE x.value NOT IN (SELECT i.cd_etapa
					                         FROM indicador_plugin.juridico_sucesso_acoes_consolidado_trabalhista i
								            WHERE i.dt_exclusao IS NULL
											  AND i.cd_juridico_sucesso_acoes_consolidado_trabalhista <> ".intval($args['cd_juridico_sucesso_acoes_consolidado_trabalhista'])."
	...
	{
		$qr_sql = "
					SELECT cd_juridico_sucesso_acoes_consolidado_trabalhista,
						   ...
						   nr_total_geral,
						   nr_meta
					  FROM indicador_plugin.juridico_sucesso_acoes_consolidado_trabalhista 
					 WHERE cd_juridico_sucesso_acoes_consolidado_trabalhista = ".intval($args['cd_juridico_sucesso_acoes_consolidado_trabalhista']).";
			      ";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
						INSERT INTO indicador_plugin.juridico_sucesso_acoes_consolidado_trabalhista 
							 (
								dt_referencia, 
						        cd_etapa,
						        nr_inicial,
		...
		{
			$qr_sql = "
						UPDATE indicador_plugin.juridico_sucesso_acoes_consolidado_trabalhista
	...
	public function get_valores($nr_ano, $cd_etapa)
	{
		$qr_sql = "
			SELECT SUM(nr_inicial) AS nr_inicial,
			       SUM(nr_improcede) AS nr_improcede,
			       SUM(nr_parcial) AS nr_parcial,
			       SUM(nr_procede) AS nr_procede,
			       SUM(nr_total) AS nr_total
			  FROM (
			SELECT nr_inicial,
			       nr_improcede,
			       nr_parcial,
			       nr_procede,
			       nr_total
			  FROM indicador_plugin.juridico_sucesso_acoes_cenco
			 WHERE TO_CHAR(dt_referencia, 'YYYY')::integer = ".intval($nr_ano)."
			   AND cd_etapa                                = ".intval($cd_etapa)."
			...
			SELECT nr_inicial,
			       ...
			       nr_procede,
			       nr_total
			  FROM indicador_plugin.juridico_sucesso_acoes_juchem
			 WHERE TO_CHAR(dt_referencia, 'YYYY')::integer = ".intval($nr_ano)."
			   AND cd_etapa                                = ".intval($cd_etapa)."
			   AND dt_exclusao                             IS NULL
			...
			SELECT nr_inicial,
			       ...
			       nr_procede,
			       nr_total
			  FROM indicador_plugin.juridico_sucesso_acoes_ribeiro
			 WHERE TO_CHAR(dt_referencia, 'YYYY')::integer = ".intval($nr_ano)."
			   AND cd_etapa                                = ".intval($cd_etapa)."
			   AND dt_exclusao                             IS NULL
