<?php
class Atend_teleatend_model extends Model
{
		...
		$qr_sql = "
					SELECT i.cd_atend_teleatend,
						   ...
						   CASE WHEN (SELECT MAX(i1.dt_referencia)
								...
								ELSE 'N'
							END AS fl_editar
		              FROM indicador_plugin.atend_teleatend i
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atend_teleatend={cd_atend_teleatend} ";
			esc( "{cd_atend_teleatend}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atend_teleatend'])==0)
		{
			$sql="
		...
		{
			$sql="
			UPDATE indicador_plugin.atend_teleatend 
			SET 
			
			cd_atend_teleatend = {cd_atend_teleatend} 
		...
		esc("{cd_atend_teleatend}", $args["cd_atend_teleatend"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atend_teleatend::varchar)='{cd_atend_teleatend}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atend_teleatend}', $id, $sql, 'str'); 
