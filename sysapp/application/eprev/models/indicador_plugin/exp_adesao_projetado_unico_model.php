<?php
class exp_adesao_projetado_unico_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_exp_adesao_projetado_unico,
				   ...
				   i.nr_percentual_f,
				   i.nr_meta
		      FROM indicador_plugin.exp_adesao_projetado_unico i
	...
	public function carrega_referencia()
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia_n,
				   nr_valor_1, 
				   cd_indicador_tabela 
			  FROM indicador_plugin.exp_adesao_projetado_unico
	...
	public function carrega($cd_exp_adesao_projetado_unico)
	{
		$qr_sql = "
            SELECT cd_exp_adesao_projetado_unico,
                   ...
                   observacao
		      FROM indicador_plugin.exp_adesao_projetado_unico 
			 WHERE cd_exp_adesao_projetado_unico = ".intval($cd_exp_adesao_projetado_unico).";";
	...
	{
		if(intval($args['cd_exp_adesao_projetado_unico']) == 0)
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.exp_adesao_projetado_unico 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_meta, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.exp_adesao_projetado_unico
			     ...
			     WHERE cd_exp_adesao_projetado_unico = ".intval($args['cd_exp_adesao_projetado_unico']).";";
		}

		$this->db->query($qr_sql);
	}
	
	public function excluir($cd_exp_adesao_projetado_unico, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_plugin.exp_adesao_projetado_unico
