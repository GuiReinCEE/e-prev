<?php
class beneficio_inc_sintae_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_beneficio_inc_sintae={cd_beneficio_inc_sintae} ";
			esc( "{cd_beneficio_inc_sintae}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_beneficio_inc_sintae'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.beneficio_inc_sintae ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.beneficio_inc_sintae 
			SET 
			
			cd_beneficio_inc_sintae = {cd_beneficio_inc_sintae} 
		...
		esc("{cd_beneficio_inc_sintae}", $args["cd_beneficio_inc_sintae"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_beneficio_inc_sintae::varchar)='{cd_beneficio_inc_sintae}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_beneficio_inc_sintae}', $id, $sql, 'str'); 
