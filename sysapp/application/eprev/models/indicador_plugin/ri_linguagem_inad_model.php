<?php
class ri_linguagem_inad_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_ri_linguagem_inad={cd_ri_linguagem_inad} ";
			esc( "{cd_ri_linguagem_inad}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_ri_linguagem_inad'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.ri_linguagem_inad ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.ri_linguagem_inad 
			SET 
			
			cd_ri_linguagem_inad = {cd_ri_linguagem_inad} 
		...
		esc("{cd_ri_linguagem_inad}", $args["cd_ri_linguagem_inad"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_ri_linguagem_inad::varchar)='{cd_ri_linguagem_inad}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_ri_linguagem_inad}', $id, $sql, 'str'); 
