<?php
class beneficio_inc_ceeeprev_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_beneficio_inc_ceeeprev={cd_beneficio_inc_ceeeprev} ";
			esc( "{cd_beneficio_inc_ceeeprev}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_beneficio_inc_ceeeprev'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.beneficio_inc_ceeeprev ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.beneficio_inc_ceeeprev 
			SET 
			
			cd_beneficio_inc_ceeeprev = {cd_beneficio_inc_ceeeprev} 
		...
		esc("{cd_beneficio_inc_ceeeprev}", $args["cd_beneficio_inc_ceeeprev"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_beneficio_inc_ceeeprev::varchar)='{cd_beneficio_inc_ceeeprev}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_beneficio_inc_ceeeprev}', $id, $sql, 'str'); 
