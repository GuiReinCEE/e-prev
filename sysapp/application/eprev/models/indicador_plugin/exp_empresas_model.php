<?php
class Exp_empresas_model extends Model
{
	...
	function listar( &$result, $args=array() )
	{
		$qr_sql = "
					SELECT i.cd_exp_empresas,
					  ...
					  FROM indicador_plugin.exp_empresas i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
					SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia_n, 
						   ...
						   cd_indicador_tabela,
				   (SELECT COUNT(*)
				      FROM indicador_plugin.exp_empresas
	...
	function carrega(&$result, $args=array())
	{
		$qr_sql = "
					SELECT cd_exp_empresas,
						   ...
						   observacao
					  FROM indicador_plugin.exp_empresas 
					 WHERE cd_exp_empresas = ".intval($args['cd_exp_empresas'])."
			      ";
		$result = $this->db->query($qr_sql);
	}
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
						INSERT INTO indicador_plugin.exp_empresas 
							 (
								dt_referencia, 
								nr_valor_1,
								nr_valor_2,
		...
		{
			$qr_sql = "
						UPDATE indicador_plugin.exp_empresas
