<?php
class controladoria_riscos_mitigados_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_controladoria_riscos_mitigados,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.controladoria_riscos_mitigados i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.controladoria_riscos_mitigados i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia_n, 
				   ...
				   cd_indicador_tabela,
				   (SELECT COUNT(*)
				      FROM indicador_plugin.controladoria_riscos_mitigados
	...
	{
		$qr_sql = "
            SELECT cd_controladoria_riscos_mitigados,
                   ...
                   observacao
		      FROM indicador_plugin.controladoria_riscos_mitigados 
			 WHERE cd_controladoria_riscos_mitigados = ".intval($args['cd_controladoria_riscos_mitigados']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.controladoria_riscos_mitigados 
				     (
						dt_referencia, 
					    nr_valor_1, 
					    nr_meta, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.controladoria_riscos_mitigados
