<?php
class administrativo_treina_orca_model extends Model
{
	...
	function listar( &$result, $args=array() )
	{
		// mount query
		$sql = "
                    SELECT i.cd_administrativo_treina_orca,
						   ...
						   CASE WHEN (SELECT MAX(i1.dt_referencia)
						                FROM indicador_plugin.administrativo_treina_orca i1
					                   WHERE i1.dt_exclusao IS NULL) = i.dt_referencia
								THEN 'S'
								ELSE 'N'
						   END AS fl_editar
					  FROM indicador_plugin.administrativo_treina_orca i
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_administrativo_treina_orca={cd_administrativo_treina_orca} ";
			esc( "{cd_administrativo_treina_orca}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_administrativo_treina_orca'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.administrativo_treina_orca ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.administrativo_treina_orca 
			SET 
			
			cd_administrativo_treina_orca = {cd_administrativo_treina_orca} 
		...
		esc("{cd_administrativo_treina_orca}", $args["cd_administrativo_treina_orca"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_administrativo_treina_orca::varchar)='{cd_administrativo_treina_orca}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_administrativo_treina_orca}', $id, $sql, 'str'); 
