<?php
class financeiro_seguro_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_financeiro_seguro={cd_financeiro_seguro} ";
			esc( "{cd_financeiro_seguro}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_financeiro_seguro'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.financeiro_seguro ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.financeiro_seguro 
			SET 
			
			cd_financeiro_seguro = {cd_financeiro_seguro} 
		...
		esc("{cd_financeiro_seguro}", $args["cd_financeiro_seguro"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_financeiro_seguro::varchar)='{cd_financeiro_seguro}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_financeiro_seguro}', $id, $sql, 'str'); 
