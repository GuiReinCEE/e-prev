<?php
class Ri_midia_fav_des_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_ri_midia_fav_des={cd_ri_midia_fav_des} ";
			esc( "{cd_ri_midia_fav_des}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_ri_midia_fav_des'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.ri_midia_fav_des ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.ri_midia_fav_des 
			SET 
			
			cd_ri_midia_fav_des = {cd_ri_midia_fav_des} 
		...
		esc("{cd_ri_midia_fav_des}", $args["cd_ri_midia_fav_des"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_ri_midia_fav_des::varchar)='{cd_ri_midia_fav_des}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_ri_midia_fav_des}', $id, $sql, 'str'); 
