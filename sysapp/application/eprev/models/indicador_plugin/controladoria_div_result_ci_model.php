<?php
class Controladoria_div_result_ci_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_controladoria_div_result_ci,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.controladoria_div_result_ci i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.controladoria_div_result_ci i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
				   nr_meta, 
				   cd_indicador_tabela 
			  FROM indicador_plugin.controladoria_div_result_ci
	...
	{
		$qr_sql = "
            SELECT cd_controladoria_div_result_ci,
                   ...
                   observacao
		      FROM indicador_plugin.controladoria_div_result_ci 
			 WHERE cd_controladoria_div_result_ci = ".intval($args['cd_controladoria_div_result_ci']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.controladoria_div_result_ci 
				     (
						dt_referencia, 
					    nr_valor_1, 
					    nr_meta, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.controladoria_div_result_ci
