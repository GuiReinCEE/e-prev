<?php
class Atend_ativo_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atend_ativo={cd_atend_ativo} ";
			esc( "{cd_atend_ativo}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atend_ativo'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.atend_ativo ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.atend_ativo 
			SET 
			
			cd_atend_ativo = {cd_atend_ativo} 
		...
		esc("{cd_atend_ativo}", $args["cd_atend_ativo"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atend_ativo::varchar)='{cd_atend_ativo}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atend_ativo}', $id, $sql, 'str'); 
