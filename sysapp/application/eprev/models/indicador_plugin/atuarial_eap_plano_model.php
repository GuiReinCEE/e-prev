<?php
class atuarial_eap_plano_model extends Model
{
	...
	function listar( &$result, $args=array() )
	{
		// mount query
		$qr_sql = "
					SELECT i.cd_atuarial_eap_plano,
						   ...
						   CASE WHEN (SELECT MAX(i1.dt_referencia)
						                FROM indicador_plugin.atuarial_eap_plano i1
					                   WHERE i1.dt_exclusao IS NULL) = i.dt_referencia
								THEN 'S'
								ELSE 'N'
						   END AS fl_editar
					  FROM indicador_plugin.atuarial_eap_plano i
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atuarial_eap_plano={cd_atuarial_eap_plano} ";
			esc( "{cd_atuarial_eap_plano}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atuarial_eap_plano'])==0)
		{
			$sql="
		...
		{
			$sql="
			UPDATE indicador_plugin.atuarial_eap_plano 
			SET 
			
			cd_atuarial_eap_plano = {cd_atuarial_eap_plano} 
		...
		esc("{cd_atuarial_eap_plano}", $args["cd_atuarial_eap_plano"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atuarial_eap_plano::varchar)='{cd_atuarial_eap_plano}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atuarial_eap_plano}', $id, $sql, 'str'); 
