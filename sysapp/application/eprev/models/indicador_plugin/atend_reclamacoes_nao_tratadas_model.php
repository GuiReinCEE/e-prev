<?php
class atend_reclamacoes_nao_tratadas_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT cd_atend_reclamacoes_nao_tratadas,
				   cd_indicador_tabela,
				   ...
				   fl_media,
				   nr_reclamacao,
				   nr_reclamacao_nao_tratadas,
				   nr_percent_nao_tratadas,
				   nr_meta,
				   observacao
			  FROM indicador_plugin.atend_reclamacoes_nao_tratadas 
	...
	public function carrega($cd_atend_reclamacoes_nao_tratadas)
	{
		$qr_sql = "
			SELECT cd_atend_reclamacoes_nao_tratadas,
				   cd_indicador_tabela,
			  ...
			  FROM indicador_plugin.atend_reclamacoes_nao_tratadas 
			 WHERE dt_exclusao IS NULL
			   AND cd_atend_reclamacoes_nao_tratadas = ".intval($cd_atend_reclamacoes_nao_tratadas)."
			 ORDER BY dt_referencia ASC;";
	...
	{
		$qr_sql = "
			INSERT INTO indicador_plugin.atend_reclamacoes_nao_tratadas
			     (
                   dt_referencia, 
                   cd_indicador_tabela, 
                   fl_media, 
            ...
            VALUES 
			     (
				   ...
				   ".(trim($args['nr_reclamacao_nao_tratadas']) != '' ? intval($args['nr_reclamacao_nao_tratadas']) : 'DEFAULT').",
				   ".(trim($args['nr_percent_nao_tratadas']) != '' ? floatval($args['nr_percent_nao_tratadas']) : 'DEFAULT').",
	...
	public function atualizar($cd_atend_reclamacoes_nao_tratadas, $args)
	{
		$qr_sql = "
			UPDATE indicador_plugin.atend_reclamacoes_nao_tratadas
			   SET dt_referencia         = ".(trim($args['dt_referencia']) != '' ? "TO_DATE('".trim($args['dt_referencia'])."', 'DD/MM/YYYY')" : 'DEFAULT').",
				   ...
				   nr_reclamacao_nao_tratadas = ".(trim($args['nr_reclamacao_nao_tratadas']) != '' ? intval($args['nr_reclamacao_nao_tratadas']) : 'DEFAULT').",
				   nr_percent_nao_tratadas = ".(trim($args['nr_percent_nao_tratadas']) != '' ? floatval($args['nr_percent_nao_tratadas']) : 'DEFAULT').",
			 ...
			 WHERE cd_atend_reclamacoes_nao_tratadas = ".intval($cd_atend_reclamacoes_nao_tratadas).";";

		$this->db->query($qr_sql);	
	}

	public function excluir($cd_atend_reclamacoes_nao_tratadas, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_plugin.atend_reclamacoes_nao_tratadas 
		...
		$qr_sql = "
            ...
            VALUES 
			     (
				   ...
				   ".(trim($args['nr_reclamacao_nao_tratadas_total']) != '' ? intval($args['nr_reclamacao_nao_tratadas_total']) : 'DEFAULT').",
				   ".(trim($args['nr_percent_nao_tratadas_total']) != '' ? floatval($args['nr_percent_nao_tratadas_total']) : 'DEFAULT').",
				   ".(trim($args['nr_meta']) != '' ? floatval($args['nr_meta']) : 'DEFAULT').",
				   ".intval($args['cd_usuario']).",
				   ".intval($args['cd_usuario'])."
				 );";
