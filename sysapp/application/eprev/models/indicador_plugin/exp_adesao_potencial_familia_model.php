<?php
class Exp_adesao_potencial_familia_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_exp_adesao_potencial_familia,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.exp_adesao_potencial_familia i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.exp_adesao_potencial_familia i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
                   ...
                   nr_valor_2,
                   nr_valor_3
			  FROM indicador_plugin.exp_adesao_potencial_familia
	...
	{
		$qr_sql = "
            SELECT cd_exp_adesao_potencial_familia,
                   ...
                   observacao
		      FROM indicador_plugin.exp_adesao_potencial_familia 
			 WHERE cd_exp_adesao_potencial_familia = ".intval($args['cd_exp_adesao_potencial_familia']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.exp_adesao_potencial_familia 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_valor_2, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.exp_adesao_potencial_familia
