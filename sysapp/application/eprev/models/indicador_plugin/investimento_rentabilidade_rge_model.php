<?php
class Investimento_rentabilidade_rge_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_investimento_rentabilidade_rge,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.investimento_rentabilidade_rge i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.investimento_rentabilidade_rge i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
                   ...
                   nr_valor_5,
                   nr_valor_6
			  FROM indicador_plugin.investimento_rentabilidade_rge
	...
	{
		$qr_sql = "
            SELECT cd_investimento_rentabilidade_rge,
                   ...
                   observacao
		      FROM indicador_plugin.investimento_rentabilidade_rge 
			 WHERE cd_investimento_rentabilidade_rge = ".intval($args['cd_investimento_rentabilidade_rge']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.investimento_rentabilidade_rge 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_valor_2, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.investimento_rentabilidade_rge
