<?php
class ri_venda_previd_patroc_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_ri_venda_previd_patroc={cd_ri_venda_previd_patroc} ";
			esc( "{cd_ri_venda_previd_patroc}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_ri_venda_previd_patroc'])==0)
		{
			$sql="
		...
		{
			$sql="
			UPDATE indicador_plugin.ri_venda_previd_patroc
			SET 
			
			cd_ri_venda_previd_patroc = {cd_ri_venda_previd_patroc}
		...
		esc("{cd_ri_venda_previd_patroc}", $args["cd_ri_venda_previd_patroc"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_ri_venda_previd_patroc::varchar)='{cd_ri_venda_previd_patroc}'
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_ri_venda_previd_patroc}', $id, $sql, 'str');
