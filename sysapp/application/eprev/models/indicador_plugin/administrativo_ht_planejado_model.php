<?php
class administrativo_ht_planejado_model extends Model
{
	...
	function listar( &$result, $args=array() )
	{
		...
		cd_administrativo_ht_planejado
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_administrativo_ht_planejado={cd_administrativo_ht_planejado} ";
			esc( "{cd_administrativo_ht_planejado}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_administrativo_ht_planejado'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.administrativo_ht_planejado (
		...
		{
			$sql="
			UPDATE indicador_plugin.administrativo_ht_planejado
			SET 
			
			cd_administrativo_ht_planejado = {cd_administrativo_ht_planejado}
		...
		esc("{cd_administrativo_ht_planejado}", $args["cd_administrativo_ht_planejado"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_administrativo_ht_planejado::varchar)='{cd_administrativo_ht_planejado}'
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_administrativo_ht_planejado}', $id, $sql, 'str');
