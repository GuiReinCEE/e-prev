<?php
class beneficio_inc_rge_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_beneficio_inc_rge={cd_beneficio_inc_rge} ";
			esc( "{cd_beneficio_inc_rge}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_beneficio_inc_rge'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.beneficio_inc_rge ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.beneficio_inc_rge 
			SET 
			
			cd_beneficio_inc_rge = {cd_beneficio_inc_rge} 
		...
		esc("{cd_beneficio_inc_rge}", $args["cd_beneficio_inc_rge"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_beneficio_inc_rge::varchar)='{cd_beneficio_inc_rge}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_beneficio_inc_rge}', $id, $sql, 'str'); 
