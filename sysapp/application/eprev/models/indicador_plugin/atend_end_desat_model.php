<?php
class Atend_end_desat_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_atend_end_desat={cd_atend_end_desat} ";
			esc( "{cd_atend_end_desat}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_atend_end_desat'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.atend_end_desat ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.atend_end_desat 
			SET 
			
			cd_atend_end_desat = {cd_atend_end_desat} 
		...
		esc("{cd_atend_end_desat}", $args["cd_atend_end_desat"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_atend_end_desat::varchar)='{cd_atend_end_desat}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_atend_end_desat}', $id, $sql, 'str'); 
