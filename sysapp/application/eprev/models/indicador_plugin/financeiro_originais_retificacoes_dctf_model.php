<?php
class Financeiro_originais_retificacoes_dctf_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT i.cd_financeiro_originais_retificacoes_dctf,
				   ...
				   CASE WHEN (SELECT MAX(i1.dt_referencia)
                                FROM indicador_plugin.financeiro_originais_retificacoes_dctf i1
					    ...
					    ELSE 'N'
				   END AS fl_editar
		      FROM indicador_plugin.financeiro_originais_retificacoes_dctf i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia, 
				   ...
				   nr_valor_1,
                   nr_valor_2
			  FROM indicador_plugin.financeiro_originais_retificacoes_dctf
	...
	{
		$qr_sql = "
            SELECT cd_financeiro_originais_retificacoes_dctf,
                   ...
                   observacao
		      FROM indicador_plugin.financeiro_originais_retificacoes_dctf 
			 WHERE cd_financeiro_originais_retificacoes_dctf = ".intval($args['cd_financeiro_originais_retificacoes_dctf']).";";
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.financeiro_originais_retificacoes_dctf 
				     (
						dt_referencia, 
					    nr_valor_1, 
                        nr_valor_2, 
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.financeiro_originais_retificacoes_dctf
