<?php
class exp_valor_contratado_model extends Model
{
	...
	{
		$qr_sql = "
					SELECT i.cd_exp_valor_contratado,
					  ...
					  FROM indicador_plugin.exp_valor_contratado i
	...
	function carrega_referencia(&$result, $args=array())
	{
		$qr_sql = "
					SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia_n, 
						   nr_meta, 
						   cd_indicador_tabela 
					  FROM indicador_plugin.exp_valor_contratado
	...
	{
		$qr_sql = "
					SELECT cd_exp_valor_contratado,
						   ...
						   observacao
					  FROM indicador_plugin.exp_valor_contratado 
					 WHERE cd_exp_valor_contratado = ".intval($args['cd_exp_valor_contratado'])."
			      ";
		$result = $this->db->query($qr_sql);
	}
	...
	function salvar(&$result, $args=array())
	{
		...
		{
			$qr_sql = "
						INSERT INTO indicador_plugin.exp_valor_contratado 
							 (
								dt_referencia, 
								nr_contratado,
								nr_meta, 
		...
		{
			$qr_sql = "
						UPDATE indicador_plugin.exp_valor_contratado
