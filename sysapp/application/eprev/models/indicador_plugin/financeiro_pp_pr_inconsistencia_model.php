<?php
class Financeiro_pp_pr_inconsistencia_model extends Model
{
	...
	{
		$qr_sql = "
			SELECT c1.cd_financeiro_pp_pr_inconsistencia,
				   ...
				   c1.dt_referencia,
				   c1.nr_total,
				   c1.nr_inconsistencia,
				   c1.nr_resultado,
				   c1.nr_meta,
				   c1.fl_media,
				   c1.ds_observacao
		      FROM indicador_plugin.financeiro_pp_pr_inconsistencia c1
	...
	public function carrega_referencia($nr_ano_referencia)
	{
		$qr_sql = "
			SELECT TO_CHAR(dt_referencia + '1 month'::interval, 'DD/MM/YYYY') AS dt_referencia_n, 
				   ...
				   cd_indicador_tabela,
				   (SELECT COUNT(*)
				      FROM indicador_plugin.financeiro_pp_pr_inconsistencia
	...
	public function carrega($cd_financeiro_pp_pr_inconsistencia)
	{
		$qr_sql = "
            SELECT cd_financeiro_pp_pr_inconsistencia,
                   ...
                   ds_observacao
		      FROM indicador_plugin.financeiro_pp_pr_inconsistencia 
			 WHERE cd_financeiro_pp_pr_inconsistencia = ".intval($cd_financeiro_pp_pr_inconsistencia).";";
	...
	{
		if(intval($args['cd_financeiro_pp_pr_inconsistencia']) == 0)
		{
			$qr_sql = "
				INSERT INTO indicador_plugin.financeiro_pp_pr_inconsistencia 
				     (
				        cd_indicador_tabela, 
						dt_referencia, 
					    fl_media,
			     ...
			     VALUES 
				      ( 
					    ...
					    ".(trim($args['nr_inconsistencia']) != '' ? floatval($args['nr_inconsistencia']) : "DEFAULT").",
		...
		{
			$qr_sql = "
				UPDATE indicador_plugin.financeiro_pp_pr_inconsistencia
				   SET dt_referencia          = ".(trim($args['dt_referencia']) != '' ? "TO_DATE('".trim($args['dt_referencia'])."', 'DD/MM/YYYY')" : "DEFAULT").",
				       ...
				       nr_inconsistencia = ".(trim($args['nr_inconsistencia']) != '' ? floatval($args['nr_inconsistencia']) : "DEFAULT").",
			     ...
			     WHERE cd_financeiro_pp_pr_inconsistencia = ".intval($args['cd_financeiro_pp_pr_inconsistencia']).";";
		}

		$this->db->query($qr_sql);
	}
	
	public function excluir($cd_financeiro_pp_pr_inconsistencia, $cd_usuario)
	{
		$qr_sql = " 
			UPDATE indicador_plugin.financeiro_pp_pr_inconsistencia
