<?php
class administrativo_evo_per_escolar_model extends Model
{
	...
	function listar( &$result, $args=array() )
	{
		// mount query
		$sql = "
					SELECT i.cd_administrativo_evo_per_escolar,
						   ...
						   CASE WHEN (SELECT MAX(i1.dt_referencia)
						                FROM indicador_plugin.administrativo_evo_per_escolar i1
					                   WHERE i1.dt_exclusao IS NULL) = i.dt_referencia
								THEN 'S'
								ELSE 'N'
						   END AS fl_editar
					  FROM indicador_plugin.administrativo_evo_per_escolar i
	...
	function carregar($cd)
	{
		...
		cd_administrativo_evo_per_escolar
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_administrativo_evo_per_escolar={cd_administrativo_evo_per_escolar} ";
			esc( "{cd_administrativo_evo_per_escolar}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_administrativo_evo_per_escolar'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.administrativo_evo_per_escolar (
		...
		{
			$sql="
			UPDATE indicador_plugin.administrativo_evo_per_escolar
			SET 
			
			cd_administrativo_evo_per_escolar = {cd_administrativo_evo_per_escolar}
		...
		esc("{cd_administrativo_evo_per_escolar}", $args["cd_administrativo_evo_per_escolar"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_administrativo_evo_per_escolar::varchar)='{cd_administrativo_evo_per_escolar}'
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_administrativo_evo_per_escolar}', $id, $sql, 'str');
