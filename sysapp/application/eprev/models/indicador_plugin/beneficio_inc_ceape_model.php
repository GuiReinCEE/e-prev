<?php
class beneficio_inc_ceape_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$sql .= " WHERE cd_beneficio_inc_ceape={cd_beneficio_inc_ceape} ";
			esc( "{cd_beneficio_inc_ceape}", intval($cd), $sql );
	...
	{
		if(intval($args['cd_beneficio_inc_ceape'])==0)
		{
			$sql="
			INSERT INTO indicador_plugin.beneficio_inc_ceape ( 
		...
		{
			$sql="
			UPDATE indicador_plugin.beneficio_inc_ceape 
			SET 
			
			cd_beneficio_inc_ceape = {cd_beneficio_inc_ceape} 
		...
		esc("{cd_beneficio_inc_ceape}", $args["cd_beneficio_inc_ceape"], $sql, "int", FALSE);
	...
	function excluir($id)
	{
		...
		WHERE md5(cd_beneficio_inc_ceape::varchar)='{cd_beneficio_inc_ceape}' 
		"; 
		esc('{cd_usuario_exclusao}', usuario_id(), $sql, 'int'); 
		esc('{cd_beneficio_inc_ceape}', $id, $sql, 'str'); 
