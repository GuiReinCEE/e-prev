<?php
class Sinprors_previdencia_inscricao_model extends Model
{
		...
		$qr_sql = "
			SELECT i.cd_interessado,
				   ...
				   (SELECT TO_CHAR(MAX(CAST(ic.dt_contato AS DATE)),'DD/MM/YYYY')
					  FROM sinprors_previdencia.interessado_contato ic
			  ...
			  FROM sinprors_previdencia.interessado i
			  JOIN sinprors_previdencia.situacao s
				ON s.cd_situacao = i.cd_situacao
			  JOIN sinprors_previdencia.usuario u
