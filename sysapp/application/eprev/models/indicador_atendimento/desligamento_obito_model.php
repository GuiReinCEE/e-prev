<?php
class desligamento_obito_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$qr_sql .= " WHERE cd_desligamento_obito=".intval($cd);
	...
	{
		if(intval($args['cd_desligamento_obito'])==0)
		{
			$qr_sql="
		...
		{
			$qr_sql="
                UPDATE indicador_atendimento.desligamento_obito
		           SET cd_desligamento_obito     = ".intval($args["cd_desligamento_obito"]).",
			     ...
			     WHERE cd_desligamento_obito = ".intval($args["cd_desligamento_obito"])."
			";
		}
	...
	function excluir($id)
	{
		$qr_sql = "
            UPDATE indicador_atendimento.desligamento_obito
		     ...
		     WHERE md5(cd_desligamento_obito::varchar)='".$id."'
		";
