<?php
class desligamento_inadi_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$qr_sql .= " WHERE cd_desligamento_inadi=".intval($cd);
	...
	{
		if(intval($args['cd_desligamento_inadi'])==0)
		{
			$qr_sql="
		...
		{
			$qr_sql="
                UPDATE indicador_atendimento.desligamento_inadi
		           SET cd_desligamento_inadi     = ".intval($args["cd_desligamento_inadi"]).",
			     ...
			     WHERE cd_desligamento_inadi = ".intval($args["cd_desligamento_inadi"])."
			";
		}
	...
	function excluir($id)
	{
		$qr_sql = "
            UPDATE indicador_atendimento.desligamento_inadi
		     ...
		     WHERE md5(cd_desligamento_inadi::varchar)='".$id."'
		"; 
