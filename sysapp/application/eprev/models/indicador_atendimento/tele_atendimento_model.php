<?php
class tele_atendimento_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$qr_sql .= " WHERE cd_tele_atendimento=".intval($cd);
	...
	{
		if(intval($args['cd_tele_atendimento'])==0)
		{
			$qr_sql="
		...
		{
			$qr_sql="
                UPDATE indicador_atendimento.tele_atendimento
		           SET cd_tele_atendimento     = ".intval($args["cd_tele_atendimento"]).",
			     ...
			     WHERE cd_tele_atendimento = ".intval($args["cd_tele_atendimento"])."
			";
		}
	...
	function excluir($id)
	{
		$qr_sql = "
            UPDATE indicador_atendimento.tele_atendimento
		     ...
		     WHERE md5(cd_tele_atendimento::varchar)='".$id."'
		"; 
