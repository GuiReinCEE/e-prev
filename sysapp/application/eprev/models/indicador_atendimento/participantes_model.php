...
class participantes_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$qr_sql .= " WHERE cd_participantes=".intval($cd);
	...
	{
		if(intval($args['cd_participantes'])==0)
		{
			$qr_sql="
		...
		{
			$qr_sql="
                UPDATE indicador_atendimento.participantes
		           SET cd_participantes     = ".intval($args["cd_participantes"]).",
			     ...
			     WHERE cd_participantes = ".intval($args["cd_participantes"])."
			";
		}
	...
	function excluir($id)
	{
		$qr_sql = "
            UPDATE indicador_atendimento.participantes
		     ...
		     WHERE md5(cd_participantes::varchar)='".$id."'
		"; 
