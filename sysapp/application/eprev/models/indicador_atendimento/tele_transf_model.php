<?php
class tele_transf_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$qr_sql .= " WHERE cd_tele_transf=".intval($cd);
	...
	{
		if(intval($args['cd_tele_transf'])==0)
		{
			$qr_sql="
		...
		{
			$qr_sql="
                UPDATE indicador_atendimento.tele_transf
		           SET cd_tele_transf     = ".intval($args["cd_tele_transf"]).",
			     ...
			     WHERE cd_tele_transf = ".intval($args["cd_tele_transf"])."
			";
		}
	...
	function excluir($id)
	{
		$qr_sql = "
            UPDATE indicador_atendimento.tele_transf
		     ...
		     WHERE md5(cd_tele_transf::varchar)='".$id."'
		";
