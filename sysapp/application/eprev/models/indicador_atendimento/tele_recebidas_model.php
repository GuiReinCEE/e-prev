<?php
class tele_recebidas_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$qr_sql .= " WHERE cd_tele_recebidas=".intval($cd);
	...
	{
		if(intval($args['cd_tele_recebidas'])==0)
		{
			$qr_sql="
		...
		{
			$qr_sql="
                UPDATE indicador_atendimento.tele_recebidas
		           SET cd_tele_recebidas     = ".intval($args["cd_tele_recebidas"]).",
			     ...
			     WHERE cd_tele_recebidas = ".intval($args["cd_tele_recebidas"])."
			";
		}
	...
	function excluir($id)
	{
		$qr_sql = "
            UPDATE indicador_atendimento.tele_recebidas
		     ...
		     WHERE md5(cd_tele_recebidas::varchar)='".$id."'
		"; 
