<?php
class ingressos_inst_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$qr_sql .= " WHERE cd_ingressos_inst=".intval($cd);
	...
	{
		if(intval($args['cd_ingressos_inst'])==0)
		{
			$qr_sql="
		...
		{
			$qr_sql="
                UPDATE indicador_atendimento.ingressos_inst
		           SET cd_ingressos_inst     = ".intval($args["cd_ingressos_inst"]).",
			     ...
			     WHERE cd_ingressos_inst = ".intval($args["cd_ingressos_inst"])."
			";
		}
	...
	function excluir($id)
	{
		$qr_sql = "
            UPDATE indicador_atendimento.ingressos_inst
		     ...
		     WHERE md5(cd_ingressos_inst::varchar)='".$id."'
		"; 
