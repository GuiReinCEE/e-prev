<?php
class ingressos_patro_model extends Model
{
	...
	function carregar($cd)
	{
		...
		if( intval($cd)>0 )
		{
			$qr_sql .= " WHERE cd_ingressos_patro=".intval($cd);
	...
	{
		if(intval($args['cd_ingressos_patro'])==0)
		{
			$qr_sql="
		...
		{
			$qr_sql="
                UPDATE indicador_atendimento.ingressos_patro
		           SET cd_ingressos_patro     = ".intval($args["cd_ingressos_patro"]).",
			     ...
			     WHERE cd_ingressos_patro = ".intval($args["cd_ingressos_patro"])."
			";
		}
	...
	function excluir($id)
	{
		$qr_sql = "
            UPDATE indicador_atendimento.ingressos_patro
		     ...
		     WHERE md5(cd_ingressos_patro::varchar)='".$id."'
		"; 
