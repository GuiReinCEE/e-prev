...
class Pauta_sg extends Controller
{
	...
	function __construct()
    {
		...
		*/		
		...
		$this->dir_own_aprovado = 'GESTAO/DOCUMENTOS_APROVADOS/';
		$this->dir_own_de       = 'GESTAO/DIRETORIA/';
		$this->dir_own_cd       = 'GESTAO/DELIBERATIVO/';
		$this->dir_own_cf       = 'GESTAO/FISCAL/';
	...
	/*
	public function cria_pasta_own($cd_pauta_sg)
	{
		$this->load->model('gestao/pauta_sg_model');

		$pauta_sg = $this->pauta_sg_model->carrega(intval($cd_pauta_sg));
		
		$this->integracao_own($pauta_sg['dt_pauta'], $pauta_sg['nr_ata'], $pauta_sg['fl_sumula']);
	}
	*/
	private function integracao_own($dt_pauta, $nr_ata, $fl_sumula)
	{
		$this->load->library('owncloud');
		
		$dir_own = '';
		$dir_own_aprovado = $this->dir_own_aprovado;

		if(trim($fl_sumula) == 'DE')
		{
			$dir_own = $this->dir_own_de;
		}
		else if(trim($fl_sumula) == 'CF')
		{
			$dir_own = $this->dir_own_cf;
		}
		else if(trim($fl_sumula) == 'CD')
		{
			$dir_own = $this->dir_own_cd;
		}
		...
		#### CRIA DIRETORIO ANO ####
		$dir_own_tmp = $dir_own.$fl_sumula."/".$ar_data[2];
		
		$this->owncloud->cria($dir_own_tmp);
		
		$dir_own_aprovado = $dir_own_aprovado.$fl_sumula."/".$ar_data[2];
		$this->owncloud->cria($dir_own_aprovado);
		#### CRIA DIRETORIO ANO ####
		
		#### CRIA DIRETORIO DA REUNIAO ####
		$dir_own_tmp .= '/Reuniao '.$nr_ata.' de '.$dt_data;
		$this->owncloud->cria($dir_own_tmp);
		
		$dir_own_aprovado .= '/Reuniao '.$nr_ata.' de '.$dt_data;
		$this->owncloud->cria($dir_own_aprovado);	
		#### CRIA DIRETORIO DA REUNIAO ####
		
		#### CRIA DIRETORIO DOCUMENTOS DA REUNIAO ####
		$dir_own_tmp .= '/documentos';
		$this->owncloud->cria($dir_own_tmp);
		
		$dir_own_aprovado .= '/documentos';
		$this->owncloud->cria($dir_own_aprovado);	
		#### CRIA DIRETORIO DOCUMENTOS DA REUNIAO ####
	}
	
	/*
	public function rmn_own($cd_pauta_sg)
	{
		$this->renomeia_pasta_own($cd_pauta_sg, '06/01/2025', 899, 'CD');
	}
	*/
	private function renomeia_pasta_own($cd_pauta_sg, $dt_pauta, $nr_ata, $fl_sumula)
	{
		$this->load->library('owncloud');
		...
		$ar_data = explode('-', str_replace('/', '-', $pauta_sg['dt_pauta']));
		$dt_data_old = str_replace('/', '-', $pauta_sg['dt_pauta']);
		
		$dir_old = $fl_sumula."/".$ar_data[2].'/Reuniao '.$pauta_sg['nr_ata'].' de '.$dt_data_old;
		
		$ar_data = explode('-', str_replace('/', '-', $dt_pauta));
		$dt_pauta = str_replace('/', '-', $dt_pauta);
		
		$dir = $fl_sumula."/".$ar_data[2].'/Reuniao '.$nr_ata.' de '.$dt_pauta;
		
		$this->owncloud->renomeia($dir_own.$dir_old, $dir_own.$dir);
		$this->owncloud->renomeia($dir_own_aprovado.$dir_old, $dir_own_aprovado.$dir);
	}

	private function envia_arquivo_documentos_own($cd_pauta_sg, $dir_file, $fl_sumula, $name_file, $fl_documentos = TRUE)
	{
		$this->load->library('owncloud');
		...
		$ar_data = explode('-', str_replace('/', '-', $pauta_sg['dt_pauta']));
		$dt_data = str_replace('/', '-', $pauta_sg['dt_pauta']);
		
		$dir = $dir_own.$fl_sumula."/".$ar_data[2].'/Reuniao '.$pauta_sg['nr_ata'].' de '.$dt_data;
		...
		$this->owncloud->envia($dir, $dir_file, $name_file);
	}
	
	private function excluir_pauta_own($cd_pauta_sg)
	{
		$this->load->library('owncloud');
		...
		$dir = $dir_own.$fl_sumula."/".$ar_data[2].'/Reuniao '.$pauta_sg['nr_ata'].' de '.$dt_data.'/Pauta_'.intval($row['nr_ata']).'.pdf';
		
		$this->owncloud->deleta($dir);
		
		$dir = $dir_own_aprovado.$fl_sumula."/".$ar_data[2].'/Reuniao '.$pauta_sg['nr_ata'].' de '.$dt_data.'/Pauta_'.intval($row['nr_ata']).'.pdf';
		
		$this->owncloud->deleta($dir);
	}
	
	private function excluir_arquivo_documentos_own($cd_pauta_sg, $fl_sumula, $name_file, $fl_documentos = TRUE)
	{
		$this->load->library('owncloud');
		...
		$this->owncloud->deleta($dir, $dir_file, $name_file);
	}
    ...
    private function integracao_link($dir, $dt_pauta, $nr_ata, $fl_sumula)
	{
		...
		$this->integracao_own($dt_pauta, $nr_ata, $fl_sumula);
	...
	private function altera_integracao_link($integracao_arq, $dt_pauta, $nr_ata, $fl_sumula)
	{
		...
		$this->renomeia_pasta_own($cd_pauta_sg, $dt_pauta, $nr_ata, $fl_sumula);		
				...
				if(count($anexos) > 0)
				{
					if(trim($integracao_arq) != '')
					{
						foreach ($anexos as $key => $item) 
						{
							...
							$this->envia_arquivo_documentos_own(
								$cd_pauta_sg, 
								'/u/www/cieprev/up/pauta/'.$item['arquivo'],
								trim($args['fl_sumula']),
								str_pad($item['nr_item_sumula'] ,3,'0', STR_PAD_LEFT).' - '.fixUTF8($item['arquivo_nome'])
							);
						}
					}
                }
			...
			else
			{
				...
				if(intval($args['nr_ata']) != intval($row['nr_ata']) OR trim($this->input->post('dt_pauta', TRUE)) != trim($row['dt_pauta']))
				{
					if(file_exists(trim($row['integracao_arq']).'/Pauta_'.intval($row['nr_ata']).'.pdf'))
					{
						...
						$this->excluir_pauta_own($cd_pauta_sg);
		...
		if(count($anexos))
		{
			...
			if(trim($pauta['integracao_arq']) != '')
			{
				...
				foreach ($anexos as $key => $item) 
				{
					...
					if(file_exists($pauta['integracao_arq'].'/documentos/'.str_pad($assunto['nr_item_sumula'] ,3,'0', STR_PAD_LEFT).' - '.fixUTF8($item['arquivo_nome'])))
					{
						...
						*/
						rename(
							$pauta['integracao_arq'].'/documentos/'.str_pad($assunto['nr_item_sumula'] ,3,'0', STR_PAD_LEFT).' - '.fixUTF8($item['arquivo_nome']), 
							$pauta['integracao_arq'].'/documentos/'.str_pad($args['nr_item_sumula'] ,3,'0', STR_PAD_LEFT).' - '.fixUTF8($item['arquivo_nome'])
						);						
					}
				}

				#if(file_exists(fixUTF8($pauta['integracao_arq'].'/documentos/'.$assunto['nr_item_sumula'].' - Apresentacao.pdf')))
				if(file_exists($pauta['integracao_arq'].'/documentos/'.str_pad($assunto['nr_item_sumula'] ,3,'0', STR_PAD_LEFT).' - '.fixUTF8('Apresentacao.pdf')))
				{
					/*
					rename(
						fixUTF8($pauta['integracao_arq'].'/documentos/'.$assunto['nr_item_sumula'].' - Apresentacao.pdf'), 
						fixUTF8($pauta['integracao_arq'].'/documentos/'.$args['nr_item_sumula'].' - Apresentacao.pdf')
					);
					*/
					rename(
						$pauta['integracao_arq'].'/documentos/'.str_pad($assunto['nr_item_sumula'] ,3,'0', STR_PAD_LEFT).' - '.fixUTF8('Apresentacao.pdf'), 
						$pauta['integracao_arq'].'/documentos/'.str_pad($args['nr_item_sumula'] ,3,'0', STR_PAD_LEFT).' - '.fixUTF8('Apresentacao.pdf')
					);					
				}
			}
		}
	...
	public function pauta_gera_rds($cd_pauta_sg)
	{
        $this->load->model('gestao/pauta_sg_model');
				...
				if(count($arquivo) > 0)
				{
					...
					if(count($controle_rds) == 0)
					{
						$args = array(
							...
							'nr_ano_rds'          => trim($item['nr_ano_rds']),
							'nr_rds'              => trim($item['nr_rds']),
		...
		if($this->get_permissao())
		{
			...
			if($qt_arquivo > 0)
			{
				...
				while($nr_conta < $qt_arquivo)
				{
					...
					$cd_pauta_sg_assunto_anexo = $this->pauta_sg_model->anexo_salvar($args);
					...
					if(is_dir($pauta['integracao_arq']))
					{
						...
						copy('../cieprev/up/pauta/'.$anexo['arquivo'], $pauta['integracao_arq'].'/documentos/'.str_pad($assunto['nr_item_sumula'] ,3,'0', STR_PAD_LEFT).' - '.fixUTF8($anexo['arquivo_nome']));
					
						### MIGRADO OWN ###
						$this->envia_arquivo_documentos_own(
							$cd_pauta_sg, 
							'/u/www/cieprev/up/pauta/'.$anexo['arquivo'],
							$pauta['fl_sumula'],
							str_pad($assunto['nr_item_sumula'] ,3,'0', STR_PAD_LEFT).' - '.fixUTF8($anexo['arquivo_nome'])
						);
					}
				}
			}		
	...
	public function historico($cd_pauta_sg, $cd_pauta_sg_assunto, $cd_pauta_sg_assunto_anexo)
	{
		if($this->get_permissao())
		{
			$this->load->model('gestao/pauta_sg_model');
			...
			$data = array(
				'row'        => $this->pauta_sg_model->carrega(intval($cd_pauta_sg)),
				'assunto'    => $this->pauta_sg_model->assunto_carrega(intval($cd_pauta_sg_assunto)),
				'collection' => $this->pauta_sg_model->historico_listar($cd_pauta_sg_assunto_anexo)
			);

			$this->load->view('gestao/pauta_sg/historico', $data);
		}
		else
		{
			exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function importa_ano($nr_ano, $fl_sumula)
	{

		$this->load->model('gestao/pauta_sg_model');
		
		$pautas = $this->pauta_sg_model->listar_ano(intval($nr_ano), $fl_sumula);

		foreach($pautas as $key => $item)
		{;
			$this->set_own($item['cd_pauta_sg']);
		}
	}
	
	public function set_own($cd_pauta_sg)
	{
		$this->load->model('gestao/pauta_sg_model');
		
		$pauta = $this->pauta_sg_model->carrega(intval($cd_pauta_sg));
		
		$this->integracao_own($pauta['dt_pauta'], $pauta['nr_ata'], $pauta['fl_sumula']);
		
		$this->pauta_gerar($cd_pauta_sg);
		
		if(trim($pauta['fl_sumula']) == 'DE')
		{
			$sumula = $this->pauta_sg_model->sumula_diretoria($cd_pauta_sg);
			
			if(isset($sumula['arquivo']) AND trim($sumula['arquivo']) != '')
			{
				$this->envia_arquivo_documentos_own(
					$cd_pauta_sg, 
					'/u/www/cieprev/up/sumula/'.$sumula['arquivo'],
					$pauta['fl_sumula'],
					'Sumula_'.$sumula['nr_sumula'].'.pdf',
					false
				);
			}
			
			if(isset($sumula['arquivo_ata']) AND trim($sumula['arquivo_ata']) != '')
			{
				$this->envia_arquivo_documentos_own(
					$cd_pauta_sg, 
					'/u/www/cieprev/up/sumula/'.$sumula['arquivo_ata'],
					$pauta['fl_sumula'],
					'ade-'.$sumula['nr_sumula'].'.pdf',
					false
				);
			}
		}
		
		if(trim($pauta['fl_sumula']) == 'CF')
		{
			$sumula = $this->pauta_sg_model->sumula_conselho_fiscal($cd_pauta_sg);
			
			if(isset($sumula['arquivo']) AND trim($sumula['arquivo']) != '')
			{
				$this->envia_arquivo_documentos_own(
					$cd_pauta_sg, 
					'/u/www/cieprev/up/sumula_conselho_fiscal/'.$sumula['arquivo'],
					$pauta['fl_sumula'],
					'Sumula_'.$sumula['nr_sumula_conselho_fiscal'].'.pdf',
					false
				);
			}
			...
			if(isset($sumula['arquivo_ata']) AND trim($sumula['arquivo_ata']) != '')
			{
				$this->envia_arquivo_documentos_own(
					$cd_pauta_sg, 
					'/u/www/cieprev/up/sumula_conselho_fiscal/'.$sumula['arquivo_ata'],
					$pauta['fl_sumula'],
					'ade-'.$sumula['nr_sumula_conselho_fiscal'].'.pdf',
					false
				);
			}
		}
		
		if(trim($pauta['fl_sumula']) == 'CD')
		{
			$sumula = $this->pauta_sg_model->sumula_conselho_deliberativo($cd_pauta_sg);
			
			if(isset($sumula['arquivo']) AND trim($sumula['arquivo']) != '')
			{
				$this->envia_arquivo_documentos_own(
					$cd_pauta_sg, 
					'/u/www/cieprev/up/sumula_conselho/'.$sumula['arquivo'],
					$pauta['fl_sumula'],
					'Sumula_'.$sumula['nr_sumula_conselho'].'.pdf',
					false
				);
			}
			...
			if(isset($sumula['arquivo_ata']) AND trim($sumula['arquivo_ata']) != '')
			{
				$this->envia_arquivo_documentos_own(
					$cd_pauta_sg, 
					'/u/www/cieprev/up/sumula_conselho/'.$sumula['arquivo_ata'],
					$pauta['fl_sumula'],
					'ade-'.$sumula['nr_sumula_conselho'].'.pdf',
					false
				);
			}
		}
		
		$assuntos = $this->pauta_sg_model->assunto_listar(intval($cd_pauta_sg), 'S');
		
		foreach($assuntos as $key => $item)
		{
			$cd_pauta_sg_assunto = $item['cd_pauta_sg_assunto'];
	...
	public function set_pydio($cd_pauta_sg, $cd_pauta_sg_assunto)
	{
		$this->load->model('gestao/pauta_sg_model');
		...
		if(count($anexos))
		{
			...
			if(trim($pauta['integracao_arq']) != '')
			{
				foreach ($anexos as $key => $item) 
				{
					if(!file_exists($pauta['integracao_arq'].'/documentos/'.str_pad($assunto['nr_item_sumula'] ,3,'0', STR_PAD_LEFT).' - '.fixUTF8($item['arquivo_nome'])))
					{
						copy('../cieprev/up/pauta/'.$item['arquivo'], $pauta['integracao_arq'].'/documentos/'.str_pad($assunto['nr_item_sumula'] ,3,'0', STR_PAD_LEFT).' - '.fixUTF8($item['arquivo_nome']));	
					}
					...
					$this->envia_arquivo_documentos_own(
						...
						'/u/www/cieprev/up/pauta/'.$item['arquivo'],
						$pauta['fl_sumula'],
						str_pad($assunto['nr_item_sumula'] ,3,'0', STR_PAD_LEFT).' - '.fixUTF8($item['arquivo_nome'])
					);
				}
	...
	private function pauta_gerar($cd_pauta_sg, $fl_autoatendimento = FALSE)
	{
		if($this->get_permissao())
		{
			...
			if(is_dir($row['integracao_arq']))
			{
				...
				$this->envia_arquivo_documentos_own(
					$cd_pauta_sg, 
					'/u/www/cieprev/up/pauta/'.$arq_pauta,
					$row['fl_sumula'],
					'Pauta_'.$row['nr_ata'].'.pdf',
					false
				);
			...
			else
			{
				...
				echo $row['integracao_arq'].' n entro';
	...
	public function capa_assunto($cd_pauta_sg, $cd_pauta_sg_assunto)
	{
		if($this->get_permissao())
		{
			$this->load->model('gestao/pauta_sg_model');
	        ...
	        $ob_pdf->MultiCell(190, 4.5, 'ANEXO', 0, 'C');

	        $ob_pdf->SetY($ob_pdf->GetY() + 30);

	        $ob_pdf->MultiCell(190, 4.5, $assunto['nr_item_sumula'].') '.$assunto['ds_pauta_sg_assunto'], 0, 'C');

	        $ob_pdf->SetFont('Courier','I',6);
            $ob_pdf->SetY($ob_pdf->GetY() + 160);
            $ob_pdf->MultiCell(190, 4.5,'Fundação Família Previdência',0,'C');
            $ob_pdf->MultiCell(190, 4.5,'Rua dos Andradas, 702 - Porto Alegre/RS, 90020-004 - Tel: 51 3027 3100 - www.fundacaofamiliaprevidencia.com.br',0,'C');
    ...
    public function responder_salvar()
    {
		...
		if(intval($cd_usuario) == intval($assunto['cd_usuario_responsavel']) OR intval($cd_usuario) == intval($assunto['cd_usuario_substituto']))
		{
			...
			if(trim($fl_aplica_rds) == 'S')
			{
				...
				$this->pauta_sg_model->set_historico_rds($cd_pauta_sg_assunto_anexo, $args);
			}
			else
			{
			...
			if(trim($fl_ordem_fornecimento) == 'S')
			{
				...
				if(is_dir($pauta['integracao_arq']))
				{
					...
					$this->envia_arquivo_documentos_own(
						$cd_pauta_sg, 
						'/u/www/cieprev/up/pauta/'.$args['arquivo'],
						$pauta['fl_sumula'],
						str_pad($assunto['nr_item_sumula'] ,3,'0', STR_PAD_LEFT).' - '.fixUTF8($args['arquivo_nome'])
					);
				}
			}
    ...
    public function responder_anexo_salvar()
	{
		...
		if(intval($cd_usuario) == intval($assunto['cd_usuario_responsavel']) OR intval($cd_usuario) == intval($assunto['cd_usuario_substituto']))
		{
			...
			if($qt_arquivo > 0)
			{
				...
				while($nr_conta < $qt_arquivo)
				{					
					$args['fl_quadro_comparativo']  = "";
					$args['fl_ordem_fornecimento']  = "";
					$args['arquivo_nome']  = $this->input->post('arquivo_m_'.$nr_conta.'_name', TRUE);
					$args['arquivo']       = $this->input->post('arquivo_m_'.$nr_conta.'_tmpname', TRUE);
	...
	public function responder_anexo_excluir($cd_pauta_sg, $cd_pauta_sg_assunto, $cd_pauta_sg_assunto_anexo)
	{
		...
		if(intval($cd_usuario) == intval($assunto['cd_usuario_responsavel']) OR intval($cd_usuario) == intval($assunto['cd_usuario_substituto']))
		{
			...
			if(trim($pauta['integracao_arq']) != '')
			{
				...
				### MIGRADO OWN ###
				$this->excluir_arquivo_documentos_own(
					$cd_pauta_sg, 
					$pauta['fl_sumula'], 
					str_pad($assunto['nr_item_sumula'] ,3,'0', STR_PAD_LEFT).' - '.fixUTF8($anexo['arquivo_nome'])
				);
	...
	public function apresentacao_gerar($cd_pauta_sg, $cd_pauta_sg_assunto, $fl_pdf_output="N")
	{
		$this->load->model('gestao/pauta_sg_model');
        ...
        if(trim($assunto['ds_recomendacao']) != '')
        {
	        if(trim($pauta['fl_sumula']) == 'DE')
			{	
				$this->monta_pagina($ob_pdf, 'Recomendação da Gerência');
			}
			else
			{
				$this->monta_pagina($ob_pdf, 'Recomendação da Diretoria Executiva');
			}

	        $ob_pdf->SetTextColor(0, 60, 85);

	        $this->set_fonte_texto($ob_pdf, $assunto['ds_recomendacao']);

	        $ob_pdf->MultiCell(280, 9, trim($assunto['ds_recomendacao']), '0', 'J');
    	}

        if(count($anexos) > 0)
        {
        	$this->monta_pagina($ob_pdf, 'Anexos');
        }

        $ob_pdf->SetTextColor(50,50,220);

        $ob_pdf->SetFont('Arial', '', 15);
        ...
        foreach ($anexos as $key => $item) 
        {
        	...
        	$ob_pdf->Write(5, $item['arquivo_nome'], base_url().'up/pauta/'.$item['arquivo']);
        	$ob_pdf->SetY($ob_pdf->GetY()+8);
        }
        
        $arq_pauta = $assunto['nr_item_sumula'].'_Apresentacao.pdf';
		...
		#### INTEGRAÇÃO LINK - fceee.com.br/arq ####
		if(is_dir($pauta['integracao_arq']))
		{
			$ob_pdf->Output('up/pauta/'.$arq_pauta, 'F');

			#copy('../cieprev/up/pauta/'.$arq_pauta, $pauta['integracao_arq'].'/documentos/'.$assunto['nr_item_sumula'].' - '.fixUTF8('Apresentacao').'.pdf');
			copy('../cieprev/up/pauta/'.$arq_pauta, $pauta['integracao_arq'].'/documentos/'.str_pad($assunto['nr_item_sumula'] ,3,'0', STR_PAD_LEFT).' - '.fixUTF8('Apresentacao.pdf'));
			...
			$this->envia_arquivo_documentos_own(
				...
				'/u/www/cieprev/up/pauta/'.$arq_pauta,
				$pauta['fl_sumula'],
				str_pad($assunto['nr_item_sumula'] ,3,'0', STR_PAD_LEFT).' - '.fixUTF8('Apresentacao.pdf')
			);
    ...
    public function getPautaAssinatura()
    {
		if (gerencia_in(array('GC')))
        {
			$this->load->model('gestao/pauta_sg_model');
			...
			$args['cd_pauta_sg'] = $this->input->post("cd_pauta_sg", TRUE); 

			$this->pauta_sg_model->getPautaAssinatura($result, $args);

			$data['collection'] = $result->result_array();

			$this->load->view('gestao/pauta_sg/cadastro_assinatura_result', $data);
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
