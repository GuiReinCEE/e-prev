...
class Pauta_sg extends Controller
{
	...
	private function get_integrante($cd_pauta_sg)
	{
        ...
        foreach ($this->pauta_sg_model->get_presentes(intval($cd_pauta_sg)) as $key => $item)
        {
        	if(trim($item['fl_secretaria']) == 'N')
        	{
            	...
            	{
            		$cd_titular = $item['cd_titular'];
            		$ds_titular = $item['ds_titular'].' - Presidente';
            	}
	...
	public function enviar_colegiado($cd_pauta_sg)
	{
		if($this->get_permissao())
		{
			$this->load->model(array(
			...
			$tags = array('[NR_ATA]', '[DS_TIPO_REUNIAO]', '[DS_LINK_DIRETO]', '[LINK_DIRETO]');
			
			$ds_link_direto = 'Clique neste link para acesso direto aos documentos desta pauta: ';
			
			$link_direto    = 'https://www.fundacaofamiliaprevidencia.com.br/link/?p='.$row['cd_pauta_sg_md5'];
			$subs = array($row['nr_ata'], $row['ds_tipo_reuniao'], $ds_link_direto, $link_direto);
	...
	public function sumula($cd_pauta_sg)
	{
		if(gerencia_in(array("SG", "DE")))
		{
	        ...
	        $ob_pdf->MultiCell(190, 4.5, 'SÚMULA DA REUNIÃO D'.(trim($row['fl_sumula']) == 'IN' ? 'O' : 'A'), 0, 'C');
	        ...
	        if(count($collection['titular']) > 0)
	        {
		        ...
		        {
		        	$ob_pdf->Text(20,$linha+35, trim($item['ds_secretaria']));	 
		        ...
		        foreach ($titular as $key => $item)
		        {
		        	$ob_pdf->Text(20,$linha+20, trim($item['ds_titular']));	 
		        ...
		        foreach ($suplente as $key => $item)
		        {
		        	$ob_pdf->Text(120,$linha1+20, trim($item['ds_suplente']));	 
	...
	public function apresentacao_gerar($cd_pauta_sg, $cd_pauta_sg_assunto)
	{
		...
		if(is_dir($pauta['integracao_arq']))
		{
			...
			copy('../cieprev/up/pauta/'.$arq_pauta, $pauta['integracao_arq'].'/documentos/'.$assunto['nr_item_sumula'].' - '.fixUTF8('Apresentação').'.pdf');
		}
		else
		{
			$ob_pdf->Output('up/pauta/'.$arq_pauta,'F');
