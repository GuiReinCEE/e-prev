<?php
class Apresentacao_pga extends Controller
{
	...
	{
		$this->load->model('gestao/apresentacao_pga_model');

		$data['collection'] = $this->apresentacao_pga_model->get_indicador_lista();

		$this->load->view('gestao/apresentacao_pga/apresentacao', $data);
	}
	...
	public function indicador_result()
	{
		...
		$indicador_tabela = $this->apresentacao_pga_model->get_indicador(intval($cd_indicador));

		$indicador_tabela = array_map('arrayToUTF8', $indicador_tabela);		

		$parametro = $this->apresentacao_pga_model->get_indicador_parametro(intval($indicador_tabela['cd_indicador_tabela']));
		...
		$this->load->view('gestao/apresentacao_pga/apresentacao_result', $data);
	}
	...
	public function salvar_imagem()
	{
		...
		$file = '../cieprev/up/apresentacao_pga/'.$arq.'.png';
	...
	public function gera_pdf($qt)
	{
		...
		$ob_pdf->header_titulo_texto = 'Apresentação Indicadores PGA';
		...
		while($i < $qt)
		{
			...
			$arq = './up/apresentacao_pga/'.strtolower($this->session->userdata('usuario'))."_".$i.".png";
