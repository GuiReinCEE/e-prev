<?php
	...
	#require_once('/u/www/cieprev/sysapp/plugins/fpdf186/fpdf.php');
	#require_once('/u/www/cieprev/sysapp/plugins/fpdi212/srcFPDI/autoload.php');
	#require_once('/u/www/cieprev/sysapp/plugins/fpdi212/srcFPDIParser/autoload.php');
	#require_once('/u/www/cieprev/sysapp/plugins/fpdi212/srcFPDIProtection/autoload.php');		
...
class Controle_rds extends Controller
{
	...
	function rds_pdf($cd_controle_rds)
	{
		...
		{
			$ob_pdf = initializePdfProtection('P', 'mm', 'A4');
			...
			for($i = 1; $i <=  $nr_pag; $i++)
			{
				...
				$ob_pdf->addPage($ar_size['height'] > $ar_size['width'] ? 'P' : 'L', array($ar_size['width'], $ar_size['height']));
				$ob_pdf->useTemplate($idx);

				$x = ((35 * $ar_size['width']) / 100);
				$y = ((98 * $ar_size['height']) / 100);
