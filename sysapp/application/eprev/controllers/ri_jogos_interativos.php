<?php
class ri_jogos_interativos extends Controller
{
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RI_RESPONDENTES_JOGOS_INTERATIVOS);
		
		$this->load->helper(array('indicador'));
		
		CheckLogin();
		
		$this->load->model('indicador_plugin/ri_jogos_interativos_model');
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(), 'GRI'))
		{
	        ...
	        $this->load->view('indicador_plugin/ri_jogos_interativos/index',$data);
		}
    }
    ...
    function listar()
    {
		if(indicador_db::verificar_permissao(usuario_id(), 'GRI'))
        {
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->ri_jogos_interativos_model->listar($result, $args);
				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/ri_jogos_interativos/partial_result', $data);
			}
			else
			{
	...
	function cadastro($cd = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(), 'GRI'))
		{
			...
			if(intval($args['cd']) == 0)
			{
				$this->ri_jogos_interativos_model->carrega_novo($result, $args);
				$row = $result->row_array();
				
				$data['row'] = array(
					'cd_ri_jogos_interativos'  => $cd,
			...
			{
				$this->ri_jogos_interativos_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}
			
			$this->load->view('indicador_plugin/ri_jogos_interativos/cadastro', $data);
		}
	}
	...
	function salvar()
	{
		if(indicador_db::verificar_permissao( usuario_id(), 'GRI' ) )
		{
			...
			$args['cd_ri_jogos_interativos']  = intval($this->input->post('cd_ri_jogos_interativos', true));
			...
			$this->ri_jogos_interativos_model->salvar($result, $args);
			...
			{
				redirect("indicador_plugin/ri_jogos_interativos", "refresh");
			}
		}
	}
	...
	function excluir($cd)
	{
		if(indicador_db::verificar_permissao( usuario_id(), 'GRI' ) )
		{
			...
			$this->ri_jogos_interativos_model->excluir($result, $args);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(), 'GRI'))
		{
			...
			else
			{
				...
				$this->ri_jogos_interativos_model->fecha_indicador( $result, $args );
			}
		}

		redirect( 'indicador_plugin/ri_jogos_interativos' );
	}
}
?>