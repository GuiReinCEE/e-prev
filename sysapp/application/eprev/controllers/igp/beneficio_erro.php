...
{
    var	$label_0 = "Mês";
	var	$label_1 = "Benef Concedidos";
	var	$label_2 = "Benef c/ Erro";
	var	$label_3 = "% Incorreções";
    var	$label_4 = "Meta";
    var	$label_5 = "Benef Concedido Acumulado";
    var	$label_6 = "Benef c/Erro Acumulado";
    var	$label_7 = "% Incor Acum";
    var	$label_8 = "Peso";
    ...
    {
        parent::Controller();
        $this->enum_indicador = intval(enum_indicador::BENEFICIO);
    ...
    function index()
    {
		if(CheckLogin())
		{
			...
			$this->load->view('igp/beneficio_erro/index.php', $data);
		}
    }
	...
	function detalhe($cd = 0)
	{
		...
		if(($this->session->userdata('indic_12') == "*") or (indicador_db::verificar_permissao(usuario_id(),'GP')) or (gerencia_in(array('GC'))))
		{
			...
			{
				$this->load->model('igp/Beneficio_erro_model');
				$data['row'] = $this->Beneficio_erro_model->carregar(intval($cd));
	...
	function criar_indicador()
	{
		...
		if(($this->session->userdata('indic_12') == "*") or (indicador_db::verificar_permissao(usuario_id(),'GP')))
		{
			...
			foreach($collection as $item)
			{
				...
				$row['beneficio_incorreto']            = number_format($item['nr_erro'], 0,',', '.');
				$row['percentual_beneficio_incorreto'] = number_format($item['nr_erro_percentual'],2,',','.');
			...
			for($i = $nr_ini; $i < $nr_fim; $i++)
			{
				...
				$sql.=indicador_db::sql_inserir_celula( $INDICADOR_TABELA_CODIGO, 2, $linha, $indicador[$i]['beneficio_incorreto'], 'right');
				$sql.=indicador_db::sql_inserir_celula( $INDICADOR_TABELA_CODIGO, 3, $linha, $indicador[$i]['percentual_beneficio_incorreto'], 'right');
				...
				$media += floatval( app_decimal_para_db( $indicador[$i]['percentual_beneficio_incorreto']) );
				$contador_media++;
				
				if( strpos($indicador[$i]['mes_ano'],"/". $tabela[0]['nr_ano_referencia'] )>-1 )
				{
					$media_ano += floatval(app_decimal_para_db($indicador[$i]['percentual_beneficio_incorreto']));
					$contador_media_ano++;
				}
