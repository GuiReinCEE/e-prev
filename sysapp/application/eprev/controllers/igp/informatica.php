...
class informatica extends Controller
{
    ...
    {
        parent::Controller();
        $this->enum_indicador = intval(enum_indicador::INFORMATICA);
    ...
    function index()
    {
		if(CheckLogin())
		{
	        ...
	        $this->load->view('igp/informatica/index.php', $data);
		}
    }
	...
	{
		echo("Este indicador deve ser atualizado pelo indicador de área");
		exit;		
		...
		if(($this->session->userdata('indic_12') == "*") or (indicador_db::verificar_permissao(usuario_id(),'GGS')) or (gerencia_in(array('GC'))))
		{
			...
			if(intval($cd) == 0)
			{
				...
				$data['row']['cd_informatica'] = 0;
				$data['row']['dt_referencia']  = $ar_ant['mes_referencia'];
				$data['row']['nr_expediente']  = "";
				$data['row']['nr_bco_fora']    = "";
				$data['row']['nr_tempo_perc']  = "";
			...
			{
				$this->load->model('igp/Informatica_model');
				$data['row'] = $this->Informatica_model->carregar(intval($cd));
	...
	function criar_indicador()
	{
		...
		if(($this->session->userdata('indic_12') == "*") or (indicador_db::verificar_permissao(usuario_id(),'GGS')))
		{
			...
			foreach($collection as $item)
			{
				...
				$row['VALOR_1']     = number_format($item['nr_expediente'], 0,',', '.');
				$row['VALOR_2']     = number_format($item["nr_bco_fora"], 0,',', '.');
				$row['PERCENTUAL']  = number_format($item["nr_tempo_perc"], 2,',', '.');
				$row['ACUMULADO']   = number_format($item["nr_percentual_indisponivel"], 2,',', '.');
				...
				if(strpos($item['mes_referencia'],"12/") > -1)
				{
					...
					$hst_resultado = floatval($item["nr_percentual_indisponivel"]);
			...
			$sql.=indicador_db::sql_inserir_celula( $INDICADOR_TABELA_CODIGO, 6,0, utf8_encode($data['label_14']), 'background,center');
			...
			for($i = $nr_ini; $i < $nr_fim; $i++)
			{
				...
				$sql.=indicador_db::sql_inserir_celula( $INDICADOR_TABELA_CODIGO, 3, $linha, $indicador[$i]['PERCENTUAL'], 'center', 'S', 2, 'S' );
				$sql.=indicador_db::sql_inserir_celula( $INDICADOR_TABELA_CODIGO, 4, $linha, $indicador[$i]['ACUMULADO'], 'center', 'S', 2, 'S');
				$sql.=indicador_db::sql_inserir_celula( $INDICADOR_TABELA_CODIGO, 5, $linha, $indicador[$i]['META'], 'center', 'S', 2, 'S');
				$sql.=indicador_db::sql_inserir_celula( $INDICADOR_TABELA_CODIGO, 6, $linha, $indicador[$i]['MEDIA_MOVEL'], 'center', 'S', 2, 'S');
