...
class satisfacao_colab extends Controller
{
    ...
    function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::SATISFACAO_COLABORADOR);
    }
    ...
    function index()
    {
		if(CheckLogin())
		{
	        ...
	        $this->load->view('igp/satisfacao_colab/index.php', $data);
		}
    }
    ...
    function listar()
    {
		...
		#### BUSCA GRÁFICO DA ORIGEM NA GGS ####
		$data['grafico'] =  indicador_tabela_aberta(intval(enum_indicador::RH_SATISFACAO_COLABORADOR));
	...
	function detalhe($cd = 0)
	{
		...
		if(($this->session->userdata('indic_12') == "*") or (indicador_db::verificar_permissao(usuario_id(),'GGS')) or (gerencia_in(array('GC'))))
		{
			...
			if(intval($cd) == 0)
			{
				...
				$data['row']['cd_satisfacao_colab'] = 0;
				$data['row']['dt_referencia']      = $ar_ant['mes_referencia'];
				$data['row']['nr_satisfacao']      = 0;
			...
			{
				$this->load->model('igp/Satisfacao_colab_model');
				$data['row'] = $this->Satisfacao_colab_model->carregar(intval($cd));
	...
	{
		#### GRAFICO GERADO NA ORIGEM NA GGS ####
		CheckLogin();
		if(($this->session->userdata('indic_12') == "*") or (indicador_db::verificar_permissao(usuario_id(),'GGS')))
		{
			$tabela = indicador_tabela_aberta( intval($this->enum_indicador) );
