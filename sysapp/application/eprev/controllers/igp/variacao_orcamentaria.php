...
class variacao_orcamentaria extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Orçado";
	var	$label_2 = "Realizado";
	var	$label_3 = "% Realiz. Orçament. Mês";
	var	$label_4 = "Meta";
    var	$label_5 = "Orçado Acumulado";
    var	$label_6 = "Realizado Acumulado";
    var	$label_7 = "% Orc Acum";
    var	$label_8 = "Peso";
    ...
    {
        parent::Controller();
        $this->enum_indicador = intval(enum_indicador::REALIZACAO_ORCAMENTARIA);
    ...
    function index()
    {
		if(CheckLogin())
		{
	        ...
	        $this->load->view('igp/variacao_orcamentaria/index.php', $data);
		}
    }
	...
	function detalhe($cd = 0)
	{
		...
		if(($this->session->userdata('indic_12') == "*") or (indicador_db::verificar_permissao(usuario_id(),'GC')) OR (indicador_db::verificar_permissao(usuario_id(),'GFC')) or (gerencia_in(array('GC'))))
		{
			...
			if(intval($cd) == 0)
			{
				...
				$data['row']['cd_variacao_orcamentaria'] = 0;
				$data['row']['dt_referencia']            = $ar_ant['mes_referencia'];
				$data['row']['nr_orcado']                = "";
				$data['row']['nr_realizado']             = "";
			...
			{
				$this->load->model('igp/Variacao_orcamentaria_model');
				$data['row'] = $this->Variacao_orcamentaria_model->carregar(intval($cd));
			}			
	...
	function criar_indicador()
	{
		...
		if(($this->session->userdata('indic_12') == "*") or (indicador_db::verificar_permissao(usuario_id(),'GC')) OR (indicador_db::verificar_permissao(usuario_id(),'GFC')))
		{
			...
			foreach($collection as $item)
			{
				...
				$row['VALOR_1']         = number_format($item['nr_orcado'], 2,',', '.');
				$row['VALOR_2']         = number_format($item['nr_realizado'], 2,',', '.');
				$row['PERCENTUAL']      = number_format($item['nr_realizado_percentual'],2,',','.');
				$row['ACUMULADO']       = number_format($item['nr_realizado_percentual_acumulado'], 2,',','.');
				...
				if( strpos($item['mes_referencia'],"12/")>-1 )
				{
					...
					$hst_resultado = floatval($item["nr_realizado_percentual"]);					
			...
			$sql.=indicador_db::sql_inserir_celula( $INDICADOR_TABELA_CODIGO, 4,0, utf8_encode($data['label_11']), 'background,center');
			$sql.=indicador_db::sql_inserir_celula( $INDICADOR_TABELA_CODIGO, 5,0, utf8_encode($data['label_4']), 'background,center');
			$sql.=indicador_db::sql_inserir_celula( $INDICADOR_TABELA_CODIGO, 6,0, utf8_encode($data['label_13']), 'background,center');
	...
	function fechar_periodo()
	{
		...
		if(($this->session->userdata('indic_12') == "*") or (indicador_db::verificar_permissao(usuario_id(),'GC')) OR (indicador_db::verificar_permissao(usuario_id(),'GFC')))
		{
			...
			redirect('igp/variacao_orcamentaria');
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
