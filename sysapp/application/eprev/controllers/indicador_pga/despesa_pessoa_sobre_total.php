<?php
class despesa_pessoa_sobre_total extends Controller
{
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::PGA_DESPESA_PESSOAL_SOBRE_TOTAL);
		...
		$this->load->model('indicador_pga/despesa_pessoa_sobre_total_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
	        ...
	        $this->load->view('indicador_pga/despesa_pessoa_sobre_total/index',$data);
		}
        else
        {
    ...
    function listar()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			$this->despesa_pessoa_sobre_total_model->listar( $result, $args );
			$data['collection'] = $result->result_array();

			$this->load->view('indicador_pga/despesa_pessoa_sobre_total/index_result', $data);
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function cadastro($cd_despesa_pessoa_sobre_total = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$args['cd_despesa_pessoa_sobre_total'] = $cd_despesa_pessoa_sobre_total;

			if(intval($args['cd_despesa_pessoa_sobre_total']) == 0)
			{
				$this->despesa_pessoa_sobre_total_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_despesa_pessoa_sobre_total'] = $args['cd_despesa_pessoa_sobre_total'];
			...
			{
				$this->despesa_pessoa_sobre_total_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_pga/despesa_pessoa_sobre_total/cadastro', $data);
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			{
				$nr_resultado = (floatval($nr_valor_2) / floatval($nr_valor_1)) * 100;
			}
			
			$args['cd_despesa_pessoa_sobre_total'] = intval($this->input->post('cd_despesa_pessoa_sobre_total', true));
			...
			$args["nr_resultado"]                         = $nr_resultado;
			$args["nr_meta"]                              = app_decimal_para_db($this->input->post("nr_meta", true));
            $args["observacao"]                           = $this->input->post("observacao", true);
			$args["cd_usuario"]                           = $this->session->userdata('codigo');

			$this->despesa_pessoa_sobre_total_model->salvar($result, $args);
			$this->criar_indicador();
			
			redirect("indicador_pga/despesa_pessoa_sobre_total", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_despesa_pessoa_sobre_total)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$this->despesa_pessoa_sobre_total_model->excluir($result, $args);
			$this->criar_indicador();
	...
	function criar_indicador()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			foreach( $collection as $item )
			{
				...
				//if(true)
				{
					...
					if( intval($item['ano_referencia']) == intval($tabela[0]['nr_ano_referencia']) && $item['fl_media'] != 'S' )
					{
						...
						$ultimo_mes_r = 0;
						$ultima_meta = $nr_meta;
						$ultimo_mes_1_acum = $item['nr_valor_1_acum'];
						$ultimo_mes_2_acum = $item['nr_valor_2_acum'];
					}
					...
					$indicador[$linha][3] = $item['nr_valor_1_acum'];
					$indicador[$linha][4] = $item['nr_valor_2_acum'];
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				if(floatval($ultimo_mes_1) > 0)
				{
					$ultimo_mes_r = (floatval($ultimo_mes_2_acum) / floatval($ultimo_mes_1_acum)) * 100;
				}
				...
				$indicador[$linha][3] = app_decimal_para_php($ultimo_mes_1_acum);
				$indicador[$linha][4] = app_decimal_para_php($ultimo_mes_2_acum);
				$indicador[$linha][5] = app_decimal_para_php($ultimo_mes_r);
				$indicador[$linha][6] = app_decimal_para_php($ultima_meta);
				$indicador[$linha][7] = '';
				$indicador[$linha][8] = '';
			}
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			foreach( $collection as $item )
			{
				...
				{
					$contador_ano_atual++;
					$ultimo_mes_1   += $item['nr_valor_1'];
					$ultimo_mes_2   += $item['nr_valor_2'];
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				if(floatval($ultimo_mes_1) > 0)
				{
					$nr_resultado = (floatval($ultimo_mes_2) / floatval($ultimo_mes_1)) * 100;
				}

				$args['cd_despesa_pessoa_sobre_total'] = 0;
			...
			$this->despesa_pessoa_sobre_total_model->fechar_periodo($result, $args);
