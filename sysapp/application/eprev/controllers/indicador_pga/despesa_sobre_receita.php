<?php
class Despesa_sobre_receita extends Controller
{
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::PGA_DESPESA_SOBRE_RECEITA);
		...
		$this->load->model('indicador_pga/despesa_sobre_receita_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
	        ...
	        $this->load->view('indicador_pga/despesa_sobre_receita/index',$data);
		}
        else
        {
    ...
    function listar()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			$this->despesa_sobre_receita_model->listar( $result, $args );
			$data['collection'] = $result->result_array();

			$this->load->view('indicador_pga/despesa_sobre_receita/index_result', $data);
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function cadastro($cd_despesa_sobre_receita = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$args['cd_despesa_sobre_receita'] = $cd_despesa_sobre_receita;

			if(intval($args['cd_despesa_sobre_receita']) == 0)
			{
				$this->despesa_sobre_receita_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_despesa_sobre_receita'] = $args['cd_despesa_sobre_receita'];
			...
			{
				$this->despesa_sobre_receita_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_pga/despesa_sobre_receita/cadastro', $data);
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_despesa_sobre_receita'] = intval($this->input->post('cd_despesa_sobre_receita', true));
			...
			$this->despesa_sobre_receita_model->salvar($result, $args);
			$this->criar_indicador();
			
			redirect("indicador_pga/despesa_sobre_receita", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_despesa_sobre_receita)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$this->despesa_sobre_receita_model->excluir($result, $args);
			$this->criar_indicador();
	...
	function criar_indicador()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$indicador[$linha][5] = app_decimal_para_php(($ultimo_mes_1_acum / $ultimo_mes_2_acum));
			...
			$sql.=indicador_db::sql_inserir_grafico(
				...
				'5,5,0,0;6,6,0,0;8,8,0,0',
				"0,0,1,$linha_sem_media",
				"5,5,1,$linha_sem_media-barra;6,6,1,$linha_sem_media-linha;8,8,1,$linha_sem_media-linha",
				usuario_id(),
				$coluna_para_ocultar,
				1,
				2
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				$args['cd_despesa_sobre_receita'] = 0;
			...
			$this->despesa_sobre_receita_model->fechar_periodo($result, $args);
