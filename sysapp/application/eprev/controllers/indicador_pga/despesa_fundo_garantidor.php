<?php
class despesa_fundo_garantidor extends Controller
{
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::PGA_DESPESA_SOBRE_FUNDO_GARANTIDOR);
		...
		$this->load->model('indicador_pga/despesa_fundo_garantidor_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
	        ...
	        $this->load->view('indicador_pga/despesa_fundo_garantidor/index',$data);
		}
        else
        {
    ...
    function listar()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			$this->despesa_fundo_garantidor_model->listar( $result, $args );
			$data['collection'] = $result->result_array();

			$this->load->view('indicador_pga/despesa_fundo_garantidor/index_result', $data);
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function cadastro($cd_despesa_fundo_garantidor = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$args['cd_despesa_fundo_garantidor'] = $cd_despesa_fundo_garantidor;

			if(intval($args['cd_despesa_fundo_garantidor']) == 0)
			{
				$this->despesa_fundo_garantidor_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_despesa_fundo_garantidor'] = $args['cd_despesa_fundo_garantidor'];
			...
			{
				$this->despesa_fundo_garantidor_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_pga/despesa_fundo_garantidor/cadastro', $data);
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_despesa_fundo_garantidor'] = intval($this->input->post('cd_despesa_fundo_garantidor', true));
			...
			$this->despesa_fundo_garantidor_model->salvar($result, $args);
			$this->criar_indicador();
			
			redirect("indicador_pga/despesa_fundo_garantidor", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_despesa_fundo_garantidor)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$this->despesa_fundo_garantidor_model->excluir($result, $args);
			$this->criar_indicador();
	...
	function criar_indicador()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$indicador[$linha][3] = app_decimal_para_php($ultimo_mes_r);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$args['cd_despesa_fundo_garantidor'] = 0;
			...
			$this->despesa_fundo_garantidor_model->fechar_periodo($result, $args);
