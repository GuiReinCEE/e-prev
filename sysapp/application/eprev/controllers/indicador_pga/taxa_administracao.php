<?php
class Taxa_administracao extends Controller
{
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::PGA_TAXA_DE_ADMINISTRACAO);
		...
		$this->load->model('indicador_pga/taxa_administracao_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
	        ...
	        $this->load->view('indicador_pga/taxa_administracao/index',$data);
		}
        else
        {
    ...
    function listar()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			$this->taxa_administracao_model->listar( $result, $args );
			$data['collection'] = $result->result_array();

			$this->load->view('indicador_pga/taxa_administracao/index_result', $data);
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function cadastro($cd_taxa_administracao = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$args['cd_taxa_administracao'] = $cd_taxa_administracao;

			if(intval($args['cd_taxa_administracao']) == 0)
			{
				$this->taxa_administracao_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_taxa_administracao'] = $args['cd_taxa_administracao'];
			...
			{
				$this->taxa_administracao_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_pga/taxa_administracao/cadastro', $data);
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_taxa_administracao'] = intval($this->input->post('cd_taxa_administracao', true));
			...
			$this->taxa_administracao_model->salvar($result, $args);
			$this->criar_indicador();
			
			redirect("indicador_pga/taxa_administracao", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_taxa_administracao)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$this->taxa_administracao_model->excluir($result, $args);
			$this->criar_indicador();
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			$this->taxa_administracao_model->fechar_periodo($result, $args);
