<?php
class Taxa_administracao_consolidada extends Controller
{
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::PGA_TAXA_DE_ADMINISTRACAO_CONSOLIDADA);
		...
		$this->load->model('indicador_pga/taxa_administracao_consolidada_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
	        ...
	        $this->load->view('indicador_pga/taxa_administracao_consolidada/index',$data);
		}
        else
        {
    ...
    function listar()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			$this->taxa_administracao_consolidada_model->listar( $result, $args );
			$data['collection'] = $result->result_array();

			$this->load->view('indicador_pga/taxa_administracao_consolidada/index_result', $data);
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function cadastro($cd_taxa_administracao_consolidada = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$args['cd_taxa_administracao_consolidada'] = $cd_taxa_administracao_consolidada;

			if(intval($args['cd_taxa_administracao_consolidada']) == 0)
			{
				$this->taxa_administracao_consolidada_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_taxa_administracao_consolidada'] = $args['cd_taxa_administracao_consolidada'];
			...
			{
				$this->taxa_administracao_consolidada_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_pga/taxa_administracao_consolidada/cadastro', $data);
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_taxa_administracao_consolidada'] = intval($this->input->post('cd_taxa_administracao_consolidada', true));
			...
			$this->taxa_administracao_consolidada_model->salvar($result, $args);
			$this->criar_indicador();
			
			redirect("indicador_pga/taxa_administracao_consolidada", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_taxa_administracao_consolidada)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$this->taxa_administracao_consolidada_model->excluir($result, $args);
			$this->criar_indicador();
	...
	function criar_indicador()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$indicador[$linha][3] = app_decimal_para_php(($ultimo_mes_1 / $ultimo_mes_2) * 100);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				$args['cd_taxa_administracao_consolidada'] = 0;
			...
			$this->taxa_administracao_consolidada_model->fechar_periodo($result, $args);
