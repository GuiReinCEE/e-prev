<?php
class Evolucao_fundo_adm extends Controller
{
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::PGA_FUNDO_ADMINISTRATIVO);
		...
		$this->load->model('indicador_pga/evolucao_fundo_adm_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
	        ...
	        $this->load->view('indicador_pga/evolucao_fundo_adm/index',$data);
		}
        else
        {
    ...
    function listar()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			$this->evolucao_fundo_adm_model->listar( $result, $args );
			$data['collection'] = $result->result_array();

			$this->load->view('indicador_pga/evolucao_fundo_adm/index_result', $data);
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function cadastro($cd_evolucao_fundo_adm = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$args['cd_evolucao_fundo_adm'] = $cd_evolucao_fundo_adm;

			if(intval($args['cd_evolucao_fundo_adm']) == 0)
			{
				$this->evolucao_fundo_adm_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_evolucao_fundo_adm'] = $args['cd_evolucao_fundo_adm'];
			...
			{
				$this->evolucao_fundo_adm_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_pga/evolucao_fundo_adm/cadastro', $data);
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_evolucao_fundo_adm'] = intval($this->input->post('cd_evolucao_fundo_adm', true));
			...
			$this->evolucao_fundo_adm_model->salvar($result, $args);
			$this->criar_indicador();
			
			redirect("indicador_pga/evolucao_fundo_adm", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_evolucao_fundo_adm)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$this->evolucao_fundo_adm_model->excluir($result, $args);
			$this->criar_indicador();
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			$this->evolucao_fundo_adm_model->fechar_periodo($result, $args);
