<?php
class despesa_patrimonio_participante extends Controller
{
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::PGA_DESPESA_PATRIMONIO_PARTICIPANTE);
		...
		$this->load->model('indicador_pga/despesa_patrimonio_participante_model');
    }
    ...
    public function index()
    {
		if(indicador_db::verificar_permissao($this->cd_usuario,'GC') OR indicador_db::verificar_permissao($this->cd_usuario,'GFC'))
		{
	        ...
	        $this->load->view('indicador_pga/despesa_patrimonio_participante/index',$data);
		}
        else
        {
    ...
    public function listar()
    {
		if(indicador_db::verificar_permissao($this->cd_usuario,'GC') OR indicador_db::verificar_permissao($this->cd_usuario,'GFC'))
		{
			...
			$data['collection'] = $this->despesa_patrimonio_participante_model->listar($data['tabela'][0]['cd_indicador_tabela']);

			$this->load->view('indicador_pga/despesa_patrimonio_participante/index_result', $data);
		}
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function cadastro($cd_despesa_patrimonio_participante = 0)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'GC') OR indicador_db::verificar_permissao($this->cd_usuario,'GFC'))
		{
			$data['label_0'] = $this->label_0;
			...
			if(intval($cd_despesa_patrimonio_participante) == 0)
			{
				$row = $this->despesa_patrimonio_participante_model->carrega_referencia($data['tabela'][0]['cd_indicador_tabela']);
				
				$data['row'] = array(
					'cd_despesa_patrimonio_participante' => intval($cd_despesa_patrimonio_participante),
			...
			{
				$data['row'] = $this->despesa_patrimonio_participante_model->carrega(intval($cd_despesa_patrimonio_participante));
			}

			$this->load->view('indicador_pga/despesa_patrimonio_participante/cadastro', $data);
		}
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function salvar()
	{
		...
		{		
			...
			{
				#=K67/(L67*M67)
				#Despesa acumulada 12 meses/(Quantidade de participantes x Ativo dividido por milhão)
				$nr_resultado = floatval($nr_valor_1) / (floatval($nr_valor_2) * floatval($nr_valor_3));
			}

			$args = array(
				'cd_despesa_patrimonio_participante' => intval($this->input->post('cd_despesa_patrimonio_participante', true)),
			...
			$this->despesa_patrimonio_participante_model->salvar($args);

			$this->criar_indicador();
			
			redirect('indicador_pga/despesa_patrimonio_participante', 'refresh');
		}
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function criar_indicador()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'GC') OR indicador_db::verificar_permissao($this->cd_usuario,'GFC'))
		{
			...
			$collection = $this->despesa_patrimonio_participante_model->listar($tabela[0]['cd_indicador_tabela']);
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$indicador[$linha][4] = app_decimal_para_php($nr_resultado);
			...
			$sql.=indicador_db::sql_inserir_grafico(
				...
				"4,4,1,$linha_sem_media-linha;5,5,1,$linha_sem_media-linha",
				$this->cd_usuario,
				$coluna_para_ocultar,
				1,
				3
	...
	public function excluir($cd_despesa_patrimonio_participante)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'GC') OR indicador_db::verificar_permissao($this->cd_usuario,'GFC'))
		{
			$this->despesa_patrimonio_participante_model->excluir($cd_despesa_patrimonio_participante, $this->cd_usuario);
	...
	public function fechar_periodo()
	{
		...
		{			
			...
			{
				$args = array(
					'cd_despesa_patrimonio_participante' => 0,
			...
			$this->despesa_patrimonio_participante_model->fechar_periodo($tabela[0]['cd_indicador_tabela'], $this->cd_usuario);
