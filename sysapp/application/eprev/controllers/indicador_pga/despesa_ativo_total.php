<?php
class Despesa_ativo_total extends Controller
{
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::PGA_DESPESA_SOBRE_O_ATIVO_TOTAL);
		...
		$this->load->model('indicador_pga/despesa_ativo_total_model');
    }
    ...
    public function index()
    {
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GC') OR indicador_db::verificar_permissao($this->cd_usuario,'GFC'))
		{
	        ...
	        $this->load->view('indicador_pga/despesa_ativo_total/index',$data);
		}
        else
        {
    ...
    public function listar()
    {
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GC') OR indicador_db::verificar_permissao($this->cd_usuario,'GFC'))
		{
			...
			$data['collection'] = $this->despesa_ativo_total_model->listar($data['tabela'][0]['cd_indicador_tabela']);

			$this->load->view('indicador_pga/despesa_ativo_total/index_result', $data);
		}
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function cadastro($cd_despesa_ativo_total = 0)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'GC') OR indicador_db::verificar_permissao($this->cd_usuario,'GFC'))
		{ 
			$data['label_0'] = $this->label_0;
			...
			if(intval($cd_despesa_ativo_total) == 0)
			{
				$row = $this->despesa_ativo_total_model->carrega_referencia($data['tabela'][0]['cd_indicador_tabela']);

				$data['row'] = array(
					'cd_despesa_ativo_total' => intval($cd_despesa_ativo_total),
			...
			{
				$data['row'] = $this->despesa_ativo_total_model->carrega(intval($cd_despesa_ativo_total));
			}

			$this->load->view('indicador_pga/despesa_ativo_total/cadastro', $data);
		}
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function salvar()
	{
		...
		{		
			...
			{
				$nr_resultado = (floatval($nr_valor_1) / floatval($nr_valor_2)) * 100;
			}

			$args = array(
				'cd_despesa_ativo_total' => intval($this->input->post('cd_despesa_ativo_total', true)),
			...
			$this->despesa_ativo_total_model->salvar($args);

			$this->criar_indicador();
			
			redirect('indicador_pga/despesa_ativo_total', 'refresh');
		}
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function excluir($cd_despesa_ativo_total)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'GC') OR indicador_db::verificar_permissao($this->cd_usuario,'GFC'))
		{
			$this->despesa_ativo_total_model->excluir($cd_despesa_ativo_total, $this->cd_usuario);
	...
	public function criar_indicador()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'GC') OR indicador_db::verificar_permissao($this->cd_usuario,'GFC'))
		{
			...
			$collection = $this->despesa_ativo_total_model->listar($tabela[0]['cd_indicador_tabela']);
	...
	public function fechar_periodo()
	{
		...
		{			
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$args = array(
					'cd_despesa_ativo_total' => 0,
					'dt_referencia'          => '01/01/'.intval($tabela[0]['nr_ano_referencia']),
					'nr_valor_1'             => $nr_valor_1_total,
					'nr_valor_2'             => $nr_valor_2_total,
					'nr_resultado'           => $nr_resultado,
					'nr_meta'                => $nr_meta ,
			...
			$this->despesa_ativo_total_model->fechar_periodo($tabela[0]['cd_indicador_tabela'], $this->cd_usuario);
