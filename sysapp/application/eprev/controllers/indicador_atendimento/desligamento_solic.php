<?php
class desligamento_solic extends Controller
{
	var	$label_0 = "Mês";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::AREA_DESLIGAMENTO_SOLIC);
    ...
    function index()
    {
		if($this->fl_permissao)
		{
	        ...
	        $this->load->view('indicador_atendimento/desligamento_solic/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if($this->fl_permissao)
        {
	        $this->load->model( 'indicador_atendimento/desligamento_solic_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->desligamento_solic_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_atendimento/desligamento_solic/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->load->model('indicador_atendimento/desligamento_solic_model');
			$row=$this->desligamento_solic_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
			...
			$this->load->view('indicador_atendimento/desligamento_solic/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		if($this->fl_permissao)
		{
			...
			$args['cd_desligamento_solic']  = intval($this->input->post('cd_desligamento_solic', true));
			...
			$msg=array();
			$retorno = $this->desligamento_solic_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_atendimento/desligamento_solic", "refresh" );
				}
			}
			else
			{
	...
	function excluir($id)
	{
		if($this->fl_permissao)
		{
			...
			$this->desligamento_solic_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		redirect( 'indicador_atendimento/desligamento_solic' );
