<?php
class tele_nao_atendidas extends Controller
{
	var	$label_0 = "Mês";
	var	$label_1 = "Lig. Transf. Não Atendidas";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::AREA_TELE_TRANSF_NAO_ATEND);
    ...
    function index()
    {
		if($this->fl_permissao)
		{
	        ...
	        $this->load->view('indicador_atendimento/tele_nao_atendidas/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if($this->fl_permissao)
        {
	        $this->load->model( 'indicador_atendimento/tele_nao_atendidas_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->tele_nao_atendidas_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_atendimento/tele_nao_atendidas/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->load->model('indicador_atendimento/tele_nao_atendidas_model');
			$row=$this->tele_nao_atendidas_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
			...
			$this->load->view('indicador_atendimento/tele_nao_atendidas/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		if($this->fl_permissao)
		{
			...
			$args['cd_tele_nao_atendidas']  = intval($this->input->post('cd_tele_nao_atendidas', true));
			...
			$msg=array();
			$retorno = $this->tele_nao_atendidas_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_atendimento/tele_nao_atendidas", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		if($this->fl_permissao)
		{
			...
			$this->tele_nao_atendidas_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		redirect( 'indicador_atendimento/tele_nao_atendidas' );
