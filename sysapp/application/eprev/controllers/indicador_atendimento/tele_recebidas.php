<?php
class tele_recebidas extends Controller
{
	var	$label_0 = "Mês";
	var	$label_1 = "Lig. Recebidas";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::AREA_TELE_RECEBIDAS);
    ...
    function index()
    {
		if($this->fl_permissao)
		{
	        ...
	        $this->load->view('indicador_atendimento/tele_recebidas/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if($this->fl_permissao)
        {
	        $this->load->model( 'indicador_atendimento/tele_recebidas_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->tele_recebidas_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_atendimento/tele_recebidas/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->load->model('indicador_atendimento/tele_recebidas_model');
			$row=$this->tele_recebidas_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					$sql = "
                        SELECT TO_CHAR(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS mes_referencia,
                               ...
                               nr_meta,
                               cd_indicador_tabela
					      FROM indicador_atendimento.tele_recebidas
			...
			$this->load->view('indicador_atendimento/tele_recebidas/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		if($this->fl_permissao)
		{
			...
			$args['cd_tele_recebidas']  = intval($this->input->post('cd_tele_recebidas', true));
			...
			$msg=array();
			$retorno = $this->tele_recebidas_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_atendimento/tele_recebidas", "refresh" );
				}
			}
			else
			{
	...
	function excluir($id)
	{
		if($this->fl_permissao)
		{
			...
			$this->tele_recebidas_model->excluir($id);
	...
	function fechar_periodo()
	{
		if($this->fl_permissao)
		{
			...
			else
			{
				...
				if(sizeof($media_ano)>0)
				{
					...
					*/
					$sql.=sprintf("
                        INSERT INTO indicador_atendimento.tele_recebidas
					              (
                                   dt_referencia,
                                   dt_inclusao,
                                   cd_usuario_inclusao,
		...
		redirect( 'indicador_atendimento/tele_recebidas' );
