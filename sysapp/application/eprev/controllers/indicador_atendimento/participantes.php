...
class participantes extends Controller
{
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::AREA_PARTICIPANTES);
    ...
    function index()
    {
		if($this->fl_permissao)
		{
	        ...
	        $this->load->view('indicador_atendimento/participantes/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if($this->fl_permissao)
        {
	        $this->load->model( 'indicador_atendimento/participantes_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->participantes_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_atendimento/participantes/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->load->model('indicador_atendimento/participantes_model');
			$row=$this->participantes_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					$sql = " 
                        SELECT TO_CHAR(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS mes_referencia,
                               ...
                               nr_meta,
                               cd_indicador_tabela
					      FROM indicador_atendimento.participantes
			...
			$this->load->view('indicador_atendimento/participantes/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		if($this->fl_permissao)
		{
			...
			$args['cd_participantes']  = intval($this->input->post('cd_participantes', true));
			...
			$msg=array();
			$retorno = $this->participantes_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_atendimento/participantes", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		if($this->fl_permissao)
		{
			...
			$this->participantes_model->excluir($id);
	...
	function fechar_periodo()
	{
		if($this->fl_permissao)
		{
			...
			else
			{
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf("
                        INSERT INTO indicador_atendimento.participantes
					              (
                                   dt_referencia,
                                   dt_inclusao,
                                   cd_usuario_inclusao,
		...
		redirect( 'indicador_atendimento/participantes' );
