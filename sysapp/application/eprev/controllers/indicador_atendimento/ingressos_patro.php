<?php
class ingressos_patro extends Controller
{
	var	$label_0 = "Mês";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::AREA_INGRESSOS_PATROCINADORAS);
    ...
    function index()
    {
		if($this->fl_permissao)
		{
	        ...
	        $this->load->view('indicador_atendimento/ingressos_patro/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if($this->fl_permissao)
        {
	        $this->load->model( 'indicador_atendimento/ingressos_patro_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->ingressos_patro_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_atendimento/ingressos_patro/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->load->model('indicador_atendimento/ingressos_patro_model');
			$row=$this->ingressos_patro_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					$sql = " 
                        SELECT TO_CHAR(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS mes_referencia,
                               ...
                               nr_meta,
                               cd_indicador_tabela
					      FROM indicador_atendimento.ingressos_patro
			...
			$this->load->view('indicador_atendimento/ingressos_patro/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		if($this->fl_permissao)
		{
			...
			$args['cd_ingressos_patro']  = intval($this->input->post('cd_ingressos_patro', true));
			...
			$msg=array();
			$retorno = $this->ingressos_patro_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_atendimento/ingressos_patro", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		if($this->fl_permissao)
		{
			...
			$this->ingressos_patro_model->excluir($id);
	...
	function fechar_periodo()
	{
             ...
             //   $soma_vl_4  = 0;
               ...
               //     $soma_vl_4  += $nr_valor_4;
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf("
                        INSERT INTO indicador_atendimento.ingressos_patro
					              (
                                   dt_referencia,
                                   dt_inclusao,
                                   cd_usuario_inclusao,
		...
		redirect( 'indicador_atendimento/ingressos_patro' );
