<?php
class tele_call_center extends Controller
{
	var	$label_0 = "Mês";
	var	$label_1 = "Tp. Espera Call Center";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::AREA_TELE_CALL_CENTER);
    ...
    function index()
    {
		if($this->fl_permissao)
		{
	        ...
	        $this->load->view('indicador_atendimento/tele_call_center/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if($this->fl_permissao)
        {
	        $this->load->model( 'indicador_atendimento/tele_call_center_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->tele_call_center_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_atendimento/tele_call_center/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->load->model('indicador_atendimento/tele_call_center_model');
			$row=$this->tele_call_center_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
			...
			$this->load->view('indicador_atendimento/tele_call_center/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		if($this->fl_permissao)
		{
			...
			$args['cd_tele_call_center']  = intval($this->input->post('cd_tele_call_center', true));
			...
			$msg=array();
			$retorno = $this->tele_call_center_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_atendimento/tele_call_center", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		if($this->fl_permissao)
		{
			...
			$this->tele_call_center_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		redirect( 'indicador_atendimento/tele_call_center' );
