<?php
class tele_atend_email extends Controller
{
	var	$label_0 = "Mês";
	var	$label_1 = "% Atend. por Email";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::AREA_ATEND_EMAIL);
    ...
    function index()
    {
		if($this->fl_permissao)
		{
	        ...
	        $this->load->view('indicador_atendimento/tele_atend_email/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if($this->fl_permissao)
        {
	        $this->load->model( 'indicador_atendimento/tele_atend_email_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->tele_atend_email_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_atendimento/tele_atend_email/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->load->model('indicador_atendimento/tele_atend_email_model');
			$row=$this->tele_atend_email_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
			...
			$this->load->view('indicador_atendimento/tele_atend_email/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		if($this->fl_permissao)
		{
			...
			$args['cd_tele_atend_email']  = intval($this->input->post('cd_tele_atend_email', true));
			...
			$msg=array();
			$retorno = $this->tele_atend_email_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_atendimento/tele_atend_email", "refresh" );
				}
			}
			else
			{
	...
	function excluir($id)
	{
		if($this->fl_permissao)
		{
			...
			$this->tele_atend_email_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		redirect( 'indicador_atendimento/tele_atend_email' );
