<?php
class desligamento_obito extends Controller
{
	var	$label_0 = "Mês";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::AREA_DESLIGAMENTO_OBITO);
    ...
    function index()
    {
		if($this->fl_permissao)
		{
	        ...
	        $this->load->view('indicador_atendimento/desligamento_obito/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if($this->fl_permissao)
        {
	        $this->load->model( 'indicador_atendimento/desligamento_obito_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->desligamento_obito_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_atendimento/desligamento_obito/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->load->model('indicador_atendimento/desligamento_obito_model');
			$row=$this->desligamento_obito_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					$sql = "
                        SELECT TO_CHAR(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS mes_referencia,
                               ...
                               nr_meta,
                               cd_indicador_tabela
					      FROM indicador_atendimento.desligamento_obito
			...
			$this->load->view('indicador_atendimento/desligamento_obito/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		if($this->fl_permissao)
		{
			...
			$args['cd_desligamento_obito']  = intval($this->input->post('cd_desligamento_obito', true));
			...
			$msg=array();
			$retorno = $this->desligamento_obito_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_atendimento/desligamento_obito", "refresh" );
				}
			}
			else
			{
	...
	function excluir($id)
	{
		if($this->fl_permissao)
		{
			...
			$this->desligamento_obito_model->excluir($id);
	...
	function fechar_periodo()
	{
		if($this->fl_permissao)
		{
			...
			else
			{
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf("
                        INSERT INTO indicador_atendimento.desligamento_obito
					              (
                                   dt_referencia,
                                   dt_inclusao,
                                   cd_usuario_inclusao,
		...
		redirect( 'indicador_atendimento/desligamento_obito' );
