<?php
class central_tp_atendimento extends Controller
{
	var	$label_0 = "Mês";
	var	$label_1 = "Tempo Médio de Atendimento";
	var	$label_2 = "Observação";
    var $label_3 = "Tendência";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::AREA_CENTRAL_TP_ATENDIMENTO);
    ...
    function index()
    {
		if($this->fl_permissao)
		{
	        ...
	        $this->load->view('indicador_atendimento/central_tp_atendimento/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if($this->fl_permissao)
        {
	        $this->load->model( 'indicador_atendimento/central_tp_atendimento_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->central_tp_atendimento_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_atendimento/central_tp_atendimento/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->load->model('indicador_atendimento/central_tp_atendimento_model');
			$row=$this->central_tp_atendimento_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					$sql = " 
                        SELECT TO_CHAR(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS mes_referencia,
                               ...
                               nr_meta,
                               cd_indicador_tabela
					      FROM indicador_atendimento.central_tp_atendimento
			...
			$this->load->view('indicador_atendimento/central_tp_atendimento/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		if($this->fl_permissao)
		{
			...
			$args['cd_central_tp_atendimento']  = intval($this->input->post('cd_central_tp_atendimento', true));
			...
			$msg=array();
			$retorno = $this->central_tp_atendimento_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_atendimento/central_tp_atendimento", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		if($this->fl_permissao)
		{
			...
			$this->central_tp_atendimento_model->excluir($id);
	...
	function fechar_periodo()
	{
		if($this->fl_permissao)
		{
			...
			else
			{
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf("
                        INSERT INTO indicador_atendimento.central_tp_atendimento
					              (
                                   dt_referencia,
                                   dt_inclusao,
                                   cd_usuario_inclusao,
		...
		redirect( 'indicador_atendimento/central_tp_atendimento' );
