...
class Controle_carro extends Controller
{
    ...
    {
        if(gerencia_in(array('GCM','GN','GNR','GFC')))
        {
            return TRUE;
        }
        else
    ...
    {		
		...
		if (in_array($this->session->userdata('divisao'), array('GCM','GS','GNR')))
		{
			$cd_controle_carro_veiculo = 6;
		}
		...
		{
			$cd_controle_carro_veiculo = 0;
		}		

		$data['collection'] = $this->controle_carro_model->listar($cd_controle_carro_veiculo, $args);
	...
	public function cadastro($cd_controle_carro = 0)
	{
		if($this->get_permissao())
		{
			...
			if(intval($cd_controle_carro) == 0)
			{
				...
				if (in_array($this->session->userdata('divisao'), array('GCM','GN','GNR')))
				{
					$cd_controle_carro_veiculo = 6;
				}
				else if($this->session->userdata('divisao') == 'GFC')
