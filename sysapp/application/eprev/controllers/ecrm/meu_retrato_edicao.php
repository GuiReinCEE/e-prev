...
class Meu_retrato_edicao extends Controller
{
    ...
    public function setGerar()
    {
		#### GERAR EM LOTE A PARTIR DA TELA DE LISTA ####
		...
		if(gerencia_in(array('GS')))
		{
			...
			$args = array(
				...
				'tp_participante' => $this->input->post('tp_participante', TRUE),
				'cd_gerencia'     => ($this->session->userdata('codigo') == 1 ? 'GTI' : $this->session->userdata('divisao')),
				'edicao_selecionada' => $this->input->post('edicao_selecionada', TRUE)
			); 
			
			$ar_edicao_selecionada = explode(",",$args['edicao_selecionada']);
			
			foreach($ar_edicao_selecionada as $cd_edicao)
			{
			...
			#echo "<PRE>".print_r($args,true).print_r($ar_edicao_selecionada,true); exit;
