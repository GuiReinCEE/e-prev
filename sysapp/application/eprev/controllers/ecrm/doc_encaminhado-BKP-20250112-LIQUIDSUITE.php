    ...
    public function enviar_documento_liquid()
    {
        if($this->permissao())
        {
            ...
            foreach ($doc_encaminhado_arquivo as $key => $item) 
            {
                ...
                $args['senha']         = 'c8ml09';
                $args['nomeDocumento'] = utf8_encode($nome_arquivo.'.'.$extensao['extension']);
                $args['pasta']         = utf8_encode($pasta);
                $args['codigoFicha']   = $codigo_ficha;
                $args['camposFicha']   = utf8_encode($campos_ficha);
                $args['nomeArquivo']   = utf8_encode($nome_arquivo.'.'.$extensao['extension']);
                $args['arquivoBase64'] = base64_encode(file_get_contents($dir.$documento['ds_documento']));
