<?php
class originais_retificacoes_dctf extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::AREA_ORIGINAIS_RETIFICACOES_DCTF);
		...
		$this->load->model('indicador_controladoria/originais_retificacoes_dctf_model' );
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
	        ...
	        $this->load->view('indicador_controladoria/originais_retificacoes_dctf/index',$data);
		}
    }
    ...
    function listar()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
        {
			...
			$this->originais_retificacoes_dctf_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_controladoria/originais_retificacoes_dctf/index_result', $data);
        }
    }
    
    function cadastro($cd_originais_retificacoes_dctf = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$args['cd_originais_retificacoes_dctf'] = $cd_originais_retificacoes_dctf;
			
			if(intval($args['cd_originais_retificacoes_dctf']) == 0)
			{
				$this->originais_retificacoes_dctf_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_originais_retificacoes_dctf'] = $args['cd_originais_retificacoes_dctf'];
			...
			{
				$this->originais_retificacoes_dctf_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_controladoria/originais_retificacoes_dctf/cadastro', $data);
		}
	}
    ...
    function salvar()
	{
		...
		{		
			...
			$args['cd_originais_retificacoes_dctf'] = intval($this->input->post('cd_originais_retificacoes_dctf', true));
			...
			$this->originais_retificacoes_dctf_model->salvar($result, $args);
			$this->criar_indicador();
			
			redirect("indicador_controladoria/originais_retificacoes_dctf", "refresh");
		}
        else
        {
    ...
    function excluir($cd_originais_retificacoes_dctf)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC'))
		{
			$args   = array();
			...
			$this->originais_retificacoes_dctf_model->excluir($result, $args);
			$this->criar_indicador();
    ...
    function criar_indicador()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC') OR indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
            ...
            foreach($collection as $item)
            {
                if((intval($item['ano_referencia']) >= intval($tabela[0]['nr_ano_referencia']) - intval($tabela[0]['qt_periodo_anterior'])) OR ($fl_perido))
				{
                    ...
                    if(trim($item['fl_media']) == 'N')
                    {
                        ...
                        $nr_retificacoes_total         += $item["nr_retificacao_1"] + $item["nr_retificacao_2"] + $item["nr_retificacao_3"] + $item["nr_retificacao_4"] + $item["nr_retificacao_5"];
                        $nr_declaracoes_entregue_total += $item["nr_declaracoes_entregue"];
                        $nr_meta_ano                    = $item["nr_meta"];
                        $observacoes                   .= (trim($item["observacao"]) != ''? $item["observacao"]."<br/>" : "");
                    }
                    else if(trim($item['fl_media']) == 'S')
                    {                           
            ...
            if(intval($contador_ano_atual) > 0)
            {
                ...
                $indicador[$linha][1] = intval($nr_declaracoes_entregue_total);
                $indicador[$linha][2] = intval($nr_original_total);
                $indicador[$linha][3] = intval($nr_retificacoes_total);
                $indicador[$linha][4] = intval($nr_meta_ano);			
                $indicador[$linha][5] = nl2br($observacoes);

                $ar_tendencia[] = $nr_declaracoes_entregue_total;
            }
    ...
    function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC'))
		{
            ...
            if(intval($contador_ano_atual) > 0)
			{
                $args["cd_originais_retificacoes_dctf"] = 0;
            ...
            $this->originais_retificacoes_dctf_model->fechar_periodo($result, $args);
        }
