<?php
class participantes_patro extends Controller
{
	var	$label_0 = "Mês";
	var	$label_1 = "Participantes";#"Participantes (Somente Patrocinadoras)";
	var	$label_2 = "";
	var	$label_3 = "";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::PODER_PARTICIPANTES_PATRO);
    ...
    function index()
    {
		...
		if($this->fl_permissao)
		{
	        ...
	        $this->load->view('indicador_poder/participantes_patro/index.php',$data);
		}
		else
		{
    ...
    function listar()
    {
		...
		if($this->fl_permissao)
        {
	        $this->load->model( 'indicador_poder/participantes_patro_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->participantes_patro_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_poder/participantes_patro/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->load->model('indicador_poder/participantes_patro_model');
			$row=$this->participantes_patro_model->carregar( $cd );
				...
				if($cd==0)
				{
					$sql = "
                        SELECT TO_CHAR(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS mes_referencia,
                               ...
                               nr_meta,
                               cd_indicador_tabela
                          FROM indicador_poder.participantes_patro
			...
			$this->load->view('indicador_poder/participantes_patro/detalhe', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		if($this->fl_permissao)
		{
			...
			$args['cd_participantes_patro'] = intval($this->input->post('cd_participantes_patro', true));
			...
			$msg=array();
			$retorno = $this->participantes_patro_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_poder/participantes_patro", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->participantes_patro_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if($this->fl_permissao)
		{
			...
			redirect( 'indicador_poder/participantes_patro' );
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
