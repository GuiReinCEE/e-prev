<?php
class equilibrio_atu_plano extends Controller
{
	var	$label_0 = "Mês";
	var	$label_1 = "Reservas Técnicas";
	var	$label_2 = "Provisões Matemáticas";
	var	$label_3 = "Relação Percentual";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::PODER_EQUILIBRIO_ATUARIAL_PLANO);
    ...
    function index()
    {
		...
		if($this->fl_permissao)
		{
	        ...
	        $this->load->view('indicador_poder/equilibrio_atu_plano/index.php',$data);
		}
		else
		{
    ...
    function listar()
    {
		...
		if($this->fl_permissao)
        {
	        $this->load->model( 'indicador_poder/equilibrio_atu_plano_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->equilibrio_atu_plano_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_poder/equilibrio_atu_plano/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->load->model('indicador_poder/equilibrio_atu_plano_model');
			$row=$this->equilibrio_atu_plano_model->carregar( $cd );
				...
				if($cd==0)
				{
					$sql = "
                        SELECT TO_CHAR(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS mes_referencia,
                               ...
                               nr_meta,
                               cd_indicador_tabela
                          FROM indicador_poder.equilibrio_atu_plano
			...
			$this->load->view('indicador_poder/equilibrio_atu_plano/detalhe', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		if($this->fl_permissao)
		{
			...
			$args['cd_equilibrio_atu_plano'] = intval($this->input->post('cd_equilibrio_atu_plano', true));
            ...
            {
                $args['nr_percentual_f'] = (( floatval($args["nr_valor_1"])/floatval($args["nr_valor_2"]) )-1)*100;
            }
            else
            {
			...
			$msg=array();
			$retorno = $this->equilibrio_atu_plano_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_poder/equilibrio_atu_plano", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->equilibrio_atu_plano_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if($this->fl_permissao)
		{
			...
			redirect( 'indicador_poder/equilibrio_atu_plano' );
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
