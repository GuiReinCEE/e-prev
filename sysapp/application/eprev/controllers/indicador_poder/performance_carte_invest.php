<?php
class performance_carte_invest extends Controller
{
	var	$label_0 = "Mês";
	var	$label_1 = "Rentab. Mês";
	var	$label_2 = "Benchmark Mês";
	var	$label_3 = "Result. Acum. 12 meses";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::PODER_PERFOM_CARTEIRA_INVESTIMENTO);
		...
		elseif (gerencia_in(array('GC', 'GFC', 'GIN')))
		{
			$this->fl_permissao = TRUE;
		}			
    ...
    function index()
    {
		...
		if($this->fl_permissao)
		{
	        ...
	        $this->load->view('indicador_poder/performance_carte_invest/index.php',$data);
		}
		else
		{
    ...
    function listar()
    {
		...
		if($this->fl_permissao)
        {
	        $this->load->model( 'indicador_poder/performance_carte_invest_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->performance_carte_invest_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_poder/performance_carte_invest/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->load->model('indicador_poder/performance_carte_invest_model');
			$row=$this->performance_carte_invest_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					$sql = "
                        SELECT TO_CHAR(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS mes_referencia,
                               ...
                               nr_meta,
                               cd_indicador_tabela
					      FROM indicador_poder.performance_carte_invest
			...
			$this->load->view('indicador_poder/performance_carte_invest/detalhe', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		if($this->fl_permissao)
		{
			...
			$args['cd_performance_carte_invest'] = intval($this->input->post('cd_performance_carte_invest', true));
			...
			$msg=array();
			$retorno = $this->performance_carte_invest_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_poder/performance_carte_invest", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->performance_carte_invest_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if($this->fl_permissao)
		{
			...
			redirect( 'indicador_poder/performance_carte_invest' );
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
