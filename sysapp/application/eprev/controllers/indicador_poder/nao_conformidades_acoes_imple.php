<?php
class nao_conformidades_acoes_imple extends Controller
{
	var	$label_0 = "Mês";
	var	$label_1 = "Impl. fora do prazo";
	var	$label_2 = "Prevista p/ o período";
	var	$label_3 = "% impl. fora do prazo";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::PODER_NAO_CONFORMIDADES_ACOES_IMPLEMENTADAS);
    ...
    function index()
    {
		...
		if($this->fl_permissao)
		{
	        ...
	        $this->load->view('indicador_poder/nao_conformidades_acoes_imple/index.php',$data);
		}
		else
		{
    ...
    function listar()
    {
		...
		if($this->fl_permissao)
        {
	        $this->load->model( 'indicador_poder/nao_conformidades_acoes_imple_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->nao_conformidades_acoes_imple_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_poder/nao_conformidades_acoes_imple/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->load->model('indicador_poder/nao_conformidades_acoes_imple_model');
			$row=$this->nao_conformidades_acoes_imple_model->carregar( $cd );
			...
			$this->load->view('indicador_poder/nao_conformidades_acoes_imple/detalhe', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		if($this->fl_permissao)
		{
			...
			$args['cd_nao_conformidades_acoes_imple'] = intval($this->input->post('cd_nao_conformidades_acoes_imple', true));
			...
			$msg=array();
			$retorno = $this->nao_conformidades_acoes_imple_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_poder/nao_conformidades_acoes_imple", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->nao_conformidades_acoes_imple_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if($this->fl_permissao)
		{
			...
			redirect( 'indicador_poder/nao_conformidades_acoes_imple' );
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
