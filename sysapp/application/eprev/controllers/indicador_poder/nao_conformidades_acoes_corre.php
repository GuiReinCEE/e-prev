<?php
class nao_conformidades_acoes_corre extends Controller
{
	var	$label_0 = "Mês";
	var	$label_1 = "Fora do prazo";#"Ações Corretivas Propostas em mais de 15 dias acumulado no período";
	var	$label_2 = "Prevista";#"NC com data limite p/ proposição ação corretica acumulado no período";
	var	$label_3 = "% Fora do prazo";#"% AC proposta fora do prazo";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::PODER_NAO_CONFORMIDADES_ACOES_CORRETIVAS);
		...
		elseif($this->session->userdata('indic_12') == "*") # Comitê da Qualidade
		{
			$this->fl_permissao = TRUE;
		}		
    ...
    function index()
    {
		...
		if($this->fl_permissao)
		{
	        ...
	        $this->load->view('indicador_poder/nao_conformidades_acoes_corre/index.php',$data);
		}
		else
		{
    ...
    function listar()
    {
		...
		if($this->fl_permissao)
        {
	        $this->load->model( 'indicador_poder/nao_conformidades_acoes_corre_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->nao_conformidades_acoes_corre_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_poder/nao_conformidades_acoes_corre/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->load->model('indicador_poder/nao_conformidades_acoes_corre_model');
			$row=$this->nao_conformidades_acoes_corre_model->carregar( $cd );
				...
				if($cd==0)
				{
					$sql = "
                        SELECT TO_CHAR(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS mes_referencia,
                               ...
                               nr_meta,
                               cd_indicador_tabela
                          FROM indicador_poder.nao_conformidades_acoes_corre
			...
			$this->load->view('indicador_poder/nao_conformidades_acoes_corre/detalhe', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		if($this->fl_permissao)
		{
			...
			$args['cd_nao_conformidades_acoes_corre'] = intval($this->input->post('cd_nao_conformidades_acoes_corre', true));
            ...
            {
                $args['nr_percentual_f'] = (( floatval($args["nr_valor_1"])/floatval($args["nr_valor_2"]))*100);
            }
            else
            {
                $args['nr_percentual_f'] = 0;
            }

			$msg=array();
			$retorno = $this->nao_conformidades_acoes_corre_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_poder/nao_conformidades_acoes_corre", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->nao_conformidades_acoes_corre_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if($this->fl_permissao)
		{
			...
			redirect( 'indicador_poder/nao_conformidades_acoes_corre' );
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
