<?php
class acoes_preventivas extends Controller
{
	var	$label_0 = "Mês";
	var	$label_1 = "Implementadas (acum.)";
	var	$label_2 = "";
	var	$label_3 = "";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::PODER_ACOES_PREVENTIVAS);
    ...
    function index()
    {
		...
		if($this->fl_permissao)
		{
	        ...
	        $this->load->view('indicador_poder/acoes_preventivas/index.php',$data);
		}
		else
		{
    ...
    function listar()
    {
		...
		if($this->fl_permissao)
        {
	        $this->load->model( 'indicador_poder/acoes_preventivas_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->acoes_preventivas_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_poder/acoes_preventivas/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->load->model('indicador_poder/acoes_preventivas_model');
			$row=$this->acoes_preventivas_model->carregar( $cd );
				...
				if($cd==0)
				{
					$sql = "
                        SELECT TO_CHAR(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS mes_referencia,
                               ...
                               nr_meta,
                               cd_indicador_tabela
                          FROM indicador_poder.acoes_preventivas
			...
			$this->load->view('indicador_poder/acoes_preventivas/detalhe', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		if($this->fl_permissao)
		{
			...
			$args['cd_acoes_preventivas'] = intval($this->input->post('cd_acoes_preventivas', true));
			...
			$msg=array();
			$retorno = $this->acoes_preventivas_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_poder/acoes_preventivas", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->acoes_preventivas_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if($this->fl_permissao)
		{
			...
			redirect( 'indicador_poder/acoes_preventivas');
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
