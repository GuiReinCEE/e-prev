<?php
class realizacao_orcam extends Controller
{
	var	$label_0 = "Mês";
	var	$label_1 = "Orçado - Acumulado";
	var	$label_2 = "Realizado - Acumulado";
	var	$label_3 = "% Realizado";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::PODER_REALIZACAO_ORCAMENTARIA);
    ...
    function index()
    {
		...
		if($this->fl_permissao)
		{
	        ...
	        $this->load->view('indicador_poder/realizacao_orcam/index.php',$data);
		}
		else
		{
    ...
    function listar()
    {
		...
		if($this->fl_permissao)
        {
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->load->view('indicador_poder/realizacao_orcam/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->load->model('indicador_poder/realizacao_orcam_model');
			$row=$this->realizacao_orcam_model->carregar( $cd );
				...
				if($cd==0)
				{
					$sql = "
                        SELECT TO_CHAR(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS mes_referencia,
                               ...
                               nr_meta,
                               cd_indicador_tabela
                          FROM indicador_poder.realizacao_orcam
			...
			$this->load->view('indicador_poder/realizacao_orcam/detalhe', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		if($this->fl_permissao)
		{
			...
			$args['cd_realizacao_orcam'] = intval($this->input->post('cd_realizacao_orcam', true));
            ...
            if( floatval($args["nr_valor_1"])>0 )
            {
                $args['nr_percentual_f'] = (( floatval($args["nr_valor_2"])/floatval($args["nr_valor_1"]) ))*100;
            }
            else
            {
			...
			$msg=array();
			$retorno = $this->realizacao_orcam_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_poder/realizacao_orcam", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->realizacao_orcam_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if($this->fl_permissao)
		{
			...
			redirect( 'indicador_poder/realizacao_orcam' );
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
