<?php
class custo_est_operacional extends Controller
{
	var	$label_0 = "Mês";
	var	$label_1 = "Desp. Adm. (Acum.12m)"; //"Média Desp. Adm. (cta 4200)";
	var	$label_2 = "Nr. Partic.";//"Nr Médio de Partic.";
	var	$label_3 = "Custo Estrutura";//"Custo Médio Estrutura (R\$)";
    var $label_4 = "Meta Mínima (R\$)";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::PODER_CUSTO_ESTRUTURA_OPERACIONAL);
		...
		elseif (gerencia_in(array('GC', 'GFC')))
		{
			$this->fl_permissao = TRUE;
		}
    ...
    function index()
    {
		...
		if($this->fl_permissao)
		{
	        ...
	        $this->load->view('indicador_poder/custo_est_operacional/index.php',$data);
		}
		else
		{
    ...
    function listar()
    {
		...
		if($this->fl_permissao)
        {
	        $this->load->model( 'indicador_poder/custo_est_operacional_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->custo_est_operacional_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_poder/custo_est_operacional/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->load->model('indicador_poder/custo_est_operacional_model');
			$row=$this->custo_est_operacional_model->carregar( $cd );
				...
				if($cd==0)
				{
					$sql = "
                        SELECT TO_CHAR(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS mes_referencia,
                               ...
                               nr_meta,
                               cd_indicador_tabela
                          FROM indicador_poder.custo_est_operacional
			...
			$this->load->view('indicador_poder/custo_est_operacional/detalhe', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		if($this->fl_permissao)
		{
			...
			$args['cd_custo_est_operacional'] = intval($this->input->post('cd_custo_est_operacional', true));
            ...
            {
                $args['nr_percentual_f'] = ( floatval($args["nr_valor_1"])/floatval($args["nr_valor_2"]));
            }
            else
            {
			...
			$msg=array();
			$retorno = $this->custo_est_operacional_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_poder/custo_est_operacional", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->custo_est_operacional_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if($this->fl_permissao)
		{
			...
			redirect( 'indicador_poder/custo_est_operacional' );
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
