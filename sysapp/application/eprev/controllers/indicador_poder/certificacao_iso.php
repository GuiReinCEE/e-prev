<?php
class certificacao_iso extends Controller
{
	var	$label_0 = "Mês";
	var	$label_1 = "Meta (1 = Manter Certificacao)";
	var	$label_2 = "Resultado";
	var	$label_3 = "";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::PODER_CERTIFICACAO_ISO);
    ...
    function index()
    {
		...
		if($this->fl_permissao)
		{
	        ...
	        $this->load->view('indicador_poder/certificacao_iso/index.php',$data);
		}
		else
		{
    ...
    function listar()
    {
		...
		if($this->fl_permissao)
        {
	        $this->load->model( 'indicador_poder/certificacao_iso_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->certificacao_iso_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_poder/certificacao_iso/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->load->model('indicador_poder/certificacao_iso_model');
			$row=$this->certificacao_iso_model->carregar( $cd );
			...
			$this->load->view('indicador_poder/certificacao_iso/detalhe', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		if($this->fl_permissao)
		{
			...
			$args['cd_certificacao_iso'] = intval($this->input->post('cd_certificacao_iso', true));
			...
			$msg=array();
			$retorno = $this->certificacao_iso_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_poder/certificacao_iso", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->certificacao_iso_model->excluir($id);
		...
		if($this->fl_permissao)
		{
			...
			else
			{
				...
				for( $i=0; $i<sizeof($indicador); $i++ )
				{
					...
					$sql .= indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 4, $linha, utf8_encode($indicador[$i][5]), 'center' );
	...
	function fechar_periodo()
	{
		...
		if($this->fl_permissao)
		{
			...
			redirect( 'indicador_poder/certificacao_iso' );
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
