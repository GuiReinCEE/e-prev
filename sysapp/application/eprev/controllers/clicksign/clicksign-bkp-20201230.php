...
class clicksign extends Controller
{
	...
	function __construct()
    {
		...
		$this->token_acesso        = md5('integracaoenviarCLICKSIGN');       #83eaa4b96dfed1a3a92238b43fe90cec
		$this->token_acesso_eletro = md5('integracaoenviarCLICKSIGNeletro'); #7276d8ca1a90585cffb624260eb76876
	...
	private function assinador($_NOME, $_EMAIL, $_TELEFONE, $_GRUPO, $_CD_DOCUMENTO)
	{
	...
	function documento()
	{
		...
		{
			if (strtoupper(trim($this->input->post('ds_nome_'.$i))) != "") 
			{
				if((trim($this->input->post('ds_nome_'.$i)) != "") AND (trim($this->input->post('ds_email_'.$i)) != "") AND (trim($this->input->post('nr_telefone_'.$i)) != ""))
				{
					$ar_user[] = array(
									"nome"  => strtoupper(trim($this->input->post('ds_nome_'.$i))),
									"email" => strtolower(trim($this->input->post('ds_email_'.$i))),
									"nr_telefone" => str_replace("(","",str_replace(")","",(trim($this->input->post('nr_telefone_'.$i))))),
									"grupo" => 1
								); 
				}
			}
		...
		if($args["token"] == $this->token_acesso_eletro)
		{
			...
			if($fl_campo_obrigatorio)
			{
				...
				$ds_json = '
							{
								"document": {
									...
									"deadline_at":"'.trim($args["deadline_at"]).' T23:59:59-03:00",
									"remind_interval":"2",
									"auto_close":"true",
				...
				if(!array_key_exists("errors", $_AR_DOC))
				{				
					#### ADICIONAR SIGNATARIOS ####
					$_AR_ASSINADOR = Array();
					foreach($ar_user as $ar_item)
					{
						$_AR_ASSINADOR[] = $this->assinador($ar_item['nome'], $ar_item['email'], $ar_item['nr_telefone'], $ar_item['grupo'], trim($_AR_DOC["ARRAY"]['document']['key']));
					}				
