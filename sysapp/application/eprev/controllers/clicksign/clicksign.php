...
class clicksign extends Controller
{
	...
	private function submit_clicksign_simples($url, $fl_patch = false, $data_string = "")
	{
		$ch = curl_init($url);			   
	...
	private function assinador($_NOME, $_EMAIL, $_TELEFONE, $TIPO_ASSINATURA, $TIPO_TOKEN, $FL_DOC_IDENFICACAO, $_GRUPO, $_CD_DOCUMENTO)
	{
		...
		$_EMAIL = str_replace("bernardob@familiaprevidencia.com.br","bbaggio@familiaprevidencia.com.br",$_EMAIL); 
		...
		else
		{
			...
			$ds_json = '
						{
							"signer": {
							    ...
							    '.(strtoupper(trim($FL_DOC_IDENFICACAO)) == "S" ? '"official_document_enabled": true,' : '').'
			...
			$ds_arq_log = $this->LOG_CLICK."_log-clicksign-eletro-signatario-".date("Ymd").".txt";
	...
	function documento()
	{
		...
		$args["cd_tipo_documento"]     = $this->input->post("cd_tipo_documento", TRUE);
		...
		for($i=1; $i <= intval($args["qt_signatario"]); $i++)
		{
			$ar_user[] = array(
								...
								"fl_doc_idenficacao" => (strtoupper(trim($this->input->post('fl_doc_idenficacao_'.$i))) == "S" ? "S" : "N"),
		...
		if (in_array($args["token"], $this->AR_TOKEN))
		{
			...
			if($fl_campo_obrigatorio)
			{
				...
				{				
					...
					{
						$_AR_ASSINADOR[] = $this->assinador($ar_item['nome'], $ar_item['email'], $ar_item['nr_telefone'], $ar_item['tp_assinatura'], $ar_item['tp_token'], $ar_item['fl_doc_idenficacao'], $ar_item['grupo'], trim($_AR_DOC["ARRAY"]['document']['key']));
					}				
					...
					if(trim($cd_documento) != "")
					{
						...
						$_AR_LOG['cd_empresa']            = $args["cd_empresa"];         
						$_AR_LOG['cd_registro_empregado'] = $args["cd_registro_empregado"];
						$_AR_LOG['seq_dependencia']       = $args["seq_dependencia"];     
						$_AR_LOG['cd_tipo_documento']     = $args["cd_tipo_documento"];     
			...
			if($fl_campo_obrigatorio)
			{
				...
				{				
					...
					*/	
					...
					foreach($_AR_DOC["ARRAY"]['document']['events'] as $ar_ev)
					{
						if(trim(strtoupper($ar_ev['name'])) == "DEADLINE")
						{
							$fl_deadline = TRUE;
						}
					}
					...
					elseif(($fl_deadline == TRUE) AND ($fl_status == "CLOSED"))
					{
						$ds_status = "Documento cancelado";
						$fl_status = "CANCELED";
					}					
					...
					if(trim($cd_documento) != "")
					{
						...
						$ar_ret["ds_documento"] = trim($_AR_DOC["ARRAY"]['document']['filename']);
	...
	function documento_datafim()
	{
		...
		if (in_array($args["token"], $this->AR_TOKEN))
		{
			...
			if($fl_campo_obrigatorio)
			{
				echo ($this->FL_DEBUG ? "ALTERAR DATA FIM <BR>" : "");
				#### ALTERAR DATA FIM ####
				$ds_json = '
								{
								  "deadline_at":"'.trim($args["deadline_at"]).' T'.(trim($args["deadline_hr"])).'-03:00"
						   ...
						   ';
				$_AR_DOC = $this->submit_clicksign_simples($this->API_URL."/documents/".trim($args["cd_documento"])."?access_token=".$this->API_TOKEN, TRUE, $ds_json);
				
				echo ($this->FL_DEBUG ? print_r($_AR_DOC,TRUE) : "");		
				
				#### GRAVA LOG EXECUCAO ####
				$ds_arq_log = $this->LOG_CLICK."_log-clicksign-eletro-datafim-".date("Ymd").".txt";
				$ob_arq = fopen($ds_arq_log, "a");
				fwrite($ob_arq, '########################################################################################'.chr(10).chr(10));
				fwrite($ob_arq, print_r($args,true).chr(10));		
				fwrite($ob_arq, print_r($_AR_DOC,true).chr(10));
				...
				{
					$ar_ret["cd_documento"]  = trim($args["cd_documento"]);
					$ar_ret["deadline_at"]   = trim($args["deadline_at"]).' T'.(trim($args["deadline_hr"]));
					$ar_ret["retorno"]       = "ALTERACAO DA DATA realizada com sucesso";
				}
				else
				{
					$ar_ret["fl_erro"] = "S";
