<?php
	...
	if(intval($_SESSION['PLANO']) == 2)
	{
		echo '<meta http-equiv="refresh" content="0; url=auto_atendimento_contribuicao_mais_ceeeprev.php">';
		exit;
	}
	...
	$ds_arq   = "tpl/tpl_auto_atendimento_contribuicao_mais.html";
		...
		if(intval($_RETORNO['error']['status']) == 0)
		{
			...
			if(count($boleto) > 0)		
			{
				...
				foreach ($boleto as $key => $item) 
				{
					...
					{
						$link = '<a href="auto_atendimento_contribuicao_gerado.php?cd_log='.MD5($item['cd_log']).'">'.$item['dt_solicitacao'].'</a>';
					}
