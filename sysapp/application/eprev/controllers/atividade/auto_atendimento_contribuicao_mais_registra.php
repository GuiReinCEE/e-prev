	...
	$qr_sql = "
		...
		VALUES
			 (
			   ".$_SESSION['SID'].",
			   CURRENT_TIMESTAMP,
			   'CONTRIBUICAO_MAIS_REGISTRA'
			 );";
	@pg_query($db,$qr_sql);

	if(($_REQUEST['vl_temp'] != '') AND ($_REQUEST['dt_vencimento'] != ''))
	{
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, APP_SRV."/srvautoatendimento/index.php/registra_boleto_eventual");
		curl_setopt($ch, CURLOPT_POST, 1);
		curl_setopt($ch, CURLOPT_POSTFIELDS,"id_app=".$_APP_ID."&re_cripto=".$_SESSION['RE_CRIPTO']."&vl_total_pagar=".$_REQUEST['vl_temp']."&dt_vencimento=".$_REQUEST['dt_vencimento']);
		...
		if($FL_RETORNO)
		{
			if(intval($_RETORNO['error']['status']) == 0)
			{
				...
				header('Location: auto_atendimento_contribuicao_gerado.php?cd_log='.MD5($cd_log)); 
			}
			else
			{
				echo "
