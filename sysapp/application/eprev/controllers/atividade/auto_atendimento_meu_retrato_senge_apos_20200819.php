<?php
	...
	#### SENGE ####
	$_CD_PLANO = 7;
	if(!in_array($_SESSION['EMP'], array(7))) 
	{
		echo "
				<br><br><br>
				<center>
	...
	{
		$tpl = new TemplatePower('meu_retrato/tpl/tpl_meu_retrato_'.trim($template).'-apos-20200819.html');
	}
	$tpl->prepare();	
	...
	else
	{
		...
		## FRENTE ##
		$ob_pdf->AddPage();
		$ob_pdf->Image('meu_retrato/img/'.trim($template).'-APOS-20200819.jpg', 0, 0, ConvertSize(800,$ob_pdf->pgwidth), ConvertSize(1131,$ob_pdf->pgwidth),'','',false);
		...
		$ob_pdf->Text(118,60, "Data Base:");
		$ob_pdf->SetFont('segoeuil','',12);
		$ob_pdf->Text(118,67, $ar_pessoal['dt_base_extrato']);
		
		$ob_pdf->SetFont('segoeuil','',8);
		$ob_pdf->Text(155,59, "Data Ingresso:");		
		$ob_pdf->SetFont('segoeuil','',12);
		$ar_item = getMeuRetratoItem("PARTICIPANTE_DT_INGRESSO");
		$ob_pdf->Text(155,64, $ar_item['ds_linha']);
		
		$ob_pdf->SetFont('segoeuil','',8);
		$ob_pdf->Text(155,68, "CNPB:");		
		$ob_pdf->SetFont('segoeuil','',11);
		$ar_item = getMeuRetratoItem("PARTICIPANTE_CNPB_PLANO");
		$ob_pdf->Text(155,72, $ar_item['ds_linha']);
		...
		## GRAFICO ##
		$ob_pdf->AddPage();
		$ob_pdf->Image('meu_retrato/img/'.trim($template).'-APOS-GRAF-20200819.jpg', 0, 0, ConvertSize(800,$ob_pdf->pgwidth), ConvertSize(1131,$ob_pdf->pgwidth),'','',false);
		$ob_pdf->SetFont('Courier','',6);
		$ob_pdf->Text(15,284, $ar_pessoal['dt_hoje']);	
		...
		$ob_pdf->Image('meu_retrato/img/'.trim($template).'-APOS-FAQ-20200819.jpg', 0, 0, ConvertSize(800,$ob_pdf->pgwidth), ConvertSize(1131,$ob_pdf->pgwidth),'','',false);
		$ob_pdf->SetFont('Courier','',6);
		$ob_pdf->Text(15,284, $ar_pessoal['dt_hoje']);
		...
		$ar_faq[0] = array("PERGUNTA" => "Data de Ingresso", "RESPOSTA" => "Corresponde à data de inscrição no plano, quando ocorreu a primeira contribuição para o SENGE Previdência.");
		$ar_faq[1] = array("PERGUNTA" => "Benefício do Mês", "RESPOSTA" => "Valor do benefício pago no mês de referência do extrato.");
		$ar_faq[2] = array("PERGUNTA" => "Data de Aposentadoria na Fundação", "RESPOSTA" => "Corresponde à data em que o participante passou para condição de assistido do Plano SENGE Previdência.");
		...
		$ar_faq[6] = array("PERGUNTA" => "Rendimento Financeiro", "RESPOSTA" => "Valor correspondente à rentabilidade do SENGE Previdência no mês de referência do extrato.");		
