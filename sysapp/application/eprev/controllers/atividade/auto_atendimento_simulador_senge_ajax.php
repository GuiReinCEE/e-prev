<?php
	...
	if($_POST)
	{
		if($_POST['ds_funcao'] == "calculaBeneficio")
		{
			...
			$_POST['nr_prazo'], $_POST['fl_contribuicao_empregador'], $_POST['vl_contribuicao_empregador'], $_POST['qt_contribuicao_empregador'], $_POST['fl_adiantamento'], $_POST['vl_adiantamento']		
			);
		}
	...
	function calculaBeneficio($cd_empresa, $cd_registro_empregado, $seq_dependencia, $dt_ingresso, $dt_simulacao,$dt_nascimento,$nr_idade_apos,$vl_contribuicao,$vl_rentabilidade, $nr_prazo, $fl_contribuicao_empregador, $vl_contribuicao_empregador, $qt_contribuicao_empregador, $fl_adiantamento, $vl_adiantamento)
	{
		$debug = FALSE;
	...
	if(in_array($_POST['user_md5'], $user_md5)) #### CRISTIANO | DENIS
	{
		...
		#### SOMAS AS CIP's #####
		$vl_cip = ($vl_cip + $vl_cip_atual + $vl_saldo_empregador);		
		...
		if (($nr_prazo_maximo == 0) or ($nr_prazo > $nr_prazo_maximo))
		{
			...
			echo "
						...
						<BR>
						Altere os parâmetros da simulação para que seu benefício fique igual ou superior ao valor da Unidade Referencial de R$ <b>".number_format(getURSENGE(),2,',','.')."</b>.
					</div>
					<BR>
					<BR>
				 ";
				...
				<tr>
					<td colspan='2'>
						...
						<div style='text-align:center'>
							<input type='button' value='Evolução do Benefício' class='botao' id='btEvoluiBeneficio' onclick=\"$('#obEvolucaoBeneficio').show(); $('#btEvoluiBeneficio').hide();\">
						</div>
					</td>
				</tr>
				<tr>
					<td colspan='2' id='obEvolucaoBeneficio' style='display:none;'>
					".evoluiBeneficio($vl_cip_acumulada,$vl_contribuicao_inicial,$vl_rentabilidade,$nr_prazo)."
					</td>
				</tr>				
			</table>
			<BR>
		...
		$qr_sql = "
					SELECT (COALESCE(qt_cip,0) + COALESCE(qt_port,0)) AS qt_cota
					  FROM (SELECT fnc_saldo_cd_contas::NUMERIC AS qt_cip 
					          FROM oracle.fnc_saldo_cd_contas(7, ".$cd_empresa.", ".$cd_registro_empregado.", ".$seq_dependencia.", '".$dt_ingresso."', 11)) a,
					       (SELECT fnc_saldo_cd_contas::NUMERIC AS qt_port 
					          FROM oracle.fnc_saldo_cd_contas(7, ".$cd_empresa.", ".$cd_registro_empregado.", ".$seq_dependencia.", '".$dt_ingresso."', 1401)) c					
	              ";
		*/
		
		$qr_sql = "
					SELECT senge_cip_quota AS qt_cota
					  FROM simulador.senge_cip_quota(".$cd_empresa.", ".$cd_registro_empregado.", ".$seq_dependencia.", TO_DATE('".$dt_ingresso."','DD/MM/YYYY'));
		          ";				  
		$ob_resul = pg_query($db, $qr_sql);
		$ar_reg   = pg_fetch_array($ob_resul);
	...
	{
		$nr_prazo = (int) (($vl_cip  / getURSENGE()) / 13); 
		if($nr_prazo < 5)
		{
			$nr_prazo = 0;
		}
	...
	function evoluiBeneficio($vl_saldo,$vl_renda,$vl_rentabilidade,$nr_prazo)
	{
		...
		$ds_retorno = '
						...
						<table class="sort-table" id="table-1" align="center" width="50%" cellspacing="2" cellpadding="2">
							<thead>
							<tr height="25">
								<th>
									Ano
								...
								<th>
									Saldo <nobr>no início</nobr><BR><nobr>do Ano</nobr>
								</th>	
								<th>
									Benefício
								</th>
