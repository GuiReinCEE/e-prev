<?php
	...
	if($_REQUEST['pdf'] != "S")
	{
		...
		while($ar_reg = pg_fetch_array($ob_res))
		{
			...
			{
				$tpl->assign('GRAFICO_COMPARATIVO', 'grafico_compara_SINPRORS_'.$ar_reg['ano_base_extrato'].'.png');
			}			
		}
	...
	else
	{
		...
		$ar_faq[0] = array("PERGUNTA" => "Data de Ingresso", "RESPOSTA" => "Corresponde à data de inscrição no plano, quando ocorreu a primeira contribuição para o SINPRORS Previdência.");
		$ar_faq[1] = array("PERGUNTA" => "Benefício do Mês", "RESPOSTA" => "Valor do benefício pago no mês de referência do extrato.");
		$ar_faq[2] = array("PERGUNTA" => "Data de Aposentadoria na Fundação", "RESPOSTA" => "Corresponde à data em que o participante passou para condição de assistido do Plano SINPRORS Previdência.");
		...
		$ar_faq[6] = array("PERGUNTA" => "Rendimento Financeiro", "RESPOSTA" => "Valor correspondente à rentabilidade do SINPRORS Previdência no mês de referência do extrato.");		
