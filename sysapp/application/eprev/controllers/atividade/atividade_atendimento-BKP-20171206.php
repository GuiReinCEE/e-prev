    ...
    public function index($cd_atividade = 0)
    {
		...
		{   
			...
			if(count($data['ar_atividade']) > 0)
			{
				...
				{
					if(in_array($data['ar_atividade']['cd_gerencia_destino'], array("GI","GAP","GB","GAD","GF","GC","GA","GRI")))
					{
						...
						if(in_array($data['ar_atividade']['cd_gerencia_destino'], array("GI","GRI")))
						{
						...
						$this->load->view('atividade/atividade_atendimento/index_'.strtolower(trim($data['ar_atividade']['cd_gerencia_destino'])), $data);
					}
					else
					{
						#exibir_mensagem("REDIRECIONAR PARA O MIGRE");
