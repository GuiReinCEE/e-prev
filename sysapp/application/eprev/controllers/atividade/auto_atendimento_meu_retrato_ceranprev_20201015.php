<?php
	...
	else
	{
		...
		$ob_pdf->SetXY(117,69);
		$ob_pdf->MultiCell(66, 4,"Carência do Plano", 0, "R");				
		...
		$ob_pdf->Text(31, 105.3, "CONTRIBUIÇÃO ACUMULADA"); 		
		...
		#### CONTRIBUICAO ####
		$ob_pdf->SetFont('segoeuib','',17);
		$ob_pdf->Text(31, 168, "CONTRIBUIÇÃO DO MÊS"); 				
		...
		$ob_pdf->MultiCell(154, 3.5, 'O seu saldo acumulado no plano depende de três fatores: nível de contribuição, tempo de acumulação e rentabilidade.
Confira, no cenário abaixo, como seu dinheiro pode crescer');			
		...
		$ob_pdf->MultiCell(190, 3, 'O valor do Benefício Inicial Simulado foi calculado com base nas regras regulamentares. Consideramos as contribuições que serão realizadas e a rentabilidade real destas fixada em 8,00% até o momento da aposentadoria.
O Saldo Acumulado Simulado foi revertido em um benefício inicial equivalente a 1,5% do saldo. A renda resultante para pagamento de benefícios deve ser igual ou  maior que uma Unidade Referencial do CERANPrev, gerando pagamentos no prazo mínimo de cinco anos. Caso seja inferior, o simulador não gera benefício, apresentando somente o saldo acumulado.
Você pode fazer outras simulações clicando no link acima.
Você pode escolher como será calculado seu benefício: Opção 1: percentual do saldo de conta no plano (entre 0,1% a 1,5%). Opção 2: definição de um prazo para recebimento do benefício (mínimo 5 anos).
O valor do Benefício Inicial Simulado não considera o desconto de impostos e taxas administrativas.', 0, "J");						
		...
		$ob_pdf->MultiCell($nr_tam, $nr_lin, "Corresponde ao total de contribuições previdenciárias (do participante e da patrocinadora) acrescido do rendimento financeiro.", 0, "J");		
		
		$ob_pdf->SetY($ob_pdf->GetY() + 1.2);
		$ob_pdf->SetFont('segoeuib','',$font_1);
		$ob_pdf->MultiCell($nr_tam, $nr_lin, "Data de Ingresso:", 0, "J");			
		$ob_pdf->SetFont('segoeuil','',$font_2);
		$ob_pdf->MultiCell($nr_tam, $nr_lin, "Corresponde ao mês da inscrição no plano, quando ocorreu a primeira contribuição para o CERANPREV.", 0, "J");					

		$ob_pdf->SetY($ob_pdf->GetY() + 1.2);
		$ob_pdf->SetFont('segoeuib','',$font_1);
		$ob_pdf->MultiCell($nr_tam, $nr_lin, "Minha portabilidade:", 0, "J");			
		$ob_pdf->SetFont('segoeuil','',$font_2);
		$ob_pdf->MultiCell($nr_tam, $nr_lin, "Valor transferido de outro plano previdenciário para o CERANPREV.", 0, "J");			
		
		$ob_pdf->SetY($ob_pdf->GetY() + 1.2);
		$ob_pdf->SetFont('segoeuib','',$font_1);
		$ob_pdf->MultiCell($nr_tam, $nr_lin, "Minha contribuição: ", 0, "J");			
		$ob_pdf->SetFont('segoeuil','',$font_2);
		$ob_pdf->MultiCell($nr_tam, $nr_lin, "Corresponde à contribuição previdenciária ao CERANPREV, acrescida da contribuição adicional e de aportes (se houver).", 0, "J");		

		$ob_pdf->SetY($ob_pdf->GetY() + 1.2);
		$ob_pdf->SetFont('segoeuib','',$font_1);
		$ob_pdf->MultiCell($nr_tam, $nr_lin, "Contribuição da Empresa: ", 0, "J");			
		$ob_pdf->SetFont('segoeuil','',$font_2);
		$ob_pdf->MultiCell($nr_tam, $nr_lin, "A empresa também contribui com o mesmo valor que você para a sua conta individual. Caso a sua contribuição apareça com valor maior que a da empresa, possivelmente você tenha feito contribuições adicionais e/ou contribuições voluntárias.", 0, "J");		
		
		$ob_pdf->SetY($ob_pdf->GetY() + 1.2);
		$ob_pdf->SetFont('segoeuib','',$font_1);
		$ob_pdf->MultiCell($nr_tam, $nr_lin, "Rendimento Financeiro:", 0, "J");			
		$ob_pdf->SetFont('segoeuil','',$font_2);
		$ob_pdf->MultiCell($nr_tam, $nr_lin, "Valor correspondente à rentabilidade acumulada do CERANPREV desde seu ingresso no plano.", 0, "J");			
		
		$ob_pdf->SetY($ob_pdf->GetY() + 1.2);
		$ob_pdf->SetFont('segoeuib','',$font_1);
		$ob_pdf->MultiCell($nr_tam, $nr_lin, "Minha contribuição deste mês:", 0, "J");			
		$ob_pdf->SetFont('segoeuil','',$font_2);
		$ob_pdf->MultiCell($nr_tam, $nr_lin, "Corresponde a um percentual variável de 0,5% a 6% do Valor Base de Contribuição. Esse percentual é de livre escolha do participante.", 0, "J");		
		
		$ob_pdf->SetY($ob_pdf->GetY() + 1.2);
		$ob_pdf->SetFont('segoeuib','',$font_1);
		$ob_pdf->MultiCell($nr_tam, $nr_lin, "Contribuição da empresa deste mês: ", 0, "J");			
		$ob_pdf->SetFont('segoeuil','',$font_2);
		$ob_pdf->MultiCell($nr_tam, $nr_lin, "A empresa também contribui com o mesmo valor que você. Caso a sua contribuição apareça com valor maior que a da empresa, possivelmente você tenha feito contribuições adicionais e/ou contribuições voluntárias.", 0, "J");		
		
		$ob_pdf->SetY($ob_pdf->GetY() + 1.2);
		$ob_pdf->SetFont('segoeuib','',$font_1);
		$ob_pdf->MultiCell($nr_tam, $nr_lin, "Valor Base de Contribuição: ", 0, "J");			
		$ob_pdf->SetFont('segoeuil','',$font_2);
		$ob_pdf->MultiCell($nr_tam, $nr_lin, "É o valor sobre o qual incidem as contribuições para o CERANPREV, dado pelo salário-base do Participante.", 0, "J");		
		
		$ob_pdf->SetY($ob_pdf->GetY() + 1.2);
		$ob_pdf->SetFont('segoeuib','',$font_1);
		$ob_pdf->MultiCell($nr_tam, $nr_lin, "Carência do plano:", 0, "J");			
		$ob_pdf->SetFont('segoeuil','',$font_2);
		$ob_pdf->MultiCell($nr_tam, $nr_lin, "São as condições regulamentares para Aposentadoria: 55 anos de idade, 10 anos de vínculo ao plano e rescisão do vínculo com a patrocinadora (CERAN).", 0, "J");			
		
		$ob_pdf->SetY($ob_pdf->GetY() + 1.2);
		$ob_pdf->SetFont('segoeuib','',$font_1);
		$ob_pdf->MultiCell($nr_tam, $nr_lin, "Rentabilidade da simulação:", 0, "J");			
		$ob_pdf->SetFont('segoeuil','',$font_2);
		$ob_pdf->MultiCell($nr_tam, $nr_lin, "A simulação fixa uma meta de desempenho do plano. Porém, esse desempenho pode ser maior ou menor conforme a conjuntura econômica, impactando diretamente no resultado do benefício.", 0, "J");			
		
		$ob_pdf->SetY($ob_pdf->GetY() + 1.2);
		$ob_pdf->SetFont('segoeuib','',$font_1);
		$ob_pdf->MultiCell($nr_tam, $nr_lin, "Saldo acumulado simulado:", 0, "J");			
		$ob_pdf->SetFont('segoeuil','',$font_2);
		$ob_pdf->MultiCell($nr_tam, $nr_lin, "Corresponde à projeção de crescimento do seu fundo previdenciário até o cumprimento das condições regulamentares para recebimento do benefício de aposentadoria: 55 anos de idade, 10 anos de vinculação ao plano e rescisão do contrato de trabalho com a empresa patrocinadora (CERAN).", 0, "J");	

		$ob_pdf->SetY($ob_pdf->GetY() + 1.2);
		$ob_pdf->SetFont('segoeuib','',$font_1);
		$ob_pdf->MultiCell($nr_tam, $nr_lin, "Benefício Inicial Simulado:", 0, "J");			
		$ob_pdf->SetFont('segoeuil','',$font_2);
		$ob_pdf->MultiCell($nr_tam, $nr_lin, "Corresponde à projeção do seu benefício no plano quando você completar as condições regulamentares para aposentadoria: 55 anos de idade, 10 anos de vinculação ao plano e rescisão do contrato de trabalho com a empresa patrocinadora (CERAN). Esta projeção simulada de seu benefício no CERANPREV serve para você acompanhar o crescimento de sua poupança previdenciária. O desempenho real do plano dependerá dos resultados obtidos ao longo dos próximos anos.", 0, "J");				
	...
	function getComparativoIMG($cd_edicao)
	{
		...
		$MyData->addPoints(array("FUNDAÇÃO","POUPANÇA","INPC","IGPM"),"Browsers");
