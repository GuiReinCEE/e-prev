...
class reuniao_sg extends Controller
{
    ...
    function __construct()
    {
        ...
        if($this->session->userdata('codigo') == 424) #Vanessa Silva Alves
        {
            $this->fl_acesso = true;
        }
        ...
        elseif ($this->session->userdata('codigo') == 251) #Luciano Rodriguez (GI)
        {
            $this->fl_acesso = true;
        }
		elseif ($this->session->userdata('codigo') == 489) #Julia Gabrieli Freitas de Oliveira (GC)
        {
            $this->fl_acesso = true;
        }
