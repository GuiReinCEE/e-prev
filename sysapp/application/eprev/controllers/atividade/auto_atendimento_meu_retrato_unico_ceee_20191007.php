<?php
	...
	#### PLANO UNICO CEEE ####
	$_CD_PLANO = 1;
	if(!in_array($_SESSION['EMP'], array(0))) 
	{
		echo "
				<br><br><br>
				<center>
	...
	$tpl = new TemplatePower('meu_retrato/tpl/tpl_meu_retrato_'.trim($template).'_20191007.html');
	$tpl->prepare();	
	...
	else
	{
		...
		## FRENTE ##
		$ob_pdf->AddPage();
		$ob_pdf->Image('meu_retrato/img/'.trim($template).'_20191007.jpg', 0, 0, ConvertSize(800,$ob_pdf->pgwidth), ConvertSize(1131,$ob_pdf->pgwidth),'','',false);
		...
		*/
		...
		## GRAFICO ##
		$ob_pdf->AddPage();
		$ob_pdf->Image('meu_retrato/img/'.trim($template).'-GRAF_20191007.jpg', 0, 0, ConvertSize(800,$ob_pdf->pgwidth), ConvertSize(1131,$ob_pdf->pgwidth),'','',false);
		$ob_pdf->SetFont('Courier','',6);
		$ob_pdf->Text(15,284, $ar_pessoal['dt_hoje']);	
		...
		$ob_pdf->Image('meu_retrato/img/'.trim($template).'-FAQ_20191007.jpg', 0, 0, ConvertSize(800,$ob_pdf->pgwidth), ConvertSize(1131,$ob_pdf->pgwidth),'','',false);
		$ob_pdf->SetFont('Courier','',6);
		$ob_pdf->Text(15,284, $ar_pessoal['dt_hoje']);
		...
		$fl_ajuda_pdf = "S";
		include("meu_retrato/inc/ajuda_UNICO_CEEE.php");
