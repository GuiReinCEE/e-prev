<?php
	...
	{
		$tpl = new TemplatePower('meu_retrato/tpl/tpl_meu_retrato_'.trim($template).'-20210118.html');
	}
	$tpl->prepare();	
	...
	if($_REQUEST['pdf'] != "S")
	{
		...
		$tpl->assign('GRAF_SALDO_RENDIMENTO_VALOR',        floatval($ar_item['vl_valor']));			
		...
		while($ar_reg = pg_fetch_array($ob_res))
		{
			...
			{
				$tpl->assign('GRAFICO_COMPARATIVO', 'grafico_compara_SENGE_'.$ar_reg['ano_base_extrato'].'.png');
			}			
		}
	...
	else
	{
		...
		#$ob_pdf->Text(100,284, "1/5");		

		#### DADOS PLANO ####
		$ob_pdf->SetFont('segoeuil','',8);
		$ob_pdf->Text(118,49, "Data Base:");
		$ob_pdf->SetFont('segoeuil','',12);
		$ob_pdf->Text(118,56, $ar_pessoal['dt_base_extrato']);
		...
		#### CONTRIBUICAO RISCO
		$ob_pdf->SetFont('segoeuib','',17);
		$ob_pdf->Text(31, 202.5, "CONTRIBUIÇÃO DO MÊS"); 		
		...
		$ob_pdf->Image('meu_retrato/img/'.trim($template).'-SIMULACAO_20190917.jpg', 0, 0, ConvertSize(800,$ob_pdf->pgwidth), ConvertSize(1131,$ob_pdf->pgwidth),'','',false);
		$ob_pdf->SetFont('Courier','',6);
		$ob_pdf->Text(15,284, $ar_pessoal['dt_hoje']);
		#$ob_pdf->Text(100,284, "2/5");
...
Confira, nos cenários abaixo, como seu dinheiro pode crescer no SENGE Previdência.", 0, "C");		
		...
		while($cd_cenario <= 3)
		{
			...
			$ob_pdf->MultiCell(50, 4.5, "POR ".intval($ar_tempo['vl_valor'])." COM UMA RENTABILIDADE ANUAL DE ".number_format($ar_rentab['vl_valor'],2,",",".")."%, MEU SALDO ACUMULADO SERÁ DE:", 0, "L");		
		...
		$ar_faq[] = array("PERGUNTA" => "Data de Ingresso", "RESPOSTA" => "Data da primeira contribuição para o plano SENGE Previdência.");
		$ar_faq[] = array("PERGUNTA" => "Minha portabilidade", "RESPOSTA" => "Valor transferido de outro plano previdenciário para o plano SENGE Previdência.");
		$ar_faq[] = array("PERGUNTA" => "Minha contribuição", "RESPOSTA" => "Corresponde ao total das suas contribuições previdenciárias ao SENGE Previdência. Este valor vai para a sua conta individual no plano. É destinado para a cobertura dos benefícios de Aposentadoria Normal, Abono Anual e Pecúlio por Morte.");
		$ar_faq[] = array("PERGUNTA" => "Aporte do Empregador", "RESPOSTA" => "Total dos aportes feitos por seu empregador. A empresa a qual você está vinculado também pode contribuir para a sua poupança previdenciária. Esta contribuição é facultativa, podendo ser contratada pelo empregador junto ao SENGE Previdência.");
		$ar_faq[] = array("PERGUNTA" => "Rendimento financeiro", "RESPOSTA" => "Valor correspondente à rentabilidade acumulada do plano SENGE Previdência. A rentabilidade impacta diretamente no benefício do plano.");
		$ar_faq[] = array("PERGUNTA" => "Minha contribuição deste mês", "RESPOSTA" => "Corresponde à contribuição previdenciária ao SENGE Previdência.");
		$ar_faq[] = array("PERGUNTA" => "Contribuição Administrativa", "RESPOSTA" => "O valor da Contribuição Administrativa é fixo, atualizado anualmente pela variação acumulada do INPC do ano anterior. É destinada à cobertura das despesas da Fundação CEEE para administração do plano SENGE Previdência. Não é resgatável.");
		...
		$ar_faq[] = array("PERGUNTA" => "Contribuição Total de Risco", "RESPOSTA" => "Somatório das contribuições destinadas à cobertura dos benefícios de invalidez total e permanente e pensão por morte do participante. Esses benefícios são contratados facultativamente pelo participante do SENGE Previdência.");
		$ar_faq[] = array("PERGUNTA" => "Contribuição de Risco - Invalidez", "RESPOSTA" => "Contribuição destinada somente para a cobertura do benefício de invalidez total e permanente. Esse benefício é contratado facultativamente pelo participante do SENGE Previdência.");
		$ar_faq[] = array("PERGUNTA" => "Contribuição de Risco - Pensão por Morte", "RESPOSTA" => "Contribuição destinada somente para a cobertura do benefício de pensão por morte do participante. Esse benefício é contratado facultativamente pelo participante do SENGE Previdência.");
		$ar_faq[] = array("PERGUNTA" => "Capital de Risco - Invalidez", "RESPOSTA" => "Corresponde ao valor contratado junto ao SENGE Previdência. Caso o participante se invalide, ele receberá o valor da cobertura contratada dividido em parcelas (mínimo de 60 parcelas).");
		$ar_faq[] = array("PERGUNTA" => "Capital de Risco - Pensão por Morte", "RESPOSTA" => "Corresponde ao valor contratado junto ao SENGE Previdência. Em caso de falecimento do participante, seus beneficiários receberão o valor da cobertura contratada dividido em parcelas (mínimo de 60 parcelas).");
	...
	function getRentabilidadeGrafico($cd_edicao, $cd_empresa, $cd_plano)
	{
		...
		*/
		...
		$myPicture->drawText(150,30,'Rentabilidade do Plano '.$ds_plano.' - '.$nr_ano,array("FontSize"=>12,"Align"=>TEXT_ALIGN_BOTTOMMIDDLE)); 
