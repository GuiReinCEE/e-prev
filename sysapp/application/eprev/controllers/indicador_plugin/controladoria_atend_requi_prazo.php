<?php
class controladoria_atend_requi_prazo extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::CONTROLADORIA_ATENDIMENTO_REQUISICAO_NO_PRAZO);
		...
		$this->load->model('indicador_plugin/controladoria_atend_requi_prazo_model');
    }
	...
	function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GC'))
		{
	        ...
	        $this->load->view('indicador_plugin/controladoria_atend_requi_prazo/index', $data);
		}
		else
		{
	...
	function listar()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GC'))
        {
			...
			$this->controladoria_atend_requi_prazo_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/controladoria_atend_requi_prazo/index_result', $data);
        }
        else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function cadastro($cd_controladoria_atend_requi_prazo = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC'))
		{
			$args   = array();
			...
			$args['cd_controladoria_atend_requi_prazo'] = $cd_controladoria_atend_requi_prazo;
			
			if(intval($args['cd_controladoria_atend_requi_prazo']) == 0)
			{
				$args['nr_ano_referencia'] = intval($data['tabela'][0]['nr_ano_referencia']);
				
				$this->controladoria_atend_requi_prazo_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_controladoria_atend_requi_prazo'] = $args['cd_controladoria_atend_requi_prazo'];
				$data['row']['nr_valor_1']                          = "";
				$data['row']['nr_valor_2']                          = "";
				$data['row']['fl_media']                            = "";
				$data['row']['observacao']                          = "";
			...
			{
				$this->controladoria_atend_requi_prazo_model->carrega($result, $args);
				$data['row'] = $result->row_array();
				$data['row']['qt_ano'] = 1;
			}

			$this->load->view('indicador_plugin/controladoria_atend_requi_prazo/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_controladoria_atend_requi_prazo'] = intval($this->input->post('cd_controladoria_atend_requi_prazo', true));
			...
			$this->controladoria_atend_requi_prazo_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/controladoria_atend_requi_prazo", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_controladoria_atend_requi_prazo)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC'))
		{
			$args   = array();
			...
			$this->controladoria_atend_requi_prazo_model->excluir($result, $args);
		...
		if(indicador_db::verificar_permissao(usuario_id(),'GC'))
		{
			...
			foreach($collection as $item)
			{
				if(intval($item['ano_referencia']) >= intval($tabela[0]['nr_ano_referencia']) -5)
				{
					...
					$indicador[$linha][4] = app_decimal_para_php($nr_meta_f);
			...
			*/
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				if( $nr_valor_1_total > 0)
				{
					$resultado = ($nr_valor_2_total/ $nr_valor_1_total) * 100;
				}
				...
				$indicador[$linha][3] = number_format($resultado, 2);
				$indicador[$linha][4] = '';
				$indicador[$linha][5] = app_decimal_para_php($nr_meta);;
				$indicador[$linha][6] = '';
			}
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC'))
		{
			...
			$args['cd_controladoria_atend_requi_prazo'] = 0;
			...
			$this->controladoria_atend_requi_prazo_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
