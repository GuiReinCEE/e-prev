<?php
class administrativo_area_nloc extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Área não Locada";
	var	$label_2 = "Área Total para Locação";
	var	$label_3 = "% Área não Locada";
    var $label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RH_AREA_NAO_LOCADA);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/administrativo_area_nloc/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
        {
	        $this->load->model( 'indicador_plugin/administrativo_area_nloc_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->administrativo_area_nloc_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/administrativo_area_nloc/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$this->load->model('indicador_plugin/administrativo_area_nloc_model');
			$row=$this->administrativo_area_nloc_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
			...
			$this->load->view('indicador_plugin/administrativo_area_nloc/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$args['cd_administrativo_area_nloc']=intval($this->input->post('cd_administrativo_area_nloc', true));
			...
			$msg=array();
			$retorno = $this->administrativo_area_nloc_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/administrativo_area_nloc", "refresh" );
				}
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$this->administrativo_area_nloc_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.administrativo_area_nloc
		...
		redirect( 'indicador_plugin/administrativo_area_nloc' );
