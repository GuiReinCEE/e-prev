<?php
class atuarial_eap_ceee extends Controller
{
	var	$label_0 = "Mês";
	var	$label_1 = "Reservas Técnicas";
	var	$label_2 = "Provisões Matemáticas";
	var	$label_3 = "Variação Mensal";
    var $label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::ATUARIAL_CEEE);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
	        ...
	        $this->load->view('indicador_plugin/atuarial_eap_ceee/index.php', $data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
        {
	        ...
	        $this->load->model( 'indicador_plugin/Atuarial_eap_ceee_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->Atuarial_eap_ceee_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/atuarial_eap_ceee/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
			...
			$this->load->model('indicador_plugin/Atuarial_eap_ceee_model');
			$row=$this->Atuarial_eap_ceee_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.atuarial_eap_ceee 
			...
			$this->load->view('indicador_plugin/atuarial_eap_ceee/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
			...
			$args['cd_atuarial_eap_ceee']=intval($this->input->post('cd_atuarial_eap_ceee', true));
			...
			$msg=array();
			$retorno = $this->Atuarial_eap_ceee_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/atuarial_eap_ceee", "refresh" );	
				}			
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
			...
			$this->Atuarial_eap_ceee_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.atuarial_eap_ceee
		...
		redirect( 'indicador_plugin/atuarial_eap_ceee' );
