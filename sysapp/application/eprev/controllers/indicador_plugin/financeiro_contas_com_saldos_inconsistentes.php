<?php
class Financeiro_contas_com_saldos_inconsistentes extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::FINANCEIRO_CONTAS_COM_SALDOS_INCONSISTENTES);
		...
		$this->load->model('indicador_plugin/financeiro_contas_com_saldos_inconsistentes_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
	        ...
	        $this->load->view('indicador_plugin/financeiro_contas_com_saldos_inconsistentes/index',$data);
		}
    }
    ...
    function listar()
    {
    	if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
        {
			...
			$this->financeiro_contas_com_saldos_inconsistentes_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/financeiro_contas_com_saldos_inconsistentes/index_result', $data);
        }
        else
		{
    ...
    function cadastro($cd_financeiro_contas_com_saldos_inconsistentes = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$args['cd_financeiro_contas_com_saldos_inconsistentes'] = $cd_financeiro_contas_com_saldos_inconsistentes;
			
			if(intval($args['cd_financeiro_contas_com_saldos_inconsistentes']) == 0)
			{
				$this->financeiro_contas_com_saldos_inconsistentes_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_financeiro_contas_com_saldos_inconsistentes'] = $args['cd_financeiro_contas_com_saldos_inconsistentes'];
			...
			{
				$this->financeiro_contas_com_saldos_inconsistentes_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/financeiro_contas_com_saldos_inconsistentes/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_financeiro_contas_com_saldos_inconsistentes'] = intval($this->input->post('cd_financeiro_contas_com_saldos_inconsistentes', true));
			...
			$this->financeiro_contas_com_saldos_inconsistentes_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/financeiro_contas_com_saldos_inconsistentes", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_financeiro_contas_com_saldos_inconsistentes)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$this->financeiro_contas_com_saldos_inconsistentes_model->excluir($result, $args);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->financeiro_contas_com_saldos_inconsistentes_model->atualiza_fechar_periodo($result, $args);
			}

			$this->financeiro_contas_com_saldos_inconsistentes_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
