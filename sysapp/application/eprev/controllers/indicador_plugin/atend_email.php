<?php
class atend_email extends Controller
{
	var	$label_0 = "Mês";
	var	$label_1 = "Total de Atendimentos por E-mail";
	var	$label_2 = "Atendimentos (Pessoal + URA+E-mail)";
	var	$label_3 = "% de Atendimentos por E-mail";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::ATENDIMENTO_PERCENTUAL_DE_ATENDIMENTOS_POR_EMAILS);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/atend_email/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
        {
	        ...
	        $this->load->model( 'indicador_plugin/Atend_email_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->Atend_email_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/atend_email/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->load->model('indicador_plugin/Atend_email_model');
			$row=$this->Atend_email_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.atend_email 
			...
			$this->load->view('indicador_plugin/atend_email/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$args['cd_atend_email']=intval($this->input->post('cd_atend_email', true));
			...
			$msg=array();
			$retorno = $this->Atend_email_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/atend_email", "refresh" );
				}
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->Atend_email_model->excluir($id);
	...
	function criar_indicador()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			{#tabela_existe
				...
				$sql.=indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 0,0, utf8_encode($data['label_1']), 'background,center');
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.atend_email
					( dt_referencia,dt_inclusao,cd_usuario_inclusao, nr_percentual_f, fl_media ) 
		...
		redirect( 'indicador_plugin/atend_email' );
