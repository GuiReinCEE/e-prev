<?php
class administrativo_inventario extends Controller
{
	var	$label_0 = "Ano";
	var	$label_1 = "Nr. De Itens Errados";
	var	$label_2 = "Nr. Total e Itens do Ativo Permanente";
	var	$label_3 = "% de Itens Errados";
    var	$label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RH_ITENS_ERRADOS_INVENTARIO);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/administrativo_inventario/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
        {
	        $this->load->model( 'indicador_plugin/administrativo_inventario_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->administrativo_inventario_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/administrativo_inventario/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$this->load->model('indicador_plugin/administrativo_inventario_model');
			$row=$this->administrativo_inventario_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.administrativo_inventario 
			...
			$this->load->view('indicador_plugin/administrativo_inventario/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$args['cd_administrativo_inventario']=intval($this->input->post('cd_administrativo_inventario', true));
			...
			$msg=array();
			$retorno = $this->administrativo_inventario_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/administrativo_inventario", "refresh" );	
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$this->administrativo_inventario_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.administrativo_inventario
		...
		redirect( 'indicador_plugin/administrativo_inventario' );
