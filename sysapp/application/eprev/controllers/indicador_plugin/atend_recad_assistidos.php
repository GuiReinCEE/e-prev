<?php
class atend_recad_assistidos extends Controller
{
	var	$label_0 = "Semestre/Ano";
	var	$label_1 = "Total de Assistidos e pensionistras Recadastrados";
	var	$label_2 = "Total de Assistidos e pensionistas a se recadastrarem";
    var	$label_3 = "% de recdastrados";
    var	$label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::ATENDIMENTO_RECADASTRAMENTO_DE_ASSISTIDOS);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/atend_recad_assistidos/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
        {
	        $this->load->model( 'indicador_plugin/atend_recad_assistidos_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->atend_recad_assistidos_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/atend_recad_assistidos/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->load->model('indicador_plugin/atend_recad_assistidos_model');
			$row=$this->atend_recad_assistidos_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.atend_recad_assistidos
			...
			$this->load->view('indicador_plugin/atend_recad_assistidos/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$args['cd_atend_recad_assistidos']=intval($this->input->post('cd_atend_recad_assistidos', true));
			...
			$msg=array();
			$retorno = $this->atend_recad_assistidos_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/atend_recad_assistidos", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->atend_recad_assistidos_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.atend_recad_assistidos
				...
				*/
				...
				// indicar que o período foi fechado para o indicador_tabela
				$sql=" 
						INSERT INTO indicador_plugin.atend_recad_assistidos
							 ( 
							   dt_referencia,
							   dt_inclusao,
							   cd_usuario_inclusao, 
							 ...
							 (
								SELECT aep.dt_referencia,
									   ...
									   aep.nr_meta,
									   'S'
								  FROM indicador_plugin.atend_recad_assistidos aep
								 WHERE aep.dt_exclusao IS NULL
								   ...
								   AND aep.dt_referencia = (SELECT MAX(eap1.dt_referencia) 
															  FROM indicador_plugin.atend_recad_assistidos eap1
		...
		redirect( 'indicador_plugin/atend_recad_assistidos' );
