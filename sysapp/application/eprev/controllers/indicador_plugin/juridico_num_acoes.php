<?php
class Juridico_num_acoes extends Controller
{
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::JURIDICO_NUMERO_DE_ACOES);
		...
		$this->load->model('indicador_plugin/juridico_num_acoes_model' );
    }	
	...
	function index()
    {
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
	        ...
	        $this->load->view('indicador_plugin/juridico_num_acoes/index',$data);
		}
    }	
	...
	function listar()
    {
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
        {
			...
			$this->juridico_num_acoes_model->listar($result, $args);
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/juridico_num_acoes/partial_result', $data);
        }
    }

	function detalhe($cd_juridico_num_acoes = 0)
	{
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
			$args   = array();
			...
			$args['cd_juridico_num_acoes'] = intval($cd_juridico_num_acoes);
			
			if(intval($args['cd_juridico_num_acoes']) == 0)
			{
				$this->juridico_num_acoes_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_juridico_num_acoes'] = $args['cd_juridico_num_acoes'];
				$data['row']['nr_nova']       = "";
				$data['row']['qt_acoes']      = "";
			...
			{
				$this->juridico_num_acoes_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/juridico_num_acoes/detalhe', $data);
		}
	}	
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_juridico_num_acoes'] = intval($this->input->post('cd_juridico_num_acoes', true));
			...
			$this->juridico_num_acoes_model->salvar($result, $args);
			$this->criar_indicador();
			
			redirect("indicador_plugin/juridico_num_acoes", "refresh");
		}
        else
        {
	...
	function excluir($cd_juridico_num_acoes = 0)
	{
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
			$args   = array();
			...
			$this->juridico_num_acoes_model->excluir($result, $args);
			$this->criar_indicador();
	...
	function fechar_periodo()
	{
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->juridico_num_acoes_model->atualiza_fechar_periodo($result, $args);
			}

			$this->juridico_num_acoes_model->fechar_periodo($result, $args);
