<?php
class ri_venda_previd extends Controller
{
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RI_VENDA_DOS_PLANOS_PREVIDENCIARIOS);
		
		$this->load->helper(array('indicador'));
		
		CheckLogin();
		
		$this->load->model('indicador_plugin/ri_venda_previd_model');
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(), 'AC') OR ((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*")))
		{
	        ...
	        $this->load->view('indicador_plugin/ri_venda_previd/index',$data);
		}
    }
    ...
    function listar()
    {
		if(indicador_db::verificar_permissao(usuario_id(), 'AC') OR ((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*")))
        {
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->ri_venda_previd_model->listar($result, $args);
				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/ri_venda_previd/partial_result', $data);
			}
			else
			{
	...
	function cadastro($cd = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(), 'AC') OR ((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*")))
		{
			...
			if(intval($args['cd']) == 0)
			{
				$this->ri_venda_previd_model->carrega_novo($result, $args);
				$row = $result->row_array();
				
				$data['row'] = array(
					'cd_ri_venda_previd'  => $cd,
			...
			{
				$this->ri_venda_previd_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}
			
			$this->load->view('indicador_plugin/ri_venda_previd/cadastro', $data);
		}
	}
	...
	function salvar()
	{
		if(indicador_db::verificar_permissao( usuario_id(), 'AC' ) )
		{
			...
			$args['cd_ri_venda_previd']  = intval($this->input->post('cd_ri_venda_previd', true));
			...
			$this->ri_venda_previd_model->salvar($result, $args);
			...
			{
				redirect("indicador_plugin/ri_venda_previd", "refresh");
			}
		}
	}
	...
	function excluir($cd)
	{
		if(indicador_db::verificar_permissao( usuario_id(), 'AC' ) )
		{
			...
			$this->ri_venda_previd_model->excluir($result, $args);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(), 'AC') OR ((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*")))
		{
			...
			else
			{
				...
				$args['nr_tot_ano_inscricao'] = $nr_tot_ano_inscricao;
				$args['resultado_ano']        = ($nr_tot_ano_inscricao / $nr_tot_ano_contato) * 100;
				$args['nr_meta']              = $nr_meta;
				$args['cd_indicador_tabela']  = $tabela[0]['cd_indicador_tabela'];
				$args['cd_usuario']           = usuario_id();
				
				$this->ri_venda_previd_model->fecha_indicador( $result, $args );
			}
		}

		redirect( 'indicador_plugin/ri_venda_previd' );
	}
}
?>