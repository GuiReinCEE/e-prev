<?php
class Exp_adesao_potencial_crmprev extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::EXP_NUMERO_DE_ADESAO_X_NUMERO_POTENCIAL_CRMPREV);
		...
		$this->load->model('indicador_plugin/exp_adesao_potencial_crmprev_model');
    }
    ...
    public function index()
    {
		if((indicador_db::verificar_permissao($this->cd_usuario,'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{
			...
			$this->load->view('indicador_plugin/exp_adesao_potencial_crmprev/index',$data);
		}
    }
    ...
    public function listar()
    {
    	if((indicador_db::verificar_permissao($this->cd_usuario,'GE')) OR ($this->session->userdata('indic_12') == "*"))
        {
			...
			$data['collection'] = $this->exp_adesao_potencial_crmprev_model->listar($data['tabela'][0]['cd_indicador_tabela']);
	        
			$this->load->view('indicador_plugin/exp_adesao_potencial_crmprev/index_result', $data);
		}
        else
		{
    ...
    public function cadastro($cd_exp_adesao_potencial_crmprev = 0)
	{
		if((indicador_db::verificar_permissao($this->cd_usuario,'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{
			$data['label_0'] = $this->label_0;
			...
			if(intval($cd_exp_adesao_potencial_crmprev) == 0)
			{
				$row = $this->exp_adesao_potencial_crmprev_model->carrega_referencia();
								
				$data['row'] =array(
					'cd_exp_adesao_potencial_crmprev' => $cd_exp_adesao_potencial_crmprev,
					'mes_referencia'				=> '',
			...
			{
				$data['row'] = $this->exp_adesao_potencial_crmprev_model->carrega($cd_exp_adesao_potencial_crmprev);
			}

			$this->load->view('indicador_plugin/exp_adesao_potencial_crmprev/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	public function salvar()
	{
		if((indicador_db::verificar_permissao($this->cd_usuario,'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{		
			$nr_valor_4      = app_decimal_para_db($this->input->post('nr_valor_4', true));
			$nr_valor_2      = app_decimal_para_db($this->input->post('nr_valor_2', true));
			$nr_percentual_f = 0;

			if(floatval($nr_valor_2) > 0)
			{
				$nr_percentual_f = (($nr_valor_4/$nr_valor_2)*100);
			}

			$args = array(
				'cd_exp_adesao_potencial_crmprev' => $this->input->post('cd_exp_adesao_potencial_crmprev', true),
			...
			$this->exp_adesao_potencial_crmprev_model->salvar($args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/exp_adesao_potencial_crmprev", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	public function excluir($cd_exp_adesao_potencial_crmprev)
	{
		if((indicador_db::verificar_permissao($this->cd_usuario,'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{
			$this->exp_adesao_potencial_crmprev_model->excluir($cd_exp_adesao_potencial_crmprev, $this->cd_usuario);
	...
	public function criar_indicador()
	{
		if((indicador_db::verificar_permissao($this->cd_usuario,'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{
			...
			$collection = $this->exp_adesao_potencial_crmprev_model->listar($tabela[0]['cd_indicador_tabela']);
