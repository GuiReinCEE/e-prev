<?php
class administrativo_rotatividade extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Maior Efetivo do mês";
	var	$label_2 = "Total de demissões do mês";
	var	$label_3 = "% Rotatividade";
	var	$label_4 = "Limite máximo";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RH_ROTATIVIDADE);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/administrativo_rotatividade/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC' ) )
        {
	        $this->load->model( 'indicador_plugin/administrativo_rotatividade_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->administrativo_rotatividade_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/administrativo_rotatividade/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC' ) )
		{
			...
			$this->load->model('indicador_plugin/administrativo_rotatividade_model');
			$row=$this->administrativo_rotatividade_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.administrativo_rotatividade 
			...
			$this->load->view('indicador_plugin/administrativo_rotatividade/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC' ) )
		{
			...
			$args['cd_administrativo_rotatividade']=intval($this->input->post('cd_administrativo_rotatividade', true));
			...
			$msg=array();
			$retorno = $this->administrativo_rotatividade_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/administrativo_rotatividade", "refresh" );			
				}
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC' ) )
		{
			...
			$this->administrativo_rotatividade_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($total_ano)>0)
				{
					...
					$sql.=	sprintf(" 
									INSERT INTO indicador_plugin.administrativo_rotatividade
									     ( 
										   dt_referencia,
										   dt_inclusao,
										   cd_usuario_inclusao, 
		...
		redirect( 'indicador_plugin/administrativo_rotatividade' );
