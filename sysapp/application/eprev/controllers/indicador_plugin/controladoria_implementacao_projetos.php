<?php
class Controladoria_implementacao_projetos extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::CONTROLADORIA_IMPLEMENTACAO_PROJETOS);
		...
		$this->load->model('indicador_plugin/controladoria_implementacao_projetos_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GC'))
		{
	        ...
	        $this->load->view('indicador_plugin/controladoria_implementacao_projetos/index', $data);
		}
		else
		{
    ...
    function listar()
    {
    	if(indicador_db::verificar_permissao($this->cd_usuario, 'GC'))
        {
			...
			$data['collection'] = $this->controladoria_implementacao_projetos_model->listar($data['tabela'][0]['cd_indicador_tabela']);
			
			$this->load->view('indicador_plugin/controladoria_implementacao_projetos/index_result', $data);
        }
        else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
    ...
    function cadastro($cd_controladoria_implementacao_projetos = 0)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GC'))
		{
			$this->load->model('indicador_plugin/controladoria_implementacao_projetos_model');
			...
			if(intval($cd_controladoria_implementacao_projetos) == 0)
			{
				$row = $this->controladoria_implementacao_projetos_model->carrega_referencia(intval($data['tabela'][0]['nr_ano_referencia']));

				$data['row'] = array(
					'cd_controladoria_implementacao_projetos' => 0,
					'fl_media' 								  => '',
					'ds_observacao' 						  => '',
					'nr_tarefas' 							  => '',
					'nr_realizadas' 						  => '',
			...
			{
				$data['row'] = $this->controladoria_implementacao_projetos_model->carrega($cd_controladoria_implementacao_projetos);
				$data['row']['qt_ano'] = 1;
			}

			$this->load->view('indicador_plugin/controladoria_implementacao_projetos/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GC'))
		{
			...
			$cd_controladoria_implementacao_projetos = $this->input->post('cd_controladoria_implementacao_projetos', true);
			...
			if(intval($cd_controladoria_implementacao_projetos) == 0)
			{
				$this->controladoria_implementacao_projetos_model->salvar($args);
			}
			else
			{
				$this->controladoria_implementacao_projetos_model->atualizar($cd_controladoria_implementacao_projetos, $args);
			}
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/controladoria_implementacao_projetos", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_controladoria_implementacao_projetos)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GC'))
		{
			$this->load->model('indicador_plugin/controladoria_implementacao_projetos_model');
			
			$this->controladoria_implementacao_projetos_model->excluir($cd_controladoria_implementacao_projetos, $this->session->userdata('codigo'));
	...
	function criar_indicador()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GC'))
		{
			...
			$collection = $this->controladoria_implementacao_projetos_model->listar($tabela[0]['cd_indicador_tabela']);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GC'))
		{
			...
			$this->controladoria_implementacao_projetos_model->fechar_periodo($tabela[0]['cd_indicador_tabela'], $this->session->userdata('codigo'));
