<?php
class Financeiro_variacao_patrimonial extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::FINANCEIRO_VARIACAO_PATRIMONIAL);
		...
		$this->load->model('indicador_plugin/financeiro_variacao_patrimonial_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
	        ...
	        $this->load->view('indicador_plugin/financeiro_variacao_patrimonial/index',$data);
		}
    }
    ...
    function listar()
    {
    	if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
        {
			...
			$this->financeiro_variacao_patrimonial_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/financeiro_variacao_patrimonial/index_result', $data);
        }
        else
		{
    ...
    function cadastro($cd_financeiro_variacao_patrimonial = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$args['cd_financeiro_variacao_patrimonial'] = $cd_financeiro_variacao_patrimonial;
			
			if(intval($args['cd_financeiro_variacao_patrimonial']) == 0)
			{
				$this->financeiro_variacao_patrimonial_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_financeiro_variacao_patrimonial'] = $args['cd_financeiro_variacao_patrimonial'];
			...
			{
				$this->financeiro_variacao_patrimonial_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/financeiro_variacao_patrimonial/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_financeiro_variacao_patrimonial'] = intval($this->input->post('cd_financeiro_variacao_patrimonial', true));
			...
			$this->financeiro_variacao_patrimonial_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/financeiro_variacao_patrimonial", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_financeiro_variacao_patrimonial)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$this->financeiro_variacao_patrimonial_model->excluir($result, $args);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->financeiro_variacao_patrimonial_model->atualiza_fechar_periodo($result, $args);
			}

			$this->financeiro_variacao_patrimonial_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
