<?php
class atuarial_eap_rge extends Controller
{
	var	$label_0 = "Mês";
	var	$label_1 = "Reservas Técnicas";
	var	$label_2 = "Provisões Matemáticas";
	var	$label_3 = "Variação";
    var $label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::ATUARIAL_RGE);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
	        ...
	        $this->load->view('indicador_plugin/atuarial_eap_rge/index.php', $data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
        {
	        $this->load->model( 'indicador_plugin/Atuarial_eap_rge_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->Atuarial_eap_rge_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/atuarial_eap_rge/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
			...
			$this->load->model('indicador_plugin/Atuarial_eap_rge_model');
			$row=$this->Atuarial_eap_rge_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.atuarial_eap_rge 
			...
			$this->load->view('indicador_plugin/atuarial_eap_rge/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
			...
			$args['cd_atuarial_eap_rge']=intval($this->input->post('cd_atuarial_eap_rge', true));
			...
			$msg=array();
			$retorno = $this->Atuarial_eap_rge_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/atuarial_eap_rge", "refresh" );	
				}					
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
			...
			$this->Atuarial_eap_rge_model->excluir($id);
				...
				foreach( $collection as $item )
				{
					...
					if( intval($item['ano_referencia'])>=intval($tabela[0]['nr_ano_referencia'])-5 )
					{
						...
						if($item['fl_media'] != 'S')
						{
							...
							{
								$variacao = ( ( floatval($nr_reserva_tecnica)/floatval($nr_provisao_matematica) ));
							}
							else
							{
								$variacao='';
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.atuarial_eap_rge
					( dt_referencia,dt_inclusao,cd_usuario_inclusao, nr_reserva_tecnica, nr_provisao_matematica,  nr_variacao_f, nr_meta, fl_media ) 
					VALUES ( '%s/01/01',current_timestamp,%s, %s, %s, 'S' ); "
					, intval($tabela[0]['nr_ano_referencia']) , usuario_id(),  floatval($nr_reserva_tecnica),  floatval($nr_provisao_matematica), floatval($variacao), floatval( app_decimal_para_db($nr_meta) ));
				}
		...
		redirect( 'indicador_plugin/atuarial_eap_rge' );
