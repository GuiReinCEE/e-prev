<?php
class beneficio_inc_foz extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Número de Contrib. Calc. Mês";
	var	$label_2 = "Número de Contrib. Calc. Incorreta Mês";
	var	$label_3 = "% de Incorreções Contrib.";
    var $label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::BENEFICIO_INCORRECOES_NO_CALCULO_DAS_CONTRIBUICOESFOZ);
    }
    ...
    function index()
    {
		...
		{
			// VERIFICA SE EXISTE TABELA NO PERÃODO ABERTO, SE NÃƒO EXISTIR, CRIAR TABELA NO PERÃODO QUE ESTIVER ABERTO
	        ...
	        $this->load->view('indicador_plugin/beneficio_inc_foz/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
        {
	        $this->load->model( 'indicador_plugin/beneficio_inc_foz_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->beneficio_inc_foz_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/beneficio_inc_foz/partial_result', $data);
			}
			else
			{
				echo "Nenhum perÃ­odo aberto para o indicador.";
			}
        }
    }
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->load->model('indicador_plugin/beneficio_inc_foz_model');
			$row=$this->beneficio_inc_foz_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.beneficio_inc_foz 
			...
			$this->load->view('indicador_plugin/beneficio_inc_foz/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$args['cd_beneficio_inc_foz']=intval($this->input->post('cd_beneficio_inc_foz', true));
			...
			$msg=array();
			$retorno = $this->beneficio_inc_foz_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/beneficio_inc_foz", "refresh" );	
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->beneficio_inc_foz_model->excluir($id);
	...
	function criar_indicador()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			{#tabela_existe
				...
				return false;
				#echo "NÃ£o foi identificado perÃ­odo aberto para o Indicador";
			...
			{#tabela_existe
				...
				// LINHA DE TENDÃŠNCIA - CURVA LOGARITMICA
				...
				echo anchor( "indicador/apresentacao/detalhe/".intval( $tabela[0]['cd_indicador_tabela'] ), 'Exibir apresentaÃ§Ã£o' );*/
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.beneficio_inc_foz
				...
				// indicar que o perÃ­odo foi fechado para o indicador_tabela
		...
		redirect( 'indicador_plugin/beneficio_inc_foz' );
		// echo 'perÃ­odo encerrado com sucesso';
	}
}
?>