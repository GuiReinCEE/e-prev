<?php
class financeiro_exec_fin extends Controller
{
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::FINANCEIRO_EXECUCAO_FINANCEIRA_TITULOS_DEVOLVIDOS_DE_ITENS);
		...
		$this->load->model('indicador_plugin/financeiro_exec_fin_model' );
    }
	...
	function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(), 'GFC'))
		{
	        ...
	        $this->load->view('indicador_plugin/financeiro_exec_fin/index',$data);
		}
    }
	...
	function listar()
    {
		if(indicador_db::verificar_permissao(usuario_id(), 'GFC'))
        {
			...
			$this->financeiro_exec_fin_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/financeiro_exec_fin/index_result', $data);
        }
    }
	
	function cadastro($cd_financeiro_exec_fin = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(), 'GFC'))
		{
			$args   = array();
			...
			$args['cd_financeiro_exec_fin'] = $cd_financeiro_exec_fin;
			
			if(intval($args['cd_financeiro_exec_fin']) == 0)
			{
				$this->financeiro_exec_fin_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_financeiro_exec_fin'] = $args['cd_financeiro_exec_fin'];
				$data['row']['nr_valor_1']             = "";
				$data['row']['nr_valor_2']             = "";
				$data['row']['fl_media']               = "";
				$data['row']['observacao']             = "";
			...
			{
				$this->financeiro_exec_fin_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/financeiro_exec_fin/cadastro', $data);
		}
	}
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_financeiro_exec_fin'] = intval($this->input->post('cd_financeiro_exec_fin', true));
			...
			$this->financeiro_exec_fin_model->salvar($result, $args);
			$this->criar_indicador();
			
			redirect("indicador_plugin/financeiro_exec_fin", "refresh");
		}
        else
        {
	...
	function excluir($cd_financeiro_exec_fin)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$this->financeiro_exec_fin_model->excluir($result, $args);
			$this->criar_indicador();
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GP' ))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->financeiro_exec_fin_model->atualiza_fechar_periodo($result, $args);
			}

			$this->financeiro_exec_fin_model->fechar_periodo($result, $args);
