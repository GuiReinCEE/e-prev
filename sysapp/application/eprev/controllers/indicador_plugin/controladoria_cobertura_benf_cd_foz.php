<?php
class Controladoria_cobertura_benf_cd_foz extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::CONTROLADORIA_COBERTURA_BENEFICIO_CD_FOZ);
		...
		$this->load->model('indicador_plugin/controladoria_cobertura_benf_cd_foz_model');
    }
	...
	function index()
    {
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
		{
	        ...
	        $this->load->view('indicador_plugin/controladoria_cobertura_benf_cd_foz/index', $data);
		}
		else
		{
	...
	function listar()
    {
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
        {
			...
			$data['collection'] = $this->controladoria_cobertura_benf_cd_foz_model->listar($data['tabela'][0]['cd_indicador_tabela']);
			
			$this->load->view('indicador_plugin/controladoria_cobertura_benf_cd_foz/index_result', $data);
        }
        else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function cadastro($cd_controladoria_cobertura_benf_cd_foz = 0)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
		{
			$data = array();
			...
			if(intval($cd_controladoria_cobertura_benf_cd_foz) == 0)
			{
				$row = $this->controladoria_cobertura_benf_cd_foz_model->carrega_referencia($data['tabela'][0]['cd_indicador_tabela']);
				
				$data['row'] = array(
					'cd_controladoria_cobertura_benf_cd_foz' => $cd_controladoria_cobertura_benf_cd_foz,
					'nr_valor_1'            			 => 0,
					'nr_valor_2'            			 => 0,
			...
			{
				$data['row'] = $this->controladoria_cobertura_benf_cd_foz_model->carrega($cd_controladoria_cobertura_benf_cd_foz);
			}

			$this->load->view('indicador_plugin/controladoria_cobertura_benf_cd_foz/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args = array(
				'cd_controladoria_cobertura_benf_cd_foz' => intval($this->input->post('cd_controladoria_cobertura_benf_cd_foz', true)),
			...
			$this->controladoria_cobertura_benf_cd_foz_model->salvar($args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/controladoria_cobertura_benf_cd_foz", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_controladoria_cobertura_benf_cd_foz)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
		{
			$this->controladoria_cobertura_benf_cd_foz_model->excluir($cd_controladoria_cobertura_benf_cd_foz, $this->cd_usuario);
	...
	function criar_indicador()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
		{
			...
			$collection = $this->controladoria_cobertura_benf_cd_foz_model->listar($tabela[0]['cd_indicador_tabela']);			 
