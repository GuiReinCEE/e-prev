<?php class Atuarial_cgtee extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::ATUARIAL_PLANO_CGTEE);
		...
		$this->load->model('indicador_plugin/atuarial_cgtee_model');
    }
	...
	function index()
    {
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
		{
			...
			$fl_novo_periodo = indicador_db::abrir_periodo_para_indicador($this->enum_indicador, $this->cd_usuario);
	        ...
	        $this->load->view('indicador_plugin/atuarial_cgtee/index', $data);
		}
		else
		{
	...
	function listar()
    {
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
        {
	        ...
	        $data['collection'] = $this->atuarial_cgtee_model->listar($data['tabela'][0]['cd_indicador_tabela']);
			
			$this->load->view('indicador_plugin/atuarial_cgtee/index_result', $data);
        }
        else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function cadastro($cd_atuarial_cgtee = 0)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
		{
			$data = array();
			...
			if(intval($cd_atuarial_cgtee) == 0)
			{
				$row = $this->atuarial_cgtee_model->carrega_referencia($data['tabela'][0]['cd_indicador_tabela']);
				
				$data['row'] = array(
					'cd_atuarial_cgtee' => $cd_atuarial_cgtee,
			...
			{
				$data['row'] = $this->atuarial_cgtee_model->carrega($cd_atuarial_cgtee);
			}

			$this->load->view('indicador_plugin/atuarial_cgtee/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args = array(
				'cd_atuarial_cgtee'   => intval($this->input->post('cd_atuarial_cgtee', true)),
			...
			$this->atuarial_cgtee_model->salvar($args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/atuarial_cgtee", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_atuarial_cgtee)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
		{
			$this->atuarial_cgtee_model->excluir($cd_atuarial_cgtee, $this->cd_usuario);
	...
	function criar_indicador()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
		{
			...
			$collection = $this->atuarial_cgtee_model->listar($tabela[0]['cd_indicador_tabela']);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
		{
			...
			$this->atuarial_cgtee_model->fechar_periodo($tabela[0]['cd_indicador_tabela'], $this->cd_usuario);
