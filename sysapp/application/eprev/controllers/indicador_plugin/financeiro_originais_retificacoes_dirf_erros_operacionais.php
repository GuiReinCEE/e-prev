<?php
class Financeiro_originais_retificacoes_dirf_erros_operacionais extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::FINANCEIRO_DIRF_ORIGINAIS_X_RETIFICACOES_ERROS_OPERACIONAIS);
		...
		$this->load->model('indicador_plugin/financeiro_originais_retificacoes_dirf_erros_operacionais_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
	        ...
	        $this->load->view('indicador_plugin/financeiro_originais_retificacoes_dirf_erros_operacionais/index',$data);
		}
    }
    ...
    function listar()
    {
    	if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
        {
			...
			$this->financeiro_originais_retificacoes_dirf_erros_operacionais_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/financeiro_originais_retificacoes_dirf_erros_operacionais/index_result', $data);
        }
        else
		{
    ...
    function cadastro($cd_financeiro_originais_retificacoes_dirf_erros_operacionais = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$args['cd_financeiro_originais_retificacoes_dirf_erros_operacionais'] = $cd_financeiro_originais_retificacoes_dirf_erros_operacionais;
			
			if(intval($args['cd_financeiro_originais_retificacoes_dirf_erros_operacionais']) == 0)
			{
				$this->financeiro_originais_retificacoes_dirf_erros_operacionais_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_financeiro_originais_retificacoes_dirf_erros_operacionais'] = $args['cd_financeiro_originais_retificacoes_dirf_erros_operacionais'];
			...
			{
				$this->financeiro_originais_retificacoes_dirf_erros_operacionais_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/financeiro_originais_retificacoes_dirf_erros_operacionais/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_financeiro_originais_retificacoes_dirf_erros_operacionais'] = intval($this->input->post('cd_financeiro_originais_retificacoes_dirf_erros_operacionais', true));
			...
			$this->financeiro_originais_retificacoes_dirf_erros_operacionais_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/financeiro_originais_retificacoes_dirf_erros_operacionais", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_financeiro_originais_retificacoes_dirf_erros_operacionais)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$this->financeiro_originais_retificacoes_dirf_erros_operacionais_model->excluir($result, $args);
	...
	function criar_indicador()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$indicador[$linha][3] = app_decimal_para_php(($nr_valor_1_total / $nr_valor_2_total) * 100);
				$indicador[$linha][4] = app_decimal_para_php($nr_meta);
				$indicador[$linha][5] = '';
				$indicador[$linha][6] = '';
			}
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->financeiro_originais_retificacoes_dirf_erros_operacionais_model->atualiza_fechar_periodo($result, $args);
			}

			$this->financeiro_originais_retificacoes_dirf_erros_operacionais_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
