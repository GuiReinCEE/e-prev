<?php
class Atuarial_exigencia_previc_regulamento extends Controller
{
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::ATUARIAL_EXIGENCIA_PREVIC_EM_APROVACAO_DE_REGULAMENTO);
		...
		$this->load->model('indicador_plugin/atuarial_exigencia_previc_regulamento_model');
    }
    ...
    public function index()
    {
		...
		{			
	        ...
	        $this->load->view('indicador_plugin/atuarial_exigencia_previc_regulamento/index', $data);
		}
        else
        {
    ...
    public function listar()
    {
		...
		{	
			...
			$data['collection'] = $this->atuarial_exigencia_previc_regulamento_model->listar($data['tabela'][0]['cd_indicador_tabela']);

			$this->load->view('indicador_plugin/atuarial_exigencia_previc_regulamento/index_result', $data);
		}
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
    ...
    public function cadastro($cd_atuarial_exigencia_previc_regulamento = 0)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GP'))
		{			
			$data['label_0']  = $this->label_0;
			...
			if(intval($cd_atuarial_exigencia_previc_regulamento) == 0)
			{
				$row = $this->atuarial_exigencia_previc_regulamento_model->carrega_referencia($data['tabela'][0]['cd_indicador_tabela']);

				$data['row'] = array(
					'cd_atuarial_exigencia_previc_regulamento' => intval($cd_atuarial_exigencia_previc_regulamento),
					'ds_evento'                               => '',
					'nr_houve_exigencia'                      => '',
			...
			{
				$data['row'] = $this->atuarial_exigencia_previc_regulamento_model->carrega(
					intval($cd_atuarial_exigencia_previc_regulamento)
				);
			}

			$this->load->view('indicador_plugin/atuarial_exigencia_previc_regulamento/cadastro', $data);
		}
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function salvar()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GP'))
		{		
			$cd_atuarial_exigencia_previc_regulamento = intval($this->input->post('cd_atuarial_exigencia_previc_regulamento', true));
			...
			if(intval($cd_atuarial_exigencia_previc_regulamento) == 0)
			{
				$this->atuarial_exigencia_previc_regulamento_model->salvar($args);
			}
			else
			{
				$this->atuarial_exigencia_previc_regulamento_model->atualizar($cd_atuarial_exigencia_previc_regulamento, $args);
			}

			$this->criar_indicador();
			
			redirect('indicador_plugin/atuarial_exigencia_previc_regulamento', 'refresh');
		}
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function excluir($cd_atuarial_exigencia_previc_regulamento)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GP'))
		{
			$this->atuarial_exigencia_previc_regulamento_model->excluir(
				intval($cd_atuarial_exigencia_previc_regulamento), 
				$this->cd_usuario
			);
    ...
    public function criar_indicador()
    {
    	if(indicador_db::verificar_permissao($this->cd_usuario, 'GP'))
		{
			...
			$collection = $this->atuarial_exigencia_previc_regulamento_model->listar($tabela[0]['cd_indicador_tabela']);
