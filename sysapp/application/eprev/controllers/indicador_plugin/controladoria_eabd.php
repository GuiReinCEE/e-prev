<?php
class Controladoria_eabd extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::CONTROLADORIA_EQUILIBRIO_ATUARIAL_BD);
		...
		$this->load->model('indicador_plugin/controladoria_eabd_model');
    }
	...
	function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GP'))
		{
	        ...
	        $this->load->view('indicador_plugin/controladoria_eabd/index', $data);
		}
		else
		{
	...
	function listar()
    {
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
        {
	        ...
	        $data['collection'] = $this->controladoria_eabd_model->listar($data['tabela'][0]['cd_indicador_tabela']);
			
			$this->load->view('indicador_plugin/controladoria_eabd/index_result', $data);
        }
        else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function cadastro($cd_controladoria_eabd = 0)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
		{
			$data = array();
			...
			if(intval($cd_controladoria_eabd) == 0)
			{
				$row = $this->controladoria_eabd_model->carrega_referencia($data['tabela'][0]['cd_indicador_tabela']);
				
				$data['row'] = array(
					'cd_controladoria_eabd' => $cd_controladoria_eabd,
			...
			{
				$data['row'] = $this->controladoria_eabd_model->carrega($cd_controladoria_eabd);
			}

			$this->load->view('indicador_plugin/controladoria_eabd/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args = array(
				'cd_controladoria_eabd'  => intval($this->input->post('cd_controladoria_eabd', true)),
			...
			$this->controladoria_eabd_model->salvar($args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/controladoria_eabd", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_controladoria_eabd)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
		{
			$this->controladoria_eabd_model->excluir($cd_controladoria_eabd,$this->cd_usuario);
	...
	function criar_indicador()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GP'))
		{
			...
			$collection = $this->controladoria_eabd_model->listar($tabela[0]['cd_indicador_tabela']);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
		{
			...
			$this->controladoria_eabd_model->fechar_periodo($tabela[0]['cd_indicador_tabela'], $this->cd_usuario);
