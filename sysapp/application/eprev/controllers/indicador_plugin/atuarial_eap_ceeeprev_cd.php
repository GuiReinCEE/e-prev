<?php
class atuarial_eap_ceeeprev_cd extends Controller
{
	var	$label_0 = "Mês";
	var	$label_1 = "Reservas Técnicas";
	var	$label_2 = "Rendimento Esperado";
	var	$label_3 = "Rendimento Obtido";
    var $label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::ATUARIAL_CEEEPREV_CD);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
	        ...
	        $this->load->view('indicador_plugin/atuarial_eap_ceeeprev_cd/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
        {
	        $this->load->model( 'indicador_plugin/atuarial_eap_ceeeprev_cd_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->atuarial_eap_ceeeprev_cd_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/atuarial_eap_ceeeprev_cd/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
			...
			$this->load->model('indicador_plugin/atuarial_eap_ceeeprev_cd_model');
			$row=$this->atuarial_eap_ceeeprev_cd_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.atuarial_eap_ceeeprev_cd 
			...
			$this->load->view('indicador_plugin/atuarial_eap_ceeeprev_cd/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
			...
			$args['cd_atuarial_eap_ceeeprev_cd']=intval($this->input->post('cd_atuarial_eap_ceeeprev_cd', true));
			...
			$msg=array();
			$retorno = $this->atuarial_eap_ceeeprev_cd_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/atuarial_eap_ceeeprev_cd", "refresh" );	
					#redirect( "indicador_plugin/atuarial_eap_ceeeprev_cd/detalhe/0", "refresh" );	
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
			...
			$this->atuarial_eap_ceeeprev_cd_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.atuarial_eap_ceeeprev_cd
		...
		redirect( 'indicador_plugin/atuarial_eap_ceeeprev_cd' );
