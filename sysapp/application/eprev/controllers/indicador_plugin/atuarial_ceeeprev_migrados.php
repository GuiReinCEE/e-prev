<?php
class Atuarial_ceeeprev_migrados extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::ATUARIAL_PLANO_CEEEPREV_MIGRADOS);
		...
		$this->load->model('indicador_plugin/atuarial_ceeeprev_migrados_model');
    }
	...
	function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GP'))
		{
	        ...
	        $this->load->view('indicador_plugin/atuarial_ceeeprev_migrados/index', $data);
		}
		else
		{
	...
	function listar()
    {
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
        {
	        ...
	        $data['collection'] = $this->atuarial_ceeeprev_migrados_model->listar($data['tabela'][0]['cd_indicador_tabela']);
			
			$this->load->view('indicador_plugin/atuarial_ceeeprev_migrados/index_result', $data);
        }
        else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function cadastro($cd_atuarial_ceeeprev_migrados = 0)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
		{
			$data = array();
			...
			if(intval($cd_atuarial_ceeeprev_migrados) == 0)
			{
				$row = $this->atuarial_ceeeprev_migrados_model->carrega_referencia($data['tabela'][0]['cd_indicador_tabela']);
				
				$data['row'] = array(
					'cd_atuarial_ceeeprev_migrados' => $cd_atuarial_ceeeprev_migrados,
			...
			{
				$data['row'] = $this->atuarial_ceeeprev_migrados_model->carrega($cd_atuarial_ceeeprev_migrados);
			}

			$this->load->view('indicador_plugin/atuarial_ceeeprev_migrados/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args = array(
				'cd_atuarial_ceeeprev_migrados' => intval($this->input->post('cd_atuarial_ceeeprev_migrados', true)),
			...
			$this->atuarial_ceeeprev_migrados_model->salvar($args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/atuarial_ceeeprev_migrados", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_atuarial_ceeeprev_migrados)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
		{
			$this->atuarial_ceeeprev_migrados_model->excluir($cd_atuarial_ceeeprev_migrados,$this->cd_usuario);
	...
	function criar_indicador()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GP'))
		{
			...
			$collection = $this->atuarial_ceeeprev_migrados_model->listar($tabela[0]['cd_indicador_tabela']);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$args = array(
					'cd_atuarial_ceeeprev_migrados' => 0,
					'cd_indicador_tabela'           => $tabela[0]['cd_indicador_tabela'],
			...
			$this->atuarial_ceeeprev_migrados_model->fechar_periodo($tabela[0]['cd_indicador_tabela'], $this->cd_usuario);
