<?php
class secretaria_rd extends Controller
{
    /*
	var	$label_0 = "Mês";
	var	$label_1 = "Total de RD elaboradas";
	var	$label_2 = "Total de RD divulgadas fora do prazo (10 dias)";
	var	$label_3 = "% de RD divulgadas fora do prazo";
    var	$label_4 = "Meta";
	...
	*/
    ...
    {
        parent::Controller();
        $this->enum_indicador = intval(enum_indicador::SECRETARIA_RD_ENCAMINHADA_DIVULGACAO_FORA_PRAZO);
    ...
    {
		$this->load->helper( array('indicador') );
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'SG' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/secretaria_rd/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'SG' ) )
        {
	        $this->load->model( 'indicador_plugin/secretaria_rd_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->secretaria_rd_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/secretaria_rd/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'SG' ) )
		{
			...
			$this->load->model('indicador_plugin/secretaria_rd_model');
			$row=$this->secretaria_rd_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.secretaria_rd 
			...
			$this->load->view('indicador_plugin/secretaria_rd/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'SG' ) )
		{
			...
			$args['cd_secretaria_rd']=intval($this->input->post('cd_secretaria_rd', true));
			...
			$msg=array();
			$retorno = $this->secretaria_rd_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/secretaria_rd", "refresh" );	
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'SG' ) )
		{
			...
			$this->secretaria_rd_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'SG' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.secretaria_rd
		...
		redirect( 'indicador_plugin/secretaria_rd' );
