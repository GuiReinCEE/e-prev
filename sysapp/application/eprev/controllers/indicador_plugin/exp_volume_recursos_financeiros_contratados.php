<?php
class Exp_volume_recursos_financeiros_contratados extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::EXP_VOLUME_DE_RECURSOS_FINANCEIROS_CONTRATADOS);
		...
		$this->load->model('indicador_plugin/exp_volume_recursos_financeiros_contratados_model');
    }
	...
	public function index()
    {
    	if((indicador_db::verificar_permissao($this->cd_usuario, 'GN')) OR ($this->session->userdata('indic_12') == '*'))
		{
	        ...
	        $this->load->view('indicador_plugin/exp_volume_recursos_financeiros_contratados/index', $data);
		}
		else
        {
	...
	public function listar()
    {
		if((indicador_db::verificar_permissao($this->cd_usuario, 'GN')) OR ($this->session->userdata('indic_12') == '*'))
        {
			...
			$data['collection'] = $this->exp_volume_recursos_financeiros_contratados_model->listar($data['tabela'][0]['cd_indicador_tabela']);
			
			$this->load->view('indicador_plugin/exp_volume_recursos_financeiros_contratados/index_result', $data);
        }
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function cadastro($cd_exp_volume_recursos_financeiros_contratados = 0)
	{
		if((indicador_db::verificar_permissao($this->cd_usuario, 'GN')) OR ($this->session->userdata('indic_12') == '*'))
		{
			$data = array();
			...
			if(intval($cd_exp_volume_recursos_financeiros_contratados) == 0)
			{
				$row = $this->exp_volume_recursos_financeiros_contratados_model->carrega_referencia($data['tabela'][0]['cd_indicador_tabela'], intval($data['tabela'][0]['nr_ano_referencia']));
				
				$data['row'] = array(
					'cd_exp_volume_recursos_financeiros_contratados' => intval($cd_exp_volume_recursos_financeiros_contratados),
			...
			{
				$data['row'] = $this->exp_volume_recursos_financeiros_contratados_model->carrega(intval($cd_exp_volume_recursos_financeiros_contratados));
				$data['row']['qt_ano'] = 1;
			}

			$this->load->view('indicador_plugin/exp_volume_recursos_financeiros_contratados/cadastro', $data);
		}
		else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function salvar()
	{
		...
		{		
			$cd_exp_volume_recursos_financeiros_contratados = intval($this->input->post('cd_exp_volume_recursos_financeiros_contratados', true));
			...
			if(intval($cd_exp_volume_recursos_financeiros_contratados) == 0)
			{
				$this->exp_volume_recursos_financeiros_contratados_model->salvar($args);
			}
			else
			{
				$this->exp_volume_recursos_financeiros_contratados_model->atualizar($cd_exp_volume_recursos_financeiros_contratados, $args);
			}

			$this->criar_indicador();
			
			redirect('indicador_plugin/exp_volume_recursos_financeiros_contratados', 'refresh');
		}
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function excluir($cd_exp_volume_recursos_financeiros_contratados)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GN'))
		{
			$this->exp_volume_recursos_financeiros_contratados_model->excluir(intval($cd_exp_volume_recursos_financeiros_contratados), $this->cd_usuario);
	...
	public function criar_indicador()
    {
    	if((indicador_db::verificar_permissao($this->cd_usuario, 'GN')) OR ($this->session->userdata('indic_12') == '*'))
		{
			...
			$collection = $this->exp_volume_recursos_financeiros_contratados_model->listar($tabela[0]['cd_indicador_tabela']);
			...
			foreach($collection as $key => $item)
			{
				if(intval($item['ano_referencia']) >= intval($tabela[0]['nr_ano_referencia'])-10)
				{
					...
					if((intval($item['ano_referencia']) == intval($tabela[0]['nr_ano_referencia'])) && ($item['fl_media'] != 'S'))
					{
						...
						$indicador[$linha][3] = ($contador_ano_atual > 0 ? $nr_contratado_total : $item['nr_contratado']);
						$indicador[$linha][4] = $item['nr_meta'];
						$indicador[$linha][5] = ($contador_ano_atual > 0 ? $nr_meta_total : $item['nr_meta']);
			...
			$sql.=indicador_db::sql_inserir_grafico(
				...
				'3,3,1,'.$linha_sem_media.';5,5,1,'.$linha_sem_media,
				$this->cd_usuario,
				$coluna_para_ocultar,
				1,
				2
	...
	public function fechar_periodo()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GN'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->exp_volume_recursos_financeiros_contratados_model->fechar_ano($args);
			}

			$this->exp_volume_recursos_financeiros_contratados_model->fechar_indicador($tabela[0]['cd_indicador_tabela'], $this->cd_usuario);
