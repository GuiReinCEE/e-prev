<?php
class Investimento_rentabilidade_crm extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::INVESTIMENTO_RENTABILIDADE_CRM);
		...
		$this->load->model('indicador_plugin/investimento_rentabilidade_crm_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
	        ...
	        $this->load->view('indicador_plugin/investimento_rentabilidade_crm/index',$data);
		}
    }
    ...
    function listar()
    {
    	if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
        {
			...
			$this->investimento_rentabilidade_crm_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/investimento_rentabilidade_crm/index_result', $data);
        }
        else
		{
    ...
    function cadastro($cd_investimento_rentabilidade_crm = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
			$args   = array();
			...
			$args['cd_investimento_rentabilidade_crm'] = $cd_investimento_rentabilidade_crm;
			
			if(intval($args['cd_investimento_rentabilidade_crm']) == 0)
			{
				$this->investimento_rentabilidade_crm_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_investimento_rentabilidade_crm'] = $args['cd_investimento_rentabilidade_crm'];
			...
			{
				$this->investimento_rentabilidade_crm_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/investimento_rentabilidade_crm/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_investimento_rentabilidade_crm'] = intval($this->input->post('cd_investimento_rentabilidade_crm', true));
			...
			$this->investimento_rentabilidade_crm_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/investimento_rentabilidade_crm", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_investimento_rentabilidade_crm)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
			$args   = array();
			...
			$this->investimento_rentabilidade_crm_model->excluir($result, $args);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->investimento_rentabilidade_crm_model->atualiza_fechar_periodo($result, $args);
			}

			$this->investimento_rentabilidade_crm_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
