<?php
class atend_ssocial_indiv extends Controller
{
	var	$label_0 = "Mês";
	var	$label_1 = "Atendimentos";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::ATENDIMENTO_ATENDIMENTOS);
    }
    ...
    function index()
    {
		...
		if( CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
	        ...
	        $this->load->view( 'indicador_plugin/atend_ssocial_indiv/index.php', $data );
		}
    }
    ...
    function listar()
    {
		...
		if( CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
        {
	        $this->load->model( 'indicador_plugin/atend_ssocial_indiv_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->atend_ssocial_indiv_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/atend_ssocial_indiv/partial_result', $data);
			}
			else
			{
	...
	function detalhe( $cd=0 )
	{
		...
		if( CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->load->model('indicador_plugin/atend_ssocial_indiv_model');
			$row=$this->atend_ssocial_indiv_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
			...
			$this->load->view('indicador_plugin/atend_ssocial_indiv/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if( CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$args['cd_atend_ssocial_indiv']=intval($this->input->post('cd_atend_ssocial_indiv', true));
			...
			$msg=array();
			$retorno=$this->atend_ssocial_indiv_model->salvar( $args, $msg );
			...
			if( $retorno )
			{
				...
				{
					redirect( "indicador_plugin/atend_ssocial_indiv", "refresh" );
				}
			}
			else
			{
	...
	function excluir( $id )
	{
		...
		if( CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->atend_ssocial_indiv_model->excluir($id);
	...
	function criar_indicador()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			{#tabela_existe
				...
				#echo "<PRE>".print_r($indicador,true)."</PRE>";	exit;
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			{#tabela_existe
				...
				foreach( $collection as $item )
				{
					...
					if($item["fl_media"] != "S")
					{
						$acumulado+=floatval($nr_valor_1);
					}
				}

				$sql="";

				// gravar somatório do ano e fechar o período para o indicador
				$sql.=sprintf("
				INSERT INTO indicador_plugin.atend_ssocial_indiv (
					dt_referencia,dt_inclusao,cd_usuario_inclusao, nr_valor_1, fl_media ) 
				VALUES ( 
					'%s/01/01', current_timestamp, %s, %s, 'S' )
				;" , intval($tabela[0]['nr_ano_referencia']), usuario_id(), floatval($acumulado) );
		...
		redirect( 'indicador_plugin/atend_ssocial_indiv' );
