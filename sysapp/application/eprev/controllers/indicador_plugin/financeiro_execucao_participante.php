<?php
class financeiro_execucao_participante extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Valor da Carga de Cobrança";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::FINANCEIRO_EXECUCAO_TOTAL_PARTICIPANTE);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/financeiro_execucao_participante/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
        {
	        $this->load->model( 'indicador_plugin/Financeiro_execucao_participante_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->Financeiro_execucao_participante_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/financeiro_execucao_participante/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$this->load->model('indicador_plugin/Financeiro_execucao_participante_model');
			$row=$this->Financeiro_execucao_participante_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.financeiro_execucao_participante 
			...
			$this->load->view('indicador_plugin/financeiro_execucao_participante/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$args['cd_financeiro_execucao_participante']=intval($this->input->post('cd_financeiro_execucao_participante', true));
			...
			$msg=array();
			$retorno = $this->Financeiro_execucao_participante_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/financeiro_execucao_participante", "refresh" );
				}
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$this->Financeiro_execucao_participante_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.financeiro_execucao_participante
		...
		redirect( 'indicador_plugin/financeiro_execucao_participante' );
