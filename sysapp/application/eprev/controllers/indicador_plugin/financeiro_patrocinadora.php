<?php
class financeiro_patrocinadora extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Valor da Carga de Cobrança Total";
	var	$label_2 = "Execução Financeira";
	var	$label_3 = "% de Execução";
    var $label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::FINANCEIRO_EXECUCAO_TOTAL_DAS_PATROCINADORAS);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/financeiro_patrocinadora/index.php');
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
        {
	        $this->load->model( 'indicador_plugin/Financeiro_patrocinadora_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->Financeiro_patrocinadora_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/financeiro_patrocinadora/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$this->load->model('indicador_plugin/Financeiro_patrocinadora_model');
			$row=$this->Financeiro_patrocinadora_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
			...
			$this->load->view('indicador_plugin/financeiro_patrocinadora/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$args['cd_financeiro_patrocinadora']=intval($this->input->post('cd_financeiro_patrocinadora', true));
			...
			$args["nr_execucao"] = app_decimal_para_db($this->input->post("nr_execucao", true));
			$args["nr_meta"] = app_decimal_para_db($this->input->post("nr_meta", true));
            $args["observacao"] = $this->input->post("observacao", true);

			$msg=array();
			$retorno = $this->Financeiro_patrocinadora_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/financeiro_patrocinadora", "refresh" );
				}	
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$this->Financeiro_patrocinadora_model->excluir($id);
				...
				foreach( $collection as $item )
				{
					...
					if( intval($item['ano_referencia'])>=intval($tabela[0]['nr_ano_referencia'])-5 )
					{
						...
						else
						{
							...
							$nr_execucao = $item["nr_execucao"];
							
							if( floatval($nr_valor)>0 )
							{
								$nr_percentual_f = ( floatval($nr_execucao)/floatval($nr_valor) ) * 100;
							}
							else
							{
								$nr_percentual_f = '0';
						...
						$indicador[$linha][1] = app_decimal_para_php($nr_valor);
						$indicador[$linha][2] = app_decimal_para_php($nr_execucao);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.financeiro_patrocinadora
		...
		redirect( 'indicador_plugin/financeiro_patrocinadora' );
