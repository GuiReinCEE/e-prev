<?php
class financeiro_adm_contratos extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Nr contratos Ativos";
	var	$label_2 = "Nr Contratos em Atraso";
	var	$label_3 = "% de contratos atrasados";
    var $label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::FINANCEIRO_ATRASOS_ADMINISTRATIVO_CONTRATOS);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/financeiro_adm_contratos/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
        {
	        $this->load->model( 'indicador_plugin/financeiro_adm_contratos_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->financeiro_adm_contratos_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/financeiro_adm_contratos/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$this->load->model('indicador_plugin/financeiro_adm_contratos_model');
			$row=$this->financeiro_adm_contratos_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.financeiro_adm_contratos 
			...
			$this->load->view('indicador_plugin/financeiro_adm_contratos/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$args['cd_financeiro_adm_contratos']=intval($this->input->post('cd_financeiro_adm_contratos', true));
			...
			$msg=array();
			$retorno = $this->financeiro_adm_contratos_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/financeiro_adm_contratos", "refresh" );	
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$this->financeiro_adm_contratos_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.financeiro_adm_contratos
		...
		redirect( 'indicador_plugin/financeiro_adm_contratos' );
