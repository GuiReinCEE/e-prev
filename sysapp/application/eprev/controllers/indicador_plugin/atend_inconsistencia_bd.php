<?php
class atend_inconsistencia_bd extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Total de inconsistências";
	var	$label_2 = "Total inform.banco de dados";
	var	$label_3 = "% de Inconsistência";
    var $label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::ATENDIMENTO_INCONSISTENCIAS_NO_BANCO_DE_DADOS);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/atend_inconsistencia_bd/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
        {
	        ...
	        $this->load->model( 'indicador_plugin/Atend_inconsistencia_bd_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->Atend_inconsistencia_bd_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/atend_inconsistencia_bd/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
		{
			...
			$this->load->model('indicador_plugin/Atend_inconsistencia_bd_model');
			$row=$this->Atend_inconsistencia_bd_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.atend_inconsistencia_bd 
			...
			$this->load->view('indicador_plugin/atend_inconsistencia_bd/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
		{
			...
			$args['cd_atend_inconsistencia_bd']=intval($this->input->post('cd_atend_inconsistencia_bd', true));
			...
			$msg=array();
			$retorno = $this->Atend_inconsistencia_bd_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/atend_inconsistencia_bd", "refresh" );
				}
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
		{
			...
			$this->Atend_inconsistencia_bd_model->excluir($id);
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$media = number_format(( $media / sizeof($media_ano) ),3 );
					...
					$indicador[$linha][1] = app_decimal_para_php($nr_media_1/sizeof($media_ano));
					$indicador[$linha][2] = app_decimal_para_php($nr_media_2/sizeof($media_ano));
				...
				for( $i=0; $i<sizeof($indicador); $i++ )
				{
					...
					$sql .= indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 3, $linha, app_decimal_para_php($indicador[$i][3]), 'center', 'S', 3, 'S' );
					$sql .= indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 4, $linha, app_decimal_para_php($indicador[$i][4]), 'center', 'S', 3, 'S' );
					$sql .= indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 5, $linha, app_decimal_para_php($indicador[$i][5]), 'center', 'S', 3, 'S' );
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.atend_inconsistencia_bd
		...
		redirect( 'indicador_plugin/atend_inconsistencia_bd' );
