<?php
class administrativo_impressao extends Controller
{
    var	$label_0 = "Mês";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RH_VALOR_REALIZADO_IMPRESSAO);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/administrativo_impressao/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
        {
	        $this->load->model( 'indicador_plugin/administrativo_impressao_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->administrativo_impressao_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/administrativo_impressao/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$this->load->model('indicador_plugin/administrativo_impressao_model');
			$row=$this->administrativo_impressao_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.administrativo_impressao 
			...
			$this->load->view('indicador_plugin/administrativo_impressao/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$args['cd_administrativo_impressao']=intval($this->input->post('cd_administrativo_impressao', true));
			...
			$msg=array();
			$retorno = $this->administrativo_impressao_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/administrativo_impressao", "refresh" );			
				}
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$this->administrativo_impressao_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.administrativo_impressao
		...
		redirect( 'indicador_plugin/administrativo_impressao' );
