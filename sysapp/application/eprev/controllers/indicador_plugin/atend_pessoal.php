<?php
class atend_pessoal extends Controller
{
    var	$label_0 = "Mês";
    var $label_1 = 'Atendimento por RE';
	var	$label_2 = "Tempo Total para Atender";
	var	$label_3 = "Número de Atendimentos";
	var	$label_4 = "Tempo Médio";
    var $label_5 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::ATENDIMENTO_TEMPO_MEDIO_DE_ATENDIMENTO_PESSOAL);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/atend_pessoal/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
        {
	        ...
	        $this->load->model( 'indicador_plugin/Atend_pessoal_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->Atend_pessoal_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/atend_pessoal/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
		{
			...
			$this->load->model('indicador_plugin/Atend_pessoal_model');
			$row=$this->Atend_pessoal_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					$sql = "
					SELECT to_char(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS mes_referencia, dt_referencia, nr_meta, cd_indicador_tabela, nr_percentual_f
					FROM indicador_plugin.atend_pessoal 
			...
			$this->load->view('indicador_plugin/atend_pessoal/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
		{
			...
			$args['cd_atend_pessoal']    = intval($this->input->post('cd_atend_pessoal', true));
			...
			$args["nr_atendimento_re"]   = app_decimal_para_db($this->input->post("nr_atendimento_re", true));
			...
			$msg=array();
			$retorno = $this->Atend_pessoal_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/atend_pessoal", "refresh" );
				}
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
		{
			...
			$this->Atend_pessoal_model->excluir($id);
			...
			{#tabela_existe
				...
				foreach( $collection as $item )
				{
					...
					if( intval($item['ano_referencia'])>=intval($tabela[0]['nr_ano_referencia'])-5 )
					{
						...
						$indicador[$linha][1] = $item['nr_atendimento_re'];
				...
				for( $i=0; $i<sizeof($indicador); $i++ )
				{
                    ...
                    $sql .= indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 6, $linha, app_decimal_para_php($indicador[$i][6]), 'left');
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" 
						INSERT INTO indicador_plugin.atend_pessoal
							( 
								dt_referencia,
								dt_inclusao,
								cd_usuario_inclusao, 
		...
		redirect( 'indicador_plugin/atend_pessoal' );
