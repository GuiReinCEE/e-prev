<?php
class controladoria_inf_gerencial extends Controller
{
	var	$label_0 = "Trimestre";
	var	$label_1 = "Questões Respondidas";
	var	$label_2 = "Satisfeitos";
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::CONTROLADORIA_SATISFACAO_INFORMATIVO_GERENCIAL);
    }
    ...
    function index()
    {
		...
		if(indicador_db::verificar_permissao( usuario_id(), 'GC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/controladoria_inf_gerencial/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(indicador_db::verificar_permissao( usuario_id(), 'GC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
        {
	        $this->load->model( 'indicador_plugin/controladoria_inf_gerencial_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->controladoria_inf_gerencial_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/controladoria_inf_gerencial/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(indicador_db::verificar_permissao( usuario_id(), 'GC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$this->load->model('indicador_plugin/controladoria_inf_gerencial_model');
			$row=$this->controladoria_inf_gerencial_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
			...
			$this->load->view('indicador_plugin/controladoria_inf_gerencial/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(indicador_db::verificar_permissao( usuario_id(), 'GC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$args['cd_controladoria_inf_gerencial']=intval($this->input->post('cd_controladoria_inf_gerencial', true));
			...
			$msg=array();
			$retorno = $this->controladoria_inf_gerencial_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/controladoria_inf_gerencial", "refresh" );	
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(indicador_db::verificar_permissao( usuario_id(), 'GC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$this->controladoria_inf_gerencial_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(indicador_db::verificar_permissao( usuario_id(), 'GC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.controladoria_inf_gerencial
		...
		redirect( 'indicador_plugin/controladoria_inf_gerencial' );
