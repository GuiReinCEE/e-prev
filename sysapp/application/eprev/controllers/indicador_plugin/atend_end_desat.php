<?php
class atend_end_desat extends Controller
{
	var	$label_0 = "Mês";
	var	$label_1 = "Total de endereços desatualizados";
	var	$label_2 = "Total de participantes";
	var	$label_3 = "% de endereços desatualizados"; 
    var $label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::ATENDIMENTO_ENDERECOS_DESATUALIZADOS);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/atend_end_desat/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
        {
	        ...
	        $this->load->model( 'indicador_plugin/Atend_end_desat_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->Atend_end_desat_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/atend_end_desat/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
		{
			...
			$this->load->model('indicador_plugin/Atend_end_desat_model');
			$row=$this->Atend_end_desat_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					(SELECT COUNT(*)
				      FROM indicador_plugin.atend_end_desat
			...
			$this->load->view('indicador_plugin/atend_end_desat/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
		{
			...
			$args['cd_atend_end_desat']=intval($this->input->post('cd_atend_end_desat', true));
			...
			$msg=array();
			$retorno = $this->Atend_end_desat_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/atend_end_desat", "refresh" );
				}
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
		{
			...
			$this->Atend_end_desat_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.atend_end_desat
		...
		redirect( 'indicador_plugin/atend_end_desat' );
