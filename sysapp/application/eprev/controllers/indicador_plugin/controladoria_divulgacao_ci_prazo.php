<?php
class Controladoria_divulgacao_ci_prazo extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::CONTROLADORIA_DIVULGACAO_RESULTADO_CI_PRAZO);
		...
		$this->load->model('indicador_plugin/controladoria_divulgacao_ci_prazo_model');
    }
    ...
    public function index()
    {
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GC'))
		{
	        ...
	        $this->load->view('indicador_plugin/controladoria_divulgacao_ci_prazo/index', $data);
		}
		else
		{
    ...
    public function listar()
    {
        ...
        {			
			...
			$data['collection'] = $this->controladoria_divulgacao_ci_prazo_model->listar($data['tabela'][0]['cd_indicador_tabela'] );
			
			$this->load->view('indicador_plugin/controladoria_divulgacao_ci_prazo/index_result', $data);
        }
        else
		{
			exibir_mensagem('ACESSO NÃO PERMITIDO');
    ...
    public function cadastro($cd_controladoria_divulgacao_ci_prazo = 0)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GC'))
		{			
			$data['label_0'] = $this->label_0;
			...
			if(intval($cd_controladoria_divulgacao_ci_prazo) == 0)
			{
				$row = $this->controladoria_divulgacao_ci_prazo_model->carrega_referencia(intval($data['tabela'][0]['nr_ano_referencia']));
				
				$data['row'] = array(
					'cd_controladoria_divulgacao_ci_prazo' => intval($cd_controladoria_divulgacao_ci_prazo),
				    'fl_media'             		                  => '',
				    'nr_total'                           => 0,
				    'nr_atendidas'                      => 0,
			...
			{
				$data['row'] = $this->controladoria_divulgacao_ci_prazo_model->carrega($cd_controladoria_divulgacao_ci_prazo);
				$data['row']['qt_ano'] = 1;
			}

			$this->load->view('indicador_plugin/controladoria_divulgacao_ci_prazo/cadastro', $data);
		}
		else
		{
			exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function salvar()
	{
		...
		{		
			...
			$args = array(
				'cd_controladoria_divulgacao_ci_prazo' => intval($this->input->post('cd_controladoria_divulgacao_ci_prazo', true)),
			...
			$this->controladoria_divulgacao_ci_prazo_model->salvar($args);
			
			$this->criar_indicador();
	
			redirect('indicador_plugin/controladoria_divulgacao_ci_prazo', 'refresh');
		}
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function excluir($cd_controladoria_divulgacao_ci_prazo)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GC'))
		{
			$this->controladoria_divulgacao_ci_prazo_model->excluir($cd_controladoria_divulgacao_ci_prazo, $this->cd_usuario);
	...
	public function criar_indicador()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GC'))
		{
			...
			$collection = $this->controladoria_divulgacao_ci_prazo_model->listar($tabela[0]['cd_indicador_tabela']);
	...
	public function fechar_periodo()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$args = array(
					'cd_controladoria_divulgacao_ci_prazo' => 0, 
			...
			$this->controladoria_divulgacao_ci_prazo_model->fechar_periodo($tabela[0]['cd_indicador_tabela'], $this->cd_usuario);
