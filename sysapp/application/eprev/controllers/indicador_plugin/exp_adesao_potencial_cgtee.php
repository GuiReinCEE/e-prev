<?php
class Exp_adesao_potencial_cgtee extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::EXP_NUMERO_DE_ADESAO_X_NUMERO_POTENCIAL_CGTEE);
		...
		$this->load->model('indicador_plugin/exp_adesao_potencial_cgtee_model');
    }
    ...
    function index()
    {
		if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{
	        ...
	        $this->load->view('indicador_plugin/exp_adesao_potencial_cgtee/index',$data);
		}
    }
    ...
    function listar()
    {
    	if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
        {
			...
			$this->exp_adesao_potencial_cgtee_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/exp_adesao_potencial_cgtee/index_result', $data);
        }
        else
		{
    ...
    function cadastro($cd_exp_adesao_potencial_cgtee = 0)
	{
		if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{
			$args   = array();
			...
			$args['cd_exp_adesao_potencial_cgtee'] = $cd_exp_adesao_potencial_cgtee;
			
			if(intval($args['cd_exp_adesao_potencial_cgtee']) == 0)
			{
				$this->exp_adesao_potencial_cgtee_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_exp_adesao_potencial_cgtee'] = $args['cd_exp_adesao_potencial_cgtee'];
			...
			{
				$this->exp_adesao_potencial_cgtee_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/exp_adesao_potencial_cgtee/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_exp_adesao_potencial_cgtee'] = intval($this->input->post('cd_exp_adesao_potencial_cgtee', true));
			...
			$this->exp_adesao_potencial_cgtee_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/exp_adesao_potencial_cgtee", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_exp_adesao_potencial_cgtee)
	{
		if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{
			$args   = array();
			...
			$this->exp_adesao_potencial_cgtee_model->excluir($result, $args);
	...
	function fechar_periodo()
	{
		if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->exp_adesao_potencial_cgtee_model->atualiza_fechar_periodo($result, $args);
			}

			$this->exp_adesao_potencial_cgtee_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
