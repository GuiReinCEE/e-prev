<?php
class financeiro_seguro extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Coberturas Pagas";
	var	$label_2 = "Coberturas pagas fora do Prazo";
	var	$label_3 = "% Pagas fora do Prazo";
    var $label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::FINANCEIRO_COBERTURAS_PAGAS_FORA_DO_PRAZO);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/financeiro_seguro/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
        {
	        $this->load->model( 'indicador_plugin/financeiro_seguro_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->financeiro_seguro_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/financeiro_seguro/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$this->load->model('indicador_plugin/financeiro_seguro_model');
			$row=$this->financeiro_seguro_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.financeiro_seguro 
			...
			$this->load->view('indicador_plugin/financeiro_seguro/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$args['cd_financeiro_seguro']=intval($this->input->post('cd_financeiro_seguro', true));
			...
			$msg=array();
			$retorno = $this->financeiro_seguro_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/financeiro_seguro", "refresh" );	
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$this->financeiro_seguro_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.financeiro_seguro
		...
		redirect( 'indicador_plugin/financeiro_seguro' );
