<?php
class ri_sat_fun_revista extends Controller
{
	var	$label_0 = "Ano";
	var	$label_1 = "Total de pesquisas respondidas";
	var	$label_2 = "Total de pessoas Satisfeitas";
	var	$label_3 = "% de satisfação com a Revista";
    var	$label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RI_SATISFACAO_COM_FUNDACAO_EM_REVISTA);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
	        ...
	        $this->load->view('indicador_plugin/ri_sat_fun_revista/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
        {
	        $this->load->model( 'indicador_plugin/ri_sat_fun_revista_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->ri_sat_fun_revista_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/ri_sat_fun_revista/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$this->load->model('indicador_plugin/ri_sat_fun_revista_model');
			$row=$this->ri_sat_fun_revista_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
			...
			$this->load->view('indicador_plugin/ri_sat_fun_revista/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$args['cd_ri_sat_fun_revista']=intval($this->input->post('cd_ri_sat_fun_revista', true));
			...
			$msg=array();
			$retorno = $this->ri_sat_fun_revista_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/ri_sat_fun_revista", "refresh" );
					#redirect( "indicador_plugin/ri_sat_fun_revista/detalhe/0", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$this->ri_sat_fun_revista_model->excluir($id);
