<?php
class ri_acessos_boletim_em_pauta extends Controller
{
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RI_ACESSOS_BOLETIM_EM_PAUTA);
		
		$this->load->helper(array('indicador'));
		
		CheckLogin();
		
		$this->load->model('indicador_plugin/ri_acessos_boletim_em_pauta_model');
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(), 'AC') || indicador_db::verificar_permissao(usuario_id(), 'GE'))
		{
	        ...
	        $this->load->view('indicador_plugin/ri_acessos_boletim_em_pauta/index',$data);
		}
    }
    ...
    function listar()
    {
		if(indicador_db::verificar_permissao(usuario_id(), 'AC') || indicador_db::verificar_permissao(usuario_id(), 'GE'))
        {
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->ri_acessos_boletim_em_pauta_model->listar($result, $args);
				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/ri_acessos_boletim_em_pauta/partial_result', $data);
			}
			else
			{
	...
	function cadastro($cd = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(), 'AC') || indicador_db::verificar_permissao(usuario_id(), 'GE'))
		{
			...
			if(intval($args['cd']) == 0)
			{
				$this->ri_acessos_boletim_em_pauta_model->carrega_novo($result, $args);
				$row = $result->row_array();
				
				$data['row'] = array(
					'cd_ri_acessos_boletim_em_pauta'  => $cd,
			...
			{
				$this->ri_acessos_boletim_em_pauta_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}
			
			$this->load->view('indicador_plugin/ri_acessos_boletim_em_pauta/cadastro', $data);
		}
	}
	...
	function salvar()
	{
		if(indicador_db::verificar_permissao( usuario_id(), 'AC' ) )
		{
			...
			$args['cd_ri_acessos_boletim_em_pauta']  = intval($this->input->post('cd_ri_acessos_boletim_em_pauta', true));
			...
			$this->ri_acessos_boletim_em_pauta_model->salvar($result, $args);
			...
			{
				redirect("indicador_plugin/ri_acessos_boletim_em_pauta", "refresh");
			}
		}
	}
	...
	function excluir($cd)
	{
		if(indicador_db::verificar_permissao( usuario_id(), 'AC' ) )
		{
			...
			$this->ri_acessos_boletim_em_pauta_model->excluir($result, $args);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(), 'AC') || indicador_db::verificar_permissao(usuario_id(), 'GE'))
		{
			...
			else
			{
				...
				$this->ri_acessos_boletim_em_pauta_model->fecha_indicador( $result, $args );
			}
		}

		redirect( 'indicador_plugin/ri_acessos_boletim_em_pauta' );
	}
}
?>