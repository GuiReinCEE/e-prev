<?php
class Administrativo_contratacao_servico extends Controller
{
	var	$label_0 = "Ano";
	var	$label_1 = "";
	var	$label_2 = "";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RH_MEDIA_AVALIACAO_CONTRATACAO_SERVICO_ANUAL);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GTI' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/administrativo_contratacao_servico/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GTI' ) )
        {
	        $this->load->model( 'indicador_plugin/administrativo_contratacao_servico_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->administrativo_contratacao_servico_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/administrativo_contratacao_servico/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GTI' ) )
		{
			...
			$this->load->model('indicador_plugin/administrativo_contratacao_servico_model');
			$row=$this->administrativo_contratacao_servico_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
			...
			$this->load->view('indicador_plugin/administrativo_contratacao_servico/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GTI' ) )
		{
			...
			$args['cd_administrativo_contratacao_servico']=intval($this->input->post('cd_administrativo_contratacao_servico', true));
			$args["dt_referencia"] = $this->input->post("dt_referencia", true);
			$args["cd_usuario_inclusao"] = usuario_id();
			$args["cd_indicador_tabela"] = $this->input->post("cd_indicador_tabela", true);
			$args["fl_media"] = $this->input->post("fl_media", true);

			$args["nr_percentual_f"] = app_decimal_para_db($this->input->post("nr_percentual_f", true));
			$args["nr_meta"] = app_decimal_para_db($this->input->post("nr_meta", true));
            $args["observacao"] = $this->input->post("observacao", true);

			$msg=array();
			$retorno = $this->administrativo_contratacao_servico_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/administrativo_contratacao_servico", "refresh" );	
					#redirect( "indicador_plugin/administrativo_contratacao_servico", "refresh" );	
				}				
			}
			else
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GTI' ) )
		{
			...
			$this->administrativo_contratacao_servico_model->excluir($id);
	...
	function criar_indicador()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GTI' ) )
		{
			...
			{#tabela_existe
				...
				$sql.=indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 1,0, utf8_encode($data['label_3']), 'background,center');
                ...
                $sql.=indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 4,0, utf8_encode(nl2br($data['label_6'])), 'background,center');
				...
				foreach( $collection as $item )
				{
					...
					if(true)
					{
                        ...
                        $indicador[$linha][6] = nl2br($observacao);
				...
				for( $i=0; $i<sizeof($indicador); $i++ )
				{
					...
					$sql .= indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 1, $linha, app_decimal_para_php($indicador[$i][3]), 'center', 'S', 2);
					$sql .= indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 2, $linha, app_decimal_para_php($indicador[$i][4]), 'center', 'S', 2, 'S' );
					$sql .= indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 3, $linha, app_decimal_para_php($indicador[$i][5]), 'center', 'S', 2, 'S' );
                    $sql .= indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 4, $linha, utf8_encode(nl2br($indicador[$i][6])), 'left');
