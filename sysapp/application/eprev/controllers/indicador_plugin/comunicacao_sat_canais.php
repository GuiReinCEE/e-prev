<?php
class Comunicacao_sat_canais extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::COMUNICACAO_SATISFACAO_COM_CANAIS_COMUNICACAO);
		...
		$this->load->model('indicador_plugin/comunicacao_sat_canais_model');
    }
	...
	function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'AC'))
		{
	        ...
	        $this->load->view('indicador_plugin/comunicacao_sat_canais/index', $data);
		}
		else
		{
	...
	function listar()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'AC'))
        {
			...
			$this->comunicacao_sat_canais_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/comunicacao_sat_canais/index_result', $data);
        }
        else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function cadastro($cd_comunicacao_sat_canais = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'AC'))
		{
			$args   = array();
			...
			$args['cd_comunicacao_sat_canais'] = $cd_comunicacao_sat_canais;
			
			if(intval($args['cd_comunicacao_sat_canais']) == 0)
			{
				$this->comunicacao_sat_canais_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_comunicacao_sat_canais'] = $args['cd_comunicacao_sat_canais'];
			...
			{
				$this->comunicacao_sat_canais_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/comunicacao_sat_canais/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_comunicacao_sat_canais'] = intval($this->input->post('cd_comunicacao_sat_canais', true));
			...
			$this->comunicacao_sat_canais_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/comunicacao_sat_canais", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_comunicacao_sat_canais)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'AC'))
		{
			$args   = array();
			...
			$this->comunicacao_sat_canais_model->excluir($result, $args);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'AC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->comunicacao_sat_canais_model->atualiza_fechar_periodo($result, $args);
			}

			$this->comunicacao_sat_canais_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
