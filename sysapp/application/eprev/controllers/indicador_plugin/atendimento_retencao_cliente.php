<?php
class Atendimento_retencao_cliente extends Controller
{
    function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::ATENDIMENTO_RETENCAO_CLIENTE);
		...
		$this->load->model('indicador_plugin/atendimento_retencao_cliente_model');
    }
    ...
    public function index()
    {
        if(indicador_db::verificar_permissao($this->cd_usuario, 'GCM'))
        {
            ...
            $this->load->view('indicador_plugin/atendimento_retencao_cliente/index', $data);
        }
        else
        {
    ...
    public function listar()
    {
        if(indicador_db::verificar_permissao($this->cd_usuario, 'GCM'))
        {
            ...
            $data['collection'] = $this->atendimento_retencao_cliente_model->listar($data['tabela'][0]['cd_indicador_tabela'] );

            $this->load->view('indicador_plugin/atendimento_retencao_cliente/index_result', $data);
        }
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
    ...
    public function cadastro($cd_atendimento_retencao_cliente = 0)
    {
        if(indicador_db::verificar_permissao($this->cd_usuario, 'GCM'))
        {
            $data['label_0'] = $this->label_0;
            ...
            if(intval($cd_atendimento_retencao_cliente) == 0)
            {
                $row = $this->atendimento_retencao_cliente_model->carrega_referencia(intval($data['tabela'][0]['nr_ano_referencia']));
                    
                $data['row'] = array(
                    'cd_atendimento_retencao_cliente' => intval($cd_atendimento_retencao_cliente),
                    'ds_observacao'                   => '',
            ...
            {
                $data['row'] = $this->atendimento_retencao_cliente_model->carrega($cd_atendimento_retencao_cliente);
				$data['row']['qt_ano'] = 1;
            }

            $this->load->view('indicador_plugin/atendimento_retencao_cliente/cadastro', $data);
        }
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	{
        /*
        $row = $this->atendimento_retencao_cliente_model->get_valores($ano, $mes);
    ...
    public function salvar()
    {
        if(indicador_db::verificar_permissao($this->cd_usuario, 'GCM'))
        {
            $cd_atendimento_retencao_cliente = $this->input->post('cd_atendimento_retencao_cliente', TRUE);
            ...
            if(intval($cd_atendimento_retencao_cliente) == 0)
            {
                $this->atendimento_retencao_cliente_model->salvar($args);
            }
            else
            {
                $this->atendimento_retencao_cliente_model->atualizar($cd_atendimento_retencao_cliente, $args);
            }

            $this->criar_indicador();

            redirect('indicador_plugin/atendimento_retencao_cliente', 'refresh');
        }
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
    ...
    public function excluir($cd_atendimento_retencao_cliente)
    {
        if(indicador_db::verificar_permissao($this->cd_usuario, 'GCM'))
        {
            $this->atendimento_retencao_cliente_model->excluir(
                $cd_atendimento_retencao_cliente, 
    ...
    public function criar_indicador()
    {
        if(indicador_db::verificar_permissao($this->cd_usuario, 'GCM'))
        {
            ...
            $collection = $this->atendimento_retencao_cliente_model->listar($tabela[0]['cd_indicador_tabela']);
            ...
            foreach($collection as $item)
            {
                ...
                $indicador[$linha][1]  = $item['nr_interesse'];
                $indicador[$linha][2]  = $item['nr_cliente'];
                $indicador[$linha][3]  = $item['nr_efetivas'];
                $indicador[$linha][4]  = $item['nr_nao_retido'];
                $indicador[$linha][5]  = $item['nr_negociacao'];
            ...
            if(intval($contador_ano_atual) > 0)
            {
                ...
                $indicador[$linha][1]  = $nr_total_interesse;
                $indicador[$linha][2]  = $nr_total_cliente;
                $indicador[$linha][3]  = $nr_total_efetivas;
                $indicador[$linha][4]  = $nr_total_nao_retido;
                $indicador[$linha][5]  = $nr_total_negociacao;
            ...
            for($i=0; $i<sizeof($indicador); $i++)
            {
                ...
                $sql .= indicador_db::sql_inserir_celula($tabela[0]['cd_indicador_tabela'], 8, $linha, utf8_encode($indicador[$i][8]), 'justify');
    ...
    public function fechar_periodo()
    {
        if(indicador_db::verificar_permissao($this->cd_usuario, 'GCM'))
        {
            ...
            if(intval($contador_ano_atual) > 0)
            {
                ...
                $args = array(
                    'cd_atendimento_retencao_cliente' => 0, 
            ...
            $this->atendimento_retencao_cliente_model->fechar_periodo($tabela[0]['cd_indicador_tabela'], $this->cd_usuario);
