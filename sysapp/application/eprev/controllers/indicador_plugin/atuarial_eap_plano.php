<?php
class atuarial_eap_plano extends Controller
{
	var	$label_0 = "Ano";
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
	        ...
	        $this->load->view('indicador_plugin/atuarial_eap_plano/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
        {
	        $this->load->model( 'indicador_plugin/atuarial_eap_plano_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->atuarial_eap_plano_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/atuarial_eap_plano/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
			...
			$this->load->model('indicador_plugin/atuarial_eap_plano_model');
			$row=$this->atuarial_eap_plano_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.atuarial_eap_plano 
			...
			$this->load->view('indicador_plugin/atuarial_eap_plano/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
			...
			$args['cd_atuarial_eap_plano']=intval($this->input->post('cd_atuarial_eap_plano', true));
			...
			$msg=array();
			$retorno = $this->atuarial_eap_plano_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/atuarial_eap_plano", "refresh" );	
					#redirect( "indicador_plugin/atuarial_eap_plano/detalhe/0", "refresh" );	
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
			...
			$this->atuarial_eap_plano_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.atuarial_eap_plano
				...
				*/
				...
				// indicar que o período foi fechado para o indicador_tabela
				$sql=" 
						INSERT INTO indicador_plugin.atuarial_eap_plano
							 ( 
							   dt_referencia,
							   dt_inclusao,
							   cd_usuario_inclusao, 
							 ...
							 (
								SELECT aep.dt_referencia,
									   ...
									   aep.nr_meta,
									   'S'
								  FROM indicador_plugin.atuarial_eap_plano aep
								 WHERE aep.dt_exclusao IS NULL
								   ...
								   AND aep.dt_referencia = (SELECT MAX(eap1.dt_referencia) 
															  FROM indicador_plugin.atuarial_eap_plano eap1
		...
		redirect( 'indicador_plugin/atuarial_eap_plano' );
