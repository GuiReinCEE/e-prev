<?php
class Exp_volume_recursos_cgtee extends Controller
{	
	var $enum_indicador = 3;
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::EXP_VOLUME_DE_RECURSOS_FINANCEIROS_CONTRATADOS_CGTEE);
		...
		$this->load->model('indicador_plugin/exp_volume_recursos_cgtee_model');
    }
	...
	public function index()
    {
    	if((indicador_db::verificar_permissao($this->cd_usuario, 'GE')) OR ($this->session->userdata('indic_12') == '*'))
		{
	        ...
	        $this->load->view('indicador_plugin/exp_volume_recursos_cgtee/index', $data);
		}
		else
        {
	...
	public function listar()
    {
		if((indicador_db::verificar_permissao($this->cd_usuario, 'GE')) OR ($this->session->userdata('indic_12') == '*'))
        {
			...
			$data['collection'] = $this->exp_volume_recursos_cgtee_model->listar($data['tabela'][0]['cd_indicador_tabela']);
			
			$this->load->view('indicador_plugin/exp_volume_recursos_cgtee/index_result', $data);
        }
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function cadastro($cd_exp_volume_recursos_cgtee = 0)
	{
		if((indicador_db::verificar_permissao($this->cd_usuario, 'GE')) OR ($this->session->userdata('indic_12') == '*'))
		{
			$data = array();
			...
			if(intval($cd_exp_volume_recursos_cgtee) == 0)
			{
				$row = $this->exp_volume_recursos_cgtee_model->carrega_referencia($data['tabela'][0]['cd_indicador_tabela']);
				
				$data['row'] = array(
					'cd_exp_volume_recursos_cgtee' => intval($cd_exp_volume_recursos_cgtee),
			...
			{
				$data['row'] = $this->exp_volume_recursos_cgtee_model->carrega(intval($cd_exp_volume_recursos_cgtee));
			}

			$this->load->view('indicador_plugin/exp_volume_recursos_cgtee/cadastro', $data);
		}
		else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function salvar()
	{
		...
		{		
			$cd_exp_volume_recursos_cgtee = intval($this->input->post('cd_exp_volume_recursos_cgtee', true));
			...
			if(intval($cd_exp_volume_recursos_cgtee) == 0)
			{
				$this->exp_volume_recursos_cgtee_model->salvar($args);
			}
			else
			{
				$this->exp_volume_recursos_cgtee_model->atualizar($cd_exp_volume_recursos_cgtee, $args);
			}

			$this->criar_indicador();
			
			redirect('indicador_plugin/exp_volume_recursos_cgtee', 'refresh');
		}
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function excluir($cd_exp_volume_recursos_cgtee)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GE'))
		{
			$this->exp_volume_recursos_cgtee_model->excluir(intval($cd_exp_volume_recursos_cgtee), $this->cd_usuario);
	...
	public function criar_indicador()
    {
    	if((indicador_db::verificar_permissao($this->cd_usuario, 'GE')) OR ($this->session->userdata('indic_12') == '*'))
		{
			...
			$collection = $this->exp_volume_recursos_cgtee_model->listar($tabela[0]['cd_indicador_tabela']);
	...
	public function fechar_periodo()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GE'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->exp_volume_recursos_cgtee_model->fechar_ano($args);
			}

			$this->exp_volume_recursos_cgtee_model->fechar_indicador($tabela[0]['cd_indicador_tabela'], $this->cd_usuario);
