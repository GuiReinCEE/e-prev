<?php
class atend_desligamento extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Total de desligamentos dos planos";
	var	$label_2 = "Total de participantes";
	var	$label_3 = "% de desligamentos";
    var $label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::ATENDIMENTO_DESLIGAMENTOS_DE_PARTICIPANTES_DOS_PLANOS);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/atend_desligamento/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
        {
	        $this->load->model( 'indicador_plugin/Atend_desligamento_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->Atend_desligamento_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/atend_desligamento/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->load->model('indicador_plugin/Atend_desligamento_model');
			$row=$this->Atend_desligamento_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.atend_desligamento 
			...
			$this->load->view('indicador_plugin/atend_desligamento/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$args['cd_atend_desligamento']=intval($this->input->post('cd_atend_desligamento', true));
			...
			$msg=array();
			$retorno = $this->Atend_desligamento_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/atend_desligamento", "refresh" );
				}
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->Atend_desligamento_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.atend_desligamento
		...
		redirect( 'indicador_plugin/atend_desligamento' );
