<?php
class atuarial_eap_ceeeprev_bd extends Controller
{
	var	$label_0 = "Mês";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::ATUARIAL_CEEEPREV_BD);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
	        ...
	        $this->load->view('indicador_plugin/atuarial_eap_ceeeprev_bd/index.php', $data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
        {
	        ...
	        $this->load->model( 'indicador_plugin/Atuarial_eap_ceeeprev_bd_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->Atuarial_eap_ceeeprev_bd_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/atuarial_eap_ceeeprev_bd/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
			...
			$this->load->model('indicador_plugin/Atuarial_eap_ceeeprev_bd_model');
			$row=$this->Atuarial_eap_ceeeprev_bd_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.atuarial_eap_ceeeprev_bd 
			...
			$this->load->view('indicador_plugin/atuarial_eap_ceeeprev_bd/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
			...
			$args['cd_atuarial_eap_ceeeprev_bd']=intval($this->input->post('cd_atuarial_eap_ceeeprev_bd', true));
			...
			$msg=array();
			$retorno = $this->Atuarial_eap_ceeeprev_bd_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/atuarial_eap_ceeeprev_bd", "refresh" );	
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
			...
			$this->Atuarial_eap_ceeeprev_bd_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.atuarial_eap_ceeeprev_bd
		...
		redirect( 'indicador_plugin/atuarial_eap_ceeeprev_bd' );
