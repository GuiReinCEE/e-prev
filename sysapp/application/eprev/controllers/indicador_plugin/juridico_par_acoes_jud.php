<?php
class juridico_par_acoes_jud extends Controller
{
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::JURIDICO_PARTICIPANTES_COM_ACOES_JUDICIAIS);
		...
		$this->load->model('indicador_plugin/juridico_par_acoes_jud_model' );       
    }
    ...
    function index()
    {
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
	        ...
	        $this->load->view('indicador_plugin/juridico_par_acoes_jud/index',$data);
		}        
    }
    ...
    function listar()
    {
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
        {
			...
			$this->juridico_par_acoes_jud_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/juridico_par_acoes_jud/partial_result', $data);
        }        
    }

	function detalhe($cd_juridico_par_acoes_jud = 0)
	{
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
			$args   = array();
			...
			$args['cd_juridico_par_acoes_jud'] = $cd_juridico_par_acoes_jud;
			
			if(intval($args['cd_juridico_par_acoes_jud']) == 0)
			{
				$this->juridico_par_acoes_jud_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_juridico_par_acoes_jud']    = $args['cd_juridico_par_acoes_jud'];
			...
			{
				$this->juridico_par_acoes_jud_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/juridico_par_acoes_jud/detalhe', $data);
		}        
	}
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_juridico_par_acoes_jud']    = intval($this->input->post('cd_juridico_par_acoes_jud', true));
			...
			$this->juridico_par_acoes_jud_model->salvar($result, $args);
			$this->criar_indicador();
			
			redirect("indicador_plugin/juridico_par_acoes_jud", "refresh");
		}
        else
        {
	...
	function excluir($cd_juridico_par_acoes_jud)
	{
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
			$args   = array();
			...
			$this->juridico_par_acoes_jud_model->excluir($result, $args);
			$this->criar_indicador();
	...
	function fechar_periodo()
	{
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
			...
			if(count($indicador) > 0)
			{
				...
				$args['nr_meta']             = $indicador['nr_meta'];

				$args["cd_usuario"]          = $this->session->userdata('codigo');

				$this->juridico_par_acoes_jud_model->atualiza_fechar_periodo($result, $args);
			}

			$this->juridico_par_acoes_jud_model->fechar_periodo($result, $args);
		}
		redirect("indicador_plugin/juridico_par_acoes_jud", "refresh");
	}	
}
