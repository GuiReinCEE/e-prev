<?php
class administrativo_ht_planejado extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Hr Planejado";
	var	$label_2 = "Hr Não planejado";
	var	$label_3 = "Hr Total";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RH_HORA_TREINAMENTO_PLANEJADO_X_NAO);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/administrativo_ht_planejado/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC' ) )
        {
	        $this->load->model( 'indicador_plugin/administrativo_ht_planejado_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->administrativo_ht_planejado_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/administrativo_ht_planejado/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC' ) )
		{
			...
			$this->load->model('indicador_plugin/administrativo_ht_planejado_model');
			$row=$this->administrativo_ht_planejado_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.administrativo_ht_planejado
			...
			$this->load->view('indicador_plugin/administrativo_ht_planejado/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC' ) )
		{
			...
			$args['cd_administrativo_ht_planejado']=intval($this->input->post('cd_administrativo_ht_planejado', true));
			...
			$msg=array();
			$retorno = $this->administrativo_ht_planejado_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/administrativo_ht_planejado", "refresh" );
				}
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC' ) )
		{
			...
			$this->administrativo_ht_planejado_model->excluir($id);
			...
			{#tabela_existe
				...
				foreach( $collection as $item )
				{
					...
					if(true)
                    {
						...
						$indicador[$linha][3] = ($soma);

                        if($soma > 0)
                        {
                           $indicador[$linha][4] = (($nr_valor_1 / $soma) *100);
                           $indicador[$linha][5] = (($nr_valor_2 / $soma) *100);
                        }
                        else
                        {
                           $indicador[$linha][4] = (0);
				...
				if(sizeof($media_ano)>0)
				{
					...
					$indicador[$linha][0] = 'Total '.intval($tabela[0]['nr_ano_referencia']);
					$indicador[$linha][1] = $soma_1;
					$indicador[$linha][2] = $soma_2;
					$indicador[$linha][3] = $soma_f;
                    $indicador[$linha][4] = '';
                    $indicador[$linha][5] = '';
                    $indicador[$linha][6] = '';
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					$sql.=sprintf(" INSERT INTO indicador_plugin.administrativo_ht_planejado
					( dt_referencia,dt_inclusao,cd_usuario_inclusao, nr_percentual_f, nr_valor_1, nr_valor_2, fl_media )
					VALUES ( '%s/01/01',current_timestamp,%s, %s, %s, %s, 'S' ); "
					, intval($tabela[0]['nr_ano_referencia']) , usuario_id(), floatval($soma), floatval($soma_1), floatval($soma_2), floatval( app_decimal_para_db($nr_meta) ));
				}
		...
		redirect( 'indicador_plugin/administrativo_ht_planejado' );
