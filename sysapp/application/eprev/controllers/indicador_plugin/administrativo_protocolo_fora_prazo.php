<?php
class administrativo_protocolo_fora_prazo extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::RH_PROTOCOLO_FORA_DO_PRAZO);
		...
		$this->load->model('indicador_plugin/administrativo_protocolo_fora_prazo_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(), 'GFC'))
		{
	        ...
	        $this->load->view('indicador_plugin/administrativo_protocolo_fora_prazo/index',$data);
		}
    }
    ...
    function listar()
    {
    	if(indicador_db::verificar_permissao(usuario_id(), 'GFC'))
        {
			...
			$this->administrativo_protocolo_fora_prazo_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/administrativo_protocolo_fora_prazo/index_result', $data);
        }
        else
		{
    ...
    function cadastro($cd_administrativo_protocolo_fora_prazo = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(), 'GFC'))
		{
			$args   = array();
			...
			$args['cd_administrativo_protocolo_fora_prazo'] = $cd_administrativo_protocolo_fora_prazo;
			
			if(intval($args['cd_administrativo_protocolo_fora_prazo']) == 0)
			{
				$arr = $this->administrativo_protocolo_fora_prazo_model->carrega_referencia(intval($data['tabela'][0]['nr_ano_referencia']));
				
				$data['row']['cd_administrativo_protocolo_fora_prazo'] = $args['cd_administrativo_protocolo_fora_prazo'];
			...
			{
				$this->administrativo_protocolo_fora_prazo_model->carrega($result, $args);
				$data['row'] = $result->row_array();
				$data['row']['qt_ano'] = 1;
			}

			$this->load->view('indicador_plugin/administrativo_protocolo_fora_prazo/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_administrativo_protocolo_fora_prazo'] = intval($this->input->post('cd_administrativo_protocolo_fora_prazo', true));
			...
			$this->administrativo_protocolo_fora_prazo_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/administrativo_protocolo_fora_prazo", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_administrativo_protocolo_fora_prazo)
	{
		if(indicador_db::verificar_permissao(usuario_id(), 'GFC'))
		{
			$args   = array();
			...
			$this->administrativo_protocolo_fora_prazo_model->excluir($result, $args);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(), 'GFC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->administrativo_protocolo_fora_prazo_model->atualiza_fechar_periodo($result, $args);
			}

			$this->administrativo_protocolo_fora_prazo_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function get_valores()
	{
		if(indicador_db::verificar_permissao(usuario_id(), 'GFC' ))
		{
			...
			$this->administrativo_protocolo_fora_prazo_model->total( $result, $args );
            $data = $result->row_array();

			$row["nr_documentos_total"] = (count($data) > 0 ? intval($data['total']) : 0);

			$this->administrativo_protocolo_fora_prazo_model->fora_prazo( $result, $args );
            $data = $result->row_array();

			$row["nr_documentos_fora"] = (count($data) > 0 ? intval($data['total']) : 0);
