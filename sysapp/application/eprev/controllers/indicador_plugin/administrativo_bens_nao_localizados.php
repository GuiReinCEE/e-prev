<?php
class Administrativo_bens_nao_localizados extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::ADMINISTRATIVO_BENS_NAO_LOCALIZADOS);
		...
		$this->load->model('indicador_plugin/administrativo_bens_nao_localizados_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
	        ...
	        $this->load->view('indicador_plugin/administrativo_bens_nao_localizados/index',$data);
		}
    }
    ...
    function listar()
    {
    	if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
        {
			...
			$this->administrativo_bens_nao_localizados_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/administrativo_bens_nao_localizados/index_result', $data);
        }
        else
		{
    ...
    function cadastro($cd_administrativo_bens_nao_localizados = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$args['cd_administrativo_bens_nao_localizados'] = $cd_administrativo_bens_nao_localizados;
			
			if(intval($args['cd_administrativo_bens_nao_localizados']) == 0)
			{
				$this->administrativo_bens_nao_localizados_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_administrativo_bens_nao_localizados'] = $args['cd_administrativo_bens_nao_localizados'];
			...
			{
				$this->administrativo_bens_nao_localizados_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/administrativo_bens_nao_localizados/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_administrativo_bens_nao_localizados'] = intval($this->input->post('cd_administrativo_bens_nao_localizados', true));
			...
			$this->administrativo_bens_nao_localizados_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/administrativo_bens_nao_localizados", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_administrativo_bens_nao_localizados)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$this->administrativo_bens_nao_localizados_model->excluir($result, $args);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->administrativo_bens_nao_localizados_model->atualiza_fechar_periodo($result, $args);
			}

			$this->administrativo_bens_nao_localizados_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
