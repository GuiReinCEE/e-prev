<?php
class Atend_nr_ingresso_nr_desl extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::ATENDIMENTO_NUMERO_INGRESSO_NUMERO_DESLIGADOS);
		...
		$this->load->model('indicador_plugin/atend_nr_ingresso_nr_desl_model');
    }
	...
	function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GCM'))
		{
	        ...
	        $this->load->view('indicador_plugin/atend_nr_ingresso_nr_desl/index', $data);
		}
		else
		{
	...
	function listar()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GCM'))
        {
			...
			$this->atend_nr_ingresso_nr_desl_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/atend_nr_ingresso_nr_desl/index_result', $data);
        }
        else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function cadastro($cd_atend_nr_ingresso_nr_desl = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GCM'))
		{
			$args   = array();
			...
			$args['cd_atend_nr_ingresso_nr_desl'] = $cd_atend_nr_ingresso_nr_desl;
			
			if(intval($args['cd_atend_nr_ingresso_nr_desl']) == 0)
			{
				$arr = $this->atend_nr_ingresso_nr_desl_model->carrega_referencia(intval($data['tabela'][0]['nr_ano_referencia']));
				
				$data['row']['cd_atend_nr_ingresso_nr_desl'] = $args['cd_atend_nr_ingresso_nr_desl'];
			...
			{
				$this->atend_nr_ingresso_nr_desl_model->carrega($result, $args);
				$data['row'] = $result->row_array();
				$data['row']['qt_ano'] = 1;
			}

			$this->load->view('indicador_plugin/atend_nr_ingresso_nr_desl/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_atend_nr_ingresso_nr_desl'] = intval($this->input->post('cd_atend_nr_ingresso_nr_desl', true));
			...
			$this->atend_nr_ingresso_nr_desl_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/atend_nr_ingresso_nr_desl", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_atend_nr_ingresso_nr_desl)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GCM'))
		{
			$args   = array();
			...
			$this->atend_nr_ingresso_nr_desl_model->excluir($result, $args);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GP'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->atend_nr_ingresso_nr_desl_model->atualiza_fechar_periodo($result, $args);
			}

			$this->atend_nr_ingresso_nr_desl_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
