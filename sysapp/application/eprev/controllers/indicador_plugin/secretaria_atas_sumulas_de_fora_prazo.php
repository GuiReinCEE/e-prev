<?php
class Secretaria_atas_sumulas_de_fora_prazo extends Controller
{
    function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::SECRETARIA_ATAS_SUMULAS_DE_FORA_PRAZO);
		...
		$this->load->model('indicador_plugin/secretaria_atas_sumulas_de_fora_prazo_model');
    }
    ...
    public function index()
    {
        if(indicador_db::verificar_permissao($this->cd_usuario, 'GC'))
        {
            ...
            $this->load->view('indicador_plugin/secretaria_atas_sumulas_de_fora_prazo/index', $data);
        }
        else
        {
    ...
    public function listar()
    {
        if(indicador_db::verificar_permissao($this->cd_usuario, 'GC'))
        {
            ...
            $data['collection'] = $this->secretaria_atas_sumulas_de_fora_prazo_model->listar($data['tabela'][0]['cd_indicador_tabela'] );

            $this->load->view('indicador_plugin/secretaria_atas_sumulas_de_fora_prazo/index_result', $data);        
        }
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
    ...
    public function cadastro($cd_secretaria_atas_sumulas_de_fora_prazo = 0)
    {
        if(indicador_db::verificar_permissao($this->cd_usuario, 'GC'))
        {
            $data['label_0'] = $this->label_0;
            ...
            if(intval($cd_secretaria_atas_sumulas_de_fora_prazo) == 0)
            {
                $row = $this->secretaria_atas_sumulas_de_fora_prazo_model->carrega_referencia();
                    
                $data['row'] = array(
                    'cd_secretaria_atas_sumulas_de_fora_prazo' => intval($cd_secretaria_atas_sumulas_de_fora_prazo),
                    'ds_observacao'                            => '',
            ...
            {
                $data['row'] = $this->secretaria_atas_sumulas_de_fora_prazo_model->carrega($cd_secretaria_atas_sumulas_de_fora_prazo);
            }

            $this->criar_indicador();
            
            $this->load->view('indicador_plugin/secretaria_atas_sumulas_de_fora_prazo/cadastro', $data);
        }
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
    ...
    public function salvar()
    {
        if(indicador_db::verificar_permissao($this->cd_usuario, 'GC'))
        {
            $cd_secretaria_atas_sumulas_de_fora_prazo = $this->input->post('cd_secretaria_atas_sumulas_de_fora_prazo', TRUE);
            ...
            if(intval($cd_secretaria_atas_sumulas_de_fora_prazo) == 0)
            {
                $this->secretaria_atas_sumulas_de_fora_prazo_model->salvar($args);
            }
            else
            {
                $this->secretaria_atas_sumulas_de_fora_prazo_model->atualizar($cd_secretaria_atas_sumulas_de_fora_prazo, $args);
            }

            $this->criar_indicador();

            redirect('indicador_plugin/secretaria_atas_sumulas_de_fora_prazo', 'refresh');
        }
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
    ...
    public function excluir($cd_secretaria_atas_sumulas_de_fora_prazo)
    {
        if(indicador_db::verificar_permissao($this->cd_usuario, 'GC'))
        {
            $this->secretaria_atas_sumulas_de_fora_prazo_model->excluir(
                $cd_secretaria_atas_sumulas_de_fora_prazo, 
    ...
    public function criar_indicador()
    {
        if(indicador_db::verificar_permissao($this->cd_usuario, 'GC'))
        {
            ...
            $collection = $this->secretaria_atas_sumulas_de_fora_prazo_model->listar($tabela[0]['cd_indicador_tabela']);
    ...
    public function fechar_periodo()
    {
        if(indicador_db::verificar_permissao($this->cd_usuario, 'GC'))
        {
            ...
            if(intval($contador_ano_atual) > 0)
            {
                ...
                $args = array(
                    'cd_secretaria_atas_sumulas_de_fora_prazo' => 0, 
            ...
            $this->secretaria_atas_sumulas_de_fora_prazo_model->fechar_periodo($tabela[0]['cd_indicador_tabela'], $this->cd_usuario);
