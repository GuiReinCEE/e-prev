<?php
class Atend_nr_ingresso_nr_desl_b extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::ATENDIMENTO_NUMERO_INGRESSO_NUMERO_DESLIGADOS_B);
		...
		$this->load->model('indicador_plugin/atend_nr_ingresso_nr_desl_b_model');
    }
	...
	function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GCM'))
		{
	        ...
	        $this->load->view('indicador_plugin/atend_nr_ingresso_nr_desl_b/index', $data);
		}
		else
		{
	...
	function listar()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GCM'))
        {
			...
			$this->atend_nr_ingresso_nr_desl_b_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/atend_nr_ingresso_nr_desl_b/index_result', $data);
        }
        else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function cadastro($cd_atend_nr_ingresso_nr_desl_b = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GCM'))
		{
			$args   = array();
			...
			$args['cd_atend_nr_ingresso_nr_desl_b'] = $cd_atend_nr_ingresso_nr_desl_b;
			
			if(intval($args['cd_atend_nr_ingresso_nr_desl_b']) == 0)
			{
				$this->atend_nr_ingresso_nr_desl_b_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_atend_nr_ingresso_nr_desl_b'] = $args['cd_atend_nr_ingresso_nr_desl_b'];
			...
			{
				$this->atend_nr_ingresso_nr_desl_b_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/atend_nr_ingresso_nr_desl_b/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_atend_nr_ingresso_nr_desl_b'] = intval($this->input->post('cd_atend_nr_ingresso_nr_desl_b', true));
			...
			$this->atend_nr_ingresso_nr_desl_b_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/atend_nr_ingresso_nr_desl_b", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_atend_nr_ingresso_nr_desl_b)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GCM'))
		{
			$args   = array();
			...
			$this->atend_nr_ingresso_nr_desl_b_model->excluir($result, $args);
	...
	function criar_indicador()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GCM'))
		{
			...
			foreach($collection as $item)
			{
				if(intval($item['ano_referencia']) >= intval($tabela[0]['nr_ano_referencia']) -5)
				{
					...
					if(intval($item['ano_referencia']) == intval($tabela[0]['nr_ano_referencia']))
					{
						...
						$nr_meta_media    += $item["nr_meta"];
					}

					$indicador[$linha][0] = $referencia;
					$indicador[$linha][1] = app_decimal_para_php($nr_valor_1);
					$indicador[$linha][2] = app_decimal_para_php($nr_valor_3);
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$indicador[$linha][2] = app_decimal_para_php($nr_valor_3_total);
				$indicador[$linha][3] = app_decimal_para_php($nr_valor_2_total);
				$indicador[$linha][4] = app_decimal_para_php((($nr_valor_2_total*100) / ($nr_valor_1_total > 0 ? $nr_valor_1_total : 1)));
				$indicador[$linha][5] = app_decimal_para_php($nr_meta_media);
				$indicador[$linha][6] = '';
				$indicador[$linha][7] = '';
			}
			...
			// gerar gráfico
			$coluna_para_ocultar='6, 4';
			$sql.=indicador_db::sql_inserir_grafico(
				$tabela[0]['cd_indicador_tabela'],
				enum_indicador_grafico_tipo::LINHA,
				'3,3,0,0;5,5,0,0',
				"0,0,1,$linha_sem_media",
				"3,3,1,$linha_sem_media;5,5,1,$linha_sem_media-linha",
				usuario_id(),
				$coluna_para_ocultar,
				1,
				2
		...
		if(indicador_db::verificar_permissao(usuario_id(),'GP'))
		{
			...
			{			 
				...
				if( intval($item['ano_referencia']) == intval($tabela[0]['nr_ano_referencia']) )
				{
					...
					$nr_valor_total_3 += $item["nr_valor_3"];
				}
			}
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$args['nr_valor_1']          = app_decimal_para_db(number_format($nr_valor_total_1, 2, ",", ""));
				$args['nr_valor_2']          = app_decimal_para_db(number_format($nr_valor_total_2, 2, ",", ""));
				$args['nr_valor_3']          = app_decimal_para_db(number_format($nr_valor_total_3, 2, ",", ""));
				$args["nr_meta"]             = app_decimal_para_db($nr_meta);
				$args["cd_usuario"]          = $this->session->userdata('codigo');

				$this->atend_nr_ingresso_nr_desl_b_model->atualiza_fechar_periodo($result, $args);
			}

			$this->atend_nr_ingresso_nr_desl_b_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
