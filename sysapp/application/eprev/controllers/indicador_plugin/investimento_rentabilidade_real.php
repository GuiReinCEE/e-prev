<?php
class Investimento_rentabilidade_real extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::INVESTIMENTO_RENTABILIDADE_REAL);
		...
		$this->load->model('indicador_plugin/investimento_rentabilidade_real_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
	        ...
	        $this->load->view('indicador_plugin/investimento_rentabilidade_real/index',$data);
		}
    }
    ...
    function listar()
    {
    	if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
        {
			...
			$this->investimento_rentabilidade_real_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/investimento_rentabilidade_real/index_result', $data);
        }
        else
		{
    ...
    function cadastro($cd_investimento_rentabilidade_real = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
			$args   = array();
			...
			$args['cd_investimento_rentabilidade_real'] = $cd_investimento_rentabilidade_real;
			
			if(intval($args['cd_investimento_rentabilidade_real']) == 0)
			{
				$this->investimento_rentabilidade_real_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_investimento_rentabilidade_real'] = $args['cd_investimento_rentabilidade_real'];
			...
			{
				$this->investimento_rentabilidade_real_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/investimento_rentabilidade_real/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_investimento_rentabilidade_real'] = intval($this->input->post('cd_investimento_rentabilidade_real', true));
			...
			$nr_valor_4 = calculo_acumulado($arr_valor_4, intval($this->input->post("mes_referencia", true)));

			$nr_valor_7 = (((($nr_valor_3/100)+1)/(($nr_valor_4/100)+1))-1)*100;
			...
			$this->investimento_rentabilidade_real_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/investimento_rentabilidade_real", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_investimento_rentabilidade_real)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
			$args   = array();
			...
			$this->investimento_rentabilidade_real_model->excluir($result, $args);
	...
	public function get_valores()
	{
		...
		$this->integracao_caderno_cci_indicador->set_descricao('rentabilidade_real');
	...
	function criar_indicador()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
			...
			foreach($collection as $item)
			{
				...
				{
					$data['label_3'] = str_replace("1", number_format($item["nr_valor_1"],1,",","."), $data['label_3']);
					$data['label_5'] = str_replace("1", number_format($item["nr_valor_1"],1,",","."), $data['label_5']);

					$data['label_4'] = str_replace("2", number_format($item["nr_valor_2"],1,",","."), $data['label_4']);
					$data['label_6'] = str_replace("2", number_format($item["nr_valor_2"],1,",","."), $data['label_6']);
					break;
				}
			}
			...
			foreach($collection as $item)
			{
				if(intval($item['ano_referencia']) >= intval($tabela[0]['nr_ano_referencia']) -5)
				{
					...
					$indicador[$linha][3] = app_decimal_para_php($nr_valor_1);
					//$indicador[$linha][4] = app_decimal_para_php($nr_valor_2);
					$indicador[$linha][4] = app_decimal_para_php($nr_valor_5);
					//$indicador[$linha][6] = app_decimal_para_php($nr_valor_6);
					$indicador[$linha][5] = app_decimal_para_php($nr_valor_7);
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$indicador[$linha][1] = '';
				$indicador[$linha][2] = '';
				$indicador[$linha][3] = app_decimal_para_php($nr_valor_1_total);
				//$indicador[$linha][4] = app_decimal_para_php($nr_valor_2_total);
				$indicador[$linha][4] = app_decimal_para_php($nr_valor_5_total);
				//$indicador[$linha][6] = app_decimal_para_php($nr_valor_6_total);
				$indicador[$linha][5] = app_decimal_para_php($nr_valor_7_total);
				$indicador[$linha][6] = '';
			}
			...
			for($i=0; $i<sizeof($indicador); $i++)
			{
				...
				$sql .= indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 2, $linha, app_decimal_para_php($indicador[$i][5]), 'center', 'S', 4, 'S' );
				$sql .= indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 3, $linha, app_decimal_para_php($indicador[$i][2]), 'center', 'S', 4, 'S' );
				$sql .= indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 4, $linha, app_decimal_para_php($indicador[$i][4]), 'center', 'S', 4, 'S' );
				//$sql .= indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 5, $linha, app_decimal_para_php($indicador[$i][6]), 'center', 'S', 4, 'S' );
				$sql .= indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 5, $linha, app_decimal_para_php($indicador[$i][3]), 'center', 'S', 4, 'S' );
				//$sql .= indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 7, $linha, app_decimal_para_php($indicador[$i][4]), 'center', 'S', 4, 'S' );
			...
			// GINrar gráfico
			$coluna_para_ocultar='1';
			$sql.=indicador_db::sql_inserir_grafico(
				$tabela[0]['cd_indicador_tabela'],
				enum_indicador_grafico_tipo::LINHA,
				'2,2,0,0;4,4,0,0',
				"0,0,1,$linha_sem_media",
				"2,2,1,$linha_sem_media;4,4,1,$linha_sem_media-linha",
				usuario_id(),
				$coluna_para_ocultar,
				1,
				2
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->investimento_rentabilidade_real_model->atualiza_fechar_periodo($result, $args);
			}

			$this->investimento_rentabilidade_real_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
