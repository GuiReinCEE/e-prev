<?php
class Secretaria_sumulas_cd extends Controller
{
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::SECRETARIA_DIVUGACAO_SUMULAS_CD);
		...
		$this->load->model('indicador_plugin/secretaria_sumulas_cd_model');		
	}
    ...
    function index()
    {
		if(indicador_db::verificar_permissao( usuario_id(), 'SG' ))
		{
	        ...
	        $this->load->view('indicador_plugin/secretaria_sumulas_cd/index.php',$data);
		}
		else
		{
    ...
    function listar()
    {
        if(indicador_db::verificar_permissao(usuario_id(),'SG'))
        {
			...
			$this->secretaria_sumulas_cd_model->listar($result, $args);
			$data['collection'] = $result->result_array();

			$this->load->view('indicador_plugin/secretaria_sumulas_cd/index_result', $data);
		}
		else
		{
	...
	public function cadastro($cd_secretaria_sumulas_cd=0)
	{
	    if(indicador_db::verificar_permissao(usuario_id(),'SG'))
		{
			$args   = array();
			...
			$args['cd_secretaria_sumulas_cd'] = $cd_secretaria_sumulas_cd;

			if(intval($args['cd_secretaria_sumulas_cd']) == 0)
			{
				$this->secretaria_sumulas_cd_model->carrega_referencia($result);
				$arr = $result->row_array();

				$data['row']['cd_secretaria_sumulas_cd'] = (isset($arr['cd_secretaria_sumulas_cd']) ? $arr['cd_secretaria_sumulas_cd'] : "");
			...
			{
				$this->secretaria_sumulas_cd_model->carregar($result, $args);
				$data['row'] = $result->row_array(); 
			}

			$this->load->view('indicador_plugin/secretaria_sumulas_cd/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{	
			...
			$args   = array(
				   'cd_secretaria_sumulas_cd' => intval($this->input->post('cd_secretaria_sumulas_cd', true)),
            ...
            $this->secretaria_sumulas_cd_model->salvar($args);
            
           $this->criar_indicador();

            redirect( "indicador_plugin/secretaria_sumulas_cd", "refresh" );
	...
	function excluir($cd_secretaria_sumulas_cd)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'SG'))
		{
			$this->load->model('indicador_plugin/secretaria_sumulas_cd_model');

			$this->secretaria_sumulas_cd_model->excluir($cd_secretaria_sumulas_cd, $this->session->userdata('codigo'));
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'SG'))
		{
			...
			foreach( $collection as $item )
			{
				...
				{
					$media_ano[] = $nr_percentual_f;
					$soma_v1    += $item["nr_valor_1"];
					$soma_v2    += $item["nr_valor_2"];
				}
			}
			...
			if(sizeof($media_ano)>0)
			{
				...
				$this->secretaria_sumulas_cd_model->atualiza_fechar_periodo($result, $args);
			}

			$this->secretaria_sumulas_cd_model->fechar_periodo($result, $args);	
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
