<?php
class ri_sat_cliente_internos extends Controller
{
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::RI_SATISFACAO_COM_CLIENTES_INTERNOS);
		...
		$this->load->model('indicador_plugin/ri_sat_cliente_internos_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(), 'AC') || indicador_db::verificar_permissao(usuario_id(), 'GE'))
		{
	        ...
	        $this->load->view('indicador_plugin/ri_sat_cliente_internos/index', $data);
		}
		else
        {
    ...
    function listar()
    {
		if(indicador_db::verificar_permissao(usuario_id(), 'AC') || indicador_db::verificar_permissao(usuario_id(), 'GE'))
        {
			...
			$this->ri_sat_cliente_internos_model->listar($result, $args);
			$data['collection'] = $result->result_array();

			$this->load->view('indicador_plugin/ri_sat_cliente_internos/index_result', $data);
        }
		else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function cadastro($cd_ri_sat_cliente_internos = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'AC'))
		{
			...
			$args['cd_ri_sat_cliente_internos'] = $cd_ri_sat_cliente_internos;

			if(intval($args['cd_ri_sat_cliente_internos']) == 0)
			{
				$this->ri_sat_cliente_internos_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_ri_sat_cliente_internos'] = $args['cd_ri_sat_cliente_internos'];
				$data['row']['nr_valor_1']                 = "";
				$data['row']['nr_valor_2']                 = "";
				$data['row']['fl_media']                   = "";
				$data['row']['observacao']                 = "";
			...
			{
				$this->ri_sat_cliente_internos_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/ri_sat_cliente_internos/cadastro', $data);
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_ri_sat_cliente_internos'] = intval($this->input->post('cd_ri_sat_cliente_internos', true));
			...
			$this->ri_sat_cliente_internos_model->salvar($result, $args);
			$this->criar_indicador();
			
			redirect("indicador_plugin/ri_sat_cliente_internos", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_ri_sat_cliente_internos)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'AC'))
		{
			$args   = array();
			...
			$this->ri_sat_cliente_internos_model->excluir($result, $args);
			$this->criar_indicador();
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'AC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				$args['cd_ri_sat_cliente_internos'] = 0;
			...
			$this->ri_sat_cliente_internos_model->fechar_periodo($result, $args);
