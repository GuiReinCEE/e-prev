<?php
class Reclamatoria_indenizacao_demandante extends Controller
{
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::RECLAMATORIA_CUSTO_MEDIO_INDENIZACAO_POR_DEMANADANTE);
		...
		$this->load->model('indicador_plugin/reclamatoria_indenizacao_demandante_model' );
    }	
	...
	function index()
    {
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
	        ...
	        $this->load->view('indicador_plugin/reclamatoria_indenizacao_demandante/index',$data);
		}
    }
	...
	function listar()
    {
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
        {
			...
			$this->reclamatoria_indenizacao_demandante_model->listar($result, $args);
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/reclamatoria_indenizacao_demandante/index_result', $data);
        }
    }

	function cadastro($cd_reclamatoria_indenizacao_demandante = 0)
	{
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
			$args   = array();
			...
			$args['cd_reclamatoria_indenizacao_demandante'] = intval($cd_reclamatoria_indenizacao_demandante);
			
			if(intval($args['cd_reclamatoria_indenizacao_demandante']) == 0)
			{
				$this->reclamatoria_indenizacao_demandante_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_reclamatoria_indenizacao_demandante'] = $args['cd_reclamatoria_indenizacao_demandante'];
				$data['row']['vl_indenizacao'] = 0;
				$data['row']['nr_liquidada']   = 0;
				$data['row']['nr_demandante']  = 0;
				$data['row']['fl_media']       = "";
				$data['row']['observacao']     = "";
			...
			{
				$this->reclamatoria_indenizacao_demandante_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/reclamatoria_indenizacao_demandante/cadastro', $data);
		}
	}	
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_reclamatoria_indenizacao_demandante'] = intval($this->input->post('cd_reclamatoria_indenizacao_demandante', true));
			...
			$this->reclamatoria_indenizacao_demandante_model->salvar($result, $args);
			$this->criar_indicador();
			
			redirect("indicador_plugin/reclamatoria_indenizacao_demandante", "refresh");
		}
        else
        {
	...
	function excluir($cd_reclamatoria_indenizacao_demandante = 0)
	{
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
			$args   = array();
			...
			$this->reclamatoria_indenizacao_demandante_model->excluir($result, $args);
			$this->criar_indicador();
		...
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
			...
			foreach($collection as $item)
			{
				if((intval($item['ano_referencia']) >= intval($tabela[0]['nr_ano_referencia']) - intval($tabela[0]['qt_periodo_anterior'])) OR ($fl_periodo))
				{
					...
					$indicador[$linha][7] = $item["nr_resultado"];
					$indicador[$linha][8] = 0; #tendencia
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$indicador[$linha][4] = $vl_indenizacao_liquidada_total;
				$indicador[$linha][5] = $vl_indenizacao_demandante_total;
				$indicador[$linha][6] = $nr_meta;
				$indicador[$linha][7] = $vl_resultado_total;
				$indicador[$linha][8] = '';
				$indicador[$linha][9] = '';
			}
			...
			for($i=0; $i<sizeof($indicador); $i++)
			{
				...
				$sql .= indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 6, $linha, app_decimal_para_php($indicador[$i][7]), 'center', 'S', 2);
				$sql .= indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 7, $linha, app_decimal_para_php($indicador[$i][6]), 'center', 'S', 2);
				$sql .= indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 8, $linha, app_decimal_para_php($indicador[$i][8]), 'center', 'S', 2);
	...
	function fechar_periodo()
	{
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				$args['cd_reclamatoria_indenizacao_demandante'] = 0;
			...
			$this->reclamatoria_indenizacao_demandante_model->fechar_periodo($result, $args);
		}
	...
	function fechar_periodoxxxx()
	{
		...
		if(CheckLogin() AND (indicador_db::verificar_permissao(usuario_id(), 'AJ') OR indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.reclamatoria_indenizacao_demandante
		...
		redirect( 'indicador_plugin/reclamatoria_indenizacao_demandante' );
