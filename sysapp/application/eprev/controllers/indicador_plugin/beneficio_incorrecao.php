<?php
class beneficio_incorrecao extends Controller
{
	var	$label_0 = "Mês";
	var	$label_1 = "Total de Benefícios Processados";
	var	$label_2 = "Benef. Processados com Erro";
	var	$label_3 = "% de Incorreções no Líquido";
    var $label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::BENEFICIO_INCORRECOES_NO_VALOR_DO_BENEFICIO_LIQUIDO);
    }
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/beneficio_incorrecao/index.php');
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
        {
	        $this->load->model( 'indicador_plugin/Beneficio_incorrecao_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->Beneficio_incorrecao_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/beneficio_incorrecao/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->load->model('indicador_plugin/Beneficio_incorrecao_model');
			$row=$this->Beneficio_incorrecao_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					$sql = "
					SELECT to_char(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS mes_referencia, dt_referencia, nr_meta, 
					cd_indicador_tabela,
				   (SELECT COUNT(*)
				      FROM indicador_plugin.beneficio_incorrecao
			...
			$this->load->view('indicador_plugin/beneficio_incorrecao/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$args['cd_beneficio_incorrecao']=intval($this->input->post('cd_beneficio_incorrecao', true));
			$args["dt_referencia"] = $this->input->post("dt_referencia", true);
			$args["cd_usuario_inclusao"] = usuario_id();

			$args["nr_beneficio"] = app_decimal_para_db($this->input->post("nr_beneficio", true));
			$args["nr_beneficio_erro"] = app_decimal_para_db($this->input->post("nr_beneficio_erro", true));
			...
			$msg=array();
			$retorno = $this->Beneficio_incorrecao_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/beneficio_incorrecao", "refresh" );
				}
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->Beneficio_incorrecao_model->excluir($id);
			...
			{#tabela_existe
				...
				foreach( $collection as $item )
				{
					...
					if( intval($item['ano_referencia'])>=intval($tabela[0]['nr_ano_referencia'])-5 )
					{
						...
						$indicador[$linha][1] = app_decimal_para_php($nr_beneficio);
						$indicador[$linha][2] = app_decimal_para_php($nr_beneficio_erro);
				...
				if(sizeof($media_ano)>0)
				{
					...
					$indicador[$linha][1] = intval($nr_total_beneficio);
					$indicador[$linha][2] = intval($nr_total_beneficio_erro);
					$indicador[$linha][3] = $media;
					$indicador[$linha][4] = $nr_total_meta;
					$indicador[$linha][5] = '';
                    $indicador[$linha][6] = '';
				}
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.beneficio_incorrecao
		...
		redirect( 'indicador_plugin/beneficio_incorrecao' );
