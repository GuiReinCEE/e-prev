<?php
class Secretaria_retorno extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::SECRETARIA_RETORNO_DECISOES_SOLICITACOES_FORA_PRAZO);
		...
		$this->load->model('indicador_plugin/secretaria_retorno_model');
    }
    ...
    public function index()
    {
		if(indicador_db::verificar_permissao($this->cd_usuario, 'SG'))
		{
	        ...
	        $this->load->view('indicador_plugin/secretaria_retorno/index', $data);
		}
		else
		{
    ...
    public function listar()
    {
        ...
        {			
			...
			$data['collection'] = $this->secretaria_retorno_model->listar($data['tabela'][0]['cd_indicador_tabela'] );
			
			$this->load->view('indicador_plugin/secretaria_retorno/index_result', $data);
        }
        else
		{
			exibir_mensagem('ACESSO NÃO PERMITIDO');
    ...
    public function cadastro($cd_secretaria_retorno = 0)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'SG'))
		{			
			$data['label_0'] = $this->label_0;
			...
			if(intval($cd_secretaria_retorno) == 0)
			{
				$row = $this->secretaria_retorno_model->carrega_referencia();
				
				$data['row'] = array(
					'cd_secretaria_retorno' => intval($cd_secretaria_retorno),
				    'nr_valor_1'            => '',
				    'nr_valor_2'            => '',
				    'fl_media'             	=> '',
				    'observacao'            => '',
			...
			{
				$data['row'] = $this->secretaria_retorno_model->carrega($cd_secretaria_retorno);
			}

			$this->load->view('indicador_plugin/secretaria_retorno/cadastro', $data);
		}
		else
		{
			exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function salvar()
	{
		...
		{		
			...
			$nr_sumula_disp_24h    = app_decimal_para_db($this->input->post('nr_sumula_disp_24h', true));

			$nr_percentual_f         = 0; 
			$nr_percent_disp_24h = 0;

			if(floatval($nr_valor_1) > 0)
			{
				$nr_percentual_f         = ($nr_valor_2 / $nr_valor_1) * 100;
				$nr_percent_disp_24h = ($nr_sumula_disp_24h / $nr_valor_1) * 100;
			}
				
			$args = array(
				'cd_secretaria_retorno' => intval($this->input->post('cd_secretaria_retorno', true)),
			    ...
			    'nr_sumula_disp_24h'    => $nr_sumula_disp_24h,
			    'nr_percentual_f'		=> $nr_percentual_f,
			    'nr_percent_disp_24h'   => $nr_percent_disp_24h,
                'observacao'            => $this->input->post('observacao', true),
			    'cd_usuario'            => $this->session->userdata('codigo')
			);

			$this->secretaria_retorno_model->salvar($args);
			
			$this->criar_indicador();
	
			redirect('indicador_plugin/secretaria_retorno', 'refresh');
		}
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function excluir($cd_secretaria_retorno)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'SG'))
		{
			$this->secretaria_retorno_model->excluir($cd_secretaria_retorno, $this->cd_usuario);
	...
	public function criar_indicador()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'SG'))
		{
			...
			$collection = $this->secretaria_retorno_model->listar($tabela[0]['cd_indicador_tabela']);
			...
			foreach($collection as $item)
			{
				if(intval($item['ano_referencia']) >= intval($tabela[0]['nr_ano_referencia']) -5)
				{
					...
					if(intval($item['ano_referencia']) == intval($tabela[0]['nr_ano_referencia']) && (trim($item['fl_media']) != 'S'))
					{
						...
						$nr_sumula_disp_24h += $item['nr_sumula_disp_24h'];
						$nr_meta            = $item['nr_meta'];
					}

					$indicador[$linha][0] = $referencia;
					$indicador[$linha][1] = app_decimal_para_php($item['nr_valor_1']);
					$indicador[$linha][2] = app_decimal_para_php($item['nr_sumula_disp_24h']);
					$indicador[$linha][3] = app_decimal_para_php($item['nr_percent_disp_24h']);
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$indicador[$linha][2] = $nr_sumula_disp_24h;
				$indicador[$linha][3] = $nr_percent_disp_24h;
	...
	public function fechar_periodo()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'SG'))
		{
			...
			$this->secretaria_retorno_model->fechar_periodo($tabela[0]['cd_indicador_tabela'], $this->cd_usuario);
