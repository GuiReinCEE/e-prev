<?php
class atend_enc_vida extends Controller
{
	var	$label_0 = "Ano";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::ATENDIMENTO_DE_SATISFACAO_ENCONTRO_MAIS_VIDA);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/atend_enc_vida/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
        {
	        $this->load->model( 'indicador_plugin/atend_enc_vida_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->atend_enc_vida_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/atend_enc_vida/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->load->model('indicador_plugin/atend_enc_vida_model');
			$row=$this->atend_enc_vida_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.atend_enc_vida
			...
			$this->load->view('indicador_plugin/atend_enc_vida/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$args['cd_atend_enc_vida']=intval($this->input->post('cd_atend_enc_vida', true));
			...
			$msg=array();
			$retorno = $this->atend_enc_vida_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/atend_enc_vida", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->atend_enc_vida_model->excluir($id);
				...
				foreach( $collection as $item )
				{
					...
					if(true)
					{
						...
						else
						{
							...
							if( floatval($nr_valor_3)>0 )
                            {
                                $nr_percentual_f = ( (floatval($nr_valor_2)/floatval($nr_valor_3)) )*100;
                            }
                            else
                            {
                                $nr_percentual_f = '0';
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.atend_enc_vida
				...
				*/
				...
				// indicar que o período foi fechado para o indicador_tabela
				$sql=" 
						INSERT INTO indicador_plugin.atend_enc_vida
							 ( 
							   dt_referencia,
							   dt_inclusao,
							   cd_usuario_inclusao, 
							 ...
							 (
								SELECT aep.dt_referencia,
									   ...
									   aep.nr_meta,
									   'S'
								  FROM indicador_plugin.atend_enc_vida aep
								 WHERE aep.dt_exclusao IS NULL
								   ...
								   AND aep.dt_referencia = (SELECT MAX(eap1.dt_referencia) 
															  FROM indicador_plugin.atend_enc_vida eap1
		...
		redirect( 'indicador_plugin/atend_enc_vida' );
