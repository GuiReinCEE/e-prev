<?php
class ri_venda_previd_instit extends Controller
{
    ...
    function __construct()
    {
        ...
        $this->enum_indicador = intval(enum_indicador::RI_VENDA_DOS_PLANOS_PREVIDENCIARIOS_INSTITUIDORA);
        ...
        $this->load->model('indicador_plugin/ri_venda_previd_instit_model');
    }

    function index()
    {
        if((indicador_db::verificar_permissao(usuario_id(), 'AC')) OR (indicador_db::verificar_permissao(usuario_id(), 'GE')))
        {
            $args   = array();
            $data   = array();
            $result = null;
            ...
            $this->load->view('indicador_plugin/ri_venda_previd_instit/index', $data);
        }
        else
        {
    ...
    function listar()
    {
        if((indicador_db::verificar_permissao(usuario_id(), 'AC')) OR (indicador_db::verificar_permissao(usuario_id(), 'GE')))
        {
            ...
            $this->ri_venda_previd_instit_model->listar($result, $args);
            $data['collection'] = $result->result_array();

            $this->load->view('indicador_plugin/ri_venda_previd_instit/index_result', $data);
        }
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
    ...
    function cadastro($cd_ri_venda_previd_instit = 0)
    {
        if((indicador_db::verificar_permissao(usuario_id(), 'AC')) OR (indicador_db::verificar_permissao(usuario_id(), 'GE')))
        {
            $args   = array();
            ...
            $args['cd_ri_venda_previd_instit'] = $cd_ri_venda_previd_instit;

            if(intval($args['cd_ri_venda_previd_instit']) == 0)
            {
                $this->ri_venda_previd_instit_model->carrega_referencia($result, $args);
                $arr = $result->row_array();

                $data['row']['cd_ri_venda_previd_instit'] = $args['cd_ri_venda_previd_instit'];
                $data['row']['nr_valor_1']                = "";
                $data['row']['nr_valor_2']                = "";
                $data['row']['nr_valor_3']                = "";
                $data['row']['fl_media']                  = "";
            ...
            {
                $this->ri_venda_previd_instit_model->carrega($result, $args);
                $data['row'] = $result->row_array();
            }

            $this->load->view('indicador_plugin/ri_venda_previd_instit/cadastro', $data);
        }
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
    ...
    function salvar()
    {
        ...
        {		
            ...
            $args['cd_ri_venda_previd_instit'] = intval($this->input->post('cd_ri_venda_previd_instit', true));
            ...
            $this->ri_venda_previd_instit_model->salvar($result, $args);
            $this->criar_indicador();

            redirect("indicador_plugin/ri_venda_previd_instit", "refresh");
        }
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
    ...
    function excluir($cd_ri_venda_previd_instit)
    {
        if((indicador_db::verificar_permissao(usuario_id(), 'AC')) OR (indicador_db::verificar_permissao(usuario_id(), 'GE')))
        {
            $args   = array();
            ...
            $this->ri_venda_previd_instit_model->excluir($result, $args);
            $this->criar_indicador();
    ...
    function criar_indicador()
    {
        if((indicador_db::verificar_permissao(usuario_id(), 'AC')) OR (indicador_db::verificar_permissao(usuario_id(), 'GE')))
        {
            ...
            if(intval($contador_ano_atual) > 0)
            {
                ...
                $indicador[$linha][1] = number_format($nr_valor_1_total,0,',','.');
                $indicador[$linha][2] = number_format($nr_valor_2_total,0,',','.');
                $indicador[$linha][3] = number_format($nr_valor_3_total,0,',','.');
                $indicador[$linha][4] = number_format($nr_resultado_f_total,2,',','.');
    ...
    function fechar_periodo()
    {
        if((indicador_db::verificar_permissao(usuario_id(), 'AC')) OR (indicador_db::verificar_permissao(usuario_id(), 'GE')))
        {
            ...
            if(intval($contador_ano_atual) > 0)
            {
                $args['cd_ri_venda_previd_instit'] = 0;
            ...
            $this->ri_venda_previd_instit_model->fechar_periodo($result, $args);
