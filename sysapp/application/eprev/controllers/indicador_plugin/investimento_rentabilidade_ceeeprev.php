<?php
class Investimento_rentabilidade_ceeeprev extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::INVESTIMENTO_RENTABILIDADE_CEEEE);
		...
		$this->load->model('indicador_plugin/investimento_rentabilidade_ceeeprev_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
	        ...
	        $this->load->view('indicador_plugin/investimento_rentabilidade_ceeeprev/index',$data);
		}
    }
    ...
    function listar()
    {
    	if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
        {
			...
			$this->investimento_rentabilidade_ceeeprev_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/investimento_rentabilidade_ceeeprev/index_result', $data);
        }
        else
		{
    ...
    function cadastro($cd_investimento_rentabilidade_ceeeprev = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
			$args   = array();
			...
			$args['cd_investimento_rentabilidade_ceeeprev'] = $cd_investimento_rentabilidade_ceeeprev;
			
			if(intval($args['cd_investimento_rentabilidade_ceeeprev']) == 0)
			{
				$this->investimento_rentabilidade_ceeeprev_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_investimento_rentabilidade_ceeeprev'] = $args['cd_investimento_rentabilidade_ceeeprev'];
			...
			{
				$this->investimento_rentabilidade_ceeeprev_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/investimento_rentabilidade_ceeeprev/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_investimento_rentabilidade_ceeeprev'] = intval($this->input->post('cd_investimento_rentabilidade_ceeeprev', true));
			...
			$args["nr_valor_5"]                   = calculo_projetado_mensal($args["nr_valor_2"], intval($this->input->post("mes_referencia", true)));

			$arr_valor_1[] = $args["nr_valor_1"];
			$arr_valor_3[] = $args["nr_valor_3"];

			$args["nr_valor_4"]                    = calculo_acumulado($arr_valor_1, intval($this->input->post("mes_referencia", true)));
			$args["nr_valor_6"]                    = calculo_acumulado($arr_valor_3, intval($this->input->post("mes_referencia", true)));

			$args["nr_meta"]                       = app_decimal_para_db($this->input->post("nr_meta", true));
            $args["observacao"]                    = $this->input->post("observacao", true);
			$args["cd_usuario"]                    = $this->session->userdata('codigo');

			$this->investimento_rentabilidade_ceeeprev_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/investimento_rentabilidade_ceeeprev", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_investimento_rentabilidade_ceeeprev)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
			$args   = array();
			...
			$this->investimento_rentabilidade_ceeeprev_model->excluir($result, $args);
	...
	function criar_indicador()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
			...
			$sql.=indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 2,0, utf8_encode($data['label_4']), 'background,center');
			...
			$sql.=indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 6,0, utf8_encode($data['label_2']), 'background,center');
			...
			for($i=0; $i<sizeof($indicador); $i++)
			{
				...
				$sql .= indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 6, $linha, app_decimal_para_php($indicador[$i][2]), 'center', 'S', 4, 'S' );
			...
			$sql.=indicador_db::sql_inserir_grafico(
				...
				'2,2,0,0;5,5,0,0;4,4,0,0',
				"0,0,1,$linha_sem_media",
				"2,2,1,$linha_sem_media;5,5,1,$linha_sem_media;4,4,1,$linha_sem_media",
				usuario_id(),
				$coluna_para_ocultar,
				1,
				2
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->investimento_rentabilidade_ceeeprev_model->atualiza_fechar_periodo($result, $args);
			}

			$this->investimento_rentabilidade_ceeeprev_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
