<?php
class ri_midia_fav_des extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Total de mídia";
	var	$label_2 = "Total desfavorável";
	var	$label_3 = "% Desfavorável";
    var	$label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RI_MIDIA_FAVORAVEL_X_MIDIA_DESFAVORAVEL);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
	        ...
	        $this->load->view('indicador_plugin/ri_midia_fav_des/index.php');
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
        {
	        $this->load->model( 'indicador_plugin/Ri_midia_fav_des_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->Ri_midia_fav_des_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/ri_midia_fav_des/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$this->load->model('indicador_plugin/Ri_midia_fav_des_model');
			$row=$this->Ri_midia_fav_des_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.ri_midia_fav_des 
			...
			$this->load->view('indicador_plugin/ri_midia_fav_des/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$args['cd_ri_midia_fav_des']=intval($this->input->post('cd_ri_midia_fav_des', true));
			...
			$msg=array();
			$retorno = $this->Ri_midia_fav_des_model->salvar( $args,$msg );
			
			if($retorno)
			{
				redirect( "indicador_plugin/ri_midia_fav_des", "refresh" );			
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$this->Ri_midia_fav_des_model->excluir($id);
	...
	function criar_indicador()
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			{#tabela_existe
				...
				{
					$total_percentual = number_format( ( floatval($sum_v2)/floatval($sum_v1) ) * 100 , 2 );
				}
				else
				{
					$total_percentual = 0;
				...
				$indicador[$linha][0] = '<b>Totais de '.intval($tabela[0]['nr_ano_referencia']).'</b>';
				$indicador[$linha][1] = $sum_v1;
				$indicador[$linha][2] = $sum_v2;
				$indicador[$linha][3] = $total_percentual;
				$indicador[$linha][4] = '';
				$indicador[$linha][5] = '';
                $indicador[$linha][6] = '';
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			{#tabela_existe
				...
				if(floatval($sum_v1)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.ri_midia_fav_des
					( dt_referencia,dt_inclusao,cd_usuario_inclusao, nr_percentual_f, nr_meta, fl_media ) 
					VALUES ( '%s/01/01',current_timestamp,%s, %s, %s, 'S' ); "
					, intval($tabela[0]['nr_ano_referencia']) , usuario_id(), floatval($total_percentual), floatval( app_decimal_para_db($nr_meta) ));
				}
		...
		redirect( 'indicador_plugin/ri_midia_fav_des' );
