<?php
class ri_retrabalho extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Total de trabalhos realizados";
	var	$label_2 = "Total de Trabalhos refeitos";
	var	$label_3 = "% retrabalho";
    var	$label_4 = 'Meta';
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RI_RETRABALHO);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
	        ...
	        $this->load->view('indicador_plugin/ri_retrabalho/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
        {
	        $this->load->model( 'indicador_plugin/ri_retrabalho_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->ri_retrabalho_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/ri_retrabalho/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$this->load->model('indicador_plugin/ri_retrabalho_model');
			$row=$this->ri_retrabalho_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.ri_retrabalho 
			...
			$this->load->view('indicador_plugin/ri_retrabalho/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$args['cd_ri_retrabalho']=intval($this->input->post('cd_ri_retrabalho', true));
			...
			$msg=array();
			$retorno = $this->ri_retrabalho_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/ri_retrabalho", "refresh" );	
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			{
				redirect( "indicador_plugin/ri_retrabalho", "refresh" );
				#redirect( "indicador_plugin/ri_retrabalho/detalhe/0", "refresh" );
			}
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.ri_retrabalho
		...
		redirect( 'indicador_plugin/ri_retrabalho' );
