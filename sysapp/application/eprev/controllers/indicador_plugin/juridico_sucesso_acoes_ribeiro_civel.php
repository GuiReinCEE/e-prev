<?php
class Juridico_sucesso_acoes_ribeiro_civel extends Controller
{
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::JURIDICO_SUCESSO_DAS_ACOES_RIBEIRO_CIVEL);
		...
		$this->load->model('indicador_plugin/juridico_sucesso_acoes_ribeiro_civel_model' );   		
    }

    function index()
    {
		if((indicador_db::verificar_permissao(usuario_id(), 'GJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
			$args   = array();
			$data   = array();
			$result = null;
	        ...
	        $this->load->view('indicador_plugin/juridico_sucesso_acoes_ribeiro_civel/index',$data);
		}		
    }
    ...
    function listar()
    {
		if((indicador_db::verificar_permissao(usuario_id(), 'GJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
        {
			...
			$this->juridico_sucesso_acoes_ribeiro_civel_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/juridico_sucesso_acoes_ribeiro_civel/partial_result', $data);
        }    	
    }

	function detalhe($cd_juridico_sucesso_acoes_ribeiro_civel = 0)
	{
		if((indicador_db::verificar_permissao(usuario_id(), 'GJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
			$args   = array();
			...
			$args['cd_juridico_sucesso_acoes_ribeiro_civel'] = $cd_juridico_sucesso_acoes_ribeiro_civel;
			
			if(intval($args['cd_juridico_sucesso_acoes_ribeiro_civel']) == 0)
			{
				$data['row']['cd_juridico_sucesso_acoes_ribeiro_civel'] = $args['cd_juridico_sucesso_acoes_ribeiro_civel'];
				$data['row']['cd_etapa']       = "";
				$data['row']['nr_inicial']     = "";
			...
			{
				$this->juridico_sucesso_acoes_ribeiro_civel_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}
			
			$args['cd_indicador_tabela'] = $tabela[0]['cd_indicador_tabela'];
			$this->juridico_sucesso_acoes_ribeiro_civel_model->etapa($result, $args);
			$data['ar_fase'] = $result->result_array();			
			
			$this->load->view('indicador_plugin/juridico_sucesso_acoes_ribeiro_civel/detalhe', $data);
		}        
	}	
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_juridico_sucesso_acoes_ribeiro_civel'] = intval($this->input->post('cd_juridico_sucesso_acoes_ribeiro_civel', true));
			...
			$this->juridico_sucesso_acoes_ribeiro_civel_model->salvar($result, $args);
			$this->criar_indicador();
			
			redirect("indicador_plugin/juridico_sucesso_acoes_ribeiro_civel", "refresh");
		}
        else
        {
	...
	function excluir($cd_juridico_sucesso_acoes_ribeiro_civel)
	{
		if((indicador_db::verificar_permissao(usuario_id(), 'GJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
			$args   = array();
			...
			$this->juridico_sucesso_acoes_ribeiro_civel_model->excluir($result, $args);
			$this->criar_indicador();
	...
	function fechar_periodo()
	{
		if((indicador_db::verificar_permissao(usuario_id(), 'GJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
			...
			$this->juridico_sucesso_acoes_ribeiro_civel_model->fechar_periodo($result, $args);
		}
		redirect("indicador_plugin/juridico_sucesso_acoes_ribeiro_civel", "refresh");
	}	
}
