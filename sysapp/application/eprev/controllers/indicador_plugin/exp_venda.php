<?php
class exp_venda extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::EXP_VENDA);
		...
		$this->load->model('indicador_plugin/exp_venda_model');
    }
	...
	function index()
    {
		if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{
	        ...
	        $this->load->view('indicador_plugin/exp_venda/index',$data);
		}
    }
	...
	function listar()
    {
		if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
        {
			...
			$this->exp_venda_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/exp_venda/index_result', $data);
        }
    }
	
	function cadastro($cd_exp_venda = 0)
	{
		if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{
			$args   = array();
			...
			$args['cd_exp_venda'] = $cd_exp_venda;
			
			if(intval($args['cd_exp_venda']) == 0)
			{
				$this->exp_venda_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_exp_venda']    = $args['cd_exp_venda'];
				$data['row']['nr_venda']        = "";
				$data['row']['nr_ingresso']     = "";
				$data['row']['nr_desligamento'] = "";
				$data['row']['fl_media']        = "";
				$data['row']['observacao']      = "";
			...
			{
				$this->exp_venda_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/exp_venda/cadastro', $data);
		}
	}
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_exp_venda']        = intval($this->input->post('cd_exp_venda', true));
			$args["cd_indicador_tabela"] = $this->input->post("cd_indicador_tabela", true);
			$args["dt_referencia"]       = $this->input->post("dt_referencia", true);
			$args["fl_media"]            = $this->input->post("fl_media", true);
			$args["nr_venda"]            = app_decimal_para_db($this->input->post("nr_venda", true));
			$args["nr_ingresso"]         = app_decimal_para_db($this->input->post("nr_ingresso", true));
			$args["nr_desligamento"]     = app_decimal_para_db($this->input->post("nr_desligamento", true));
			$args["nr_meta"]             = app_decimal_para_db($this->input->post("nr_meta", true));
            $args["observacao"]          = $this->input->post("observacao", true);
			$args["cd_usuario"]          = $this->session->userdata('codigo');

			$this->exp_venda_model->salvar($result, $args);
			$this->criar_indicador();
			
			redirect("indicador_plugin/exp_venda", "refresh");
		}
        else
        {
	...
	function excluir($cd_exp_venda)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GE' ))
		{
			...
			$this->exp_venda_model->excluir($result, $args);
			$this->criar_indicador();
		...
		if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{
			...
			foreach($collection as $item)
			{
				if((intval($item['ano_referencia']) >= intval($tabela[0]['nr_ano_referencia']) - intval($tabela[0]['qt_periodo_anterior'])) OR ($fl_periodo))
				{
					...
					{
						$nr_venda_ano        += $item['nr_venda'];
						$nr_ingresso_ano     += $item['nr_ingresso'];
						$nr_desligamento_ano += $item['nr_desligamento'];
			...
			{
				$indicador[$i][5] = ($tend[$i] < 0 ? 0 : $tend[$i]);
			}	
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$ar_status = indicador_status_check($nr_venda_ano, 0, $nr_meta_ano, $tp_analise);
				...
				$indicador[$linha][2] = $nr_venda_ano;
			...
			$sql.=indicador_db::sql_inserir_grafico(
				...
				'2,2,0,0;3,3,0,0;5,5,0,0',
				"0,0,1,$linha_sem_media",
				"2,2,1,$linha_sem_media;3,3,1,$linha_sem_media;5,5,1,$linha_sem_media",
				usuario_id(),
				$coluna_para_ocultar,
				1,4
			);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GE' ))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$args['nr_venda']            = floatval($nr_venda_ano);
				$args['nr_ingresso']         = floatval($nr_ingresso_ano);
				$args['nr_desligamento']     = floatval($nr_desligamento_ano);
				$args['nr_meta']             = floatval($nr_meta_ano);
				$args["cd_usuario"]          = $this->session->userdata('codigo');

				$this->exp_venda_model->fechar_periodo($result, $args);
			}
		}
