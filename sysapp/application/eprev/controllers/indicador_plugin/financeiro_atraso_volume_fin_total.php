<?php
class financeiro_atraso_volume_fin_total extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Valor da Carteira";
	var	$label_2 = "Valor dos Atrasos";
	var	$label_3 = "% de Atrasos";
    var $label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::FINANCEIRO_ATRASOS_EM_VOLUME_FINANCEIRO_TOTAL);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/financeiro_atraso_volume_fin_total/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
        {
	        $this->load->model( 'indicador_plugin/financeiro_atraso_volume_fin_total_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->financeiro_atraso_volume_fin_total_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/financeiro_atraso_volume_fin_total/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$this->load->model('indicador_plugin/financeiro_atraso_volume_fin_total_model');
			$row=$this->financeiro_atraso_volume_fin_total_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					cd_indicador_tabela,
				   (SELECT COUNT(*)
				      FROM indicador_plugin.financeiro_atraso_volume_fin_total
			...
			$this->load->view('indicador_plugin/financeiro_atraso_volume_fin_total/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$args['cd_financeiro_atraso_volume_fin_total']=intval($this->input->post('cd_financeiro_atraso_volume_fin_total', true));
			...
			$msg=array();
			$retorno = $this->financeiro_atraso_volume_fin_total_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/financeiro_atraso_volume_fin_total", "refresh" );	
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$this->financeiro_atraso_volume_fin_total_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.financeiro_atraso_volume_fin_total
		...
		redirect( 'indicador_plugin/financeiro_atraso_volume_fin_total' );
