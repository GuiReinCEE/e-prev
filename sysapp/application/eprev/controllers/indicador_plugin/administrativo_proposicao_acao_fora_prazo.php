<?php
class Administrativo_proposicao_acao_fora_prazo extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::RH_PREPOSICAO_DE_ACOES_CORRETIVAS_FORA_DO_PRAZO);
		...
		$this->load->model('indicador_plugin/administrativo_proposicao_acao_fora_prazo_model');
    }
    ...
    function index()
    {
		if($this->fl_permissao)
		{
	        ...
	        $this->load->view('indicador_plugin/administrativo_proposicao_acao_fora_prazo/index',$data);
		}
    }
    ...
    function listar()
    {
    	if($this->fl_permissao)
        {
			...
			$this->administrativo_proposicao_acao_fora_prazo_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/administrativo_proposicao_acao_fora_prazo/index_result', $data);
        }
        else
		{
    ...
    function cadastro($cd_administrativo_proposicao_acao_fora_prazo = 0)
	{
		if($this->fl_permissao)
		{
			$args   = array();
			...
			$args['cd_administrativo_proposicao_acao_fora_prazo'] = $cd_administrativo_proposicao_acao_fora_prazo;
			
			if(intval($args['cd_administrativo_proposicao_acao_fora_prazo']) == 0)
			{
				$this->administrativo_proposicao_acao_fora_prazo_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_administrativo_proposicao_acao_fora_prazo'] = $args['cd_administrativo_proposicao_acao_fora_prazo'];
				$data['row']['nr_valor_1']            = (isset($arr['nr_valor_1']) ? $arr['nr_valor_1'] : "");
				$data['row']['nr_valor_2']            = (isset($arr['nr_valor_2']) ? $arr['nr_valor_2'] : "");
				$data['row']['fl_media']              = "";
				$data['row']['observacao']            = "";
			...
			{
				$this->administrativo_proposicao_acao_fora_prazo_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/administrativo_proposicao_acao_fora_prazo/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_administrativo_proposicao_acao_fora_prazo'] = intval($this->input->post('cd_administrativo_proposicao_acao_fora_prazo', true));
			...
			$this->administrativo_proposicao_acao_fora_prazo_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/administrativo_proposicao_acao_fora_prazo", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_administrativo_proposicao_acao_fora_prazo)
	{
		if($this->fl_permissao)
		{
			$args   = array();
			...
			$this->administrativo_proposicao_acao_fora_prazo_model->excluir($result, $args);
	...
	function fechar_periodo()
	{
		if($this->fl_permissao)
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->administrativo_proposicao_acao_fora_prazo_model->atualiza_fechar_periodo($result, $args);
			}

			$this->administrativo_proposicao_acao_fora_prazo_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
