<?php
class Administrativo_nr_correspondencia extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::RH_NUMERO_CORRESPONDECIAS);
		...
		$this->load->model('indicador_plugin/administrativo_nr_correspondencia_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
	        ...
	        $this->load->view('indicador_plugin/administrativo_nr_correspondencia/index',$data);
		}
    }
    ...
    function listar()
    {
    	if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
        {
			...
			$this->administrativo_nr_correspondencia_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/administrativo_nr_correspondencia/index_result', $data);
        }
        else
		{
    ...
    function cadastro($cd_administrativo_nr_correspondencia = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$args['cd_administrativo_nr_correspondencia'] = $cd_administrativo_nr_correspondencia;
			
			if(intval($args['cd_administrativo_nr_correspondencia']) == 0)
			{
				$this->administrativo_nr_correspondencia_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_administrativo_nr_correspondencia'] = $args['cd_administrativo_nr_correspondencia'];
			...
			{
				$this->administrativo_nr_correspondencia_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/administrativo_nr_correspondencia/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_administrativo_nr_correspondencia'] = intval($this->input->post('cd_administrativo_nr_correspondencia', true));
			...
			$this->administrativo_nr_correspondencia_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/administrativo_nr_correspondencia", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_administrativo_nr_correspondencia)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$this->administrativo_nr_correspondencia_model->excluir($result, $args);
		...
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			foreach($collection as $item)
			{
				if(intval($item['ano_referencia']) >= intval($tabela[0]['nr_ano_referencia']) -5)
				{
					...
					$indicador[$linha][4] = nl2br($observacao);
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$indicador[$linha][2] = app_decimal_para_php($nr_meta_ano);
				$indicador[$linha][3] = '';
				$indicador[$linha][4] = '';
			}
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->administrativo_nr_correspondencia_model->atualiza_fechar_periodo($result, $args);
			}

			$this->administrativo_nr_correspondencia_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
