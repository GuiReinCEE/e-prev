<?php
class Exp_volume_recursos_inpel extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::EXP_VOLUME_DE_RECURSOS_FINANCEIROS_CONTRATADOS_INPEL);
		...
		$this->load->model('indicador_plugin/exp_volume_recursos_inpel_model');
    }
	...
	public function index()
    {
    	if((indicador_db::verificar_permissao($this->cd_usuario, 'GN')) OR ($this->session->userdata('indic_12') == '*'))
		{
	        ...
	        $this->load->view('indicador_plugin/exp_volume_recursos_inpel/index', $data);
		}
		else
        {
	...
	public function listar()
    {
		if((indicador_db::verificar_permissao($this->cd_usuario, 'GN')) OR ($this->session->userdata('indic_12') == '*'))
        {
			...
			$data['collection'] = $this->exp_volume_recursos_inpel_model->listar($data['tabela'][0]['cd_indicador_tabela']);
			
			$this->load->view('indicador_plugin/exp_volume_recursos_inpel/index_result', $data);
        }
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function cadastro($cd_exp_volume_recursos_inpel = 0)
	{
		if((indicador_db::verificar_permissao($this->cd_usuario, 'GN')) OR ($this->session->userdata('indic_12') == '*'))
		{
			$data = array();
			...
			if(intval($cd_exp_volume_recursos_inpel) == 0)
			{
				$row = $this->exp_volume_recursos_inpel_model->carrega_referencia($data['tabela'][0]['cd_indicador_tabela']);
				
				$data['row'] = array(
					'cd_exp_volume_recursos_inpel' => intval($cd_exp_volume_recursos_inpel),
			...
			{
				$data['row'] = $this->exp_volume_recursos_inpel_model->carrega(intval($cd_exp_volume_recursos_inpel));
			}

			$this->load->view('indicador_plugin/exp_volume_recursos_inpel/cadastro', $data);
		}
		else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function salvar()
	{
		...
		{		
			$cd_exp_volume_recursos_inpel = intval($this->input->post('cd_exp_volume_recursos_inpel', true));
			...
			if(intval($cd_exp_volume_recursos_inpel) == 0)
			{
				$this->exp_volume_recursos_inpel_model->salvar($args);
			}
			else
			{
				$this->exp_volume_recursos_inpel_model->atualizar($cd_exp_volume_recursos_inpel, $args);
			}

			$this->criar_indicador();
			
			redirect('indicador_plugin/exp_volume_recursos_inpel', 'refresh');
		}
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function excluir($cd_exp_volume_recursos_inpel)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GN'))
		{
			$this->exp_volume_recursos_inpel_model->excluir(intval($cd_exp_volume_recursos_inpel), $this->cd_usuario);
	...
	public function criar_indicador()
    {
    	if((indicador_db::verificar_permissao($this->cd_usuario, 'GN')) OR ($this->session->userdata('indic_12') == '*'))
		{
			...
			$collection = $this->exp_volume_recursos_inpel_model->listar($tabela[0]['cd_indicador_tabela']);
	...
	public function fechar_periodo()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GN'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->exp_volume_recursos_inpel_model->fechar_ano($args);
			}

			$this->exp_volume_recursos_inpel_model->fechar_indicador($tabela[0]['cd_indicador_tabela'], $this->cd_usuario);
