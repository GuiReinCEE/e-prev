<?php
class Exp_adesao_potencial_ceeeprev extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::EXP_NUMERO_DE_ADESAO_X_NUMERO_POTENCIAL_CEEEPREV);
		...
		$this->load->model('indicador_plugin/exp_adesao_potencial_ceeeprev_model');
    }
    ...
    function index()
    {
		if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{
	        ...
	        $this->load->view('indicador_plugin/exp_adesao_potencial_ceeeprev/index',$data);
		}
    }
    ...
    function listar()
    {
    	if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
        {
			...
			$this->exp_adesao_potencial_ceeeprev_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/exp_adesao_potencial_ceeeprev/index_result', $data);
        }
        else
		{
    ...
    function cadastro($cd_exp_adesao_potencial_ceeeprev = 0)
	{
		if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{
			$args   = array();
			...
			$args['cd_exp_adesao_potencial_ceeeprev'] = $cd_exp_adesao_potencial_ceeeprev;
			
			if(intval($args['cd_exp_adesao_potencial_ceeeprev']) == 0)
			{
				$this->exp_adesao_potencial_ceeeprev_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_exp_adesao_potencial_ceeeprev'] = $args['cd_exp_adesao_potencial_ceeeprev'];
			...
			{
				$this->exp_adesao_potencial_ceeeprev_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/exp_adesao_potencial_ceeeprev/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_exp_adesao_potencial_ceeeprev'] = intval($this->input->post('cd_exp_adesao_potencial_ceeeprev', true));
			...
			$this->exp_adesao_potencial_ceeeprev_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/exp_adesao_potencial_ceeeprev", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_exp_adesao_potencial_ceeeprev)
	{
		if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{
			$args   = array();
			...
			$this->exp_adesao_potencial_ceeeprev_model->excluir($result, $args);
	...
	function criar_indicador()
	{
		if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{
			...
			$sql.=indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 4,0, utf8_encode($data['label_7']), 'background,center');
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$indicador[$linha][1] = app_decimal_para_php($nr_valor_3_resultado);
				$indicador[$linha][2] = app_decimal_para_php($nr_valor_1_resultado);
				$indicador[$linha][3] = app_decimal_para_php($nr_valor_2_resultado);
				$indicador[$linha][4] = app_decimal_para_php($nr_percentual_resultado);
				$indicador[$linha][5] = app_decimal_para_php($nr_percentual_resultado);
				$indicador[$linha][6] = '';
			}
			*/
			$linha = 1;
			...
			$sql.=indicador_db::sql_inserir_grafico(
				...
				"5,5,1,$linha_sem_media;6,6,1,$linha_sem_media-linha",
				usuario_id(),
				$coluna_para_ocultar,
				1,
				2
	...
	function fechar_periodo()
	{
		if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->exp_adesao_potencial_ceeeprev_model->atualiza_fechar_periodo($result, $args);
			}

			$this->exp_adesao_potencial_ceeeprev_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
