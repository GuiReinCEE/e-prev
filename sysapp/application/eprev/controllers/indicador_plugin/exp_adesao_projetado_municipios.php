<?php
class exp_adesao_projetado_municipios extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::EXP_NUMERO_DE_ADESAO_X_NUMERO_PROJETADO_MUNICIPIOS);
		...
		$this->load->model('indicador_plugin/exp_adesao_projetado_municipios_model');
    }
    ...
    public function index()
    {
		if((indicador_db::verificar_permissao($this->cd_usuario, 'GN')) OR ($this->session->userdata('indic_12') == '*'))
		{
			...
			$this->load->view('indicador_plugin/exp_adesao_projetado_municipios/index',$data);
		}
		else
        {
    ...
    public function listar()
    {
    	if((indicador_db::verificar_permissao($this->cd_usuario, 'GN')) OR ($this->session->userdata('indic_12') == '*'))
        {
			...
			$data['collection'] = $this->exp_adesao_projetado_municipios_model->listar($data['tabela'][0]['cd_indicador_tabela']);
	        
			$this->load->view('indicador_plugin/exp_adesao_projetado_municipios/index_result', $data);
        }
        else
		{
			exibir_mensagem('ACESSO NÃO PERMITIDO');
    ...
    public function cadastro($cd_exp_adesao_projetado_municipios = 0)
	{
		if((indicador_db::verificar_permissao($this->cd_usuario, 'GN')) OR ($this->session->userdata('indic_12') == '*'))
		{
			$data['label_0'] = $this->label_0;
			...
			if(intval($cd_exp_adesao_projetado_municipios) == 0)
			{
				$row = $this->exp_adesao_projetado_municipios_model->carrega_referencia();
								
				$data['row'] =array(
					'cd_exp_adesao_projetado_municipios' => $cd_exp_adesao_projetado_municipios,
			...
			{
				$data['row'] = $this->exp_adesao_projetado_municipios_model->carrega($cd_exp_adesao_projetado_municipios);
			}

			$this->load->view('indicador_plugin/exp_adesao_projetado_municipios/cadastro', $data);
		}
		else
		{
			exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function salvar()
	{
		...
		{		
			...
			$args = array(
				'cd_exp_adesao_projetado_municipios' => $this->input->post('cd_exp_adesao_projetado_municipios', true),
			...
			$this->exp_adesao_projetado_municipios_model->salvar($args);
			
			$this->criar_indicador();
			
			redirect('indicador_plugin/exp_adesao_projetado_municipios', 'refresh');
		}
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function excluir($cd_exp_adesao_projetado_municipios)
	{
		if((indicador_db::verificar_permissao($this->cd_usuario,'GN')) OR ($this->session->userdata('indic_12') == '*'))
		{
			$this->exp_adesao_projetado_municipios_model->excluir($cd_exp_adesao_projetado_municipios, $this->cd_usuario);
	...
	public function criar_indicador()
	{
		if((indicador_db::verificar_permissao($this->cd_usuario, 'GN')) OR ($this->session->userdata('indic_12') == '*'))
		{
			...
			$collection = $this->exp_adesao_projetado_municipios_model->listar($tabela[0]['cd_indicador_tabela']);
	...
	public function fechar_periodo()
	{
		if((indicador_db::verificar_permissao($this->cd_usuario, 'GN')) OR ($this->session->userdata('indic_12') == '*'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$args = array(
					'cd_exp_adesao_projetado_municipios' => 0,
			...
			$this->exp_adesao_projetado_municipios_model->fechar_periodo($tabela[0]['cd_indicador_tabela'], $this->cd_usuario);
