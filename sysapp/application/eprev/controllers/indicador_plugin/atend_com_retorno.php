<?php
class Atend_com_retorno extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::ATENDIMENTO_COM_RETORNO);
		...
		$this->load->model('indicador_plugin/atend_com_retorno_model');
    }
	...
	function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GP'))
		{
	        ...
	        $this->load->view('indicador_plugin/atend_com_retorno/index', $data);
		}
		else
		{
	...
	function listar()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GP'))
        {
			...
			$this->atend_com_retorno_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/atend_com_retorno/index_result', $data);
        }
        else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function cadastro($cd_atend_com_retorno = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GP'))
		{
			$args   = array();
			...
			$args['cd_atend_com_retorno'] = $cd_atend_com_retorno;
			
			if(intval($args['cd_atend_com_retorno']) == 0)
			{
				$this->atend_com_retorno_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_atend_com_retorno'] = $args['cd_atend_com_retorno'];
				$data['row']['nr_valor_1']            = "";
				$data['row']['nr_valor_2']            = "";
				$data['row']['fl_media']              = "";
				$data['row']['observacao']            = "";
			...
			{
				$this->atend_com_retorno_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/atend_com_retorno/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_atend_com_retorno'] = intval($this->input->post('cd_atend_com_retorno', true));
			...
			$this->atend_com_retorno_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/atend_com_retorno", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_atend_com_retorno)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GP'))
		{
			$args   = array();
			...
			$this->atend_com_retorno_model->excluir($result, $args);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GP'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->atend_com_retorno_model->atualiza_fechar_periodo($result, $args);
			}

			$this->atend_com_retorno_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
