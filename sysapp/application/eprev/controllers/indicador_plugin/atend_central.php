<?php
class atend_central extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Central";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::ATENDIMENTO_TOTAL_DE_PARTICIPANTES_ATENDIDOS_NA_CENTRAL);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/atend_central/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
        {
	        $this->load->model( 'indicador_plugin/atend_central_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->atend_central_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/atend_central/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if( CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->load->model('indicador_plugin/atend_central_model');
			$row=$this->atend_central_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.atend_central 
			...
			$this->load->view('indicador_plugin/atend_central/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if( CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$args['cd_atend_central']=intval($this->input->post('cd_atend_central', true));
			...
			$msg=array();
			$retorno=$this->atend_central_model->salvar( $args, $msg );
			...
			if( $retorno )
			{
				...
				{
					redirect( "indicador_plugin/atend_central", "refresh" );
				}
			}
			else
			{
	...
	function excluir( $id )
	{
		...
		if( CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->atend_central_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$args['cd_indicador_tabela'] = $tabela[0]['cd_indicador_tabela'];
	        $this->atend_central_model->listar( $result, $args );
			$ar_atend_central = $result->result_array();
			...
			{#tabela_existe
				...
				$total=0;
				foreach( $ar_atend_central as $item )
				{
					$total+=floatval($item['nr_valor_1']);
				}

				if(count($ar_atend_central) >= 12)
				{
					$sql="
							INSERT INTO indicador_plugin.atend_central
								 (
									dt_referencia, 
									cd_usuario_inclusao, 
									fl_media, 
		...
		redirect( 'indicador_plugin/atend_central' );
