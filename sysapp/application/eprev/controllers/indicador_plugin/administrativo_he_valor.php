<?php
class administrativo_he_valor extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Valor Total da Folha";
	var	$label_2 = "Valor Total Horas Extras";
	var	$label_3 = "% Horas Extras";
    var	$label_4 = "Meta";
    var	$label_5 = "Tendência";
    var	$label_6 = "Referencial";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RH_HORA_EXTRA_VALOR);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GGS' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/administrativo_he_valor/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GGS' ) )
        {
	        $this->load->model( 'indicador_plugin/administrativo_he_valor_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->administrativo_he_valor_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/administrativo_he_valor/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GGS' ) )
		{
			...
			$this->load->model('indicador_plugin/administrativo_he_valor_model');
			$row=$this->administrativo_he_valor_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					$sql = "
					SELECT to_char(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS mes_referencia, dt_referencia, nr_meta, nr_referencial, cd_indicador_tabela
					FROM indicador_plugin.administrativo_he_valor 
			...
			$this->load->view('indicador_plugin/administrativo_he_valor/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GGS' ) )
		{
			...
			$args['cd_administrativo_he_valor']=intval($this->input->post('cd_administrativo_he_valor', true));
			...
			$msg=array();
			$retorno = $this->administrativo_he_valor_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/administrativo_he_valor", "refresh" );			
				}
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GGS' ) )
		{
			...
			$this->administrativo_he_valor_model->excluir($id);
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GGS' ) )
		{
			...
			{#tabela_existe
				...
				$sql.=indicador_db::sql_inserir_grafico(
					...
					"3,3,1,$linha_sem_media;4,4,1,$linha_sem_media;5,5,1,$linha_sem_media;6,6,1,$linha_sem_media",
					usuario_id(),
					$coluna_para_ocultar,
                    1,
                    2,
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GGS' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.administrativo_he_valor
					( dt_referencia,dt_inclusao,cd_usuario_inclusao, nr_percentual_f, nr_meta, fl_media, nr_referencial ) 
					VALUES ( '%s/01/01',current_timestamp,%s, %s, %s, 'S', %s ); "
					, intval($tabela[0]['nr_ano_referencia']) , usuario_id(), floatval($media), floatval( $nr_meta ),  floatval( $nr_referencial));
				}
		...
		redirect( 'indicador_plugin/administrativo_he_valor' );
