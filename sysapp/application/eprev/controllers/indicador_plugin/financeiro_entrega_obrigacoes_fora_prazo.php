<?php
class Financeiro_entrega_obrigacoes_fora_prazo extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::FINANCEIRO_ENTREGA_OBRIGACOES_FORA_DO_PRAZO);
		...
		$this->load->model('indicador_plugin/financeiro_entrega_obrigacoes_fora_prazo_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
	        ...
	        $this->load->view('indicador_plugin/financeiro_entrega_obrigacoes_fora_prazo/index',$data);
		}
    }
    ...
    function listar()
    {
    	if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
        {
			...
			$this->financeiro_entrega_obrigacoes_fora_prazo_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/financeiro_entrega_obrigacoes_fora_prazo/index_result', $data);
        }
        else
		{
    ...
    function cadastro($cd_financeiro_entrega_obrigacoes_fora_prazo = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$args['cd_financeiro_entrega_obrigacoes_fora_prazo'] = $cd_financeiro_entrega_obrigacoes_fora_prazo;
			
			if(intval($args['cd_financeiro_entrega_obrigacoes_fora_prazo']) == 0)
			{
				$this->financeiro_entrega_obrigacoes_fora_prazo_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_financeiro_entrega_obrigacoes_fora_prazo'] = $args['cd_financeiro_entrega_obrigacoes_fora_prazo'];
			...
			{
				$this->financeiro_entrega_obrigacoes_fora_prazo_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/financeiro_entrega_obrigacoes_fora_prazo/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_financeiro_entrega_obrigacoes_fora_prazo'] = intval($this->input->post('cd_financeiro_entrega_obrigacoes_fora_prazo', true));
			...
			$this->financeiro_entrega_obrigacoes_fora_prazo_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/financeiro_entrega_obrigacoes_fora_prazo", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_financeiro_entrega_obrigacoes_fora_prazo)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$this->financeiro_entrega_obrigacoes_fora_prazo_model->excluir($result, $args);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->financeiro_entrega_obrigacoes_fora_prazo_model->atualiza_fechar_periodo($result, $args);
			}

			$this->financeiro_entrega_obrigacoes_fora_prazo_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
