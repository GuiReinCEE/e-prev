<?php
class Juridico_imapcto_adm_acoes extends Controller
{
    ...
    function __construct()
    {
        ...
        $this->enum_indicador = intval(enum_indicador::JURIDICO_IMPACTO_ADM_DAS_ACOES);
        ...
        $this->load->model('indicador_plugin/juridico_imapcto_adm_acoes_model');
    }
    ...
    function index()
    {
        if(indicador_db::verificar_permissao(usuario_id(), 'GP'))
        {
            ...
            $this->load->view('indicador_plugin/juridico_imapcto_adm_acoes/index', $data);
        }
        else
        {
    ...
    function listar()
    {
        if(indicador_db::verificar_permissao(usuario_id(), 'GP'))
        {
            ...
            $this->juridico_imapcto_adm_acoes_model->listar($result, $args);
            $data['collection'] = $result->result_array();

            $this->load->view('indicador_plugin/juridico_imapcto_adm_acoes/index_result', $data);
        }
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
    ...
    function cadastro($cd_juridico_imapcto_adm_acoes = 0)
    {
        if(indicador_db::verificar_permissao(usuario_id(), 'GP'))
        {
            $args   = array();
            ...
            $args['cd_juridico_imapcto_adm_acoes'] = $cd_juridico_imapcto_adm_acoes;

            if(intval($args['cd_juridico_imapcto_adm_acoes']) == 0)
            {
                $this->juridico_imapcto_adm_acoes_model->carrega_referencia($result, $args);
                $arr = $result->row_array();

                $data['row']['cd_juridico_imapcto_adm_acoes'] = $args['cd_juridico_imapcto_adm_acoes'];
            ...
            {
                $this->juridico_imapcto_adm_acoes_model->carrega($result, $args);
                $data['row'] = $result->row_array();
            }

            $this->load->view('indicador_plugin/juridico_imapcto_adm_acoes/cadastro', $data);
        }
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
    ...
    function salvar()
    {
        ...
        {		
            ...
            $args['cd_juridico_imapcto_adm_acoes'] = intval($this->input->post('cd_juridico_imapcto_adm_acoes', true));
            ...
            $this->juridico_imapcto_adm_acoes_model->salvar($result, $args);
            $this->criar_indicador();

            redirect("indicador_plugin/juridico_imapcto_adm_acoes", "refresh");
        }
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
    ...
    function excluir($cd_juridico_imapcto_adm_acoes)
    {
        if(indicador_db::verificar_permissao(usuario_id(), 'GP'))
        {
            $args   = array();
            ...
            $this->juridico_imapcto_adm_acoes_model->excluir($result, $args);
            $this->criar_indicador();
    ...
    function criar_indicador()
    {
        if(indicador_db::verificar_permissao(usuario_id(), 'GP'))
        {
            ...
            if(intval($contador_ano_atual) > 0)
            {
                ...
                $indicador[$linha][3] = app_decimal_para_php($nr_resultado_f_total);
                $indicador[$linha][4] = app_decimal_para_php($nr_meta);
                $indicador[$linha][5] = '';
                $indicador[$linha][6] = '';
            }
        ...
        if(indicador_db::verificar_permissao(usuario_id(), 'GP'))
        {
            ...
            {			 
                if( intval($item['ano_referencia']) == intval($tabela[0]['nr_ano_referencia']) )
                {
                    ...
                    $nr_resultado_f_total += $item["nr_percentual_f"];
            ...
            if(intval($contador_ano_atual) > 0)
            {
                ...
                $nr_resultado_f_total = ($nr_resultado_f_total / $contador_ano_atual);

                $args['cd_juridico_imapcto_adm_acoes'] = 0;
                $args["cd_indicador_tabela"]       = $args['cd_indicador_tabela'];
                $args["dt_referencia"]             = '01/01/'.intval($tabela[0]['nr_ano_referencia']);
                $args["fl_media"]                  = 'S';
                $args["nr_percentual_f"]           = $nr_resultado_f_total;
                $args["nr_meta"]                   = app_decimal_para_db($nr_meta);
                $args["observacao"]                = "";
                $args["cd_usuario"]                = $this->session->userdata('codigo');

                $this->juridico_imapcto_adm_acoes_model->salvar_fechar_periodo($result, $args);
            }

            $this->juridico_imapcto_adm_acoes_model->fechar_periodo($result, $args);
