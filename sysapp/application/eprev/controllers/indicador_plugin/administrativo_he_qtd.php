<?php
class administrativo_he_qtd extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Carga Total de Trabalho";
	var	$label_2 = "Quantidade de Horas Extras";
	var	$label_3 = "% Horas Extras - Quantidade";
    var	$label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RH_HORA_EXTRA_QUANTIDADE);
    }

    function index()
    {
		$this->load->helper( array('indicador') );
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GS' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/administrativo_he_qtd/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GS' ) )
        {
	        $this->load->model( 'indicador_plugin/administrativo_he_qtd_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->administrativo_he_qtd_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/administrativo_he_qtd/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GS' ) )
		{
			...
			$this->load->model('indicador_plugin/administrativo_he_qtd_model');
			$row=$this->administrativo_he_qtd_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.administrativo_he_qtd 
			...
			$this->load->view('indicador_plugin/administrativo_he_qtd/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GS' ) )
		{
			...
			$args['cd_administrativo_he_qtd']=intval($this->input->post('cd_administrativo_he_qtd', true));
			...
			$msg=array();
			$retorno = $this->administrativo_he_qtd_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/administrativo_he_qtd", "refresh" );
				}
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GS' ) )
		{
			...
			$this->administrativo_he_qtd_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GS' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.administrativo_he_qtd
		...
		redirect( 'indicador_plugin/administrativo_he_qtd' );
