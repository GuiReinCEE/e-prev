<?php
class ri_cumprimento_planej_marketing extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Total etapas projetos previstas";
	var	$label_2 = "Total etapas realizadas";
	var	$label_3 = "% cumprimento de planejado";
    var $label_4 = 'Meta';
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RI_CUMPRIMENTO_PLANEJAMENTO_DE_MARKETING);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
	        ...
	        $this->load->view('indicador_plugin/ri_cumprimento_planej_marketing/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
        {
	        $this->load->model( 'indicador_plugin/ri_cumprimento_planej_marketing_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->ri_cumprimento_planej_marketing_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/ri_cumprimento_planej_marketing/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$this->load->model('indicador_plugin/ri_cumprimento_planej_marketing_model');
			$row=$this->ri_cumprimento_planej_marketing_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.ri_cumprimento_planej_marketing
			...
			$this->load->view('indicador_plugin/ri_cumprimento_planej_marketing/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$args['cd_ri_cumprimento_planej_marketing']=intval($this->input->post('cd_ri_cumprimento_planej_marketing', true));
			...
			$msg=array();
			$retorno = $this->ri_cumprimento_planej_marketing_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/ri_cumprimento_planej_marketing", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$this->ri_cumprimento_planej_marketing_model->excluir($id);
	...
	function criar_indicador()
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			{#tabela_existe
				...
				if(floatval($nr_valor_f1)>0)
				{
					$media = 0;
                    $media = ((floatval($nr_valor_f2)/floatval($nr_valor_f1))*100);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			{#tabela_existe
				...
				if(floatval($nr_valor_1)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.ri_cumprimento_planej_marketing
		...
		redirect( 'indicador_plugin/ri_cumprimento_planej_marketing' );
