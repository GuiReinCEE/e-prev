<?php
class administrativo_doc_digitalizados extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Total";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RH_DOCUMENTOS_DIGITALIZADOS);
    ...
    function index()
    {
		if($this->fl_permissao)
		{
	        ...
	        $this->load->view('indicador_plugin/administrativo_doc_digitalizados/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if($this->fl_permissao)
		{
	        $this->load->model( 'indicador_plugin/administrativo_doc_digitalizados_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->administrativo_doc_digitalizados_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/administrativo_doc_digitalizados/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if($this->fl_permissao)
		{
			...
			$this->load->model('indicador_plugin/administrativo_doc_digitalizados_model');
			$row=$this->administrativo_doc_digitalizados_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
			...
			$this->load->view('indicador_plugin/administrativo_doc_digitalizados/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		if($this->fl_permissao)
		{
			...
			$args['cd_administrativo_doc_digitalizados']=intval($this->input->post('cd_administrativo_doc_digitalizados', true));
			...
			$msg=array();
			$retorno=$this->administrativo_doc_digitalizados_model->salvar( $args, $msg );
			...
			if( $retorno )
			{
				...
				{
					redirect( "indicador_plugin/administrativo_doc_digitalizados", "refresh" );
				}
			}
			else
			{
	...
	function excluir( $id )
	{
		if($this->fl_permissao)
		{
			...
			$this->administrativo_doc_digitalizados_model->excluir($id);
		...
		if($this->fl_permissao)
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.administrativo_doc_digitalizados
					( dt_referencia,dt_inclusao,cd_usuario_inclusao, nr_valor_1,  fl_media )
					VALUES ( '%s/01/01',current_timestamp,%s, %s, 'S' ); "
					, intval($tabela[0]['nr_ano_referencia']) , usuario_id(), floatval($soma) );
				}
                ...
                $this->load->model('indicador_plugin/administrativo_doc_digitalizados_acum_model');
                $this->administrativo_doc_digitalizados_acum_model->fechar_periodo();

			} #tabela_existe
		}

		redirect( 'indicador_plugin/administrativo_doc_digitalizados' );
