<?php
class info_satisfacao extends Controller
{
	var	$label_0 = "Ano";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::INFO_SATISFACAO);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GTI' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/info_satisfacao/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GTI' ) )
        {
	        $this->load->model( 'indicador_plugin/info_satisfacao_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->info_satisfacao_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/info_satisfacao/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GTI' ) )
		{
			...
			$this->load->model('indicador_plugin/info_satisfacao_model');
			$row=$this->info_satisfacao_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.info_satisfacao
			...
			$this->load->view('indicador_plugin/info_satisfacao/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GTI' ) )
		{
			...
			$args['cd_info_satisfacao']=intval($this->input->post('cd_info_satisfacao', true));
			...
			$msg=array();
			$retorno = $this->info_satisfacao_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/info_satisfacao", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GTI' ) )
		{
			...
			$this->info_satisfacao_model->excluir($id);
			...
			{#tabela_existe
				...
				foreach( $collection as $item )
				{
					...
					if(true)
					{
						...
						$indicador[$linha][3] = floatval($nr_meta);
