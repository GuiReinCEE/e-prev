<?php
class Atuarial_divulgacao_extratos_prazo extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::ATUARIAL_DIVULGACAO_EXTRATO_PRAZO);
		...
		$this->load->model('indicador_plugin/atuarial_divulgacao_extratos_prazo_model');
    }
    ...
    public function index()
    {
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GAP.'))
		{
	        ...
	        $this->load->view('indicador_plugin/atuarial_divulgacao_extratos_prazo/index', $data);
		}
		else
		{
    ...
    public function listar()
    {
        ...
        {			
			...
			$data['collection'] = $this->atuarial_divulgacao_extratos_prazo_model->listar($data['tabela'][0]['cd_indicador_tabela'] );
			
			$this->load->view('indicador_plugin/atuarial_divulgacao_extratos_prazo/index_result', $data);
        }
        else
		{
			exibir_mensagem('ACESSO NÃO PERMITIDO');
    ...
    public function cadastro($cd_atuarial_divulgacao_extratos_prazo = 0)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GAP.'))
		{			
			$data['label_0'] = $this->label_0;
			...
			if(intval($cd_atuarial_divulgacao_extratos_prazo) == 0)
			{
				$row = $this->atuarial_divulgacao_extratos_prazo_model->carrega_referencia(intval($data['tabela'][0]['nr_ano_referencia']));
				
				$data['row'] = array(
					'cd_atuarial_divulgacao_extratos_prazo' => intval($cd_atuarial_divulgacao_extratos_prazo),
				    'fl_media'             		            => '',
				    'nr_total'                              => 0,
				    'nr_atendidas'                          => 0,
			...
			{
				$data['row'] = $this->atuarial_divulgacao_extratos_prazo_model->carrega($cd_atuarial_divulgacao_extratos_prazo);
				$data['row']['qt_ano'] = 1;
			}

			$this->load->view('indicador_plugin/atuarial_divulgacao_extratos_prazo/cadastro', $data);
		}
		else
		{
			exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function salvar()
	{
		...
		{		
			...
			$nr_atendidas = app_decimal_para_db($this->input->post('nr_atendidas', true));
			$nr_resultado = ($nr_atendidas / $nr_total) * 100;

			$args = array(
				'cd_atuarial_divulgacao_extratos_prazo' => intval($this->input->post('cd_atuarial_divulgacao_extratos_prazo', true)),
			    'cd_indicador_tabela'                         => $this->input->post('cd_indicador_tabela', true),			    
			    'dt_referencia'                               => $this->input->post('dt_referencia', true),
			    'fl_media'                                    => $this->input->post('fl_media', true),
			    'nr_total'                           => $nr_total,
			    'nr_atendidas'                      => $nr_atendidas,
			...
			$this->atuarial_divulgacao_extratos_prazo_model->salvar($args);
			
			$this->criar_indicador();
	
			redirect('indicador_plugin/atuarial_divulgacao_extratos_prazo', 'refresh');
		}
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function excluir($cd_atuarial_divulgacao_extratos_prazo)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GAP.'))
		{
			$this->atuarial_divulgacao_extratos_prazo_model->excluir($cd_atuarial_divulgacao_extratos_prazo, $this->cd_usuario);
	...
	public function criar_indicador()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GAP.'))
		{
			...
			$collection = $this->atuarial_divulgacao_extratos_prazo_model->listar($tabela[0]['cd_indicador_tabela']);
			...
			foreach($collection as $item)
			{
				if(intval($item['ano_referencia']) >= intval($tabela[0]['nr_ano_referencia']) -10)
				{
					...
					$indicador[$linha][2] = app_decimal_para_php($item['nr_atendidas']);
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$indicador[$linha][2] = $nr_atendidas;
	...
	public function fechar_periodo()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GAP.'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$args = array(
					'cd_atuarial_divulgacao_extratos_prazo' => 0, 
			...
			$this->atuarial_divulgacao_extratos_prazo_model->fechar_periodo($tabela[0]['cd_indicador_tabela'], $this->cd_usuario);
