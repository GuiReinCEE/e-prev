<?php
class Juridico_indenizacoes_pagas extends Controller
{
    ...
    function __construct()
    {
        ...
        $this->enum_indicador = intval(enum_indicador::JURIDICO_INDENIZACOES_PAGAS);
        ...
        $this->load->model('indicador_plugin/juridico_indenizacoes_pagas_model');
    }
    ...
    function index()
    {
        if(indicador_db::verificar_permissao(usuario_id(), 'GP'))
        {
            ...
            $this->load->view('indicador_plugin/juridico_indenizacoes_pagas/index', $data);
        }
        else
        {
    ...
    function listar()
    {
        if(indicador_db::verificar_permissao(usuario_id(), 'GP'))
        {
            ...
            $this->juridico_indenizacoes_pagas_model->listar($result, $args);
            $data['collection'] = $result->result_array();

            $this->load->view('indicador_plugin/juridico_indenizacoes_pagas/index_result', $data);
        }
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
    ...
    function cadastro($cd_juridico_indenizacoes_pagas = 0)
    {
        if(indicador_db::verificar_permissao(usuario_id(), 'GP'))
        {
            $args   = array();
            ...
            $args['cd_juridico_indenizacoes_pagas'] = $cd_juridico_indenizacoes_pagas;

            if(intval($args['cd_juridico_indenizacoes_pagas']) == 0)
            {
                $this->juridico_indenizacoes_pagas_model->carrega_referencia($result, $args);
                $arr = $result->row_array();

                $data['row']['cd_juridico_indenizacoes_pagas'] = $args['cd_juridico_indenizacoes_pagas'];
                $data['row']['nr_valor_1']                = "";
                $data['row']['nr_valor_2']                = "";
                $data['row']['nr_valor_3']                = "";
                $data['row']['fl_media']                  = "";
            ...
            {
                $this->juridico_indenizacoes_pagas_model->carrega($result, $args);
                $data['row'] = $result->row_array();
            }

            $this->load->view('indicador_plugin/juridico_indenizacoes_pagas/cadastro', $data);
        }
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
    ...
    function salvar()
    {
        ...
        {		
            ...
            $args['cd_juridico_indenizacoes_pagas'] = intval($this->input->post('cd_juridico_indenizacoes_pagas', true));
            ...
            $this->juridico_indenizacoes_pagas_model->salvar($result, $args);
            $this->criar_indicador();

            redirect("indicador_plugin/juridico_indenizacoes_pagas", "refresh");
        }
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
    ...
    function excluir($cd_juridico_indenizacoes_pagas)
    {
        if(indicador_db::verificar_permissao(usuario_id(), 'GP'))
        {
            $args   = array();
            ...
            $this->juridico_indenizacoes_pagas_model->excluir($result, $args);
            $this->criar_indicador();
    ...
    function criar_indicador()
    {
        if(indicador_db::verificar_permissao(usuario_id(), 'GP'))
        {
            ...
            $nr_resultado_f_total = ($nr_valor_3_total > 0 ? ($nr_valor_1_total / $nr_valor_3_total) : 0);
            ...
            if(intval($contador_ano_atual) > 0)
            {
                ...
                $indicador[$linha][2] = number_format($nr_valor_2_total,0,',','.');
                $indicador[$linha][3] = number_format($nr_valor_3_total,0,',','.');
    ...
    function fechar_periodo()
    {
        if(indicador_db::verificar_permissao(usuario_id(), 'GP'))
        {
            ...
            if(intval($contador_ano_atual) > 0)
            {
                $args['cd_juridico_indenizacoes_pagas'] = 0;
                ...
                $args["nr_valor_2"]                = app_decimal_para_db($nr_valor_2_total);
                $args["nr_valor_3"]                = app_decimal_para_db($nr_valor_3_total);
            ...
            $this->juridico_indenizacoes_pagas_model->fechar_periodo($result, $args);
