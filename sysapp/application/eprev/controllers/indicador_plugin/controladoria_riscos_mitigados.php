<?php
class Controladoria_riscos_mitigados extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::CONTROLADORIA_QUANT_RISCOS_MITIGADOS);
		...
		$this->load->model('indicador_plugin/controladoria_riscos_mitigados_model');
    }
	...
	function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GC'))
		{
	        ...
	        $this->load->view('indicador_plugin/controladoria_riscos_mitigados/index', $data);
		}
		else
		{
	...
	function listar()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GC'))
        {
			...
			$this->controladoria_riscos_mitigados_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/controladoria_riscos_mitigados/index_result', $data);
        }
        else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function cadastro($cd_controladoria_riscos_mitigados = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC'))
		{
			$args   = array();
			...
			$args['cd_controladoria_riscos_mitigados'] = $cd_controladoria_riscos_mitigados;
			
			if(intval($args['cd_controladoria_riscos_mitigados']) == 0)
			{
				$args['nr_ano_referencia'] = intval($data['tabela'][0]['nr_ano_referencia']);
				$this->controladoria_riscos_mitigados_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_controladoria_riscos_mitigados'] = $args['cd_controladoria_riscos_mitigados'];
				$data['row']['nr_valor_1']                                  = "";
				$data['row']['fl_media']                                    = "";
				$data['row']['observacao']                                  = "";
			...
			{
				$this->controladoria_riscos_mitigados_model->carrega($result, $args);
				$data['row'] = $result->row_array();
				$data['row']['qt_ano'] = 1;
			}

			$this->load->view('indicador_plugin/controladoria_riscos_mitigados/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_controladoria_riscos_mitigados'] = intval($this->input->post('cd_controladoria_riscos_mitigados', true));
			...
			$this->controladoria_riscos_mitigados_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/controladoria_riscos_mitigados/cadastro", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_controladoria_riscos_mitigados)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC'))
		{
			$args   = array();
			...
			$this->controladoria_riscos_mitigados_model->excluir($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/controladoria_riscos_mitigados", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function criar_indicador()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$indicador[$linha][2] = app_decimal_para_php($nr_meta_media / $contador_ano_atual);
				$indicador[$linha][3] = '';
				$indicador[$linha][4] = '';
			}
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->controladoria_riscos_mitigados_model->atualiza_fechar_periodo($result, $args);
			}

			$this->controladoria_riscos_mitigados_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
