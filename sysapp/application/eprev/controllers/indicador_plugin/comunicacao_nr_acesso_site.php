<?php
class Comunicacao_nr_acesso_site extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::COMUNICACAO_NUMERO_ACESSO_SITE);
		...
		$this->load->model('indicador_plugin/comunicacao_nr_acesso_site_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'AC'))
		{
	        ...
	        $this->load->view('indicador_plugin/comunicacao_nr_acesso_site/index',$data);
		}
    }
    ...
    function listar()
    {
    	if(indicador_db::verificar_permissao(usuario_id(),'AC'))
        {
			...
			$this->comunicacao_nr_acesso_site_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/comunicacao_nr_acesso_site/index_result', $data);
        }
        else
		{
    ...
    function cadastro($cd_comunicacao_nr_acesso_site = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'AC'))
		{
			$args   = array();
			...
			$args['cd_comunicacao_nr_acesso_site'] = $cd_comunicacao_nr_acesso_site;
			
			if(intval($args['cd_comunicacao_nr_acesso_site']) == 0)
			{
				$this->comunicacao_nr_acesso_site_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_comunicacao_nr_acesso_site'] = $args['cd_comunicacao_nr_acesso_site'];
				$data['row']['nr_valor_1']            = "";
				$data['row']['fl_media']              = "";
				$data['row']['observacao']            = "";
			...
			{
				$this->comunicacao_nr_acesso_site_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/comunicacao_nr_acesso_site/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_comunicacao_nr_acesso_site'] = intval($this->input->post('cd_comunicacao_nr_acesso_site', true));
			...
			$this->comunicacao_nr_acesso_site_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/comunicacao_nr_acesso_site", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_comunicacao_nr_acesso_site)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'AC'))
		{
			$args   = array();
			...
			$this->comunicacao_nr_acesso_site_model->excluir($result, $args);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'AC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$args["nr_meta"]             = app_decimal_para_db($nr_meta_total);
				$args["cd_usuario"]          = $this->session->userdata('codigo');

				$this->comunicacao_nr_acesso_site_model->atualiza_fechar_periodo($result, $args);
			}

			$this->comunicacao_nr_acesso_site_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
