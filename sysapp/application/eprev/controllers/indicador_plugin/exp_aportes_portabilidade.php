<?php
class Exp_aportes_portabilidade extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::EXP_APORTES_PORTABILIDADE);
		...
		$this->load->model('indicador_plugin/exp_aportes_portabilidade_model');
    }
	...
	public function index()
    {
    	if((indicador_db::verificar_permissao($this->cd_usuario, 'GE')) OR ($this->session->userdata('indic_12') == '*'))
		{
	        ...
	        $this->load->view('indicador_plugin/exp_aportes_portabilidade/index', $data);
		}
		else
        {
	...
	public function listar()
    {
		if((indicador_db::verificar_permissao($this->cd_usuario, 'GE')) OR ($this->session->userdata('indic_12') == '*'))
		{
			...
			$data['collection'] = $this->exp_aportes_portabilidade_model->listar($data['tabela'][0]['cd_indicador_tabela']);
			
			$this->load->view('indicador_plugin/exp_aportes_portabilidade/index_result', $data);
        }
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function cadastro($cd_exp_aportes_portabilidade = 0)
	{
		if((indicador_db::verificar_permissao($this->cd_usuario, 'GE')) OR ($this->session->userdata('indic_12') == '*'))
		{
			$data = array();
			...
			if(intval($cd_exp_aportes_portabilidade) == 0)
			{
				$row = $this->exp_aportes_portabilidade_model->carrega_referencia($data['tabela'][0]['cd_indicador_tabela']);
				
				$data['row'] = array(
					'cd_exp_aportes_portabilidade' => intval($cd_exp_aportes_portabilidade),
			...
			{
				$data['row'] = $this->exp_aportes_portabilidade_model->carrega(intval($cd_exp_aportes_portabilidade));
			}

			$this->load->view('indicador_plugin/exp_aportes_portabilidade/cadastro', $data);
		}
		else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function salvar()
	{
		...
		{		
			$cd_exp_aportes_portabilidade = intval($this->input->post('cd_exp_aportes_portabilidade', true));
			...
			if(intval($cd_exp_aportes_portabilidade) == 0)
			{
				$this->exp_aportes_portabilidade_model->salvar($args);
			}
			else
			{
				$this->exp_aportes_portabilidade_model->atualizar($cd_exp_aportes_portabilidade, $args);
			}

			$this->criar_indicador();
			
			redirect('indicador_plugin/exp_aportes_portabilidade', 'refresh');
		}
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function excluir($cd_exp_aportes_portabilidade)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GE'))
		{
			$this->exp_aportes_portabilidade_model->excluir(intval($cd_exp_aportes_portabilidade), $this->cd_usuario);
	...
	public function criar_indicador()
    {
    	if((indicador_db::verificar_permissao($this->cd_usuario, 'GE')) OR ($this->session->userdata('indic_12') == '*'))
		{
			...
			$collection = $this->exp_aportes_portabilidade_model->listar($tabela[0]['cd_indicador_tabela']);
	...
	public function fechar_periodo()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GE'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->exp_aportes_portabilidade_model->fechar_ano($args);
			}

			$this->exp_aportes_portabilidade_model->fechar_indicador($tabela[0]['cd_indicador_tabela'], $this->cd_usuario);
