<?php
class Atuarial_exigencia_previc_novo_plano extends Controller
{
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::ATUARIAL_EXIGENCIA_PREVIX_EM_APROVACAO_DE_NOVO_PLANO);
		...
		$this->load->model('indicador_plugin/atuarial_exigencia_previc_novo_plano_model');
    }
    ...
    public function index()
    {
		...
		{			
	        ...
	        $this->load->view('indicador_plugin/atuarial_exigencia_previc_novo_plano/index', $data);
		}
        else
        {
    ...
    public function listar()
    {
		...
		{	
			...
			$data['collection'] = $this->atuarial_exigencia_previc_novo_plano_model->listar($data['tabela'][0]['cd_indicador_tabela']);

			$this->load->view('indicador_plugin/atuarial_exigencia_previc_novo_plano/index_result', $data);
		}
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
    ...
    public function cadastro($cd_atuarial_exigencia_previc_novo_plano = 0)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GP'))
		{			
			$data['label_0']  = $this->label_0;
			...
			if(intval($cd_atuarial_exigencia_previc_novo_plano) == 0)
			{
				$row = $this->atuarial_exigencia_previc_novo_plano_model->carrega_referencia($data['tabela'][0]['cd_indicador_tabela']);

				$data['row'] = array(
					'cd_atuarial_exigencia_previc_novo_plano' => intval($cd_atuarial_exigencia_previc_novo_plano),
					'ds_evento'                               => '',
					'nr_houve_exigencia'                      => '',
			...
			{
				$data['row'] = $this->atuarial_exigencia_previc_novo_plano_model->carrega(
					intval($cd_atuarial_exigencia_previc_novo_plano)
				);
			}

			$this->load->view('indicador_plugin/atuarial_exigencia_previc_novo_plano/cadastro', $data);
		}
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function salvar()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GP'))
		{		
			$cd_atuarial_exigencia_previc_novo_plano = intval($this->input->post('cd_atuarial_exigencia_previc_novo_plano', true));
			...
			if(intval($cd_atuarial_exigencia_previc_novo_plano) == 0)
			{
				$this->atuarial_exigencia_previc_novo_plano_model->salvar($args);
			}
			else
			{
				$this->atuarial_exigencia_previc_novo_plano_model->atualizar($cd_atuarial_exigencia_previc_novo_plano, $args);
			}

			$this->criar_indicador();
			
			redirect('indicador_plugin/atuarial_exigencia_previc_novo_plano', 'refresh');
		}
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function excluir($cd_atuarial_exigencia_previc_novo_plano)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GP'))
		{
			$this->atuarial_exigencia_previc_novo_plano_model->excluir(
				intval($cd_atuarial_exigencia_previc_novo_plano), 
				$this->cd_usuario
			);
    ...
    public function criar_indicador()
    {
    	if(indicador_db::verificar_permissao($this->cd_usuario, 'GP'))
		{
			...
			$collection = $this->atuarial_exigencia_previc_novo_plano_model->listar($tabela[0]['cd_indicador_tabela']);
