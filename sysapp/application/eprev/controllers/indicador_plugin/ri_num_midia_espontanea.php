<?php
class ri_num_midia_espontanea extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Midia Espontânea - Gratuita";
	var	$label_2 = "Midia de Grande Circulação s/custo";
	var	$label_3 = "Midia de Grande Circulação c/custo";
    var	$label_4 = "Total de Midias";
    var $label_5 = 'Meta';
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RI_NUMERO_DE_MIDIA_ESPONTANEA);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
	        ...
	        $this->load->view('indicador_plugin/ri_num_midia_espontanea/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
        {
	        $this->load->model( 'indicador_plugin/ri_num_midia_espontanea_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->ri_num_midia_espontanea_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/ri_num_midia_espontanea/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$this->load->model('indicador_plugin/ri_num_midia_espontanea_model');
			$row=$this->ri_num_midia_espontanea_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.ri_num_midia_espontanea
			...
			$this->load->view('indicador_plugin/ri_num_midia_espontanea/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$args['cd_ri_num_midia_espontanea']=intval($this->input->post('cd_ri_num_midia_espontanea', true));
            ...
            $args["nr_percentual_f"] = floatval($args["nr_valor_1"]) + floatval($args["nr_valor_2"]) + floatval($args["nr_valor_3"]);
			$args["nr_meta"] = app_decimal_para_db($this->input->post("nr_meta", true));
            $args["observacao"] = $this->input->post("observacao", true);

			$msg=array();
			$retorno = $this->ri_num_midia_espontanea_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/ri_num_midia_espontanea", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$this->ri_num_midia_espontanea_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.ri_num_midia_espontanea
		...
		redirect( 'indicador_plugin/ri_num_midia_espontanea' );
