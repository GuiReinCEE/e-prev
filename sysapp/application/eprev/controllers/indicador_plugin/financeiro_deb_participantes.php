<?php
class financeiro_deb_participantes extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Carga Total no Mês";
	var	$label_2 = "Não Pagos no Mês";
	var	$label_3 = "% Débitos no Mês";
    var $label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::FINANCEIRO_NAO_PAGOS_NO_MES_PARTICIPANTE);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/financeiro_deb_participantes/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
        {
	        $this->load->model( 'indicador_plugin/financeiro_deb_participantes_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->financeiro_deb_participantes_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/financeiro_deb_participantes/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$this->load->model('indicador_plugin/financeiro_deb_participantes_model');
			$row=$this->financeiro_deb_participantes_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.financeiro_deb_participantes 
			...
			$this->load->view('indicador_plugin/financeiro_deb_participantes/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$args['cd_financeiro_deb_participantes']=intval($this->input->post('cd_financeiro_deb_participantes', true));
			...
			$msg=array();
			$retorno = $this->financeiro_deb_participantes_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/financeiro_deb_participantes", "refresh" );	
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			$this->financeiro_deb_participantes_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GFC' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.financeiro_deb_participantes
		...
		redirect( 'indicador_plugin/financeiro_deb_participantes' );
