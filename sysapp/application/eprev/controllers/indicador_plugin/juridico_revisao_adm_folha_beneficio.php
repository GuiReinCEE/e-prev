<?php
class Juridico_revisao_adm_folha_beneficio extends Controller
{
    ...
    function __construct()
    {
        ...
        $this->enum_indicador = intval(enum_indicador::JURIDICO_INCREMENTO_REVISAO_ADM_EM_RELACAO_FOLHA_DE_BENEFICIO);
        ...
        $this->load->model('indicador_plugin/juridico_revisao_adm_folha_beneficio_model');
    }
    ...
    function index()
    {
        if(indicador_db::verificar_permissao(usuario_id(), 'GP'))
        {
            ...
            $this->load->view('indicador_plugin/juridico_revisao_adm_folha_beneficio/index', $data);
        }
        else
        {
    ...
    function listar()
    {
        if(indicador_db::verificar_permissao(usuario_id(), 'GP'))
        {
            ...
            $this->juridico_revisao_adm_folha_beneficio_model->listar($result, $args);
            $data['collection'] = $result->result_array();

            $this->load->view('indicador_plugin/juridico_revisao_adm_folha_beneficio/index_result', $data);
        }
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
    ...
    function cadastro($cd_juridico_revisao_adm_folha_beneficio = 0)
    {
        if(indicador_db::verificar_permissao(usuario_id(), 'GP'))
        {
            $args   = array();
            ...
            $args['cd_juridico_revisao_adm_folha_beneficio'] = $cd_juridico_revisao_adm_folha_beneficio;

            if(intval($args['cd_juridico_revisao_adm_folha_beneficio']) == 0)
            {
                $this->juridico_revisao_adm_folha_beneficio_model->carrega_referencia($result, $args);
                $arr = $result->row_array();

                $data['row']['cd_juridico_revisao_adm_folha_beneficio'] = $args['cd_juridico_revisao_adm_folha_beneficio'];
            ...
            {
                $this->juridico_revisao_adm_folha_beneficio_model->carrega($result, $args);
                $data['row'] = $result->row_array();
            }

            $this->load->view('indicador_plugin/juridico_revisao_adm_folha_beneficio/cadastro', $data);
        }
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
    ...
    function salvar()
    {
        ...
        {		
            ...
            $args['cd_juridico_revisao_adm_folha_beneficio'] = intval($this->input->post('cd_juridico_revisao_adm_folha_beneficio', true));
            ...
            $this->juridico_revisao_adm_folha_beneficio_model->salvar($result, $args);
            $this->criar_indicador();

            redirect("indicador_plugin/juridico_revisao_adm_folha_beneficio", "refresh");
        }
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
    ...
    function excluir($cd_juridico_revisao_adm_folha_beneficio)
    {
        if(indicador_db::verificar_permissao(usuario_id(), 'GP'))
        {
            $args   = array();
            ...
            $this->juridico_revisao_adm_folha_beneficio_model->excluir($result, $args);
            $this->criar_indicador();
    ...
    function criar_indicador()
    {
        if(indicador_db::verificar_permissao(usuario_id(), 'GP'))
        {
            ...
            $nr_resultado_f_total = ($nr_valor_2_total > 0 ? (($nr_valor_1_total / $nr_valor_2_total) * 100) : 0);
            ...
            if(intval($contador_ano_atual) > 0)
            {
                ...
                $indicador[$linha][4] = app_decimal_para_php($nr_resultado_f_total);
                $indicador[$linha][5] = app_decimal_para_php($nr_meta);
                $indicador[$linha][6] = '';
                $indicador[$linha][7] = '';
            }
    ...
    function fechar_periodo()
    {
        if(indicador_db::verificar_permissao(usuario_id(), 'GP'))
        {
            ...
            if(intval($contador_ano_atual) > 0)
            {
                $args['cd_juridico_revisao_adm_folha_beneficio'] = 0;
                ...
                $args["nr_valor_3"]                = $nr_valor_3_total;
            ...
            $this->juridico_revisao_adm_folha_beneficio_model->fechar_periodo($result, $args);
