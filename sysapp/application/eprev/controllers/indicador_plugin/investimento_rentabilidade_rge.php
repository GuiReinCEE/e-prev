<?php
class Investimento_rentabilidade_rge extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::INVESTIMENTO_RENTABILIDADE_RGE);
		...
		$this->load->model('indicador_plugin/investimento_rentabilidade_rge_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
	        ...
	        $this->load->view('indicador_plugin/investimento_rentabilidade_rge/index',$data);
		}
    }
    ...
    function listar()
    {
    	if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
        {
			...
			$this->investimento_rentabilidade_rge_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/investimento_rentabilidade_rge/index_result', $data);
        }
        else
		{
    ...
    function cadastro($cd_investimento_rentabilidade_rge = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
			$args   = array();
			...
			$args['cd_investimento_rentabilidade_rge'] = $cd_investimento_rentabilidade_rge;
			
			if(intval($args['cd_investimento_rentabilidade_rge']) == 0)
			{
				$this->investimento_rentabilidade_rge_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_investimento_rentabilidade_rge'] = $args['cd_investimento_rentabilidade_rge'];
			...
			{
				$this->investimento_rentabilidade_rge_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/investimento_rentabilidade_rge/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_investimento_rentabilidade_rge'] = intval($this->input->post('cd_investimento_rentabilidade_rge', true));
			...
			$this->investimento_rentabilidade_rge_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/investimento_rentabilidade_rge", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_investimento_rentabilidade_rge)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
			$args   = array();
			...
			$this->investimento_rentabilidade_rge_model->excluir($result, $args);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->investimento_rentabilidade_rge_model->atualiza_fechar_periodo($result, $args);
			}

			$this->investimento_rentabilidade_rge_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
