<?php
class administrativo_contrato extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Realizado";
    var $label_2 = "Tendência";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RH_VALOR_CONTRATO_PRESTADOR_SERVICO);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GGS' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/administrativo_contrato/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if( CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GGS' ) )
        {
	        $this->load->model( 'indicador_plugin/administrativo_contrato_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->administrativo_contrato_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/administrativo_contrato/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GGS' ) )
		{
			...
			$this->load->model('indicador_plugin/administrativo_contrato_model');
			$row=$this->administrativo_contrato_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
			...
			$this->load->view('indicador_plugin/administrativo_contrato/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GGS' ) )
		{
			...
			$args['cd_administrativo_contrato']=intval($this->input->post('cd_administrativo_contrato', true));
			...
			$msg=array();
			$retorno = $this->administrativo_contrato_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/administrativo_contrato", "refresh" );
				}
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GGS' ) )
		{
			...
			$this->administrativo_contrato_model->excluir($id);
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GGS' ) )
		{
			...
			{#tabela_existe
				...
				for( $i=0; $i<sizeof($indicador); $i++ )
				{
                    ...
                    $sql .= indicador_db::sql_inserir_celula( $tabela[0]['cd_indicador_tabela'], 3, $linha, app_decimal_para_php($indicador[$i][3]), 'left' );
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GGS' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.administrativo_contrato
		...
		redirect( 'indicador_plugin/administrativo_contrato' );
