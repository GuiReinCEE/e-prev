<?php
class beneficio_pg_30dias extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Benefícios concedidos no mês";
	var	$label_2 = "Número de Benefícios conc. com mais de 30 dias";
	var	$label_3 = "% de Benefícios conc. em mais de 30 diase";
    var $label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::BENEFICIO_BENEFICIOS_PAGOS_A_PARTIR_DA_SOLICITACAO_EM_MAIS_DE_30_DIAS);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/beneficio_pg_30dias/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
        {
	        $this->load->model( 'indicador_plugin/beneficio_pg_30dias_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->beneficio_pg_30dias_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/beneficio_pg_30dias/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->load->model('indicador_plugin/beneficio_pg_30dias_model');
			$row=$this->beneficio_pg_30dias_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					cd_indicador_tabela,
				   (SELECT COUNT(*)
				      FROM indicador_plugin.beneficio_pg_30dias
			...
			$this->load->view('indicador_plugin/beneficio_pg_30dias/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$args['cd_beneficio_pg_30dias']=intval($this->input->post('cd_beneficio_pg_30dias', true));
			...
			$msg=array();
			$retorno = $this->beneficio_pg_30dias_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/beneficio_pg_30dias", "refresh" );	
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->beneficio_pg_30dias_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.beneficio_pg_30dias
		...
		redirect( 'indicador_plugin/beneficio_pg_30dias' );
