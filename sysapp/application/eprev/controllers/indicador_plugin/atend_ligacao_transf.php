<?php
class atend_ligacao_transf extends Controller
{
	var	$label_0 = "Mês";
	var	$label_1 = "Total de ligações transferidas";
	var	$label_2 = "Total de ligações recebidas";
	var	$label_3 = "Percentual de ligações";
    var $label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::ATENDIMENTO_LIGACOES_RECEBIDAS_QUE_FORAM_TRANSFERIDAS_PARA_OS_ATENDENTES_CALL_CENTER);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/atend_ligacao_transf/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
        {
	        ...
	        $this->load->model( 'indicador_plugin/Atend_ligacao_transf_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->Atend_ligacao_transf_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/atend_ligacao_transf/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->load->model('indicador_plugin/Atend_ligacao_transf_model');
			$row=$this->Atend_ligacao_transf_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.atend_ligacao_transf 
			...
			$this->load->view('indicador_plugin/atend_ligacao_transf/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$args['cd_atend_ligacao_transf']=intval($this->input->post('cd_atend_ligacao_transf', true));
			...
			$msg=array();
			$retorno = $this->Atend_ligacao_transf_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/atend_ligacao_transf", "refresh" );
				}
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->Atend_ligacao_transf_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.atend_ligacao_transf
		...
		redirect( 'indicador_plugin/atend_ligacao_transf' );
