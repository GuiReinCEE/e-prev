<?php
class ri_pub_presente_eventos_externos extends Controller
{
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::RI_PUBLICO_PRESENTE_EM_EVENTOS_EXTERNOS_2013);
		...
		$this->load->model('indicador_plugin/ri_pub_presente_eventos_externos_model');
    }
	...
	function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(), 'AC') || indicador_db::verificar_permissao(usuario_id(), 'GE'))
		{
	        ...
	        $this->load->view('indicador_plugin/ri_pub_presente_eventos_externos/index', $data);
		}
		else
        {
	...
	function listar()
    {
		if(indicador_db::verificar_permissao(usuario_id(), 'AC') || indicador_db::verificar_permissao(usuario_id(), 'GE'))
        {
			...
			$this->ri_pub_presente_eventos_externos_model->listar($result, $args);
			$data['collection'] = $result->result_array();

			$this->load->view('indicador_plugin/ri_pub_presente_eventos_externos/index_result', $data);
        }
		else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function cadastro($cd_ri_pub_presente_eventos_externos = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'AC'))
		{
			...
			$args['cd_ri_pub_presente_eventos_externos'] = $cd_ri_pub_presente_eventos_externos;

			if(intval($args['cd_ri_pub_presente_eventos_externos']) == 0)
			{
				$this->ri_pub_presente_eventos_externos_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_ri_pub_presente_eventos_externos'] = $args['cd_ri_pub_presente_eventos_externos'];
				$data['row']['nr_valor_1']                          = "";
				$data['row']['nr_valor_2']                          = "";
				$data['row']['nr_valor_3']                          = "";
				$data['row']['fl_media']                            = "";
			...
			{
				$this->ri_pub_presente_eventos_externos_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/ri_pub_presente_eventos_externos/cadastro', $data);
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_ri_pub_presente_eventos_externos'] = intval($this->input->post('cd_ri_pub_presente_eventos_externos', true));
			...
			$this->ri_pub_presente_eventos_externos_model->salvar($result, $args);
			$this->criar_indicador();
			
			redirect("indicador_plugin/ri_pub_presente_eventos_externos", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_ri_pub_presente_eventos_externos)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'AC'))
		{
			$args   = array();
			...
			$this->ri_pub_presente_eventos_externos_model->excluir($result, $args);
			$this->criar_indicador();
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'AC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				$args['cd_ri_pub_presente_eventos_externos'] = 0;
			...
			$this->ri_pub_presente_eventos_externos_model->fechar_periodo($result, $args);
