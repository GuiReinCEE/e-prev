<?php
class Secretaria_atas_de_assinadas_fora_prazo extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::SECRETARIA_ATAS_DE_ASSINADAS_FORA_PRAZO);
		...
		$this->load->model('indicador_plugin/secretaria_atas_de_assinadas_fora_prazo_model');
    }
    ...
    public function index()
    {
		if(indicador_db::verificar_permissao($this->cd_usuario, 'SG'))
		{
	        ...
	        $this->load->view('indicador_plugin/secretaria_atas_de_assinadas_fora_prazo/index', $data);
		}
		else
		{
    ...
    public function listar()
    {
        ...
        {			
			...
			$data['collection'] = $this->secretaria_atas_de_assinadas_fora_prazo_model->listar($data['tabela'][0]['cd_indicador_tabela'] );
			
			$this->load->view('indicador_plugin/secretaria_atas_de_assinadas_fora_prazo/index_result', $data);
        }
        else
		{
			exibir_mensagem('ACESSO NÃO PERMITIDO');
    ...
    public function cadastro($cd_secretaria_atas_de_assinadas_fora_prazo = 0)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'SG'))
		{			
			$data['label_0'] = $this->label_0;
			...
			if(intval($cd_secretaria_atas_de_assinadas_fora_prazo) == 0)
			{
				$row = $this->secretaria_atas_de_assinadas_fora_prazo_model->carrega_referencia();
				
				$data['row'] = array(
					'cd_secretaria_atas_de_assinadas_fora_prazo' => intval($cd_secretaria_atas_de_assinadas_fora_prazo),
				    'nr_valor_1'            					 => '',
				    'nr_valor_2'            					 => '',
				    'fl_media'             					     => '',
				    'observacao'            					 => '',
			...
			{
				$data['row'] = $this->secretaria_atas_de_assinadas_fora_prazo_model->carrega($cd_secretaria_atas_de_assinadas_fora_prazo);
			}

			$this->load->view('indicador_plugin/secretaria_atas_de_assinadas_fora_prazo/cadastro', $data);
		}
		else
		{
			exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function salvar()
	{
		...
		{		
			...
			$args = array(
				'cd_secretaria_atas_de_assinadas_fora_prazo' => intval($this->input->post('cd_secretaria_atas_de_assinadas_fora_prazo', true)),
			...
			$this->secretaria_atas_de_assinadas_fora_prazo_model->salvar($args);
			
			$this->criar_indicador();
	
			redirect('indicador_plugin/secretaria_atas_de_assinadas_fora_prazo', 'refresh');
		}
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function excluir($cd_secretaria_atas_de_assinadas_fora_prazo)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'SG'))
		{
			$this->secretaria_atas_de_assinadas_fora_prazo_model->excluir($cd_secretaria_atas_de_assinadas_fora_prazo, $this->cd_usuario);
	...
	public function criar_indicador()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'SG'))
		{
			...
			$collection = $this->secretaria_atas_de_assinadas_fora_prazo_model->listar($tabela[0]['cd_indicador_tabela']);
	...
	public function fechar_periodo()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'SG'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$args = array(
					'cd_secretaria_atas_de_assinadas_fora_prazo' => 0, 
			...
			$this->secretaria_atas_de_assinadas_fora_prazo_model->fechar_periodo($tabela[0]['cd_indicador_tabela'], $this->cd_usuario);
