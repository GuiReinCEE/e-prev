<?php
class beneficio_inicial_incorreto extends Controller
{
	var	$label_0 = "Mês";
	var	$label_1 = "Total de Benefícios concedidos no mês";
	var	$label_2 = "Benef. C/cálculo inicial incorreto";
	var	$label_3 = "% de Benefícios com o cálculo inicial incorreto";
    var $label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::BENEFICIO_BENEFICIOS_COM_O_CALCULO_INICIAL_INCORRETO);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/beneficio_inicial_incorreto/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
        {
	        $this->load->model( 'indicador_plugin/beneficio_inicial_incorreto_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->beneficio_inicial_incorreto_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/beneficio_inicial_incorreto/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->load->model('indicador_plugin/beneficio_inicial_incorreto_model');
			$row=$this->beneficio_inicial_incorreto_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.beneficio_inicial_incorreto 
			...
			$this->load->view('indicador_plugin/beneficio_inicial_incorreto/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$args['cd_beneficio_inicial_incorreto']=intval($this->input->post('cd_beneficio_inicial_incorreto', true));
			...
			$msg=array();
			$retorno = $this->beneficio_inicial_incorreto_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/beneficio_inicial_incorreto", "refresh" );
				}
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->beneficio_inicial_incorreto_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.beneficio_inicial_incorreto
		...
		redirect( 'indicador_plugin/beneficio_inicial_incorreto' );
