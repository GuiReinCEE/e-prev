<?php
class administrativo_treina_orca extends Controller
{
	var	$label_0 = "Período";
	var	$label_1 = "Orçamento (Realizado)";
	var	$label_2 = "Vlr Realizado em treinamento";
	var	$label_3 = "% do orçamento investido em treinamento";
	var $label_4 = "Referêncial";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RH_ORCAMENTO_REALIZADO_INVESTIDO_TREINAMENTO);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/administrativo_treina_orca/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC' ) )
        {
	        $this->load->model( 'indicador_plugin/administrativo_treina_orca_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->administrativo_treina_orca_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/administrativo_treina_orca/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC' ) )
		{
			...
			$this->load->model('indicador_plugin/administrativo_treina_orca_model');
			$row=$this->administrativo_treina_orca_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					$sql = "
							SELECT TO_CHAR(dt_referencia + '1 month'::INTERVAL,'DD/MM/YYYY') AS mes_referencia, 
								   ...
								   nr_meta, 
								   cd_indicador_tabela
					          FROM indicador_plugin.administrativo_treina_orca 
			...
			$this->load->view('indicador_plugin/administrativo_treina_orca/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC' ) )
		{
			...
			$args['cd_administrativo_treina_orca']=intval($this->input->post('cd_administrativo_treina_orca', true));
			...
			$msg=array();
			$retorno = $this->administrativo_treina_orca_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/administrativo_treina_orca", "refresh" );	
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC' ) )
		{
			...
			$this->administrativo_treina_orca_model->excluir($id);
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC' ) )
		{
			...
			{#tabela_existe
				...
				$sql.=indicador_db::sql_inserir_grafico(
					...
					"3,3,1,$linha_sem_media;4,4,1,$linha_sem_media-linha;5,5,1,$linha_sem_media-linha",
					usuario_id(),
					$coluna_para_ocultar,
                    -1,
                    2,
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.administrativo_treina_orca
		...
		redirect( 'indicador_plugin/administrativo_treina_orca' );
