...
class administrativo_ept extends Controller
{
	...
	var	$label_3 = "% execução do Plano de Treinamento";
	var	$label_4 = "Meta";
				...
				foreach( $collection as $item )
				{
					...
					if( intval($item['ano_referencia'])>=intval($tabela[0]['nr_ano_referencia'])-5 )
					{
						...
						if( $item['fl_media']=='S' )
						{
							...
							$nr_valor_2= '';
                            $observacao = '';
							$nr_percentual_f = $item['nr_percentual_f']; // valor da média dos anos anteriores é gravada nessa coluna quando o período é fechado
						}
						else
						{
