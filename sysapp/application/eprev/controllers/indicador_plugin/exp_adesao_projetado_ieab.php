<?php
class exp_adesao_projetado_ieab extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::EXP_NUMERO_DE_ADESAO_X_NUMERO_PROJETADO_IEABPREV);
		...
		$this->load->model('indicador_plugin/exp_adesao_projetado_ieab_model');
    }
    ...
    public function index()
    {
		if((indicador_db::verificar_permissao($this->cd_usuario, 'GN')) OR ($this->session->userdata('indic_12') == '*'))
		{
			...
			$this->load->view('indicador_plugin/exp_adesao_projetado_ieab/index',$data);
		}
		else
        {
    ...
    public function listar()
    {
    	if((indicador_db::verificar_permissao($this->cd_usuario, 'GN')) OR ($this->session->userdata('indic_12') == '*'))
        {
			...
			$data['collection'] = $this->exp_adesao_projetado_ieab_model->listar($data['tabela'][0]['cd_indicador_tabela']);
	        
			$this->load->view('indicador_plugin/exp_adesao_projetado_ieab/index_result', $data);
        }
        else
		{
			exibir_mensagem('ACESSO NÃO PERMITIDO');
    ...
    public function cadastro($cd_exp_adesao_projetado_ieab = 0)
	{
		if((indicador_db::verificar_permissao($this->cd_usuario, 'GN')) OR ($this->session->userdata('indic_12') == '*'))
		{
			$data['label_0'] = $this->label_0;
			...
			if(intval($cd_exp_adesao_projetado_ieab) == 0)
			{
				$row = $this->exp_adesao_projetado_ieab_model->carrega_referencia();
								
				$data['row'] =array(
					'cd_exp_adesao_projetado_ieab' => $cd_exp_adesao_projetado_ieab,
			...
			{
				$data['row'] = $this->exp_adesao_projetado_ieab_model->carrega($cd_exp_adesao_projetado_ieab);
			}

			$this->load->view('indicador_plugin/exp_adesao_projetado_ieab/cadastro', $data);
		}
		else
		{
			exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function salvar()
	{
		...
		{		
			...
			$args = array(
				'cd_exp_adesao_projetado_ieab' => $this->input->post('cd_exp_adesao_projetado_ieab', true),
			...
			$this->exp_adesao_projetado_ieab_model->salvar($args);
			
			$this->criar_indicador();
			
			redirect('indicador_plugin/exp_adesao_projetado_ieab', 'refresh');
		}
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function excluir($cd_exp_adesao_projetado_ieab)
	{
		if((indicador_db::verificar_permissao($this->cd_usuario,'GN')) OR ($this->session->userdata('indic_12') == '*'))
		{
			$this->exp_adesao_projetado_ieab_model->excluir($cd_exp_adesao_projetado_ieab, $this->cd_usuario);
	...
	public function criar_indicador()
	{
		if((indicador_db::verificar_permissao($this->cd_usuario, 'GN')) OR ($this->session->userdata('indic_12') == '*'))
		{
			...
			$collection = $this->exp_adesao_projetado_ieab_model->listar($tabela[0]['cd_indicador_tabela']);
	...
	public function fechar_periodo()
	{
		if((indicador_db::verificar_permissao($this->cd_usuario, 'GN')) OR ($this->session->userdata('indic_12') == '*'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$args = array(
					'cd_exp_adesao_projetado_ieab' => 0,
			...
			$this->exp_adesao_projetado_ieab_model->fechar_periodo($tabela[0]['cd_indicador_tabela'], $this->cd_usuario);
