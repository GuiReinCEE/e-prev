<?php
class atend_sat_cli_atendimento extends Controller
{
	var	$label_0 = "Ano";
	var	$label_1 = "Pessoal";
    var	$label_2 = "Meta Pessoal";
	var	$label_3 = "Tele-Atendimento";
    var	$label_4 = "Meta Tele-Atendimento";
	var	$label_5 = "Auto-Atendimento";
    var	$label_6 = "Meta Auto-Atendimento";
    var	$label_7 = "Tendência Pessoal";
    var	$label_8 = "Tendência Auto-Atendimento";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::ATENDIMENTO_DE_SATISFACAO_DO_CLIENTE_COM_O_ATENDIMENTO);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/atend_sat_cli_atendimento/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
        {
	        $this->load->model( 'indicador_plugin/atend_sat_cli_atendimento_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->atend_sat_cli_atendimento_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/atend_sat_cli_atendimento/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->load->model('indicador_plugin/atend_sat_cli_atendimento_model');
			$row=$this->atend_sat_cli_atendimento_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					$sql = "
					SELECT to_char(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS mes_referencia, dt_referencia, nr_meta_1, nr_meta_2, nr_meta_3, cd_indicador_tabela
					FROM indicador_plugin.atend_sat_cli_atendimento
					...
					if($row_atual)
					{
						...
						$row['nr_meta_1'] = $row_atual['nr_meta_1'];
                        $row['nr_meta_2'] = $row_atual['nr_meta_2'];
                        $row['nr_meta_3'] = $row_atual['nr_meta_3'];
			...
			$this->load->view('indicador_plugin/atend_sat_cli_atendimento/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$args['cd_atend_sat_cli_atendimento']=intval($this->input->post('cd_atend_sat_cli_atendimento', true));
			...
			$args["nr_meta_1"] = app_decimal_para_db($this->input->post("nr_meta_1", true));
            $args["nr_meta_2"] = app_decimal_para_db($this->input->post("nr_meta_2", true));
            $args["nr_meta_3"] = app_decimal_para_db($this->input->post("nr_meta_3", true));
            $args["observacao"] = $this->input->post("observacao", true);

			$msg=array();
			$retorno = $this->atend_sat_cli_atendimento_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/atend_sat_cli_atendimento", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->atend_sat_cli_atendimento_model->excluir($id);
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			{#tabela_existe
				...
				foreach( $collection as $item )
				{
					...
					if(true)
					{
						$nr_meta_1 = $item["nr_meta_1"];
                        $nr_meta_2 = $item["nr_meta_2"];
                        $nr_meta_3 = $item["nr_meta_3"];
						...
						$indicador[$linha][4] = app_decimal_para_php($nr_meta_1);
                        $indicador[$linha][5] = app_decimal_para_php($nr_meta_2);
                        $indicador[$linha][6] = app_decimal_para_php($nr_meta_3);
                        $indicador[$linha][9] = $observacao;
                        $ar_tendencia_1[] = $nr_valor_1;
                        $ar_tendencia_2[] = $nr_valor_2;

						$linha++;
					}
				}

                list($a,$b,$tend) = calcular_tendencia_logaritmica( $ar_tendencia_1 );
				for($i=0;$i<sizeof($ar_tendencia_1);$i++)
				{
					$indicador[$i][7] = $tend[$i];
				}

                list($a,$b,$tend) = calcular_tendencia_logaritmica( $ar_tendencia_2 );
				for($i=0;$i<sizeof($ar_tendencia_2);$i++)
				{
				...
				// gerar gráfico
				$coluna_para_ocultar='4,5,6,7,8';
				$sql.=indicador_db::sql_inserir_grafico(
					$tabela[0]['cd_indicador_tabela'],
					enum_indicador_grafico_tipo::BARRA_MULTIPLO,
					'1,1,0,0;2,2,0,0;3,3,0,0;7,7,0,0;8,8,0,0',
					"0,0,1,$linha_sem_media",
					"1,1,1,$linha_sem_media;2,2,1,$linha_sem_media;3,3,1,$linha_sem_media;7,7,1,$linha_sem_media-linha;8,8,1,$linha_sem_media-linha",
					usuario_id(),
					$coluna_para_ocultar
				);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.atend_sat_cli_atendimento
				...
				*/
				...
				// indicar que o período foi fechado para o indicador_tabela
				$sql=" 
						INSERT INTO indicador_plugin.atend_sat_cli_atendimento
							 ( 
							   dt_referencia,
							   dt_inclusao,
							   cd_usuario_inclusao, 
							 ...
							 (
								SELECT aep.dt_referencia,
									   ...
									   aep.nr_meta,
									   'S'
								  FROM indicador_plugin.atend_sat_cli_atendimento aep
								 WHERE aep.dt_exclusao IS NULL
								   ...
								   AND aep.dt_referencia = (SELECT MAX(eap1.dt_referencia) 
															  FROM indicador_plugin.atend_sat_cli_atendimento eap1
		...
		redirect( 'indicador_plugin/atend_sat_cli_atendimento' );
