<?php
class Exp_numero_vendas_inscricoes_ingressos extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::EXP_NUMERO_DE_VENDAS_X_INSCRICOES_X_INGRESSOS);
		...
		$this->load->model('indicador_plugin/exp_numero_vendas_inscricoes_ingressos_model');
    }
    ...
    function index()
    {
		if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{
	        ...
	        $this->load->view('indicador_plugin/exp_numero_vendas_inscricoes_ingressos/index',$data);
		}
    }
    ...
    function listar()
    {
    	if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
        {
			...
			$this->exp_numero_vendas_inscricoes_ingressos_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/exp_numero_vendas_inscricoes_ingressos/index_result', $data);
        }
        else
		{
    ...
    function cadastro($cd_exp_numero_vendas_inscricoes_ingressos = 0)
	{
		if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{
			$args   = array();
			...
			$args['cd_exp_numero_vendas_inscricoes_ingressos'] = $cd_exp_numero_vendas_inscricoes_ingressos;
			
			if(intval($args['cd_exp_numero_vendas_inscricoes_ingressos']) == 0)
			{
				$this->exp_numero_vendas_inscricoes_ingressos_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_exp_numero_vendas_inscricoes_ingressos'] = $args['cd_exp_numero_vendas_inscricoes_ingressos'];
			...
			{
				$this->exp_numero_vendas_inscricoes_ingressos_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/exp_numero_vendas_inscricoes_ingressos/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_exp_numero_vendas_inscricoes_ingressos'] = intval($this->input->post('cd_exp_numero_vendas_inscricoes_ingressos', true));
			...
			$this->exp_numero_vendas_inscricoes_ingressos_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/exp_numero_vendas_inscricoes_ingressos", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_exp_numero_vendas_inscricoes_ingressos)
	{
		if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{
			$args   = array();
			...
			$this->exp_numero_vendas_inscricoes_ingressos_model->excluir($result, $args);
	...
	function criar_indicador()
	{
		if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$indicador[$linha][4] = app_decimal_para_php(($nr_valor_3_total / $nr_valor_1_total) * 100);
				$indicador[$linha][5] = app_decimal_para_php($nr_meta);
				$indicador[$linha][6] = '';
				$indicador[$linha][7] = '';
			}
	...
	function fechar_periodo()
	{
		if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$args['nr_valor_3']          = app_decimal_para_db($nr_valor_3);
				$args["nr_meta"]             = app_decimal_para_db($nr_meta);
				$args["cd_usuario"]          = $this->session->userdata('codigo');

				$this->exp_numero_vendas_inscricoes_ingressos_model->atualiza_fechar_periodo($result, $args);
			}

			$this->exp_numero_vendas_inscricoes_ingressos_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
