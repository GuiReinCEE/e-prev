<?php
class Financeiro_inadimplencia_patrocinadoras extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::FINANCEIRO_INADIMPLENCIA_PATROCINADORAS);
		...
		$this->load->model('indicador_plugin/financeiro_inadimplencia_patrocinadoras_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
	        ...
	        $this->load->view('indicador_plugin/financeiro_inadimplencia_patrocinadoras/index',$data);
		}
    }
    ...
    function listar()
    {
    	if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
        {
			...
			$this->financeiro_inadimplencia_patrocinadoras_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/financeiro_inadimplencia_patrocinadoras/index_result', $data);
        }
        else
		{
    ...
    function cadastro($cd_financeiro_inadimplencia_patrocinadoras = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$args['cd_financeiro_inadimplencia_patrocinadoras'] = $cd_financeiro_inadimplencia_patrocinadoras;
			
			if(intval($args['cd_financeiro_inadimplencia_patrocinadoras']) == 0)
			{
				$this->financeiro_inadimplencia_patrocinadoras_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_financeiro_inadimplencia_patrocinadoras'] = $args['cd_financeiro_inadimplencia_patrocinadoras'];
			...
			{
				$this->financeiro_inadimplencia_patrocinadoras_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/financeiro_inadimplencia_patrocinadoras/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_financeiro_inadimplencia_patrocinadoras'] = intval($this->input->post('cd_financeiro_inadimplencia_patrocinadoras', true));
			...
			$this->financeiro_inadimplencia_patrocinadoras_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/financeiro_inadimplencia_patrocinadoras", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_financeiro_inadimplencia_patrocinadoras)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$this->financeiro_inadimplencia_patrocinadoras_model->excluir($result, $args);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->financeiro_inadimplencia_patrocinadoras_model->atualiza_fechar_periodo($result, $args);
			}

			$this->financeiro_inadimplencia_patrocinadoras_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
