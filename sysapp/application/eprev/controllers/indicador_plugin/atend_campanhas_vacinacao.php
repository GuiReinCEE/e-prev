<?php
class atend_campanhas_vacinacao extends Controller
{
	var	$label_0 = "Ano";
	var	$label_1 = "Nº de colaboradores vacinados";
	var	$label_2 = "Nº de colaboradores / estagiários";
    var	$label_3 = "% de Vacinação";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::ATENDIMENTO_CAMPANHAS_DE_VACINACAO);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/atend_campanhas_vacinacao/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
        {
	        $this->load->model( 'indicador_plugin/atend_campanhas_vacinacao_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->atend_campanhas_vacinacao_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/atend_campanhas_vacinacao/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->load->model('indicador_plugin/atend_campanhas_vacinacao_model');
			$row=$this->atend_campanhas_vacinacao_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
			...
			$this->load->view('indicador_plugin/atend_campanhas_vacinacao/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$args['cd_atend_campanhas_vacinacao']=intval($this->input->post('cd_atend_campanhas_vacinacao', true));
			...
			$msg=array();
			$retorno = $this->atend_campanhas_vacinacao_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/atend_campanhas_vacinacao", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->atend_campanhas_vacinacao_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.atend_campanhas_vacinacao
						...
						INSERT INTO indicador_plugin.atend_campanhas_vacinacao
							 ...
							 (
								SELECT aep.dt_referencia,
									   ...
									   aep.nr_meta,
									   'S'
								  FROM indicador_plugin.atend_campanhas_vacinacao aep
								 WHERE aep.dt_exclusao IS NULL
								   ...
								   AND aep.dt_referencia = (SELECT MAX(eap1.dt_referencia) 
															  FROM indicador_plugin.atend_campanhas_vacinacao eap1
		...
		redirect( 'indicador_plugin/atend_campanhas_vacinacao' );
