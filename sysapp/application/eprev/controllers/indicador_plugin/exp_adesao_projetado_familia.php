<?php
class exp_adesao_projetado_familia extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::EXP_NUMERO_DE_ADESAO_X_NUMERO_PROJETADO_FAMILIA);
		...
		$this->load->model('indicador_plugin/exp_adesao_projetado_familia_model');

		$this->token_acesso = md5('EXP_NUMERO_DE_ADESAO_X_NUMERO_PROJETADO_FAMILIA'); 
    }
    ...
    public function index()
    {
		...
		if((indicador_db::verificar_permissao($this->cd_usuario, 'GN')) OR ($this->session->userdata('indic_12') == '*'))
		{
			...
			$this->load->view('indicador_plugin/exp_adesao_projetado_familia/index',$data);
		}
		else
        {
    ...
    public function listar()
    {
    	...
    	if((indicador_db::verificar_permissao($this->cd_usuario, 'GN')) OR ($this->session->userdata('indic_12') == '*'))
        {
			...
			$data['collection'] = $this->exp_adesao_projetado_familia_model->listar($data['tabela'][0]['cd_indicador_tabela']);
	        
			$this->load->view('indicador_plugin/exp_adesao_projetado_familia/index_result', $data);
        }
        else
		{
			exibir_mensagem('ACESSO NÃO PERMITIDO');
    ...
    public function cadastro($cd_exp_adesao_projetado_familia = 0)
	{
		CheckLogin();
		...
		if((indicador_db::verificar_permissao($this->cd_usuario, 'GN')) OR ($this->session->userdata('indic_12') == '*'))
		{
			...
			if(intval($cd_exp_adesao_projetado_familia) == 0)
			{
				$row = $this->exp_adesao_projetado_familia_model->carrega_referencia();
								
				$data['row'] =array(
					'cd_exp_adesao_projetado_familia' => $cd_exp_adesao_projetado_familia,
			...
			{
				$data['row'] = $this->exp_adesao_projetado_familia_model->carrega($cd_exp_adesao_projetado_familia);
			}

			$this->load->view('indicador_plugin/exp_adesao_projetado_familia/cadastro', $data);
		}
		else
		{
			exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function salvar()
	{
		...
		{		
			...
			$args = array(
				'cd_exp_adesao_projetado_familia' => $this->input->post('cd_exp_adesao_projetado_familia', true),
			...
			$this->exp_adesao_projetado_familia_model->salvar($args);
			
			$this->criar_indicador();
			
			redirect('indicador_plugin/exp_adesao_projetado_familia', 'refresh');
		}
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function salvar_integracao_eletro()
	{
		$_POST = array_merge($_POST, (array) json_decode(file_get_contents('php://input')));

		if(trim($_POST['token_acesso']) != '')
		{
			if(trim($_POST['token_acesso']) == $this->token_acesso)
			{
				if(trim($_POST['dt_referencia']) == '' OR trim($_POST['nr_percentual_f']) == '' OR trim($_POST['ds_usuario']) == '')
				{
					$ar_ret['fl_erro'] = 'S';
					$ar_ret['cd_erro'] = '3';
					$ar_ret['retorno'] = utf8_encode('ERRO: campos obrigatórios não enviados');	
				}
				else
				{
	...
	public function excluir($cd_exp_adesao_projetado_familia)
	{
		CheckLogin();
		...
		if((indicador_db::verificar_permissao($this->cd_usuario,'GN')) OR ($this->session->userdata('indic_12') == '*'))
		{
			$this->exp_adesao_projetado_familia_model->excluir($cd_exp_adesao_projetado_familia, $this->cd_usuario);
	...
	public function criar_indicador()
	{
		...
		if((indicador_db::verificar_permissao($this->cd_usuario, 'GN')) OR ($this->session->userdata('indic_12') == '*'))
		{
			...
			$collection = $this->exp_adesao_projetado_familia_model->listar($tabela[0]['cd_indicador_tabela']);
	...
	public function fechar_periodo()
	{
		...
		if((indicador_db::verificar_permissao($this->cd_usuario, 'GN')) OR ($this->session->userdata('indic_12') == '*'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$args = array(
					'cd_exp_adesao_projetado_familia' => 0,
			...
			$this->exp_adesao_projetado_familia_model->fechar_periodo($tabela[0]['cd_indicador_tabela'], $this->cd_usuario);
