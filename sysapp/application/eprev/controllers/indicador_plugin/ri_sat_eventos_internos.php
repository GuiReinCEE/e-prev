<?php
class ri_sat_eventos_internos extends Controller
{
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::RI_SATISFACAO_COM_EVENTOS_INTERNOS);
		...
		$this->load->model( 'indicador_plugin/ri_sat_eventos_internos_model' );
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(), 'AC') || indicador_db::verificar_permissao(usuario_id(), 'GE'))
		{
	        ...
	        $this->load->view('indicador_plugin/ri_sat_eventos_internos/index',$data);
		}
    }
    ...
    function listar()
    {
		if(indicador_db::verificar_permissao(usuario_id(), 'AC') || indicador_db::verificar_permissao(usuario_id(), 'GE'))
        {
			...
			$this->ri_sat_eventos_internos_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/ri_sat_eventos_internos/index_result', $data);
        }
    }

	function cadastro($cd_ri_sat_eventos_internos = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(), 'AC') || indicador_db::verificar_permissao(usuario_id(), 'GE'))
		{
			$args   = array();
			...
			$args['cd_ri_sat_eventos_internos'] = $cd_ri_sat_eventos_internos;
			
			if(intval($args['cd_ri_sat_eventos_internos']) == 0)
			{
				$this->ri_sat_eventos_internos_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_ri_sat_eventos_internos'] = $args['cd_ri_sat_eventos_internos'];
				$data['row']['nr_valor_1']                 = "";
				$data['row']['nr_valor_2']                 = "";
				$data['row']['fl_media']                   = "";
				$data['row']['observacao']                 = "";
			...
			{
				$this->ri_sat_eventos_internos_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/ri_sat_eventos_internos/cadastro', $data);
		}
	}
	...
	function salvar()
	{
		if(indicador_db::verificar_permissao(usuario_id(), 'AC') || indicador_db::verificar_permissao(usuario_id(), 'GE'))
		{
			...
			$args['cd_ri_sat_eventos_internos'] = $this->input->post('cd_ri_sat_eventos_internos', true);
			...
			$this->ri_sat_eventos_internos_model->salvar($result, $args);
			$this->criar_indicador();
			
			redirect("indicador_plugin/ri_sat_eventos_internos", "refresh");
		}
	}
	
	function excluir($cd_ri_sat_eventos_internos)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'AC'))
		{
			...
			$this->ri_sat_eventos_internos_model->excluir($result, $args);
			$this->criar_indicador();
