<?php
class Juridico_sucesso_acoes_judiciais_cons_mensal extends Controller
{
	function __construct()
	{
		...
		$this->enum_indicador = intval(enum_indicador::JURIDICO_SUCESSO_ACOES_JUDICIAIS_CONS_MENSAL);
		...
		$this->load->model('indicador_plugin/juridico_sucesso_acoes_judiciais_cons_mensal_model');
	}
	...
	public function index()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GJ'))
		{
			...
			$this->load->view('indicador_plugin/juridico_sucesso_acoes_judiciais_cons_mensal/index', $data);
		}
		else
		{
	...
	public function listar()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GJ'))
		{
			...
			$data['collection'] = $this->juridico_sucesso_acoes_judiciais_cons_mensal_model->listar($data['tabela'][0]['cd_indicador_tabela'] );
			
			$this->load->view('indicador_plugin/juridico_sucesso_acoes_judiciais_cons_mensal/index_result', $data);
		}
		else
		{
			exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function cadastro($cd_juridico_sucesso_acoes_judiciais_cons_mensal = 0)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GJ'))
		{
			$data['label_0']  = $this->label_0;
			...
			if(intval($cd_juridico_sucesso_acoes_judiciais_cons_mensal) == 0)
			{
				$row = $this->juridico_sucesso_acoes_judiciais_cons_mensal_model->carrega_referencia();
				
				$data['row'] = array(
					'cd_juridico_sucesso_acoes_judiciais_cons_mensal' => intval($cd_juridico_sucesso_acoes_judiciais_cons_mensal),
					'fl_media'             		                              => '',
					'nr_inicial'                                              => 0,
					'nr_improcede_1'                                          => 0,
					'nr_parcial_1'                                            => 0,
			...
			{
				$data['row'] = $this->juridico_sucesso_acoes_judiciais_cons_mensal_model->carrega($cd_juridico_sucesso_acoes_judiciais_cons_mensal);
			}

			$this->criar_indicador();

			$this->load->view('indicador_plugin/juridico_sucesso_acoes_judiciais_cons_mensal/cadastro', $data);
		}
		else
		{
			exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	function get_valores()
	{
		...
		$row = $this->juridico_sucesso_acoes_judiciais_cons_mensal_model->get_valores($ano, $mes);
    ...
    public function salvar()
    {
        if(indicador_db::verificar_permissao($this->cd_usuario, 'GJ'))
        {
            $cd_juridico_sucesso_acoes_judiciais_cons_mensal = $this->input->post('cd_juridico_sucesso_acoes_judiciais_cons_mensal', TRUE);
            ...
            if(intval($cd_juridico_sucesso_acoes_judiciais_cons_mensal) == 0)
            {
                $this->juridico_sucesso_acoes_judiciais_cons_mensal_model->salvar($args);
            }
            else
            {
                $this->juridico_sucesso_acoes_judiciais_cons_mensal_model->atualizar($cd_juridico_sucesso_acoes_judiciais_cons_mensal, $args);
            }

            $this->criar_indicador();

            redirect('indicador_plugin/juridico_sucesso_acoes_judiciais_cons_mensal', 'refresh');
        }
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
    ...
    private function set_status_atividade($args)
    {
    	...
    	$row = $this->juridico_sucesso_acoes_judiciais_cons_mensal_model->carrega_referencia_status($args['dt_referencia_db'], $ds_calculo);

    	$improcede = $this->juridico_sucesso_acoes_judiciais_cons_mensal_model->get_status(
    		$args['pr_improcede'],
    	...
    	$parcial   = $this->juridico_sucesso_acoes_judiciais_cons_mensal_model->get_status(
    		$args['pr_parcial'],
    	...
    	$procede   = $this->juridico_sucesso_acoes_judiciais_cons_mensal_model->get_status(
    		$args['pr_procede'],
    ...
    public function excluir($cd_juridico_sucesso_acoes_judiciais_cons_mensal)
    {
        if(indicador_db::verificar_permissao($this->cd_usuario, 'GJ'))
        {
            $this->juridico_sucesso_acoes_judiciais_cons_mensal_model->excluir(
                $cd_juridico_sucesso_acoes_judiciais_cons_mensal, 
    ...
    public function criar_indicador()
    {
        if(indicador_db::verificar_permissao($this->cd_usuario, 'GJ'))
        {
            ...
            $collection = $this->juridico_sucesso_acoes_judiciais_cons_mensal_model->listar($tabela[0]['cd_indicador_tabela']);
        ...
        if(indicador_db::verificar_permissao($this->cd_usuario, 'GJ'))
        {
           ...
           /* $collection = $this->juridico_sucesso_acoes_judiciais_cons_mensal_model->listar($tabela[0]['cd_indicador_tabela']);
            ...
            if(intval($contador_ano_atual) > 0)
            {
                ...
                $args = array(
                    'cd_juridico_sucesso_acoes_judiciais_cons_mensal' => 0, 
            ...
            $this->juridico_sucesso_acoes_judiciais_cons_mensal_model->fechar_periodo($tabela[0]['cd_indicador_tabela'], $this->cd_usuario);
