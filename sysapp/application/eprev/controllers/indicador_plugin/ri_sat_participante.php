<?php
class ri_sat_participante extends Controller
{
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::RI_SATISFACAO_DO_PARTICIPANTE);
		...
		$this->load->model('indicador_plugin/ri_sat_participante_model');	
    }
    ...
    public function index()
    {
		if(indicador_db::verificar_permissao($this->cd_usuario,'AC') OR indicador_db::verificar_permissao($this->cd_usuario, 'GE'))
		{
	        ...
	        $this->load->view('indicador_plugin/ri_sat_participante/index', $data);
		}
		else
		{
    ...
    public function listar()
    {
		if(indicador_db::verificar_permissao($this->cd_usuario,'AC') OR indicador_db::verificar_permissao($this->cd_usuario, 'GE'))
        {
            ...
            $data['collection'] = $this->ri_sat_participante_model->listar($data['tabela'][0]['cd_indicador_tabela']);

            $this->load->view('indicador_plugin/ri_sat_participante/index_result', $data);
        }
        else
		{
			exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function cadastro($cd_ri_sat_participante = 0)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'AC') OR indicador_db::verificar_permissao($this->cd_usuario, 'GE'))
		{
			$data = array();
			...
			if(intval($cd_ri_sat_participante) == 0)
			{
				$row = $this->ri_sat_participante_model->carrega_referencia();
				
				$data['row'] = array(
					'cd_ri_sat_participante' => $cd_ri_sat_participante,
					'nr_valor_1'            => 0,
					'nr_valor_2'            => 0,
			...
			{
				$data['row'] = $this->ri_sat_participante_model->carrega($cd_ri_sat_participante);
			}

			$this->load->view('indicador_plugin/ri_sat_participante/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	public function salvar()
	{
		...
		{		
			...
			$args = array(
				'cd_ri_sat_participante' => intval($this->input->post('cd_ri_sat_participante', true)),
			...
			$this->ri_sat_participante_model->salvar($args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/ri_sat_participante", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	public function excluir($cd_ri_sat_participante)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'AC') OR indicador_db::verificar_permissao($this->cd_usuario, 'GE'))
		{
			$this->ri_sat_participante_model->excluir($cd_ri_sat_participante, $this->cd_usuario);
	...
	public function criar_indicador()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'AC') OR indicador_db::verificar_permissao($this->cd_usuario, 'GE'))
		{
			...
			$collection = $this->ri_sat_participante_model->listar($tabela[0]['cd_indicador_tabela']);			 
