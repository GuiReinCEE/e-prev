<?php
class atuarial_rge extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::ATUARIAL_PLANO_RGE);
		...
		$this->load->model('indicador_plugin/atuarial_rge_model');
    }
	...
	function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GP'))
		{
	        ...
	        $this->load->view('indicador_plugin/atuarial_rge/index', $data);
		}
		else
		{
	...
	function listar()
    {
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
        {
	        ...
	        $data['collection'] = $this->atuarial_rge_model->listar($data['tabela'][0]['cd_indicador_tabela']);
			
			$this->load->view('indicador_plugin/atuarial_rge/index_result', $data);
        }
        else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function cadastro($cd_atuarial_rge = 0)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
		{
			$data = array();
			...
			if(intval($cd_atuarial_rge) == 0)
			{
				$row = $this->atuarial_rge_model->carrega_referencia($data['tabela'][0]['cd_indicador_tabela']);
				
				$data['row'] = array(
					'cd_atuarial_rge' => $cd_atuarial_rge,
			...
			{
				$data['row'] = $this->atuarial_rge_model->carrega($cd_atuarial_rge);
			}

			$this->load->view('indicador_plugin/atuarial_rge/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args = array(
				'cd_atuarial_rge'     => intval($this->input->post('cd_atuarial_rge', true)),
			...
			$this->atuarial_rge_model->salvar($args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/atuarial_rge", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_atuarial_rge)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
		{
			$this->atuarial_rge_model->excluir($cd_atuarial_rge,$this->cd_usuario);
	...
	function criar_indicador()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GP'))
		{
			...
			$collection = $this->atuarial_rge_model->listar($tabela[0]['cd_indicador_tabela']);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
		{
			...
			$this->atuarial_rge_model->fechar_periodo($tabela[0]['cd_indicador_tabela'], $this->cd_usuario);
