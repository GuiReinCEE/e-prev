<?php
class ri_sat_divul_interna extends Controller
{
	var	$label_0 = "Ano";
	var	$label_1 = "Total de pesq. Respondidas";
	var	$label_2 = "Total de pessoas Satisfeitas";
	var	$label_3 = "% satisfação com divulgação interna/externa";
    var	$label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RI_SATISFACAO_COM_A_DIVULGACAO_INTERNA);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
	        ...
	        $this->load->view('indicador_plugin/ri_sat_divul_interna/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
        {
	        $this->load->model( 'indicador_plugin/ri_sat_divul_interna_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->ri_sat_divul_interna_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/ri_sat_divul_interna/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$this->load->model('indicador_plugin/ri_sat_divul_interna_model');
			$row=$this->ri_sat_divul_interna_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.ri_sat_divul_interna
			...
			$this->load->view('indicador_plugin/ri_sat_divul_interna/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$args['cd_ri_sat_divul_interna']=intval($this->input->post('cd_ri_sat_divul_interna', true));
			...
			$msg=array();
			$retorno = $this->ri_sat_divul_interna_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/ri_sat_divul_interna", "refresh" );
					#redirect( "indicador_plugin/ri_sat_divul_interna/detalhe/0", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$this->ri_sat_divul_interna_model->excluir($id);
