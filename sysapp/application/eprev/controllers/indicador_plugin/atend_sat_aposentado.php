...
class atend_sat_aposentado extends Controller
{
    var $label_0 = "Mês";
    var $label_1 = "Total de participantes no evento";
    var $label_2 = "Total de satisfeitos";
    var $label_3 = "Total de avaliações preenchidas";
    var $label_4 = "% de Satisfação";
    var $label_5 = "Meta";
    ...
    {
        parent::Controller();
        $this->enum_indicador = intval(enum_indicador::ATENDIMENTO_SATISFACAO_COM_O_EVENTO_ENCONTRO_APOSENTADOS);
    }
    ...
    function index()
    {
        ...
        if (CheckLogin() && indicador_db::verificar_permissao(usuario_id(), 'GP' ))
        {
            ...
            $this->load->view('indicador_plugin/atend_sat_aposentado/index.php', $data);
        }
    }
    ...
    function listar()
    {
        ...
        if (CheckLogin() && indicador_db::verificar_permissao(usuario_id(), 'GP' ))
        {
            $this->load->model('indicador_plugin/atend_sat_aposentado_model');
            ...
            if (sizeof($tabela) > 0)
            {
                ...
                $this->atend_sat_aposentado_model->listar($result, $args);

                $data['collection'] = $result->result_array();

                $this->load->view('indicador_plugin/atend_sat_aposentado/partial_result', $data);
            }
            else
            {
    ...
    function detalhe($cd=0)
    {
        ...
        if (CheckLogin() && indicador_db::verificar_permissao(usuario_id(), 'GP' ))
        {
            ...
            $this->load->model('indicador_plugin/atend_sat_aposentado_model');
            $row = $this->atend_sat_aposentado_model->carregar($cd);
            if ($row)
            {
                if ($cd == 0)
                {
                    $sql = "
                        SELECT TO_CHAR(dt_referencia + '1 month'::interval,'DD/MM/YYYY') AS mes_referencia, 
                               ...
                               nr_meta, 
                               cd_indicador_tabela
                          FROM indicador_plugin.atend_sat_aposentado 
            ...
            $this->load->view('indicador_plugin/atend_sat_aposentado/detalhe', $data);
        }
    }
    ...
    function salvar()
    {
        ...
        if (CheckLogin() && indicador_db::verificar_permissao(usuario_id(), 'GP' ))
        {
            ...
            $args['cd_atend_sat_aposentado'] = intval($this->input->post('cd_atend_sat_aposentado', true));
            $args["dt_referencia"]           = $this->input->post("dt_referencia", true);
            $args["cd_usuario_inclusao"]     = usuario_id();
            $args["cd_indicador_tabela"]     = $this->input->post("cd_indicador_tabela", true);
            $args["fl_media"]                = $this->input->post("fl_media", true);

            $args["nr_valor_0"] = app_decimal_para_db($this->input->post("nr_valor_0", true));
            ...
            $msg = array();
            $retorno = $this->atend_sat_aposentado_model->salvar($args, $msg);
            ...
            if ($retorno)
            {
                ...
                {
                    redirect("indicador_plugin/atend_sat_aposentado", "refresh");
                }
            }
            else
            {
    ...
    function excluir($id)
    {
        ...
        if (CheckLogin() && indicador_db::verificar_permissao(usuario_id(), 'GP' ))
        {
            ...
            $this->atend_sat_aposentado_model->excluir($id);
            ...
            {#tabela_existe
                ...
                foreach ($collection as $item)
                {
                    ...
                    if (intval($item['ano_referencia']) >= intval($tabela[0]['nr_ano_referencia']) - 5)
                    {
                        ...
                        $indicador[$linha][1] = app_decimal_para_php($nr_valor_0);
                ...
                $sql.=indicador_db::sql_inserir_grafico(
                        $tabela[0]['cd_indicador_tabela'], enum_indicador_grafico_tipo::LINHA, '4,4,0,0;5,5,0,0;6,6,0,0', "0,0,1,$linha_sem_media", "4,4,1,$linha_sem_media;5,5,1,$linha_sem_media;6,6,1,$linha_sem_media", usuario_id(), $coluna_para_ocultar, 1, 2
                );
    ...
    function fechar_periodo()
    {
        ...
        if (CheckLogin() && indicador_db::verificar_permissao(usuario_id(), 'GP' ))
        {
            ...
            {#tabela_existe
                ...
                if (sizeof($media_ano) > 0)
                {
                    ...
                    $sql.=sprintf(" INSERT INTO indicador_plugin.atend_sat_aposentado
        ...
        redirect('indicador_plugin/atend_sat_aposentado');
