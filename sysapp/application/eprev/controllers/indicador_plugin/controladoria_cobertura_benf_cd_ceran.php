<?php
class Controladoria_cobertura_benf_cd_ceran extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::CONTROLADORIA_COBERTURA_BENEFICIO_CD_CERAN);
		...
		$this->load->model('indicador_plugin/controladoria_cobertura_benf_cd_ceran_model');
    }
	...
	function index()
    {
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
		{
	        ...
	        $this->load->view('indicador_plugin/controladoria_cobertura_benf_cd_ceran/index', $data);
		}
		else
		{
	...
	function listar()
    {
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
        {
			...
			$data['collection'] = $this->controladoria_cobertura_benf_cd_ceran_model->listar($data['tabela'][0]['cd_indicador_tabela']);
			
			$this->load->view('indicador_plugin/controladoria_cobertura_benf_cd_ceran/index_result', $data);
        }
        else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function cadastro($cd_controladoria_cobertura_benf_cd_ceran = 0)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
		{
			$data = array();
			...
			if(intval($cd_controladoria_cobertura_benf_cd_ceran) == 0)
			{
				$row = $this->controladoria_cobertura_benf_cd_ceran_model->carrega_referencia($data['tabela'][0]['cd_indicador_tabela']);
				
				$data['row'] = array(
					'cd_controladoria_cobertura_benf_cd_ceran' => $cd_controladoria_cobertura_benf_cd_ceran,
					'nr_valor_1'            			 => 0,
					'nr_valor_2'            			 => 0,
			...
			{
				$data['row'] = $this->controladoria_cobertura_benf_cd_ceran_model->carrega($cd_controladoria_cobertura_benf_cd_ceran);
			}

			$this->load->view('indicador_plugin/controladoria_cobertura_benf_cd_ceran/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args = array(
				'cd_controladoria_cobertura_benf_cd_ceran' => intval($this->input->post('cd_controladoria_cobertura_benf_cd_ceran', true)),
			...
			$this->controladoria_cobertura_benf_cd_ceran_model->salvar($args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/controladoria_cobertura_benf_cd_ceran", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_controladoria_cobertura_benf_cd_ceran)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
		{
			$this->controladoria_cobertura_benf_cd_ceran_model->excluir($cd_controladoria_cobertura_benf_cd_ceran, $this->cd_usuario);
	...
	function criar_indicador()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario,'GP'))
		{
			...
			$collection = $this->controladoria_cobertura_benf_cd_ceran_model->listar($tabela[0]['cd_indicador_tabela']);			 
