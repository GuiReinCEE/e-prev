<?php
class atend_call extends Controller
{
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::ATENDIMENTO_TEMPO_MEDIO_DE_ESPERA_CALL_CENTER);
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/atend_call/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
        {
	        $this->load->model( 'indicador_plugin/Atend_call_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->Atend_call_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/atend_call/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
		{
			...
			$this->load->model('indicador_plugin/Atend_call_model');
			$row=$this->Atend_call_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					cd_indicador_tabela, nr_percentual_f,
				   (SELECT COUNT(*)
				      FROM indicador_plugin.atend_call
			...
			$this->load->view('indicador_plugin/atend_call/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
		{
			...
			$args['cd_atend_call']=intval($this->input->post('cd_atend_call', true));
			...
			$msg=array();
			$retorno = $this->Atend_call_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/atend_call", "refresh" );
				}
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
		{
			...
			$this->Atend_call_model->excluir($id);
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$indicador[$linha][1] = '';
					$indicador[$linha][2] = app_decimal_para_php($nr_media_2);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GCM' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.atend_call
		...
		redirect( 'indicador_plugin/atend_call' );
