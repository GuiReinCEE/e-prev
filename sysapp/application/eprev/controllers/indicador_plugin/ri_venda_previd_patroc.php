<?php
class ri_venda_previd_patroc extends Controller
{
	var	$label_0 = "Mês";
	var	$label_1 = "Nº de Visitas";
	var	$label_2 = "Nº de Contatos";
	var	$label_3 = "Inscrições no período";
    var	$label_4 = "% Venda";
    var	$label_5 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RI_VENDA_DOS_PLANOS_PREVIDENCIARIOS_PATROCINADORA);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
	        ...
	        $this->load->view('indicador_plugin/ri_venda_previd_patroc/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
        {
	        $this->load->model( 'indicador_plugin/ri_venda_previd_patroc_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->ri_venda_previd_patroc_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/ri_venda_previd_patroc/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$this->load->model('indicador_plugin/ri_venda_previd_patroc_model');
			$row=$this->ri_venda_previd_patroc_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.ri_venda_previd_patroc
			...
			$this->load->view('indicador_plugin/ri_venda_previd_patroc/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$args['cd_ri_venda_previd_patroc']=intval($this->input->post('cd_ri_venda_previd_patroc', true));
			...
			$msg=array();
			$retorno = $this->ri_venda_previd_patroc_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/ri_venda_previd_patroc", "refresh" );
				}
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$this->ri_venda_previd_patroc_model->excluir($id);
				...
				foreach( $collection as $item )
				{
					...
					if( intval($item['ano_referencia'])>=intval($tabela[0]['nr_ano_referencia'])-5 )
					{
						...
						else
						{
							...
							{
								$nr_percentual_f = ( floatval($nr_valor_3)/floatval($nr_valor_2) )*100;
							}
							else
							{
								$nr_percentual_f = '0';
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			{#tabela_existe
				...
				$resultado_ano = ($nr_tot_ano_inscricao / $nr_tot_ano_contato) * 100;
				
				$sql = "
						INSERT INTO indicador_plugin.ri_venda_previd_patroc
							 ( 
								dt_referencia,
								dt_inclusao,
								cd_usuario_inclusao, 
		...
		redirect( 'indicador_plugin/ri_venda_previd_patroc' );
