<?php
class Investimento_rentabilidade_pga extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::INVESTIMENTO_RENTABILIDADE_PGA);
		...
		$this->load->model('indicador_plugin/investimento_rentabilidade_pga_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
	        ...
	        $this->load->view('indicador_plugin/investimento_rentabilidade_pga/index',$data);
		}
    }
    ...
    function listar()
    {
    	if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
        {
			...
			$this->investimento_rentabilidade_pga_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/investimento_rentabilidade_pga/index_result', $data);
        }
        else
		{
    ...
    function cadastro($cd_investimento_rentabilidade_pga = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
			$args   = array();
			...
			$args['cd_investimento_rentabilidade_pga'] = $cd_investimento_rentabilidade_pga;
			
			if(intval($args['cd_investimento_rentabilidade_pga']) == 0)
			{
				$this->investimento_rentabilidade_pga_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_investimento_rentabilidade_pga'] = $args['cd_investimento_rentabilidade_pga'];
			...
			{
				$this->investimento_rentabilidade_pga_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/investimento_rentabilidade_pga/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_investimento_rentabilidade_pga'] = intval($this->input->post('cd_investimento_rentabilidade_pga', true));
			...
			$this->investimento_rentabilidade_pga_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/investimento_rentabilidade_pga", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_investimento_rentabilidade_pga)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
			$args   = array();
			...
			$this->investimento_rentabilidade_pga_model->excluir($result, $args);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->investimento_rentabilidade_pga_model->atualiza_fechar_periodo($result, $args);
			}

			$this->investimento_rentabilidade_pga_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
