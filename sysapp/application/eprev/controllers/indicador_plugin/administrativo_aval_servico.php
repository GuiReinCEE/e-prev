<?php
class administrativo_aval_servico extends Controller
{
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RH_MEDIA_AVALIACAO_CONTRATACAO_SERVICO_SEMESTRE);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GGS' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/administrativo_aval_servico/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GGS' ) )
        {
	        $this->load->model( 'indicador_plugin/administrativo_aval_servico_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->administrativo_aval_servico_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/administrativo_aval_servico/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GGS' ) )
		{
			...
			$this->load->model('indicador_plugin/administrativo_aval_servico_model');
			$row=$this->administrativo_aval_servico_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.administrativo_aval_servico 
			...
			$this->load->view('indicador_plugin/administrativo_aval_servico/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GGS' ) )
		{
			...
			$args['cd_administrativo_aval_servico']=intval($this->input->post('cd_administrativo_aval_servico', true));
			...
			$msg=array();
			$retorno = $this->administrativo_aval_servico_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/administrativo_aval_servico", "refresh" );	
					#redirect( "indicador_plugin/administrativo_aval_servico", "refresh" );	
				}				
			}
			else
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GGS' ) )
		{
			...
			$this->administrativo_aval_servico_model->excluir($id);
