<?php
class Controladoria_regul_solic_previc extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::CONTROLADORIA_ALT_REG_SOLICITACAO_PREVIC);
		...
		$this->load->model('indicador_plugin/controladoria_regul_solic_previc_model');
    }
	...
	function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GC'))
		{
	        ...
	        $this->load->view('indicador_plugin/controladoria_regul_solic_previc/index', $data);
		}
		else
		{
	...
	function listar()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GC'))
        {
			...
			$this->controladoria_regul_solic_previc_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/controladoria_regul_solic_previc/index_result', $data);
        }
        else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function cadastro($cd_controladoria_regul_solic_previc = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC'))
		{
			$args   = array();
			...
			$args['cd_controladoria_regul_solic_previc'] = $cd_controladoria_regul_solic_previc;
			
			if(intval($args['cd_controladoria_regul_solic_previc']) == 0)
			{
				$this->controladoria_regul_solic_previc_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_controladoria_regul_solic_previc'] = $args['cd_controladoria_regul_solic_previc'];
				$data['row']['ds_evento']                           = "";
				$data['row']['nr_valor_1']                          = "";
				$data['row']['nr_valor_2']                          = "";
				$data['row']['nr_meta_f']                           = (isset($arr['nr_meta_f']) ? $arr['nr_meta_f'] : 0);
				$data['row']['fl_media']                            = "";
				$data['row']['observacao']                          = "";
			...
			{
				$this->controladoria_regul_solic_previc_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/controladoria_regul_solic_previc/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_controladoria_regul_solic_previc'] = intval($this->input->post('cd_controladoria_regul_solic_previc', true));
			...
			$args["nr_meta_f"]                           = $this->input->post("nr_meta_f", true);
            $args["observacao"]                          = $this->input->post("observacao", true);
			$args["cd_usuario"]                          = $this->session->userdata('codigo');

			$this->controladoria_regul_solic_previc_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/controladoria_regul_solic_previc", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_controladoria_regul_solic_previc)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC'))
		{
			$args   = array();
			...
			$this->controladoria_regul_solic_previc_model->excluir($result, $args);
		...
		if(indicador_db::verificar_permissao(usuario_id(),'GC'))
		{
			...
			foreach($collection as $item)
			{
				if(intval($item['ano_referencia']) >= intval($tabela[0]['nr_ano_referencia']) -5)
				{
					...
					$indicador[$linha][3] = app_decimal_para_php($nr_meta_f);
					$indicador[$linha][4] = (intval($nr_percentual_f) == 0 ? 'Não Atingido' : 'Atingido');
			...
			*/
			...
			$sql.=indicador_db::sql_inserir_grafico(
				...
				"2,2,1,$linha_sem_media;3,3,1,$linha_sem_media",
				usuario_id(),
				$coluna_para_ocultar,
				1,
				2
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GC'))
		{
			...
			$this->controladoria_regul_solic_previc_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
