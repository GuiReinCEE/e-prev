<?php
class Exp_adesao_potencial_sintae extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::EXP_NUMERO_DE_ADESAO_X_NUMERO_POTENCIAL_SINTAE);
		...
		$this->load->model('indicador_plugin/exp_adesao_potencial_sintae_model');
    }
    ...
    function index()
    {
		if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{
	        ...
	        $this->load->view('indicador_plugin/exp_adesao_potencial_sintae/index',$data);
		}
    }
    ...
    function listar()
    {
    	if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
        {
			...
			$this->exp_adesao_potencial_sintae_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/exp_adesao_potencial_sintae/index_result', $data);
        }
        else
		{
    ...
    function cadastro($cd_exp_adesao_potencial_sintae = 0)
	{
		if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{
			$args   = array();
			...
			$args['cd_exp_adesao_potencial_sintae'] = $cd_exp_adesao_potencial_sintae;
			
			if(intval($args['cd_exp_adesao_potencial_sintae']) == 0)
			{
				$this->exp_adesao_potencial_sintae_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_exp_adesao_potencial_sintae'] = $args['cd_exp_adesao_potencial_sintae'];
			...
			{
				$this->exp_adesao_potencial_sintae_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/exp_adesao_potencial_sintae/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_exp_adesao_potencial_sintae'] = intval($this->input->post('cd_exp_adesao_potencial_sintae', true));
			...
			$this->exp_adesao_potencial_sintae_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/exp_adesao_potencial_sintae", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_exp_adesao_potencial_sintae)
	{
		if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{
			$args   = array();
			...
			$this->exp_adesao_potencial_sintae_model->excluir($result, $args);
	...
	function fechar_periodo()
	{
		if((indicador_db::verificar_permissao(usuario_id(),'GE')) OR ($this->session->userdata('indic_12') == "*"))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->exp_adesao_potencial_sintae_model->atualiza_fechar_periodo($result, $args);
			}

			$this->exp_adesao_potencial_sintae_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
