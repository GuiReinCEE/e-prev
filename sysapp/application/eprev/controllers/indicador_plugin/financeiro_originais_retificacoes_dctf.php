<?php
class Financeiro_originais_retificacoes_dctf extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::FINANCEIRO_DCTF_ORIGINAIS_X_RETIFICACOES);
		...
		$this->load->model('indicador_plugin/financeiro_originais_retificacoes_dctf_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
	        ...
	        $this->load->view('indicador_plugin/financeiro_originais_retificacoes_dctf/index',$data);
		}
    }
    ...
    function listar()
    {
    	if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
        {
			...
			$this->financeiro_originais_retificacoes_dctf_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/financeiro_originais_retificacoes_dctf/index_result', $data);
        }
        else
		{
    ...
    function cadastro($cd_financeiro_originais_retificacoes_dctf = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$args['cd_financeiro_originais_retificacoes_dctf'] = $cd_financeiro_originais_retificacoes_dctf;
			
			if(intval($args['cd_financeiro_originais_retificacoes_dctf']) == 0)
			{
				$this->financeiro_originais_retificacoes_dctf_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_financeiro_originais_retificacoes_dctf'] = $args['cd_financeiro_originais_retificacoes_dctf'];
			...
			{
				$this->financeiro_originais_retificacoes_dctf_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/financeiro_originais_retificacoes_dctf/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_financeiro_originais_retificacoes_dctf'] = intval($this->input->post('cd_financeiro_originais_retificacoes_dctf', true));
			...
			$this->financeiro_originais_retificacoes_dctf_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/financeiro_originais_retificacoes_dctf", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_financeiro_originais_retificacoes_dctf)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$this->financeiro_originais_retificacoes_dctf_model->excluir($result, $args);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->financeiro_originais_retificacoes_dctf_model->atualiza_fechar_periodo($result, $args);
			}

			$this->financeiro_originais_retificacoes_dctf_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
