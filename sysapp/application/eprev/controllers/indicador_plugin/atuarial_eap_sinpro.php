<?php
class atuarial_eap_sinpro extends Controller
{
	var	$label_0 = "Mês";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::ATUARIAL_SINPRO);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
	        ...
	        $this->load->view('indicador_plugin/atuarial_eap_sinpro/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
        {
	        $this->load->model( 'indicador_plugin/atuarial_eap_sinpro_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->atuarial_eap_sinpro_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/atuarial_eap_sinpro/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
			...
			$this->load->model('indicador_plugin/atuarial_eap_sinpro_model');
			$row=$this->atuarial_eap_sinpro_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.atuarial_eap_sinpro 
			...
			$this->load->view('indicador_plugin/atuarial_eap_sinpro/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
			...
			$args['cd_atuarial_eap_sinpro']=intval($this->input->post('cd_atuarial_eap_sinpro', true));
			...
			$msg=array();
			$retorno = $this->atuarial_eap_sinpro_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/atuarial_eap_sinpro", "refresh" );	
					#redirect( "indicador_plugin/atuarial_eap_sinpro/detalhe/0", "refresh" );	
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
			...
			$this->atuarial_eap_sinpro_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC') )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.atuarial_eap_sinpro
		...
		redirect( 'indicador_plugin/atuarial_eap_sinpro' );
