<?php
class atend_doc_faltante extends Controller
{
	var	$label_0 = "Mês";
	var	$label_1 = "Total Maior que 60dias";
	var	$label_2 = "Total Geral por RE.d";
	var	$label_3 = "% Docum. Faltantes";
    var $label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::ATENDIMENTO_PERCENTUAL_DE_DOCUMENTOS_FALTANTES);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/atend_doc_faltante/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
        {
	        $this->load->model( 'indicador_plugin/atend_doc_faltante_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->atend_doc_faltante_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/atend_doc_faltante/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->load->model('indicador_plugin/atend_doc_faltante_model');
			$row=$this->atend_doc_faltante_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.atend_doc_faltante 
			...
			$this->load->view('indicador_plugin/atend_doc_faltante/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$args['cd_atend_doc_faltante']=intval($this->input->post('cd_atend_doc_faltante', true));
			...
			$msg=array();
			$retorno = $this->atend_doc_faltante_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/atend_doc_faltante", "refresh" );
				}
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->atend_doc_faltante_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.atend_doc_faltante
		...
		redirect( 'indicador_plugin/atend_doc_faltante' );
