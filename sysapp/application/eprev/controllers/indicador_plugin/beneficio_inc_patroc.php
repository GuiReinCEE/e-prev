<?php
class beneficio_inc_patroc extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Nº Contrib. Calc. Mês";
	var	$label_2 = "Nº Contrib. Calc. Incorreta mês";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::BENEFICIO_GERACAO_DA_RECEITA_INCORRECOES_NO_CALCULO_DAS_CONTRIBUICOES);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/beneficio_inc_patroc/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
        {
	        $this->load->model( 'indicador_plugin/beneficio_inc_patroc_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->beneficio_inc_patroc_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/beneficio_inc_patroc/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->load->model('indicador_plugin/beneficio_inc_patroc_model');
			$row=$this->beneficio_inc_patroc_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					cd_indicador_tabela,
				   (SELECT COUNT(*)
				      FROM indicador_plugin.beneficio_inc_patroc
			...
			$this->load->view('indicador_plugin/beneficio_inc_patroc/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$args['cd_beneficio_inc_patroc']=intval($this->input->post('cd_beneficio_inc_patroc', true));
			...
			$msg=array();
			$retorno = $this->beneficio_inc_patroc_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/beneficio_inc_patroc", "refresh" );	
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->beneficio_inc_patroc_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.beneficio_inc_patroc
		...
		redirect( 'indicador_plugin/beneficio_inc_patroc' );
