<?php
class ri_empresas_pospectadas extends Controller
{
	var	$label_0 = "Ano";
	var	$label_1 = "Nº de empresas a ser visitadas ";
	var	$label_2 = "Nº visitas";
	var	$label_3 = "% de visitas";
    var	$label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RI_EMPRESAS_PROSPECTADAS);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao(usuario_id(),'GN')) OR ($this->session->userdata('indic_12') == "*") )
		{
	        ...
	        $this->load->view('indicador_plugin/ri_empresas_pospectadas/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao(usuario_id(),'GN')) OR ($this->session->userdata('indic_12') == "*") )
        {
	        $this->load->model( 'indicador_plugin/ri_empresas_pospectadas_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->ri_empresas_pospectadas_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/ri_empresas_pospectadas/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao(usuario_id(),'GN')) OR ($this->session->userdata('indic_12') == "*") )
		{
			...
			$this->load->model('indicador_plugin/ri_empresas_pospectadas_model');
			$row=$this->ri_empresas_pospectadas_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
			...
			$this->load->view('indicador_plugin/ri_empresas_pospectadas/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao(usuario_id(),'GN')) OR ($this->session->userdata('indic_12') == "*") )
		{
			...
			$args['cd_ri_empresas_pospectadas']=intval($this->input->post('cd_ri_empresas_pospectadas', true));
			...
			$msg=array();
			$retorno = $this->ri_empresas_pospectadas_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/ri_empresas_pospectadas", "refresh" );
					#redirect( "indicador_plugin/ri_empresas_pospectadas/detalhe/0", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao(usuario_id(),'GN')) OR ($this->session->userdata('indic_12') == "*") )
		{
			...
			$this->ri_empresas_pospectadas_model->excluir($id);
