<?php
class administrativo_ept_hora extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Horas de Treinamento Efetivadas (Acumulado)";
	var	$label_2 = "Horas de Treinamento Previstas (Acumulado)";
	var	$label_3 = "% execução Plano Treinamento";
    var $label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RH_EXECUCAO_PLANO_TREINAMENTO_HORA);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/administrativo_ept_hora/index.php',$data);
		}
    }
    ...
    function listar()
    {
        ...
        {
			$this->load->helper( array('indicador') );
	        $this->load->model( 'indicador_plugin/Administrativo_ept_hora_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->Administrativo_ept_hora_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/administrativo_ept_hora/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC' ) )
		{
			...
			$this->load->model('indicador_plugin/Administrativo_ept_hora_model');
			$row=$this->Administrativo_ept_hora_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
			...
			$this->load->view('indicador_plugin/administrativo_ept_hora/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC' ) )
		{
			...
			$args['cd_administrativo_ept_hora']=intval($this->input->post('cd_administrativo_ept_hora', true));
			...
			$msg=array();
			$retorno = $this->Administrativo_ept_hora_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/administrativo_ept_hora", "refresh" );
				}
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC' ) )
		{
			...
			$this->Administrativo_ept_hora_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GC' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.administrativo_ept_hora
					( dt_referencia,dt_inclusao,cd_usuario_inclusao, nr_percentual_f, nr_meta, fl_media ) 
					VALUES ( '%s/01/01',current_timestamp,%s, %s, %s, 'S' ); "
					, intval($tabela[0]['nr_ano_referencia']) , usuario_id(), floatval($nr_percentual_f), floatval( app_decimal_para_db($nr_meta) ));
				}
		...
		redirect( 'indicador_plugin/administrativo_ept_hora' );
