<?php
class atend_partic_email extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Total de Partcipantes";
	var	$label_2 = "Total de E-mails";
	var	$label_3 = "% de mails cadastrados";
    var $label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::ATENDIMENTO_PARTICIPANTES_COM_EMAILS_CADASTRADOS);
    }
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/atend_partic_email/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
        {
	        ...
	        $this->load->model( 'indicador_plugin/Atend_partic_email_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->Atend_partic_email_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/atend_partic_email/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->load->model('indicador_plugin/Atend_partic_email_model');
			$row=$this->Atend_partic_email_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
			...
			$this->load->view('indicador_plugin/atend_partic_email/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$args['cd_atend_partic_email']=intval($this->input->post('cd_atend_partic_email', true));
			...
			$msg=array();
			$retorno = $this->Atend_partic_email_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/atend_partic_email", "refresh" );
				}
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->Atend_partic_email_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.atend_partic_email
		...
		redirect( 'indicador_plugin/atend_partic_email' );
