<?php
class juridico_sucesso_acoes_juchem extends Controller
{
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::JURIDICO_SUCESSO_DAS_ACOES_JUCHEM);
		...
		$this->load->model('indicador_plugin/juridico_sucesso_acoes_juchem_model' );   		
    }
    ...
    function index()
    {
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
	        ...
	        $this->load->view('indicador_plugin/juridico_sucesso_acoes_juchem/index',$data);
		}		
    }
    ...
    function listar()
    {
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
        {
			...
			$this->juridico_sucesso_acoes_juchem_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/juridico_sucesso_acoes_juchem/partial_result', $data);
        }    	
    }

	function detalhe($cd_juridico_sucesso_acoes_juchem = 0)
	{
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
			$args   = array();
			...
			$args['cd_juridico_sucesso_acoes_juchem'] = $cd_juridico_sucesso_acoes_juchem;
			
			if(intval($args['cd_juridico_sucesso_acoes_juchem']) == 0)
			{
				$data['row']['cd_juridico_sucesso_acoes_juchem'] = $args['cd_juridico_sucesso_acoes_juchem'];
				$data['row']['cd_etapa']       = "";
				$data['row']['nr_inicial']     = "";
			...
			{
				$this->juridico_sucesso_acoes_juchem_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}
			
			$args['cd_indicador_tabela'] = $tabela[0]['cd_indicador_tabela'];
			$this->juridico_sucesso_acoes_juchem_model->etapa($result, $args);
			$data['ar_fase'] = $result->result_array();			
			
			$this->load->view('indicador_plugin/juridico_sucesso_acoes_juchem/detalhe', $data);
		}        
	}	
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_juridico_sucesso_acoes_juchem'] = intval($this->input->post('cd_juridico_sucesso_acoes_juchem', true));
			...
			$this->juridico_sucesso_acoes_juchem_model->salvar($result, $args);
			$this->criar_indicador();
			
			redirect("indicador_plugin/juridico_sucesso_acoes_juchem", "refresh");
		}
        else
        {
	...
	function excluir($cd_juridico_sucesso_acoes_juchem)
	{
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
			$args   = array();
			...
			$this->juridico_sucesso_acoes_juchem_model->excluir($result, $args);
			$this->criar_indicador();
	...
	function fechar_periodo()
	{
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
			...
			$this->juridico_sucesso_acoes_juchem_model->fechar_periodo($result, $args);
		}
		redirect("indicador_plugin/juridico_sucesso_acoes_juchem", "refresh");
	}	
}
