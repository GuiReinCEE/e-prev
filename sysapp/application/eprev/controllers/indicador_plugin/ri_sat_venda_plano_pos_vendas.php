<?php
class ri_sat_venda_plano_pos_vendas extends Controller
{
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::RI_SATISFACAO_VENDA_PLANOS_POS_VENDA);
		...
		$this->load->model('indicador_plugin/ri_sat_venda_plano_pos_vendas_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(), 'AC') || indicador_db::verificar_permissao(usuario_id(), 'GE'))
		{
	        ...
	        $this->load->view('indicador_plugin/ri_sat_venda_plano_pos_vendas/index', $data);
		}
		else
        {
    ...
    function listar()
    {
		if(indicador_db::verificar_permissao(usuario_id(), 'AC') || indicador_db::verificar_permissao(usuario_id(), 'GE'))
        {
			...
			$this->ri_sat_venda_plano_pos_vendas_model->listar($result, $args);
			$data['collection'] = $result->result_array();

			$this->load->view('indicador_plugin/ri_sat_venda_plano_pos_vendas/index_result', $data);
        }
		else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function cadastro($cd_ri_sat_venda_plano_pos_vendas = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'AC'))
		{
			...
			$args['cd_ri_sat_venda_plano_pos_vendas'] = $cd_ri_sat_venda_plano_pos_vendas;

			if(intval($args['cd_ri_sat_venda_plano_pos_vendas']) == 0)
			{
				$this->ri_sat_venda_plano_pos_vendas_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_ri_sat_venda_plano_pos_vendas'] = $args['cd_ri_sat_venda_plano_pos_vendas'];
				$data['row']['nr_valor_1']                 = "";
				$data['row']['nr_valor_2']                 = "";
				$data['row']['fl_media']                   = "";
				$data['row']['observacao']                 = "";
			...
			{
				$this->ri_sat_venda_plano_pos_vendas_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/ri_sat_venda_plano_pos_vendas/cadastro', $data);
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_ri_sat_venda_plano_pos_vendas'] = intval($this->input->post('cd_ri_sat_venda_plano_pos_vendas', true));
			...
			$this->ri_sat_venda_plano_pos_vendas_model->salvar($result, $args);
			$this->criar_indicador();
			
			redirect("indicador_plugin/ri_sat_venda_plano_pos_vendas", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_ri_sat_venda_plano_pos_vendas)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'AC'))
		{
			$args   = array();
			...
			$this->ri_sat_venda_plano_pos_vendas_model->excluir($result, $args);
			$this->criar_indicador();
	...
	function criar_indicador()
	{
		if(indicador_db::verificar_permissao(usuario_id(), 'AC') || indicador_db::verificar_permissao(usuario_id(), 'GE'))
		{
			...
			$nr_resultado_f_total = ($nr_valor_1_total > 0 ? (($nr_valor_2_total / $nr_valor_1_total) * 100) : 0);
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$indicador[$linha][3] = number_format($nr_resultado_f_total,2,',','.');
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'AC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				$args['cd_ri_sat_venda_plano_pos_vendas'] = 0;
			...
			$this->ri_sat_venda_plano_pos_vendas_model->fechar_periodo($result, $args);
