<?php
class Financeiro_pp_pr_inconsistencia extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::FINANCEIRO_PP_PR_INCONSISTENCIA);
		...
		$this->load->model('indicador_plugin/financeiro_pp_pr_inconsistencia_model');
    }
    ...
    public function index()
    {
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GFC'))
		{
	        ...
	        $this->load->view('indicador_plugin/financeiro_pp_pr_inconsistencia/index', $data);
		}
		else
		{
    ...
    public function listar()
    {
        ...
        {			
			...
			$data['collection'] = $this->financeiro_pp_pr_inconsistencia_model->listar($data['tabela'][0]['cd_indicador_tabela'] );
			
			$this->load->view('indicador_plugin/financeiro_pp_pr_inconsistencia/index_result', $data);
        }
        else
		{
			exibir_mensagem('ACESSO NÃO PERMITIDO');
    ...
    public function cadastro($cd_financeiro_pp_pr_inconsistencia = 0)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GFC'))
		{			
			$data['label_0'] = $this->label_0;
			...
			if(intval($cd_financeiro_pp_pr_inconsistencia) == 0)
			{
				$row = $this->financeiro_pp_pr_inconsistencia_model->carrega_referencia(intval($data['tabela'][0]['nr_ano_referencia']));
				
				$data['row'] = array(
					'cd_financeiro_pp_pr_inconsistencia' => intval($cd_financeiro_pp_pr_inconsistencia),
				    'fl_media'             		                  => '',
				    'nr_total'                           => 0,
				    'nr_inconsistencia'                      => 0,
			...
			{
				$data['row'] = $this->financeiro_pp_pr_inconsistencia_model->carrega($cd_financeiro_pp_pr_inconsistencia);
				$data['row']['qt_ano'] = 1;
			}

			$this->load->view('indicador_plugin/financeiro_pp_pr_inconsistencia/cadastro', $data);
		}
		else
		{
			exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function salvar()
	{
		...
		{		
			...
			$nr_inconsistencia = app_decimal_para_db($this->input->post('nr_inconsistencia', true));
			$nr_resultado = ($nr_inconsistencia / $nr_total) * 100;

			$args = array(
				'cd_financeiro_pp_pr_inconsistencia' => intval($this->input->post('cd_financeiro_pp_pr_inconsistencia', true)),
			    'cd_indicador_tabela'                         => $this->input->post('cd_indicador_tabela', true),			    
			    'dt_referencia'                               => $this->input->post('dt_referencia', true),
			    'fl_media'                                    => $this->input->post('fl_media', true),
			    'nr_total'                           => $nr_total,
			    'nr_inconsistencia'                      => $nr_inconsistencia,
			...
			$this->financeiro_pp_pr_inconsistencia_model->salvar($args);
			
			$this->criar_indicador();
	
			redirect('indicador_plugin/financeiro_pp_pr_inconsistencia', 'refresh');
		}
        else
        {
            exibir_mensagem('ACESSO NÃO PERMITIDO');
	...
	public function excluir($cd_financeiro_pp_pr_inconsistencia)
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GFC'))
		{
			$this->financeiro_pp_pr_inconsistencia_model->excluir($cd_financeiro_pp_pr_inconsistencia, $this->cd_usuario);
	...
	public function criar_indicador()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GFC'))
		{
			...
			$collection = $this->financeiro_pp_pr_inconsistencia_model->listar($tabela[0]['cd_indicador_tabela']);
			...
			foreach($collection as $item)
			{
				if(intval($item['ano_referencia']) >= intval($tabela[0]['nr_ano_referencia']) -10)
				{
					...
					if(intval($item['ano_referencia']) == intval($tabela[0]['nr_ano_referencia']) && (trim($item['fl_media']) != 'S'))
					{
						...
						$nr_inconsistencia += $item['nr_inconsistencia'];
						$nr_meta                = $item['nr_meta'];
					}

					$indicador[$linha][0] = $referencia;
					$indicador[$linha][1] = app_decimal_para_php($item['nr_total']);
					$indicador[$linha][2] = app_decimal_para_php($item['nr_inconsistencia']);
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$indicador[$linha][2] = $nr_inconsistencia;
	...
	public function fechar_periodo()
	{
		if(indicador_db::verificar_permissao($this->cd_usuario, 'GFC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$args = array(
					'cd_financeiro_pp_pr_inconsistencia' => 0, 
			...
			$this->financeiro_pp_pr_inconsistencia_model->fechar_periodo($tabela[0]['cd_indicador_tabela'], $this->cd_usuario);
