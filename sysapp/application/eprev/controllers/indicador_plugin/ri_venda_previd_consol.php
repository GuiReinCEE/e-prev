<?php
class ri_venda_previd_consol extends Controller
{
	var	$label_0 = "Ano";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RI_VENDA_DOS_PLANOS_PREVIDENCIARIOS_CONSOLIDADO);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
	        ...
	        $this->load->view('indicador_plugin/ri_venda_previd_consol/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
        {
	        $this->load->model( 'indicador_plugin/ri_venda_previd_consol_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->ri_venda_previd_consol_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/ri_venda_previd_consol/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$this->load->model('indicador_plugin/ri_venda_previd_consol_model');
			$row=$this->ri_venda_previd_consol_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.ri_venda_previd_consol
			...
			$this->load->view('indicador_plugin/ri_venda_previd_consol/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$args['cd_ri_venda_previd_consol']=intval($this->input->post('cd_ri_venda_previd_consol', true));
			...
			$msg=array();
			$retorno = $this->ri_venda_previd_consol_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/ri_venda_previd_consol", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$this->ri_venda_previd_consol_model->excluir($id);
