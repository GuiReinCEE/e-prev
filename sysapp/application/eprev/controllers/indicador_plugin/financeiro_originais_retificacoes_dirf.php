<?php
class Financeiro_originais_retificacoes_dirf extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::FINANCEIRO_DIRF_ORIGINAIS_X_RETIFICACOES);
		...
		$this->load->model('indicador_plugin/financeiro_originais_retificacoes_dirf_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
	        ...
	        $this->load->view('indicador_plugin/financeiro_originais_retificacoes_dirf/index',$data);
		}
    }
    ...
    function listar()
    {
    	if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
        {
			...
			$this->financeiro_originais_retificacoes_dirf_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/financeiro_originais_retificacoes_dirf/index_result', $data);
        }
        else
		{
    ...
    function cadastro($cd_financeiro_originais_retificacoes_dirf = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$args['cd_financeiro_originais_retificacoes_dirf'] = $cd_financeiro_originais_retificacoes_dirf;
			
			if(intval($args['cd_financeiro_originais_retificacoes_dirf']) == 0)
			{
				$this->financeiro_originais_retificacoes_dirf_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_financeiro_originais_retificacoes_dirf'] = $args['cd_financeiro_originais_retificacoes_dirf'];
			...
			{
				$this->financeiro_originais_retificacoes_dirf_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/financeiro_originais_retificacoes_dirf/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_financeiro_originais_retificacoes_dirf'] = intval($this->input->post('cd_financeiro_originais_retificacoes_dirf', true));
			...
			$this->financeiro_originais_retificacoes_dirf_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/financeiro_originais_retificacoes_dirf", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_financeiro_originais_retificacoes_dirf)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			$args   = array();
			...
			$this->financeiro_originais_retificacoes_dirf_model->excluir($result, $args);
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GFC'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->financeiro_originais_retificacoes_dirf_model->atualiza_fechar_periodo($result, $args);
			}

			$this->financeiro_originais_retificacoes_dirf_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
