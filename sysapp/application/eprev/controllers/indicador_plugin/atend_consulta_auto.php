<?php
class atend_consulta_auto extends Controller
{
    var	$label_0 = "Mês";
	var	$label_1 = "Total de consultas ao auto-atendimento";
	var	$label_2 = "Total de acessos (call center + internet)";
	var	$label_3 = "% de consultas";
    var $label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::ATENDIMENTO_TOTAL_DE_CONSULTAS_AO_AUTO_ATENDIMENTO_CALL_CENTER_INTERNET);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/atend_consulta_auto/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
        {
	        $this->load->model( 'indicador_plugin/Atend_consulta_auto_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->Atend_consulta_auto_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/atend_consulta_auto/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->load->model('indicador_plugin/Atend_consulta_auto_model');
			$row=$this->Atend_consulta_auto_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.atend_consulta_auto 
			...
			$this->load->view('indicador_plugin/atend_consulta_auto/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$args['cd_atend_consulta_auto']=intval($this->input->post('cd_atend_consulta_auto', true));
			...
			$msg=array();
			$retorno = $this->Atend_consulta_auto_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/atend_consulta_auto", "refresh" );
				}
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			$this->Atend_consulta_auto_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'GP' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.atend_consulta_auto
		...
		redirect( 'indicador_plugin/atend_consulta_auto' );
