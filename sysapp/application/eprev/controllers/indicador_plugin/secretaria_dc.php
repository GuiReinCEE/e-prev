<?php
class secretaria_dc extends Controller
{
    /*
	var	$label_0 = "Mês";
	var	$label_1 = "Total de DC elaboradas";
	var	$label_2 = "Total de DC divulgadas fora do prazo (15 dias)";
	var	$label_3 = "% de DC divulgadas fora do prazo";
    var	$label_4 = "Meta";
	...
	*/
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::SECRETARIA_DC_ENCAMINHADA_PARA_DIVULGACAO_FORA_PRAZO);
    ...
    function index()
    {
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'SG' ) )
		{
	        ...
	        $this->load->view('indicador_plugin/secretaria_dc/index.php');
		}
    }
    ...
    function listar()
    {
        ...
        {
			$this->load->helper( array('indicador') );
	        $this->load->model( 'indicador_plugin/Secretaria_dc_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->Secretaria_dc_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/secretaria_dc/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'SG' ) )
		{
			...
			$this->load->model('indicador_plugin/Secretaria_dc_model');
			$row=$this->Secretaria_dc_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.secretaria_dc 
			...
			$this->load->view('indicador_plugin/secretaria_dc/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'SG' ) )
		{
			...
			$args['cd_secretaria_dc']=intval($this->input->post('cd_secretaria_dc', true));
			...
			$msg=array();
			$retorno = $this->Secretaria_dc_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/secretaria_dc", "refresh" );
				}
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'SG' ) )
		{
			...
			$this->Secretaria_dc_model->excluir($id);
	...
	function fechar_periodo()
	{
		...
		if(CheckLogin() && indicador_db::verificar_permissao( usuario_id(), 'SG' ) )
		{
			...
			{#tabela_existe
				...
				if(sizeof($media_ano)>0)
				{
					...
					$sql.=sprintf(" INSERT INTO indicador_plugin.secretaria_dc
		...
		redirect( 'indicador_plugin/secretaria_dc' );
