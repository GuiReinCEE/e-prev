<?php
class Investimento_evolucao_inpc extends Controller
{	
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::INVESTIMENTO_EVOLUCAO_INPC);
		...
		$this->load->model('indicador_plugin/investimento_evolucao_inpc_model');
    }
    ...
    function index()
    {
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
	        ...
	        $this->load->view('indicador_plugin/investimento_evolucao_inpc/index',$data);
		}
    }
    ...
    function listar()
    {
    	if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
        {
			...
			$this->investimento_evolucao_inpc_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/investimento_evolucao_inpc/index_result', $data);
        }
        else
		{
    ...
    function cadastro($cd_investimento_evolucao_inpc = 0)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
			$args   = array();
			...
			$args['cd_investimento_evolucao_inpc'] = $cd_investimento_evolucao_inpc;
			
			if(intval($args['cd_investimento_evolucao_inpc']) == 0)
			{
				$this->investimento_evolucao_inpc_model->carrega_referencia($result, $args);
				$arr = $result->row_array();
				
				$data['row']['cd_investimento_evolucao_inpc'] = $args['cd_investimento_evolucao_inpc'];
			...
			{
				$this->investimento_evolucao_inpc_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}

			$this->load->view('indicador_plugin/investimento_evolucao_inpc/cadastro', $data);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function salvar()
	{
		...
		{		
			...
			foreach ($collection as $key => $item) 
			{
				...
				{
					$valores[] = $item["nr_valor_2"];
				}
			}

			$args['cd_investimento_evolucao_inpc'] = intval($this->input->post('cd_investimento_evolucao_inpc', true));
			...
			$args["nr_valor_3"]                    = calculo_projetado_mensal($args["nr_valor_1"], intval($this->input->post("mes_referencia", true)));

			$valores[] = $args["nr_valor_2"];

			$args["nr_valor_4"]                    = calculo_acumulado($valores, intval($this->input->post("mes_referencia", true)));
			$args["nr_meta"]                       = app_decimal_para_db($this->input->post("nr_meta", true));
            $args["observacao"]                    = $this->input->post("observacao", true);
			$args["cd_usuario"]                    = $this->session->userdata('codigo');

			$this->investimento_evolucao_inpc_model->salvar($result, $args);
			
			$this->criar_indicador();
			
			redirect("indicador_plugin/investimento_evolucao_inpc", "refresh");
		}
        else
        {
            exibir_mensagem("ACESSO NÃO PERMITIDO");
	...
	function excluir($cd_investimento_evolucao_inpc)
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
			$args   = array();
			...
			$this->investimento_evolucao_inpc_model->excluir($result, $args);
	...
	public function get_valores()
	{
		...
		$this->integracao_caderno_cci_indicador->set_descricao('evolucao_inpc');
	...
	function criar_indicador()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
			...
			$sql.=indicador_db::sql_inserir_grafico(
				...
				"2,2,1,$linha_sem_media-linha;3,3,1,$linha_sem_media",
				usuario_id(),
				$coluna_para_ocultar,
				1,
				2
	...
	function fechar_periodo()
	{
		if(indicador_db::verificar_permissao(usuario_id(),'GIN'))
		{
			...
			if(intval($contador_ano_atual) > 0)
			{
				...
				$this->investimento_evolucao_inpc_model->atualiza_fechar_periodo($result, $args);
			}

			$this->investimento_evolucao_inpc_model->fechar_periodo($result, $args);
		}
		else
		{
			exibir_mensagem("ACESSO NÃO PERMITIDO");
