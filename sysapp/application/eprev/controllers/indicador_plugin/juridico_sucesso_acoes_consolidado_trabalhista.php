<?php
class Juridico_sucesso_acoes_consolidado_trabalhista extends Controller
{
	...
	function __construct()
    {
		...
		$this->enum_indicador = intval(enum_indicador::JURIDICO_SUCESSO_DAS_ACOES_CONSOLIDADO_TRABALHISTA);
		...
		$this->load->model('indicador_plugin/juridico_sucesso_acoes_consolidado_trabalhista_model');   		
    }
    ...
    function index()
    {
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
	        ...
	        $this->load->view('indicador_plugin/juridico_sucesso_acoes_consolidado_trabalhista/index',$data);
		}		
    }
    ...
    function listar()
    {
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
        {
			...
			$this->juridico_sucesso_acoes_consolidado_trabalhista_model->listar( $result, $args );
			$data['collection'] = $result->result_array();
			
			$this->load->view('indicador_plugin/juridico_sucesso_acoes_consolidado_trabalhista/partial_result', $data);
        }    	
    }

	function detalhe($cd_juridico_sucesso_acoes_consolidado_trabalhista = 0)
	{
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
			$args   = array();
			...
			$args['cd_juridico_sucesso_acoes_consolidado_trabalhista'] = $cd_juridico_sucesso_acoes_consolidado_trabalhista;
			
			if(intval($args['cd_juridico_sucesso_acoes_consolidado_trabalhista']) == 0)
			{
				$data['row']['cd_juridico_sucesso_acoes_consolidado_trabalhista'] = $args['cd_juridico_sucesso_acoes_consolidado_trabalhista'];
				$data['row']['cd_etapa']       = "";
				$data['row']['nr_inicial']     = "";
			...
			{
				$this->juridico_sucesso_acoes_consolidado_trabalhista_model->carrega($result, $args);
				$data['row'] = $result->row_array();
			}
			
			$args['cd_indicador_tabela'] = $tabela[0]['cd_indicador_tabela'];
			$this->juridico_sucesso_acoes_consolidado_trabalhista_model->etapa($result, $args);
			$data['ar_fase'] = $result->result_array();			
			
			$this->load->view('indicador_plugin/juridico_sucesso_acoes_consolidado_trabalhista/detalhe', $data);
		}        
	}	
	...
	function salvar()
	{
		...
		{		
			...
			$args['cd_juridico_sucesso_acoes_consolidado_trabalhista'] = intval($this->input->post('cd_juridico_sucesso_acoes_consolidado_trabalhista', true));
			...
			$this->juridico_sucesso_acoes_consolidado_trabalhista_model->salvar($result, $args);
			$this->criar_indicador();
			
			redirect("indicador_plugin/juridico_sucesso_acoes_consolidado_trabalhista", "refresh");
		}
        else
        {
	...
	public function get_valores()
	{
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
			...
			$row = $this->juridico_sucesso_acoes_consolidado_trabalhista_model->get_valores(
				$this->input->post('nr_ano', true),
				$this->input->post('cd_etapa', true)
			);
	...
	function excluir($cd_juridico_sucesso_acoes_consolidado_trabalhista)
	{
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
			$args   = array();
			...
			$this->juridico_sucesso_acoes_consolidado_trabalhista_model->excluir($result, $args);
			$this->criar_indicador();
	...
	function fechar_periodo()
	{
		if((indicador_db::verificar_permissao(usuario_id(), 'AJ')) OR (indicador_db::verificar_permissao(usuario_id(), 'GP')))
		{
			...
			$this->juridico_sucesso_acoes_consolidado_trabalhista_model->fechar_periodo($result, $args);
		}
		redirect("indicador_plugin/juridico_sucesso_acoes_consolidado_trabalhista", "refresh");
	}	
}
