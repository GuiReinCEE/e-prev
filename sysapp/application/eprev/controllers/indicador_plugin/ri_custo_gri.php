<?php
class ri_custo_gri extends Controller
{
	var	$label_0 = "Ano";
	var	$label_1 = "Custo Administrativo Fundação";
	var	$label_2 = "Total Custo GRI";
	var	$label_3 = "% de custo GRI";
    var	$label_4 = "Meta";
    ...
    {
        parent::Controller();
		$this->enum_indicador = intval(enum_indicador::RI_CUSTO_GRI);
    }
    ...
    function index()
    {
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
	        ...
	        $this->load->view('indicador_plugin/ri_custo_gri/index.php',$data);
		}
    }
    ...
    function listar()
    {
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
        {
	        $this->load->model( 'indicador_plugin/ri_custo_gri_model' );
			...
			if( sizeof($tabela)>0 )
			{
				...
				$this->ri_custo_gri_model->listar( $result, $args );

				$data['collection'] = $result->result_array();

				$this->load->view('indicador_plugin/ri_custo_gri/partial_result', $data);
			}
			else
			{
	...
	function detalhe($cd=0)
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$this->load->model('indicador_plugin/ri_custo_gri_model');
			$row=$this->ri_custo_gri_model->carregar( $cd );
			if($row)
			{
				if($cd==0)
				{
					...
					FROM indicador_plugin.ri_custo_gri
			...
			$this->load->view('indicador_plugin/ri_custo_gri/detalhe', $data);
		}
	}
	...
	function salvar()
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$args['cd_ri_custo_gri']=intval($this->input->post('cd_ri_custo_gri', true));
			...
			$msg=array();
			$retorno = $this->ri_custo_gri_model->salvar( $args,$msg );
			...
			if($retorno)
			{
				...
				{
					redirect( "indicador_plugin/ri_custo_gri", "refresh" );
					#redirect( "indicador_plugin/ri_custo_gri/detalhe/0", "refresh" );
				}				
			}
			else
			{
	...
	function excluir($id)
	{
		...
		if(CheckLogin() && (indicador_db::verificar_permissao( usuario_id(), 'AC' ) OR indicador_db::verificar_permissao( usuario_id(), 'GE' )) )
		{
			...
			$this->ri_custo_gri_model->excluir($id);
